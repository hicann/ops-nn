...
extern "C" __global__ __aicore__ void mat_mul_v2_compress_dequant(GM_ADDR x1, GM_ADDR x2, GM_ADDR compress_index,
                                                                  GM_ADDR deq_scale, GM_ADDR bias, GM_ADDR offset_w,
                                                                  GM_ADDR out, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    gm_to_ub<ArchType::ASCEND_V200, int32_t>(ub_tiling, gm_tiling, 0, 1, 512 / 32, 0, 0);
    SET_FLAG(MTE2, S, EVENT_ID0);
    WAIT_FLAG(MTE2, S, EVENT_ID0);

    kernel.Init(reinterpret_cast<__gm__ uint8_t*>(x1), reinterpret_cast<__gm__ uint8_t*>(x2),
                reinterpret_cast<__gm__ uint8_t*>(bias), reinterpret_cast<__gm__ uint8_t*>(deq_scale),
                reinterpret_cast<__gm__ uint8_t*>(compress_index), reinterpret_cast<__gm__ uint8_t*>(out),
