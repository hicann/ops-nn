...
static ge::graphStatus InferShapeForGemmV2(InferShapeContext* context)
{
    ...
    OP_LOGD(context->GetNodeName(), "a_shape: %s, b_shape: %s, c_shape:%s, transpose_a: %d, transpose_b: %d",
            Ops::Base::ToString(*shape_a).c_str(), Ops::Base::ToString(*shape_b).c_str(),
            Ops::Base::ToString(*shape_c).c_str(), *trans_a, *trans_b);
...
IMPL_OP_INFERSHAPE(GemmV2).InferShape(InferShapeForGemmV2).InferDataType(InferDataTypeForGemmV2);
}
