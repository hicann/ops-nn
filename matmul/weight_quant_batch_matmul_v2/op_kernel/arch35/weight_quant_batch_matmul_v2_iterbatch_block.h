...
/*!
 * \file weight_quant_batch_matmul_v2_iterbatch_block.h
 * \brief
 */
...
__aicore__ inline void WeightQuantBmmIterBatchBlock::Init(
...
__aicore__ inline void WeightQuantBmmIterBatchBlock::UpdateBatchInfo()
{
    if (tilingData_->params.batchA == 1 || tilingData_->params.batchB == 1 ||
        ...
        params_.broadcastFlag = NO_NEED_BROADCAST;
...
__aicore__ inline void WeightQuantBmmIterBatchBlock::GetBatchInfo(uint64_t batchNum)
{
    if (params_.innerBatchNum == 0) {
        params_.batchANum = Min(batchNum, static_cast<uint64_t>(tilingData_->params.batchA));
        params_.batchBNum = Min(batchNum, static_cast<uint64_t>(tilingData_->params.batchB));
        params_.batchAIndex = Min(params_.batchIndex, static_cast<uint64_t>(tilingData_->params.batchA - 1));
        params_.batchBIndex = Min(params_.batchIndex, static_cast<uint64_t>(tilingData_->params.batchB - 1));
        ...
        params_.batchANum = Min(params_.innerBatchNum, batchNum);
        ...
        params_.batchBNum = Min(params_.innerBatchNum, batchNum);
...
__aicore__ inline void WeightQuantBmmIterBatchBlock::GetMultiBatchInfo(uint64_t loopIndex)
{
    // main loop
    if (loopIndex + 1 < params_.loopTimes) {
        params_.batchIndex = loopIndex * params_.mainLoopPerCoreBatchNum * params_.useCoreNum +
                             blockIdx_ * params_.mainLoopPerCoreBatchNum;
        GetBatchInfo(params_.mainLoopPerCoreBatchNum);
    } else if (blockIdx_ < params_.lastLoopBlockNum) { // last loop
        params_.batchIndex = loopIndex * params_.mainLoopPerCoreBatchNum * params_.useCoreNum +
                             blockIdx_ * (params_.lastLoopPreCoreBatchNum + 1);
        GetBatchInfo(params_.lastLoopPreCoreBatchNum + 1);
    } else {
        params_.batchIndex = loopIndex * params_.mainLoopPerCoreBatchNum * params_.useCoreNum +
                             params_.lastLoopBlockNum * (params_.lastLoopPreCoreBatchNum + 1) +
                             (blockIdx_ - params_.lastLoopBlockNum) * params_.lastLoopPreCoreBatchNum;

        GetBatchInfo(params_.lastLoopPreCoreBatchNum);
    }
}

__aicore__ inline void WeightQuantBmmIterBatchBlock::GetBroadcastBatchOffset(uint64_t b1Index, uint64_t b2Index,
                                                                             uint64_t b3Index)
{
...
__aicore__ inline void WeightQuantBmmIterBatchBlock::CalcGMOffset()
{
