...
void GetXWeightInputShape(WeightQuantBatchMatmulInfo& matmulInfo, const gert::StorageShape* xShape,
...
ge::graphStatus WeightQuantBatchMatmulV2Tiling::GetShapeAttrsInfo()
{
    ...
    OP_LOGD(opName_,
            ...
            ge::TypeUtils::FormatToAscendString(matmulInfoPtr_->bFormat).GetString());
...
void WeightQuantBatchMatmulV2Tiling::SetCommonTilingKeyElement(TilingKeyConfigure& tilingKeyConfigure) const
{
    ...
    tilingKeyConfigure.transposeSituation = (static_cast<uint16_t>(matmulInfoPtr_->transA) << 1) |
                                            static_cast<uint16_t>(matmulInfoPtr_->transB);
    ...
    tilingKeyConfigure.weightFormat = static_cast<uint8_t>(
        matmulInfoPtr_->bFormat == ge::FORMAT_ND ? WeightFormat::ND : WeightFormat::FRACTAL_NZ);
...
ge::graphStatus WeightQuantBatchMatmulV2Tiling::GetPlatformInfo()
{
    ...
    OP_TILING_CHECK(compileInfoPtr->aivNum == 0 || compileInfoPtr->aicNum == 0,
                    VECTOR_INNER_ERR_REPORT_TILIING(opName_, "aivNum and aicNum >= 0 but they are %u and %u",
                     ...
                     const gert::StorageShape* weightShape)
{
    ...
    if (inputParams->bDtype == ge::DT_INT4 || inputParams->bDtype == ge::DT_INT32 ||
        ...
        OP_TILING_CHECK((innerAxisDim & 1) != 0,
                        VECTOR_INNER_ERR_REPORT_TILIING(
                            inputParams->opName,
                            "Inner dim of weight must be even when dtype is int4/float4, but is [%lu]", innerAxisDim),
                        return false);
    }
    return true;
}
    ...
    if (antiQuantScaleShapeSize != 1) {
        if (antiQuantScaleDimNum == MM_SHAPE_LEN_ND) {
            ...
            OP_TILING_CHECK(
                ...
                VECTOR_INNER_ERR_REPORT_TILIING(inputParams->opName, "Antiquant shape expect %s, but is %s",
...
bool CheckAntiQuantOffsetShape(WeightQuantBatchMatmulInfo* inputParams, const gert::StorageShape* antiQuantScaleShape,
                               const gert::StorageShape* antiQuantOffsetShape)
{
    ...
    if (inputParams->hasAntiQuantOffset) {
        OP_TILING_CHECK(
            inputParams->bDtype == ge::DT_FLOAT4_E2M1 || inputParams->bDtype == ge::DT_FLOAT8_E4M3FN ||
                inputParams->bDtype == ge::DT_HIFLOAT8 || inputParams->bDtype == ge::DT_FLOAT,
            VECTOR_INNER_ERR_REPORT_TILIING(
                inputParams->opName, "AntiQuantOffset is not supported with FP32/FP4_E2M1/FP8_E4M3/HIFLOAT8 weights."),
            return false);
    }
    if (inputParams->hasAntiQuantOffset) {
...
bool CheckAntiQuantShape(WeightQuantBatchMatmulInfo* inputParams, const gert::StorageShape* antiQuantScaleShape,
                         const gert::StorageShape* antiQuantOffsetShape)
{
    OP_TILING_CHECK(!CheckAntiQuantScaleShape(inputParams, antiQuantScaleShape),
                    VECTOR_INNER_ERR_REPORT_TILIING(inputParams->opName, "Check antiquantsacle shape failed"),
                    return false);
    OP_TILING_CHECK(!CheckAntiQuantOffsetShape(inputParams, antiQuantScaleShape, antiQuantOffsetShape),
                    VECTOR_INNER_ERR_REPORT_TILIING(inputParams->opName, "Check antiquantoffset shape failed"),
                    return false);
    return true;
}
...
*/
bool CheckShape(gert::TilingContext* context, WeightQuantBatchMatmulInfo* inputParams, NpuArch npuArch)
{
    size_t idx = 0;
    auto xShape = context->GetInputShape(idx++);
    auto weightShape = context->GetInputShape(idx++);
    ...
    OP_TILING_CHECK(!CheckShapeDims(inputParams, npuArch),
                    VECTOR_INNER_ERR_REPORT_TILIING(inputParams->opName, "Check shape dims failed"), return false);
    return true;
}

bool CheckBiasDtype(gert::TilingContext* context, WeightQuantBatchMatmulInfo* inputParams,
                    platform_ascendc::SocVersion socVersion)
{
    ...
    if (inputParams->aDtype == ge::DT_BF16) {
        ...
        OP_TILING_CHECK(biasDtype != inputParams->aDtype,
                        VECTOR_INNER_ERR_REPORT_TILIING(inputParams->opName,
                                                        "Bias dtype must be fp16 when x dtype is fp16, but is [%s]",
                                                        ge::TypeUtils::DataTypeToAscendString(biasDtype).GetString()),
                        return false);
    }
                     ...
                     platform_ascendc::SocVersion socVersion)
{
    OP_TILING_CHECK(
        ...
        VECTOR_INNER_ERR_REPORT_TILIING(inputParams->opName, "Input x dtype must be fp16 or bf16, but is [%s]",
                                        ge::TypeUtils::DataTypeToAscendString(inputParams->aDtype).GetString()),
        return false);
    OP_TILING_CHECK(
                         ...
                         platform_ascendc::SocVersion socVersion)
{
    ...
    if (inputParams->antiQuantScaleDtype == ge::DT_UINT64 || inputParams->antiQuantScaleDtype == ge::DT_INT64) {
        ...
        OP_TILING_CHECK(inputParams->hasAntiQuantOffset,
                        VECTOR_INNER_ERR_REPORT_TILIING(
                            inputParams->opName,
...
bool CheckQuantDtype(gert::TilingContext* context, WeightQuantBatchMatmulInfo* inputParams)
{
    ...
    if (quantScaleDesc != nullptr) {
        ...
        OP_TILING_CHECK(
            ...
            VECTOR_INNER_ERR_REPORT_TILIING(inputParams->opName, "Quant scale dtype should be uint64, but is [%s]",
...
bool CheckTempLimit(WeightQuantBatchMatmulInfo* inputParams, const gert::TilingContext* context)
{
    ...
    OP_TILING_CHECK(!CheckCoreNum(inputParams, context), OP_LOGE(inputParams->opName, "Check CoreNum fail."),
                    return false);

    // not support transposeA and int8 out
    OP_TILING_CHECK(
        inputParams->transA,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(inputParams->opName, "transpose_x",
                                              std::to_string(inputParams->transA).c_str(), "transpose_x must be false"),
        return false);
    OP_TILING_CHECK(
        inputParams->cDtype == ge::DT_INT8,
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(inputParams->opName, "y", "INT8", "The dtype of y can not be INT8"),
        return false);
    ...
    if (std::find(BIT8_WEIGHT_DTYPE_LIST.begin(), BIT8_WEIGHT_DTYPE_LIST.end(), inputParams->bDtype) !=
        ...
        OP_TILING_CHECK(!(inputParams->antiQuantType == QuantType::PER_CHANNEL ||
...
ge::graphStatus CheckPara(gert::TilingContext* context, platform_ascendc::SocVersion socVersion)
{
    ...
    OP_TILING_CHECK(!CheckAttr(context, &inputParams),
                    VECTOR_INNER_ERR_REPORT_TILIING(inputParams.opName, "Check attr failed"), return ge::GRAPH_FAILED);
    OP_TILING_CHECK(!CheckShape(context, &inputParams, npuArch),
    ...
    if (inputParams.bDtype == ge::DT_FLOAT8_E4M3FN || inputParams.bDtype == ge::DT_HIFLOAT8) {
        OP_TILING_CHECK(
    ...
    if (npuArch == NpuArch::DAV_3510) {
        OP_TILING_CHECK(!CheckTempLimit(&inputParams, context),
