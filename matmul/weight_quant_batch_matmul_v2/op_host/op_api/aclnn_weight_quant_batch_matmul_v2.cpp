...
using TupleTensor = std::tuple<const aclTensor*&, const aclTensor*&, const aclTensor*&, const aclTensor*&,
...
static const std::initializer_list<DataType> DAV3510_WEIGHT_DTYPE_SUPPORT_LIST = {
    DataType::DT_INT8, DataType::DT_INT4, DataType::DT_FLOAT8_E4M3FN, DataType::DT_HIFLOAT8, DataType::DT_FLOAT4_E2M1};
...
static const std::initializer_list<DataType> ASCEND910B_Y_DTYPE_SUPPORT_LIST = {DataType::DT_FLOAT16, DataType::DT_BF16,
                                                                                DataType::DT_INT8};
static const std::initializer_list<DataType> DAV3510_Y_DTYPE_SUPPORT_LIST = {DataType::DT_FLOAT16, DataType::DT_BF16};
...
static bool CheckShapeValid(const aclTensor* x, const aclTensor* weight, const aclTensor* out)
{
    ...
    OP_CHECK(BroadcastInferShape(xBroadcastShape, weightBroadcastShape, broadcastShape),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Broadcast %s and %s failed.",
                     op::ToString(x->GetViewShape()).GetString(), op::ToString(weight->GetViewShape()).GetString()),
             return false);
    OP_CHECK(broadcastShape.GetDimNum() == outDimNum,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "%s and %s can't broadcast to %s.",
                     op::ToString(x->GetViewShape()).GetString(), op::ToString(weight->GetViewShape()).GetString(),
...
static bool CheckAntiquantGroupSize(const aclTensor* x, int antiquantGroupSize)
{
    ...
    OP_CHECK(static_cast<int64_t>(antiquantGroupSize) + 1 <= kX,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID,
...
static bool CheckMicroScalingCondition(const aclTensor* weight, const aclTensor* antiquantScale,
                                       const aclTensor* antiquantOffsetOptional, int antiquantGroupSize)
{
    ...
    if (microScalingFlag) {
        OP_CHECK(antiquantGroupSize == ANTIQUANT_GROUP_SIZE_MIN_VALUE,
                 ...
                 return false);
        OP_CHECK(antiquantOffsetOptional == nullptr,
                 OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                                     ...
                                     const aclTensor* antiquantOffsetOptional, int antiquantGroupSize)
{
    ...
    if (antiquantScale->GetViewShape().GetDimNum() == ANTIQUANT_DIM_MAX_VALUE) {
        ...
        if (antiquantGroupSize > 0) {
            int64_t kAntiquantScaleValue = (kWeight + static_cast<int64_t>(antiquantGroupSize - 1)) /
                                           static_cast<int64_t>(antiquantGroupSize);
...
static bool CheckBiasShape(const aclTensor* biasOptional, int64_t nWeight)
{
    ...
    if (biasOptional->GetViewShape().GetDimNum() == BIAS_DIM_MAX_VALUE &&
        ...
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "when bias's shape size is 2, it's shape should be [%d, %ld], actual is %s.",
                1, nWeight, op::ToString(biasOptional->GetViewShape()).GetString());
        return false;
    }
    return true;
}
...
static bool CheckXWeightDimValue(const aclTensor* weight, int64_t kX, int64_t mX, bool transposeX, bool transposeWeight)
{
    ...
    if (weight->GetDataType() == DataType::DT_INT8) {
        if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510 &&
            ...
            OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(kOpName, "k, n",
...
static bool CheckXWeightY(const aclTensor* x, const aclTensor* weight, const aclTensor* y, int antiquantGroupSize)
{
    ...
    if (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND310P && antiquantGroupSize > 0) {
        OP_CHECK(!CheckMultiplyOverflow(mX, kX),
                 OP_LOGE(ACLNN_ERR_PARAM_INVALID, "m * k (%ld * %ld) is overflow of int64.", mX, kX), return false);
        if (mX * kX > MAX_MK_VALUE) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "m*k shouldn't be larger than %ld, actual m is %ld, k is %ld.",
                    MAX_MK_VALUE, mX, kX);
            return false;
        }
    }
    ...
    if (antiquantGroupSize > 0) {
        ...
        if (transposeX || transposeWeight) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                    ...
                    (transposeX ? "transposed" : "not transposed"),
                    (transposeWeight ? "transposed" : "not transposed"));
            return false;
        }
        ...
        if (antiquantScale->GetViewShape().GetShapeSize() == 1) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "when weight's dtype is [int4], weight's format is [FRACTAL_NZ], "
...
static bool CheckValForWeightInt8Nz(const aclTensor* x, const aclTensor* weight, int antiquantGroupSize,
...
static bool AdvancedWeightParamsCheck(const aclTensor* x, const aclTensor* weight, int antiquantGroupSize,
                                      bool transposeWeight, const aclTensor* y)
{
    return CheckXWeight(x, weight, transposeWeight) && CheckXWeightY(x, weight, y, antiquantGroupSize) &&
           CheckShapeValid(x, weight, y);
}

static bool AdvancedShapeParamsCheck(const aclTensor* weight, const aclTensor* quantScaleOptional,
...
static bool AdvancedAntiquantParamsCheck(const aclTensor* x, const aclTensor* weight, const aclTensor* antiquantScale,
                                         const aclTensor* antiquantOffsetOptional, int antiquantGroupSize)
{
...
static bool AdvancedQuantParamsNullCheckForWeightInt4Nz(const aclTensor* weight, const aclTensor* antiquantScale,
                                                        ...
                                                        const aclTensor* quantOffsetOptional, bool transposeWeight)
{
    ...
    OP_CHECK(!(weight->GetDataType() == DataType::DT_FLOAT4_E2M1 &&
               antiquantScale->GetDataType() == DataType::DT_FLOAT8_E8M0 &&
               weight->GetStorageFormat() == Format::FORMAT_FRACTAL_NZ && transposeWeight),
             OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
...
static bool CheckValForWeightFp4(const aclTensor* weight, const aclTensor* antiquantScale, int antiquantGroupSize)
{
...
static bool CheckValForWeightFp8Hif8(const aclTensor* weight, const aclTensor* antiquantScale, int antiquantGroupSize)
{
    auto weightDtype = weight->GetDataType();
...
static aclnnStatus CheckNzSocValid()
{
    ...
    switch (npuArch) {
        case NpuArch::DAV_3510:
            break;
        default: {
            OP_LOGE(ACLNN_ERR_RUNTIME_ERROR, "aclnn nz support for %s is not implemented",
...
static bool CheckAntiquantForFixpipe(const aclTensor* antiquantScale, const aclTensor* antiquantOffsetOptional)
{
    if (antiquantOffsetOptional->GetDataType() == DataType::DT_INT32) {
        if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510) {
            OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(kOpName, "antiquantOffsetOptional", "DT_INT32",
...
static bool CheckXDtypeValidForNormal(const aclTensor* x, const aclTensor* weight, const aclTensor* antiquantScale,
                                      const aclTensor* antiquantOffsetOptional)
{
    ...
    if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510 && antiquantOffsetOptional != nullptr) {
        bool unsupportOffsetFlag = weight->GetDataType() == DataType::DT_FLOAT4_E2M1 ||
                                   weight->GetDataType() == DataType::DT_FLOAT8_E4M3FN ||
    ...
    if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510) {
        if (weight->GetDataType() == DataType::DT_FLOAT4_E2M1 || weight->GetDataType() == DataType::DT_FLOAT8_E4M3FN ||
            ...
            if (biasOptional->GetDataType() != DataType::DT_BF16) {
                OP_LOGE_FOR_INVALID_DTYPE(kOpName, "biasOptional",
                                          op::ToString(biasOptional->GetDataType()).GetString(), "BF16");
                return false;
            }
            ...
            if (biasOptional->GetDataType() != DataType::DT_FLOAT) {
                OP_LOGE_FOR_INVALID_DTYPE(kOpName, "biasOptional",
                                          op::ToString(biasOptional->GetDataType()).GetString(), "FLOAT");
                return false;
            }
        } else {
            if (biasOptional->GetDataType() != DataType::DT_BF16 && biasOptional->GetDataType() != DataType::DT_FLOAT) {
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(kOpName, "biasOptional",
        ...
        if (biasOptional->GetDataType() != DataType::DT_FLOAT) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "biasOptional's dtype should be [DT_FLOAT], actual is [%s].",
                    op::ToString(biasOptional->GetDataType()).GetString());
            return false;
        }
    }
...
static bool CheckScaleDtypeValid(const aclTensor* x, const aclTensor* weight, const aclTensor* antiquantScale,
...
static bool CheckDtypeValid(const aclTensor* x, const aclTensor* weight, const aclTensor* antiquantScale,
                            const aclTensor* quantScaleOptional, const aclTensor* y)
{
...
static bool CheckOptionalFormatValid(const aclTensor* antiquantOffsetOptional, const aclTensor* quantScaleOptional,
...
static bool CheckDimRangeOptionalInputValid(const aclTensor* antiquantScale, const aclTensor* antiquantOffsetOptional,
                                                          ...
                                                          bool transposeWeight)
{
    if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510) {
        if (antiquantScale->GetViewShape().GetDimNum() == ANTIQUANT_DIM_MAX_VALUE &&
            ...
            OP_CHECK((transposeWeight && IsTransLastTwoDims(antiquantScale)) ||
                                   ...
                                   const aclTensor* antiquantOffsetOptional, const aclTensor* y)
{
    ...
    if (weight->GetStorageFormat() == Format::FORMAT_FRACTAL_NZ &&
        ...
        aclnnStatus antiQuantRes = AntiQuantScaleAndOffsetContiguousCheck(antiquantScale, antiquantOffsetOptional,
                                                                          transposeWeight);
...
static aclnnStatus OptionalContiguousCheck(const aclTensor* quantScaleOptional, const aclTensor* quantOffsetOptional,
                                           const aclTensor* biasOptional)
{
...
static aclnnStatus BasicParamsScaleCheck(const aclTensor* x, const aclTensor* weight, const aclTensor* antiquantScale,
                                         const aclTensor* antiquantOffsetOptional, const aclTensor* y)
{
...
static aclnnStatus BasicParamsBiasCheck(const aclTensor* x, const aclTensor* weight, const aclTensor* antiquantScale,
                                        const aclTensor* biasOptional)
{
...
static aclnnStatus BasicOptionalParamsCheck(const aclTensor* antiquantScale, const aclTensor* antiquantOffsetOptional,
                                            const aclTensor* quantScaleOptional, const aclTensor* quantOffsetOptional)
{
    CHECK_RET(CheckOptionalFormatValid(antiquantOffsetOptional, quantScaleOptional, quantOffsetOptional),
              ACLNN_ERR_PARAM_INVALID);

    CHECK_RET(CheckDimRangeOptionalInputValid(antiquantScale, antiquantOffsetOptional, quantScaleOptional,
                                              quantOffsetOptional),
              ACLNN_ERR_PARAM_INVALID);
...
static aclnnStatus TransposeAndTransDataForInputsGroup(const aclTensor*& weight, const aclTensor*& antiquantScale,
                                                       const aclTensor*& antiquantOffsetOptional, bool& transposeWeight,
                                                       aclOpExecutor* executor)
{
...
static bool SetShapeStrideForNZ(const aclTensor* weight, aclTensor* weightTemp, bool transposeWeight)
{
    ...
    auto newOriginalShape = transposeWeight ? SwapLastTwoDimValue(weightTemp->GetViewShape()) :
                                              weightTemp->GetViewShape();
...
static aclnnStatus PackedWeightPreProcess(const aclTensor* weight, const aclTensor*& tensorWeight,
                                          aclOpExecutor* executor)
{
    ...
    if (weight->GetDataType() == DataType::DT_INT32) {
        CHECK_RET(ModifyTensorDtype(tensorWeight, weightTemp, DataType::DT_INT4, executor) == ACLNN_SUCCESS,
                  ACLNN_ERR_PARAM_INVALID);
        OP_LOGD("The conversion of weight from int32 to int4 is completed.");
    }
    if (weight->GetDataType() == DataType::DT_FLOAT) {
        CHECK_RET(ModifyTensorDtype(tensorWeight, weightTemp, DataType::DT_FLOAT4_E2M1, executor) == ACLNN_SUCCESS,
                  ACLNN_ERR_PARAM_INVALID);
...
static aclnnStatus TensorPreProcess(TupleTensor mandatoryTensors, TupleTensor optionalTensors, aclOpExecutor* executor)
{
    ...
    if (weight->GetDataType() == DataType::DT_INT32 ||
        ...
        weightTemp->SetViewStrides(weight->GetViewStrides());
        weightTemp->SetStorageShape(weight->GetStorageShape());
        auto newOriginalShape = IsTransposeLastTwoDims(weight) ? SwapLastTwoDimValue(weight->GetViewShape()) :
                                                                 weight->GetViewShape();
        weightTemp->SetOriginalShape(newOriginalShape);
        tensorWeight = weightTemp;
...
aclnnStatus CheckContiguous(const aclTensor*& x, const aclTensor*& weight, const aclTensor*& antiquantScale,
                            ...
                            const aclTensor*& quantOffsetOptional, const aclTensor*& biasOptional,
                            const int& antiquantGroupSize, bool& transposeX, bool& transposeWeight,
                            aclOpExecutor* executor)
{
    if (op::GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510) {
        OP_CHECK(!transposeX && IsContiguous(x),
    ...
    if (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND310P) {
        if (antiquantGroupSize > 0) {
            auto ret = TransposeAndTransDataForInputsGroup(weight, antiquantScale, antiquantOffsetOptional,
                                                           transposeWeight, executor);
            CHECK_RET(ret == ACLNN_SUCCESS, ret);
        } else {
            auto ret = TransposeAndTransDataForInputs(weight, antiquantScale, antiquantOffsetOptional, transposeWeight,
                                                      executor);
            CHECK_RET(ret == ACLNN_SUCCESS, ret);
        }
    }
...
static aclnnStatus CheckParams(TupleTensor mandatoryTensors, TupleTensor optionalTensor, int& antiquantGroupSize,
                               bool& transposeWeight)
{
    ...
    res = BasicOptionalParamsCheck(antiquantScale, antiquantOffsetOptional, tensorQuantScaleOptional,
                                   quantOffsetOptional);
    CHECK_RET(res == ACLNN_SUCCESS, res);
    ...
    advanceCheckRes = AdvancedAntiquantParamsCheck(x, tensorWeight, antiquantScale, antiquantOffsetOptional,
                                                   antiquantGroupSize);
    CHECK_RET(advanceCheckRes, ACLNN_ERR_PARAM_INVALID);

    advanceCheckRes = AdvancedShapeParamsCheck(tensorWeight, tensorQuantScaleOptional, quantOffsetOptional,
                                               biasOptional);
    CHECK_RET(advanceCheckRes, ACLNN_ERR_PARAM_INVALID);
...
aclnnStatus AclnnWeightQuantBatchMatmulGetWorkspaceSizeCommonProcess(TupleTensor mandatoryTensors,
                                                                     TupleTensor optionalTensors, TupleAttr attrs,
                                                                     aclOpExecutor* executor)
{
    auto& x = std::get<INDEX_X_IN_MANDTORY_TUPLE>(mandatoryTensors);
    auto& weight = std::get<INDEX_WEIGHT_IN_MANDTORY_TUPLE>(mandatoryTensors);
    ...
    res = CheckContiguous(x, tensorWeight, antiquantScaleRef, antiquantOffsetOptional, tensorQuantScaleOptional,
                          quantOffsetOptional, biasOptional, antiquantGroupSize, transposeX, transposeWeight, executor);
    CHECK_RET(res == ACLNN_SUCCESS, res);
    ...
    aclOpExecutor** executor)
{
    ...
    aclnnStatus res = AclnnWeightQuantBatchMatmulGetWorkspaceSizeCommonProcess(
        ...
        std::tie(antiquantOffsetOptional, quantScaleOptional, tensorQuantScaleOptional, quantOffsetOptional,
                 biasOptional, reservedRef),
    ...
    auto result = l0op::WeightQuantBatchMatmulV2(x, tensorWeight, antiquantScaleRef, antiquantOffsetOptional,
                                                 tensorQuantScaleOptional, quantOffsetOptional, biasOptional,
                                                 transposeX, transposeWeight, antiquantGroupSize, dtype, 0,
    ...
    aclOpExecutor** executor)
{
    ...
    // dtype=-1表示输出dtype和输入x dtype一致
    int64_t dtype = tensorQuantScaleOptional != nullptr ? static_cast<int64_t>(y->GetDataType()) : -1;
    auto l0Result = l0op::WeightQuantBatchMatmulV2(x, tensorWeight, antiquantScaleRef, antiquantOffsetOptional,
                                                   tensorQuantScaleOptional, quantOffsetOptional, biasOptional,
                                                   transposeX, transposeWeight, antiquantGroupSize, dtype, 0,
                                                   uniqueExecutor.get()); // 0:v2接口innerPrecise参数默认传0
...
aclnnStatus aclnnWeightQuantBatchMatmulNz(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                          aclrtStream stream)
{
