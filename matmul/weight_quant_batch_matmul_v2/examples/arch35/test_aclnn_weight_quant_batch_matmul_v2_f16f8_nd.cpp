...
int AclnnWeightQuantBatchMatmulV2Test(int32_t deviceId, aclrtStream& stream)
{
    ...
    std::vector<uint8_t> weightHostData(GetShapeSize(weightShape), 0b00111000); // fp8_e4m3fn的1.0
    std::vector<float> yHostData(GetShapeSize(yShape), 0);

    std::vector<int64_t> antiquantScaleShape = {16};
    void* antiquantScaleDeviceAddr = nullptr;
    aclTensor* antiquantScale = nullptr;
    std::vector<uint16_t> antiquantScaleHostData(GetShapeSize(antiquantScaleShape), 0b0011110000000000); // fp16的1.0
    ...
    // 创建other aclTensor
    ret = CreateAclTensor(weightHostData, weightShape, &weightDeviceAddr, aclDataType::ACL_FLOAT8_E4M3FN, &weight);
