...
__simd_callee__ inline void CastWeightF4ToF16(MicroAPI::RegTensor<scaleType>& weightF16Reg0,
                                              MicroAPI::RegTensor<scaleType>& weightF16Reg1,
                                              MicroAPI::RegTensor<wType>& weightInReg0,
                                              MicroAPI::RegTensor<wType>& weightInReg1, MicroAPI::MaskReg& maskRegALL)
{
...
__aicore__ inline void AntiquantW4Pergroup32NK(ParamsGroupSize32<xType, wType, scaleType>& p)
{
    ...
    for (uint16_t outerIdx = 0; outerIdx < p.outerExtend; ++outerIdx) {
        ...
        for (uint16_t innerIdx = 0; innerIdx < p.innerExtend; ++innerIdx) {
            MicroAPI::AddrReg addrRegScale = MicroAPI::CreateAddrReg<scaleType>(outerIdx, p.outerStrideScale, innerIdx,
                                                                                OFFSET_8);
            MicroAPI::AddrReg addrRegWeight = MicroAPI::CreateAddrReg<uint8_t>(outerIdx, p.outerStrideWeight, innerIdx,
                                                                               OFFSET_FOR_4BITS);
            ...
            MicroAPI::DataCopy<scaleType, MicroAPI::LoadDist::DIST_E2B_B16>(scaleLoad, p.scaleBaseAddr, addrRegScale);
            // Interleave后变为
            // scale0:
            ...
            MicroAPI::DataCopy<uint8_t, MicroAPI::LoadDist::DIST_UNPACK4_B8>(
                (MicroAPI::RegTensor<uint8_t>&)wLoad0, (__local_mem__ uint8_t*)p.weightInBaseAddr0, addrRegWeight);
            MicroAPI::DataCopy<uint8_t, MicroAPI::LoadDist::DIST_UNPACK4_B8>(
                (MicroAPI::RegTensor<uint8_t>&)wLoad1, (__local_mem__ uint8_t*)p.weightInBaseAddr1, addrRegWeight);
...
__aicore__ inline void AntiquantW4PergroupKN(ParamsGroupKN<xType, wType, scaleType>& param)
{
    ...
    for (uint16_t n1Idx = 0; n1Idx < param.n1LoopNum; n1Idx++) {
        for (uint16_t groupIdx = 0; groupIdx < param.groupNumUb; groupIdx++) {
            MicroAPI::AddrReg scaleAddrReg = MicroAPI::CreateAddrReg<scaleType>(n1Idx, param.scaleN1Stride, groupIdx,
                                                                                param.bubNLen);
            MicroAPI::DataCopy<scaleType, MicroAPI::LoadDist::DIST_BLK>(scaleRegCompute, param.scaleBaseAddr0,
                                                                        scaleAddrReg);
            MicroAPI::DataCopy<scaleType, MicroAPI::LoadDist::DIST_BLK>(scaleRegAssist, param.scaleBaseAddr1,
                                                                        scaleAddrReg);
            ...
            for (uint16_t vLIdx = 0; vLIdx < param.vLLoopNumInGroup; vLIdx++) {
                MicroAPI::AddrReg weightInAddrReg = MicroAPI::CreateAddrReg<uint8_t>(
                    n1Idx, param.weighInN1Stride, groupIdx, param.weightInGroupIdStride, vLIdx, OFFSET_FOR_4BITS);
                MicroAPI::AddrReg weightOutAddrReg = MicroAPI::CreateAddrReg<uint8_t>(
                    n1Idx, param.weightOutN1Stride, groupIdx, param.weightOutGroupIdStride, vLIdx,
                ...
                MicroAPI::DataCopy<xType, MicroAPI::StoreDist::DIST_PACK_B16>(param.weightOutBaseAddr, weightF8SelReg0,
