...
const std::map<int, int> V4_V3_INDEX_MAP{{V4_X1_INDEX, V3_X1_INDEX},         {V4_X2_INDEX, V3_X2_INDEX},
                                         {V4_BIAS_INDEX, V3_BIAS_INDEX},     {V4_X1SCALE_INDEX, V3_PERTOKENSCALE_INDEX},
                                         {V4_X2SCALE_INDEX, V3_SCALE_INDEX}, {V4_X2OFFSET_INDEX, V3_OFFSET_INDEX}};
} // namespace
...
std::vector<PatternUniqPtr> QuantBatchMatmulV4ToV3FusionPass::Patterns()
{
    ...
    for (int optionalInputNumber = 0; optionalInputNumber <= V4_IR_OPTIONAL_INPUT_NUMBER; optionalInputNumber++) {
        ...
        auto y = es::QuantBatchMatmulV4(
            quantBatchMatmulV4Inputs[V4_X1_INDEX], quantBatchMatmulV4Inputs[V4_X2_INDEX],
...
bool QuantBatchMatmulV4ToV3FusionPass::MeetRequirements(const std::unique_ptr<MatchResult>& match_result)
{
    ...
    if (x1ScaleResult == SUCCESS && x2ScaleResult == SUCCESS) {
        bool isA8W8GBDtype = (x1Dtype == DT_INT8 && x2Dtype == DT_INT8 && x1ScaleDesc.GetDataType() == DT_FLOAT &&
                              x2ScaleDesc.GetDataType() == DT_FLOAT && yDtype == DT_BF16);
        ...
        bool isA8W8GBDim = x1Shape.GetDimNum() == x1ScaleShape.GetDimNum() &&
                           x2Shape.GetDimNum() == x2ScaleShape.GetDimNum();
        if (isA8W8GBDtype && isA8W8GBDim) {
            OPS_LOG_D(QUANT_BATCH_MATMUL_V4_TO_V3_FUSION_PASS_NAME,
                      "In G-B quantification, there is no need for graph fusion");
            return false;
        }
    }
    return true;
...
GraphUniqPtr QuantBatchMatmulV4ToV3FusionPass::Replacement(const std::unique_ptr<MatchResult>& match_result)
{
    ...
    auto y = es::QuantBatchMatmulV3(v3Input[V3_X1_INDEX], v3Input[V3_X2_INDEX], v3Input[V3_SCALE_INDEX],
                                    v3Input[V3_OFFSET_INDEX], v3Input[V3_BIAS_INDEX], v3Input[V3_PERTOKENSCALE_INDEX],
                                    dtype, transpose_x1, transpose_x2, group_size);
