...
static const std::initializer_list<DataType> WEIGHT_DTYPE_SUPPORT_LIST = {DataType::DT_INT32, DataType::DT_FLOAT};
static const std::initializer_list<DataType> WEIGHTINT4PACK_DTYPE_SUPPORT_LIST = {
    DataType::DT_INT4, DataType::DT_INT32, DataType::DT_FLOAT4_E2M1, DataType::DT_FLOAT};
                                   ...
                                   int64_t weightInt4PackDimLast)
{
    ...
    if (weightInt4Pack->GetDataType() == DataType::DT_INT4) {
        int64_t lastDimSize = npuArch == NpuArch::DAV_3510 ? storageDim3 : INT4_NUM_IN_32B;
    ...
    if (weightInt4Pack->GetDataType() == DataType::DT_INT32) {
        int64_t lastDimSize = npuArch == NpuArch::DAV_3510 ? C0_16 / INT4_NUM_IN_INT32 : INT4_NUM_IN_INT32;
        int64_t expDim0 = CeilDiv(weightInt4PackDimLast, lastDimSize);
        if ((storageDim0 != expDim0) || (storageDim1 != expDim1) || (storageDim2 != CUBE_BLOCK_SIZE) ||
            (storageDim3 != lastDimSize)) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID,
...
static bool TransNdToNz(std::vector<int8_t>& weightArray, const Shape& weightShape, bool c032Flag)
{
    ...
    int64_t n = weightShape.GetDim(weightDimNum - 1) /
                2; // weightDimNum - 1为k轴索引，因为用int8_t表示int4，所以需要除以2
    int64_t g = 1;
    if (n == 0) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The inner axis of weightInt4PackData should not be 0.");
        return false;
    }
    ...
    for (size_t idxG = 0; idxG < static_cast<size_t>(g); idxG++) {
        for (size_t idx = 0; idx < static_cast<size_t>(k * n); idx++) {
            ...
            weightNzArray[idxN1 * k1 * CUBE_BLOCK_SIZE * c0SizeInt8 + idxK1 * c0SizeInt8 * CUBE_BLOCK_SIZE +
                          idxK0 * c0SizeInt8 + idxN0 + idxG * k * n] = weightArray[idx + idxG * k * n];
        }
    }
...
aclnnStatus aclnnConvertWeightToINT4PackGetWorkspaceSize(const aclTensor* weight, aclTensor* weightInt4Pack,
                                                         ...
                                                         [[maybe_unused]] aclOpExecutor** executor)
{
    ...
    // 若传入的weightInt4Pack的storageShape的最后一维为32，且为3510,则为s8s4分支
    bool c032Flag = weightInt4PackDimLast == C0_32 && GetCurrentPlatformInfo().GetCurNpuArch() == NpuArch::DAV_3510;
...
aclnnStatus aclnnConvertWeightToINT4Pack([[maybe_unused]] void* workspace, [[maybe_unused]] uint64_t workspaceSize,
                                         [[maybe_unused]] aclOpExecutor* executor, [[maybe_unused]] aclrtStream stream)
{
    return ACLNN_SUCCESS;
}
