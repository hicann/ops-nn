...
class FunctionaclnnFlatQuant(BaseApi):
    ...
    def init_by_input_data(self, input_data: InputDataset):
        if self.device == "pyaclnn":
            input_data.kwargs["clipRatio"] = ctypes.c_double(
                input_data.kwargs["clipRatio"]
            )
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        if self.output is None:
            ...
            if isinstance(self.output, int):
                out = input_data.args[self.output]
            elif isinstance(self.output, str):
                out = input_data.kwargs[self.output]
            else:
...
class FunctionaclnnFlatQuant_pgacl(AclnnBaseApi):
    def init_by_input_data(self, input_data: InputDataset):
        ...
        out = torch.zeros(
            x.shape[0], x.shape[1], x.shape[2] // 8, dtype=torch.int32
        ).npu()
