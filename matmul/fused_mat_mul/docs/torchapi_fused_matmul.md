...
- 计算公式：
  ...
  - "add"：$y = (x @ x2 + bias) + x3$。
  - "mul"：$y = (x @ x2 + bias) * x3$。
  - "gelu_erf"：$y = gelu\_erf(x @ x2 + bias)$。
  - "gelu_tanh"：$y = gelu\_tanh(x @ x2 + bias)$。
...
| bias | Tensor | 可选 | 矩阵乘的偏置。仅当fused_op_type为""、"16cast32"、"relu"、"add"、"mul"、"gelu_erf"或"gelu_tanh"时生效。 | torch.float16、torch.bfloat16、torch.float32 | 1-2维 |
