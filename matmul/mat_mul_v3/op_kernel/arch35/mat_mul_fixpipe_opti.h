...
__aicore__ inline void MatmulFixpipeOptiKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::Init(
    ...
    uint64_t roundIdx)
{
    if ASCEND_IS_AIV {
        ...
        uint32_t dstStride = static_cast<uint32_t>(
                                 (block_.matmulTilingData_->tCubeTiling.N - block_.params_.singleCoreN)) *
                             sizeof(C_T);
...
protected:
    ...
    MatmulImpl<A_TYPE, B_TYPE, C_TYPE_FIXPIPE_OPTI, BIAS_TYPE, MM_CFG, MatmulCallBackFunc<nullptr, nullptr, nullptr>,
               MatmulCommon::MMCustomMatmulPolicy>
        mm_;
    GlobalTensor<A_T> aGlobal_;
    GlobalTensor<B_T> bGlobal_;
    GlobalTensor<C_T> cGlobal_;
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void MatmulFixpipeOptiDualDstKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::Init(
    GM_ADDR aGM, GM_ADDR bGM, GM_ADDR cGM, GM_ADDR biasGM, GM_ADDR offsetWGM, GM_ADDR workspaceGM,
    const void* tilingData, TPipe* pipe)
{
    pipe_ = pipe;
...
MatmulFixpipeOptiDualDstKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::AivProcess(uint64_t roundIdx)
{
    if ASCEND_IS_AIV {
        LocalTensor<C_T> ubTensor(TPosition::LCM, 0, TOTAL_UB_SIZE / sizeof(C_T));
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void
MatmulFixpipeOptiDualDstKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::AicProcess(bool aicNeedWaitAiv)
{
    if ASCEND_IS_AIC {
        // fixpipe_l0c2ub
        mm_.SetSingleShape(block_.params_.singleCoreM, block_.params_.singleCoreN,
