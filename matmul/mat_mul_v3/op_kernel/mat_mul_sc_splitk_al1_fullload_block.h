...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE>
__aicore__ inline void MatmulSingleCoreSplitKAL1FullLoadBlock::Init(const MatmulTilingData* matmulTilingData)
{
    ...
    params_.isSingleKStrip = params_.kAL1FullLoadSize >= static_cast<uint64_t>(matmulTilingData_->matmulTiling.Ka);
    ...
    params_.nBaseTileCeilPerCore = MMV3DivCeil(params_.nCnt, matmulTilingData_->matmulTiling.usedCoreNum);
...
__aicore__ inline void MatmulSingleCoreSplitKAL1FullLoadBlock::InitBlockIndex()
{
    if (params_.coreIdx < params_.preCoreNum) {
        ...
        params_.nBaseTileGlobalIdx = params_.coreIdx * (params_.nBaseTileCeilPerCore - 1UL) + params_.preCoreNum;
...
__aicore__ inline void MatmulSingleCoreSplitKAL1FullLoadBlock::CalcGMOffset(uint64_t kIndex)
{
    ...
    if constexpr (B_TYPE::format == CubeFormat::ND) {
        if (params_.isTransposeB) {
            offset_.offsetB = params_.currentKBlockStart + nColOffset * matmulTilingData_->matmulTiling.Kb;
        } else {
            offset_.offsetB = params_.currentKBlockStart * matmulTilingData_->matmulTiling.N + nColOffset;
        }
    } else if constexpr (B_TYPE::format == CubeFormat::NZ) {
        if (params_.isTransposeB) {
            offset_.offsetB = params_.currentKBlockStart * params_.alignedOriN + nColOffset * params_.c0Size;
        } else {
            offset_.offsetB = params_.currentKBlockStart * params_.c0Size + nColOffset * params_.alignedKbSize;
        }
    }
