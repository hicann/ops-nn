...
template <class T>
__aicore__ inline void DataCopy2L1(const LocalTensor<int8_t>& matrix, const __gm__ void* gm, uint64_t l1Size,
                                   uint64_t calCount)
{
...
__aicore__ inline void DataCopyL1FullLoad(bool isAl1FullLoad, const LocalTensor<int8_t>& matrix, const __gm__ void* gm,
                                          uint64_t useM, uint64_t useK, uint64_t useN,
                                          const MatmulTilingData& tilingData)
{
    ...
    if constexpr (B_FORMAT == CubeFormat::NZ) {
        ...
        dataCopyParams.blockLen = MMV3CeilAlign(dDim, c0) * MMV3CeilAlign(nDim, static_cast<uint64_t>(BLOCK_SIZE)) *
                                  ctx.inputDtypeSize / BLOCK_BYTE_SIZE;
...
__aicore__ inline void CopyAL1(const LocalTensor<int8_t>& aMatrix, const __gm__ void* gm, int row, int col, int useM,
...
template <CubeFormat B_FORMAT = CubeFormat::ND>
__aicore__ inline void CopyBL1(const LocalTensor<int8_t>& bMatrix, const __gm__ void* gm, int row, int col, int useK,
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void MatmulBaseKernelAL1FullLoad<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::Process(
    uint64_t index, uint8_t enAtomic)
{
    ...
    uint64_t mAligned = A_TYPE::isTrans ?
                            MMV3CeilAlign(static_cast<uint64_t>(block_.matmulTilingData_->matmulTiling.singleCoreM),
                                          innerAlignedBlock) :
                            MMV3CeilAlign(static_cast<uint64_t>(block_.matmulTilingData_->matmulTiling.singleCoreM),
                                          BLOCK_SIZE);
    uint64_t kaAligned = A_TYPE::isTrans ?
                             MMV3CeilAlign(static_cast<uint64_t>(block_.matmulTilingData_->matmulTiling.Ka),
                                           BLOCK_SIZE) :
                             MMV3CeilAlign(static_cast<uint64_t>(block_.matmulTilingData_->matmulTiling.Ka),
                                           innerAlignedBlock);
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void MatmulBaseKernelAL1FullLoad<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::CopyInA1(
    const MatmulTilingData& matmulTilingData, LocalTensor<A_T> al1Local)
{
    Nd2NzParams nd2nzParams;
    nd2nzParams.ndNum = 1;
...
MatmulBaseKernelBL1FullLoad<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::SetOrgShapeFull()
{
    if constexpr (A_TYPE::format == CubeFormat::NZ && B_TYPE_NEW::format == CubeFormat::NZ) {
        ...
        mm_.SetOrgShape(this->block_.matmulTilingData_->matmulTiling.M, this->block_.params_.singleCoreN,
                        this->block_.matmulTilingData_->matmulTiling.Ka,
                        this->block_.matmulTilingData_->matmulTiling.Kb,
                        this->block_.matmulTilingData_->matmulTiling.N);
    }
}

template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void MatmulBaseKernelBL1FullLoad<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::ProcessRound(
    uint64_t mTileIndex, uint64_t nTileIndex, uint8_t enAtomic)
{
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void MatmulBaseKernelBL1FullLoad<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::Process(
    uint64_t index, uint8_t enAtomic)
{
    if ASCEND_IS_AIV {
        return;
    ...
    uint64_t nAligned = B_TYPE_NEW::isTrans ?
                            MMV3CeilAlign(
                                static_cast<uint64_t>(this->block_.matmulTilingData_->matmulTiling.singleCoreN),
                                BLOCK_SIZE) :
                            MMV3CeilAlign(
                                static_cast<uint64_t>(this->block_.matmulTilingData_->matmulTiling.singleCoreN),
                                innerAlignedBlock);
    uint64_t kbAligned = B_TYPE_NEW::isTrans ?
                             MMV3CeilAlign(static_cast<uint64_t>(this->block_.matmulTilingData_->matmulTiling.Kb),
                                           innerAlignedBlock) :
                             MMV3CeilAlign(static_cast<uint64_t>(this->block_.matmulTilingData_->matmulTiling.Kb),
                                           BLOCK_SIZE);
                                             ...
                                             uint64_t instrN)
{
    ...
    nd2nzParams.srcNdMatrixStride = 0;
    nd2nzParams.srcDValue = static_cast<uint64_t>(B_TYPE::isTrans ? matmulTilingData.matmulTiling.Kb :
                                             ...
                                             bool isNMultiCore, DataCopyParams& dataCopyParams, uint64_t instrN)
{
    if (B_TYPE::isTrans && isNMultiCore) {
        ...
        dataCopyParams.blockLen = block.params_.alignedKbSize * MMV3CeilAlign(instrN, block.params_.alignedOriN) *
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG>
__aicore__ inline void MatmulBaseKernelBL1FullLoad<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG>::CopyInB1(
    const MatmulTilingData& matmulTilingData, LocalTensor<B_T> bl1Local)
{
    bool isNMultiCore = matmulTilingData.matmulTiling.singleCoreN < matmulTilingData.matmulTiling.N;
    ...
    if constexpr (B_TYPE_NEW::format == CubeFormat::ND) {
        ...
        DataCopyParams dataCopyParams;
        CalCopyBL1Nz2NzParams<B_TYPE_NEW, B_T, BLOCK_TYPE>(this->block_, matmulTilingData, isNMultiCore, dataCopyParams,
                                                           instrN);
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG,
          class MM_CB>
__aicore__ inline void MatmulBaseUnAlignedKernelBL1FullLoad<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG,
