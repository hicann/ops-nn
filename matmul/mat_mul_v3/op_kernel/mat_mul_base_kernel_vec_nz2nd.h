    ...
    const void* tilingData, TPipe* pipe)
{
    ...
    this->tempCGlobal_.SetGlobalBuffer(
        reinterpret_cast<__gm__ C_T*>(workspaceGM),
...
__aicore__ inline void MatmulBaseVectorNz2NdKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG,
                                                   MM_CB>::AicProcess(GlobalTensor<C_T>& cTensor, uint8_t enAtomic,
...
template <class A_TYPE, class B_TYPE, class C_TYPE, class BIAS_TYPE, class BLOCK_TYPE, const MatmulConfig& MM_CFG,
          class MM_CB>
__aicore__ inline void MatmulBaseVectorNz2NdKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG,
                                                   MM_CB>::AivNz2NdProcess(GlobalTensor<C_T> cNzGlobal,
                                                                           uint8_t pingPongId)
{
    if ASCEND_IS_AIV {
        ...
        if (std::is_same_v<C_T, bfloat16_t> || std::is_same_v<C_T, half>) {
            ...
            dataCopyExtParams.dstStride = static_cast<uint32_t>(
                (this->block_.matmulTilingData_->matmulTiling.N - nBlocks) * sizeof(C_T));
...
MatmulBaseVectorNz2NdKernel<A_TYPE, B_TYPE, C_TYPE, BIAS_TYPE, BLOCK_TYPE, MM_CFG, MM_CB>::Process(uint64_t index,
                                                                                                   uint8_t enAtomic)
{
