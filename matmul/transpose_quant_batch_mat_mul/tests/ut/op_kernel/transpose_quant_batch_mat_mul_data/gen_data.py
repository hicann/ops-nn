...
def gen_mxfp8_data(M, Batch, K, N, perm_x1, perm_x2, perm_y, group_size):
    ...
    if perm_x2 == [0, 2, 1]:
        x2 = np.random.randint(-127, 127, [Batch, N, K]).astype(np.int8)
        x2_scale = np.random.uniform(2, 10, [Batch, N, num_groups_x2, 2]).astype(
            np.uint8
        )
    else:
        x2 = np.random.randint(-127, 127, [Batch, K, N]).astype(np.int8)
        x2_scale = np.random.uniform(2, 10, [Batch, num_groups_x2, N, 2]).astype(
            np.uint8
        )
    output_shape = [M, Batch, N]
    ...
    print(
        f"MXFP8 data generated: x1={x1.shape}, x2={x2.shape}, output={output.shape}, group_size={group_size}"
    )
