...
static constexpr MicroAPI::CastTrait B32_TO_B16_TRAIT_PART_ONE = {MicroAPI::RegLayout::ZERO, MicroAPI::SatMode::NO_SAT,
...
__simd_vf__ inline void InitUbToBias(uint16_t count, __ubuf__ float* srcAddr, __ubuf__ float* dstAddr)
{
    ...
    for (uint16_t idx = 0; idx < count; idx++) {
        MicroAPI::StoreAlign<float, MicroAPI::StoreDist::DIST_NORM_B32>(dstAddr + idx * biasStoreLen, biasReg1,
                                                                        maskAll);
        MicroAPI::StoreAlign<float, MicroAPI::StoreDist::DIST_NORM_B32>(dstAddr + storeDataOffset + idx * biasStoreLen,
                                                                        biasReg2, maskAll);
    }
}
                                     ...
                                     uint16_t mL0Size, uint16_t x1ScaleGroupSizeNum)
{
    ...
    for (uint16_t mIdx = 0; mIdx < mL0Size; mIdx++) {
        ...
        MicroAPI::LoadAlign<float, MicroAPI::LoadDist::DIST_NORM>(cFp32Reg2,
                                                                  cFp32Addr + mIdx * L0C_BASE_N + VEC_MAX_ELEM_B32);
        ...
        if constexpr (!IsSameType<typename MicroAPI::TypeGet<yType>::T, vector_f32>::value) {
            ...
            MicroAPI::StoreAlign<yType, MicroAPI::StoreDist::DIST_NORM>(cFp32Addr + mIdx * L0C_BASE_N, cFp32Reg1,
                                                                        maskAll);
