...
protected:
    static void SetUpTestCase() { std::cout << "l2_einsum_test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "l2_einsum_test TearDown" << std::endl; }
};
...
TEST_F(l2_einsum_test, case_nullptr_1)
{
    ...
    auto ut0 = OP_API_UT(aclnnEinsum, INPUT(nullptr, equation), OUTPUT(out));
...
TEST_F(l2_einsum_test, case_nullptr_2)
{
    ...
    auto ut0 = OP_API_UT(aclnnEinsum, INPUT(tensorListDesc, equation), OUTPUT(nullptr));
    uint64_t workspace_size = 0;
    aclnnStatus aclRet = ut0.TestGetWorkspaceSize(&workspace_size);
    EXPECT_EQ(aclRet, ACLNN_ERR_PARAM_NULLPTR);
}
