...
void BatchMatmulV3BaseTiling::CheckandSetDiagonalConflict(uint64_t mCnt, uint64_t nCnt, uint64_t batch,
                                                          ...
                                                          uint64_t newMcnt)
{
    ...
    bmmTilingData_.matmulTiling.tileL2cacheTiling.mTileCntL2 = static_cast<uint32_t>(cBatchDimAll_ *
                                                                                     ops::CeilDiv(mCnt, newMcnt));
...
void BatchMatmulV3BaseTiling::DoL2CacheAndCalOrderTiling()
{
    ...
    // 使用全载tilingkey作为判断标准
    bool isFullLoad = ((tilingEnable_.tilingEnableMultiBatchL1FullLoad == TilingEnableMultiBatchL1FullLoad::IS_FALSE) &&
    ...
    if (!isFullLoad) {
        if (isL2Tile) {
            bmmTilingData_.matmulTiling.tileL2cacheTiling.mTileCntL2 = static_cast<uint32_t>(runInfo_.l2Info.mTile *
                                                                                             cBatchDimAll_);
            if (isBigSize) {
                bmmTilingData_.matmulTiling.tileL2cacheTiling.mTileCntL2 = static_cast<uint32_t>(
                    cBatchDimAll_ * ops::CeilDiv(mCnt, newMcnt));
                bmmTilingData_.matmulTiling.tileL2cacheTiling.mTileBlock = newMcnt;
                bmmTilingData_.matmulTiling.tileL2cacheTiling.nTileBlock = nCnt;
            }
        } else {
...
static void CalcBaseMN(uint64_t& baseM, uint64_t& baseN, const matmul_v3::MatmulV3Args& args, uint64_t divisor = 1UL)
{
    ...
    auto getBestBaseM = [dtypeSize, divisor](bool transX1, uint64_t m, uint64_t n, uint64_t k) -> uint64_t {
        if (!transX1) {
            uint64_t maxM = k >= 32UL ? 1024UL :
                                        2048UL; // 防止baseK太小,限制baseM最大为2048B, k大于32时进一步限制为1024B
...
void BatchMatmulV3BaseTiling::DoCommonTiling()
{
    ...
    uint64_t depthA1 = (totalL1Size / NUM_TWO / aDtypeSize_ / (baseM * baseK) / 4UL) *
                       4UL; // DB后FP32下，L1可以存65536个数值
    uint64_t depthB1 = (totalL1Size / NUM_TWO / aDtypeSize_ / (baseN * baseK) / 4UL) *
                       4UL; // DB后FP32下，L1可以存65536个数值
...
void BatchMatmulV3BaseTiling::DoMultiBatchTiling()
{
    ...
    if (args_.nd2nzA || args_.nd2nzB) {
        ...
        tilingEnable_.tilingEnableLoadMode = TilingEnableLoadMode::BASE;
        tilingEnable_.tilingEnableMultiBatchOut = isMultiBatchOut ? TilingEnableMultiBatchOut::IS_TRUE :
                                                                    TilingEnableMultiBatchOut::IS_FALSE;
        tilingEnable_.tilingEnableMixNd2Nz = TilingEnableMixNd2Nz::IS_TRUE;
...
static void TuneDownBaseBlock(bool isTensorA, const matmul_v3::MatmulV3Args& args, uint64_t l1Size, uint64_t& baseMN,
                              uint64_t& baseK)
{
...
{
    if (compileInfo_.npuArch == NpuArch::DAV_2002 || std::string(context_->GetNodeType()) == "TransposeBatchMatMul") {
...
bool BatchMatmulV3BaseTiling::CalcVectorShapeInfo(VectorShapeInfo& shapeInfo) const
{
    ...
    if (dimNum < VECTOR_DIM_THRESHOLD) {
        return false;
    }
...
bool BatchMatmulV3BaseTiling::CalcVectorCoreParams(const VectorShapeInfo& shapeInfo, VectorCoreParams& coreParams) const
{
    ...
    if (coreParams.coreData == 0 || coreParams.rowsPerCore == 0) {
        return false;
    }
    return true;
}
...
void BatchMatmulV3BaseTiling::DoVectorTiling()
{
    ...
    if (!CalcVectorShapeInfo(shapeInfo)) {
        return;
    }

    VectorCoreParams coreParams{};
    if (!CalcVectorCoreParams(shapeInfo, coreParams)) {
        return;
    }
...
bool BatchMatmulV3BaseTiling::CheckVectorShapeDims()
{
    ...
    if (aDims < MIN_DIM || aDims > MAX_DIM || bDims < MIN_DIM || bDims > MAX_DIM) {
        OP_LOGD(args_.opName,
                "BatchMatmulV3BaseTiling: A and B must have between %d and %d dimensions. "
                "Bmm vector opt version not supported.",
                MIN_DIM, MAX_DIM);
        return false;
    }
...
bool BatchMatmulV3BaseTiling::CheckVectorComputationCondition()
{
    ...
    if (!CheckVectorDtypeAndKAxis()) {
        return false;
    }
    if (!CheckVectorBatchBroadcast()) {
        return false;
    }
...
uint64_t BatchMatmulV3BaseTiling::GetTilingKey() const { return tilingKey_; }
