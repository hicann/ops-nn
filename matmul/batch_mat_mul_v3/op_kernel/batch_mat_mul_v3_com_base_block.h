...
__aicore__ inline void BatchMatMulCommonBaseBlock::Init(const void* tilingData)
{
    ...
    params_.alignedOriN = MMV3DivCeil(batchMatmulTilingData_->matmulTiling.matmulTiling.N, params_.c0Size) *
                          params_.c0Size;
    params_.alignedKaSize = MMV3DivCeil(batchMatmulTilingData_->matmulTiling.matmulTiling.Ka, params_.c0Size) *
                            params_.c0Size;
    ...
    if constexpr (A_TYPE::isTrans) {
        params_.alignedOriM = MMV3DivCeil(batchMatmulTilingData_->matmulTiling.matmulTiling.M, params_.c0Size) *
                              params_.c0Size;
        params_.alignedKaSize = MMV3DivCeil(batchMatmulTilingData_->matmulTiling.matmulTiling.Ka, ALIGNED_H) *
                                ALIGNED_H;
    }
    if constexpr (B_TYPE::isTrans) {
        ...
        params_.alignedKbSize = MMV3DivCeil(batchMatmulTilingData_->matmulTiling.matmulTiling.Kb, params_.c0Size) *
                                params_.c0Size;
    }
}
...
__aicore__ inline void BatchMatMulCommonBaseBlock::InitBatchCount()
{
    ...
    params_.nBaseTail = batchMatmulTilingData_->matmulTiling.matmulTiling.N -
                        (params_.nCnt - 1) *
                            batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreN; // n方向上的baseN尾块
    params_.mBaseTail = batchMatmulTilingData_->matmulTiling.matmulTiling.M -
                        (params_.mCnt - 1) *
                            batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreM; // m方向上的baseM尾块
    ...
    params_.preCoreNum = params_.totalCnt % batchMatmulTilingData_->multiBatchInfo
                                                .batchUsedCoreNum; // 从0core开始有多少个core会多做一次base块
...
__aicore__ inline void BatchMatMulCommonBaseBlock::InitL2Cache()
{
    ...
    if (params_.enableL2Cache) {
        ...
        params_.totalTileCnt = params_.mTileCnt * params_.nTileCnt;
        params_.mCntTail = params_.mCnt - (params_.mTileCnt *
                                           (MMV3DivCeil(params_.mCnt, params_.mTileCnt) - 1)); // M方向上mtile尾块的个数
    ...
    if (params_.rowOrder == 0) {
        ...
        if (GetBlockIdx() < params_.preCoreNum) {
            if (params_.rowOrder == ROW_FIRST) {
                ...
                params_.index = params_.preTotalBlock / params_.mCnt +
                                params_.preTotalBlock % params_.mCnt * params_.nCnt;
            }
...
__aicore__ inline void BatchMatMulCommonBaseBlock::UpdateBasicIndex(const uint64_t roundIdx, uint64_t mTileIndex,
                                                                    uint64_t nTileIndex)
{
    uint64_t newBlockIdx = (GetBlockIdx() + batchMatmulTilingData_->matmulTiling.matmulTiling.usedCoreNum -
...
__aicore__ inline void BatchMatMulCommonBaseBlock::UpdateBlockCnt(uint64_t bmTileIndex, uint64_t nTileIndex)
{
    ...
    params_.mTileAddrOffset = curBatchCnt * batchMatmulTilingData_->matmulTiling.matmulTiling.M +
                              curBatchTileIdx * params_.mTileCnt *
                                  batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreM;
    params_.nTileAddrOffset = nTileIndex * params_.nTileCnt *
    ...
    params_.round = MMV3DivCeil(params_.totalTileCnt,
    ...
    if (batchMatmulTilingData_->matmulTiling.matmulTiling.isBias) {
        offset_.offsetBias = params_.biasWithBatch ?
                                 batchCIndex *
                                     ...
                                     nCntIndex * static_cast<uint64_t>(
                                                     batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreN) +
                                                               ...
                                                               uint64_t mInnerBatchOffset)
{
    if constexpr (A_TYPE::format == CubeFormat::ND) {
        ...
        offset_.offsetA = A_TYPE::isTrans ?
                              (offsetABatch +
                               (mCntIndex * batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreM) +
                               mInnerBatchOffset) :
                              (offsetABatch +
                               (mCntIndex *
                                static_cast<uint64_t>(batchMatmulTilingData_->matmulTiling.matmulTiling.Ka) *
                                batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreM) +
        ...
        offset_.offsetA = A_TYPE::isTrans ? offsetABatch +
                                                (mCntIndex * batchMatmulTilingData_->matmulTiling.matmulTiling.baseM *
                                                               ...
                                                               uint64_t nCntIndex, uint64_t mInnerBatchOffset)
{
    if constexpr (C_TYPE::format == CubeFormat::ND) {
        ...
        offset_.offsetC = batchCIndex * static_cast<uint64_t>(batchMatmulTilingData_->matmulTiling.matmulTiling.M) *
                          ...
                          nCntIndex * batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreN +
                          mCntIndex *
                              static_cast<uint64_t>(batchMatmulTilingData_->matmulTiling.matmulTiling.singleCoreM) *
                              batchMatmulTilingData_->matmulTiling.matmulTiling.N +
                          mInnerBatchOffset * batchMatmulTilingData_->matmulTiling.matmulTiling.N +
                          params_.nTileAddrOffset;
#endif
