...
int main()
{
    ...
    ret = CreateAclTensor(logitsMaxLocalOutHostData, logitsMaxLocalOutShape, &logitsMaxLocalOutDeviceAddr,
                          aclDataType::ACL_FLOAT, &logitsMaxLocalOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建sumExpLogitsLocalOut aclTensor
    ret = CreateAclTensor(sumExpLogitsLocalOutHostData, sumExpLogitsLocalOutShape, &sumExpLogitsLocalOutDeviceAddr,
                          aclDataType::ACL_FLOAT, &sumExpLogitsLocalOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建predictedLogitsLocalOut aclTensor
    ret = CreateAclTensor(predictedLogitsLocalOutHostData, predictedLogitsLocalOutShape,
                          &predictedLogitsLocalOutDeviceAddr, aclDataType::ACL_FLOAT, &predictedLogitsLocalOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建targetMaskOut aclTensor
    ret = CreateAclTensor(targetMaskOutHostData, targetMaskOutShape, &targetMaskOutDeviceAddr, aclDataType::ACL_UINT8,
                          &targetMaskOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建maskedTargetOut aclTensor
    ret = CreateAclTensor(maskedTargetOutHostData, maskedTargetOutShape, &maskedTargetOutDeviceAddr,
                          aclDataType::ACL_INT32, &maskedTargetOut);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建vocabParallelLogitsOutOptional aclTensor
    ret = CreateAclTensor(vocabParallelLogitsOutOptionalHostData, vocabParallelLogitsOutOptionalShape,
                          &vocabParallelLogitsOutOptionalDeviceAddr, aclDataType::ACL_FLOAT16,
    ...
    ret = aclnnFusedLinearOnlineMaxSumGetWorkspaceSize(input, weight, target, vocabStartIndex, vocabEndIndex,
                                                       logitsMaxLocalOut, sumExpLogitsLocalOut, predictedLogitsLocalOut,
    ...
    ret = aclrtMemcpy(
        logitsMaxLocalOutResultData.data(), logitsMaxLocalOutResultData.size() * sizeof(logitsMaxLocalOutResultData[0]),
        logitsMaxLocalOutDeviceAddr, size * sizeof(logitsMaxLocalOutResultData[0]), ACL_MEMCPY_DEVICE_TO_HOST);
    ...
    ret = aclrtMemcpy(sumExpLogitsLocalOutResultData.data(),
                      sumExpLogitsLocalOutResultData.size() * sizeof(sumExpLogitsLocalOutResultData[0]),
                      sumExpLogitsLocalOutDeviceAddr, size * sizeof(sumExpLogitsLocalOutResultData[0]),
    ...
    ret = aclrtMemcpy(predictedLogitsLocalOutResultData.data(),
                      predictedLogitsLocalOutResultData.size() * sizeof(predictedLogitsLocalOutResultData[0]),
                      predictedLogitsLocalOutDeviceAddr, size * sizeof(predictedLogitsLocalOutResultData[0]),
                      ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    for (int64_t i = 0; i < size; i++) {
    ...
    ret = aclrtMemcpy(targetMaskOutResultData.data(),
                      targetMaskOutResultData.size() * sizeof(targetMaskOutResultData[0]), targetMaskOutDeviceAddr,
                      size * sizeof(targetMaskOutResultData[0]), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    for (int64_t i = 0; i < size; i++) {
    ...
    ret = aclrtMemcpy(
        maskedTargetOutResultData.data(), maskedTargetOutResultData.size() * sizeof(maskedTargetOutResultData[0]),
        maskedTargetOutDeviceAddr, size * sizeof(maskedTargetOutResultData[0]), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    for (int64_t i = 0; i < size; i++) {
    ...
    ret = aclrtMemcpy(
        vocabParallelLogitsOutOptionalResultData.data(),
        vocabParallelLogitsOutOptionalResultData.size() * sizeof(vocabParallelLogitsOutOptionalResultData[0]),
        vocabParallelLogitsOutOptionalDeviceAddr, size * sizeof(vocabParallelLogitsOutOptionalResultData[0]),
        ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    for (int64_t i = 0; i < size; i++) {
        LOG_PRINT("vocabParallelLogitsOutOptional[%ld] is: %f\n", i,
                  static_cast<float>(vocabParallelLogitsOutOptionalResultData[i]));
    }
