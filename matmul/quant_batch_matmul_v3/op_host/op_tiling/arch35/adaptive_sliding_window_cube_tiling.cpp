...
{
    bool isCubePerTensor = inputParams_.isPerTensor &&
                           ((inputParams_.aDtype == ge::DT_INT8 && inputParams_.biasDtype == ge::DT_INT32 &&
                             !inputParams_.isPertoken) ||
    ...
    bool isCubePerChannel = inputParams_.isPerChannel &&
                            (inputParams_.scaleDtype == ge::DT_UINT64 || inputParams_.scaleDtype == ge::DT_INT64 ||
...
void AdaptiveSlidingWindowCubeTiling::UpdateBFullLoadStatus()
{
    ...
    uint64_t singleCoreBSizeWithFullLoad = adaptiveWin_.baseN *
                                           (inputParams_.transB ?
                                                ops::CeilAlign(
    ...
    isBFullLoad_ = singleCoreBSizeWithFullLoad <= aicoreParams_.l1Size / qmmv3_tiling_const::NUM_HALF &&
                   adaptiveWin_.mBlockCnt > adaptiveWin_.nBlockCnt &&
                   (((adaptiveWin_.nBlockCnt == 1UL || adaptiveWin_.nBlockCnt == 2UL ||
                      adaptiveWin_.nBlockCnt == 4UL) &&
                     aicoreParams_.aicNum >= adaptiveWin_.nBlockCnt &&
...
void AdaptiveSlidingWindowCubeTiling::UpdateABFullLoadStatus()
{
    ...
    uint64_t singleCoreASizeWithFullLoad = adaptiveWin_.baseM *
                                           (inputParams_.transA ?
                                                GetSizeWithDataType(
                                                    ops::CeilAlign(inputParams_.kSize, qmmv3_tiling_const::CUBE_BLOCK),
                                                    inputParams_.aDtype) :
    ...
    bool hasEnoughL1 = aicoreParams_.l1Size >= singleCoreASizeWithFullLoad &&
...
REGISTER_TILING_TEMPLATE_WITH_ARCH(QuantBatchMatmulV3, AdaptiveSlidingWindowCubeTiling, supportedNpuArch,
                                   TILING_PRIORITY);
} // namespace optiling
