...
uint32_t QuantBatchMatmulV3TilingBase::GetX1Idx() const { return X1_INDEX; }
uint32_t QuantBatchMatmulV3TilingBase::GetX2Idx() const { return X2_INDEX; }
uint32_t QuantBatchMatmulV3TilingBase::GetScaleIdx() const { return SCALE_INDEX; }
uint32_t QuantBatchMatmulV3TilingBase::GetOffsetIdx() const { return OFFSET_INDEX; }
uint32_t QuantBatchMatmulV3TilingBase::GetBiasIdx() const { return BIAS_INDEX; }
uint32_t QuantBatchMatmulV3TilingBase::GetPertokenIdx() const { return PERTOKEN_SCALE_INDEX; }
...
void ResetQuantBatchMatmulV3InputParams() { g_quantBatchMatmulInfoFactory.Get()->Reset(); }
...
const char* QuantBatchMatmulV3TilingBase::GetDefaultOpName() const { return DEFAULT_OP_NAME; }
...
but x1 dimension: %zu, x2 dimension: %zu",
    ...
    if (inputParams_.aDtype == ge::DT_INT4 && !inputParams_.isPertoken) {
        OP_TILING_CHECK(x2ShapeLen != A4W4_X2_LEN,
                        CUBE_INNER_ERR_REPORT(inputParams_.opName, "If input dtype is int4 and without pertoken scale, \
the dimension of x2 must be 2."),
                        return false);
    }
    return true;
}
                                                              ...
                                                              size_t& x2StorageDim) const
{
    ...
    OP_TILING_CHECK(
        ...
        return false);
    OP_TILING_CHECK(
        x2StorageShape[x2StorageDim - LAST_SECOND_DIM_INDEX] != WEIGHTNZ_STORAGE_PENULTIMATE_DIM,
        CUBE_INNER_ERR_REPORT(inputParams_.opName,
    ...
    if (!inputParams_.transB) {
        OP_TILING_CHECK(
            !CheckStorageShape4WeightNzDimAlignment(inputParams_.kSize, inputParams_.nSize, x2StorageLastThirdDim,
                                                    x2StorageLastFourthDim),
            CUBE_INNER_ERR_REPORT(inputParams_.opName, "Dim check does not pass, please read the error report above"),
            return false;);
    } else {
        OP_TILING_CHECK(
            !CheckStorageShape4WeightNzDimAlignment(inputParams_.nSize, inputParams_.kSize, x2StorageLastThirdDim,
                                                    x2StorageLastFourthDim),
            CUBE_INNER_ERR_REPORT(inputParams_.opName, "Dim check does not pass, please read the error report above"),
            return false;);
    }
...
bool QuantBatchMatmulV3TilingBase::CheckStorageShape4WeightNzDimAlignment(uint64_t innerDim, uint64_t outerDim,
                                                                          uint64_t x2StorageLastThirdDim,
                                                                          uint64_t x2StorageLastFourthDim) const
{
    ...
    OP_TILING_CHECK(
        isNotLastThirdDimAligned,
        CUBE_INNER_ERR_REPORT(
            inputParams_.opName,
            "When format of x2 is FRACTAL_NZ, the last third dim of x2 should be ceil(k/16) = %lu, actual is %lu.",
            ops::CeilDiv(innerDim, WEIGHTNZ_STORAGE_PENULTIMATE_DIM), x2StorageLastThirdDim),
        return false);
    OP_TILING_CHECK(
        ...
        CUBE_INNER_ERR_REPORT(inputParams_.opName,
                ...
                = %lu for int8, or ceil(n/64) = %lu for int4, actual is %lu.",
                              ...
                              ops::CeilDiv(outerDim, WEIGHTNZ_STORAGE_LAST_DIM_64), x2StorageLastFourthDim),
        return false);
    return true;
}
...
bool QuantBatchMatmulV3TilingBase::IsMicroScaling() const { return inputParams_.scaleDtype == ge::DT_FLOAT8_E8M0; }
...
bool QuantBatchMatmulV3TilingBase::AnalyzeInputs()
{
    ...
    OP_TILING_CHECK(resultCheckFusionBatchA == OUTPUT_INFER_FAIL,
                    CUBE_INNER_ERR_REPORT(inputParams_.opName, "The fused M [%lu] exceed INT32_MAX [%d] in a4w4 case",
...
bool QuantBatchMatmulV3TilingBase::ReCalcGroupSize(uint64_t& groupSize, uint64_t inputSize, uint64_t scaleSize,
                                                   const char* dimensionName)
{
    if (groupSize == 0ULL) {
        ...
        OP_TILING_CHECK(scaleSize == 0ULL,
                        CUBE_INNER_ERR_REPORT(inputParams_.opName, "The %s dimension of %s is 0, invalid shape!",
                                                    ...
                                                    const gert::StorageShape* pertokenShape)
{
    ...
    if ((scaleInner == 1 && scaleOuter == inputParams_.nSize) ||
        (scaleInner == inputParams_.nSize && scaleOuter == 1) ||
...
bool QuantBatchMatmulV3TilingBase::GetUbDequantExtreSpace() { return false; }
...
bool QuantBatchMatmulV3TilingBase::CheckDtype() const { return false; }

ge::graphStatus QuantBatchMatmulV3TilingBase::CalcUbTiling() { return ge::GRAPH_FAILED; }

uint64_t QuantBatchMatmulInfo::GetMatmulApiMSize() const { return mSizePerNpu > 0U ? mSizePerNpu : mSize; }
