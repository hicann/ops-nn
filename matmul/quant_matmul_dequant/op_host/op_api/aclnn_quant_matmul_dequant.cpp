...
aclnnStatus aclnnQuantMatmulDequantGetWorkspaceSize(const aclTensor* x, const aclTensor* weight,
                                                    ...
                                                    const aclTensor* smoothScaleOptional, char* xQuantMode,
                                                    bool transposeWeight, const aclTensor* out, uint64_t* workspaceSize,
                                                    aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnQuantMatmulDequant,
                   DFX_IN(x, weight, weightScale, biasOptional, xScaleOptional, xOffsetOptional, smoothScaleOptional,
                          xQuantMode, transposeWeight),
                   DFX_OUT(out));
    // 创建OpExecutor
    ...
    if (weightDimNum == ND_DIMNUM && weight->GetStorageFormat() == op::Format::FORMAT_ND) {
        ...
        if (weight->GetViewShape().GetDim(K_IDX) % FRACTAL_K_INT8 != 0) {
            ...
            auto valueTensor = uniqueExecutor.get()->ConvertToTensor(uniqueExecutor.get()->AllocScalar(0),
                                                                     DataType::DT_INT8);
        ...
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                "weight is not in 2-dim shape (N, K) or 4-dim (K//32, N//16, 16, 32), please check");
        return ACLNN_ERR_PARAM_INVALID;
    }
    ...
    auto matmulRet = l0op::QuantMatmulDequant(xContiguous, weight, weightScaleContiguous, biasOptional,
                                              xScaleContiguous, xOffsetOptional, smoothScaleContiguous, xQuantMode,
                                              transposeWeight, out, uniqueExecutor.get());
...
aclnnStatus aclnnQuantMatmulDequant(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                    aclrtStream stream)
{
