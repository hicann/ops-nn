...
template <typename BaseT>
bool QuantBatchMatmulInplaceAddHelper<BaseT>::InitMatmulSize(const gert::Shape& x1Shape, const gert::Shape& x2Shape)
{
    ...
    if (x1ShapeLen < X1_MINIMUM_DIMENSION_LENGTH || x2ShapeLen < X2_MINIMUM_DIMENSION_LENGTH) {
        OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(this->inputParams_.opName, "x1, x2",
                                                  FormatString("%zuD, %zuD", x1ShapeLen, x2ShapeLen).c_str(),
...
template <typename BaseT>
bool QuantBatchMatmulInplaceAddHelper<BaseT>::ValidateQuantParams(const gert::Shape& x1ScaleShape,
...
bool QuantBatchMatmulInplaceAddHelper<BaseT>::IsHiFloat8TTQuant() const
{
    return IsHiFloat8Dtype(this->inputParams_.aDtype) && IsHiFloat8Dtype(this->inputParams_.bDtype) &&
           ...
           this->inputParams_.isDoubleScale && this->inputParams_.isPerTensor;
}
...
bool QuantBatchMatmulInplaceAddHelper<BaseT>::CheckDtype() const
{
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(this->inputParams_.opName, "output",
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(this->inputParams_.opName, "transposeX1, transposeX2",
                                               FormatString("%s, %s", this->inputParams_.transA ? "true" : "false",
                                                            this->inputParams_.transB ? "true" : "false")
                                                   .c_str(),
...
template <typename BaseT>
bool QuantBatchMatmulInplaceAddHelper<BaseT>::CheckShapeVaild(const gert::Shape& x1Shape,
                                                              const gert::Shape& x2Shape) const
{
    ...
    OP_CHECK_IF(
        ...
        return false);
    auto x2KDimValue = static_cast<uint64_t>(this->inputParams_.transB ? x2Shape.GetDim(1) : x2Shape.GetDim(0));
    auto x1KDimValue = static_cast<uint64_t>(this->inputParams_.transA ? x1Shape.GetDim(0) : x1Shape.GetDim(1));
...
template <typename BaseT>
bool QuantBatchMatmulInplaceAddHelper<BaseT>::CheckParamsForMxQuant(const gert::Shape& x1ScaleShape,
                                                                    const gert::Shape& x2ScaleShape) const
{
    ...
    OP_CHECK_IF(x2ScaleDimNum != MX_X2_SCALE_DIM,
                ...
                return false);
    auto x2ScaleNDim = static_cast<uint64_t>(this->inputParams_.transB ? x2ScaleShape.GetDim(0) :
                                                                         x2ScaleShape.GetDim(1));
    auto x2ScaleKDim = static_cast<uint64_t>(this->inputParams_.transB ? x2ScaleShape.GetDim(1) :
                                                                         x2ScaleShape.GetDim(0));
    auto x1ScaleMDim = static_cast<uint64_t>(this->inputParams_.transA ? x1ScaleShape.GetDim(1) :
                                                                         x1ScaleShape.GetDim(0));
    auto x1ScaleKDim = static_cast<uint64_t>(this->inputParams_.transA ? x1ScaleShape.GetDim(0) :
...
template <typename BaseT>
bool QuantBatchMatmulInplaceAddHelper<BaseT>::CheckParamsForPerTensorQuant(const gert::Shape& x1ScaleShape,
                                                                           const gert::Shape& x2ScaleShape) const
{
    OP_CHECK_IF(x1ScaleShape.GetDimNum() != 1 || x2ScaleShape.GetDimNum() != 1,
                ...
                return false);
    OP_CHECK_IF(x1ScaleShape.GetDim(0) != 1 || x2ScaleShape.GetDim(0) != 1,
...
bool QuantBatchMatmulInplaceAddHelper<BaseT>::AnalyzeInputs()
{
    ...
    OP_LOGD(this->inputParams_.opName, "batchA: %lu, batchB: %lu, batchC: %lu, isPerTensor: %d, isPertoken: %d",
