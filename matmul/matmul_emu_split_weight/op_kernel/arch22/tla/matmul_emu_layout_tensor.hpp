...
#ifndef MATMUL_EMU_LAYOUT_TENSOR_HPP
#define MATMUL_EMU_LAYOUT_TENSOR_HPP

#include "matmul_emu_layout_make.hpp"
...
struct PrependDimLayout<Layout, NewShapeT, NewStrideT, NewOriginT, tla::seq<Is...>> {
    ...
    using StrideNew = tla::Stride<NewStrideT,
                                  tla::remove_cvref_t<decltype(tla::get<Is>(std::declval<StrideOld>()))>...>;
...
struct underscore_count_from<Coord, I, std::enable_if_t<(I >= 0)>> {
    static constexpr int value = (coord_elem_is_underscore<Coord, I>::value ? 1 : 0) +
                                 underscore_count_from<Coord, I - 1>::value;
};
...
template <class Coord>
using underscore_indices = typename underscore_indices_impl<
    Coord, 0, (int)tla::tuple_size<tla::remove_cvref_t<Coord>>::value>::type;
...
struct Tensor {
    ...
    CATLASS_HOST_DEVICE constexpr decltype(auto) tensor() const { return *this; }

    CATLASS_HOST_DEVICE constexpr decltype(auto) data() const { return get<0>(rep_); }

    CATLASS_HOST_DEVICE constexpr decltype(auto) data() { return get<0>(rep_); }

    CATLASS_HOST_DEVICE constexpr decltype(auto) layout() const { return get<1>(rep_); }

    CATLASS_HOST_DEVICE constexpr decltype(auto) coord() const { return get<2>(rep_); }

    CATLASS_HOST_DEVICE constexpr decltype(auto) shape() const { return layout().shape(); }

    CATLASS_HOST_DEVICE constexpr decltype(auto) stride() const { return layout().stride(); }

    CATLASS_HOST_DEVICE constexpr decltype(auto) originShape() const { return layout().originShape(); }
...
#endif // MATMUL_EMU_LAYOUT_TENSOR_HPP
