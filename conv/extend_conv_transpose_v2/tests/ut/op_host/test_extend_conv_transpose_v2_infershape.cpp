...
/*!
 * \file test_extend_conv_transpose_v2_infershape.cpp
 * \brief
 */
...
class ExtendConvTransposeV2ProtoTest : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "ExtendConvTransposeV2 Proto Test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "ExtendConvTransposeV2 Proto Test TearDown" << std::endl; }
};

TEST_F(ExtendConvTransposeV2ProtoTest, basic)
{
    ...
    gert::StorageShape output_shape_y1 = {{}, {}};
    ...
    auto holder = gert::InferShapeContextFaker()
                      ...
                      .OutputShapes({&output_shape, &output_shape_y1})
                      .NodeAttrs(
                      ...
                      .NodeOutputTd(1, ge::DT_FLOAT16, ge::FORMAT_NCDHW, ge::FORMAT_NCDHW)
                      .Build();

    auto infer_shape_func = gert::OpImplRegistry::GetInstance().GetOpImpl("ExtendConvTransposeV2")->infer_shape;
    ...
    ASSERT_EQ(Ops::Base::ToString(*output1), "[4, 256, 1, 40, 32]");
}

TEST_F(ExtendConvTransposeV2ProtoTest, dynamic)
{
    vector<int64_t> strides({1, 1, 1, 1, 1});
    vector<int64_t> pads({0, 0, 0, 0, 0, 0});
    vector<int64_t> dilations({1, 1, 1, 1, 1});
...
TEST_F(ExtendConvTransposeV2ProtoTest, base_dtype)
{
    vector<int64_t> strides({1, 1, 1, 1, 1});
    vector<int64_t> pads({0, 0, 0, 0, 0, 0});
    vector<int64_t> dilations({1, 1, 1, 1, 1});
    ...
    ge::DataType y1Dtype = ge::DT_INT8;
    ...
    auto holder = gert::InferDataTypeContextFaker()
                      ...
                      .NodeInputTd(2, ge::DT_FLOAT16, ge::FORMAT_NCDHW, ge::FORMAT_NCDHW)
                      .NodeOutputTd(0, ge::DT_FLOAT16, ge::FORMAT_NCDHW, ge::FORMAT_NCDHW)
                      .NodeOutputTd(1, ge::DT_INT8, ge::FORMAT_NCDHW, ge::FORMAT_NCDHW)
                      .InputDataTypes({&input_sizeDtype, &xDtype, &filterDtype})
                      .OutputDataTypes({&yDtype, &y1Dtype})
                      .Build();

    auto inferDtypeFunc = gert::OpImplRegistry::GetInstance().GetOpImpl("ExtendConvTransposeV2")->infer_datatype;
...
TEST_F(ExtendConvTransposeV2ProtoTest, 2d_extend_3d)
{
    vector<int64_t> strides({1, 1, 1, 1, 1});
    vector<int64_t> pads({0, 0, 0, 0, 0, 0});
    vector<int64_t> dilations({1, 1, 1, 1, 1});
...
TEST_F(ExtendConvTransposeV2ProtoTest, compute_from_scratch_ncdhw)
{
...
TEST_F(ExtendConvTransposeV2ProtoTest, compute_from_scratch_ndhwc)
{
...
TEST_F(ExtendConvTransposeV2ProtoTest, compute_from_scratch_pads_3)
{
    vector<int64_t> strides({1, 1, 1, 1, 1});
...
TEST_F(ExtendConvTransposeV2ProtoTest, compute_from_scratch_groups_2)
{
    vector<int64_t> strides({1, 1, 1, 1, 1});
    vector<int64_t> pads({0, 0, 0, 0, 0, 0});
    vector<int64_t> dilations({1, 1, 1, 1, 1});
