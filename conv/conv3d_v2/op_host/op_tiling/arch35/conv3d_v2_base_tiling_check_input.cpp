...
ge::graphStatus Conv3dBaseTilingV2::ParseWeightShape()
{
    ...
    oriShapeAttrInfo_.oriWeightD = weightShape.GetDim(
        paramInfo_.paramsIdxVec[paramInfo_.WEIGHT_PARAM_IDX][IDX_LIST_D_IDX]);
...
ge::graphStatus Conv3dBaseTilingV2::CheckFmapShape()
{
    if (oriShapeAttrInfo_.oriFmapN < 1 || oriShapeAttrInfo_.oriFmapN > MAX_N_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.FMAP_PARAM_IDX][IDX_LIST_N_IDX], 1, MAX_N_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriFmapC < 1 || oriShapeAttrInfo_.oriFmapC > MAX_CIN_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetInputShapeVec(context_, INPUT_FMAP_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.FMAP_PARAM_IDX][IDX_LIST_C_IDX], 1, MAX_CIN_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriFmapD < 1 || oriShapeAttrInfo_.oriFmapD > MAX_FM_D_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetInputShapeVec(context_, INPUT_FMAP_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.FMAP_PARAM_IDX][IDX_LIST_D_IDX], 1, MAX_FM_D_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriFmapH < 1 || oriShapeAttrInfo_.oriFmapH > MAX_FM_H_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetInputShapeVec(context_, INPUT_FMAP_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.FMAP_PARAM_IDX][IDX_LIST_H_IDX], 1, MAX_FM_H_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriFmapW < 1 || oriShapeAttrInfo_.oriFmapW > MAX_FM_W_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetInputShapeVec(context_, INPUT_FMAP_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.FMAP_PARAM_IDX][IDX_LIST_W_IDX], 1, MAX_FM_W_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
...
ge::graphStatus Conv3dBaseTilingV2::CheckWeightShape()
{
    if (oriShapeAttrInfo_.oriWeightD < 1 || oriShapeAttrInfo_.oriWeightD > MAX_KD_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.WEIGHT_PARAM_IDX][IDX_LIST_D_IDX], 1, MAX_KD_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriWeightH < 1 || oriShapeAttrInfo_.oriWeightH > MAX_KH_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetInputShapeVec(context_, INPUT_WEIGHT_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.WEIGHT_PARAM_IDX][IDX_LIST_H_IDX], 1, MAX_KH_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriWeightW < 1 || oriShapeAttrInfo_.oriWeightW > MAX_KW_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetInputShapeVec(context_, INPUT_WEIGHT_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.WEIGHT_PARAM_IDX][IDX_LIST_W_IDX], 1, MAX_KW_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriWeightN < 1 || oriShapeAttrInfo_.oriWeightN > MAX_COUT_BF16_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetInputShapeVec(context_, INPUT_WEIGHT_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.WEIGHT_PARAM_IDX][IDX_LIST_N_IDX], 1, MAX_COUT_BF16_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus Conv3dBaseTilingV2::CheckWeightNZFormatShape()
{
    ...
    if (storageDimNum != FORMAT_FRACTAL_3D_DIM) {
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeType(), "filter", std::to_string(storageDimNum).c_str(),
    ...
    int64_t dc1hw = oriShapeAttrInfo_.oriWeightD * ((oriShapeAttrInfo_.oriWeightC + c0 - 1) / c0) *
                    oriShapeAttrInfo_.oriWeightH * oriShapeAttrInfo_.oriWeightW;
...
bool Conv3dBaseTilingV2::GetPosByFormat(const ge::Format format, const std::string& pos, const std::string& inputStr,
                                        size_t& posIdx) const
{
    ...
    OP_LOGE_IF(pos.length() != 1 || formatStr.find(pos) == string::npos, false, context_->GetNodeName(),
               "%s AscendC: position %s not in 5d format: %s", paramInfo_.nodeType.c_str(), pos.c_str(),
               formatStr.c_str());
...
ge::graphStatus Conv3dBaseTilingV2::ParseBiasShape()
{
    if (flagInfo_.isConv3dDequant && !flagInfo_.hasBias) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            context_->GetNodeType(), "bias", "nullptr",
            FormatString("If the dtype of input x is int8, parameter %s must be set", "bias").c_str());
        return ge::GRAPH_FAILED;
    }
    auto biasShapePtr = flagInfo_.quantFlag ? context_->GetOptionalInputShape(QUANT_INPUT_BIAS_INDEX) :
                                              context_->GetOptionalInputShape(INPUT_BIAS_INDEX);
    ...
    if (biasDimNum != FORMAT_ND_DIM && biasDimNum != CONV3D_DIM_SIZE_LIMIT) {
        ...
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeType(), "bias", std::to_string(biasDimNum).c_str(),
                                     expectDims.c_str());
        return ge::GRAPH_FAILED;
    }
    size_t idxC = 0;
            ...
            if (biasShapePtr->GetStorageShape().GetDim(i) != weightShape.GetDim(weightIdx)) {
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    ...
                    VectorsToString(std::vector<std::vector<int64_t>>{GetInputShapeVec(context_, INPUT_BIAS_INDEX),
                                    ...
                                    IntToString<int64_t>)
                        .c_str(),
                    FormatString("Shape[%zu] of %s must be equal to shape[%zu] of %s", i, "bias", weightIdx, "filter")
                        .c_str());
                return ge::GRAPH_FAILED;
            }
...
ge::graphStatus Conv3dBaseTilingV2::CheckOutputShape()
{
    if (oriShapeAttrInfo_.oriOutputD < 1 || oriShapeAttrInfo_.oriOutputD > MAX_DOUT_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.OUT_PARAM_IDX][IDX_LIST_D_IDX], 1, MAX_DOUT_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriOutputH < 1 || oriShapeAttrInfo_.oriOutputH > MAX_HOUT_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetOutputShapeVec(context_, OUTPUT_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.OUT_PARAM_IDX][IDX_LIST_H_IDX], 1, MAX_HOUT_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    if (oriShapeAttrInfo_.oriOutputW < 1 || oriShapeAttrInfo_.oriOutputW > MAX_WOUT_SHAPE) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            VectorToString(GetOutputShapeVec(context_, OUTPUT_INDEX), IntToString<int64_t>).c_str(),
            FormatString("Shape[%zu] of this parameter must be within the range [%ld, %ld]",
                         paramInfo_.paramsIdxVec[paramInfo_.OUT_PARAM_IDX][IDX_LIST_W_IDX], 1, MAX_WOUT_SHAPE)
                .c_str());
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
...
ge::graphStatus Conv3dBaseTilingV2::ParseOutputShape()
{
    ...
    oriShapeAttrInfo_.oriOutputD = outputShape.GetDim(
...
ge::graphStatus Conv3dBaseTilingV2::CheckQuantScaleDtype()
{
    if (flagInfo_.quantFlag) {
        if (descInfo_.scaleDtype != ge::DataType::DT_INT64 && descInfo_.scaleDtype != ge::DataType::DT_UINT64) {
            OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeType(), "scale", dtypeToStrTab.at(descInfo_.scaleDtype).c_str(),
                                      "int64 or uint64");
            return ge::GRAPH_FAILED;
        }
        ...
        if (descInfo_.scaleDtype != ge::DataType::DT_FLOAT) {
            OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
                context_->GetNodeType(), "scale", GeDtypeToString(descInfo_.scaleDtype).c_str(),
                FormatString("If the dtype of input x is int8, the dtype of parameter %s must be %s", "scale",
                             "float32")
                    .c_str());
            return ge::GRAPH_FAILED;
        }
...
ge::graphStatus Conv3dBaseTilingV2::CheckQuantScaleDesc()
{
    ...
    if (flagInfo_.isConv3dDequant && !flagInfo_.hasScale) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            context_->GetNodeType(), "scale", "nullptr",
    ...
    if (static_cast<ge::Format>(descInfo_.scaleFormat) != ge::Format::FORMAT_ND) {
        OP_LOGE_FOR_INVALID_FORMAT(context_->GetNodeType(), "scale", formatToStrTab.at(descInfo_.scaleFormat).c_str(),
                                   "ND");
        return ge::GRAPH_FAILED;
    }
    ...
    if (scaleShape != static_cast<size_t>(oriShapeAttrInfo_.oriWeightN)) {
        OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
            ...
            VectorsToString(std::vector<std::vector<int64_t>>{GetInputShapeVec(context_, tmpScaleIdx),
                                                              GetInputShapeVec(context_, INPUT_WEIGHT_INDEX)},
                            IntToString<int64_t>)
                .c_str(),
