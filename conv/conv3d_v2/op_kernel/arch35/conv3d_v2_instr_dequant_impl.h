...
public:
    ...
    __aicore__ inline void SetParams(Intf* self)
    {
        ...
        mStride = self_->ctx.convTilingData->mUB * BLOCK_L0_N;
        isFirst = true;

        fixpipeParams.params.ndNum = 1;
        fixpipeParams.dstStride = self_->ctx.convTilingData->nUB;
    ...
    __aicore__ inline void CopyOut()
    {
        ...
        if constexpr (Intf::outputOrder == static_cast<int8_t>(ConvOutputOrder::M_MODE)) {
            maxMIter = CeilDiv(currentML0, self_->ctx.convTilingData->mUB) - 1;
            mUbTail = currentML0 % self_->ctx.convTilingData->mUB;
        } else {
            maxWoUbLoopTimes = CeilDiv(self_->ctx.currentWoL0, self_->ctx.convTilingData->mUB);
        ...
        for (uint32_t mIter = 0; mIter <= maxMIter; ++mIter) {
            if constexpr (Intf::outputOrder == static_cast<int8_t>(ConvOutputOrder::M_MODE)) {
                self_->ctx.currentMUb = mIter == maxMIter ? mUbTail : self_->ctx.convTilingData->mUB;
            } else {
                woUbIter = mIter % maxWoUbLoopTimes;
                self_->ctx.currentMUb = woUbIter == maxWoUbIter ? mUbTail : self_->ctx.convTilingData->mUB;
...
public:
    ...
    __aicore__ inline void SetParams(Intf* self)
    {
        ...
        calcCount = self_->ctx.convTilingData->mUB * self_->ctx.convTilingData->nUB;

        uint32_t repStride = self_->ctx.convTilingData->nUB / COUNT_ONE_BLK_B32;
    ...
    __aicore__ inline void DeQuantCalc()
    {
        ...
        for (uint32_t mIter = 0; mIter <= maxMIter; ++mIter) {
            ...
            uint32_t mOffset = mIter * UB_MAX_REPEAT_TIMES * self_->ctx.convTilingData->nUB;
...
public:
    ...
    __aicore__ inline void CopyUB2GM()
    {
        ...
        copyParams.srcStride = (self_->ctx.convTilingData->nUB - self_->ctx.currentNL0Align) / blockSize;
        ...
        if constexpr (Intf::outputOrder == static_cast<int8_t>(ConvOutputOrder::M_MODE)) {
            ...
            dstOffset += hoUbIter * self_->ctx.convTilingData->orgWo * self_->ctx.convTilingData->orgCo +
                         woUbIter * self_->ctx.convTilingData->mUB * self_->ctx.convTilingData->orgCo;
        }
