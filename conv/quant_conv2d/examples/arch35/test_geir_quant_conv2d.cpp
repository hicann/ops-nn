...
#include "../../graph/quant_conv2d_proto.h"
...
int CreateQuantConv2DInGraph(std::vector<ge::Tensor>& inputTensors, std::vector<Operator>& inputOps,
                             std::vector<Operator>& outputOps, Graph& graph)
{
    Status ret = SUCCESS;
    auto tmpOp = op::QuantConv2D("quant_conv2d");
    std::vector<int64_t> xShape = {1, 16, 1, 1};
    std::vector<int64_t> filterShape = {16, 16, 1, 1};
    std::vector<int64_t> scaleShape = {16};
    std::vector<int64_t> biasShape = {16};
    std::vector<int64_t> yShape = {1, 16, 1, 1};
    tmpOp.set_attr_dtype(ge::DT_INT8);
    tmpOp.set_attr_strides({1, 1, 1, 1});
    tmpOp.set_attr_pads({0, 0, 0, 0});
    tmpOp.set_attr_dilations({1, 1, 1, 1});
    tmpOp.set_attr_groups(1);
    tmpOp.set_attr_data_format("NCHW");
    ADD_INPUT(1, x, ge::DT_INT8, xShape);
    ADD_INPUT(2, filter, ge::DT_INT8, filterShape);
    ADD_INPUT(3, scale, ge::DT_UINT64, scaleShape);
    ADD_INPUT(4, bias, ge::DT_INT32, biasShape);
