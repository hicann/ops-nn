...
bool Conv2DBackpropFilterToV3FusionPass::GetNodeDesc(const GNode& node)
{
    ...
    // 对于2DDW，输入x、out_backprop的索引分别是0、2
    OP_CHECK_IF(!Update2DInputDesc(input0Desc), OP_LOGE(GetNodeType().GetString(), "Update input0Desc failed"),
                return false);
