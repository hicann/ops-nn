...
/*!
 * \file conv2d_backprop_filter_tf_plugin.cpp
 * \brief
 */
#include <map>
...
static Status ParseParamsConv2DBackpropFilter(const ge::Operator& op_src, ge::Operator& op)
{
    ...
    ge::TensorDesc org_tensor_w = op.GetOutputDesc(CV_NUM_0);
    org_tensor_w.SetOriginFormat(ge::FORMAT_HWCN);
    org_tensor_w.SetFormat(ge::FORMAT_HWCN);
    auto ret = op.UpdateOutputDesc(CV_NUM_0, org_tensor_w);

    OP_LOGE_IF(ret != ge::GRAPH_SUCCESS, FAILED, op_name.GetString(), "failed to update output_desc format!");
    OP_LOGD(op_name.GetString(), "update filter format succeeded, now is [%s]",
            ge::TypeUtils::FormatToAscendString(op.GetInputDesc(CV_NUM_0).GetFormat()).GetString());
