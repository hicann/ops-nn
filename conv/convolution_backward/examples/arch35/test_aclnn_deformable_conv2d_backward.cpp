...
int aclnnDeformableConv2dBackwardTest(int32_t deviceId, aclrtStream& stream)
{
    ...
    ret = CreateAclTensor(gradOffsetHostData, gradOffsetShape, &gradOffsetDeviceAddr, aclDataType::ACL_FLOAT,
                          &gradOffset);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnDeformableConv2dBackwardGetWorkspaceSize failed. ERROR: %d\n", ret);
              return ret);
    ...
    ret = aclrtMemcpy(gradWeightShapeData.data(), gradWeightShapeData.size() * sizeof(gradWeightShapeData[0]),
                      gradWeightDeviceAddr, size * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    for (int64_t i = 0; i < size; i++) {
    ...
    ret = aclrtMemcpy(gradOffsetShapeData.data(), gradOffsetShapeData.size() * sizeof(gradOffsetShapeData[0]),
                      gradOffsetDeviceAddr, size * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    for (int64_t i = 0; i < size; i++) {
    ...
    ret = aclrtMemcpy(gradBiasShapeData.data(), gradBiasShapeData.size() * sizeof(gradBiasShapeData[0]),
                      gradBiasDeviceAddr, size * sizeof(float), ACL_MEMCPY_DEVICE_TO_HOST);
    CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
    for (int64_t i = 0; i < size; i++) {
...
int main()
{
    ...
    CHECK_FREE_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnDeformableConv2dBackwardTest failed. ERROR: %d\n", ret);
                   return ret);
