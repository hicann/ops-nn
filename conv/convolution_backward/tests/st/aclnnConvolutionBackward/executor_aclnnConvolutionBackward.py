...
class AscendConvolutionBackwardInputApi(BaseApi):
    ...
    def __call__(self, input_data: InputDataset, with_output: bool = False):
        ...
        if not input_data.kwargs["outputMask"][0]:
            grad_input = torch.zeros(input_data.kwargs["input"].shape).to(
                input_data.kwargs["input"].dtype
            )
        if not input_data.kwargs["outputMask"][1]:
            grad_weight = torch.zeros(input_data.kwargs["weight"].shape).to(
                input_data.kwargs["weight"].dtype
            )
        if not input_data.kwargs["outputMask"][2]:
            if self.format == "NDHWC" or self.format == "NHWC":
                channelIndex = len(input_data.kwargs["gradOutput"].shape) - 1
            else:
                channelIndex = 1
            grad_bias = torch.zeros(
                input_data.kwargs["gradOutput"].shape[channelIndex]
            ).to(input_data.kwargs["gradOutput"].dtype)
...
class aclnnAscendConvolutionBackwardInputApi(AclnnBaseApi):
    def __init__(self, task_result: TaskResult, backend):
        super(aclnnAscendConvolutionBackwardInputApi, self).__init__(
            task_result, backend
        )
        self.input = None
    ...
    def after_call(self, output_packages):
        ...
        if not self.input.kwargs["outputMask"][0]:
            grad_input = torch.zeros(self.input.kwargs["input"].shape).to(
                self.input.kwargs["input"].dtype
            )
        if not self.input.kwargs["outputMask"][1]:
            grad_weight = torch.zeros(self.input.kwargs["weight"].shape).to(
                self.input.kwargs["weight"].dtype
            )
        if not self.input.kwargs["outputMask"][2]:
            if (
                self.input.kwargs["format"] == "NDHWC"
                or self.input.kwargs["format"] == "NHWC"
            ):
                channelIndex = len(self.input.kwargs["gradOutput"].shape) - 1
            else:
                channelIndex = 1
            grad_bias = torch.zeros(
                self.input.kwargs["gradOutput"].shape[channelIndex]
            ).to(self.input.kwargs["gradOutput"].dtype)
    ...
    def get_cpp_func_signature_type(self):
        # 校验函数
        return (
            "aclnnStatus aclnnConvolutionBackwardGetWorkspaceSize(const aclTensor *gradOutput, \
