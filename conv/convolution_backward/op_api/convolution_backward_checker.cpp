...
bool CheckFormatValid(const aclTensor* inputTensor, const string& tensorName)
{
    ...
    if (inputDim == CONV1DINPUTDIM) {
        OP_CHECK(inputFormat == op::Format::FORMAT_ND || inputFormat == op::Format::FORMAT_NCL,
                 OP_LOGE_FOR_INVALID_FORMAT(ACLNN_CONVOLUTION_BACKWARD_NAME, tensorName.c_str(), inputFormatStr.c_str(),
                                            "ND or NCL"),
                 return false);
                                  ...
                                  const ConvolutionBackwardParams& params, int64_t dimIdx)
{
    ...
    OP_CHECK(dimInputExpect,
             OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "input, padding",
                 FormatString("%ld,%ld", inputShape.GetDim(dimIdx), (*params.padding)[dimOrder]),
                 FormatString(
                     "(in_dim + pad_dim * 2) should be greater than or equal to "
                     "((weight_shape - 1) * dilation + 1), current in_dim=%d, pad_dim=%d, weight_shape=%d, dilation=%d",
                     inputShape.GetDim(dimIdx), (*params.padding)[dimOrder], weightShape[dimIdx],
                     (*params.dilation)[dimOrder])),
             return false);
    return true;
}

int64_t GetExpectNum(const op::Shape& inputShape, const op::Shape& weightShape, const ConvolutionBackwardParams& params,
                     int64_t dimIdx)
{
    int64_t dimOrder = dimIdx - 2; // 2: the dim N and D
    int64_t filterDimDilation = (weightShape[dimIdx] - 1) * (*params.dilation)[dimOrder] + 1;
...
bool GetExpectValueDHW_95(const ConvolutionBackwardInputTensor& inputTensor, const ConvolutionBackwardParams& params,
                          struct ExpectValue& expectValue, const op::Shape& inputShape, const op::Shape& weightShape)
{
...
bool ConvolutionBackwardChecker::CheckDataTypeValidForGradInput()
{
    ...
    OP_CHECK(outputTensor_.gradInput->GetDataType() == inputTensor_.input->GetDataType(),
             OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "gradInput, input",
...
bool ConvolutionBackwardChecker::CheckDataTypeValidForGradWeight()
{
    ...
    OP_CHECK(outputTensor_.gradWeight->GetDataType() == inputTensor_.weight->GetDataType(),
             OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "gradWeight, weight",
...
bool ConvolutionBackwardChecker::CheckDataTypeValidForGradBias()
{
    ...
    OP_CHECK(outputTensor_.gradBias->GetDataType() == inputTensor_.gradOutput->GetDataType(),
             OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "gradBias, gradOutput",
...
bool ConvolutionBackwardChecker::CheckParamsValidForBpFilter8bit()
{
    ...
    if (params_.transposed) {
        OP_CHECK(CheckParamsValueAllZero(params_.outputPadding),
                 OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(ACLNN_CONVOLUTION_BACKWARD_NAME, "outputPadding",
                                                       ...
                                                       "When transpose is true and the input data type is DT_HIFLOAT8 "
                                                       "or DT_FLOAT8_E4M3FN,the value of outputPadding must be all 0"),
                 return false);
    }
    ...
    OP_CHECK(params_.groups == 1,
             OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "groups", std::to_string(params_.groups).c_str(),
                 FormatString("When outputMask[1] = True and dtype = %s,the value of groups must be 1",
                              op::ToString(inputTensor_.input->GetDataType()))),
             return false);
    return true;
}
...
bool ConvolutionBackwardChecker::InterceptConvFor8bit()
{
    ...
    if ((outputTensor_.gradInput != nullptr) && (outputTensor_.gradWeight != nullptr)) {
        if (IsConv8bit(inputTensor_.gradOutput->GetDataType()) || IsConv8bit(inputTensor_.input->GetDataType()) ||
            ...
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                ACLNN_CONVOLUTION_BACKWARD_NAME, "gradOutput, input, weight, gradInput, gradWeight",
...
bool ConvolutionBackwardChecker::CheckDtypeValidFor8bit(const DataType& dType)
{
    ...
    if (outputTensor_.gradBias != nullptr) {
        OP_CHECK(!is8bitFlag || all8bitFlag,
                 OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "gradOutput, input, weight, gradInput, gradBias",
                     FormatString("%s,%s,%s,%s,%s", inputTensor_.gradOutput->GetDataType(),
                                  inputTensor_.input->GetDataType(), inputTensor_.weight->GetDataType(),
                     ...
                     FormatString("the dtypes of [gradOutput, input, weight, gradInput, gradBias] must be the %s,"
                                  "when any input or output data types is %s",
                                  op::ToString(dType).GetString(), op::ToString(dType).GetString())),
                 return false);
    } else {
        OP_CHECK(
            !is8bitFlag || all8bitFlag,
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                ACLNN_CONVOLUTION_BACKWARD_NAME, "gradOutput, input, weight, gradInput",
                FormatString("%s,%s,%s,%s", inputTensor_.gradOutput->GetDataType(), inputTensor_.input->GetDataType(),
                             inputTensor_.weight->GetDataType(), outputTensor_.gradInput->GetDataType()),
...
bool ConvolutionBackwardChecker::CheckDtypeValidForBpFilter8bit(const DataType& dType)
{
    ...
    if (outputTensor_.gradBias != nullptr) {
        OP_CHECK(!is8bitFlag || all8bitFlag,
                 OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "gradOutput, input, weight, gradWeight, gradBias",
                     FormatString("%s,%s,%s,%s,%s", inputTensor_.gradOutput->GetDataType(),
                                  inputTensor_.input->GetDataType(), inputTensor_.weight->GetDataType(),
        ...
        OP_CHECK(
            !is8bitFlag || all8bitFlag,
            OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(
                ACLNN_CONVOLUTION_BACKWARD_NAME, "gradOutput, input, weight, gradWeight",
                FormatString("%s,%s,%s,%s", inputTensor_.gradOutput->GetDataType(), inputTensor_.input->GetDataType(),
                             inputTensor_.weight->GetDataType(), outputTensor_.gradWeight->GetDataType()),
                FormatString("the dtypes of [gradOutput, input, weight, gradWeight] must be the %s,"
                             "when one of them dtype is %s",
                             op::ToString(dType).GetString(), op::ToString(dType).GetString())),
            return false);
    }
...
bool ConvolutionBackwardChecker::CheckConvParams(size_t inputDim)
{
    ...
    if (params_.transposed) {
        OP_CHECK(CheckParamsValue(params_.outputPadding, true),
                 OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "outputPadding", AclarrayToString(params_.outputPadding).c_str(),
...
bool ConvolutionBackwardChecker::CheckConvShape()
{
    ...
    OP_CHECK(gradOutputDim == inputDim,
             OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "gradOutput, input",
    ...
    OP_CHECK(strideDim == paramsDim,
             OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "stride, input", FormatString("%ld, %ld", strideDim, inputDim),
    ...
    OP_CHECK(dilationDim == paramsDim,
             OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "dilation, input", FormatString("%ld, %ld", dilationDim, inputDim),
    ...
    if (inputDim == CONV2DINPUTDIM) {
        ...
        OP_CHECK(paddingDim == paramsDim || paddingDim == CONV2DINPUTDIM,
                 OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "padding, input", FormatString("%ld,%ld", paddingDim, inputDim),
                     FormatString("the shape dim of [dilation] must be %ld or %ld, when dim of input is %ld", paramsDim,
                                  CONV2DINPUTDIM, inputDim)),
                 return false);

        OP_CHECK(outputPaddingDim == paramsDim || outputPaddingDim == CONV2DINPUTDIM,
                 OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "outputPadding, input",
        ...
        OP_CHECK(paddingDim == paramsDim,
                 OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "padding, input", FormatString("%ld,%ld", paddingDim, inputDim),
                     FormatString("the shape dim of [paddingDim] must be %ld, when dim of input is %ld", paramsDim,
                                  inputDim)),
                 return false);
        ...
        OP_CHECK(outputPaddingDim == paramsDim,
                 OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "outputPadding, input",
                     FormatString("%ld,%ld", outputPaddingDim, inputDim),
                     FormatString("the shape dim of [outputPadding] must be %ld, when dim of input is %ld", paramsDim,
                                  inputDim)),
                 return false);
    }
...
bool ConvolutionBackwardChecker::CheckConvChannelAndGroup()
{
    ...
    OP_CHECK(!channelCheck,
             OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                 ...
                 FormatString("the shape dim of inputShape[%ld] "
                              "must be divisable by the shape dim of weight[%ld]",
                              inputChannelIdx, weightCinIdx)),
             return false);
    ...
    OP_CHECK(groupCheck,
             OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                 ...
                 FormatString("inputShape[%ld], weight[%ld]", inputChannelIdx, weightCinIdx),
                 FormatString("%ld,%ld", inputShape.GetDim(inputChannelIdx), weightShape.GetDim(weightCinIdx)),
                 FormatString("The shape dim of inputShape[%ld] must be equal to (the shape dim of weight[%ld] times "
                              "the value of groups)",
                              inputChannelIdx, weightCinIdx)),
             return false);
...
aclnnStatus ConvolutionBackwardChecker::CheckParams()
{
    ...
    if (!params_.transposed && outputTensor_.gradBias != nullptr) {
        if (curArch == NpuArch::DAV_2201 || Ops::NN::AclnnUtil::IsRegbase(curArch)) {
            OP_CHECK(
                outputTensor_.gradBias->GetStorageFormat() == op::Format::FORMAT_ND,
    ...
    if ((inputTensor_.input->Size() == 0 || inputTensor_.weight->Size() == 0) &&
        (curArch == NpuArch::DAV_2201 || Ops::NN::AclnnUtil::IsRegbase(curArch))) {
        return ACLNN_SUCCESS;
    }
...
bool ConvolutionBackwardChecker::CheckParamsDim()
{
    ...
    auto validDim = [inputDim, paramsDim](bool condition, const char* paramName) -> bool {
        ...
        OP_CHECK(condition,
                 OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(ACLNN_CONVOLUTION_BACKWARD_NAME,
                                                           FormatString("%s,input", paramNameStr),
                                                           FormatString("%ld, %ld", paramsDim, inputDim),
                                                           FormatString("the shape dim of %s must be %ld,"
                                                                        ". when the input dim of input is %ld",
                                                                        paramNameStr, paramsDim, inputDim)),
                 return false);
        return true;
    };
    bool checkRes = validDim(params_.stride->Size() == paramsDim, "stride") &&
                    ...
                    validDim(params_.padding->Size() == paramsDim ||
                                 (inputDim == CONV2DINPUTDIM && params_.padding->Size() == inputDim),
                             "padding") &&
                    validDim(params_.outputPadding->Size() == paramsDim ||
                                 (inputDim == CONV2DINPUTDIM && params_.outputPadding->Size() == inputDim),
                             "outputPadding");
    return checkRes;
}
...
bool ConvolutionBackwardChecker::CheckParamsGroup()
{
    ...
    OP_CHECK((params_.groups != 0 && weightCout % params_.groups == 0),
             OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "weight[0]", FormatString("%ld", weightCout),
    ...
    if (inputChannel != 0 || weightCin != 0) {
        bool channelCheck = (weightCin == 0 || inputChannel % weightCin != 0 ||
                             inputChannel / weightCin != params_.groups);
        OP_CHECK(!channelCheck,
                 OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "input[1], weight[1]",
...
bool ConvolutionBackwardChecker::CheckShape()
{
    ...
    for (size_t i = 2; i < inputDim; i++) {
        ...
        OP_CHECK(
            dimInput >= 0,
            OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                ACLNN_CONVOLUTION_BACKWARD_NAME, FormatString("input[%ld]", i),
                (std::to_string(inputShape.GetDim(i))).c_str(),
                FormatString(
                    "(in_dim + pad_dim * 2) should >= ((weight_shape - 1) * dilation + 1) when at dimension %ld,"
...
bool ConvolutionBackwardChecker::CheckShapeTransposed()
{
    ...
    OP_CHECK(weightCo == inputC,
             OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
                 ACLNN_CONVOLUTION_BACKWARD_NAME, "inputC, weightCo",
        ...
        if (!params_.transposed) {
            OP_CHECK((*params_.biasSizes)[0] == Cout,
                     OP_LOGE_WITH_INVALID_ATTR(ACLNN_CONVOLUTION_BACKWARD_NAME, "biasSizes",
                                               std::to_string((*params_.biasSizes)[0]).c_str(),
                                               std::to_string(Cout).c_str()),
                     return false);
        } else {
    ...
    if ((*params_.outputMask)[0]) {
        ...
        OP_CHECK(inputShape.GetDimNum() > 2,
                 OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(ACLNN_CONVOLUTION_BACKWARD_NAME, "inputShape",
                                                          ...
                                                          "Dim of inputshape must be greater than 2"),
                 return false);

        OP_CHECK(inputShape[0] == 0 || inputShape[1] == 0,
                 OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                     ACLNN_CONVOLUTION_BACKWARD_NAME, "inputShape[0],inputShape[1]",
