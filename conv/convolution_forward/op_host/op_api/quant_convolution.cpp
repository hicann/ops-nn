...
aclIntArray* ConstructQuantConvNewPad(const aclIntArray* padding, aclOpExecutor* executor)
{
    ...
    if (padding->Size() == QUANT_CONV2D_PAD_DIM_4) {
        newPad = {(*padding)[PAD_4D_TOP_INDEX], (*padding)[PAD_4D_BOTTOM_INDEX], (*padding)[PAD_4D_LEFT_INDEX],
                  (*padding)[PAD_4D_RIGHT_INDEX]};
    } else if (padding->Size() == QUANT_CONV2D_PAD_DIM_2) {
        newPad = {(*padding)[PAD_2D_H_INDEX], (*padding)[PAD_2D_H_INDEX], (*padding)[PAD_2D_W_INDEX],
                  (*padding)[PAD_2D_W_INDEX]};
    } else if (padding->Size() == QUANT_CONV3D_PAD_DIM_6) {
        newPad = {(*padding)[PAD_6D_HEAD_INDEX],   (*padding)[PAD_6D_TAIL_INDEX], (*padding)[PAD_6D_TOP_INDEX],
                  (*padding)[PAD_6D_BOTTOM_INDEX], (*padding)[PAD_6D_LEFT_INDEX], (*padding)[PAD_6D_RIGHT_INDEX]};
    } else if (padding->Size() == QUANT_CONV3D_PAD_DIM_3) {
        newPad = {(*padding)[PAD_3D_D_INDEX], (*padding)[PAD_3D_D_INDEX], (*padding)[PAD_3D_H_INDEX],
                  (*padding)[PAD_3D_H_INDEX], (*padding)[PAD_3D_W_INDEX], (*padding)[PAD_3D_W_INDEX]};
    } else {
        newPad = {0};
    }
                                       ...
                                       aclTensor*& output, aclOpExecutor* executor)
{
    ...
    auto ret = INFER_SHAPE(ExtendConv2D,
                           ...
                           OP_ATTR(stride4, pad4, dilation4, groups, dataFormat, offsetx, roundMode, tmpPadMode,
                                   tmpEnableHf32, tmpEnableRelu0, tmpEnableRelu1, tmpDualOutput, tmpDtype0, tmpDtype1));
    ...
    ADD_TO_LAUNCHER_LIST_AICORE(
        ExtendConv2D, OP_INPUT(input, weight, bias, nullptr, scale, nullptr, nullptr, nullptr, nullptr, nullptr),
        OP_OUTPUT(output, output),
                                      ...
                                      aclTensor*& output, aclOpExecutor* executor)
{
    ...
    ADD_TO_LAUNCHER_LIST_AICORE(
        QuantConv3D, OP_INPUT(input, weight, scale, bias, nullptr), OP_OUTPUT(output),
        OP_ATTR(0, stride5, pad6, dilation5, groups, dataFormat, offsetx, roundMode, tmpPadMode));
                                  ...
                                  const char* roundMode, aclOpExecutor* executor)
{
    ...
    if (extendConvOut == nullptr) {
        OP_LOGE(ACLNN_ERR_INNER, "AllocTensor failed.");
        return nullptr;
    }
