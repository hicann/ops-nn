...
public:
    ...
    __aicore__ inline void Init(GM_ADDR grad, GM_ADDR x, GM_ADDR offsets, GM_ADDR grad_x, GM_ADDR grad_offsets,
...
template <typename T, typename T1>
__aicore__ inline void DeformableOffsetGrad<T, T1>::Init(GM_ADDR grad, GM_ADDR x, GM_ADDR offsets, GM_ADDR grad_x,
                                                         GM_ADDR grad_offsets,
                                                         const DeformableOffsetsGradTilingData* __restrict tilingData)
{
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline void ComputeForGetFloorValueDeformableGrad(
    __gm__ T* gradGmAddr, T1 c, T1 gradPrevIdx, float mask, float& gradOffsetsValueH, float vRow1, float Row1_2,
    float& gradOffsetsValueW, float vRow2, float Row2_2, float& gradOffsetsValueMask, float w, float vRow3)
{
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline void BilinearInterpolate(
    ...
    __gm__ T* yGradOffsetsGmAddr, T1 gradPrevIdx, float mask, T1 inputXIdx, T1 yGradXIdx, T1 gradOffsetsHIdx,
    T1 gradOffsetsWIdx, T1 gradOffsetsMaskIdx, float posH, float posW, T1 channelPerGroup, T1 inW_, T1 inChannel_,
    T1 inH_)
{
    ...
    if (lowH >= 0 && lowW >= 0) {
        ...
        for (T1 c = 0; c < channelPerGroup; c++) {
            ...
            ComputeForGetFloorValueDeformableGrad<T, T1>(gradGmAddr, c, gradPrevIdx, mask, gradOffsetsValueH, -v1, hw,
                                                         gradOffsetsValueW, -v1, hh, gradOffsetsValueMask, w1, v1);
        }
    }
    ...
    if (lowH >= 0 && HighW < inW_) {
        ...
        for (T1 c = 0; c < channelPerGroup; c++) {
            ...
            ComputeForGetFloorValueDeformableGrad<T, T1>(gradGmAddr, c, gradPrevIdx, mask, gradOffsetsValueH, -v2, lw,
                                                         gradOffsetsValueW, v2, hh, gradOffsetsValueMask, w2, v2);
        }
    }
    ...
    if (HighH < inH_ && lowW >= 0) {
        ...
        for (T1 c = 0; c < channelPerGroup; c++) {
            ...
            ComputeForGetFloorValueDeformableGrad<T, T1>(gradGmAddr, c, gradPrevIdx, mask, gradOffsetsValueH, v3, hw,
                                                         gradOffsetsValueW, -v3, lh, gradOffsetsValueMask, w3, v3);
        }
    }
    ...
    if (HighH < inH_ && HighW < inW_) {
        ...
        for (T1 c = 0; c < channelPerGroup; c++) {
            ...
            ComputeForGetFloorValueDeformableGrad<T, T1>(gradGmAddr, c, gradPrevIdx, mask, gradOffsetsValueH, v4, lw,
                                                         gradOffsetsValueW, v4, lh, gradOffsetsValueMask, w4, v4);
        }
    }
...
__simt_callee__ __aicore__ __attribute__((always_inline)) inline void DoComputePerGroup(
    __gm__ T* gradGmAddr, __gm__ T* inputXGmAddr, __gm__ T* offsetsGmAddr, __gm__ T* yGradXGmAddr,
    __gm__ T* yGradOffsetsGmAddr, T1 gradIdx, T1 inputXIdx, T1 offsetsIdx, T1 yGradXIdx, T1 yGradOffsetsIdx,
    T1 channelPerGroup, T1 iHo, T1 iWo, T1 iKh, T1 iKw, T1 outW_, T1 inW_, T1 inChannel_, T1 strideH_, T1 strideW_,
    T1 dilationH_, T1 dilationW_, T1 padsH_, T1 padsW_, T1 kSizeH_, T1 kSizeW_, T1 batchSize_, T1 deformableGroup_,
    T1 offsetsChannel_, T1 inH_)
{
    ...
    if (originOffsetH > -1 && originOffsetH < inH_ && originOffsetW > -1 && originOffsetW < inW_) {
        T1 gradYPixelIdx = gradIdx +
                           inChannel_ * (iHo * kSizeH_ * outW_ * kSizeW_ + iKh * outW_ * kSizeW_ + iWo * kSizeW_ + iKw);
        BilinearInterpolate<T, T1>((__gm__ T*)gradGmAddr, (__gm__ T*)inputXGmAddr, (__gm__ T*)offsetsGmAddr,
                                   (__gm__ T*)yGradXGmAddr, (__gm__ T*)yGradOffsetsGmAddr, gradYPixelIdx, mask,
                                   inputXIdx, yGradXIdx, gradOffsetsHIdx, gradOffsetsWIdx, gradOffsetsMaskIdx,
                                   originOffsetH, originOffsetW, channelPerGroup, inW_, inChannel_, inH_);
    } else {
...
__simt_vf__ LAUNCH_BOUND(MAX_THREAD_NUM) __aicore__
    void ComputeSetValueGradX(__gm__ T* gradGmAddr, __gm__ T* inputXGmAddr, __gm__ T* offsetsGmAddr,
                              __gm__ T* yGradXGmAddr, __gm__ T* yGradOffsetsGmAddr, T1 blockClearProcessNum,
                              T1 blockClearStartOffset)
{
    for (T1 idx = static_cast<T1>(threadIdx.x); idx < blockClearProcessNum; idx += static_cast<T1>(blockDim.x)) {
...
template <typename T, typename T1>
__simt_vf__ LAUNCH_BOUND(MAX_THREAD_NUM) __aicore__
    void ComputeDeformableOffsetGrad(__gm__ T* gradGmAddr, __gm__ T* inputXGmAddr, __gm__ T* offsetsGmAddr,
                                     __gm__ T* yGradXGmAddr, __gm__ T* yGradOffsetsGmAddr, T1 blockProcessNum,
                                     T1 blockStartOffset, T1 kSizeH_, T1 kSizeW_, T1 deformableGroup_,
                                     T1 offsetsChannel_, T1 outH_, T1 outW_, T1 inH_, T1 inW_, T1 inChannel_,
                                     T1 strideH_, T1 strideW_, T1 dilationH_, T1 dilationW_, T1 padsH_, T1 padsW_,
                                     T1 batchSize_)
{
    T1 channelPerGroup = inChannel_ / deformableGroup_;
    for (T1 idx = static_cast<T1>(threadIdx.x); idx < blockProcessNum; idx += static_cast<T1>(blockDim.x)) {
        ...
        DoComputePerGroup<T, T1>((__gm__ T*)gradGmAddr, (__gm__ T*)inputXGmAddr, (__gm__ T*)offsetsGmAddr,
                                 (__gm__ T*)yGradXGmAddr, (__gm__ T*)yGradOffsetsGmAddr, gradIdx, inputXIdx, offsetsIdx,
                                 yGradXIdx, yGradOffsetsIdx, channelPerGroup, iHo, iWo, iKh, iKw, outW_, inW_,
                                 inChannel_, strideH_, strideW_, dilationH_, dilationW_, padsH_, padsW_, kSizeH_,
                                 kSizeW_, batchSize_, deformableGroup_, offsetsChannel_, inH_);
    }
}
...
__aicore__ inline void DeformableOffsetGrad<T, T1>::Process()
{
    ...
    T1 offsetsChannel_ = tilingData_->deformableGroups * OFFSET_DIM_VALUE * tilingData_->dimKHeight *
                         tilingData_->dimKWidth;
