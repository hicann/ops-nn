...
static __aicore__ inline void InitHoDirectionValue(Intf* self)
{
    ...
    self->ctx.hoL0Tail = self->ctx.currentHoL1 % self->ctx.convTilingData->hoL0;
    self->ctx.hoL0Tail = self->ctx.hoL0Tail == 0 ? self->ctx.convTilingData->hoL0 : self->ctx.hoL0Tail;
...
static __aicore__ inline void InitWoDirectionValue(Intf* self)
{
    ...
    self->ctx.woL0Tail = self->ctx.currentWoL1 % self->ctx.convTilingData->woL0;
    self->ctx.woL0Tail = self->ctx.woL0Tail == 0 ? self->ctx.convTilingData->woL0 : self->ctx.woL0Tail;
    ...
    if constexpr (Intf::hasWL1IterFlag) {
        self->ctx.maxWoL1Iter = CeilDiv(self->ctx.singleCoreWo, self->ctx.convTilingData->woL1) - 1;
    }

    if constexpr (!Intf::isDmaFlag) {
        if (Intf::hasWL0IterFlag && self->ctx.convTilingData->hoL0 > 1 && self->ctx.woAL1Tail % BLOCK_L0_N > 0 &&
...
__aicore__ inline void CalcWoDirectionVar(Intf* self)
{
    if constexpr (Intf::hasWL1IterFlag) {
        if (self->ctx.woL1SmallTail > 0) {
            ...
            self->ctx.currentWoL1 = self->ctx.woAL1Iter == self->ctx.maxWoL1Iter ? self->ctx.woAL1Tail :
    ...
    if constexpr (Intf::hasWL0IterFlag) {
        self->ctx.woL0Tail = self->ctx.currentWoL1 % self->ctx.convTilingData->woL0;
        self->ctx.woL0Tail = self->ctx.woL0Tail == 0 ? self->ctx.convTilingData->woL0 : self->ctx.woL0Tail;
        self->ctx.maxWoL0Iter = CeilDiv(self->ctx.currentWoL1, self->ctx.convTilingData->woL0) - 1;
...
__aicore__ inline void CalcHoDirectionVar(Intf* self)
{
    ...
    if constexpr (Intf::hasHL0IterFlag) {
        self->ctx.hoL0Tail = self->ctx.currentHoL1 % self->ctx.convTilingData->hoL0;
        self->ctx.hoL0Tail = self->ctx.hoL0Tail == 0 ? self->ctx.convTilingData->hoL0 : self->ctx.hoL0Tail;
        self->ctx.maxHoL0Iter = CeilDiv(self->ctx.currentHoL1, self->ctx.convTilingData->hoL0) - 1;
