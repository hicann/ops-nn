...
void ConvTilingAlgorithmHWmode::InitPingPong()
{
    ...
    if (tilingIns_->isKernelSplit) {
        ...
        inputTiling.khL1 = tilingIns_->shapeInfo.singlekH;
        inputTiling.kwL1 = tilingIns_->shapeInfo.singlekW;
    }
    inputTiling.kBL1 = inputTiling.khL1 * inputTiling.kwL1 * tilingIns_->cubeInfo.k0;
...
void ConvTilingAlgorithmHWmode::GetKL0Tiling()
{
    if (tilingIns_->isDmaFlag) {
        ...
        uint64_t khkw = tilingIns_->isKernelSplit ? l1Params.khL1 * l1Params.kwL1 :
                                                    tilingIns_->shapeInfo.singlekH * tilingIns_->shapeInfo.singlekW;
        uint64_t kAL1WithKhKw = l1Params.kAL1 * khkw;
    ...
    bool bMNFullLoadRet = static_cast<uint64_t>(tilingIns_->shapeInfo.singleHo) <= l0Params.hoL0 &&
                          ...
                          tilingIns_->shapeInfo.singleCo1 * static_cast<uint64_t>(tilingIns_->cubeInfo.n0) <=
                              l0Params.nL0;
    bool bKFullLoadRet = l1Params.kBL1 == this->l1TilingCalc.kBL1MaxSize &&
                         l1Params.kAL1 == this->l1TilingCalc.kAL1MaxSize;
    bool bCheckL0SizeRet = (CalcAL0Size(l0Params.hoL0 * l0Params.woL0, this->l1TilingCalc.kBL1MaxSize) /
                                this->dbValue.pbAL0 <=
...
uint64_t ConvTilingAlgorithmHWmode::IterKwL1()
{
    uint32_t kwL1Idx = 0;
    L1TilingParams inputTiling;
...
uint64_t ConvTilingAlgorithmHWmode::IterKhL1(uint64_t kwL1Res)
{
    uint32_t khL1Idx = 0;
    L1TilingParams inputTiling;
    bool isExceedL1Size = false;
    ...
    inputTiling.woAL1 = l0Params.woL0;
    inputTiling.nBL1 = l0Params.nL0;
    inputTiling.kwL1 = kwL1Res;
    while (khL1Idx < this->l1TilingRange.khL1Range.size()) {
        curKhL1Value = this->l1TilingRange.khL1Range[khL1Idx];
        inputTiling.kAL1 = tilingIns_->cubeInfo.k0 * kwL1Res * curKhL1Value;
        inputTiling.kBL1 = inputTiling.kAL1;
        inputTiling.khL1 = curKhL1Value;
        isExceedL1Size = CalcCurL1Size(inputTiling, inputTiling.nBL1) > tilingIns_->platformInfo.l1Size;
        if (khL1Idx > 0 && isExceedL1Size) {
            curKhL1Value = this->l1TilingRange.khL1Range[khL1Idx - 1];
            break;
        }
        if (khL1Idx == 0 && isExceedL1Size) {
            return 0;
...
void ConvTilingAlgorithmHWmode::GetKhKwL1Tiling()
{
    if (!tilingIns_->isDmaFlag && !tilingIns_->isKernelSplit) {
        return;
    }
...
{
    uint64_t mL0Max = min(
        tilingIns_->platformInfo.l0ASize / (tilingIns_->cubeInfo.k0 * this->dbValue.pbAL0 * fMapDTypeSize),
        tilingIns_->platformInfo.l0CSize /
            (tilingIns_->cubeInfo.n0 * this->dbValue.pbCL0 * DTYPE_SIZE_TAB.at(tilingIns_->cubeInfo.madType)));
    // Get hoL0 range
    ...
    // Get nL0 range
    uint64_t nL0Max = min(
        tilingIns_->platformInfo.l0BSize / (tilingIns_->cubeInfo.k0 * this->dbValue.pbBL0 * weightDTypeSize),
        tilingIns_->platformInfo.l0CSize /
            (tilingIns_->cubeInfo.m0 * this->dbValue.pbCL0 * DTYPE_SIZE_TAB.at(tilingIns_->cubeInfo.madType)));
...
uint64_t ConvTilingAlgorithmHWmode::CalcL1SizeForL0Tiling(uint64_t currHoL0, uint64_t currWoL0, uint64_t currNL0)
{
    ...
    uint64_t wiL1 = 0;
    uint64_t hiL1 = 0;
    uint64_t tmpKhL1 = tilingIns_->isKernelSplit ? 1 : tilingIns_->shapeInfo.orgkH; // load3d split kh/kw
    uint64_t tmpKwL1 = tilingIns_->isKernelSplit ? 1 : tilingIns_->shapeInfo.orgkW;
    if (tilingIns_->isDmaFlag) {
        aL1Size = tilingIns_->cubeInfo.k0 * currWoL0 * currHoL0 * this->fMapDTypeSize;
    } else {
        wiL1 = InferWiL1(currWoL0, tmpKwL1, tilingIns_->shapeInfo.orgWi);
        hiL1 = InferHiL1(currHoL0, tmpKhL1, tilingIns_->shapeInfo.orgHi);
        aL1Size = tilingIns_->cubeInfo.k0 * wiL1 * hiL1 * this->fMapDTypeSize;
    }

    uint64_t bL1Size = tilingIns_->isDmaFlag ?
                           tilingIns_->cubeInfo.k0 * currNL0 * this->weightDTypeSize :
                           tmpKhL1 * tmpKwL1 * tilingIns_->cubeInfo.k0 * currNL0 * this->weightDTypeSize;
...
void ConvTilingAlgorithmHWmode::SetL1TilingRes()
{
    ...
    if (tilingIns_->isC04Flag) {
        ...
        if (tilingIns_->isDmaFlag) {
            ...
            uint64_t khkw = tilingIns_->isKernelSplit ? this->l1Params.khL1 * this->l1Params.kwL1 :
                                                        tilingIns_->shapeInfo.singlekH * tilingIns_->shapeInfo.singlekW;
            tilingIns_->l1TilingInfo.kAL1 = l1Params.kAL1 * khkw;
        }
    }
    ...
    tilingIns_->l1TilingInfo.khL1 = this->l1Params.khL1;
    tilingIns_->l1TilingInfo.kwL1 = this->l1Params.kwL1;
...
void ConvTilingAlgorithmHWmode::L0TilingRest(bool kFullLoadFlag)
{
    ...
    bool resetFlag = this->dbValue.pbAL0 == DOUBLE_BUFFER_NUM && this->dbValue.pbBL0 == DOUBLE_BUFFER_NUM &&
                     kFullLoadFlag;
    if (!resetFlag) {
        return;
    }
...
void ConvTilingAlgorithmHWmode::CheckL0DoubleBuffer()
{
    ...
    if (static_cast<uint64_t>(tilingIns_->shapeInfo.singleHo) <= tilingIns_->l0TilingInfo.hoL0 &&
        static_cast<uint64_t>(tilingIns_->shapeInfo.singleWo) <= tilingIns_->l0TilingInfo.woL0 && kFullLoadFlag) {
        this->dbValue.pbAL0 = SINGLE_BUFFER_NUM;
    }

    if (kFullLoadFlag && tilingIns_->shapeInfo.singleCo1 * tilingIns_->cubeInfo.n0 == tilingIns_->l0TilingInfo.nL0) {
        this->dbValue.pbBL0 = SINGLE_BUFFER_NUM;
    }
...
bool ConvTilingAlgorithmHWmode::CheckHinOverLoad3dv2Limit(uint64_t hout) const
{
    ...
    uint64_t kh = tilingIns_->isKernelSplit ? this->l1Params.khL1 : tilingIns_->shapeInfo.orgkH;
    uint64_t hiAL1 = InferHiL1(hout, kh, tilingIns_->shapeInfo.orgHi);
...
bool ConvTilingAlgorithmHWmode::CheckWinOverLoad3dv2Limit(uint64_t wout) const
{
    ...
    uint64_t kw = tilingIns_->isKernelSplit ? this->l1Params.kwL1 : tilingIns_->shapeInfo.orgkW;
    uint64_t wiAL1 = static_cast<uint64_t>(InferWiL1(wout, kw, tilingIns_->shapeInfo.orgWi));
...
void ConvTilingAlgorithmHWmode::GetL1TilingRange()
{
    if (!tilingIns_->isC04Flag) {
        ...
        if (tilingIns_->isDmaFlag) {
            VectorElementMultip(this->l1TilingRange.kAL1Range, this->l1Params.kwL1 * this->l1Params.khL1);
        }
        uint64_t khkw = tilingIns_->isKernelSplit ? this->l1Params.khL1 * this->l1Params.kwL1 :
                                                    tilingIns_->shapeInfo.singlekH * tilingIns_->shapeInfo.singlekW;
        if (!tilingIns_->isDmaFlag) {
    ...
    if (tilingIns_->isC04Flag && this->l1Flags.isWoL1MustFullLoadFlagC04) {
        ...
        uint64_t multiWoL1Max = CeilDiv(
            CeilDiv(tilingIns_->shapeInfo.singleWo, tilingIns_->cubeInfo.m0) * tilingIns_->cubeInfo.m0, l0Params.woL0);
...
{
    if (tilingIns_->isDmaFlag && this->l1Flags.isDMAKernelSplit) {
        return false;
    }
    if (tilingIns_->isKernelSplit) {
        return false;
...
bool ConvTilingAlgorithmHWmode::CheckHoWoL1FullLoad() const
{
    ...
    uint64_t tmpKh = tilingIns_->isKernelSplit ? l1Params.khL1 : tilingIns_->shapeInfo.singlekH;
    uint64_t tmpKw = tilingIns_->isKernelSplit ? l1Params.kwL1 : tilingIns_->shapeInfo.singlekW;
    uint64_t hiAL1Full = InferHiL1(hoAL1Full, tmpKh, tilingIns_->shapeInfo.orgHi);
    uint64_t wiAL1Full = InferWiL1(woAL1Full, tmpKw, tilingIns_->shapeInfo.orgWi);
    if (hiAL1Full > LOAD3D_HW_SIZE_LIMIT || wiAL1Full > LOAD3D_HW_SIZE_LIMIT) {
        return false;
    }
    return true;
}
...
{
    if (tilingIns_->isDmaFlag &&
        (this->l1TilingCalc.ubMaxSize > tilingIns_->platformInfo.ubSize || this->l1Flags.isDMAKernelSplit)) {
        return false;
    }

    if (tilingIns_->isKernelSplit) {
        return false; // if kernel is split, hiL1 and wiL1 must be not full load
    }
...
bool ConvTilingAlgorithmHWmode::IsBL1FullLoadFirst()
{
    return tilingIns_->platformInfo.abL1mte2BandWidthCof * this->l1TilingCalc.bL1MaxSize +
               this->l1TilingCalc.biasL1MaxSize + this->l1TilingCalc.fixpParamsL1MaxSize >=
...
void ConvTilingAlgorithmHWmode::RestrictRange(const std::vector<uint64_t>& inputRange,
                                              std::vector<uint64_t>& strictRange, uint64_t restriction) const
{
...
uint64_t ConvTilingAlgorithmHWmode::CalcCurL1Size(const L1TilingParams& inputTiling, uint64_t biasFixpParamNB)
{
    ...
    uint64_t tmpKh = tilingIns_->isKernelSplit ? inputTiling.khL1 : tilingIns_->shapeInfo.singlekH;
    uint64_t tmpKw = tilingIns_->isKernelSplit ? inputTiling.kwL1 : tilingIns_->shapeInfo.singlekW;
    auto hiAL1 = InferHiL1(inputTiling.hoAL1, tmpKh, tilingIns_->shapeInfo.orgHi);
    auto wiAL1 = InferWiL1(inputTiling.woAL1, tmpKw, tilingIns_->shapeInfo.orgWi);
    uint64_t feaMapSize = 0;
    uint64_t weightSize = 0;
    if (tilingIns_->isC04Flag) {
        feaMapSize = AlignB(hiAL1 * wiAL1, C0_SIZE / (fMapDTypeSize * C04_CIN_SIZE)) * this->dbValue.pbAL1 *
                     C04_CIN_SIZE * fMapDTypeSize;
        weightSize = inputTiling.kBL1 * inputTiling.nBL1 * this->dbValue.pbBL1 * weightDTypeSize;
    } else {
        if (tilingIns_->isDmaFlag) {
            feaMapSize = AlignB(
                inputTiling.hoAL1 * inputTiling.woAL1 * inputTiling.kAL1 * fMapDTypeSize * this->dbValue.pbAL1,
                C0_SIZE);
        } else {
...
void ConvTilingAlgorithmHWmode::IterKAL1(uint64_t& kAL1, uint64_t kBL1, uint64_t hoAL1, uint64_t woAL1, uint64_t nBL1)
{
    ...
    inputTiling.khL1 = this->l1Params.khL1;
    inputTiling.kwL1 = this->l1Params.kwL1;
...
void ConvTilingAlgorithmHWmode::IterKABL1(uint64_t& tmpKAL1, uint64_t& tmpKBL1, uint64_t tmpHoAL1, uint64_t tmpWoAL1,
                                          uint64_t tmpNBL1)
{
    if (tilingIns_->isC04Flag) {
    ...
    while (kAL1Idx < l1TilingRange.kAL1StrictRange.size() && kBL1Idx < l1TilingRange.kBL1StrictRange.size()) {
                                                  ...
                                                  uint64_t& tmpWoAL1, uint64_t& tmpNBL1)
{
    if (CheckKL1FullLoad(l1TilingRange.kAL1StrictRange[0], l1TilingCalc.kBL1MaxSize)) {
        ...
        IterKBL1(tmpKAL1, tmpKBL1, l1TilingRange.hoAL1StrictRange[0], l1TilingRange.woAL1StrictRange[0],
        ...
        tmpWoAL1 = tilingIns_->isC04Flag && l1Flags.isWoL1MustFullLoadFlagC04 ?
                       AlignB(tilingIns_->shapeInfo.singleWo, tilingIns_->cubeInfo.m0) :
                       l0Params.woL0;
        tmpNBL1 = l0Params.nL0;
...
void ConvTilingAlgorithmHWmode::CoreL1TilingNoneFullLoadDecision()
{
    ...
    L1TilingParams tmpL1Params = {
        tmpKAL1, tmpKBL1, tmpHoAL1, tmpWoAL1, tmpNBL1, this->l1Params.khL1, this->l1Params.kwL1};
    ...
    l1Params = {tmpKAL1, tmpKBL1, tmpHoAL1, tmpWoAL1, tmpNBL1, this->l1Params.khL1, this->l1Params.kwL1};
...
void ConvTilingAlgorithmHWmode::UpdateAL1DoubleBufferSecond()
{
    bool kAl1FullLoadFlag = tilingIns_->isC04Flag                           ? true :
                            l1Params.kAL1 == this->l1TilingCalc.kAL1MaxSize ? true :
                                                                              false;
    if (tilingIns_->isKernelSplit) {
        kAl1FullLoadFlag = false; // kh/kw split -> hiL1/wiL1 split
    }
    if (static_cast<uint64_t>(tilingIns_->shapeInfo.singleHo) <= l1Params.hoAL1 &&
        static_cast<uint64_t>(tilingIns_->shapeInfo.singleWo) <= l1Params.woAL1 && kAl1FullLoadFlag) {
        this->dbValue.pbAL1 = SINGLE_BUFFER_NUM;
        return;
    }
}
...
void ConvTilingAlgorithmHWmode::CoreL1TilingDecision()
{
    if (l1Flags.abL1Mode == L1TilingMode::ALL_FULL_LOAD) {
        l1Params = {l1TilingInit.kAL1Init, l1TilingInit.kBL1Init, l1TilingInit.hoAL1Init, l1TilingInit.woAL1Init,
                    l1TilingInit.nBL1Init, this->l1Params.khL1,   this->l1Params.kwL1};
        l1Flags.iterateMNOrder = IterateMNOrder::ITER_M_FST;
        l1Flags.isBiasFullLoad = true;
        l1Flags.isFixpParamsFullLoad = true;
        return;
    ...
    if (l1Flags.abL1Mode == L1TilingMode::FULL_LOAD_BL1) {
        ...
        std::vector<uint64_t> inputHoAL1Range = l1TilingRange.hoAL1StrictRange.empty() ?
                                                    std::vector<uint64_t>{l1TilingInit.hoAL1Init} :
                                                    l1TilingRange.hoAL1StrictRange;
        std::vector<uint64_t> inputWoAL1Range = l1TilingRange.woAL1StrictRange.empty() ?
                                                    std::vector<uint64_t>{l1TilingInit.woAL1Init} :
                                                    l1TilingRange.woAL1StrictRange;
        ...
        l1Params = {tmpKAL1,
                    l1TilingInit.kBL1Init,
                    tmpHoAL1,
                    tmpWoAL1,
                    l1TilingInit.nBL1Init,
                    this->l1Params.khL1,
                    this->l1Params.kwL1};
        l1Flags.iterateMNOrder = IterateMNOrder::ITER_M_FST;
        l1Flags.isBiasFullLoad = true;
    ...
    if (l1Flags.abL1Mode == L1TilingMode::FULL_LOAD_AL1) {
        ...
        l1Params = {l1TilingInit.kAL1Init,  tmpKBL1, l1TilingInit.hoAL1Init,
                    l1TilingInit.woAL1Init, tmpNBL1, this->l1Params.khL1,
                    this->l1Params.kwL1};
        l1Flags.iterateMNOrder = IterateMNOrder::ITER_N_FST;
        l1Flags.isBiasFullLoad = false;
        l1Flags.isFixpParamsFullLoad = false;
...
{
    uint64_t kAL1Spilt = tilingIns_->isC04Flag ? this->l1TilingCalc.kAL1MaxSize :
                         tilingIns_->isDmaFlag ? this->l1TilingCalc.ci0KhL1KwL1 :
                                                 tilingIns_->cubeInfo.k0;
    uint64_t kBL1Spilt = tilingIns_->isC04Flag ? this->l1TilingCalc.kBL1MaxSize :
                         tilingIns_->isDmaFlag ? this->l1TilingCalc.ci0KhL1KwL1 :
...
{
    uint64_t tmpkh = tilingIns_->isKernelSplit ? this->l1Params.khL1 : tilingIns_->shapeInfo.singlekH;
    uint64_t tmpkw = tilingIns_->isKernelSplit ? this->l1Params.kwL1 : tilingIns_->shapeInfo.singlekW;
    ...
    this->l1TilingCalc.ci0HkWk = tmpkh * tmpkw * tilingIns_->cubeInfo.k0;
    this->l1TilingCalc.ci0KhL1KwL1 = tilingIns_->cubeInfo.k0 * this->l1Params.khL1 * this->l1Params.kwL1;
    ...
    this->l1TilingCalc.bL1MaxSize = this->l1TilingCalc.kBL1MaxSize * tilingIns_->shapeInfo.singleCo1 *
                                    tilingIns_->cubeInfo.n0 * weightDTypeSize;
    this->l1TilingCalc.biasL1MaxSize = tilingIns_->hasBias ?
                                           tilingIns_->shapeInfo.singleCo1 * tilingIns_->cubeInfo.n0 * biasDTypeSize :
                                           0;
    ...
    if (tilingIns_->isDmaFlag) {
        ...
        this->l1TilingCalc.ubMaxSize = tilingIns_->shapeInfo.singleHo * woAL1Max * tilingIns_->cubeInfo.k0 *
                                       fMapDTypeSize;
        this->l1TilingCalc.bL1Minsize = this->l1TilingCalc.ci0KhL1KwL1 * l0Params.nL0 * weightDTypeSize *
                                        this->dbValue.pbBL1;
    } else {
        auto tmpSingleHi = InferHiL1(tilingIns_->shapeInfo.singleHo, tmpkh, tilingIns_->shapeInfo.orgHi);
        auto tmpSingleWi = InferWiL1(AlignB(tilingIns_->shapeInfo.singleWo, tilingIns_->cubeInfo.m0), tmpkw,
        ...
        this->l1TilingCalc.wiAL1Min = InferWiL1(this->l1TilingCalc.woAL1Min, tmpkw, tilingIns_->shapeInfo.orgWi);
        this->l1TilingCalc.hiAL1Min = InferHiL1(this->l1TilingCalc.hoAL1Min, tmpkh, tilingIns_->shapeInfo.orgHi);
...
void ConvTilingAlgorithmHWmode::InitCalcL1ParamsC04Mode()
{
    ...
    uint64_t tmpkw = tilingIns_->isKernelSplit ? this->l1Params.kwL1 : tilingIns_->shapeInfo.singlekW;
    this->l1TilingCalc.ci0HkWk = tmpkh * tmpkw * tilingIns_->cubeInfo.k0;
    this->l1TilingCalc.c04KSizeAlign = AlignB(C04_CIN_SIZE * tmpkh * tmpkw, tilingIns_->cubeInfo.k0);
    ...
    auto tmpSingleHi = InferHiL1(tilingIns_->shapeInfo.singleHo, tmpkh, tilingIns_->shapeInfo.orgHi);
    auto tmpSingleWi = this->l1Flags.isWoL1MustFullLoadFlagC04 ?
                           tilingIns_->shapeInfo.orgWi :
                           InferWiL1(AlignB(tilingIns_->shapeInfo.singleWo, tilingIns_->cubeInfo.m0), tmpkw,
                                     tilingIns_->shapeInfo.orgWi);
    ...
    this->l1TilingCalc.bL1Minsize = this->l1TilingCalc.kBL1MaxSize * l0Params.nL0 * weightDTypeSize *
                                    this->dbValue.pbBL1;
    this->l1TilingCalc.biasL1MinSize = tilingIns_->hasBias ? l0Params.nL0 * biasDTypeSize : 0;
    this->l1TilingCalc.fixpParamsL1MinSize = l0Params.nL0 * tilingIns_->shapeInfo.channelWiseCoeff * FP16_DTYPE_SIZE;
    ...
    this->l1TilingCalc.wiAL1Min = this->l1Flags.isWoL1MustFullLoadFlagC04 ?
                                      tilingIns_->shapeInfo.orgWi :
                                      InferWiL1(this->l1TilingCalc.woAL1Min, tmpkw, tilingIns_->shapeInfo.orgWi);
    this->l1TilingCalc.hiAL1Min = InferHiL1(this->l1TilingCalc.hoAL1Min, tmpkh, tilingIns_->shapeInfo.orgHi);
    this->l1TilingCalc.aL1Minsize = AlignB(this->l1TilingCalc.hiAL1Min * this->l1TilingCalc.wiAL1Min,
                                           C0_SIZE / (fMapDTypeSize * C04_CIN_SIZE)) *
...
{
    uint64_t minKBL1 = 0;
    uint64_t minwoAL1 = tilingIns_->shapeInfo.singleWo < tilingIns_->cubeInfo.m0 ? tilingIns_->shapeInfo.singleWo :
                                                                                   tilingIns_->cubeInfo.m0;
    if (tilingIns_->isC04Flag) {
    ...
    uint64_t minHoAL1 = tilingIns_->shapeInfo.singleWo < tilingIns_->cubeInfo.m0 ?
                            CeilDiv(tilingIns_->cubeInfo.m0, tilingIns_->shapeInfo.singleWo) :
                            1;
...
void ConvTilingAlgorithmHWmode::GetL1Tiling()
{
    GetCaseStatus();
    if (tilingIns_->isC04Flag) {
...
void ConvTilingAlgorithmHWmode::ScaleBiasUbTilingDecision()
{
    ...
    uint64_t totalScaleBiasTypeSize = tilingIns_->descInfo.biasType.dtype == ConvDtype::FLOAT32 ?
                                          TOTAL_SCALE_BIAS_32_TYPE_SIZE :
                                          TOTAL_SCALE_BIAS_16_TYPE_SIZE;
...
void ConvTilingAlgorithmHWmode::GetUbTiling() { ScaleBiasUbTilingDecision(); }
} // namespace conv_tiling_algo_hw
