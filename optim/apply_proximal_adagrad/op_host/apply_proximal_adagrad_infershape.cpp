 ...
 * - var_out dtype inherits input var dtype (fp32 / fp16 / bf16)
 * - accum 通过 input ref 原地更新, 不在输出端口暴露 (与 CANN built-in
 *   REG_OP(ApplyProximalAdagrad) 单输出兼容)。
 */
...
{
    // Input 0 = var, Output 0 = var (inplace, shape unchanged).
...
{
    // Output dtype follows input var (must be fp32 / fp16 / bf16 per spec).
