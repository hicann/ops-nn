...
/*!
 * \file aclnn_fused_adam.cpp
 * \brief
 */

#include "aclnn_fused_adam.h"
                                 ...
                                 const aclTensorList* stateSteps, const aclTensorList* maxExpAvgSqsRef)
{
    ...
    if (gradsRef->Size() != tensorCount) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "gradsRef tensor count does not match params.");
        return false;
    }
...
aclnnStatus aclnnFusedAdamGetWorkspaceSize(const aclTensorList* paramsRef, const aclTensorList* gradsRef,
                                           const aclTensorList* expAvgsRef, const aclTensorList* expAvgSqsRef,
...
{
    L2_DFX_PHASE_1(aclnnFusedAdam,
                   DFX_IN(paramsRef, gradsRef, expAvgsRef, expAvgSqsRef, maxExpAvgSqsRef, stateSteps, gradScaleOptional,
                          foundInfOptional, lr, beta1, beta2, weightDecay, eps, amsgrad, maximize),
                   DFX_OUT(paramsRef, gradsRef, expAvgsRef, expAvgSqsRef, maxExpAvgSqsRef));
    auto uniqueExecutor = CREATE_EXECUTOR();
    CHECK_RET(uniqueExecutor.get() != nullptr, ACLNN_ERR_INNER_CREATE_EXECUTOR);
    auto ret = CheckParams(paramsRef, gradsRef, expAvgsRef, expAvgSqsRef, maxExpAvgSqsRef, stateSteps, lr, beta1, beta2,
    ...
    auto [paramsOut, gradsOut, expAvgsOut, expAvgSqsOut, maxExpAvgSqsOut] = l0op::FusedAdam(
...
aclnnStatus aclnnFusedAdam(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor, aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnFusedAdam);
