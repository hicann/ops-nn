...
/*!
 * \file aclnn_fused_adam.h
 * \brief
 */

#ifndef _ACLNN_FUSED_ADAM_H_
#define _ACLNN_FUSED_ADAM_H_
...
/**
 * @brief aclnnFusedAdam的第一段接口，根据具体的计算流程，计算workspace大小。
 * @domain aclnn_ops_train
 *
 * 算子功能：融合applyadam优化器，支持params、grads、expAvgs、expAvgSqs、maxExpAvgSqs、stateSteps
 ...
 */
ACLNN_API aclnnStatus aclnnFusedAdamGetWorkspaceSize(
    const aclTensorList* paramsRef, const aclTensorList* gradsRef, const aclTensorList* expAvgsRef,
...
/**
 * @brief aclnnFusedAdam的第二段接口，用于执行计算。
 *
 * 算子功能：执行融合SGD优化器。
 ...
 * @return aclnnStatus: 返回状态码。
 */
ACLNN_API aclnnStatus aclnnFusedAdam(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                     aclrtStream stream);
