...
/*!
 * \file test_geir_inplace_apply_power_sign.cpp
 * \brief InplaceApplyPowerSign GEIR graph mode example
 *
...
// Inline definitions for op::Data and op::Const (avoids dependency on array_ops.h)
namespace ge {
REG_OP(Data)
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    auto placeholder##intputIndex = op::Data("placeholder" + std::to_string(intputIndex))                           \
                                        .set_attr_index(intputIndex - 1);                                           \
    ...
    applyPowerSignD1.set_input_##intputName(placeholder##intputIndex);                                              \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) applyPowerSignD1.set_attr_##attrName(attrValue)

#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    vector<int64_t> placeholder##intputIndex##_shape = inputShape;                                                  \
    auto placeholder##intputIndex = op::Const("placeholder" + std::to_string(intputIndex));                         \
    TensorDesc placeholder##intputIndex##_desc = TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_ND, \
                                                            intputDtype);                                           \
    placeholder##intputIndex##_desc.SetPlacement(ge::kPlacementHost);                                               \
    placeholder##intputIndex##_desc.SetFormat(FORMAT_ND);                                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    applyPowerSignD1.set_input_##intputName(placeholder##intputIndex);                                              \
    applyPowerSignD1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                               \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    applyPowerSignD1.update_output_desc_##outputName(outputName##outputIndex##_desc)
...
{
    Status ret = SUCCESS;
    auto applyPowerSignD1 = op::InplaceApplyPowerSign("applyPowerSignD1");
    ...
    // Scalar inputs: lr, logbase, sign_decay, beta
    ADD_INPUT(3, lr, inDtype, scalarShape);
    ADD_INPUT(4, logbase, inDtype, scalarShape);
    ADD_INPUT(5, sign_decay, inDtype, scalarShape);
    ADD_INPUT(6, beta, inDtype, scalarShape);
    ...
    outputs.push_back(applyPowerSignD1);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    if (ret != SUCCESS) {
        printf("%s - ERROR - [XIR]: Create operator graph failed\n", GetTime().c_str());
        return FAILED;
    }
