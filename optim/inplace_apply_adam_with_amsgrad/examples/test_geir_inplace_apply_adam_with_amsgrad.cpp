 ...
 * \brief GEIR smoke test for the InplaceApplyAdamWithAmsgrad operator.
 */
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    inplaceApplyAdamWithAmsgrad1.set_input_##intputName(placeholder##intputIndex);                                  \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) inplaceApplyAdamWithAmsgrad1.set_attr_##attrName(attrValue)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    inplaceApplyAdamWithAmsgrad1.update_output_desc_##outputName(outputName##outputIndex##_desc)
...
int CreateOppInGraph(DataType tensorDtype, std::vector<ge::Tensor>& input, std::vector<Operator>& inputs,
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    auto inplaceApplyAdamWithAmsgrad1 = op::InplaceApplyAdamWithAmsgrad("inplaceApplyAdamWithAmsgrad1");
    std::vector<int64_t> xShape = {2, 30};
    std::vector<int64_t> scalarShape = {1};
    // Five element-wise tensors (var/m/v/vhat/grad) share one dtype.
    ADD_INPUT(1, var, tensorDtype, xShape);
    ADD_INPUT(2, m, tensorDtype, xShape);
    ADD_INPUT(3, v, tensorDtype, xShape);
    ADD_INPUT(4, vhat, tensorDtype, xShape);
    // Six scalar inputs are always DT_FLOAT per the operator proto.
    ADD_INPUT(5, beta1_power, DT_FLOAT, scalarShape);
    ADD_INPUT(6, beta2_power, DT_FLOAT, scalarShape);
    ADD_INPUT(7, lr, DT_FLOAT, scalarShape);
    ADD_INPUT(8, beta1, DT_FLOAT, scalarShape);
    ADD_INPUT(9, beta2, DT_FLOAT, scalarShape);
    ADD_INPUT(10, epsilon, DT_FLOAT, scalarShape);
    ADD_INPUT(11, grad, tensorDtype, xShape);
    ADD_INPUT_ATTR(use_locking, false);

    outputs.push_back(inplaceApplyAdamWithAmsgrad1);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    DataType tensorDtype = DT_FLOAT16;

    std::cout << tensorDtype << std::endl;

    ret = CreateOppInGraph(tensorDtype, input, inputs, outputs, graph);
    if (ret != SUCCESS) {
