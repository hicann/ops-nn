...
- 计算公式：
  ...
  $y4 = input\_mul4 \times mul4\_x + \frac{y2/input\_realdiv0}{\sqrt{y3/input\_realdiv1} + add2\_y}$
...
| 调用方式 | 调用样例 | 说明 |
|---|---|---|
| 图模式调用 | [test_geir_lamb_next_m_v_with_decay](./examples/test_geir_lamb_next_m_v_with_decay.cpp) | 通过算子IR构图方式调用LambNextMVWithDecay算子。 |
