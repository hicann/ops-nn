...
/*!
 * \file test_geir_lars_v2_update.cpp
 * \brief geir example for lars_v2_update
 */
#include <iostream>
#include <fstream>
#include <string.h>
...
#include "../../op_graph/lars_v2_update_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    larsV2Update1.set_input_##intputName(placeholder##intputIndex);                                                 \
    larsV2Update1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                  \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) larsV2Update1.set_attr_##attrName(attrValue)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    larsV2Update1.update_output_desc_##outputName(outputName##outputIndex##_desc)
...
{
    Status ret = SUCCESS;
    auto larsV2Update1 = op::LarsV2Update("larsV2Update1");
    std::vector<int64_t> xShape = {2, 2};
    std::vector<int64_t> scalarShape = {1};
    ADD_INPUT(1, w, inDtype, xShape);
    ADD_INPUT(2, g, inDtype, xShape);
    ADD_INPUT(3, w_square_sum, DT_FLOAT, scalarShape);
    ADD_INPUT(4, g_square_sum, DT_FLOAT, scalarShape);
    ADD_INPUT(5, weight_decay, DT_FLOAT, scalarShape);
    ADD_INPUT(6, learning_rate, DT_FLOAT, scalarShape);
    ADD_INPUT_ATTR(hyperpara, 0.001f);
    ADD_INPUT_ATTR(epsilon, 0.00001f);
    ADD_INPUT_ATTR(use_clip, false);

    ADD_OUTPUT(0, g_new, inDtype, xShape);

    outputs.push_back(larsV2Update1);
    return SUCCESS;
}
