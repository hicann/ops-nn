...
static ge::graphStatus InferShape4LambNextMV(gert::InferShapeContext* context)
{
    ...
    for (size_t i = 0; i < OUT_NUM; i++) {
        ...
        OP_CHECK_IF(!BroadcastShape(g2, m, out),
                    OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context->GetNodeName(), "input_mul3 and input_mul0",
                                                           (ToString(*g2) + " and " + ToString(*m)).c_str(),
                                                           "shapes cannot broadcast"),
                    return ge::GRAPH_FAILED);
    }
    return GRAPH_SUCCESS;
}
static ge::graphStatus InferDataType4LambNextMV(gert::InferDataTypeContext* context)
{
    if (context == nullptr) {
        return ge::GRAPH_FAILED;
    }
    OP_LOGD(context->GetNodeName(), "InferDataType4LambNextMV enter");
    // y1 与 input_mul3 同类型
    ...
    OP_LOGD(context->GetNodeName(), "InferDataType4LambNextMV end");
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_INFERSHAPE(LambNextMV).InferShape(InferShape4LambNextMV).InferDataType(InferDataType4LambNextMV);
} // namespace ops
