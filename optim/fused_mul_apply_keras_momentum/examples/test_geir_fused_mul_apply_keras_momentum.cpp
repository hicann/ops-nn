...
#include "../op_graph/fused_mul_apply_keras_momentum_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    fused_mul_apply_keras_momentum_op.set_input_##intputName(placeholder##intputIndex);                             \
...
{
    Status ret = SUCCESS;
    auto fused_mul_apply_keras_momentum_op = op::FusedMulApplyKerasMomentum("fused_mul_apply_keras_momentum");
    fused_mul_apply_keras_momentum_op.set_attr_use_locking(false);
    fused_mul_apply_keras_momentum_op.set_attr_use_nesterov(false);
    std::vector<int64_t> tensorShape = {4, 4};
    std::vector<int64_t> scalarShape = {1};
    ADD_INPUT(1, var, inDtype, tensorShape);
    ADD_INPUT(2, accum, inDtype, tensorShape);
    ADD_INPUT(3, lr, inDtype, scalarShape);
    ADD_INPUT(4, x1, inDtype, tensorShape);
    ADD_INPUT(5, momentum, inDtype, scalarShape);
    ADD_INPUT(6, x2, inDtype, scalarShape);

    outputs.push_back(fused_mul_apply_keras_momentum_op);
    return SUCCESS;
}
