 ...
 * \brief
 */
#include "arch35/apply_came_part2.h"
#include "arch35/apply_came_part2_float16.h"
#include "arch35/apply_came_part2_typical_net.h"
#include "arch35/apply_came_part2_float16_typical_net.h"
#include "arch35/apply_came_part2_post.h"
#include "arch35/apply_came_part2_pre.h"
...
extern "C" __global__ __aicore__ void apply_came_part2(GM_ADDR grad, GM_ADDR sumGradR, GM_ADDR sumGradC,
                                                       ...
                                                       GM_ADDR sumR, GM_ADDR globalShape, GM_ADDR rOut, GM_ADDR cOut,
                                                       GM_ADDR u, GM_ADDR sumSquareU, GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(101)) {
        ...
        applyCamePart2.Init(grad, sumGradR, sumGradC, sumGradRc, rIn, cIn, beta2, sumR, globalShape, rOut, cOut, u,
                            sumSquareU, userWS, &tilingData);
    ...
    if (TILING_KEY_IS(102)) {
        ...
        op.Init(grad, sumGradR, sumGradC, sumGradRc, rIn, cIn, beta2, sumR, globalShape, rOut, cOut, u, sumSquareU,
                userWS, &tilingData);
        op.Process();
    }
...
{
    int64_t tmpMax = (tilingData_.rCoreNumToUse > tilingData_.cCoreNumToUse) ? tilingData_.rCoreNumToUse :
                                                                               tilingData_.cCoreNumToUse;
    return (tmpMax > tilingData_.rRcCoreNumToUse) ? tmpMax : tilingData_.rRcCoreNumToUse;
}

template <typename T>
__aicore__ inline void ApplyCamePart2<T>::Init(GM_ADDR grad, GM_ADDR sumGradR, GM_ADDR sumGradC, GM_ADDR sumGradRc,
                                               GM_ADDR rIn, GM_ADDR cIn, GM_ADDR beta2, GM_ADDR sumR,
                                               GM_ADDR globalShape, GM_ADDR rOut, GM_ADDR cOut, GM_ADDR u,
                                               GM_ADDR sumSquareU, GM_ADDR workspace,
                                               const ApplyCamePart2TilingData* tilingData)
{
    ...
    int64_t sumSquareUWsNum = CeilAlign(
        tilingData_.rRcCoreNumToUse * (tilingData_.rRcLoopCount + 1) * (tilingData_.cRcLoopCount + 1), 128);
...
{
    int64_t rBufferLength = (tilingData_.rNumPerLoop > tilingData_.rRcNumPerLoop) ? tilingData_.rNumPerLoop :
                                                                                    tilingData_.rRcNumPerLoop;
    int64_t cBufferLength = (tilingData_.cNumPerLoop > tilingData_.cRcNumPerLoop) ? tilingData_.cNumPerLoop :
                                                                                    tilingData_.cRcNumPerLoop;
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::CalcU(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop,
                                                int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::CopyInGrad(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop,
                                                     int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::CopyOutU(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop,
                                                   int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::CopyInNotAlignedGrad(int rLoopIdx, int cLoopIdx, int64_t curRNumInLoop,
                                                               int64_t curCNumInLoop)
{
    LocalTensor<T> gradLocal = inQueGrad_.AllocTensor<T>();
    int64_t startRowNum = rRcBlockOffset_ + rLoopIdx * tilingData_.rRcNumPerLoop;
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::BroadcastR(LocalTensor<T>& dst, LocalTensor<T>& src, int64_t numR,
                                                     int64_t numC)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::TransposeR(LocalTensor<T>& dst, LocalTensor<T>& src, int64_t numR,
                                                     int64_t numC)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::MulRC(LocalTensor<T>& dst, LocalTensor<T>& r, LocalTensor<T>& c, int64_t numR,
                                                int64_t numC)
{
    // r:(numR, numC)  c:(numC,)
...
__aicore__ inline void ApplyCamePart2<T>::CalcRcCycleMode(LocalTensor<T>& dst, LocalTensor<T>& src,
                                                          LocalTensor<T>& srcScalar, int64_t curRNumInLoop,
                                                          int64_t curCNumInLoop)
{
...
__aicore__ inline void ApplyCamePart2<T>::ComputeU(int64_t curRNumInLoop, int64_t curCNumInLoop)
{
    ...
    // ub3: r*c
    if ((curRNumInLoop % HALF_CALC_SIZE != 0) || (curCNumInLoop % HALF_CALC_SIZE != 0)) {
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::ReduceSumU(LocalTensor<T>& src, int64_t dataCount, int64_t curRNumInLoop,
                                                     int64_t curCNumInLoop)
{
...
template <typename T>
__aicore__ inline void ApplyCamePart2<T>::GetConfusionTransposeTiling(int64_t numR, int64_t numC,
