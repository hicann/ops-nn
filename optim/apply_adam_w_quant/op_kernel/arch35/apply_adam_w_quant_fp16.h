...
public:
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t n = 0; n < perCoreRowWork; n++) {
            ...
            DeQuant(stateM, qMapM, absMaxM, calcBuf, singleBlockNum, validSize);
            DeQuant(stateV, qMapV, absMaxV, calcBuf, singleBlockNum, validSize);
            DataCopyIn<T_VAR_GRAD>(var, varGm[blockOffset], validSize);
            DataCopyIn<T_VAR_GRAD>(grad, gradGm[blockOffset], validSize);
            PipeSync<AscendC::HardEvent::MTE2_V>();
            CastF16ToFp32<T, T_VAR_GRAD>(varFp32, var, validSize);
            CastF16ToFp32<T, T_VAR_GRAD>(gradFp32, grad, validSize);
            AscendC::PipeBarrier<PIPE_V>();
            UpdateStateAndParam(stateM.template ReinterpretCast<T>(), stateV.template ReinterpretCast<T>(), gradFp32,
                                varFp32, validSize);
            AscendC::PipeBarrier<PIPE_V>();
            CastFp32ToF16<T_VAR_GRAD, T>(var, varFp32, validSize);
            PipeSync<AscendC::HardEvent::V_MTE3>();
            DataCopyOut<T_VAR_GRAD>(varRefGm[blockOffset], var, validSize);
            QuantM(stateM.template ReinterpretCast<T>(), qMapM, stateM, absMaxM, gradFp32, calcBuf, singleBlockNum);
...
private:
    __aicore__ inline void ParseTilingData(const ApplyAdamWQuantRegbaseTilingData* tilingData)
    {
    ...
    __aicore__ inline void BinarySearchScaled(const AscendC::LocalTensor<T>& state, const AscendC::LocalTensor<T>& qMap,
    ...
    __aicore__ inline void CalcAbsMax(const AscendC::LocalTensor<T>& state, const AscendC::LocalTensor<T>& absMax,
                                      AscendC::TBuf<>& calcBuf, int32_t singleBlockNum)
    {
        AscendC::LocalTensor<T> absState = calcBuf.GetWithOffset<T>(singleSize, 0);
        AscendC::LocalTensor<T> absMaxTmp = calcBuf.GetWithOffset<T>(singleSize, singleSize * sizeof(T));
                                  ...
                                  int32_t singleBlockNum)
    {
        ...
        CheckSign(normStateTmp, qMap, qState, calcBuf, singleBlockNum);
    }
