...
/**
 * @file test_geir_inplace_apply_ftrl_v2_dynamic.cpp
 * @brief InplaceApplyFtrlV2 GE IR 动态 shape 调用示例
 *
 * 覆盖两种动态 shape 场景：
 ...
 *
 * 算子接口（REG_OP InplaceApplyFtrlV2）：
 *   9 输入: var, accum, linear, grad (ND state), lr, l1, l2, l2_shrinkage, lr_power (scalar {1})
 *   3 输出: var, accum, linear（状态更新语义；是否复用输入 buffer 由图编译器决定）
...
{
    auto op1 = op::InplaceApplyFtrlV2("inplace_apply_ftrl_v2_dyn");
...
static vector<ge::Tensor> MakeFeeds(const vector<int64_t>& actualShape, DataType dtype)
{
    ...
    // 5 scalar tensors: lr, l1, l2, l2_shrinkage, lr_power
    feeds.push_back(MakeScalarTensor(0.1f, desc));
    feeds.push_back(MakeScalarTensor(0.01f, desc));
    feeds.push_back(MakeScalarTensor(0.001f, desc));
