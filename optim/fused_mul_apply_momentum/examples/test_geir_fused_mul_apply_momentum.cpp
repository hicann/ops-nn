...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    fusedMomentum1.set_input_##intputName(placeholder##intputIndex);                                                \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) fusedMomentum1.set_attr_##attrName(attrValue)
...
{
    Status ret = SUCCESS;
    auto fusedMomentum1 = op::FusedMulApplyMomentum("fusedMomentum1");
    std::vector<int64_t> xShape = {8, 16};
    std::vector<int64_t> scalarShape = {1};

    ADD_INPUT(0, var, inDtype, xShape);
    ADD_INPUT(1, accum, inDtype, xShape);
    ADD_INPUT(2, lr, inDtype, scalarShape);
    ADD_INPUT(3, x1, inDtype, xShape);
    ADD_INPUT(4, momentum, inDtype, scalarShape);
    ADD_INPUT(5, x2, inDtype, scalarShape);
    ADD_INPUT_ATTR(use_nesterov, false);
    ADD_INPUT_ATTR(use_locking, false);

    outputs.push_back(fusedMomentum1);
    return SUCCESS;
}

int main()
{
    const char* graph_name = "fused_momentum_geir_test";
