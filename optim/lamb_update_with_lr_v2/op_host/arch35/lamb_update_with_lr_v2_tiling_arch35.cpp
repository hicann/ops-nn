...
/*!
 * \file lamb_update_with_lr_v2_tiling_arch35.cpp
 * \brief lamb_update_with_lr_v2_tiling_arch35 source file
 */

#include "lamb_update_with_lr_v2_tiling_arch35.h"
#include <graph/utils/type_utils.h>
#include "../../op_kernel/arch35/lamb_update_with_lr_v2_dag.h"
...
constexpr static uint64_t LAMB_UPDATE_WITH_LR_V2_TILING_PRIORITY = 0;
constexpr static int32_t INPUT_NUM = 7;
constexpr static int32_t OUTPUT_NUM = 1;

static ge::graphStatus TilingPrepareForLambUpdateWithLrV2(gert::TilingParseContext* context)
{
    auto compileInfoPtr = context->GetCompiledInfo<LambUpdateWithLrV2CompileInfo>();
...
ge::graphStatus LambUpdateWithLrV2Tiling::GetShapeAttrsInfo()
{
    auto input0Desc = context_->GetInputDesc(0);
    OP_CHECK_NULL_WITH_CONTEXT(context_, input0Desc);
    ge::DataType input0DType = input0Desc->GetDataType();
    static const char* kInputNames[] = {"x1", "x2", "x3", "x4", "x5", "greater_y", "select_e"};
    ...
    static const int32_t kScalarInputIdx[] = {0, 1, 2, 5, 6};
...
bool LambUpdateWithLrV2Tiling::IsCapable() { return true; }

ge::graphStatus LambUpdateWithLrV2Tiling::DoOpTiling()
{
    ...
    if (input0DType == ge::DT_FLOAT16) {
        BroadcastBaseTiling<LambUpdateWithLrV2Op::LambUpdateWithLrV2Compute<half, float>::OpDag> brcBaseTiling(
        ...
        BroadcastBaseTiling<LambUpdateWithLrV2Op::LambUpdateWithLrV2Compute<float, float>::OpDag> brcBaseTiling(
            context_, static_cast<uint32_t>(BROADCAST_KERNEL_TYPE::KERNEL_TYPE_NDDMA));
        OP_CHECK_IF(brcBaseTiling.DoTiling() == ge::GRAPH_FAILED,
                    OP_LOGE(context_->GetNodeName(), "Do tiling failed. Please check the detailed log."),
                    return ge::GRAPH_FAILED);
...
ge::graphStatus LambUpdateWithLrV2Tiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t LambUpdateWithLrV2Tiling::GetTilingKey() const { return tilingKey; }

ge::graphStatus LambUpdateWithLrV2Tiling::GetWorkspaceSize() { return ge::GRAPH_SUCCESS; }

ge::graphStatus LambUpdateWithLrV2Tiling::PostTiling() { return ge::GRAPH_SUCCESS; }

ge::graphStatus LambUpdateWithLrV2Tiling::GetPlatformInfo() { return ge::GRAPH_SUCCESS; }

static ge::graphStatus TilingForLambUpdateWithLrV2(gert::TilingContext* context)
{
    OP_LOGD("LambUpdateWithLrV2Tiling", "Enter TilingForLambUpdateWithLrV2");
    if (context == nullptr) {
        OP_LOGE("LambUpdateWithLrV2Tiling", "Tiling context is nullptr");
        return ge::GRAPH_FAILED;
    }
...
IMPL_OP_OPTILING(LambUpdateWithLrV2)
    .Tiling(TilingForLambUpdateWithLrV2)
    .TilingParse<LambUpdateWithLrV2CompileInfo>(TilingPrepareForLambUpdateWithLrV2);

REGISTER_OPS_TILING_TEMPLATE(LambUpdateWithLrV2, LambUpdateWithLrV2Tiling, LAMB_UPDATE_WITH_LR_V2_TILING_PRIORITY);
} // namespace optiling
