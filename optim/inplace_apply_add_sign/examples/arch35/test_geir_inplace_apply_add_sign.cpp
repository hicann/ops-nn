...
/*!
 * \file test_geir_inplace_apply_add_sign.cpp
 * \brief GEIR graph mode test for InplaceApplyAddSign operator
 */
#include <iostream>
#include <fstream>
#include <string.h>
...
#include "../../op_graph/inplace_apply_add_sign_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    applyAddSign1.set_input_##intputName(placeholder##intputIndex);                                                 \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) applyAddSign1.set_attr_##attrName(attrValue)
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    applyAddSign1.set_input_##intputName(placeholder##intputIndex);                                                 \
    applyAddSign1.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                  \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    applyAddSign1.update_output_desc_##outputName(outputName##outputIndex##_desc)
...
{
    Status ret = SUCCESS;
    auto applyAddSign1 = op::InplaceApplyAddSign("applyAddSign1");
    std::vector<int64_t> xShape = {2, 30};
    ADD_INPUT(1, var, inDtype, xShape);
    ADD_INPUT(2, m, inDtype, xShape);
    ADD_INPUT(3, lr, inDtype, std::vector<int64_t>{1});
    ADD_INPUT(4, alpha, inDtype, std::vector<int64_t>{1});
    ADD_INPUT(5, sign_decay, inDtype, std::vector<int64_t>{1});
    ADD_INPUT(6, beta, inDtype, std::vector<int64_t>{1});
    ADD_INPUT(7, grad, inDtype, xShape);
    ADD_INPUT_ATTR(use_locking, false);

    outputs.push_back(applyAddSign1);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    printf("Shape, dtype and values PASSED for [2,30]\n");
    printf("InplaceApplyAddSign static GEIR verification PASSED\n");
