...
def gen_data_and_golden(shape_str, d_type="float32"):
    ...
    for index, shape in enumerate(shape_list):
        ...
        # Compute reference (AdamW in fp32 precision)
        p = params.astype(np.float64)
        g = grads.astype(np.float64)
        m = exp_avgs.astype(np.float64)
        v = exp_avg_sqs.astype(np.float64)
        mx = max_exp_avg_sqs.astype(np.float64)
        ...
        # Save outputs (reuse same input files as ref, since output uses same tensor lists)
        save_arr(p, d_type, f"{d_type}_input_t_params_ref_{index}.bin")
        save_arr(m, d_type, f"{d_type}_input_t_exp_avgs_ref_{index}.bin")
        save_arr(v, d_type, f"{d_type}_input_t_exp_avg_sqs_ref_{index}.bin")
        if amsgrad:
            save_arr(mx, d_type, f"{d_type}_input_t_max_exp_avg_sqs_ref_{index}.bin")
