...
| [aclnnClippedSwiglu](../../activation/clipped_swiglu/docs/aclnnClippedSwiglu.md) | 带截断的Swish门控线性单元激活函数，实现x的SwiGlu计算。 | 默认确定性实现 | - |
...
| [aclnnDynamicMxQuantWithDualAxis](../../quant/dynamic_mx_quant_with_dual_axis/docs/aclnnDynamicMxQuantWithDualAxis.md) | 在-1轴和-2轴上同时进行目的数据类型为FLOAT4类、FLOAT8类的MX量化。当前仅支持scaleAlg取值0（OCP算法）。算子实现时，-2轴（不包含）之前的轴会进行合轴处理。 | - | 默认确定性实现 |
...
| [aclnnForeachPowScalarV2](../../foreach/foreach_pow_scalar/docs/aclnnForeachPowScalarV2.md) | 对输入张量列表x中的每个张量进行指数运算，指数为标量exponent。 | 默认确定性实现 | - |
...
| [aclnnFusedCrossEntropyLossWithMaxSum](../../loss/fused_cross_entropy_loss_with_max_sum/docs/aclnnFusedCrossEntropyLossWithMaxSum.md) | 词汇表并行场景下交叉熵计算模块的一部分，解决超大规模词汇表下的显存和计算效率问题，计算loss与softMax的结果。 | 默认确定性实现 | - |
...
| [aclnnMaxUnpool2d](../../index/scatter_elements/docs/aclnnMaxUnpool2d.md) | MaxPool2d的逆运算，根据indices索引在out中填入input的元素值。 | 默认确定性实现 | 默认确定性实现 |
...
| [aclnnMaxUnpool3d](../../index/scatter_elements/docs/aclnnMaxUnpool3d.md) | MaxPool3d的逆运算，根据indices索引在out中填入input的元素值。 | 默认确定性实现 | 默认确定性实现 |
...
| [aclnnPrelu](../../activation/prelu/docs/aclnnPrelu.md) | 激活函数，Tensor中value大于0，取该value，小于0时取权重与value的乘积。 | 默认确定性实现 | 默认确定性实现 |
