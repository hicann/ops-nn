...
public:
    __aicore__ inline MaxPool3DGradSimtKernel(
        TPipe* pipe, const Pool3DGradNameSpace::MaxPool3DGradSimtTilingData* __restrict__ tilingData)
...
template <typename VALUE_T, typename INDICES_T, int64_t Format_T, bool useINT64Index, typename OFFSET_T>
__aicore__ inline void MaxPool3DGradSimtKernel<VALUE_T, INDICES_T, Format_T, useINT64Index, OFFSET_T>::Init(
...
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void MaxPool3DNcdhw(
    const int64_t count, const __gm__ VAL_T* bottomData, const int64_t dSize, const int64_t height, const int64_t width,
    ...
    FASTDIV_T shift1, FASTDIV_T m2, FASTDIV_T shift2)
{
    for (FASTDIV_T index = blockIdx.x * blockDim.x + threadIdx.x; index < count;
        ...
        PROCESS_T dEnd = (dStart + (kernelD - 1) * dilationD + 1) < dSize ? (dStart + (kernelD - 1) * dilationD + 1) :
                                                                            dSize;
        PROCESS_T hEnd = (hStart + (kernelH - 1) * dilationH + 1) < height ? (hStart + (kernelH - 1) * dilationH + 1) :
                                                                             height;
        PROCESS_T wEnd = (wStart + (kernelW - 1) * dilationW + 1) < width ? (wStart + (kernelW - 1) * dilationW + 1) :
                                                                            width;
        while (dStart < 0) {
            dStart += dilationD;
        }
...
template <typename VAL_T, typename IDX_T, typename PROCESS_T, typename FASTDIV_T>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void MaxPool3DNdhwc(
    const int64_t count, const __gm__ VAL_T* bottomData, const int64_t channels, const int64_t dSize,
    const int64_t height, const int64_t width, const int32_t outputDi, const int32_t outputHeight,
...
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void MaxPool3DGradWithArgmaxNcdhw(
    __ubuf__ OFFSET_T* simtParams, const __gm__ VALUE_T* gradY,
    __ubuf__ Pool3DGradNameSpace::MaxPool3DGradSimtTilingData* tilingData_, __gm__ VALUE_T* gradX,
    __gm__ INDICES_T* argmax)
{
    ...
    using DIV_T = typename std::conditional<std::is_same<OFFSET_T, int32_t>::value, uint32_t, uint64_t>::type;
    DIV_T count = tilingData_->nDim * tilingData_->cDim * tilingData_->dInDim * tilingData_->hInDim *
                  tilingData_->wInDim;
    for (DIV_T index = blockIdx.x * blockDim.x + threadIdx.x; index < count; index += gridDim.x * blockDim.x) {
        ...
        OFFSET_T pdStarts = PStart<OFFSET_T>(d, tilingData_->padD, tilingData_->kSizeD, tilingData_->dilationD,
                                             magicStrideD, shiftStrideD);
        OFFSET_T pdEnds = PEnd<OFFSET_T>(d, tilingData_->padD, tilingData_->dOutDim, magicStrideD, shiftStrideD);
        OFFSET_T phStarts = PStart<OFFSET_T>(h, tilingData_->padH, tilingData_->kSizeH, tilingData_->dilationH,
                                             magicStrideH, shiftStrideH);
        OFFSET_T phEnds = PEnd<OFFSET_T>(h, tilingData_->padH, tilingData_->hOutDim, magicStrideH, shiftStrideH);
        OFFSET_T pwStarts = PStart<OFFSET_T>(w, tilingData_->padW, tilingData_->kSizeW, tilingData_->dilationW,
                                             magicStrideW, shiftStrideW);
        ...
        for (OFFSET_T pd = pdStarts; pd < pdEnds; ++pd) {
            for (OFFSET_T ph = phStarts; ph < phEnds; ++ph) {
                for (OFFSET_T pw = pwStarts; pw < pwEnds; ++pw) {
                    DIV_T outputIdx = n * tilingData_->cDim * tilingData_->dOutDim * tilingData_->hOutDim *
                                          tilingData_->wOutDim +
                                      c * tilingData_->dOutDim * tilingData_->hOutDim * tilingData_->wOutDim +
...
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_DIM) inline void MaxPool3DGradWithArgmaxNdhwc(
    __ubuf__ OFFSET_T* simtParam, const __gm__ VALUE_T* gradY,
    __ubuf__ Pool3DGradNameSpace::MaxPool3DGradSimtTilingData* tilingData_, __gm__ VALUE_T* gradX,
    __gm__ INDICES_T* argmax)
{
    ...
    for (DIV_T index = blockIdx.x * blockDim.x + threadIdx.x; index < count; index += gridDim.x * blockDim.x) {
        ...
        DIV_T inputIdx = d * tilingData_->hInDim * tilingData_->wInDim + h * tilingData_->wInDim + w;
        OFFSET_T pdStart = PStart<OFFSET_T>(d, tilingData_->padD, tilingData_->kSizeD, tilingData_->dilationD,
                                            magicStrideD, shiftStrideD);
        OFFSET_T pdEnd = PEnd<OFFSET_T>(d, tilingData_->padD, tilingData_->dOutDim, magicStrideD, shiftStrideD);
        OFFSET_T phStart = PStart<OFFSET_T>(h, tilingData_->padH, tilingData_->kSizeH, tilingData_->dilationH,
                                            magicStrideH, shiftStrideH);
        OFFSET_T phEnd = PEnd<OFFSET_T>(h, tilingData_->padH, tilingData_->hOutDim, magicStrideH, shiftStrideH);
        OFFSET_T pwStart = PStart<OFFSET_T>(w, tilingData_->padW, tilingData_->kSizeW, tilingData_->dilationW,
                                            magicStrideW, shiftStrideW);
        ...
        for (OFFSET_T pd = pdStart; pd < pdEnd; ++pd) {
            for (OFFSET_T ph = phStart; ph < phEnd; ++ph) {
                for (OFFSET_T pw = pwStart; pw < pwEnd; ++pw) {
                    DIV_T outputIdx = n * tilingData_->dOutDim * tilingData_->hOutDim * tilingData_->wOutDim *
                                          tilingData_->cDim +
                                      pd * tilingData_->hOutDim * tilingData_->wOutDim * tilingData_->cDim +
...
template <typename VALUE_T, typename INDICES_T, int64_t Format_T, bool useINT64Index, typename OFFSET_T>
__aicore__ inline void MaxPool3DGradSimtKernel<VALUE_T, INDICES_T, Format_T, useINT64Index, OFFSET_T>::ComputePos()
    const
{
    ...
    int64_t totalSize = tilingData_->nDim * tilingData_->cDim * tilingData_->dOutDim * tilingData_->hOutDim *
                        tilingData_->wOutDim;
    if constexpr (Format_T == 0 && !useINT64Index) {
        ...
        asc_vf_call<MaxPool3DNcdhw<VALUE_T, INDICES_T, int32_t, uint32_t>>(
            dim3(THREAD_DIM), totalSize, inputData, tilingData_->dInDim, tilingData_->hInDim, tilingData_->wInDim,
        ...
        asc_vf_call<MaxPool3DNdhwc<VALUE_T, INDICES_T, int32_t, uint32_t>>(
            dim3(THREAD_DIM), totalSize, inputData, tilingData_->cDim, tilingData_->dInDim, tilingData_->hInDim,
        ...
        asc_vf_call<MaxPool3DNcdhw<VALUE_T, INDICES_T, int64_t, uint64_t>>(
            dim3(THREAD_DIM), totalSize, inputData, tilingData_->dInDim, tilingData_->hInDim, tilingData_->wInDim,
            tilingData_->dOutDim, tilingData_->hOutDim, tilingData_->wOutDim, tilingData_->kSizeD, tilingData_->kSizeH,
            tilingData_->kSizeW, tilingData_->stridesD, tilingData_->stridesH, tilingData_->stridesW, tilingData_->padD,
            tilingData_->padH, tilingData_->padW, tilingData_->dilationD, tilingData_->dilationH,
        ...
        asc_vf_call<MaxPool3DNdhwc<VALUE_T, INDICES_T, int64_t, uint64_t>>(
            dim3(THREAD_DIM), totalSize, inputData, tilingData_->cDim, tilingData_->dInDim, tilingData_->hInDim,
            tilingData_->wInDim, tilingData_->dOutDim, tilingData_->hOutDim, tilingData_->wOutDim, tilingData_->kSizeD,
            tilingData_->kSizeH, tilingData_->kSizeW, tilingData_->stridesD, tilingData_->stridesH,
            tilingData_->stridesW, tilingData_->padD, tilingData_->padH, tilingData_->padW, tilingData_->dilationD,
...
__aicore__ inline void MaxPool3DGradSimtKernel<VALUE_T, INDICES_T, Format_T, useINT64Index, OFFSET_T>::ComputeBack()
{
    ...
    if constexpr (Format_T == 1) {
        asc_vf_call<MaxPool3DGradWithArgmaxNdhwc<VALUE_T, INDICES_T, OFFSET_T>>(
            ...
            (__ubuf__ Pool3DGradNameSpace::MaxPool3DGradSimtTilingData*)(simtTilingData.GetPhyAddr()), outputData,
            indicesData);
    } else if (Format_T == 0) {
