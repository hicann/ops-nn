...
ge::graphStatus MaxPoolV3BaseTiling::CheckShape(gert::Shape& inputShape, gert::Shape& outputShape,
                                                const ge::Format& inputFormat)
{
    ...
    if (inputShape.GetShapeSize() <= 0) {
        OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(opName_, "x", std::to_string(inputShape.GetShapeSize()).c_str(),
                                                  "shape size must be greater than 0");
        return ge::GRAPH_FAILED;
    }

    if (outputShape.GetShapeSize() <= 0) {
        OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(opName_, "y", std::to_string(outputShape.GetShapeSize()).c_str(),
                                                  "shape size must be greater than 0");
        return ge::GRAPH_FAILED;
    }
    ...
    if (inputShape.GetDim(nDim) != outputShape.GetDim(nDim)) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
            opName_, "x", std::to_string(inputShape.GetDim(nDim)).c_str(),
    ...
    if (inputShape.GetDim(cDim) != outputShape.GetDim(cDim)) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
            opName_, "x", std::to_string(inputShape.GetDim(cDim)).c_str(),
...
ge::graphStatus MaxPoolV3BaseTiling::GetShapeAndDtype()
{
    ...
    if (IsInvalidType(dtype)) {
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
            opName_, "x", std::to_string(static_cast<int32_t>(dtype)).c_str(),
            "dtype not in supported list [0(DT_FLOAT), 1(DT_FLOAT16), 27(DT_BF16), 9(DT_INT64), 3(DT_INT32), "
            "19(DT_UINT32), 5(DT_INT16), 18(DT_UINT16), 2(DT_INT8), 17(DT_UINT8)]");
        return ge::GRAPH_FAILED;
    }
    dtypeSize = ge::GetSizeByDataType(dtype);
    if (dtypeSize <= 0) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "x dtype size", std::to_string(dtypeSize).c_str(),
...
ge::graphStatus MaxPoolV3BaseTiling::GetKernelInfo()
{
    ...
    if (std::string(context_->GetNodeType()) == "MaxPoolV2") {
        ...
        if (stride->GetShapeSize() != NCHW_DIMS) {
            OP_LOGE_FOR_INVALID_LISTSIZE(opName_, "Length of strides", std::to_string(stride->GetShapeSize()).c_str(),
                                         "4");
            return ge::GRAPH_FAILED;
        }
        ...
        if (stride->GetSize() != NCHW_DIMS) {
            OP_LOGE_FOR_INVALID_LISTSIZE(opName_, "Length of strides", std::to_string(stride->GetSize()).c_str(), "4");
            return ge::GRAPH_FAILED;
        }
...
ge::graphStatus MaxPoolV3BaseTiling::GetPadInfo()
{
    ...
    if (padModeStr_ == "CALCULATED") {
        ...
        if (padding->GetSize() != NCHW_DIMS) {
            OP_LOGE_FOR_INVALID_LISTSIZE(opName_, "Length of padding", std::to_string(padding->GetSize()).c_str(), "4");
            return ge::GRAPH_FAILED;
        }
        ...
        if (topPad >= inputData.kernelSize[H_DIM] || topPad < 0 || bottomPad >= inputData.kernelSize[H_DIM] ||
            bottomPad < 0 || leftPad >= inputData.kernelSize[W_DIM] || leftPad < 0 ||
            rightPad >= inputData.kernelSize[W_DIM] || rightPad < 0) {
            OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(
                opName_, "top_pad, bottom_pad, left_pad, right_pad",
                (std::to_string(topPad) + ", " + std::to_string(bottomPad) + ", " + std::to_string(leftPad) + ", " +
                 std::to_string(rightPad))
                    .c_str(),
                ("padding must be >= 0 and < corresponding kernel size, kernel H=" +
                 std::to_string(inputData.kernelSize[H_DIM]) + ", W=" + std::to_string(inputData.kernelSize[W_DIM]))
                    .c_str());
            return ge::GRAPH_FAILED;
        }
        ...
        int64_t hPadNeed = std::max(int64_t{0}, (inputData.outShape[0] - 1) * inputData.stride[0] +
                                                    inputData.kernelSize[0] - inputData.inputShape[0]);
        int64_t topPad = hPadNeed / DIGIT_TWO;
        int64_t bottomPad = hPadNeed - topPad;
        int64_t wPadNeed = std::max(int64_t{0}, (inputData.outShape[1] - 1) * inputData.stride[1] +
                                                    inputData.kernelSize[1] - inputData.inputShape[1]);
        int64_t leftPad = wPadNeed / DIGIT_TWO;
        int64_t rightPad = wPadNeed - leftPad;
        inputData.pad = {topPad, bottomPad, leftPad, rightPad};
    } else {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "padding_mode", padModeStr_.c_str(),
                                              "only support CALCULATED, SAME, VALID");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus MaxPoolV3BaseTiling::GetAttrsInfo()
{
    ...
    if (inputFormatStr == "NCHW") {
        ...
        OP_LOGE_FOR_INVALID_FORMATS_WITH_REASON(opName_, "x", inputFormatStr.c_str(), "only support NCHW and NHWC");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus MaxPoolV3BaseTiling::CheckOutPutShapeForValid()
{
    ...
    if (inputData.outShape[0] != expectedH || inputData.outShape[1] != expectedW) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            ...
            ("when padmode is VALID, output shape must be (" + std::to_string(expectedH) + ", " +
...
ge::graphStatus MaxPoolV3BaseTiling::CheckOutPutShapeForSame() { return ge::GRAPH_SUCCESS; }

int64_t MaxPoolV3BaseTiling::InferCalculateOutShape(const int64_t ksize, const int64_t padL, const int64_t padR,
...
{
    const char* opName_ = "MaxPoolV3";
    int64_t expectedH = InferCalculateOutShape(inputData.kernelSize[0], inputData.pad[TOP_PAD_INDEX],
                                               inputData.pad[BOTTOM_PAD_INDEX], inputData.stride[0], inputData.ceilMode,
                                               inputData.inputShape[H_DIM]);
    int64_t expectedW = InferCalculateOutShape(inputData.kernelSize[1], inputData.pad[LEFT_PAD_INDEX],
                                               inputData.pad[RIGHT_PAD_INDEX], inputData.stride[1], inputData.ceilMode,
                                               inputData.inputShape[W_DIM]);
    if (inputData.outShape[0] > expectedH || inputData.outShape[1] > expectedW) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            opName_, "y",
            (std::to_string(inputData.outShape[0]) + ", " + std::to_string(inputData.outShape[1])).c_str(),
            ("when padmode is CALCULATED, output shape must not exceed (" + std::to_string(expectedH) + ", " +
             std::to_string(expectedW) + ")")
                .c_str());
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus MaxPoolV3BaseTiling::GetShapeAttrsInfo()
{
    ...
    if (inputData.globalPooling) {
        if ((inputData.inputShape[0] != 0 && inputData.outShape[0] != 1) ||
            ...
            OP_LOGE_FOR_INVALID_VALUES_WITH_REASON(
                opName_, "y[h-dim], y[w-dim]",
                (std::to_string(inputData.outShape[0]) + ", " + std::to_string(inputData.outShape[1])).c_str(),
                "when global_pooling is true, output shape in h-dim and w-dim must be 1 if the corresponding input "
                "shape is not zero");
            return ge::GRAPH_FAILED;
        }
        inputData.pad = {0, 0, 0, 0};
...
bool MaxPoolV3BaseTiling::IsCapable() { return true; }

ge::graphStatus MaxPoolV3BaseTiling::DoOpTiling() { return ge::GRAPH_SUCCESS; }

ge::graphStatus MaxPoolV3BaseTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t MaxPoolV3BaseTiling::GetTilingKey() const { return 0; }
...
ge::graphStatus MaxPoolV3BaseTiling::PostTiling() { return ge::GRAPH_SUCCESS; }
} // namespace optiling
