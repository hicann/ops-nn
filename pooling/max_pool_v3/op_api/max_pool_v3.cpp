...
// 1971支持float16和float
static const std::initializer_list<op::DataType> DTYPE_SUPPORT_910B_LIST = {op::DataType::DT_FLOAT16,
...
static const std::initializer_list<op::DataType> DTYPE_SUPPORT_950_LIST = {
    ...
    op::DataType::DT_INT16,   op::DataType::DT_INT8,  op::DataType::DT_UINT16};
...
{
    L0_DFX(MaxPoolV3AiCore, self, kernelShape4, strides4, padsMode, pads4, dataFormat, globalPooling, ceilModeBool,
           maxpoolv3Out);
    // 使用框架宏ADD_TO_LAUNCHER_LIST_AICORE，将MaxPoolV3算子加入任务队列
    // MaxPoolV3是算子的OpType，self是算子的输入，maxpoolv3Out是算子的输出
    ADD_TO_LAUNCHER_LIST_AICORE(
        MaxPoolV3, OP_INPUT(self), OP_OUTPUT(maxpoolv3Out),
        OP_ATTR(kernelShape4, strides4, padsMode, pads4, dataFormat, globalPooling, ceilModeBool));
                           ...
                           aclOpExecutor* executor)
{
    ...
    auto ret = INFER_SHAPE(MaxPoolV3, OP_INPUT(self), OP_OUTPUT(out),
                           OP_ATTR(kernelShape4, strides4, padsMode, pads4, dataFormat, globalPooling, ceilModeBool));
    ...
    if (IsAiCoreSupport(self)) {
        // 调用Aicore算子计算
        return MaxPoolV3AiCore(self, kernelShape4, strides4, padsMode, pads4, dataFormat, globalPooling, ceilModeBool,
                               out, executor);
    }
