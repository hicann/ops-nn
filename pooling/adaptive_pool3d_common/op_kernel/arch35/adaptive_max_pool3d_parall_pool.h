                                                                 ...
                                                                 int64_t indexOffset)
{
    ...
    if constexpr (IsSameType<T, float>::value) {
        PoolUtils::DataMove::TransposeB32<T>(maxOutLocal, dTransLocal, rowNumAlign, vlNum_, ubBlockSize_);
    } else {
        PoolUtils::DataMove::TransposeB16<T>(maxOutLocal, dTransLocal, rowNumAlign, vlNum_, ubBlockSize_);
    }
    PoolUtils::DataMove::TransposeB32<int32_t>(maxIndexOutLocal, maxIndexInLocal, rowNumAlign, vlNum_, ubBlockSize_);
...
__aicore__ inline void AdaptiveMaxPool3dParaPool<T, U>::Process()
{
    ...
    for (auto curIdx = startBlockIdx_; curIdx < endBlockIdx_; curIdx++) {
        if (curIdx == startBlockIdx_) {
            PoolUtils::Compute::CalInputBlockPara(curIdx, blockPara_, tilingData_, inDHW_, inHW_);
        }
        ...
        if (curIdx == startBlockIdx_) {
            PoolUtils::DataMove::CopyInput<T>(inputQue, xGm, ncNum, diDataLen, hiDataLen, wiDataLen, xOffset,
                                              ubAlignNum_, tilingData_.wIn, inHW_, inDHW_);
        }
        ...
        if (curIdx != endBlockIdx_ - 1) {
            ...
            PoolUtils::DataMove::CopyInput<T>(inputQue, xGm, blockPara_.ncNum, blockPara_.diDataLen,
                                              blockPara_.hiDataLen, blockPara_.wiDataLen, blockPara_.xOffset,
                                              ubAlignNum_, tilingData_.wIn, inHW_, inDHW_);
        }
