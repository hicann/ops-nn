...
#include "pooling/common/op_api/max_pool_with_argmax_v1.h"
    ...
    const aclIntArray* dilation4, bool ceilMode, aclTensor* out, aclTensor* indices, aclOpExecutor* executor)
{
    ...
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "MaxPoolWithArgmaxV1AiCore ADD_TO_LAUNCHER_LIST_AICORE failed."),
             return nullptrTuple);
    ...
    const aclIntArray* dilation, bool ceilMode, aclOpExecutor* executor)
{
    ...
    auto out = executor->AllocTensor(outputStorageShape, outputShape, self->GetDataType(), self->GetStorageFormat(),
    ...
    auto indices = executor->AllocTensor(maskStorageShape, maskShape, dataTpye, self->GetStorageFormat(),
                                         self->GetOriginalFormat());
    if (indices == nullptr) {
    ...
    if (IsAiCoreSupport(self, isMask)) {
        // 调用算子计算
        return MaxPoolWithArgmaxV1AiCore(self, kernelSize4, stride4, padding4, dilation4, ceilMode, out, indices,
                                         executor);
    }
