...
ge::graphStatus InferShape4AvgPoolGrad(gert::InferShapeContext* context)
{
    ...
    if (gradOriFormat != FORMAT_ND && gradOriFormat != FORMAT_NCHW && gradOriFormat != FORMAT_NHWC) {
        OP_LOGE_FOR_INVALID_FORMAT(context->GetNodeName(), "input_grad", Ops::Base::ToString(gradOriFormat).c_str(),
                                   "ND, NCHW or NHWC");
        return GRAPH_FAILED;
    }
    ...
    if (strcmp(padding, "SAME") != 0 && strcmp(padding, "VALID") != 0) {
        OP_LOGE_FOR_INVALID_VALUE(context->GetNodeName(), "padding", padding, "SAME or VALID");
        return GRAPH_FAILED;
    }
    ...
    if (inputDimNum != CHW_DIMS && inputDimNum != NCHW_DIMS) {
        OP_LOGE_FOR_INVALID_SHAPESIZE(context->GetNodeName(), "orig_input_shape", std::to_string(inputDimNum).c_str(),
                                      "3 or 4");
        return GRAPH_FAILED;
    }
