...
- **特殊值处理：**
  - NaN输入：采用NaN传播语义，与MaxPoolGrad保持一致。NaN参与最大值比较且始终胜出，窗口内存在NaN时梯度回传到该窗口内按行优先扫描顺序的最后一个NaN位置；窗口内全为NaN时同样按该规则回传。
  - Inf输入：Inf作为最大值正常参与比较和梯度传播。
  - +0.0与-0.0遵循IEEE 754标准，比较相等。
  - 多个输入位置具有相同最大值且均非NaN时，仅第一个最大值位置接收梯度（first-wins策略）。
  - padding区域不接收梯度。
...
- **确定性保证：**
  ...
  - argmax选择规则唯一：非NaN取首个最大值，窗口含NaN时取最后一个NaN，结果可复现。
