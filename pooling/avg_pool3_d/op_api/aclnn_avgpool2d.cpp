...
namespace {
template <typename T>
static const aclTensor* GenWeightIdentity(aclOpExecutor* executor, int32_t blockK, int32_t blockN,
                                          op::DataType dataType, T areaFactor)
{
...
static aclnnStatus GetPoolInfo(const aclTensor* self, const aclIntArray* kernel, const aclIntArray* stride,
                               const aclIntArray* padding, const int64_t divisorOverride, const aclTensor* out,
...
static bool CheckNotNullPtr(const aclTensor* self, const aclIntArray* kernel, const aclIntArray* stride,
                            const aclIntArray* padding, aclTensor* out)
{
...
static bool CheckFormat(const aclTensor* self, const aclTensor* out)
{
    ...
    // 检查当输入是4D时格式是否为NCHW
    if (self->GetViewShape().GetDimNum() == DIM_NUM_4D &&
        (self->GetStorageFormat() != Format::FORMAT_NCHW || out->GetStorageFormat() != Format::FORMAT_NCHW)) {
    ...
    if (self->GetViewShape().GetDimNum() == DIM_NUM_3D &&
        (self->GetStorageFormat() != Format::FORMAT_NCL || out->GetStorageFormat() != Format::FORMAT_NCL)) {
                             ...
                             const aclIntArray* padding, const bool ceilMode, const aclTensor* out)
{
    ...
    OP_CHECK(((nInputPlane != 0) && (height != 0) && (width != 0)),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Expected tensor with optional 0 dim batch size,\
                       ...
                       const aclIntArray* padding, const bool ceilmode, const aclTensor* out)
{
    ...
    // 检查out的shape是否符合预期输出
    bool ret = CheckOutputShape(self, kernel, stride, padding, ceilmode, out);
    CHECK_RET(ret, false);

    return true;
}

static bool CheckValue(const aclTensor* self, const aclIntArray* kernel, const aclIntArray* stride,
...
static aclnnStatus CheckParams(const aclTensor* self, const aclIntArray* kernelSize, const aclIntArray* stride,
                               const aclIntArray* padding, const bool ceilMode, aclTensor* out, int8_t cubeMathType)
{
    // 检查参数是否为空指针
...
static aclnnStatus BuildGlobalPoolGraph(UniqueExecutor& uniqueExecutor, const aclTensor* self,
                                        const aclIntArray* kernelSize, const int64_t divisorOverride,
                                        const aclTensor* out)
{
    // 进行ReduceMean计算
...
static const aclTensor* GenWeightDynamic(const aclTensor* self, bool exclusivePads, const aclIntArray* kernelSize,
                                         int64_t divisorOverride, aclOpExecutor* executor)
{
    float areaFactor = 0;
    if (exclusivePads) {
        areaFactor = 1.0;
...
static aclnnStatus BuildAvgPoolGraph(UniqueExecutor& uniqueExecutor, const aclTensor* self,
                                     const aclIntArray* kernelSize, const aclIntArray* stride, const aclIntArray* pad,
                                     const int64_t ceilMode, const bool countIncludePad, const int64_t divisorOverride,
                                     const aclTensor* out)
{
    ...
    const aclTensor* poolingOut = l0op::Pooling5Hd(inputNC1HWC0, deviceTensor, mode, globalPooling, kernelSize, stride,
                                                   pad, ceilMode, FORMAT_NCHW_STR, uniqueExecutor.get());
...
static bool CheckBigKernel(const aclIntArray* kernelSize, const aclTensor* avgpoolIn, const aclTensor* out)
{
    ...
    bool bigKernel = ((*kernelSize)[DIM0] > BIG_KERNEL_SINGLE_LIMIT || (*kernelSize)[DIM1] > BIG_KERNEL_SINGLE_LIMIT ||
                      (*kernelSize)[DIM2] > BIG_KERNEL_SINGLE_LIMIT) &&
...
static aclnnStatus BuildAvgPool2dTo3dGraph(UniqueExecutor& uniqueExecutor, const aclTensor* self,
                                           ...
                                           const int64_t divisorOverride, const aclTensor* out)
{
...
static aclnnStatus BuildAiCpuGraph(UniqueExecutor& uniqueExecutor, const aclTensor* self, const aclIntArray* kernelSize,
                                   const aclIntArray* strides, const aclIntArray* paddings, const bool ceilMode,
                                   const bool exclusive, const int64_t divisorOverride, const aclTensor* out)
{
    ...
    auto avgPoolingOut = l0op::AvgPoolNchw(self, kernelSize, strides, paddingMode, paddings, dataFormat, globalPooling,
                                           ceilMode, exclusive, divisorOverride, uniqueExecutor.get());
...
static aclnnStatus ProcessDataTypeConvert(const aclTensor* self, const int8_t cubeMathType,
                                          UniqueExecutor& uniqueExecutor, const aclTensor* self4d,
                                          const aclTensor*& cast4dTensor)
{
    ...
    } else if (IsCubeSupportFp32() &&
               DataType::DT_BF16 == self->GetDataType()) { // 芯片支持FP32,等同判断支持bf16,且输入是bf16
...
aclnnStatus aclnnAvgPool2dGetWorkspaceSize(const aclTensor* self, const aclIntArray* kernelSize,
                                           ...
                                           const int8_t cubeMathType, aclTensor* out, uint64_t* workspaceSize,
                                           aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(
        aclnnAvgPool2d,
        DFX_IN(self, kernelSize, strides, paddings, ceilMode, countIncludePad, divisorOverride, cubeMathType),
        DFX_OUT(out));
    ...
    if (Ops::NN::AclnnUtil::IsRegbase()) {
        auto avgPoolingOut = l0op::AvgPoolV2(self4d, kernel4, stride4, paddingMode, paddings4, dataFormat,
                                             globalPooling, ceilMode, !countIncludePad, divisorOverride,
                                             uniqueExecutor.get());
    ...
    if (CheckGlobalPool(avgPoolInfo)) {
        ...
        if (CheckAvgPool(avgPoolInfo)) {
            ...
            if (!ceilMode)
                ceilModeInt = static_cast<int64_t>(1);
            ...
            CHECK_RET(ACLNN_SUCCESS == BuildAvgPoolGraph(uniqueExecutor, cast4dTensor, kernel4, stride4, paddings4,
                                                         ceilModeInt, countIncludePad, divisorOverride, out),
                      ACLNN_ERR_INNER);
        } else {
            // 构建走AICPU分支的计算图
            ...
            CHECK_RET(ACLNN_SUCCESS == BuildAiCpuGraph(uniqueExecutor, cast4dTensor, kernel4, stride4, paddings4,
                                                       ceilMode, exclusive, divisorOverride, out),
                      ACLNN_ERR_INNER);
        }
    } else {
        if (!CheckNeedChangeTo3D(avgPoolInfo) && CheckAvgPool(avgPoolInfo) && !ceilMode && cDims < cMaxDims) {
            ...
            CHECK_RET(ACLNN_SUCCESS == BuildAvgPool2dTo3dGraph(uniqueExecutor, self4d, kernelList, strideList, padList,
                                                               ceilMode, countIncludePad, divisorOverride, out),
                      ACLNN_ERR_INNER);
        }
    }
