...
/*!
 * \file test_geir_avg_pool3_d.cpp
 * \brief AvgPool3D GE IR test example
 */
...
#include "nn_other.h"
#include "../../op_graph/avg_pool3_d_proto.h"
                      ...
                      const vector<T>& values)
{
    ...
    if (size != values.size()) {
        printf("%s - ERROR - [XIR]: GenTensorData size mismatch, expected %zu, got %zu\n", GetTime().c_str(), size,
               values.size());
        return FAILED;
    }
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputValues)                                        \
    ...
    TensorDesc placeholder##inputIndex##_desc = TensorDesc(ge::Shape(placeholder##inputIndex##_shape), FORMAT_NDHWC, \
                                                           inputDtype);                                              \
    placeholder##inputIndex##_desc.SetPlacement(ge::kPlacementHost);                                                 \
    placeholder##inputIndex##_desc.SetFormat(FORMAT_NDHWC);                                                          \
    ...
    ret = GenTensorData(placeholder##inputIndex##_shape, tensor_placeholder##inputIndex,                             \
                        placeholder##inputIndex##_desc, inputValues);                                                \
    if (ret != SUCCESS) {                                                                                            \
    ...
    avg_pool3_d.set_input_##inputName(placeholder##inputIndex);                                                      \
    inputs.push_back(placeholder##inputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) avg_pool3_d.set_attr_##attrName(attrValue)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                           \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_NDHWC, outputDtype); \
    outputName##outputIndex##_desc_.SetFormat(FORMAT_NDHWC);                                                    \
    outputName##outputIndex##_desc_.SetOriginFormat(FORMAT_NDHWC);                                              \
    avg_pool3_d.update_output_desc_##outputName(outputName##outputIndex##_desc_)

int CreateOppInGraph(std::vector<ge::Tensor>& input, std::vector<Operator>& inputs, std::vector<Operator>& outputs,
                     Graph& graph)
{
    Status ret = SUCCESS;
    auto avg_pool3_d = op::AvgPool3D("avg_pool3_d");

    // Input: 5D tensor [N, D, H, W, C] in NDHWC format (default, most widely supported)
    // x_shape: N=1, D=4, H=4, W=4, C=1
    std::vector<int64_t> x_shape = {1, 4, 4, 4, 1};
    // y_shape: N=1, D_out=2, H_out=2, W_out=2, C=1
    //   D_out = (D - ksize_d) / stride_d + 1 = (4 - 2) / 2 + 1 = 2
    //   H_out = (H - ksize_h) / stride_h + 1 = (4 - 2) / 2 + 1 = 2
    //   W_out = (W - ksize_w) / stride_w + 1 = (4 - 2) / 2 + 1 = 2
    std::vector<int64_t> y_shape = {1, 2, 2, 2, 1};

    // Generate input data with value 1.0 (1*4*4*4*1 = 64 elements)
    std::vector<float> x_data(64, 1.0f);

    ADD_INPUT(1, x, DT_FLOAT, x_shape, x_data);

    // Set attributes (ksize/strides length 5 corresponds to [N, D, H, W, C] for NDHWC)
    // N and C dimensions must have ksize=1, stride=1 (no pooling on N/C)
    ADD_INPUT_ATTR(ksize, std::vector<int64_t>({1, 2, 2, 2, 1}));
    ADD_INPUT_ATTR(strides, std::vector<int64_t>({1, 2, 2, 2, 1}));
    ADD_INPUT_ATTR(pads, std::vector<int64_t>({0, 0, 0}));
    ADD_INPUT_ATTR(ceil_mode, false);
    ADD_INPUT_ATTR(count_include_pad, true);
    ADD_INPUT_ATTR(divisor_override, static_cast<int64_t>(0));
    ADD_INPUT_ATTR(data_format, "NDHWC");

    ADD_OUTPUT(1, y, DT_FLOAT, y_shape);

    outputs.push_back(avg_pool3_d);
...
int main(int argc, char* argv[])
{
    ...
    const char* graph_name = "tc_ge_irrun_test_avg_pool3_d";
