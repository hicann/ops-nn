...
/*!
 * \file pool_2d_nhwc_small_kernel_data_move.h
 * \brief AvgPool/MaxPoolV3 NHWC small kernel 共用的多行输入搬入与结果搬出接口，区分无 pad 与 pad 两种模板。
 */

#ifndef POOL_UTILS_ARCH35_DATA_MOVE_POOL_2D_NHWC_SMALL_KERNEL_DATA_MOVE_H_
#define POOL_UTILS_ARCH35_DATA_MOVE_POOL_2D_NHWC_SMALL_KERNEL_DATA_MOVE_H_
...
// 切分模式取值，与 AvgPool/MaxPoolV3 算子侧 tiling 下发的 splitMode 取值保持一致
...
__aicore__ inline void CopyInMultiRows(AscendC::TQue<AscendC::QuePosition::VECIN, 2>& inputQue,
                                       ...
                                       int64_t blockCount, int64_t blockLen, uint32_t inColsElms, uint32_t channels,
                                       int64_t splitMode, int64_t wInDim)
{
    ...
    if (splitMode != SPLIT_COLS) {
        ...
        extParams.srcStride = (wInDim - blockLen) * channels * sizeof(T);
                                       ...
                                       int64_t wInDim, int64_t hInDim)
{
    ...
    if (n > 1) {
        ...
        loopParams.loop1SrcStride = wInDim * hInDim * channels * sizeof(T);
...
#endif // POOL_UTILS_ARCH35_DATA_MOVE_POOL_2D_NHWC_SMALL_KERNEL_DATA_MOVE_H_
