...
#include "nn_other.h"
#include "../../op_graph/max_pool_grad_grad_with_argmax_proto.h"
...
{
    Status ret = SUCCESS;
    auto add1 = op::MaxPoolGradGradWithArgmax("max_pool_grad_grad_with_argmax");
    vector<vector<int64_t>> shapes = {
        {1, 4, 4, 1}, // x shape (input)
        {1, 4, 4, 1}, // grad shape (gradient input, same as x)
        {1, 2, 2, 1}  // argmax shape (pooling output)
    };
    ...
    ADD_INPUT(2, grad, inDtype, shapes[1]);
    ADD_INPUT(3, argmax, DT_INT32, shapes[2]);
    ADD_OUTPUT(4, y, inDtype, shapes[2]);
...
int main(int argc, char* argv[])
{
    ...
    Graph graph("max_pool_grad_grad_with_argmax_graph");
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        string input_file = "./max_pool_grad_grad_with_argmax_npu_input_" + std::to_string(i) + ".bin";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./max_pool_grad_grad_with_argmax_npu_output_" + std::to_string(i) + ".bin";
        ...
        std::cout << "output y (gradient result):" << std::endl;
