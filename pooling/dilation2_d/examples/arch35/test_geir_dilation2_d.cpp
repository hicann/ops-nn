...
#include "nn_other.h"
#include "../../op_graph/dilation2d_proto.h"
...
{
    Status ret = SUCCESS;
    // Create dilation2d operator
    auto add1 = op::Dilation2d("dilation1");
    add1.set_attr_strides({1, 1, 1, 1});
    add1.set_attr_rates({1, 1, 1, 1});
    add1.set_attr_padding_mode("SAME");
    add1.set_attr_data_format("NHWC");

    std::vector<int64_t> xShape = {1, 8, 8, 4};
    std::vector<int64_t> filterShape = {3, 3, 4};
    std::vector<int64_t> yShape = {1, 8, 8, 4};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, filter, inDtype, filterShape);
