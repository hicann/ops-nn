...
/*!
 * \file test_geir_avg_pool3_d_grad.cpp
 * \brief AvgPool3DGrad GE IR test example
 */
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputValues)                                     \
    ...
    avg_pool3_d_grad.set_input_##inputName(placeholder##inputIndex);                                              \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape, inputValues)                              \
    ...
    ret = GenOnesData(placeholder##intputIndex##_shape, tensor_placeholder##intputIndex,                            \
                      placeholder##intputIndex##_desc, intputDtype, inputValues);                                   \
    if (ret != SUCCESS) {                                                                                           \
        printf("%s - ERROR - [XIR]: Generate input data failed\n", GetTime().c_str());                              \
        return FAILED;                                                                                              \
    }                                                                                                               \
    ...
    avg_pool3_d_grad.set_input_##intputName(placeholder##intputIndex);                                              \
    avg_pool3_d_grad.update_input_desc_##intputName(placeholder##intputIndex##_desc);                               \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) avg_pool3_d_grad.set_attr_##attrName(attrValue)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    avg_pool3_d_grad.update_output_desc_##outputName(outputName##outputIndex##_desc_)

int CreateOppInGraph(std::vector<ge::Tensor>& input, std::vector<Operator>& inputs, std::vector<Operator>& outputs,
                     Graph& graph)
{
    Status ret = SUCCESS;
    auto avg_pool3_d_grad = op::AvgPool3DGrad("avg_pool3_d_grad");

    // Input 0: orig_input_shape (1D tensor describing original input shape)
    std::vector<int64_t> orig_input_shape_shape = {5};
    std::vector<int32_t> orig_input_shape_data = {1, 1, 4, 4, 4};

    // Input 1: grads (5D tensor matching forward output shape)
    std::vector<int64_t> grads_shape = {1, 1, 2, 2, 2};
    std::vector<float> grads_data(8, 1.0f);

    // Output: gradient w.r.t. input (5D tensor matching forward input shape)
    std::vector<int64_t> y_shape = {1, 1, 4, 4, 4};

    ADD_CONST_INPUT(1, orig_input_shape, DT_INT32, orig_input_shape_shape, orig_input_shape_data);
    ADD_INPUT(2, grads, DT_FLOAT, grads_shape, grads_data);

    // Set attributes
    ADD_INPUT_ATTR(ksize, std::vector<int64_t>({1, 1, 2, 2, 2}));
    ADD_INPUT_ATTR(strides, std::vector<int64_t>({1, 1, 2, 2, 2}));
    ...
    ADD_INPUT_ATTR(data_format, "NCDHW");

    ADD_OUTPUT(1, output, DT_FLOAT, y_shape);

    outputs.push_back(avg_pool3_d_grad);
...
int main(int argc, char* argv[])
{
    ...
    const char* graph_name = "tc_ge_irrun_test_avg_pool3_d_grad";
