...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnBidirectionLSTMV2GetWorkspaceSize** is called to obtain the input parameters and compute the required workspace size based on the process. Then, **aclnnBidirectionLSTMV2** is called to perform computation.
...
- **Parameters:**
  ...
  <thead>
   ...
   <tbody>
      ...
      <td><ul><li>Empty tensors are supported. </li><li>When packed is set to true, the shape supports 3D (time_step, batch_size, input_size)</li><li>.When packed is set to true, the shape supports 2D (total_batch_size, input_size). `time_step` indicates the time dimension. `batch_size` indicates the number of batches to be processed at each moment. `input_size` indicates the number of input features. `total_batch_size` indicates the total number of batches at all moments when the batch size at each moment is not aligned.</li></ul></td>
      <td>FLOAT16</td>
      <td>ND</td>
      <td>3</td>
      <td>√</td>
      ...
      <td>Input</td>
      <td>Batch size that actually participates in computation in each iteration.</td>
      <td><ul><li>Empty tensors are supported. </li><li>The shape supports 1D (time_step).</li></ul></td>
      <td>INT</td>
      <td>ND</td>
      <td>1</td>
      <td>√</td>
      ...
      <td>packed</td>
      <td>Input</td>
      <td>Whether input x is compressed.</td>
      <td>This parameter takes effect only when batchSizeOptional is not empty.</td>
      <td>BOOL</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      ...
      <td><ul><li>When **bidirection** is set to **false**, the shape supports 3D (time_step, batch_size, input_size) if **packed** is set to **false** and 2D (total_batch_size, input_size) if **packed** is set to **true**. </li><li>When **bidirection** is set to **true**, the shape supports 3D (time_step, batch_size, 2 * input_size) if **packed** is set to **false** and 2D (total_batch_size, 2 * input_size) if **packed** is set to **true**.</li></ul></td>
      <td>FLOAT16</td>
      <td>ND</td>
      <td>3</td>
      <td>√</td>
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API **aclnnBidirectionLSTMV2GetWorkspaceSize**.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
int main() {
  ...
  // Call the first-phase API of aclnnBidirectionLSTMV2.
  ...
  // Call the second-phase API of aclnnBidirectionLSTMV2.
