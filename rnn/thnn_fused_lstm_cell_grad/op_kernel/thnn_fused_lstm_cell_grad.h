...
public:
    ...
    __aicore__ inline void Init(GM_ADDR dhy, GM_ADDR dc, GM_ADDR cx, GM_ADDR cy, GM_ADDR storage, GM_ADDR dgates,
                                GM_ADDR dc_prev, GM_ADDR db,
                                const ThnnFusedLstmCellGradTilingData* __restrict rnnTiling, GM_ADDR workspace,
                                TPipe* inputPipe)
    {
        pipe = inputPipe;
...
private:
    ...
    __aicore__ inline void CastDgatesBack()
    {
        ...
        for (int64_t loopIndex = 0; loopIndex < totalLoops; loopIndex++) {
            if (GetBlockIdx() == loopIndex % GetBlockNum()) {
                ...
                DataCopyPad(dgatesCastTensor, outputGm.dgatesTempGm[loopIndex * maxCalcNum], copyParamsIn,
                            padInFloatParams);
                MTE2ToVSync();
                                             ...
                                             GM_ADDR dgates, GM_ADDR dc_prev, GM_ADDR db, GM_ADDR workspace)
    {
        ...
        inputGm.cyGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(cy), tiling.batch * tiling.hiddenSize);
        inputGm.dhyGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(dhy), tiling.batch * tiling.hiddenSize);
        ...
        outputGm.dcPrevGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(dc_prev), tiling.batch * tiling.hiddenSize);
        outputGm.dgatesGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(dgates),
                                          tiling.batch * LSTM_GATE_SIZE * tiling.hiddenSize);
        int64_t reduceOffset = (tiling.isBias == 1 && !std::is_same<T, float>::value) ?
                                   tiling.batch * tiling.hiddenSize * LSTM_GATE_SIZE * FLOAT_BYTES :
                                   0;
        ...
        if (tiling.batch > tiling.maxReduceNumOnce) {
            outputGm.reduceTempGm.SetGlobalBuffer(
                reinterpret_cast<__gm__ float*>(workspace + reduceOffset),
                CeilDiv(tiling.batch, tiling.maxReduceNumOnce) * tiling.hiddenSize * LSTM_GATE_SIZE);
        }
    }

    template <typename copyType>
    __aicore__ inline void CopyOutputGate(const GlobalTensor<copyType>& gm, const LocalTensor<copyType>& ub,
                                          int64_t gateIdx, int64_t nShapeAligned)
    {
    ...
    template <typename copyType>
    __aicore__ inline void CopyGateInput(const LocalTensor<copyType>& ub, const GlobalTensor<copyType>& gm,
                                         int64_t gateIndex)
    {
        DataCopyExtParams dataCopyParams;
        dataCopyParams.blockCount = block_.mShape;
    ...
    __aicore__ inline void LoadDgatesInputData(int64_t calcSizeAlign)
    {
        if constexpr (std::is_same<T, float>::value) {
            ...
            Cast(dcnextTensor, dcnextTensor.template ReinterpretCast<T>()[calcSizeAlign], RoundMode::CAST_NONE,
                 calcSizeAlign);
            Cast(dhyTensor, dhyTensor.template ReinterpretCast<T>()[calcSizeAlign], RoundMode::CAST_NONE,
                 calcSizeAlign);
            ...
            Cast(cPreTensor, cPreTensor.template ReinterpretCast<T>()[calcSizeAlign], RoundMode::CAST_NONE,
                 calcSizeAlign);
            PipeBarrier<PIPE_V>();
        }
    }
    ...
    __aicore__ inline void StoreDgatesResults(int64_t nShapeAligned, int64_t calcSizeAlign)
    {
        ...
        if ((tiling.isBias == 1 && !std::is_same<T, float>::value)) {
            ...
            auto roundMode = std::is_same<T, bfloat16_t>::value ? RoundMode::CAST_RINT : RoundMode::CAST_NONE;
            PipeBarrier<PIPE_V>();
        ...
        if (mCoreIndex < tiling.nReduceCnt) {
            ...
            for (int64_t nCntIndex = 0; nCntIndex < incoreNCnt; nCntIndex++) {
                ...
                while (reduceNumLeft >= 1) {
                    ...
                    SubProcessReduce(reduceNumLeft, reduceBlockSize, reduceBlockSizeTail, reduceSubRepeatTimes,
                                     loopTime);
                    loopTime++;
                }
            }
    ...
    __aicore__ inline void SubProcessReduce(int64_t reduceNumLeft, int64_t reduceBlockSize, int64_t reduceBlockSizeTail,
                                            int64_t reduceSubRepeatTimes, int64_t loopTime)
    {
