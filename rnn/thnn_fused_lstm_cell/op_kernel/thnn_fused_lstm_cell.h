...
public:
    __aicore__ inline ThnnFusedLstmCell(const ThnnFusedLstmCellTilingData& tilingData) : tilingData(tilingData) {}

    __aicore__ inline void Init(GM_ADDR inputGates, GM_ADDR hiddenGates, GM_ADDR cx, GM_ADDR inputBias,
                                GM_ADDR hiddenBias, GM_ADDR hy, GM_ADDR cy, GM_ADDR storage)
    {
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint64_t taskIdx = taskIdxStart; taskIdx < taskIdxStart + tilingData.taskSingle; taskIdx++) {
            if (taskIdx >= tilingData.taskTotal)
                return;
    ...
    {
        DataCopyExtParams copyParams{1, static_cast<uint32_t>(info.x * sizeof(dtype)), 0, 0, 0};
