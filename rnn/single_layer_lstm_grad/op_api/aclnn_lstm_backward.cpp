...
static const std::initializer_list<op::DataType> BATCH_SIZES_DTYPE_SUPPORT_LIST = {op::DataType::DT_INT64};
...
static std::array<const aclTensor*, OUT_NUM> ExecLstmBackward(
    const aclTensor* input, const aclTensor* initH, const aclTensor* initC, const aclTensor* weight,
    const aclTensor* bias, const aclTensor* dy, const aclTensor* dh, const aclTensor* dc, const aclTensor* seqLength,
    const aclTensor* i, const aclTensor* j, const aclTensor* f, const aclTensor* o, const aclTensor* h,
    const aclTensor* c, const aclTensor* tanhc, aclOpExecutor* executor, bool flagDirection = false)
{
    ...
    CHECK_RET(result[RESULT_WEIGHT_GRAD_INDEX] != nullptr && result[RESULT_BIAS_GRAD_INDEX] != nullptr &&
                  result[RESULT_INPUT_GRAD_INDEX] != nullptr && result[RESULT_HIDDEN_GRAD_INDEX] != nullptr &&
                  result[RESULT_CELL_GRAD_INDEX] != nullptr,
              nullptrRes);
    return result;
}
...
static std::array<const aclTensor*, HC_TENSOR_COUNT * 2> CreateSliceTensors(int64_t layerIndex,
                                                                            const aclTensor* initHMultiLayer,
                                                                            const aclTensor* initCMultiLayer,
                                                                            const aclTensor* dh, const aclTensor* dc,
                                                                            aclOpExecutor* executor)
{
...
LstmBackwardSingleLayerDirec(const aclTensor* input, const aclTensor* initHMultiLayer, const aclTensor* initCMultiLayer,
                             const aclTensorList* params, const aclTensor* dy, const aclTensor* dh, const aclTensor* dc,
                             const aclTensor* seqLength, const aclTensorList* i, const aclTensorList* j,
                             const aclTensorList* f, const aclTensorList* o, const aclTensorList* h,
                             const aclTensorList* c, const aclTensorList* tanhc, int64_t layersTemp, int64_t numLayers,
                             bool hasBias, int64_t paramNumPerLayer, aclOpExecutor* executor)
{
    ...
    if (numLayers > 1) {
        ...
        CHECK_RET(sliceTensors[SLICE_INIT_H_INDEX] != nullptr && sliceTensors[SLICE_INIT_C_INDEX] != nullptr &&
                      sliceTensors[SLICE_DH_INDEX] != nullptr && sliceTensors[SLICE_DC_INDEX] != nullptr,
                  nullptrRes);
    ...
    auto result = ExecLstmBackward(inputCur, initHCur, initCCur, weightCur, biasCur, dy, dhCur, dcCur, seqLength,
                                   (*i)[layersTemp - 1], (*j)[layersTemp - 1], (*f)[layersTemp - 1],
                                   (*o)[layersTemp - 1], (*h)[layersTemp - 1], (*c)[layersTemp - 1],
                                   (*tanhc)[layersTemp - 1], executor, false);
...
static bool CheckTupleNotNull(
    const std::tuple<const aclTensor*, std::vector<const aclTensor*>, std::vector<const aclTensor*>,
                     std::vector<const aclTensor*>, std::vector<const aclTensor*>>& tuple)
{
    if (!std::get<0>(tuple)) {
        return false;
    }
...
static std::tuple<const aclTensor*, std::vector<const aclTensor*>, std::vector<const aclTensor*>,
                  std::vector<const aclTensor*>, std::vector<const aclTensor*>>
LstmBackwardMultiLayerDirec(const aclTensor* input, const aclTensor* initHMultiLayer, const aclTensor* initCMultiLayer,
                            const aclTensorList* params, const aclTensor* dy, const aclTensor* dh, const aclTensor* dc,
                            const aclTensor* seqLength, const aclTensorList* i, const aclTensorList* j,
                            const aclTensorList* f, const aclTensorList* o, const aclTensorList* h,
                            const aclTensorList* c, const aclTensorList* tanhc, int64_t numLayers, int64_t layersTemp,
                            bool hasBias, int64_t paramNumPerLayer, aclOpExecutor* executor)
{
    ...
    if (layersTemp == numLayers) {
        auto lastLayerOutput = LstmBackwardSingleLayerDirec(input, initHMultiLayer, initCMultiLayer, params, dy, dh, dc,
                                                            seqLength, i, j, f, o, h, c, tanhc, layersTemp, numLayers,
                                                            hasBias, paramNumPerLayer, executor);
        CHECK_RET(CheckTupleNotNull(lastLayerOutput), nullptrRes);
        return lastLayerOutput;
    } else {
        std::tie(lastLayerDy, lastLayerDhPrevVector, lastLayerDcPrevVector, lastLayerDwVector,
                 lastLayerDbVector) = LstmBackwardMultiLayerDirec(input, initHMultiLayer, initCMultiLayer, params, dy,
                                                                  dh, dc, seqLength, i, j, f, o, h, c, tanhc, numLayers,
                                                                  layersTemp + 1, hasBias, paramNumPerLayer, executor);
        CHECK_RET(CheckTupleNotNull(std::tie(lastLayerDy, lastLayerDhPrevVector, lastLayerDcPrevVector,
                                             lastLayerDwVector, lastLayerDbVector)),
                  nullptrRes);
    }
    auto weightBias = GetWeightBiasFromParams(params, hasBias, false, paramNumPerLayer, layersTemp, executor);
    const aclTensor* weightCur = weightBias[0];
    ...
    auto result = ExecLstmBackward(inputCur, sliceTensors[SLICE_INIT_H_INDEX], sliceTensors[SLICE_INIT_C_INDEX],
                                   weightCur, biasCur, lastLayerDy, sliceTensors[SLICE_DH_INDEX],
                                   sliceTensors[SLICE_DC_INDEX], seqLength, (*i)[layersTemp - 1], (*j)[layersTemp - 1],
                                   (*f)[layersTemp - 1], (*o)[layersTemp - 1], (*h)[layersTemp - 1],
...
static std::array<const aclTensor*, 5> CreateBidDirectionTensors(const aclTensor* initHMultiLayer,
                                                                 const aclTensor* initCMultiLayer, const aclTensor* dh,
                                                                 const aclTensor* dc, const aclTensor* dy,
...
static const aclTensor* CreateInputTensor(const aclTensor* input, const aclTensorList* h, int64_t layersTemp,
                                          aclOpExecutor* executor)
{
    if (layersTemp == 1) {
        return input;
...
static std::tuple<const aclTensor*, std::vector<const aclTensor*>, std::vector<const aclTensor*>,
                  std::vector<const aclTensor*>, std::vector<const aclTensor*>>
MergeResults(std::array<const aclTensor*, 5> resultForward, std::array<const aclTensor*, 5> resultBackward,
             ...
             std::vector<const aclTensor*>& dwVector, std::vector<const aclTensor*>& dbVector, aclOpExecutor* executor)
{
    auto nullptrRes = std::make_tuple(nullptr, std::vector<const aclTensor*>(), std::vector<const aclTensor*>(),
                                      std::vector<const aclTensor*>(), std::vector<const aclTensor*>());
    std::vector<const aclTensor*> catDhPrevVector = {resultForward[RESULT_HIDDEN_GRAD_INDEX],
                                                     resultBackward[RESULT_HIDDEN_GRAD_INDEX]};
    std::vector<const aclTensor*> catDcPrevVector = {resultForward[RESULT_CELL_GRAD_INDEX],
                                                     resultBackward[RESULT_CELL_GRAD_INDEX]};
...
LstmBackwardSingleLayerBidirec(const aclTensor* input, const aclTensor* initHMultiLayer,
                               const aclTensor* initCMultiLayer, const aclTensorList* params, const aclTensor* dy,
                               const aclTensor* dh, const aclTensor* dc, const aclTensor* seqLength,
                               ...
                               const aclTensorList* tanhc, int64_t layersTemp, bool hasBias, int64_t paramNumPerLayer,
                               aclOpExecutor* executor)
{
    auto nullptrRes = std::make_tuple(nullptr, std::vector<const aclTensor*>(), std::vector<const aclTensor*>(),
                                      std::vector<const aclTensor*>(), std::vector<const aclTensor*>());
    ...
    auto mergedResult = MergeResults(resultForward, resultBackward, dhPrevVector, dcPrevVector, dwVector, dbVector,
                                     executor);
    return mergedResult;
}
...
static std::tuple<const aclTensor*, std::vector<const aclTensor*>, std::vector<const aclTensor*>,
                  std::vector<const aclTensor*>, std::vector<const aclTensor*>>
LstmBackwardMultiLayerBidirec(const aclTensor* input, const aclTensor* initHMultiLayer,
                              const aclTensor* initCMultiLayer, const aclTensorList* params, const aclTensor* dy,
                              const aclTensor* dh, const aclTensor* dc, const aclTensor* seqLength,
                              const aclTensorList* i, const aclTensorList* j, const aclTensorList* f,
                              const aclTensorList* o, const aclTensorList* h, const aclTensorList* c,
                              const aclTensorList* tanhc, int64_t numLayers, int64_t layersTemp, bool hasBias,
                              int64_t paramNumPerLayer, aclOpExecutor* executor)
{
    const aclTensor* lastLayerDy = nullptr;
    std::vector<const aclTensor*> lastLayerDhPrevVector{};
    std::vector<const aclTensor*> lastLayerDcPrevVector{};
    ...
    if (layersTemp == numLayers) {
        auto lastLayerOutput = LstmBackwardSingleLayerBidirec(input, initHMultiLayer, initCMultiLayer, params, dy, dh,
                                                              dc, seqLength, i, j, f, o, h, c, tanhc, layersTemp,
                                                              hasBias, paramNumPerLayer, executor);
        CHECK_RET(CheckTupleNotNull(lastLayerOutput), nullptrRes);
        return lastLayerOutput;
    } else {
        // 递归调用处理下一层
        std::tie(lastLayerDy, lastLayerDhPrevVector, lastLayerDcPrevVector, lastLayerDwVector,
                 lastLayerDbVector) = LstmBackwardMultiLayerBidirec(input, initHMultiLayer, initCMultiLayer, params, dy,
                                                                    dh, dc, seqLength, i, j, f, o, h, c, tanhc,
                                                                    numLayers, layersTemp + 1, hasBias,
                                                                    paramNumPerLayer, executor);
        CHECK_RET(CheckTupleNotNull(std::tie(lastLayerDy, lastLayerDhPrevVector, lastLayerDcPrevVector,
                                             lastLayerDwVector, lastLayerDbVector)),
                  nullptrRes);
    }
...
static bool CheckNotNull(const aclTensor* input, const aclTensorList* hc, const aclTensorList* params,
                         const aclTensorList* i, const aclTensorList* j, const aclTensorList* f, const aclTensorList* o,
                         const aclTensorList* h, const aclTensorList* c, const aclTensorList* tanhc,
                         const aclTensor* dx, const aclTensor* dhPrev, const aclTensor* dcPrev,
                         const aclTensorList* dparams, bool hasBias, int64_t numLayers, bool bidirectional)
{
    ...
    return CheckTensorListNotNull(hc) && CheckTensorListNotNull(i) && CheckTensorListNotNull(j) &&
           CheckTensorListNotNull(f) && CheckTensorListNotNull(o) && CheckTensorListNotNull(h) &&
           CheckTensorListNotNull(c) && CheckTensorListNotNull(tanhc) && CheckTensorListNotNull(dparams);
}
...
static bool CheckTensorListsFormat(const aclTensorList* hc, const aclTensorList* params, const aclTensorList* i,
                                   const aclTensorList* j, const aclTensorList* f, const aclTensorList* o,
                                   const aclTensorList* h, const aclTensorList* c, const aclTensorList* tanhc,
                                   const aclTensorList* dparams)
{
    if (!CheckTensorListFormat(hc, "hc", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(params, "params", Format::FORMAT_ND))
        return false;
    if (!CheckTensorListFormat(i, "i", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(j, "j", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(f, "f", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(o, "o", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(h, "h", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(c, "c", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(tanhc, "tanhc", Format::FORMAT_NCL))
        return false;
    if (!CheckTensorListFormat(dparams, "dparams", Format::FORMAT_ND))
        return false;
    return true;
}

static bool CheckFormatValid(const aclTensor* input, const aclTensorList* hc, const aclTensorList* params,
                             const aclTensor* dy, const aclTensor* dh, const aclTensor* dc, const aclTensorList* i,
                             const aclTensorList* j, const aclTensorList* f, const aclTensorList* o,
                             const aclTensorList* h, const aclTensorList* c, const aclTensorList* tanhc,
                             const aclTensor* dx, const aclTensor* dhPrev, const aclTensor* dcPrev,
                             const aclTensorList* dparams, const aclTensor* batchSizes = nullptr)
{
...
// 检查张量列表的数据类型支持和一致性
static bool CheckTensorListDtype(const aclTensorList* tensors, const char* listName, ge::DataType baseDtype)
{
    for (uint64_t idx = 0; idx < tensors->Size(); idx++) {
        ...
        // 检查数据类型一致性
        if (tensor->GetDataType() != baseDtype) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "%s tensor %lu dtype inconsistent, expected: %s, actual: %s.", listName,
                    idx, op::ToString(baseDtype).GetString(), op::ToString(tensor->GetDataType()).GetString());
            return false;
        }
    }
    return true;
...
static bool CheckDtypeValid(const aclTensor* input, const aclTensorList* hc, const aclTensorList* params,
                            const aclTensor* dy, const aclTensor* dh, const aclTensor* dc, const aclTensorList* i,
                            const aclTensorList* j, const aclTensorList* f, const aclTensorList* o,
                            const aclTensorList* h, const aclTensorList* c, const aclTensorList* tanhc,
                            const aclTensor* dx, const aclTensor* dhPrev, const aclTensor* dcPrev,
                            const aclTensorList* dparams, const aclTensor* batchSizes = nullptr)
{
    ...
    if (batchSizes != nullptr && batchSizes->GetDataType() != op::DataType::DT_INT64) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "batchSizes tensor dtype inconsistent, expected: %s, actual: %s.",
                op::ToString(op::DataType::DT_INT64).GetString(), op::ToString(batchSizes->GetDataType()).GetString());
        return false;
    }
...
static bool ValidateLayerWithBiasAndBidir(const aclTensorList* params, const aclTensorList* dparams, int64_t layerIdx,
                                          ...
                                          const std::vector<int64_t>& biasDim)
{
    ...
    ok = ok && ValidateInputShape((*params)[stride * layerIdx + half + WEIGHT_INPUT_INDEX], weightInputDim, "wi") &&
...
static bool ValidateLayerWithBiasOnly(const aclTensorList* params, const aclTensorList* dparams, int64_t layerIdx,
                                      const std::vector<int64_t>& weightInputDim,
                                      const std::vector<int64_t>& weightHiddenDim, const std::vector<int64_t>& biasDim)
{
...
static bool ValidateLayerWithBidirOnly(const aclTensorList* params, const aclTensorList* dparams, int64_t layerIdx,
                                       const std::vector<int64_t>& weightInputDim,
                                       const std::vector<int64_t>& weightHiddenDim)
{
...
static bool ValidateLayerNoBiasNoBidir(const aclTensorList* params, const aclTensorList* dparams, int64_t layerIdx,
                                       const std::vector<int64_t>& weightInputDim,
                                       const std::vector<int64_t>& weightHiddenDim)
{
...
static bool CheckGateTensorsForIndex(const aclTensorList* i, const aclTensorList* j, const aclTensorList* f,
                                     const aclTensorList* o, const aclTensorList* h, const aclTensorList* c,
                                     const aclTensorList* tanhc, int64_t idx, const std::vector<int64_t>& hiddenDim)
{
    if (!ValidateInputShape((*i)[idx], hiddenDim, "i"))
        return false;
    if (!ValidateInputShape((*j)[idx], hiddenDim, "j"))
        return false;
    if (!ValidateInputShape((*f)[idx], hiddenDim, "f"))
        return false;
    if (!ValidateInputShape((*o)[idx], hiddenDim, "o"))
        return false;
    if (!ValidateInputShape((*h)[idx], hiddenDim, "h"))
        return false;
    if (!ValidateInputShape((*c)[idx], hiddenDim, "c"))
        return false;
    if (!ValidateInputShape((*tanhc)[idx], hiddenDim, "tanhc"))
        return false;
    return true;
}

static bool ValidateCoreShapes(int64_t numLayers, const aclTensor* input, const std::vector<int64_t>& inputDim,
                               const aclTensorList* hc, const std::vector<int64_t>& inithDim, const aclTensor* dx,
                               const aclTensor* dhPrev, const aclTensor* dcPrev, const aclTensor* dy,
                               const std::vector<int64_t>& outHiddenDim, const aclTensor* dh, const aclTensor* dc)
{
    if (numLayers <= 0)
        return false;
    ...
    if (dy && !ValidateInputShape(dy, outHiddenDim, "dy"))
        return false;
    if (dh && !ValidateInputShape(dh, inithDim, "dh"))
        return false;
    if (dc && !ValidateInputShape(dc, inithDim, "dc"))
        return false;
...
static bool CheckShapeValid(const aclTensor* input, const aclTensorList* hc, const aclTensorList* params,
                            ...
                            const aclTensor* dx, const aclTensor* dhPrev, const aclTensor* dcPrev,
                            const aclTensorList* dparams, bool hasBias, int64_t numLayers, bool bidirectional,
                            bool batchFirst, const aclTensor* batchSizes = nullptr)
{
    ...
    int64_t timeStep = hasSeqlength ? batchSizes->GetViewShape()[SEQUENCE_DIM] :
                       batchFirst   ? inputShape[BATCH_DIM] :
                                      inputShape[SEQUENCE_DIM];
    ...
    const std::vector<int64_t> outHiddenDim = hasSeqlength ? std::vector<int64_t>{timeStep * batch, hiddenSize * bid} :
...
static aclnnStatus CheckParams(const aclTensor* input, const aclTensorList* hc, const aclTensorList* params,
                               const aclTensor* dy, const aclTensor* dh, const aclTensor* dc, const aclTensorList* i,
                               const aclTensorList* j, const aclTensorList* f, const aclTensorList* o,
                               const aclTensorList* h, const aclTensorList* c, const aclTensorList* tanhc,
                               const aclTensor* dx, const aclTensor* dhPrev, const aclTensor* dcPrev,
...
{
    // 1. 检查参数是否为空指针
    CHECK_RET(CheckNotNull(input, hc, params, i, j, f, o, h, c, tanhc, dx, dhPrev, dcPrev, dparams, hasBias, numLayers,
                           bidirectional),
              ACLNN_ERR_PARAM_NULLPTR);
    // 2. 检查输入的数据类型是否在API支持的数据类型范围之内，需要根据api定义校验
    CHECK_RET(CheckDtypeValid(input, hc, params, dy, dh, dc, i, j, f, o, h, c, tanhc, dx, dhPrev, dcPrev, dparams,
                              batchSizes),
              ACLNN_ERR_PARAM_INVALID);
    // 3. 检查shape是否满足约束
    CHECK_RET(CheckShapeValid(input, hc, params, dy, dh, dc, i, j, f, o, h, c, tanhc, dx, dhPrev, dcPrev, dparams,
                              hasBias, numLayers, bidirectional, batchFirst, batchSizes),
              ACLNN_ERR_PARAM_INVALID);
    // 4. 检查format是否满足约束
...
static bool EmptyCheck(const aclTensor* input, const aclTensorList* hc, const aclTensorList* params,
                       ...
                       const aclTensorList* f, const aclTensorList* o, const aclTensorList* h, const aclTensorList* c,
                       const aclTensorList* tanhc, const aclTensor* batchSizes = nullptr)
{
    ...
    if (!checkTensorList(hc) || !checkTensorList(params) || !checkTensorList(i) || !checkTensorList(j) ||
        !checkTensorList(f) || !checkTensorList(o) || !checkTensorList(h) || !checkTensorList(c) ||
...
static std::tuple<const aclTensor*, const aclTensor*, std::vector<const aclTensor*>> GetLstmGradExceptDx(
    ...
    const aclTensor* input, const aclTensor* dhPrevOut, bool bidirectional, bool hasBias, int64_t numLayers,
    aclOpExecutor* executor)
{
    auto nullptrRes = std::make_tuple(nullptr, nullptr, std::vector<const aclTensor*>());
...
static aclnnStatus CreateContiguousTensors(const aclTensor* input, const aclTensorList* hx, const aclTensorList* params,
                                           const aclTensor* dy, const aclTensor* dh, const aclTensor* dc,
                                           const aclTensorList* i, const aclTensorList* g, const aclTensorList* f,
                                           const aclTensorList* o, const aclTensorList* h, const aclTensorList* c,
                                           const aclTensorList* tanhc, aclOpExecutor* executor,
                                           LSTMContinuousTensors* output, const aclTensor* batchSizes = nullptr)
{
...
static aclnnStatus ExecLstmInputBackward(const LSTMContinuousTensors* allInput, aclTensor* dxOut, aclTensor* dhPrevOut,
                                         aclTensor* dcPrevOut, aclTensorList* dparamsOut, bool batchFirst,
                                         bool bidirectional, bool hasBias, int64_t numLayers, aclOpExecutor* executor)
{
    std::tuple<const aclTensor*, std::vector<const aclTensor*>, std::vector<const aclTensor*>,
               std::vector<const aclTensor*>, std::vector<const aclTensor*>>
        output;
    ...
    int64_t paramNumPerLayer = (hasBias && bidirectional) ? NUM_WITH_B_AND_BID :
                               (hasBias || bidirectional) ? NUM_WITH_B_OR_BID :
                                                            NUM_NO_B_NO_BIDIR;
    if (!bidirectional) {
        output = LstmBackwardMultiLayerDirec(
            inputTranspose, (*(allInput->hxContiguous))[0], (*(allInput->hxContiguous))[1], allInput->paramsContiguous,
            dyTranspose, allInput->dhContiguous, allInput->dcContiguous, nullptr, allInput->iContiguous,
            allInput->gContiguous, allInput->fContiguous, allInput->oContiguous, allInput->hContiguous,
            allInput->cContiguous, allInput->tanhcContiguous, numLayers, 1, hasBias, paramNumPerLayer, executor);
    } else {
        output = LstmBackwardMultiLayerBidirec(
            inputTranspose, (*allInput->hxContiguous)[0], (*allInput->hxContiguous)[1], allInput->paramsContiguous,
            dyTranspose, allInput->dhContiguous, allInput->dcContiguous, nullptr, allInput->iContiguous,
            allInput->gContiguous, allInput->fContiguous, allInput->oContiguous, allInput->hContiguous,
            allInput->cContiguous, allInput->tanhcContiguous, numLayers, 1, hasBias, paramNumPerLayer, executor);
                                        ...
                                        bool hasBias, int64_t numLayers, aclOpExecutor* executor)
{
    ...
    if (!bidirectional) {
        output = LstmBackwardMultiLayerDirec(
            input, (*(allInput->hxContiguous))[0], (*(allInput->hxContiguous))[1], allInput->paramsContiguous,
            dyReshape, allInput->dhContiguous, allInput->dcContiguous, seqLength, allInput->iContiguous,
            allInput->gContiguous, allInput->fContiguous, allInput->oContiguous, allInput->hContiguous,
            allInput->cContiguous, allInput->tanhcContiguous, numLayers, 1, hasBias, paramNumPerLayer, executor);
    } else {
        output = LstmBackwardMultiLayerBidirec(
...
aclnnStatus PrepareLSTMBackwardNoneInputs(const aclTensor* input, const aclTensorList* hx, const aclTensor* dh,
                                          const aclTensor* dc, const aclTensor* dy, bool bidirectional,
                                          aclOpExecutor* executor, const aclTensor*& dhOut, const aclTensor*& dcOut,
                                          const aclTensor*& dyOut)
{
    ...
    if (dy == nullptr) {
        ...
        dyShape[HIDDEN_DIM_INDEX] = bidirectional ? dhShape[HIDDEN_DIM_INDEX] * BI_DIRECTION :
                                                    dhShape[HIDDEN_DIM_INDEX];
                                              ...
                                              uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    CHECK_RET(CreateContiguousTensors(input, hx, params, dyInput, dhInput, dcInput, i, g, f, o, h, c, tanhc,
                                      uniqueExecutor.get(), &allInputContiguous, batchSizesOptional) == ACLNN_SUCCESS,
              ACLNN_ERR_PARAM_NULLPTR);

    if (batchSizesOptional == nullptr) {
        ret = ExecLstmInputBackward(&allInputContiguous, dxOut, dhPrevOut, dcPrevOut, dparamsOut, batchFirst,
                                    bidirectional, hasBias, numLayers, uniqueExecutor.get());
    } else {
        ret = ExecLstmDataBackward(&allInputContiguous, dxOut, dhPrevOut, dcPrevOut, dparamsOut, bidirectional, hasBias,
                                   numLayers, uniqueExecutor.get());
    }
