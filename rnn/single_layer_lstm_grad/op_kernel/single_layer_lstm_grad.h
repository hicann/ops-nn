...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR weight, GM_ADDR bias, GM_ADDR y, GM_ADDR initH, GM_ADDR initC,
                                ...
                                GM_ADDR dx, GM_ADDR dhPrev, GM_ADDR dcPrev,
                                const SingleLayerLstmGradTilingData* __restrict rnnTiling, GM_ADDR workspace,
                                TPipe* inputPipe)
    {
        pipe = inputPipe;
        ...
        InitGlobalBuffers(x, weight, bias, y, initH, initC, h, c, dy, dh, dc, i, j, f, o, tanhct, seq_length, dw, db,
                          dx, dhPrev, dcPrev, workspace);
        InitVars();
    }
...
public:
    ...
    matmul::Matmul<matmul::MatmulType<AscendC::TPosition::GM, CubeFormat::ND, float, true>,
                   ...
                   matmul::MatmulType<AscendC::TPosition::GM, CubeFormat::ND, float>,
                   matmul::MatmulType<AscendC::TPosition::GM, CubeFormat::ND, float>, MM_WEIGHT_CFG>
        dwMM;

    matmul::Matmul<matmul::MatmulType<AscendC::TPosition::GM, CubeFormat::ND, float>,
                   matmul::MatmulType<AscendC::TPosition::GM, CubeFormat::ND, float>,
                   matmul::MatmulType<AscendC::TPosition::GM, CubeFormat::ND, float>,
                   matmul::MatmulType<AscendC::TPosition::GM, CubeFormat::ND, float>, MM_GATE_CFG>
        dgateMM;
...
private:
                                             ...
                                             GM_ADDR seqLength)
    {
        ...
        inputGm.yGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(y), tiling.timeStep * tiling.batch * tiling.hiddenSize);
        ...
        inputGm.hGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(h), tiling.timeStep * tiling.batch * tiling.hiddenSize);
        inputGm.cGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(c), tiling.timeStep * tiling.batch * tiling.hiddenSize);
        ...
        inputGm.iGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(i), tiling.timeStep * tiling.batch * tiling.hiddenSize);
        inputGm.jGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(j), tiling.timeStep * tiling.batch * tiling.hiddenSize);
                                              ...
                                              GM_ADDR dcPrev, GM_ADDR workspace)
    {
        ...
        outputGm.dcPrevGm.SetGlobalBuffer(reinterpret_cast<__gm__ T*>(dcPrev), tiling.batch * tiling.hiddenSize);
        outputGm.dgateGm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(workspace),
        ...
        if constexpr (!std::is_same<T, float>::value) {
            int64_t wFp32GmOffset = xhGmOffset + tiling.timeStep * tiling.batch *
                                                     (tiling.hiddenSize + tiling.inputSize) * FLOAT_BYTES;
            outputGm.wFp32Gm.SetGlobalBuffer(
                reinterpret_cast<__gm__ float*>(workspace + wFp32GmOffset),
                LSTM_GATE_SIZE * tiling.hiddenSize * (tiling.hiddenSize + tiling.inputSize));
            int64_t dwFp32GmOffset = wFp32GmOffset + LSTM_GATE_SIZE * tiling.hiddenSize *
                                                         (tiling.hiddenSize + tiling.inputSize) * FLOAT_BYTES;
            outputGm.dwFp32Gm.SetGlobalBuffer(
                reinterpret_cast<__gm__ float*>(workspace + dwFp32GmOffset),
                LSTM_GATE_SIZE * tiling.hiddenSize * (tiling.hiddenSize + tiling.inputSize));
            int64_t dhPrevFp32GmOffset = dwFp32GmOffset + LSTM_GATE_SIZE * tiling.hiddenSize *
                                                              (tiling.hiddenSize + tiling.inputSize) * FLOAT_BYTES;
            ...
            outputGm.dhPrevFp32Gm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(dhPrev),
                                                  tiling.batch * tiling.hiddenSize);
            outputGm.dcPrevFp32Gm.SetGlobalBuffer(reinterpret_cast<__gm__ float*>(dcPrev),
                                                  tiling.batch * tiling.hiddenSize);
            outputGm.wFp32Gm.SetGlobalBuffer(
    ...
    __aicore__ inline void ProcessDgateMM(int64_t tIdx)
    {
        if (GetBlockIdx() < dgateMMTiling.usedCoreNum) {
            ...
            if (dgateTail.nCoreIndx == dgateTail.notTailNCoreCount &&
                dgateTail.mCoreIndx == dgateTail.notTailMCoreCount) {
    ...
    __aicore__ inline void ProcessDgates(int64_t tIdx, bool firstLoop, bool useInitCH)
    {
        ...
        for (int64_t totalIndex = 0; totalIndex < tiling.mCnt * tiling.nCnt; totalIndex++) {
            if (mCoreIndex == totalIndex % aivCoreNum) {
                ...
                core_.mSingleCoreShape = core_.mCntIndex == (tiling.mCnt - 1) ? tiling.singleCoreMTail :
                                                                                tiling.singleCoreM;
                core_.nSingleCoreShape = core_.nCntIndex == (tiling.nCnt - 1) ? tiling.singleCoreNTail :
                                                                                tiling.singleCoreN;
        ...
        for (int64_t mCntIndex = 0; mCntIndex < incoreMCnt; mCntIndex++) {
            ...
            for (int64_t nCntIndex = 0; nCntIndex < incoreNCnt; nCntIndex++) {
                ...
                block_.gateOutOffset = core_.gateOutOffset +
                                       block_.mCntIndex * tiling.baseM * tiling.hiddenSize * LSTM_GATE_SIZE +
                                       block_.nCntIndex * tiling.baseN;
    ...
    __aicore__ inline void StoreDgatesResults(int64_t tIdx, GlobalTensor<float>& mixGm, bool useInitCH,
                                              int64_t nShapeAligned, int64_t calcSizeAlign)
    {
    ...
    __aicore__ inline void ProcessReduce()
    {
        ...
        if (mCoreIndex < tiling.nReduceCnt) {
            ...
            core_.nSingleCoreReduceShape = core_.reduceNCoreIndex == (tiling.nReduceCnt - 1) ?
                                               tiling.singleCoreReduceNTail :
            ...
            for (int64_t nCntIndex = 0; nCntIndex < incoreNCnt; nCntIndex++) {
                ...
                reduceBlock_.offset = core_.reduceNCoreIndex * tiling.singleCoreReduceN +
                                      reduceBlock_.nReduceCntIndx * tiling.baseReduceN;
                ...
                while (reduceNumLeft >= 1) {
                    ...
                    SubProcessReduce(outputGm.dgateGm, outputGm.dbGm, reduceNumLeft, reduceBlockSize,
                                     reduceBlockSizeTail, reduceSubRepeatTimes);
                }
            }
        }
    }
            ...
            for (int64_t i = start; i < end; i += mergeStep) {
                ...
                if (mid < rightEnd) {
                    Add(subDataTensor[i * reduceBlock_.nReduceShapeAlign],
                        ...
                        subDataTensor[mid * reduceBlock_.nReduceShapeAlign], reduceBlock_.nReduceShape);
                    PipeBarrier<PIPE_V>();
                }
            }
        }
        ...
        for (int64_t mRepeatIdx = 0; mRepeatIdx < reduceSubRepeatTimes; mRepeatIdx++) {
            ...
            for (int64_t copyLoopIdx = 0; copyLoopIdx < copyInLoop; copyLoopIdx++) {
                ...
                DataCopyExtParams dataCopyParams{
                    ...
                    static_cast<uint32_t>((tiling.hiddenSize * LSTM_GATE_SIZE - reduceBlock_.nReduceShape) *
                                          FLOAT_BYTES),
                    0u, 0u};
            ...
            DataCopyExtParams outDataCopyParams{
                static_cast<uint16_t>(1), static_cast<uint32_t>(reduceBlock_.nReduceShape * paraBytes), 0u, 0u, 0u};

            DataCopyExtParams outFp32DataCopyParams{
                static_cast<uint16_t>(1), static_cast<uint32_t>(reduceBlock_.nReduceShape * FLOAT_BYTES), 0u, 0u, 0u};
            if (reduceNumLeft <= 1) {
                // Get reduce out
                if constexpr (std::is_same<T, half>::value) {
                                     ...
                                     const LocalTensor<float>& dxhUb)
    {
        ...
        if (useInitCH && std::is_same<T, half>::value) {
            MTE2ToVSync();
            Cast(dhStartAddr.template ReinterpretCast<T>(), dhStartAddr, RoundMode::CAST_NONE,
                 tiling.hiddenSizeAligned * curMLines);
            VToMTE3Sync();
            DataCopyExtParams dataCopyDhParams{
                static_cast<uint16_t>(curMLines), static_cast<uint32_t>(tiling.hiddenSize * paraBytes),
            ...
            MTE2ToVSync();
            Cast(dhStartAddr.template ReinterpretCast<T>(), dhStartAddr, RoundMode::CAST_RINT,
                 tiling.hiddenSizeAligned * curMLines);
            VToMTE3Sync();
            DataCopyExtParams dataCopyDhParams{
                static_cast<uint16_t>(curMLines), static_cast<uint32_t>(tiling.hiddenSize * paraBytes),
            ...
            DataCopyExtParams dataCopyDhParams{
                ...
                static_cast<uint32_t>((tiling.hiddenSizeAligned - tiling.hiddenSize) / SRC_STRIDE_OFFSET_DIVISOR), 0,
                0};
            DataCopyPad(outputGm.dhPrevFp32Gm[taskIdx * tiling.hiddenSize * dxhInputTiling.copyMLines], dhStartAddr,
                        dataCopyDhParams);
        }
        MTE3ToMTE2Sync();
    }
    ...
    __aicore__ inline void ProcessDxSplitLarge(int64_t tIdx)
    {
        ...
        int64_t curCoreTaskNumC = curBlockIdx < dxhInputTiling.splitPreCore ? (dxhInputTiling.splitTaskPerCore + 1) :
                                                                              dxhInputTiling.splitTaskPerCore;
        int64_t startTaskIdC = curBlockIdx < dxhInputTiling.splitPreCore ?
                                   (curCoreTaskNumC * curBlockIdx) :
                                   (curCoreTaskNumC * curBlockIdx + dxhInputTiling.splitPreCore);

        totalUb = subDataBuf.Get<uint8_t>();
        dxUb = totalUb.ReinterpretCast<T>();
        if constexpr (!std::is_same<T, float>::value) {
            dxFp32Ub = totalUb[dxhInputTiling.copyMLines * dxhInputTiling.copyNLength * sizeof(T)]
    ...
    __aicore__ inline void ProcessDhSequenceLengthMask(bool firstLoop, int64_t curMLines, int64_t curCopyNLength,
                                                       int64_t curCopyNLengthAligned, int64_t tIdx, int64_t taskIdx,
                                                       int64_t nLoopIdx, int64_t calBlockSize)
    {
        DataCopyExtParams dataCopyInSeqParams{
            static_cast<uint16_t>(curMLines), static_cast<uint32_t>(curCopyNLength * sizeof(T)),
            static_cast<uint32_t>((tiling.hiddenSize - curCopyNLength) * sizeof(T)),
            static_cast<uint32_t>((curCopyNLengthAligned - curCopyNLength) / calBlockSize), 0};
        ...
        if (firstLoop) {
            ...
            DataCopyExtParams dataCopyInDhNextParams{
                ...
                static_cast<uint32_t>((curCopyNLengthAligned - curCopyNLength) / 8), 0};
    ...
    __aicore__ inline void InitDhSplitLargeBuffers()
    {
        ...
        if constexpr (!std::is_same<T, float>::value) {
            dhPrevFp32Ub = totalUb[dxhHiddenTiling.copyMLines * dxhHiddenTiling.copyNLength * sizeof(T)]
                               .template ReinterpretCast<float>();
            if (tiling.isSeqLength == SEQ_LENGTH_ENABLED) {
                seqUb = totalUb[dxhHiddenTiling.copyMLines * dxhHiddenTiling.copyNLength * HALF_AND_CAST_BYTES]
                            .template ReinterpretCast<T>();
                dhNextUb = totalUb[dxhHiddenTiling.copyMLines * dxhHiddenTiling.copyNLength *
                                   (HALF_AND_CAST_BYTES + sizeof(T))]
                               .template ReinterpretCast<T>();
                seqFp32Ub = totalUb[dxhHiddenTiling.copyMLines * dxhHiddenTiling.copyNLength * SEQ_FP32_OFFSET]
                                .template ReinterpretCast<float>();
                dhNextFp32Ub = totalUb[dxhHiddenTiling.copyMLines * dxhHiddenTiling.copyNLength * DH_FP32_OFFSET]
                                   .template ReinterpretCast<float>();
            }
        } else if (tiling.isSeqLength == SEQ_LENGTH_ENABLED) {
            dhPrevFp32Ub = dhPrevUb;
            seqUb = totalUb[dxhHiddenTiling.copyMLines * dxhHiddenTiling.copyNLength * sizeof(T)]
                        .template ReinterpretCast<T>();
            seqFp32Ub = seqUb;
            dhNextUb = totalUb[dxhHiddenTiling.copyMLines * dxhHiddenTiling.copyNLength * sizeof(T) * DOUBLE]
                           .template ReinterpretCast<T>();
    ...
    __aicore__ inline void ProcessDhSplitLarge(int64_t tIdx, bool firstLoop, bool useInitCH)
    {
        ...
        for (int64_t taskLoopId = 0; taskLoopId < curCoreTaskNumH; taskLoopId++) {
            ...
            int64_t curMLines = (taskIdx == dxhHiddenTiling.taskNum - 1) ? dxhHiddenTiling.copyMLinesTail :
                                                                           dxhHiddenTiling.copyMLines;
            for (int64_t nLoopIdx = 0; nLoopIdx < dxhHiddenTiling.nLoop; nLoopIdx++) {
                int64_t curCopyNLength = (nLoopIdx == dxhHiddenTiling.nLoop - 1) ? dxhHiddenTiling.copyNLengthTail :
                                                                                   dxhHiddenTiling.copyNLength;
                int64_t curCopyNLengthAligned = CeilDiv(curCopyNLength, calBlockSize) * calBlockSize;
                DataCopyExtParams dataCopyInDxParams{
                    static_cast<uint16_t>(curMLines), static_cast<uint32_t>(curCopyNLength * FLOAT_BYTES),
                    static_cast<uint32_t>((tiling.hiddenSize + tiling.inputSize - curCopyNLength) * FLOAT_BYTES),
                ...
                // dhprev do mask
                if (tiling.isSeqLength == SEQ_LENGTH_ENABLED) {
                    ProcessDhSequenceLengthMask(firstLoop, curMLines, curCopyNLength, curCopyNLengthAligned, tIdx,
                                                taskIdx, nLoopIdx, calBlockSize);
                }
                // copy out
    ...
    {
        auto curBlockIdx = GetBlockIdx();
        int64_t curCoreTaskNum = curBlockIdx < xhInputTiling.splitPreCore ? (xhInputTiling.splitTaskPerCore + 1) :
                                                                            xhInputTiling.splitTaskPerCore;
        int64_t startTaskId = curBlockIdx < xhInputTiling.splitPreCore ?
                                  (curCoreTaskNum * curBlockIdx) :
                                  (curCoreTaskNum * curBlockIdx + xhInputTiling.splitPreCore);
        InitConcatXhUb();
        ...
        for (int64_t tIdx = 0; tIdx < tiling.timeStep; tIdx++) {
            ...
            for (int64_t taskLoopId = 0; taskLoopId < curCoreTaskNum; taskLoopId++) {
                ...
                int64_t curMLines = (taskIdx == xhInputTiling.taskNum - 1) ? xhInputTiling.copyMLinesTail :
    ...
    __aicore__ inline void InitConcatXhUb()
    {
        ...
        if constexpr (!std::is_same<T, float>::value) {
            xFp32Ub = totalUb[xhInputTiling.copyMLines * (tiling.inputSizeAligned + tiling.hiddenSizeAligned) *
                              sizeof(T)]
                          .ReinterpretCast<float>();
            hFp32Ub = totalUb[xhInputTiling.copyMLines * (tiling.inputSizeAligned + tiling.hiddenSizeAligned) *
                                  sizeof(T) +
                              xhInputTiling.copyMLines * tiling.inputSizeAligned * FLOAT_BYTES]
                          .ReinterpretCast<float>();
            outXUb = xFp32Ub;
            outHUb = hFp32Ub;
        } else {
    ...
    __aicore__ inline void ProcessCopyInputXLarge()
    {
        ...
        auto curBlockIdx = GetBlockIdx();
        int64_t curCoreTaskNumC = curBlockIdx < xhInputTiling.splitPreCore ? (xhInputTiling.splitTaskPerCore + 1) :
                                                                             xhInputTiling.splitTaskPerCore;
        int64_t startTaskIdC = curBlockIdx < xhInputTiling.splitPreCore ?
                                   (curCoreTaskNumC * curBlockIdx) :
                                   (curCoreTaskNumC * curBlockIdx + xhInputTiling.splitPreCore);
        totalUb = subDataBuf.Get<uint8_t>();
        xUb = totalUb.ReinterpretCast<T>();
        if constexpr (!std::is_same<T, float>::value) {
    ...
    {
        auto curBlockIdx = GetBlockIdx();
        int64_t curCoreTaskNumH = curBlockIdx < xhHiddenTiling.splitPreCore ? (xhHiddenTiling.splitTaskPerCore + 1) :
                                                                              xhHiddenTiling.splitTaskPerCore;
        ...
        for (int64_t tIdx = 0; tIdx < tiling.timeStep; tIdx++) {
            ...
            for (int64_t taskLoopId = 0; taskLoopId < curCoreTaskNumH; taskLoopId++) {
                int64_t taskIdx = startTaskIdH + taskLoopId;
                int64_t curMLines = (taskIdx == xhHiddenTiling.taskNum - 1) ? xhHiddenTiling.copyMLinesTail :
