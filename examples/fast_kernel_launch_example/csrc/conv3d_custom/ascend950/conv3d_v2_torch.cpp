 ...
 */
static int32_t InitConvBase(const vector<int64_t>& oriInputShapeList, const vector<int64_t>& oriWeightShapeList,
                            const vector<int64_t>& oriOutputShapeList,
...
static int32_t InitAttrInfo(const vector<int64_t>& strideList, const vector<int64_t>& paddingList,
                            const vector<int64_t>& dilationList,
                            optiling::conv_ops_tiling::ConvAscendcAttrInfo& attrInfo)
{
    // 设置膨胀参数（控制卷积核元素之间的间隔）
 ...
 */
template <typename T>
void Conv3dV2CustomApi(aclrtStream stream, const at::Tensor& input, const at::Tensor& weight,
                       const torch::Tensor& output, const vector<int64_t> strideList, const vector<int64_t> paddingList,
                       const vector<int64_t> dilationList, const vector<int64_t> oriInputShapeList,
                       const vector<int64_t> oriWeightShapeList, const c10::optional<torch::Tensor>& bias)
{
    // 获取输出张量的形状
    vector<int64_t> oriOutputShapeList = {output.size(FORMAT_NCDHW_N_INDEX), output.size(FORMAT_NCDHW_C_INDEX),
                                          output.size(FORMAT_NCDHW_D_INDEX), output.size(FORMAT_NCDHW_H_INDEX),
                                          output.size(FORMAT_NCDHW_W_INDEX)};
    ...
    TORCH_CHECK(InitPlatformInfo(tilingInfo.platformInfo) == 0, "Failed to initialize platform information");
    TORCH_CHECK(InitNodeInfo(tilingInfo.nodeInfo) == 0, "Failed to initialize node information");
    ...
    // 计算需要的AI Core数量
    uint32_t g_numBlocks = tilingData.batchDim * tilingData.doDim * tilingData.hoDim * tilingData.nDim;

    // 获取tiling键（用于选择最优的kernel实现）
    optiling::conv_ops_tiling::ConvTilingKeyPara tilingKeyPara{};
    optiling::conv_ops_tiling::Conv3dV2BaseTilingKey tilingKey{
        tilingData,           tilingInfo.flagInfo,     tilingInfo.descInfo,
        tilingInfo.shapeInfo, tilingInfo.numBlocksRes, tilingInfo.convOpsConstParams};
    tilingKey.GetTemplateTilingKey(tilingKeyPara);

    // 调用Conv3dv2Template函数执行实际的3D卷积计算
    Conv3dv2Template(input_ptr, weight_ptr, bias_ptr, nullptr, nullptr, nullptr, output_ptr, nullptr, tilingData,
 ...
 * @return 输出张量
 */
torch::Tensor CreateOutputTensor(const torch::Tensor& input, const c10::IntArrayRef& oriInputShape,
                                 const c10::IntArrayRef& oriWeightShape, const c10::IntArrayRef& strides,
                                 const c10::IntArrayRef& pads, const c10::IntArrayRef& dilations)
{
    // 提取输入张量的各个维度
    ...
    vector<int64_t> kernelSize = {oriWeightShape[FORMAT_NCDHW_D_INDEX], oriWeightShape[FORMAT_NCDHW_H_INDEX],
                                  oriWeightShape[FORMAT_NCDHW_W_INDEX]};
    ...
    // 打印输出形状信息
    printf("Output shape: N=%d, Co=%d, Do=%d, Ho=%d, Wo=%d\n", static_cast<int>(N), static_cast<int>(Co),
           static_cast<int>(Do), static_cast<int>(Ho), static_cast<int>(Wo));
 ...
 */
torch::Tensor Conv3dV2CustomNpu(const torch::Tensor& input, const torch::Tensor& weight,
                                const c10::IntArrayRef& strides, const c10::IntArrayRef& pads,
                                const c10::IntArrayRef& dilations, const c10::IntArrayRef& oriInputShape,
                                const c10::IntArrayRef& oriWeightShape, const c10::optional<torch::Tensor>& bias)
{
    ...
    auto acl_call = [=]() -> int {
        AT_DISPATCH_FLOATING_TYPES_AND2(at::kHalf, at::kBFloat16, input.scalar_type(), "Conv3dV2CustomNpu", [&] {
            Conv3dV2CustomApi<scalar_t>(stream, input, weight, output, strideList, paddingList, dilationList,
                                        oriInputShapeList, oriWeightShapeList, bias);
        });
        return 0;
    };
...
{
    m.def("conv3d_v2_custom(Tensor input, Tensor weight, int[3] stride, int[3] padding, int[3] dilation, int[5] "
          "oriInputShape, int[5] oriWeightShape, Tensor? bias) -> Tensor");
}

// 注册NPU实现，将conv3d_v2_custom算子映射到Conv3dV2CustomNpu函数
TORCH_LIBRARY_IMPL(EXTENSION_MODULE_NAME, PrivateUse1, m) { m.impl("conv3d_v2_custom", Conv3dV2CustomNpu); }
