...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/activation/softmax_grad)
...
- Description: Performs backpropagation of [softmax](../../softmax_v2/docs/aclnnSoftmax_en.md).
- The Softmax function can be derived using the following formula:
  The relationship between **out** (input gradient value), **gradOutput** (output gradient of the previous layer), and **output** (Softmax forward output) can be expressed as follows:
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnSoftmaxBackwardGetWorkspaceSize** is called to obtain the workspace size required for computation and the executor that contains the operator computation process. Then, **aclnnSoftmaxBackward** is called to perform computation.
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>output</td>
      <td>Input</td>
      <td>Output value of the Softmax function, corresponding to output in the formula.</td>
      ...
      <td>dim</td>
      <td>Input</td>
      <td>Dimension of the Softmax function.</td>
      <td>-</td>
      <td>INT64</td>
      <td>-</td>
      <td>-</td>
      ...
      <td>out</td>
      <td>Output</td>
      <td>Output of the function, corresponding to the gradient of the input.</td>
      <td><ul><li> It is the derivation result of the input. Multiple data types are supported and no extra space needs to be applied for. Other data types are supported by automatic casting, but extra space is required. </li><li>The shape must be the same as that of gradOutput.</li></ul></td>
      <td>FLOAT16, FLOAT32, BFLOAT16</td>
      <td>ND</td>
      <td>0–8</td>
      <td>√</td>
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnSoftmaxBackwardGetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
int main() {
  ...
  aclOpExecutor* executor;
  int64_t dim = 0;
  // Call the first-phase API of aclnnSoftmaxBackward.
  ...
  // Call the second-phase API of aclnnSoftmaxBackward.
