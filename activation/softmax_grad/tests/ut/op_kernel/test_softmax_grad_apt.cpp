...
/*!
 * \file test_softmax_grad_apt.cpp
 * \brief
 */
...
#include "softmax_grad_tiling_def.h"
                    ...
                    uint8_t* tiling)
{
    ...
    uint8_t* softmax = (uint8_t*)AscendC::GmAlloc((fileSize + 31) / 32 * 32);
    uint8_t* grad_softmax = (uint8_t*)AscendC::GmAlloc((fileSize + 31) / 32 * 32);
    uint8_t* grad_x = (uint8_t*)AscendC::GmAlloc((fileSize + 31) / 32 * 32);
    uint8_t* workspace = (uint8_t*)AscendC::GmAlloc(workspaceFileSize);

    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(softmax_grad, realBlockNum, softmax, grad_softmax, grad_x, workspace, tiling);

    AscendC::GmFree(softmax);
    AscendC::GmFree(grad_softmax);
    AscendC::GmFree(grad_x);
    AscendC::GmFree(workspace);
    AscendC::GmFree(tiling);
}

TEST_F(softmax_grad_test, test_ar_small_r_float32)
{
...
TEST_F(softmax_grad_test, test_ar_float32)
{
...
TEST_F(softmax_grad_test, test_ar_recompute_float32)
{
...
TEST_F(softmax_grad_test, test_ara_float32)
{
...
TEST_F(softmax_grad_test, test_ara_recompute_float32)
{
