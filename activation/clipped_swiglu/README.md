...
- 接口功能：带截断的Swish门控线性单元激活函数，实现x的SwiGlu计算。本算子相较于SwiGlu算子，新增了部分输入参数：groupIndex、alpha、limit、bias、interleaved，clamp_mode，用于支持GPT-OSS模型使用的变体SwiGlu、MoE模型使用的分组场景以及部分新模型需要将clamp操作后移的场景。
...
- 计算流程：
  ...
  4. 根据输入参数alpha、limit、bias、clamp_mode进行变体SwiGlu计算，公式如下：
     ...
     当clamp_mode为0时，表示clamp操作在silu之前：
     ...
     当clamp_mode为1时，表示clamp操作在silu之后：
...
<table style="undefined;table-layout: fixed; width: 970px"><colgroup>
  ...
  <tbody>
    ...
    <tr>
      <td>clamp_mode</td>
      <td>可选属性</td>
      <td>公式中的输入clamp_mode，设置为1表示将A的clamp操作后移至silu激活之后，即对silu(A)的结果做clamp；设置为0表示A的clamp操作在silu激活之前。默认为0。</td>
      <td>INT64</td>
      <td>-</td>
    </tr>
    <tr>
      <td>y</td>
      <td>输出</td>
      <td>公式中的输出y。维度数需与输入x一致，在入参dim对应维度上为x的一半，其他维度上与x一致。</td>
      <td>FLOAT、FLOAT16、BFLOAT16</td>
      <td>ND</td>
    </tr>
  </tbody></table>
...
- <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品/Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：
  - 属性`clamp_mode`为无效值，计算逻辑同clamp_mode=0。
...
| aclnn调用 | [test_aclnn_clipped_swiglu_v2](./examples/test_aclnn_clipped_swiglu_v2.cpp) | 通过[aclnnClippedSwigluV2](./docs/aclnnClippedSwigluV2.md)接口方式调用ClippedSwiglu算子。    |
| 图模式调用 | - | 通过[算子IR](./op_graph/clipped_swiglu_proto.h)构图方式调用ClippedSwiglu算子。    |
