...
{
    return std::isinf(alpha->ToFloat()) || std::isnan(alpha->ToFloat()) || std::isinf(inputScale->ToFloat()) ||
...
static aclnnStatus ComputeEluWithInfNan(const aclTensor* contiguousSelf, const aclScalar* alpha, const aclScalar* scale,
                                        aclOpExecutor* executor, const aclTensor** eluOut)
{
