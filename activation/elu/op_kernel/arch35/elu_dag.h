...
struct EluCustom : public Vec::ElemwiseQuaternaryOP<T, T, float, float, float> {
                                ...
                                uint32_t count)
    {
        ...
        Reg::RegTensor<float, Reg::RegTraitNumOne> vregNeg;
        Reg::RegTensor<float, Reg::RegTraitNumOne> vregOutputFloat;
        Reg::RegTensor<T, Reg::RegTraitNumOne> vregOutput;
        Reg::RegTensor<float, Reg::RegTraitNumOne> vregExp;
        Reg::RegTensor<float, Reg::RegTraitNumOne> vregPoly;
        Reg::RegTensor<float, Reg::RegTraitNumOne> vregAbsZ;
        Reg::RegTensor<float, Reg::RegTraitNumOne> vregT1;
        Reg::MaskReg mask, cmpMask, cmpExpm1;
        if constexpr (std::is_same_v<T, float>) {
            __VEC_SCOPE__
            {
                for (uint16_t loopIdx = 0; loopIdx < static_cast<uint16_t>(loopNum); loopIdx++) {
                    ...
                    Reg::Muls(vregT1, vregNeg, coeff5, mask);
                    Reg::Adds(vregT1, vregT1, coeff4, mask);
                    Reg::Mul(vregT1, vregT1, vregNeg, mask);
                    Reg::Adds(vregT1, vregT1, coeff3, mask);
                    Reg::Mul(vregT1, vregT1, vregNeg, mask);
                    Reg::Adds(vregT1, vregT1, coeff2, mask);
                    Reg::Mul(vregT1, vregT1, vregNeg, mask);
                    Reg::Adds(vregT1, vregT1, coeff1, mask);
                    Reg::Mul(vregPoly, vregT1, vregNeg, mask);

                    // exp(z) - 1 for |z| >= threshold
                    Reg::Exp(vregExp, vregNeg, mask);
                    Reg::Adds(vregExp, vregExp, constNegOne, mask);

                    // Select expm1 result based on |z|
                    Reg::Abs(vregAbsZ, vregNeg, mask);
                    Reg::Compares<T, CMPMODE::LT>(cmpExpm1, vregAbsZ, expm1Threshold, mask);
                    Reg::Select<T>(vregNeg, vregPoly, vregExp, cmpExpm1);

                    Reg::Muls(vregNeg, vregNeg, alpha, mask);

                    Reg::Compares<T, CMPMODE::GT>(cmpMask, vregInput, (float)0.0, mask);
            ...
            __VEC_SCOPE__
            {
                for (uint16_t loopIdx = 0; loopIdx < static_cast<uint16_t>(loopNum); loopIdx++) {
                    ...
                    // OpCopyIn
                    Reg::LoadAlign<T, Reg::LoadDist::DIST_UNPACK_B16>(vregInput,
                                                                      (__ubuf__ T*)(srcAddr + loopIdx * vlSize));
                    Reg::Cast<float, T, castTrait0>(vregInputFloat, vregInput, mask);
                    Reg::Muls(vregNeg, vregInputFloat, inputScale, mask);

                    // Polynomial expm1(z) = z + z^2/2 + z^3/6 + z^4/24 for |z| < threshold
                    // Horner: T1 = T1 * z + coeff, then poly = T1 * z
                    Reg::Muls(vregT1, vregNeg, coeff4, mask);
                    Reg::Adds(vregT1, vregT1, coeff3, mask);
                    Reg::Mul(vregT1, vregT1, vregNeg, mask);
                    Reg::Adds(vregT1, vregT1, coeff2, mask);
                    Reg::Mul(vregT1, vregT1, vregNeg, mask);
                    ...
                    // Select expm1 result based on |z|
                    Reg::Abs(vregAbsZ, vregNeg, mask);
                    Reg::Compares<float, CMPMODE::LT>(cmpExpm1, vregAbsZ, expm1Threshold, mask);
                    Reg::Select<float>(vregNeg, vregPoly, vregExp, cmpExpm1);

                    Reg::Muls(vregNeg, vregNeg, alpha, mask);

                    Reg::Compares<float, CMPMODE::GT>(cmpMask, vregInputFloat, (float)0.0, mask);
                    Reg::Select<float>(vregOutputFloat, vregInputFloat, vregNeg, cmpMask);
                    Reg::Muls(vregOutputFloat, vregOutputFloat, scale, mask);
                    Reg::Cast<T, float, castTrait1>(vregOutput, vregOutputFloat, mask);
                    // OpCopyOut
