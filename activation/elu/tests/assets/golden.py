...
def elu_golden(
    x, *, alpha: float = 1.0, scale: float = 1.0, input_scale: float = 1.0, **kwargs
):
    """
    ...
    output_y = torch.ops.aten.elu(
        x_torch, alpha=alpha, scale=scale, input_scale=input_scale
    )
...
def _aclnn_elu_impl(input_tensor, alpha, scale, inputScale):
    ...
    return torch.ops.aten.elu(
        input_tensor, alpha=alpha_val, scale=scale_val, input_scale=input_scale_val
    )


def aclnn_elu_golden(self, alpha, scale, inputScale, out=None, **kwargs):
    """
    ...
    Returns:
        Output tensors.
    """
    return _aclnn_elu_impl(self, alpha, scale, inputScale)
