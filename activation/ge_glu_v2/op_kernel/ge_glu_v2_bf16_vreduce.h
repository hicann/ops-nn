...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::Init(GM_ADDR x, GM_ADDR y, GM_ADDR gelu, GM_ADDR workspace,
                                                           const GeGluV2TilingData* tilingData)
{
...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::Process()
{
...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::ProcessPerCore()
{
    // process core
    for (int64_t idx_3 = 0; idx_3 < this->m_tilingData.loopNum; idx_3++) {
        CopyInX(idx_3, this->m_tilingData.group);
        ComputeGeluAndMul(this->group_ub_num);
        CopyOutGelu(idx_3, this->group_ub_num, this->m_tilingData.group);
        CopyOutMul(idx_3, this->group_ub_num, this->m_tilingData.group);
    }
...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::ProcessLastCore()
{
    for (int64_t idx_3 = 0; idx_3 < this->m_tilingData.tailLoopNum; idx_3++) {
        CopyInX(idx_3, this->m_tilingData.group);
        ComputeGeluAndMul(this->group_ub_num);
        CopyOutGelu(idx_3, this->group_ub_num, this->m_tilingData.group);
        CopyOutMul(idx_3, this->group_ub_num, this->m_tilingData.group);
...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::CopyInX(const int64_t& index, const int64_t& blockCount)
{
...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::ComputeGeluAndMul(const int64_t& ub_num)
{
    ...
    if constexpr (IS_ERF) {
        ...
        this->ComputeGeluBase(ubx2, tmp_fp32_1, ub_num);

        LocalTensor<T> gelu_out = outQueueGelu.AllocTensor<T>();
        Cast(gelu_out, tmp_fp32_1, RoundMode::CAST_RINT, ub_num);

        LocalTensor<T> mul_out = outQueueMul.AllocTensor<T>();
        Mul(ubx1, ubx1, tmp_fp32_1, ub_num);
...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::CopyOutGelu(const int64_t& index, const int64_t& ub_num,
                                                                  const int64_t& group)
{
...
template <typename T, bool IS_ERF>
__aicore__ inline void GeGluV2Bf16VReduce<T, IS_ERF>::CopyOutMul(const int64_t& index, const int64_t& ub_num,
                                                                 const int64_t& group)
{
