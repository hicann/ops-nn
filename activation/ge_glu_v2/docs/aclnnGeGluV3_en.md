...
- Description: Gaussian error linear unit (GELU) activation gate function. Based on aclnnGeGlu, the direction of the data block on which the activation function is performed can be set.
- Formula:
  If **activateLeft** is set to **true**, activation is performed on the left half of $self$.

    $$
    out_{i}=GeGlu(self_{i}) = Gelu(A) \cdot B
    $$

  If **activateLeft** is set to **false**, activation is performed on the right half of $self$.

    $$
    out_{i}=GeGlu(self_{i}) = A \cdot Gelu(B)
    $$

  $A$ indicates the left half of $self$, and $B$ indicates the right half of $self$.
  
## Prototype

Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnGeGluV3GetWorkspaceSize** is called to obtain the workspace size required for computation and the executor that contains the operator computation process. Then, **aclnnGeGluV3** is called to perform computation.
...
- **Parameters:**
  ...
  <tbody>
      ...
      <td>self</td>
      <td>Input</td>
      <td>Input parameter for GeGluV3 computation, corresponding to self in the formula.</td>
      <td>-</td>
      ...
      <td>Input</td>
      <td>Optional input parameter.</td>
      <td>Activation function index used for GeGluV3 computation. The value 0 indicates that none is used, and the value 1 indicates that tanh is used.</td>
      <td>INT</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
      ...
      <td>The value false indicates that activation is performed on the right part, while the value true indicates that activation is performed on the left part.</td>
      <td>BOOL</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    ...
    <tr>
      <td>out</td>
      <td>Output</td>
      <td>Output parameter for GeGluV3 computation, corresponding to out_i in the formula.</td>
    ...
    <tr>
      <td>outGelu</td>
      <td>Output</td>
      <td>Output parameter for GeGluV3 computation.</td>
      <td><ul><li>Except the axis specified by dim, the shape of outGelu must be the same as that of self. The axis specified by dim is half of the axis corresponding to the shape of self. </li><li>The data type must be the same as that of self.</li></ul></td>
      <td>FLOAT, FLOAT16, BFLOAT16</td>
      <td>ND</td>
      <td>0–8</td>
      <td>√</td>
...
- **Returns:**
  ...
  **aclnnStatus**: status code. For details, see [aclnn Return Codes](../../../docs/en/context/aclnn_return_codes_nn.md).
  ...
  <tbody>
    ...
    <tr>
      <td>Except the axis specified by dim, the shapes of out and outGelu must be the same as that of self. The axis specified by dim is half of the axis corresponding to the shape of self.</td>
    </tr>
  </tbody></table>
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnGeGluV3GetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
int main()
{
    ...
    aclOpExecutor* executor;
    // Call the first-phase API of aclnnGeGluV3.
    ...
    // Call the second-phase API of aclnnGeGluV3.
