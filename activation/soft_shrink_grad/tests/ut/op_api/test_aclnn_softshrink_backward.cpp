...
#include "../../../op_api/aclnn_softshrink_backward.h"
...
protected:
    static void SetUpTestCase() { std::cout << "l2_softshrink_backward_test SetUp" << std::endl; }

    static void TearDownTestCase() { std::cout << "l2_softshrink_backward_test TearDown" << std::endl; }
};
...
TEST_F(l2_softshrink_backward_test, grad_nullptr)
{
    ...
    auto ut = OP_API_UT(aclnnSoftshrinkBackward, INPUT((aclTensor*)nullptr, selfDesc, lambdDesc), OUTPUT(outDesc));
...
TEST_F(l2_softshrink_backward_test, self_nullptr)
{
    ...
    auto outDesc = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND);
    auto lambdDesc = ScalarDesc(0.0f);
    auto ut = OP_API_UT(aclnnSoftshrinkBackward, INPUT(gradOutputDesc, (aclTensor*)nullptr, lambdDesc),
                        OUTPUT(outDesc));
...
TEST_F(l2_softshrink_backward_test, out_nullptr)
{
    ...
    auto outDesc = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND);
    auto lambdDesc = ScalarDesc(0.0f);
    auto ut = OP_API_UT(aclnnSoftshrinkBackward, INPUT(gradOutputDesc, selfDesc, lambdDesc),
...
TEST_F(l2_softshrink_backward_test, gradOutput_invalid_dtype)
{
    ...
    auto outDesc = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND);
    auto lambdDesc = ScalarDesc(0.0f);
    auto ut = OP_API_UT(aclnnSoftshrinkBackward, INPUT(gradOutputDesc, selfDesc, lambdDesc), OUTPUT(outDesc));
