 ...
 * \brief HardSwishGradV2 kernel class definition (arch35)
 *
...
#ifndef HARD_SWISH_GRAD_V2_H
#define HARD_SWISH_GRAD_V2_H

#include "kernel_operator.h"
#ifndef DTYPE_X
#include "kernel_tiling/kernel_tiling.h"
#include "hard_swish_grad_v2_tiling_data.h"
#include "hard_swish_grad_v2_tiling_key.h"
#endif

namespace NsHardSwishGradV2 {

using namespace AscendC;

template <typename T, int BUFFER_MODE>
class HardSwishGradV2 {
...
public:
    __aicore__ inline HardSwishGradV2(){};
    __aicore__ inline void Init(GM_ADDR gradOutput, GM_ADDR self, GM_ADDR gradInput,
                                const HardSwishGradV2Arch35TilingData* tilingData);
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void HardSwishGradV2<T, BUFFER_MODE>::Init(GM_ADDR gradOutput, GM_ADDR self, GM_ADDR gradInput,
                                                             const HardSwishGradV2Arch35TilingData* tilingData)
{
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void HardSwishGradV2<T, BUFFER_MODE>::CopyIn(int64_t progress, int64_t currentNum)
{
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void HardSwishGradV2<T, BUFFER_MODE>::Compute(int64_t currentNum)
{
    ...
    if constexpr (NEED_CAST) {
        ...
        Compares(maskLocal, selfLocal, (T)(-3.0), CMPMODE::GT,
...
template <typename T, int BUFFER_MODE>
__aicore__ inline void HardSwishGradV2<T, BUFFER_MODE>::CopyOut(int64_t progress, int64_t currentNum)
{
