...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    reluGradV2.set_input_##intputName(placeholder##intputIndex);                                                    \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    reluGradV2.set_input_##intputName(placeholder##intputIndex);                                                    \
    reluGradV2.update_input_desc_##intputName(placeholder##intputIndex##_desc);                                     \
    inputs.push_back(placeholder##intputIndex);
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto reluGradV2 = op::ReluGradV2("reluGradV2");
    std::vector<int64_t> xShape = {1, 64, 2, 64};
    ADD_INPUT(1, gradients, inDtype, xShape);
    ADD_INPUT(2, mask, DT_UINT1, xShape);

    outputs.push_back(reluGradV2);
    // 添加完毕
    return SUCCESS;
}
