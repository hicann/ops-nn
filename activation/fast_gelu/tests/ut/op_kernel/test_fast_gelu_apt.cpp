...
protected:
    static void SetUpTestCase() { cout << "fast_gelu_test SetUp\n" << endl; }
...
TEST_F(fast_gelu_test, test_case_fp32_1)
{
    ...
    ReadFile(path + "/fast_gelu_data/input_x.bin", xByteSize, x, xByteSize);
    auto KernelFastGelu = [](GM_ADDR x, GM_ADDR y, GM_ADDR workspace, GM_ADDR tiling) {
        ::fast_gelu<0, TPL_FP32>(x, y, workspace, tiling);
    };
    ICPU_SET_TILING_KEY(1003);
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(KernelFastGelu, blockDim, x, y, workspace, (uint8_t*)(tilingDatafromBin));
    WriteFile(path + "/fast_gelu_data/output.bin", y, yByteSize);
