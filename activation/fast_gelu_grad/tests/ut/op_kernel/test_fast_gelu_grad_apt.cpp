...
#include "../../../op_kernel/fast_gelu_grad_apt.cpp"
#include <cstdint>
...
class fast_gelu_grad_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "fast_gelu_grad_test SetUp\n" << endl; }
    static void TearDownTestCase()
    {
        cout << "fast_gelu_grad TearDown\n" << endl;
        kernel_ut::CleanGeneratedBinFiles("./fast_gelu_grad_data");
    }
};

TEST_F(fast_gelu_grad_test, test_case_fp32_1)
{
    ...
    uint32_t blockDim = 1;
    kernel_ut::SetupTestEnvironment("activation/fast_gelu_grad/tests/ut/op_kernel/fast_gelu_grad_data",
                                    "fast_gelu_grad_data");
    kernel_ut::RunGenData("./fast_gelu_grad_data", {"'(256)'", "float32"});
    ...
    ReadFile(path + "/fast_gelu_grad_data/input_dy.bin", dyByteSize, dy, dyByteSize);
    ReadFile(path + "/fast_gelu_grad_data/input_x.bin", xByteSize, x, xByteSize);
    auto KernelFastGeluGrad = [](GM_ADDR dy, GM_ADDR x, GM_ADDR z, GM_ADDR workspace, GM_ADDR tiling) {
        ::fast_gelu_grad<0, TPL_FP32>(dy, x, z, workspace, tiling);
    };
    ICPU_SET_TILING_KEY(1003);
    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(KernelFastGeluGrad, blockDim, dy, x, z, workspace, (uint8_t*)(tilingDatafromBin));
    WriteFile(path + "/fast_gelu_grad_data/output.bin", z, zByteSize);
