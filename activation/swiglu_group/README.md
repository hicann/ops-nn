...
| :--- | :--- | :--- | :--- | :--- |
| x | 输入 | 待计算的输入张量，最后一维被均分为两部分用于SwiGLU，需能被2整除。 | FLOAT16、BFLOAT16、FLOAT32 | ND |
...
- 当传入`group_index`时，数据类型为INT64，为count模式分组token数，`group_index`中的值须大于等于0，实际处理行数为`group_index`所有元素之和与`bs`两者中的较小值（`bs`为`x`除最后一维外的维度乘积）。
