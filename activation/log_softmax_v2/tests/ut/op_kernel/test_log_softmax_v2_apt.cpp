...
/*!
 * \file test_log_softmax_v2_apt.cpp
 * \brief
 */
...
#include "log_softmax_v2_tiling_def.h"
...
protected:
    static void SetUpTestCase() { cout << "log_softmax_v2_test SetUp\n" << endl; }
                    ...
                    uint8_t* tiling)
{
    ...
    uint8_t* logits = (uint8_t*)AscendC::GmAlloc((xFileSize + 31) / 32 * 32);
    uint8_t* logsoftmax = (uint8_t*)AscendC::GmAlloc((xFileSize + 31) / 32 * 32);
    uint8_t* workspace = (uint8_t*)AscendC::GmAlloc(workspaceFileSize);

    AscendC::SetKernelMode(KernelMode::AIV_MODE);
    ICPU_RUN_KF(log_softmax_v2, realBlockNum, logits, logsoftmax, workspace, tiling);

    AscendC::GmFree(logits);
    AscendC::GmFree(logsoftmax);
    AscendC::GmFree(workspace);
    AscendC::GmFree(tiling);
}

TEST_F(log_softmax_v2_test, test_ar_small_r_float32)
{
...
TEST_F(log_softmax_v2_test, test_ar_float32)
{
...
TEST_F(log_softmax_v2_test, test_ar_recompute_float32)
{
...
TEST_F(log_softmax_v2_test, test_ara_float32)
{
...
TEST_F(log_softmax_v2_test, test_ara_recompute_float32)
{
