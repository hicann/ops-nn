...
private:
                                   ...
                                   int64_t ubLoopIdx)
    {
        ...
        if ((tilingDataPtr_->input1Strides[tilingDataPtr_->ubSplitAxis] != 0) ||
            ...
            BroadcastNddmaWithoutLoop(inputGmX_, bufferIn0_, tilingDataPtr_->outputDims, tilingDataPtr_->outputStrides,
    ...
    __aicore__ inline void Compute1(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                    int64_t ubLoopIdx)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < vfLoopNum; i++) {
                ...
                AscendC::Reg::Cast<float, half, castTrait0>(vreg4, vreg3, preg0);
    ...
    __aicore__ inline void CopyIn2(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                   int64_t ubLoopIdx)
    {
        ...
        if ((tilingDataPtr_->input0Strides[tilingDataPtr_->ubSplitAxis] != 0) ||
            (ubLoopIdx <= 1 ||
             (AscendC::GetBlockIdx() * tilingDataPtr_->blockFormer + ubLoopIdx) % tilingDataPtr_->ubOuter <= 1)) {
            BroadcastNddmaWithoutLoop(inputGmDy_, bufferIn1_, tilingDataPtr_->outputDims, tilingDataPtr_->outputStrides,
    ...
    __aicore__ inline void Compute3(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                    int64_t ubLoopIdx)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            for (uint16_t i = 0; i < vfLoopNum; i++) {
                ...
                AscendC::Reg::Cast<half, float, castTrait1>(vreg14, vreg13, preg0);
    ...
    __aicore__ inline void CopyOut4(int64_t ubSplitSize, const int64_t (&axesIndices)[BROADCAST_MAX_DIMS],
                                    int64_t ubLoopIdx)
    {
