...
private:
                                   ...
                                   int64_t ubLoopIdx)
    {
        ...
        if ((tilingDataPtr_->input0Strides[tilingDataPtr_->ubSplitAxis] != 0) ||
            ...
            BroadcastNddmaWithLoop(inputGmDy_, bufferIn0_, tilingDataPtr_->outputDims, tilingDataPtr_->outputStrides,
                                   ...
                                   int64_t ubLoopIdx)
    {
        ...
        if ((tilingDataPtr_->input1Strides[tilingDataPtr_->ubSplitAxis] != 0) ||
            (ubLoopIdx <= 1 ||
             (AscendC::GetBlockIdx() * tilingDataPtr_->blockFormer + ubLoopIdx) % tilingDataPtr_->ubOuter <= 1)) {
            BroadcastNddmaWithLoop(inputGmX_, bufferIn1_, tilingDataPtr_->outputDims, tilingDataPtr_->outputStrides,
