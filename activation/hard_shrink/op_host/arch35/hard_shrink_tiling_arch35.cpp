 ...
 *     NEED_UPCAST 由 kernel 侧据 DTYPE_INPUT_X 编译期推导，host 仅据 dtype 计算 UB 切分，
 *     不再作为 TilingKey 维度编码（def 已通过 DTYPE_INPUT_X 宏覆盖 dtype 维度）
 */
...
static ge::graphStatus HandleEmptyTensor(gert::TilingContext* context, ge::DataType dataType)
{
    ...
    (void)dataType;
    // def 驱动 dtype：tiling_key 只编码 bufferMode，dtype 由 def 注入 DTYPE_INPUT_X 宏
...
static ge::graphStatus HardShrinkTilingFunc(gert::TilingContext* context)
{
    ...
    // def 驱动 dtype：tiling_key 只编码 bufferMode，dtype 由 def 注入 DTYPE_INPUT_X 宏，
    // NEED_UPCAST 由 kernel 侧据 DTYPE_INPUT_X 编译期推导，host 不再编码
    ASCENDC_TPL_SEL_PARAM(context, bufferMode);
