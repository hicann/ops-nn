...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnGeluMulGetWorkspaceSize”接口获取入参并根据计算流程计算所需workspace大小，再调用“aclnnGeluMul”接口执行计算。
...
- 典型场景尾轴为16的倍数，当尾轴为非32 Byte对齐时，建议走小算子拼接逻辑。
...
void PrintOutResult(std::vector<int64_t> &shape, void** deviceAddr) {
  ...
  for (int64_t i = 0; i < size; i++) {
    LOG_PRINT("aclnnGeluMul result[%ld] is: %f\n", i, resultData[i]);
  }
}
