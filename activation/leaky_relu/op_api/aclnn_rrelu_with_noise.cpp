...
static bool CheckNotNull(const aclTensor* self, const aclTensor* noise, const aclScalar* lower, const aclScalar* upper,
                         const aclTensor* out)
{
                            ...
                            int64_t noiseTensorSize)
{
    ...
    if (selfTensorSize > noiseTensorSize) {
        OP_LOGE(ACL_ERROR_INVALID_PARAM, "tesnor size of self larger than noise, self: %ld, noise: %ld.",
                selfTensorSize, noiseTensorSize);
        return false;
    }
...
static aclnnStatus CheckParams(const aclTensor* self, const aclTensor* noise, const aclScalar* lower,
                               const aclScalar* upper, bool training, const aclTensor* out, int64_t selfTensorSize,
                               int64_t noiseTensorSize)
{
    // 1. 检查参数是否为空指针
...
static aclnnStatus HandleNoiseOutput(const aclTensor* selectOut, const aclTensor* noiseContiguous,
                                     const aclTensor* self, const aclTensor* noise, const aclTensor* selfContiguous,
                                     const op::Shape& newSelfShape, const op::Shape& oldSelfShape,
                                     const op::Shape& oldNoiseShape, int64_t selfTensorSize, int64_t noiseTensorSize,
...
static aclnnStatus ComputeTrainingMode(const aclTensor* selfReshapeOut, const aclTensor* noiseContiguous,
                                       const aclTensor* self, const aclTensor* noise, const aclTensor* selfContiguous,
                                       const aclScalar* lower, const aclScalar* upper, int64_t seed, int64_t offset,
                                       const op::Shape& newSelfShape, const op::Shape& oldSelfShape,
                                       const op::Shape& oldNoiseShape, int64_t selfTensorSize, int64_t noiseTensorSize,
                                       const aclTensor** reluOut, aclOpExecutor* executor)
{
    ...
    status = HandleNoiseOutput(selectOut, noiseContiguous, self, noise, selfContiguous, newSelfShape, oldSelfShape,
                               oldNoiseShape, selfTensorSize, noiseTensorSize, &noiseOut, executor);
    CHECK_RET(status == ACLNN_SUCCESS, status);
...
aclnnStatus aclnnRReluWithNoiseGetWorkspaceSize(const aclTensor* self, const aclTensor* noise, const aclScalar* lower,
                                                const aclScalar* upper, bool training, int64_t seed, int64_t offset,
                                                aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    if (selfContiguous->GetViewShape().GetDimNum() > DIM_CONSTRAINT || self->GetViewShape() != noise->GetViewShape()) {
    ...
    if (training) {
        auto status = ComputeTrainingMode(selfReshapeOut, noiseContiguous, self, noise, selfContiguous, lower, upper,
                                          seed, offset, newSelfShape, oldSelfShape, oldNoiseShape, selfTensorSize,
                                          noiseTensorSize, &reluOut, uniqueExecutor.get());
        CHECK_RET(status == ACLNN_SUCCESS, status);
    } else {
...
aclnnStatus aclnnRReluWithNoise(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                                       ...
                                                       aclOpExecutor** executor)
{
    ...
    return aclnnRReluWithNoiseGetWorkspaceSize(self, noise, lower, upper, training, seed, offset, out, workspaceSize,
                                               executor);
}

aclnnStatus aclnnInplaceRReluWithNoise(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                       const aclrtStream stream)
{
    // 固定写法，调用框架能力，完成计算
