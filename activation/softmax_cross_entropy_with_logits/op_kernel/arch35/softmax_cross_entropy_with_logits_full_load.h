    ...
    int64_t tileNum, int64_t r, int64_t rAlign, LocalTensor<float> maxBuf, LocalTensor<T> featuresBuf)
{
    ...
    __VEC_SCOPE__
    {
        AscendC::Reg::RegTensor<T> featuresReg1;
        AscendC::Reg::RegTensor<float> maxReg;
        AscendC::Reg::RegTensor<T> featuresReg;
        AscendC::Reg::RegTensor<T> featuresRegLowest;
        AscendC::Reg::RegTensor<T> featuresRegHighest;
        AscendC::Reg::RegTensor<float> featuresRegLowest32;
        AscendC::Reg::RegTensor<float> featuresRegHighest32;
        AscendC::Reg::RegTensor<float> maxRegTemp;

        AscendC::Reg::MaskReg pregMain = AscendC::Reg::CreateMask<T, AscendC::Reg::MaskPattern::ALL>();
        AscendC::Reg::MaskReg preg = AscendC::Reg::UpdateMask<T>(tailNum);
        ...
        for (uint16_t i = 0; i < aTimes; i++) {
            AscendC::Reg::Duplicate(featuresReg, minValue);
            AscendC::Reg::LoadAlign(featuresReg1, featuresAddr + i * rAlign + repeatTimes * vfLen);
            AscendC::Reg::Max(featuresReg1, featuresReg, featuresReg1, preg);
            AscendC::Reg::Move<T, AscendC::Reg::MaskMergeMode::MERGING>(featuresReg, featuresReg1, preg);
            for (uint16_t j = 0; j < repeatTimes; j++) {
                AscendC::Reg::AddrReg offset = AscendC::Reg::CreateAddrReg<T>(i, rAlign, j, vfLen);
                AscendC::Reg::LoadAlign(featuresReg1, featuresAddr1, offset);
                AscendC::Reg::Max(featuresReg, featuresReg1, featuresReg, pregMain);
            }
            if constexpr (sizeof(T) == 2) {
                AscendC::Reg::UnPack<int32_t, int16_t, AscendC::Reg::HighLowPart::LOWEST>(
                    (AscendC::Reg::RegTensor<int32_t>&)featuresRegLowest,
                    (AscendC::Reg::RegTensor<int16_t>&)featuresReg);
                AscendC::Reg::UnPack<int32_t, int16_t, AscendC::Reg::HighLowPart::HIGHEST>(
                    (AscendC::Reg::RegTensor<int32_t>&)featuresRegHighest,
                    (AscendC::Reg::RegTensor<int16_t>&)featuresReg);
                AscendC::Reg::Cast<float, T, castB16ToB32>(featuresRegLowest32, featuresRegLowest, pregReduce);
                AscendC::Reg::Cast<float, T, castB16ToB32>(featuresRegHighest32, featuresRegHighest, pregReduce);
                AscendC::Reg::Max(maxRegTemp, featuresRegLowest32, featuresRegHighest32, pregReduce);
                AscendC::Reg::Reduce<AscendC::Reg::ReduceType::MAX>(maxReg, maxRegTemp, pregReduce);
            } else {
                AscendC::Reg::Reduce<AscendC::Reg::ReduceType::MAX>(maxReg, featuresReg, pregReduce);
            }
    ...
    LocalTensor<float> subBuf, LocalTensor<float> temp1Buf)
{
    ...
    __VEC_SCOPE__
    {
        AscendC::Reg::RegTensor<float> temp1Reg;
        ...
        for (uint16_t i = 0; i < aTimes; i++) {
            ...
            for (uint16_t j = 0; j < repeatTimes; j++) {
                AscendC::Reg::AddrReg offsetT = AscendC::Reg::CreateAddrReg<T>(i, rAlign, j, vfLen);
                AscendC::Reg::AddrReg offset = AscendC::Reg::CreateAddrReg<float>(i, rAlign, j, vfLen);
                if constexpr (sizeof(T) == 2) {
                    AscendC::Reg::LoadAlign<T, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(featuresReg, featuresAddr,
                                                                                        offsetT);
                    AscendC::Reg::Cast<float, T, castB16ToB32>(featuresReg32, featuresReg, pregMain);
                } else {
                    AscendC::Reg::LoadAlign(featuresReg32, featuresAddr, offset);
                }
                AscendC::Reg::Sub(subReg, featuresReg32, maxReg32, pregMain);
                AscendC::Reg::Exp(temp1Reg, subReg, pregMain);
                AscendC::Reg::StoreAlign(temp1Addr, temp1Reg, offset, pregMain);
            ...
            for (uint16_t k = 0; k < tailLoop; k++) {
                if constexpr (sizeof(T) == 2) {
                    ...
                    AscendC::Reg::Cast<float, T, castB16ToB32>(featuresReg32, featuresReg, preg);
                } else {
                    AscendC::Reg::LoadAlign(featuresReg32, featuresAddr + i * rAlign + repeatTimes * vfLen);
                }
                AscendC::Reg::Sub(subReg, featuresReg32, maxReg32, preg);
                AscendC::Reg::Exp(temp1Reg, subReg, preg);
                AscendC::Reg::StoreAlign(temp1Addr + i * rAlign + repeatTimes * vfLen, temp1Reg, pregAlign);
                AscendC::Reg::StoreAlign(subAddr + i * rAlign + repeatTimes * vfLen, subReg, preg);
            }
        }
    }
}
    ...
    LocalTensor<float> subBuf)
{
    ...
    __VEC_SCOPE__
    {
        ...
        AscendC::Reg::RegTensor<float> subReg;
        AscendC::Reg::RegTensor<float> temp2Reg;
        AscendC::Reg::RegTensor<T> backPropReg;
        AscendC::Reg::RegTensor<float> backPropReg32;

        AscendC::Reg::RegTensor<T> labelsReg;
        AscendC::Reg::RegTensor<float> labelsReg32;
        ...
        for (uint16_t i = 0; i < aTimes; i++) {
            ...
            for (uint16_t j = 0; j < repeatTimes; j++) {
                AscendC::Reg::AddrReg offsetT = AscendC::Reg::CreateAddrReg<T>(i, rAlign, j, vfLen);
                AscendC::Reg::AddrReg offset = AscendC::Reg::CreateAddrReg<float>(i, rAlign, j, vfLen);
                AscendC::Reg::LoadAlign(temp1Reg, temp1Addr, offset);
                AscendC::Reg::Div(temp1Reg, temp1Reg, sumReg, pregMain);
                if constexpr (sizeof(T) == 2) {
                    AscendC::Reg::LoadAlign<T, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(labelsReg, labelsAddr, offsetT);
                    AscendC::Reg::Cast<float, T, castB16ToB32>(labelsReg32, labelsReg, pregMain);
                } else {
                    AscendC::Reg::LoadAlign(labelsReg32, labelsAddr, offset);
                }
                AscendC::Reg::Sub(backPropReg32, temp1Reg, labelsReg32, pregMain);
                if constexpr (sizeof(T) == 2) {
                    AscendC::Reg::Cast<T, float, castB32ToB16>(backPropReg, backPropReg32, pregMain);
                    AscendC::Reg::StoreAlign<T, AscendC::Reg::StoreDist::DIST_PACK_B32>(backPropAddr, backPropReg,
                                                                                        offsetT, pregMain);
                } else {
                    AscendC::Reg::StoreAlign(backPropAddr, backPropReg32, offset, pregMain);
                }
                AscendC::Reg::LoadAlign(subReg, subAddr, offset);
                AscendC::Reg::Log(logReg, sumReg, pregMain);
                AscendC::Reg::Sub(temp2Reg, logReg, subReg, pregMain);
                AscendC::Reg::Mul(temp2Reg, temp2Reg, labelsReg32, pregMain);
                AscendC::Reg::StoreAlign(temp2Addr, temp2Reg, offset, pregMain);
            }

            for (uint16_t k = 0; k < tailLoop; k++) {
                AscendC::Reg::LoadAlign(temp1Reg, temp1Addr + i * rAlign + repeatTimes * vfLen);
                AscendC::Reg::Div(temp1Reg, temp1Reg, sumReg, preg);
                if constexpr (sizeof(T) == 2) {
                    AscendC::Reg::LoadAlign<T, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(
                        labelsReg, labelsAddr + i * rAlign + repeatTimes * vfLen);
                    AscendC::Reg::Cast<float, T, castB16ToB32>(labelsReg32, labelsReg, preg);
                } else {
                    AscendC::Reg::LoadAlign(labelsReg32, labelsAddr + i * rAlign + repeatTimes * vfLen);
                }
                AscendC::Reg::Sub(backPropReg32, temp1Reg, labelsReg32, preg);

                if constexpr (sizeof(T) == 2) {
                    AscendC::Reg::Cast<T, float, castB32ToB16>(backPropReg, backPropReg32, preg);
                    AscendC::Reg::StoreAlign<T, AscendC::Reg::StoreDist::DIST_PACK_B32>(
                        backPropAddr + i * rAlign + repeatTimes * vfLen, backPropReg, preg);
                } else {
                    AscendC::Reg::StoreAlign(backPropAddr + i * rAlign + repeatTimes * vfLen, backPropReg32, pregAlign);
                }
                AscendC::Reg::LoadAlign(subReg, subAddr + i * rAlign + repeatTimes * vfLen);
                AscendC::Reg::Log(logReg, sumReg, preg);
                AscendC::Reg::Sub(temp2Reg, logReg, subReg, preg);
                AscendC::Reg::Mul(temp2Reg, temp2Reg, labelsReg32, preg);
                AscendC::Reg::StoreAlign(temp2Addr + i * rAlign + repeatTimes * vfLen, temp2Reg, pregAlign);
            }
        }
    }
}
