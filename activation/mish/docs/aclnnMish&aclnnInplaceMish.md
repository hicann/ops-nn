...
- 每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnMishGetWorkspaceSize”或者“aclnnInplaceMishGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnMish”或者“aclnnInplaceMish”接口执行计算。
...
int main() {
  ...
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret); return ret);
  for (int64_t i = 0; i < size; i++) {
    LOG_PRINT("inplace result[%ld] is: %f\n", i, resultData[i]);
  }
