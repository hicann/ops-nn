...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    selu_op.set_input_##intputName(placeholder##intputIndex);                                                       \
...
{
    Status ret = SUCCESS;
    // Selu has no attributes (alpha/scale are fixed constants)
    auto selu_op = op::Selu("selu");
    std::vector<int64_t> xShape = {4, 4};
    ADD_INPUT(1, x, inDtype, xShape);

    outputs.push_back(selu_op);
    return SUCCESS;
}
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 0; i < input_num; i++) {
        ...
        uint64_t data_size = static_cast<uint64_t>(input_shape) * GetDataTypeSize(input[i].GetTensorDesc().GetDataType());
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        uint64_t data_size = static_cast<uint64_t>(output_shape) * GetDataTypeSize(output[i].GetTensorDesc().GetDataType());
