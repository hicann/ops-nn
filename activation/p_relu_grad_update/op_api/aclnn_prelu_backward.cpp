...
static inline bool CheckNotNull(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* weight,
                                const aclTensor* gradInput, const aclTensor* gradWeight)
{
...
static inline bool CheckDtypeValid(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* weight,
                                   const aclTensor* gradInput, const aclTensor* gradWeight)
{
...
static inline bool CheckShape(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* weight,
                              const aclTensor* gradInput, const aclTensor* gradWeight)
{
    ...
    if (self->GetViewShape() != broadcastShape) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                "the broadcastShape[%s] of gradOutput and self must be equal with selfShape[%s].",
...
static aclnnStatus CheckParams(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* weight,
                               const aclTensor* gradInput, const aclTensor* gradWeight)
{
    // 1. 检查参数是否为空指针
...
aclnnStatus CalcPreluBackwardL0Kernel(const aclTensor* contiguousGradOutput, const aclTensor* contiguousSelf,
                                      const aclTensor* contiguousWeight, const aclTensor*& preluGradInput,
                                      const aclTensor*& preluGradReduceOut, const op::Shape& originalSelfShape,
                                      UniqueExecutor& uniqueExecutor)
{
    ...
    preluGradReduceOut = l0op::PReluGradReduce(contiguousGradOutput, contiguousSelf, contiguousWeight, update,
                                               uniqueExecutor.get());
...
aclnnStatus aclnnPreluBackwardGetWorkspaceSize(const aclTensor* gradOutput, const aclTensor* self,
                                               const aclTensor* weight, aclTensor* gradInput, aclTensor* gradWeight,
                                               uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    CHECK_RET(CalcPreluBackwardL0Kernel(contiguousGradOutput, contiguousSelf, contiguousWeight, preluGradInput,
                                        preluGradReduceOut, originalSelfShape, uniqueExecutor) == ACLNN_SUCCESS,
