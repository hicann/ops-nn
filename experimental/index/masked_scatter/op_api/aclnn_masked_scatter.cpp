...
static bool CheckDtypeValid(const aclTensor* selfRef, const aclTensor* mask, const aclTensor* source)
{
    ...
    if (selfRef->GetDataType() != source->GetDataType()) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "source dtype %s should be same with selfRef [%s].",
...
static bool isMaskCanExpandToSelfRef(const aclTensor* selfRef, const aclTensor* mask, op::Shape* broadcastShape)
{
    ...
    for (size_t i = 0; i < selfRefShapeLen; i++) {
        if (broadcastShape->GetDim(i) != selfRefShape.GetDim(i)) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "target selfRef sizes %s ,source mask sizes %s",
...
static aclnnStatus CheckParams(const aclTensor* selfRef, const aclTensor* mask, const aclTensor* source)
{
    ...
    OP_CHECK_NULL(mask, return ACLNN_ERR_PARAM_NULLPTR);
    OP_CHECK_NULL(source, return ACLNN_ERR_PARAM_NULLPTR);

    // 2. 检查输入的数据类型是否在API支持的数据类型范围之内，需要根据api定义校验
    CHECK_RET(CheckDtypeValid(selfRef, mask, source), ACLNN_ERR_PARAM_INVALID);

    // 3. 检查格式是否支持
    if (selfRef->GetStorageFormat() != ge::Format::FORMAT_ND || mask->GetStorageFormat() != ge::Format::FORMAT_ND ||
        source->GetStorageFormat() != ge::Format::FORMAT_ND) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "selfRef, mask and source only support format ND.");
                                                      ...
                                                      aclOpExecutor** executor)
{
    ...
    // mask 0维转换维1维
    if (maskContiguous->GetViewShape().GetDimNum() == 0 && maskContiguous->Size() > 0) {
        maskContiguous = uniqueExecutor.get()->CreateView(maskContiguous, {1}, maskContiguous->GetViewOffset());
    }

    // input 0维转换维1维
    if (selfRefContiguous->GetViewShape().GetDimNum() == 0 && selfRefContiguous->Size() > 0) {
    ...
    // source 0维转换维1维
    if (sourceContiguous->GetViewShape().GetDimNum() == 0 && sourceContiguous->Size() > 0) {
