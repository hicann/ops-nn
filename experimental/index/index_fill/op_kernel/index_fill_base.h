...
public:
                                ...
                                const IndexFillTilingData* tilingData, TPipe* tPipe)
    {
        ...
        uint64_t preHalfNums = (((this->N * sizeof(half) + this->coreNum * sizeof(uint8_t) + INT8_ALIGNED_NUM - 1) /
                                 INT8_ALIGNED_NUM) *
                                INT8_ALIGNED_NUM) /
                               sizeof(half);
        this->repeatGm.SetGlobalBuffer(reinterpret_cast<__gm__ half*>(userWorkSpace) + preHalfNums);
        uint64_t preInt32Nums = (((this->N * sizeof(half) + this->coreNum * sizeof(uint8_t) + INT8_ALIGNED_NUM - 1) /
                                  INT8_ALIGNED_NUM) *
                                 INT8_ALIGNED_NUM) /
                                sizeof(int32_t);
    ...
    __aicore__ inline void GetCoreTask(uint64_t& coreDataTaskIndices, uint64_t& coreIndicesGmStart,
                                       uint64_t& coreIndicesGmEnd)
    {
    ...
    __aicore__ inline void IndicesTaskByIndicesNum()
    {
        ...
        // UB布局：wsLocal[N * sizeof(half)]，indicesLocal[INDICES_UB_LENGTH * sizeof(U)]，tilingLocal[8 *
        // sizeof(int32_t)]
        uint64_t coreDataTaskIndices = 0;
        uint64_t coreIndicesGmStart = 0;
        if (GetBlockIdx() < this->frontCoreNumTaskIndices) {
    ...
    __aicore__ inline void CopyIn(GlobalTensor<uint8_t> gmTensor, uint64_t gmStartInt8, uint64_t ubStartInt8,
                                  uint64_t mteNumsInt8)
    {
        DataCopyExtParams copyParams{1, static_cast<uint32_t>(mteNumsInt8 * sizeof(uint8_t)), 0, 0, 0};
        DataCopyPadExtParams<uint8_t> padParams{true, 0, 0, 0};
    ...
    __aicore__ inline void CopyOut(GlobalTensor<uint8_t> gmTensor, uint64_t gmStartInt8, uint64_t ubStartInt8,
                                   uint64_t mteNumsInt8)
    {
        DataCopyExtParams copyParams{1, static_cast<uint32_t>(mteNumsInt8 * sizeof(uint8_t)), 0, 0, 0};
    ...
    __aicore__ inline void coreSplit(uint64_t coreNums, uint64_t dataNums, uint64_t& frontCoreNums,
                                     uint64_t& tailCoreNums, uint64_t& frontCoreDataNums, uint64_t& tailCoreDataNums)
    {
    ...
    // 1. Centralized 8B Value Tensor Broadcasting
    __aicore__ inline void BroadcastValueTensor8B(uint64_t valTensorStartOffset, uint32_t alignedTotalElements)
    {
        if constexpr (sizeof(T) == 8) {
            LocalTensor<T> valTensorLocal = this->allUbLocal[valTensorStartOffset].template ReinterpretCast<T>();
            for (uint32_t i = 0; i < INT8_ALIGNED_NUM; i++) {
                valTensorLocal.SetValue(i, this->value);
            }
            this->PIPE_V_S();

            auto valFloat = valTensorLocal.template ReinterpretCast<float>();
            uint32_t totalFloats = alignedTotalElements * 2;
            uint32_t baseFloats = INT8_ALIGNED_NUM * 2;

            for (uint32_t i = baseFloats; i < totalFloats; i *= 2) {
                uint32_t copyLen = (i * 2 > totalFloats) ? (totalFloats - i) : i;
                DataCopy(valFloat[i], valFloat[0], copyLen);
                PipeBarrier<PIPE_V>();
            }
            this->PIPE_V_S();
        }
    ...
    // 2. Centralized Select Computation
    __aicore__ inline void ExecuteDataSelect(uint64_t xLocalStartOffset, uint64_t xLocalHalfStartOffset,
                                             uint64_t valTensorStartOffset, LocalTensor<uint8_t>& cmpMask,
                                             uint32_t alignedOrigCount, uint32_t alignedQCount)
    {
        this->xLocal = this->allUbLocal[xLocalStartOffset].template ReinterpretCast<T>();

        if constexpr (sizeof(T) == 1) {
            auto xLocalHalf = this->allUbLocal[xLocalHalfStartOffset].template ReinterpretCast<half>();
            Cast(xLocalHalf, this->xLocal, RoundMode::CAST_NONE, alignedOrigCount);
            this->PIPE_V_S();
            Select(xLocalHalf, cmpMask, xLocalHalf, static_cast<half>(this->value), SELMODE::VSEL_TENSOR_SCALAR_MODE,
                   alignedOrigCount);
            this->PIPE_V_S();
            Cast(this->xLocal, xLocalHalf, RoundMode::CAST_NONE, alignedOrigCount);
            this->PIPE_V_S();
        } else if constexpr (sizeof(T) == 8) {
            auto xLocalFloat = this->xLocal.template ReinterpretCast<float>();
            auto valTensorLocal = this->allUbLocal[valTensorStartOffset].template ReinterpretCast<T>();
            auto valueFloat = valTensorLocal.template ReinterpretCast<float>();
            Select(xLocalFloat, cmpMask, xLocalFloat, valueFloat, SELMODE::VSEL_TENSOR_TENSOR_MODE, alignedQCount);
            this->PIPE_V_S();
        } else {
            Select(this->xLocal, cmpMask, this->xLocal, this->value, SELMODE::VSEL_TENSOR_SCALAR_MODE,
                   alignedOrigCount);
            this->PIPE_V_S();
        }
    }

    // 3. Centralized Offset Gather (Adds) Computation
    __aicore__ inline void ProcessOffsetGather(LocalTensor<int32_t>& offsetLoc, uint64_t blockLength, uint32_t qCount)
    {
        uint64_t mteTimes = blockLength / INT32_ALIGNED_NUM;
        uint64_t left = blockLength - mteTimes * INT32_ALIGNED_NUM;
        for (uint64_t i = 0; i <= mteTimes; i++) {
            if (i == mteTimes && left == 0) {
                break;
            }
            uint64_t mteLength = (i == mteTimes ? left * qCount : INT32_ALIGNED_NUM * qCount);
            ...
            DataCopyPad(offsetLoc[i * INT32_ALIGNED_NUM * qCount], this->gatherGm, copyParams, padParams);
            this->PIPE_MTE2_S();

            uint32_t addsCalCount = this->AlignedToTarget(mteLength, COMPARE_ALIGNED);
            Adds(offsetLoc[i * INT32_ALIGNED_NUM * qCount], offsetLoc[i * INT32_ALIGNED_NUM * qCount],
                 static_cast<int32_t>(i * INT32_ALIGNED_NUM * sizeof(half)), addsCalCount);
            this->PIPE_V_S();
        }
        this->PIPE_MTE2_S();
    }
