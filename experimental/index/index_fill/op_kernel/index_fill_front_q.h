...
public:
    ...
    __aicore__ inline void FrontAxisQ()
    {
        ...
        uint64_t wsBlocks = this->N / WS_UB_LENGTH;
        uint64_t wsLeft = this->N - wsBlocks * WS_UB_LENGTH;

        bool singleWsBlock = (wsBlocks == 0 || (wsBlocks == 1 && wsLeft == 0));
        if (singleWsBlock) {
            uint64_t wsNums = (wsLeft > 0 ? wsLeft : WS_UB_LENGTH);
            this->CopyInWsGm(0, this->wsLocal, wsNums);
            this->PIPE_MTE2_S();
        }

        if (NCoreNum == 1 && singleWsBlock) {
            FrontAxisQFastPath();
        } else {
            FrontAxisQSlowPath(NCoreNum, wsBlocks, wsLeft, singleWsBlock);
        }
        this->PIPE_MTE3_S();
    }

    __aicore__ inline void FrontAxisQFastPath()
    {
        uint64_t totalTasks = this->P * this->N;
        uint64_t myOffset = (GetBlockIdx() >= this->corePtr) ? (GetBlockIdx() - this->corePtr) :
                                                               (GetBlockIdx() + this->coreNum - this->corePtr);

        for (uint64_t k = myOffset; k < totalTasks; k += this->coreNum) {
            uint64_t p = k / this->N;
            uint64_t n = k % this->N;
            float wsValue = this->wsLocal.GetValue(n);
            DirectCopyQ(p, n, wsValue > 0);
        }
        this->corePtr = this->WrapAdd(this->coreNum, this->corePtr, totalTasks);
    }

    __aicore__ inline void DirectCopyQ(uint64_t p, uint64_t n, bool update)
    {
        uint64_t copyTimes = this->Q / (X_UB_LENGTH / 2);
        uint64_t leftQ = this->Q - copyTimes * (X_UB_LENGTH / 2);
        for (uint64_t i = 0; i <= copyTimes; i++) {
            if (i == copyTimes && leftQ == 0) {
                break;
            }
            uint64_t QNums = (i == copyTimes ? leftQ : (X_UB_LENGTH / 2));
            uint64_t gmStartInt8 = (p * (this->N * this->Q) + n * this->Q + i * (X_UB_LENGTH / 2)) * sizeof(T);
    ...
    __aicore__ inline void FrontAxisQSlowPath(uint64_t NCoreNum, uint64_t wsBlocks, uint64_t wsLeft, bool singleWsBlock)
    {
        for (uint64_t p = 0; p < this->P; p++) {
            for (uint64_t i = 0; i <= wsBlocks; i++) {
                ...
                if (!singleWsBlock) {
                    ...
                    this->CopyInWsGm(gmStartHalf, this->wsLocal, wsNums);
                    this->PIPE_MTE2_S();
                }
    ...
    __aicore__ inline void CopyOutQ(uint64_t p, uint64_t n, uint64_t NCoreNum, bool update)
    {
        ...
        if (GetBlockIdx() - this->corePtr < frontCoreNums) {
            ...
            coreQStart = frontCoreNums * frontCoreQNums +
                         (GetBlockIdx() - this->corePtr - frontCoreNums) * tailCoreQNums;
        ...
        if (willWork) {
            ...
            for (uint64_t i = 0; i <= copyTimes; i++) {
                ...
                uint64_t QNums = (i == copyTimes ? leftQ : (X_UB_LENGTH / 2));
                uint64_t gmStartInt8 = (p * (this->N * this->Q) + n * this->Q + coreQStart + i * (X_UB_LENGTH / 2)) *
                                       sizeof(T);
                uint64_t mteNumsInt8 = QNums * sizeof(T);
                if (update) {
                    uint64_t ubStartInt8 = WS_UB_LENGTH * sizeof(half);
