...
void IndexFillTilingOp::Init()
{
    ...
    size_t userWorkspaceSize = ((tilingData.N * SIZE_OF_HALF + tilingData.coreNum * sizeof(uint8_t) + ALIGNED_NUM - 1) /
                                ALIGNED_NUM) *
                               ALIGNED_NUM;
    if (tilingData.tilingKey == 1) {
        // 增加对 8B 数据的判断，若为 8B 类型，所需 workspace 容量按 Q * 2 预留
        auto dtype = context->GetInputDesc(0)->GetDataType();
        uint32_t qMultiplier = (dtype == ge::DT_INT64 || dtype == ge::DT_DOUBLE) ? 2 : 1;
        userWorkspaceSize += tilingData.Q * qMultiplier * FLOAT_ALIGNED * sizeof(uint32_t); // front n分支
    } else if (tilingData.tilingKey == FRONT_P_KEY) {
        // 增加对 8B 的乘数 qMultiplier 支持
        auto dtype = context->GetInputDesc(0)->GetDataType();
        uint32_t qMultiplier = (dtype == ge::DT_INT64 || dtype == ge::DT_DOUBLE) ? 2 : 1;

        if (tilingData.N * tilingData.Q * qMultiplier < COMPARE_ALIGNED) {
            userWorkspaceSize += COMPARE_ALIGNED * tilingData.N * tilingData.Q * qMultiplier * SIZE_OF_HALF;
        } else {
            userWorkspaceSize += P_GM_NUMS * qMultiplier * SIZE_OF_HALF; // front p分支
        }
    } else if (tilingData.tilingKey == TAIL_N_KEY) {
        // 【新增】为 TAIL_N_KEY (3) 分支的 8B 数据预留 gatherGm 的空间
        // 最大 nBlockLength 为 6144，qCount 为 2，类型为 int32_t
        auto dtype = context->GetInputDesc(0)->GetDataType();
        if (dtype == ge::DT_INT64 || dtype == ge::DT_DOUBLE) {
            userWorkspaceSize += 6144 * 2 * sizeof(int32_t); // 49152 Bytes
        }
    } else if (tilingData.tilingKey == TAIL_P_KEY) {
        // 【修改点】增加对 8B 数据 qMultiplier 的支持，并为 gatherGm 分配内存
        auto dtype = context->GetInputDesc(0)->GetDataType();
        uint32_t qMultiplier = (dtype == ge::DT_INT64 || dtype == ge::DT_DOUBLE) ? 2 : 1;
        userWorkspaceSize += tilingData.N * qMultiplier * HALF_ALIGNED * SIZE_OF_HALF; // tail p分支
        if (qMultiplier == 2) {
            userWorkspaceSize += tilingData.N * 2 * sizeof(int32_t); // 额外为 gatherGm 分配空间
        }
    }
...
IMPL_OP_OPTILING(IndexFill).Tiling(IndexFillTilingFunc).TilingParse<IndexFillCompileInfo>(TilingPrepareForIndexFill);
} // namespace optiling
