# aclnnSigmoid 与 aclnnInplaceSigmoid
...
- `aclnnSigmoid`：对输入 Tensor 执行 Sigmoid 计算，并将结果写入独立输出 Tensor。
- `aclnnInplaceSigmoid`：对输入 Tensor 原地执行 Sigmoid 计算，结果直接覆盖输入内存。
- `experimental/activation/sigmoid_v2` 目录对外导出的 ACLNN 接口名与原实现保持一致。
...
每个算子分为[两段式接口](../../../../docs/zh/context/two_phase_api.md)，必须先调用 `aclnnSigmoidGetWorkspaceSize` 或 `aclnnInplaceSigmoidGetWorkspaceSize` 获取执行器和 workspace 大小，再调用第二段接口执行计算。
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在 Device 侧申请的 workspace 大小，由第一段接口 aclnnSigmoidGetWorkspaceSize 获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在 Device 侧申请的 workspace 大小，由第一段接口 aclnnInplaceSigmoidGetWorkspaceSize 获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 输入 dtype 仅支持 `FLOAT`、`FLOAT16`、`BFLOAT16`。
- `aclnnSigmoid` 中，`self` 和 `out` 的 dtype 必须一致。
- `aclnnSigmoid` 中，`self` 和 `out` 的 shape 必须一致。
- 维度范围为 0 到 8。
- 支持空 Tensor。
- 支持非连续 Tensor，内部会执行 `Contiguous` 和 `ViewCopy`。
- `FLOAT16` 和 `BFLOAT16` 路径在 kernel 内部采用升精度计算。

## 实现说明

- `op_host/op_api/aclnn_sigmoid.cpp` 提供对外 ACLNN 两段式接口。
- `op_host/op_api/sigmoid.h` 和 `op_host/op_api/sigmoid.cpp` 提供内部 `l0op::Sigmoid` 封装，当前由 `aclnn_sigmoid.cpp` 直接调用。

## 调用示例

完整示例见 [../examples/test_aclnn_sigmoid.cpp](../examples/test_aclnn_sigmoid.cpp)。
