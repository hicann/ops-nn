...
/**
 * \file hard_sigmoid_grad_v3_def.cpp
 * \brief HardSigmoidGradV3 operator definition
 *
 * Inputs:
...
namespace ops {
class HardSigmoidGradV3 : public OpDef {
public:
    explicit HardSigmoidGradV3(const char* name) : OpDef(name)
    {
        ...
        aicoreConfig910B.DynamicCompileStaticFlag(true)
            ...
            .ExtendCfgInfo("opFile.value", "hard_sigmoid_grad_v3");
        this->AICore().AddConfig("ascend910b", aicoreConfig910B);
    }
};
OP_ADD(HardSigmoidGradV3);
} // namespace ops
