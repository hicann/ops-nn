...
int main()
{
    ...
    std::vector<int64_t> yShape = {777, 511};
    std::vector<int64_t> dyShape = {777, 511};
    std::vector<int64_t> dxShape = {777, 511};
    ...
    // y = tanh(x), values in (-1, 1)
    auto size = GetShapeSize(yShape);
    std::vector<float> yHostData(size);
    std::vector<float> dyHostData(size);
    std::vector<float> dxHostData(size, 0.0f);
    for (int64_t i = 0; i < size; i++) {
        yHostData[i] = 0.5f + 0.5f * static_cast<float>(i) / static_cast<float>(size); // [0.5, 1.0)
        dyHostData[i] = 4.5f;
    }
