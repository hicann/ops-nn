...
- 算子功能：aclnnSiluBackward是[aclnnSilu](../../../../activation/swish/docs/aclnnSilu.md)激活函数的反向传播，用于计算SiLU激活函数的梯度。
- 计算公式：
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      <td>gradOutput</td>
      <td>输入</td>
      <td>表示SiLU激活函数正向输出的梯度，公式中的gradOutput。</td>
    ...
    <tr>
      <td>gradInput</td>
      <td>输出</td>
      <td><ul><li>backward计算的输出，为SiLU正向输入的梯度值，即对输入进行求导后的结果。</li><li>公式中的gradInput。</li></ul></td>
      <td><ul><li>支持空Tensor。</li><li>gradOutput、self与gradInput的shape一致。</li><li>gradOutput、self与gradInput的数据类型一致。</li></ul></td>
      <td>BFLOAT16、FLOAT16、FLOAT</td>
      <td>ND</td>
      <td>0-8</td>
...
int main() {
  ...
  std::vector<float> gradOutputHostData = {1, 1, 1, 1, 1, 1};
  ...
  ret = aclnnSiluBackwardGetWorkspaceSize(gradOutput, self, gradInput, &workspaceSize, &executor);
