...
void DestroyResources(aclTensor* self, aclTensor* out, void* selfDeviceAddr, void* outDeviceAddr, void* workspaceAddr,
...
template <typename T>
std::vector<float> CalculateExpectedLogSoftmax(const std::vector<T>& input, const std::vector<int64_t>& shape,
                                               int64_t dim)
{
...
int main(int argc, char* argv[])
{
    ...
    if (dataType == ACL_FLOAT) {
        ...
        CHECK_RET(ret == ACL_SUCCESS,
                  DestroyResources(self, out, selfDeviceAddr, outDeviceAddr, nullptr, stream, deviceId);
                  return ret);
        ...
        ret = aclrtMemcpy(resultData.data(), totalSize * sizeof(float), outDeviceAddr, totalSize * sizeof(float),
        ...
        ret = aclrtMemcpy(resultDataHalf.data(), totalSize * sizeof(half), outDeviceAddr, totalSize * sizeof(half),
                          ACL_MEMCPY_DEVICE_TO_HOST);
        CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("copy result from device to host failed. ERROR: %d\n", ret);
                  DestroyResources(self, out, selfDeviceAddr, outDeviceAddr, workspaceAddr, stream, deviceId);
                  return ret);
