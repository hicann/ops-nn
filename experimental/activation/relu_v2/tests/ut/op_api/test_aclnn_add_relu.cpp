...
protected:
    static void SetUpTestCase() { cout << "add_test SetUp" << endl; }

    static void TearDownTestCase() { cout << "add_test TearDown" << endl; }
};
...
TEST_F(l2_add_relu_test, case_nullptr)
{
    ...
    auto ut = OP_API_UT(aclnnAddRelu, INPUT((aclTensor*)nullptr, (aclTensor*)nullptr, (aclScalar*)nullptr),
                        OUTPUT(tensor_desc));
...
TEST_F(l2_add_relu_test, case_022)
{
    ...
    out_tensor_desc = TensorDesc({10, 5, 10, 5}, ACL_FLOAT, ACL_FORMAT_NHWC)
                          .ValueRange(-2, 2)
                          .Precision(0.0001, 0.0001);
    scalar_desc = ScalarDesc(5.9f);

    auto ut_2 = OP_API_UT(aclnnAddRelu, INPUT(self_tensor_desc, other_tensor_desc, scalar_desc),
