                                            ...
                                            aclOpExecutor** executor)
{
    ...
    // UINT8 全为非负数，ReLU 等价于恒等变换，不需走 relu 计算（kernel 也不支持 uint8），
    // 直接 ViewCopy 输出即可。
    const aclTensor* reluOpOut = nullptr;
    if (selfContiguous->GetDataType() == op::DataType::DT_UINT8) {
        reluOpOut = selfContiguous;
    } else {
        reluOpOut = l0op::ReluV2(selfContiguous, uniqueExecutor.get());
    }
