...
static bool CheckDtypeValid(const aclTensor* gradOutput, const aclTensor* result, aclTensor* gradInput)
{
    ...
    OP_CHECK_DTYPE_NOT_MATCH(result, gradOutput->GetDataType(), return false);
    OP_CHECK_DTYPE_NOT_MATCH(gradInput, gradOutput->GetDataType(), return false);
    return true;
}
                                          ...
                                          aclIntArray* valuePerm = nullptr)
{
    ...
    OP_CHECK(reshapeSelf != nullptr, OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "ReshapeLongTensor failed."), return nullptr);
    return reshapeSelf;
}
