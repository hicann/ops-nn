...
/*!
 * \file ascend_anti_quant_v2_per_channel_base.h
 * \brief ascendantiquantv2 kernel
 */

#ifndef ASCEND_ANTI_QUANT_V2_PER_CHANNEL_BASE_H_
#define ASCEND_ANTI_QUANT_V2_PER_CHANNEL_BASE_H_
...
class AscendAntiQuantV2PerChannelBase : public AscendAntiQuantV2Base<T, T1, T2, U, SqrtMode> {
public:
    __aicore__ inline AscendAntiQuantV2PerChannelBase(const AscendAntiQuantV2TilingData* tilingData)
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::Init(GM_ADDR x, GM_ADDR scale,
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::Process()
{
    ...
    for (int64_t i = 0; i < lenLoopNum; ++i) {
        ...
        if constexpr (IsSameType<T1, bfloat16_t>::value) {
            AscendC::Cast<float, bfloat16_t>(floato, oLocal, AscendC::RoundMode::CAST_NONE, tilingData_->baseLen);
        }
        ...
        if constexpr (IsSameType<T1, float>::value) {
            ...
            CopyXAndCompute(tilingData_->baseLen, gmXOffset_ + i * tilingData_->baseLen, this->floats, floato);
        }
    ...
    if (lenLoopTail != 0) {
        ...
        if constexpr (IsSameType<T1, bfloat16_t>::value) {
            // bf16
            AscendC::Cast<float, bfloat16_t>(floats, sLocal, AscendC::RoundMode::CAST_NONE, lenLoopTail);
        }
        ...
        if constexpr (SqrtMode == TPL_SQRT_MODE) {
            if constexpr (IsSameType<T1, float>::value) {
                ...
                AscendC::Mul(floats, floats, floats, lenLoopTail);
            }
        }
        // ld and cast for offset
        if constexpr (IsSameType<T1, bfloat16_t>::value) {
            AscendC::Cast<float, bfloat16_t>(floato, oLocal, AscendC::RoundMode::CAST_NONE, lenLoopTail);
        }
        ...
        if constexpr (IsSameType<T1, float>::value) {
            ...
            CopyXAndCompute(lenLoopTail, gmXOffset_ + lenLoopNum * tilingData_->baseLen, floats, floato);
        }
        inQueueScale_.FreeTensor(sLocal);
        inQueueOffset_.FreeTensor(oLocal);
    }
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::ParseCoreBlocks(
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::CopyInScale(int64_t sLen,
                                                                                            int64_t sInOffset)
{
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::CopyInOffset(int64_t sLen,
                                                                                             int64_t sInOffset)
{
    DataCopyExtParams copyParams;
    copyParams.blockCount = 1;
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::CopyXAndCompute(
    int64_t dataCount, int64_t offset, LocalTensor<float>& sFPLocal, LocalTensor<float>& oFpLocal)
{
    ...
    for (int64_t nIdx = 0; nIdx < nLoopNum; ++nIdx) {
        ...
        Compute(tilingData_->baseN, dataCount, sFPLocal, oFpLocal);
    ...
    if (nLoopTail != 0) {
        ...
        Compute(nLoopTail, dataCount, sFPLocal, oFpLocal);
        CopyOutY(nLoopTail, dataCount, xOffset);
    }
}

template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::CopyInX(int64_t xN, int64_t xLen,
                                                                                        int64_t xInOffset)
{
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::Compute(int64_t nRow, int64_t dataCount,
                                                                                        LocalTensor<float>& sFPLocal,
                                                                                        LocalTensor<float>& oFpLocal)
{
    ...
    for (uint16_t j = 0; j < static_cast<uint16_t>(nRow); ++j) {
        ...
        // cast and sd for y
        AscendC::Cast(outLocal[j * tilingData_->baseLen], floatX, AscendC::RoundMode::CAST_RINT, dataCount);
    }
...
template <typename T, typename T1, typename T2, typename U, uint64_t SqrtMode>
__aicore__ inline void AscendAntiQuantV2PerChannelBase<T, T1, T2, U, SqrtMode>::CopyOutY(int64_t yN, int64_t yLen,
                                                                                         int64_t yOutOffset)
{
