                                                                     ...
                                                                     aclOpExecutor* executor)
{
    ...
    if (sumDyXmuOut == nullptr) {
        OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "SyncBatchNormBackwardReduce AllocTensor for sumDyXmuOut failed.");
        return std::tuple<aclTensor*, aclTensor*>(nullptr, nullptr);
    }
    auto gradWeightOut = executor->AllocTensor(invstd->GetViewShape(), invstd->GetDataType(), invstd->GetViewFormat());
    if (gradWeightOut == nullptr) {
        OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "SyncBatchNormBackwardReduce AllocTensor for gradWeightOut failed.");
        return std::tuple<aclTensor*, aclTensor*>(nullptr, nullptr);
    }
