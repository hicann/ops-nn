...
static const std::initializer_list<op::DataType> SAMPLE_COUNT_DTYPE_SUPPORT_LIST = {
...
{
    OP_CHECK_DTYPE_NOT_SUPPORT(totalSum, DTYPE_SUPPORT_LIST, return false);
    OP_CHECK_DTYPE_NOT_SUPPORT(totalSquareSum, DTYPE_SUPPORT_LIST, return false);
    OP_CHECK_DTYPE_NOT_SUPPORT(sampleCount, SAMPLE_COUNT_DTYPE_SUPPORT_LIST, return false);
    OP_CHECK_DTYPE_NOT_SUPPORT(mean, DTYPE_SUPPORT_LIST, return false);
    OP_CHECK_DTYPE_NOT_SUPPORT(variance, DTYPE_SUPPORT_LIST, return false);
    OP_CHECK_DTYPE_NOT_SUPPORT(batchMean, DTYPE_SUPPORT_LIST, return false);
    OP_CHECK_DTYPE_NOT_SUPPORT(batchInvstd, DTYPE_SUPPORT_LIST, return false);
    ...
    OP_CHECK_DTYPE_NOT_SAME(totalSum, batchMean, return false);
...
static bool CheckAttrs(float momentum, float eps)
{
    if (!(eps > 0.0f)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "eps must be greater than 0, but got %f.", eps);
        return false;
    }
    if (!(momentum >= 0.0f && momentum <= 1.0f)) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "momentum must be in [0, 1], but got %f.", momentum);
        return false;
    }
    return true;
}
                               ...
                               float momentum, float eps)
{
    ...
    CHECK_RET(CheckShape(totalSum, totalSquareSum, sampleCount, mean, variance, batchMean, batchInvstd),
              ACLNN_ERR_PARAM_INVALID);
    CHECK_RET(CheckAttrs(momentum, eps), ACLNN_ERR_PARAM_INVALID);
                                        ...
                                        aclOpExecutor* executor)
{
    ...
    // 固定写法，将输入 sampleCount 转换成连续的 tensor
    ...
    std::array<aclTensor*, SYNC_BATCH_NORM_OUTPUT_NUM> outTensor = l0op::SyncBatchNormGatherStats(
        totalSumContiguous, totalSquareSumContiguous, sampleCountContiguous, meanFinal, varFinal, momentum, eps,
        executor);
                                                          ...
                                                          uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    // 固定写法，参数检查
    auto ret = CheckParams(totalSum, totalSquareSum, sampleCount, mean, variance, batchMean, batchInvstd, momentum,
                           eps);
