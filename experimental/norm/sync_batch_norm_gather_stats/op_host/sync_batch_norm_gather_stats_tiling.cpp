...
/*!
 * \file sync_batch_norm_gather_stats_tiling.cpp
 * \brief
 */
#include <algorithm>
#include <cstdlib>
#include "sync_batch_norm_gather_stats_tiling.h"
#include "log/log.h"
...
// 每核 C 下限：310P arch20 cache line 正确性约束，见 CheckArch20CacheLineInvariant()
static constexpr int16_t PER_CORE_MIN_C_BYTE = 64;
static constexpr int16_t PER_CORE_MIN_C_ELEM = 16;
static constexpr int64_t MIN_BLOCK_NUM = 4;
...
// 每个 C 元素的 UB 占用，与 op_kernel InitBuffers 逐项对应，改 buffer 构成须同步
static constexpr int64_t UB_PER_C_NTILE_B32 = 8;  // sumQueue + squareSumQueue，各 nTileNum*4B
static constexpr int64_t UB_PER_C_CONST_B32 = 48; // 2*4(running in) + 4*4(outQueue) + 6*4(fp32 暂存)
static constexpr int64_t UB_PER_C_NTILE_B16 = 12; // 上面两个队列(各 2B) + 两个 fp32 cast buf(各 4B)
static constexpr int64_t UB_PER_C_CONST_B16 = 36; // 2*2(running in) + 4*2(outQueue) + 6*4(fp32 暂存)
static constexpr int64_t BLOCK_BYTE_SIZE = 32;
static constexpr int64_t BIG_C_N_TILE_NUM_B32 = 16;
static constexpr int64_t BIG_C_N_TILE_NUM_B16 = 32;
static constexpr int64_t SAMPLE_COUNT_TILE_NUM_B32 = 256;
static constexpr int64_t BIG_C_MIN_CLOOP_B32 = 2;
static constexpr int64_t BIG_C_MIN_CLOOP_B16 = 8;
...
static constexpr int16_t CONST_ONE_HUNDRED_TWENTY_EIGHT = 128;
static constexpr int16_t CONST_FIVE_HUNDRED_TWELVE = 512;
...
bool SyncBatchNormGatherStatsTiling::TotalSumShapeCheck()
{
    ...
    OP_CHECK_IF(
        (totalSumDType_ != ge::DT_FLOAT16 && totalSumDType_ != ge::DT_FLOAT),
...
bool SyncBatchNormGatherStatsTiling::TotalSquareSumShapeCheck()
{
    ...
    OP_CHECK_IF((totalSquareSumDType_ != ge::DT_FLOAT16 && totalSquareSumDType_ != ge::DT_FLOAT),
...
bool SyncBatchNormGatherStatsTiling::SampleCountShapeCheck()
{
    ...
    OP_CHECK_IF(
        (sampleCountDType_ != ge::DT_INT32 && sampleCountDType_ != ge::DT_FLOAT16 && sampleCountDType_ != ge::DT_FLOAT),
        OP_LOGE_FOR_INVALID_DTYPE(opName, "sample_count",
                                  ge::TypeUtils::DataTypeToSerialString(sampleCountDType_).c_str(),
                                  "int32, float16 and float"),
        return false);
...
bool SyncBatchNormGatherStatsTiling::MeanShapeCheck()
{
    ...
    OP_CHECK_IF((meanDType_ != ge::DT_FLOAT16 && meanDType_ != ge::DT_FLOAT),
...
bool SyncBatchNormGatherStatsTiling::VarShapeCheck()
{
    ...
    OP_CHECK_IF((varDType_ != ge::DT_FLOAT16 && varDType_ != ge::DT_FLOAT),
...
bool SyncBatchNormGatherStatsTiling::BatchMeanCheck()
{
    ...
    OP_CHECK_IF(
        (batchMeanDType_ != ge::DT_FLOAT16 && batchMeanDType_ != ge::DT_FLOAT),
...
bool SyncBatchNormGatherStatsTiling::BatchInvstdCheck()
{
    ...
    OP_CHECK_IF((batchInvStdDType_ != ge::DT_FLOAT16 && batchInvStdDType_ != ge::DT_FLOAT),
...
bool SyncBatchNormGatherStatsTiling::RunningMeanCheck()
{
    ...
    OP_CHECK_IF(
        (runningMeanDType_ != ge::DT_FLOAT16 && runningMeanDType_ != ge::DT_FLOAT),
...
bool SyncBatchNormGatherStatsTiling::RunningVarCheck()
{
    ...
    OP_CHECK_IF(
        (runningVarDType_ != ge::DT_FLOAT16 && runningVarDType_ != ge::DT_FLOAT),
...
// Get attributes
ge::graphStatus SyncBatchNormGatherStatsTiling::GetAttrs()
{
...
ge::graphStatus SyncBatchNormGatherStatsTiling::GetPlatformInfo()
{
    ...
    socIs310P_ = compileInfo->socIs310P;
...
ge::graphStatus SyncBatchNormGatherStatsTiling::DoNNotFullLoadTiling()
{
    ...
    if (CheckArch20CacheLineInvariant(dTypeSize, cBlockFactor) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }

    if (cLen <= ALIGN_FACTOR_BASE_BYTE_LEN) {
        ComputeSmallCNNotFullLoad(dTypeSize, cBlockFactor, cTileBlockFactor);
        FillNNotFullLoadTilingData();
        return ge::GRAPH_SUCCESS;
    }
    if (SearchNNotFullLoadTiling(dTypeSize, cBlockFactor, cTileBlockFactor) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    FillNNotFullLoadTilingData();
    return ge::GRAPH_SUCCESS;
...
void SyncBatchNormGatherStatsTiling::ComputeSmallCNNotFullLoad(uint32_t dTypeSize, int64_t cBlockFactor,
                                                               int64_t cTileBlockFactor)
{
    cTileNum_ = cBlockFactor;
    nTileNum_ = (dTypeSize == CONST_FOUR) ? CONST_FIVE_HUNDRED_TWELVE : CONST_ONE_HUNDRED_TWENTY_EIGHT;
    const int64_t nLoop = nLen / nTileNum_;
    const int64_t nTotalLoop = Ops::Base::CeilDiv(nLen, nTileNum_);
    basicBlockLoop_ = FindNearestPower2(nTotalLoop);
    mainFoldCount_ = nLoop - basicBlockLoop_;
    nTail_ = nLen - nLoop * nTileNum_;
    cacheBufferCount_ = CONST_ONE;
    resultCacheID_ = CONST_ZERO;
    if (basicBlockLoop_ != 0) {
        cacheBufferCount_ = CONST_SIXTY_FOUR - __builtin_clzl(basicBlockLoop_);
        resultCacheID_ = GetCacheID(basicBlockLoop_ - 1);
    }
    cLoopMain_ = CONST_ONE;
    cTailMain_ = cBlockFactor;
    cLoopTail_ = CONST_ONE;
    cTailTail_ = cTileBlockFactor;
}

ge::graphStatus SyncBatchNormGatherStatsTiling::SearchNNotFullLoadTiling(uint32_t dTypeSize, int64_t cBlockFactor,
                                                                         int64_t cTileBlockFactor)
{
    if (dTypeSize == 0) {
        return ge::GRAPH_FAILED;
    }
    int64_t cTileNumBase = BLOCK_BYTE_SIZE / dTypeSize;
    if (cTileNumBase > cBlockFactor) {
        cTileNumBase = cBlockFactor;
    }

    std::vector<int64_t> nTileNumList = {64, 128, 256, 512};
    for (size_t idx = 0; idx < nTileNumList.size(); ++idx) {
        if (!TrySearchNTileNum(nTileNumList[idx], idx == 0, dTypeSize, cTileNumBase, cBlockFactor, cTileBlockFactor)) {
            break;
        }
    }
    ...
    const int64_t bigCNTileNum = (dTypeSize == CONST_FOUR) ? BIG_C_N_TILE_NUM_B32 : BIG_C_N_TILE_NUM_B16;
    const int64_t minCLoopToApply = (dTypeSize == CONST_FOUR) ? BIG_C_MIN_CLOOP_B32 : BIG_C_MIN_CLOOP_B16;
    if (cLoopMain_ >= minCLoopToApply) {
        const int64_t baselineCTile = cTileNum_;
        const int64_t baselineNTile = nTileNum_;
        bool searchOk = false;
        if (dTypeSize == CONST_FOUR) {
            searchOk = TrySearchNTileNum(bigCNTileNum, true, dTypeSize, cTileNumBase, cBlockFactor, cTileBlockFactor);
            if (!searchOk || cTileNum_ < cBlockFactor) {
                searchOk = TrySearchNTileNum(CONST_ONE, true, dTypeSize, cTileNumBase, cBlockFactor, cTileBlockFactor);
            }
        } else {
            searchOk = TrySearchNTileNum(bigCNTileNum, true, dTypeSize, cTileNumBase, cBlockFactor, cTileBlockFactor);
        }
        if (!searchOk) {
            TrySearchNTileNum(baselineNTile, true, dTypeSize, cTileNumBase, cBlockFactor, cTileBlockFactor);
        }
        OP_LOGD(opName, "big-C bandwidth tiling: cFactor %ld -> %ld, nFactor %ld -> %ld, blockLen %ld -> %ld B",
                baselineCTile, cTileNum_, baselineNTile, nTileNum_, baselineCTile * dTypeSize, cTileNum_ * dTypeSize);
    }
...
bool SyncBatchNormGatherStatsTiling::TrySearchNTileNum(int64_t nTileNum, bool isFirst, uint32_t dTypeSize,
                                                       int64_t cTileNumBase, int64_t cBlockFactor,
                                                       int64_t cTileBlockFactor)
{
    if (dTypeSize == 0 || cTileNumBase == 0 || nTileNum == 0) {
        return false;
    }
    ...
    int64_t cTileNum = 0;
    int64_t countTileNum = nTileNum;
    if (dTypeSize == CONST_FOUR && (nTileNum == CONST_ONE || nTileNum == BIG_C_N_TILE_NUM_B32)) {
        countTileNum = std::min(nLen, SAMPLE_COUNT_TILE_NUM_B32);
    }
    const int64_t nAlignCount = Ops::Base::CeilDiv(countTileNum, static_cast<int64_t>(BLOCK_BYTE_SIZE / CONST_FOUR)) *
                                (BLOCK_BYTE_SIZE / CONST_FOUR);
    const int64_t ubAvail = ubSize_ - UBSIZE_RSV - nAlignCount * CONST_FOUR - blockSize_;
    if (dTypeSize == CONST_FOUR) {
        cTileNum = ubAvail / (UB_PER_C_NTILE_B32 * nTileNum + UB_PER_C_CONST_B32);
    } else if (dTypeSize == CONST_TWO) {
        cTileNum = ubAvail / (UB_PER_C_NTILE_B16 * nTileNum + UB_PER_C_CONST_B16);
    }
    cTileNum = cTileNum / cTileNumBase * cTileNumBase;

    cTileNum = std::min(cTileNum, cBlockFactor);

    if ((cTileNum <= cBlockFactor && !isFirst) || cTileNum <= 0) {
        return false;
    }

    cTileNum_ = cTileNum;
    nTileNum_ = std::min(nTileNum, nLen);
    basicBlockLoop_ = basicBlockLoop;
    mainFoldCount_ = mainFoldCount;
    nTail_ = nTail;
    cacheBufferCount_ = cacheBufferCount;
    resultCacheID_ = resultCacheID;
    cLoopMain_ = Ops::Base::CeilDiv(cBlockFactor, cTileNum);
    cTailMain_ = cBlockFactor - (cLoopMain_ - 1) * cTileNum;
    cLoopTail_ = Ops::Base::CeilDiv(cTileBlockFactor, cTileNum);
    cTailTail_ = cTileBlockFactor - (cLoopTail_ - 1) * cTileNum;
    return true;
}

void SyncBatchNormGatherStatsTiling::FillNNotFullLoadTilingData()
{
    auto* tilingData = context_->GetTilingData<SyncBatchNormGatherStatsNNotFullLoadTilingData>();
    tilingData->blockDim = blockNum;
    tilingData->cLen = cLen;
    tilingData->cFactor = cTileNum_;
    tilingData->cLoopMainBlock = cLoopMain_;
    tilingData->cTileMainBlock = cTailMain_;
    tilingData->cLoopTailBlock = cLoopTail_;
    tilingData->cTailTailBlock = cTailTail_;
    tilingData->nFactor = nTileNum_;
    tilingData->nLoop = basicBlockLoop_;
    tilingData->nMainFoldCount = mainFoldCount_;
    tilingData->nTail = nTail_;
    tilingData->cacheBufferCount = cacheBufferCount_;
    tilingData->resultCacheId = resultCacheID_;
...
ge::graphStatus SyncBatchNormGatherStatsTiling::DoOpTiling()
{
    ...
    if (!socIs310P_ && dTypeSize == FLOAT_DTYPE_SIZE && blockFormer > 1) {
        return DoNNotFullLoadTiling();
    }
    blockTail = ubOuter % blockFormer;
    blockTail = (blockTail == 0) ? blockFormer : blockTail;
    blockNum = (ubOuter + blockFormer - 1) / blockFormer;
    RebalanceBlocksForHalfCore(dTypeSize);
    AlignPerCoreToCacheLine(dTypeSize);

    if (CheckArch20CacheLineInvariant(dTypeSize, cTileNum_) != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
...
{
    auto* tilingData = context_->GetTilingData<SyncBatchNormGatherStatsTilingData>();
    tilingData->blockDim = blockNum;
    tilingData->blockFormer = blockFormer;
    tilingData->blockTail = blockTail;
    tilingData->nLen = nLen;
    tilingData->cLen = cLen;
    tilingData->ubFormer = cTileNum_;
    tilingData->ubTail = ubTail;
    tilingData->momentum = momentum_;
    tilingData->eps = eps_;
}

// 断言 310P arch20 每核 C 切片 >= 一条 GM cache line(64B)，防多核回写互覆
ge::graphStatus SyncBatchNormGatherStatsTiling::CheckArch20CacheLineInvariant(uint32_t dTypeSize,
                                                                              int64_t perCoreCNum) const
{
    if (!socIs310P_ || blockNum <= 1) {
        return ge::GRAPH_SUCCESS;
    }
    const int64_t perCoreBytes = perCoreCNum * static_cast<int64_t>(dTypeSize);
    OP_CHECK_IF(perCoreBytes < static_cast<int64_t>(PER_CORE_MIN_C_BYTE),
                OP_LOGE(context_->GetNodeName(),
                        "ascend310p requires at least %d bytes of C per core when blockDim > 1, but got %ld "
                        "bytes (blockDim=%u). The arch20 scalar kernel writes GM through the data cache and "
                        "flushes whole cache lines, so smaller slices lose stores.",
                        static_cast<int>(PER_CORE_MIN_C_BYTE), perCoreBytes, blockNum),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(perCoreBytes % static_cast<int64_t>(PER_CORE_MIN_C_BYTE) != 0,
                OP_LOGE(context_->GetNodeName(),
                        "ascend310p requires the per-core C slice to be a multiple of %d bytes (one GM data "
                        "cache line) when blockDim > 1, but got %ld bytes (blockDim=%u). Otherwise two cores "
                        "share a cache line and their whole-line write-backs overwrite each other.",
                        static_cast<int>(PER_CORE_MIN_C_BYTE), perCoreBytes, blockNum),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

// 310P: 每核 C 对齐到 PER_CORE_MIN_C_BYTE(64) 整数倍，消除跨核共享 cache line
void SyncBatchNormGatherStatsTiling::AlignPerCoreToCacheLine(uint32_t dTypeSize)
{
    if (!socIs310P_ || dTypeSize == 0 || cTileNum_ <= 0) {
        return;
    }
    const int64_t elemsPerLine = static_cast<int64_t>(PER_CORE_MIN_C_BYTE) / dTypeSize;
    if (elemsPerLine <= 0 || cTileNum_ % elemsPerLine == 0) {
        return;
    }
    const int64_t aligned = cTileNum_ / elemsPerLine * elemsPerLine;
    cTileNum_ = (aligned > 0) ? aligned : elemsPerLine;

    // 重算分核参数（与 RebalanceBlocksForHalfCore 末尾一致）
    ubOuter = (cLen + cTileNum_ - 1) / cTileNum_;
    ubTail = cLen % cTileNum_;
    ubTail = (ubTail == 0) ? cTileNum_ : ubTail;
    blockFormer = (ubOuter + coreNum_ - 1) / coreNum_;
...
void SyncBatchNormGatherStatsTiling::RebalanceBlocksForHalfCore(uint32_t dTypeSize)
{
    if (dTypeSize == 0) {
        return;
    }
    ...
    if (static_cast<int64_t>(blockNum) < (coreNum_ / HALF_CORE_NUM)) {
        ...
        // 每核不足下限元素数时按下限取
        const int64_t byteLowest = static_cast<int64_t>(PER_CORE_MIN_C_BYTE) / dTypeSize;
        lowestcTileNum = byteLowest;
        if (!socIs310P_) {
            const int64_t tileByByte = std::max(alignDimPerCore, byteLowest);
            const int64_t blockByByte = (cLen + tileByByte - 1) / tileByByte;
            if (blockByByte < MIN_BLOCK_NUM) {
                lowestcTileNum = static_cast<int64_t>(PER_CORE_MIN_C_ELEM);
            }
        }
        if (alignDimPerCore < lowestcTileNum) {
            alignDimPerCore = lowestcTileNum;
        }
...
ge::graphStatus SyncBatchNormGatherStatsTiling::DoTiling()
{
    ...
    if (DoLibApiTiling() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    if (GetWorkspaceSize() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    if (PostTiling() != ge::GRAPH_SUCCESS) {
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus SyncBatchNormGatherStatsParse(gert::TilingParseContext* context)
{
    ...
    compileInfoPtr->socIs310P = (ascendcPlatform.GetSocVersion() == platform_ascendc::SocVersion::ASCEND310P);
