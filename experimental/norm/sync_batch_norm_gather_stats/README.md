...
- 不支持空 Tensor：所有输入（total_sum、total_square_sum、sample_count、mean、variance）与输出（batch_mean、batch_invstd）均不可为空 Tensor。
- 维度约束：total_sum、total_square_sum 为 2 维 [N, C]，且两者的 N、C 分别相等；sample_count、mean、variance 为 1 维 [C]。
- 数据类型：total_sum、total_square_sum、mean、variance、batch_mean、batch_invstd 支持 FLOAT16、FLOAT32，且这些张量的数据类型必须一致；sample_count 支持 INT32、FLOAT16、FLOAT32。
- 属性约束：momentum、eps 均为必需（REQUIRED）的 float 属性；其中 eps 必须大于 0，momentum 取值范围为 [0, 1]。
...
| ---- | ---- | ---- | ---- | ---- |
| 干炒牛河 | 个人开发者 | SyncBatchNormGatherStats | 2026/07 | SyncBatchNormGatherStats算子适配开源仓 |
