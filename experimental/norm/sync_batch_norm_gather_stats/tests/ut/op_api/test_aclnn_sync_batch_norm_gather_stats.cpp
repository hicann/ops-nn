...
// eps 必须为正：eps <= 0 时 sqrt(batchVar + eps) 失去正数保证，方差为 0 的通道会得到 Inf。
TEST_F(l2_sync_batch_norm_gather_stats_test, batch_norm_stats_Attr_eps_zero)
{
    auto totalSum = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4, 5, 6});
    auto totalSquareSum = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4, 5, 6});
    auto sampleCount = TensorDesc({2}, ACL_INT32, ACL_FORMAT_ND).Value(vector<float>{1, 2});
...
TEST_F(l2_sync_batch_norm_gather_stats_test, batch_norm_stats_Attr_eps_negative)
{
    auto totalSum = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4, 5, 6});
    auto totalSquareSum = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4, 5, 6});
    auto sampleCount = TensorDesc({2}, ACL_INT32, ACL_FORMAT_ND).Value(vector<float>{1, 2});
...
// momentum 是指数移动平均权重，取值域为 [0, 1]；越界会让 running 统计量发散。
TEST_F(l2_sync_batch_norm_gather_stats_test, batch_norm_stats_Attr_momentum_out_of_range)
{
    auto totalSum = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4, 5, 6});
    auto totalSquareSum = TensorDesc({2, 3}, ACL_FLOAT, ACL_FORMAT_ND).Value(vector<float>{1, 2, 3, 4, 5, 6});
    auto sampleCount = TensorDesc({2}, ACL_INT32, ACL_FORMAT_ND).Value(vector<float>{1, 2});
