...
- 接口功能：RmsNorm算子是大模型常用的归一化操作，相比LayerNorm算子，其去掉了减去均值的部分。FusedAddRmsNorm算子将RmsNorm前的ScaledAdd算子融合起来，完成`x = x1 * scale + x2`以及后续RmsNorm计算，减少搬入搬出操作。
- 计算公式：
...
每个算子分为[两段式接口](../../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnFusedAddRmsNormGetWorkspaceSize`接口获取入参并根据计算流程所需workspace大小，再调用`aclnnFusedAddRmsNorm`接口执行计算。

```Cpp
aclnnStatus aclnnFusedAddRmsNormGetWorkspaceSize(
  const aclTensor *x1,
  const aclTensor *x2,
...
```Cpp
aclnnStatus aclnnFusedAddRmsNorm(
  void          *workspace,
  uint64_t       workspaceSize,
  aclOpExecutor *executor,
  aclrtStream    stream)
...
## aclnnFusedAddRmsNormGetWorkspaceSize
...
- **参数说明：**
  ...
  <tbody>
    <tr>
      <td>x1</td>
      <td>输入</td>
      <td>表示用于ScaledAdd计算的第一个输入。对应公式中的`x1`。</td>
      <td>不支持空Tensor。</td>
    ...
    <tr>
      <td>x2</td>
      <td>输入</td>
      <td>表示用于ScaledAdd计算的第二个输入。对应公式中的`x2`。</td>
    ...
    <tr>
      <td>scale</td>
      <td>输入</td>
      <td>表示ScaledAdd阶段中`x1`的缩放系数。对应公式中的`scale`。</td>
      <td>默认值为1.0，MiniCPM场景通常按模型配置传入。</td>
      <td>DOUBLE</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    ...
    <tr>
      <td>xOut</td>
      <td>输出</td>
      <td>表示ScaledAdd计算的结果。对应公式中的`x`。</td>
      <td><ul><li>不支持空Tensor。</li><li>shape、数据类型与输入`x1`的shape、数据类型保持一致。</li></ul></td>
      <td>FLOAT32、FLOAT16、BFLOAT16</td>
      <td>ND</td>
      <td>1-8</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnFusedAddRmsNormGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
