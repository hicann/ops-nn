...
static aclnnStatus CheckParams(const aclTensor* self, const aclTensor* target, int64_t reduction, aclTensor* result)
{
    // 1. 检查参数是否为空指针
...
static bool CheckSameShape(const aclTensor** self, const aclTensor** target)
{
    if ((*self)->GetViewShape() != (*target)->GetViewShape()) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "predict and label shapes must match, broadcast is not supported.");
        return false;
    }
    return true;
}
                                                ...
                                                const aclTensor** targetPost)
{
    ...
    auto ret = CheckSameShape(&selfContiguous, &targetContiguous);
    CHECK_RET(ret, ACLNN_ERR_PARAM_INVALID);

    auto promoteType = op::PromoteType(self->GetDataType(), target->GetDataType());
    *selfPost = InputPreProcess(selfContiguous, reduction, promoteType, executor);
    CHECK_RET(*selfPost != nullptr, ACLNN_ERR_INNER_NULLPTR);

    *targetPost = InputPreProcess(targetContiguous, reduction, promoteType, executor);
    CHECK_RET(*targetPost != nullptr, ACLNN_ERR_INNER_NULLPTR);
    return ACLNN_SUCCESS;
}
                                                ...
                                                aclOpExecutor* executor, const aclTensor** formattedResult)
{
    ...
    CHECK_RET(*formattedResult != nullptr, ACLNN_ERR_INNER_NULLPTR);
    return ACLNN_SUCCESS;
}

static aclnnStatus CastAndCopyToOutput(const aclTensor* input, aclTensor* result, aclOpExecutor* executor)
{
    auto smoothL1LossResultCast = l0op::Cast(input, result->GetDataType(), executor);
    CHECK_RET(smoothL1LossResultCast != nullptr, ACLNN_ERR_INNER_NULLPTR);

    auto viewCopyResult = l0op::ViewCopy(smoothL1LossResultCast, result, executor);
    CHECK_RET(viewCopyResult != nullptr, ACLNN_ERR_INNER_NULLPTR);
    return ACLNN_SUCCESS;
}
                                              ...
                                              aclOpExecutor** executor)
{
    ...
    const aclTensor* selfPost = nullptr;
    const aclTensor* targetPost = nullptr;
    ret = PrepareInputsForSmoothL1Loss(self, target, reduction, uniqueExecutor.get(), &selfPost, &targetPost);
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
