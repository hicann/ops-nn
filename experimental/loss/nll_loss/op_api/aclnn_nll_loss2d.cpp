...
// non-regbase path: [N,C,H,W] -> permute/reshape to [N*H*W, C], run NLLLoss, reshape loss back for reduction==none.
static aclnnStatus NLLLoss2dComputeNonRegbase(const aclTensor* self, const aclTensor* target, const aclTensor* selfCast,
                                              const aclTensor* targetContiguous, const aclTensor* weightCasted,
                                              int64_t reduction, int64_t ignoreIndex, aclOpExecutor* executor,
                                              const aclTensor** loss, const aclTensor** totalWeight)
{
    auto selfFormat = l0op::ReFormat(selfCast, static_cast<op::Format>(ACL_FORMAT_ND));
    const aclTensor* selfPermute = selfFormat;
    if (selfFormat->GetViewShape().GetDim(1) != 1) {
        const int64_t permuteList[] = {0, 2, 3, 1};
        selfPermute = l0op::Transpose(selfFormat, executor->AllocIntArray(permuteList, PERMUTE_DIM4), executor);
        CHECK_RET(selfPermute != nullptr, ACLNN_ERR_INNER_NULLPTR);
    }
    const int64_t reshapeList[] = {-1, self->GetViewShape().GetDim(1)};
    auto selfReshape = l0op::Reshape(selfPermute, executor->AllocIntArray(reshapeList, 2), executor);
    CHECK_RET(selfReshape != nullptr, ACLNN_ERR_INNER_NULLPTR);

    const int64_t tarReshapeList[] = {-1};
    auto targetReshape = l0op::Reshape(targetContiguous, executor->AllocIntArray(tarReshapeList, 1), executor);
    op::DataType targetPromoteType = target->GetDataType() == op::DataType::DT_INT64 ? op::DataType::DT_INT64 :
                                                                                       op::DataType::DT_INT32;
    auto targetCasted = l0op::Cast(targetReshape, targetPromoteType, executor);
    CHECK_RET(targetCasted != nullptr, ACLNN_ERR_INNER_NULLPTR);

    std::array<const aclTensor*, 2> lossOut = l0op::NLLLoss(selfReshape, targetCasted, weightCasted,
                                                            GetReductionStr(reduction), ignoreIndex, executor);
    ...
    *loss = l0op::Reshape(lossOut[0], executor->AllocIntArray(outSquence, dims), executor);
    CHECK_RET(*loss != nullptr, ACLNN_ERR_INNER_NULLPTR);
    return ACLNN_SUCCESS;
}

// regbase path: run NLLLoss directly on the 4D tensors.
static aclnnStatus NLLLoss2dComputeRegbase(const aclTensor* selfCast, const aclTensor* targetContiguous,
                                           const aclTensor* weightCasted, int64_t reduction, int64_t ignoreIndex,
                                           aclOpExecutor* executor, const aclTensor** loss,
                                           const aclTensor** totalWeight)
{
    auto targetCasted = l0op::Cast(targetContiguous, targetContiguous->GetDataType(), executor);
    CHECK_RET(targetCasted != nullptr, ACLNN_ERR_INNER_NULLPTR);
    std::array<const aclTensor*, 2> lossOut = l0op::NLLLoss(selfCast, targetCasted, weightCasted,
                                                            GetReductionStr(reduction), ignoreIndex, executor);
    CHECK_RET(lossOut[0] != nullptr, ACLNN_ERR_INNER_NULLPTR);
    CHECK_RET(lossOut[1] != nullptr, ACLNN_ERR_INNER_NULLPTR);
    *loss = lossOut[0];
...
static aclnnStatus NLLLoss2dCopyToOutput(const aclTensor* loss, const aclTensor* totalWeight, int64_t reduction,
                                         aclTensor* out, aclTensor* totalWeightOut, aclOpExecutor* executor)
{
    ...
    if (reduction != 0) {
        ...
        auto viewCopyResultWeight = l0op::ViewCopy(castTotalWeightOut, totalWeightOut, executor);
                                           ...
                                           aclTensor* totalWeightOut, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    op::DataType promoteType = Ops::NN::AclnnUtil::IsRegbase() ?
                                   self->GetDataType() :
    ...
    const aclTensor* totalWeight = nullptr;
    if (!Ops::NN::AclnnUtil::IsRegbase()) {
        ret = NLLLoss2dComputeNonRegbase(self, target, selfCast, targetContiguous, weightCasted, reduction, ignoreIndex,
                                         uniqueExecutor.get(), &loss, &totalWeight);
    } else {
        ret = NLLLoss2dComputeRegbase(selfCast, targetContiguous, weightCasted, reduction, ignoreIndex,
                                      uniqueExecutor.get(), &loss, &totalWeight);
    }
    CHECK_RET(ret == ACLNN_SUCCESS, ret);

    ret = NLLLoss2dCopyToOutput(loss, totalWeight, reduction, out, totalWeightOut, uniqueExecutor.get());
    CHECK_RET(ret == ACLNN_SUCCESS, ret);
