...
constexpr int64_t INPUT_IDX_TARGET = 2;
constexpr int64_t INPUT_SIZE_TWO = 2;
constexpr int64_t INPUT_SIZE_FOUR = 4;
constexpr int64_t NUMBER_TWO = 2;
constexpr int64_t NUMBER_THREE = 3;
constexpr int64_t IDX_ZERO = 0;

static inline bool IsUnknownRankShape(const gert::Shape& shape)
{
    return shape.GetDimNum() == 1 && shape.GetDim(0) == -2;
}

static inline void SetUnknownRankShape(gert::Shape& shape)
{
    shape.SetDimNum(1);
    shape.SetDim(0, -2);
}
...
static ge::graphStatus InferShapeForNLLLossGrad(gert::InferShapeContext* context)
{
    ...
    if (IsUnknownRankShape(*x_shape)) {
        ...
        SetUnknownRankShape(*out_y_shape);
...
static ge::graphStatus InferDtypeForNLLLossGrad(gert::InferDataTypeContext* context)
{
...
IMPL_OP_INFERSHAPE(NllLossGrad).InferShape(InferShapeForNLLLossGrad).InferDataType(InferDtypeForNLLLossGrad);
