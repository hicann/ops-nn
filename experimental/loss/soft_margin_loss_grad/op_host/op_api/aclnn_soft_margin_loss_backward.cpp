...
static inline bool CheckDtypeValid(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* target,
                                   const aclTensor* out)
{
    const std::initializer_list<op::DataType> currentDtypeSupportList = GetDtypeSupportListV1(
        ASCEND910B_DTYPE_SUPPORT_LIST, ASCEND910_DTYPE_SUPPORT_LIST);
                                                        ...
                                                        uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    // 进行计算
    auto grad = l0op::SoftMarginLossGrad(gradOutputBroadcast, selfBroadcast, targetBroadcast, REDUCTION_STR[reduction],
                                         uniqueExecutor.get());
