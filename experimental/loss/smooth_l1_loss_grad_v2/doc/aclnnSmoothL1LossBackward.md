...
- 计算公式：
  ...
  当 |diff| < beta 时，梯度为 diff / beta；

  当 |diff| >= beta 时，梯度为 sign(diff)。

  即：

  $$
  \frac{\partial \text{SmoothL1Loss}(x, y)}{\partial x} =
  \begin{cases}
  \dfrac{x - y}{\beta}, & \text{if } |x - y| < \beta \\
  \operatorname{sign}(x - y), & \text{otherwise}
  \end{cases}
  $$
