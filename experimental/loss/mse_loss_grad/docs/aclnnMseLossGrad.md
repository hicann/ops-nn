...
[📄 查看源码](https://gitcode.com/cann/ops-nn/tree/master/experimental/loss/mse_loss_grad)
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td class="tg-0pky">reduction（char *）</td>
      <td class="tg-0pky">可选属性</td>
      <td class="tg-0pky">指定要应用到输出的缩减，公式中的参数`reduction`。</td>
      <td class="tg-0pky">支持'none'|'mean'|'sum'，默认'mean'。<br>-'none'表示不应用缩减<br>-'mean'表示输出的总和将除以输出中的元素数<br>-'sum'表示输出将被求和</td>
  ...
  <col style="width: 833px">
      ...
      <tbody>
        ...
        <tr>
          <td>workspaceSize</td>
          <td>输入</td>
          <td>在Device侧申请的workspace大小，由第一段接口aclnnMseLossGradGetWorkspaceSize获取。</td>
        </tr>
        <tr>
          <td>executor</td>
          <td>输入</td>
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnMseLossGrad第一段接口
  ret = aclnnMseLossGradGetWorkspaceSize(self, target, gradOutput, (char*)reduction.c_str(), out, &workspaceSize, &executor);
