        ...
        if op_type:
            shell_exec(
                [
                    "bash",
                    "-c",
                    f"""sed -i 's/{op_type}_op_resource.h/op_resource_nn.h/g' {aclnn_cpp}""",
                ],
                shell=False,
            )
            shell_exec(
                [
                    "bash",
                    "-c",
                    f"""sed -i 's/{op_type}_RESOURCES/AUTO_GEN_OP_RESOURCE({op_type})/g' {aclnn_cpp}""",
                ],
                shell=False,
            )
            shell_exec(
                [
                    "bash",
                    "-c",
                    f"sed -i '{index}i\\EXTERN_OP_RESOURCE({op_type})' {aclnn_cpp}",
                ],
                shell=False,
            )
    return
