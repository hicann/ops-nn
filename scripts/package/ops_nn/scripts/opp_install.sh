...
install_cann_ops_nn_whl() {
  ...
  local whl_file=$(find "${whl_dir}" -name "cann_ops_nn-*.whl" 2>/dev/null | head -1)

  if [ -n "${whl_file}" ] && [ -f "${whl_file}" ]; then
    logandprint "[INFO]: Found whl package: ${whl_file}"
    logandprint "[INFO]: Installing cann_ops_nn whl package to ${target_python_dir}"
    ...
    logandprint "[INFO]: cann_ops_nn whl package installed successfully"
  else
    logandprint "[INFO]: No cann_ops_nn whl package found, skipping"
  fi
}
