...
static inline bool IsNullptr(const aclIntArray* intArr, const char* name)
{
    if (intArr == nullptr) {
        OP_LOGE(ACLNN_ERR_PARAM_NULLPTR,
                "Expected a value of type List[int] for argument %s but instead found type null.", name);
        return true;
    }
    return false;
}
...
static inline bool IsNullptr(const aclBoolArray* boolArr, const char* name)
{
    if (boolArr == nullptr) {
        OP_LOGE(ACLNN_ERR_PARAM_NULLPTR,
                "Expected a value of type List[bool] for argument %s but instead found type null.", name);
        return true;
    }
    return false;
}
...
#define OP_CHECK_DTYPE_NOT_MATCH(tensor, expectedDtype, retExpr)                                           \
    if (tensor->GetDataType() != expectedDtype) {                                                          \
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Tensor %s expected dtype is %s but found %s.", #tensor,          \
...
#define OP_CHECK_BROADCAST(tensor1, tensor2, retExpr)                                                               \
    if (!CheckBroadcastShape(tensor1->GetViewShape(), tensor2->GetViewShape())) {                                   \
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The size of tensor %s %s must match the size of tensor %s %s.", #tensor1, \
...
#define OP_CHECK_BROADCAST_WITH_SHAPE(tensor, shape, retExpr)                                         \
    if (!CheckBroadcastShape(tensor->GetViewShape(), shape)) {                                        \
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The size of tensor %s %s must match the size %s.", #tensor, \
...
#define OP_CHECK_SHAPE_NOT_EQUAL_WITH_EXPECTED_SIZE(tensor, shape, retExpr)                                      \
    if (tensor->GetViewShape() != shape) {                                                                       \
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Expected tensor for %s to have same size as %s, but got %s.", #tensor, \
...
#define OP_CHECK_MAX_DIM(tensor, maxDim, retExpr)                                                        \
    if (tensor->GetViewShape().GetDimNum() > static_cast<size_t>(maxDim)) {                              \
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The %s tensor cannot be larger than %zu dimensions.", #tensor, \
...
#define OP_CHECK_MIN_DIM(tensor, minDim, retExpr)                                                     \
    if (tensor->GetViewShape().GetDimNum() < static_cast<size_t>(minDim)) {                           \
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "The %s tensor must have at least %zu dimensions.", #tensor, \
