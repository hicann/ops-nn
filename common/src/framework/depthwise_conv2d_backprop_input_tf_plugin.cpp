...
/*!
 * \file depthwise_conv2d_backprop_input_tf_plugin.cpp
 * \brief
 */
#include <vector>
...
namespace domi {
namespace {
const int32_t kInputFilter = 1;
}

/*!
 * @brief Replace GE ParseParams fuction to process graph depthwise_conv2d_backprop_input node attrs
 * @param opSrc the source op info from tf.
 * @param op the dest GE op.
 * @return status whether this operation success.
 */
static Status ParseParamsDepthwiseConv2DBackpropInput(const ge::Operator& opSrc, ge::Operator& op)
{
    ...
    OP_LOGD(GetOpName(op).c_str(), "update filter format success.");
...
REGISTER_CUSTOM_OP("DepthwiseConv2DBackpropInput")
    ...
    .ParseParamsByOperatorFn(ParseParamsDepthwiseConv2DBackpropInput)
