...
- **Parameters**
  ...
  - **self** (aclTensor\*, compute input): input `self` in the formula, which is an aclTensor on the device. The shape is (N, C) or (C). N indicates the batch size, and C indicates the number of classes. The data types of **self**, **out**, and **isTarget** are the same. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
    - <term>Atlas training series products</term>: The data type can be FLOAT16 or FLOAT. If the number of elements in `self` is greater than 15000\*20000, a runtime timeout error (507034 Vector Core execution timed out) may occur.
    - <term>Atlas A2 training series products/Atlas A2 inference series products</term> and <term>Atlas A3 training series products/Atlas A3 inference series products</term>: The data type can be BFLOAT16, FLOAT16, or FLOAT. If the number of elements in `self` is greater than 15000\*20000, a runtime timeout error (507034 Vector Core execution timed out) may occur.
  - **target** (aclTensor*, compute input): `target` in the formula, aclTensor on the device, indicating the real label, with shape (N, C) or (C). The range of each element is [-1, C - 1], and -1 is padded. The label before the first -1 indicates the real label of the sample. The data type can be INT64 or INT32. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
  - **reduction** (int64_t, compute input): `reduction` in the formula, an integer on the host, specifying the reduction to be applied to the output. The supported values are 0 (none) | 1 (mean) | 2 (sum). **none** indicates that no reduction is applied; **mean** indicates that the sum of the output will be divided by the number of samples in the output; **sum** indicates that the output will be summed.
  - **out** (aclTensor\*, compute output): `out` in the formula, aclTensor on the device. The data types of **self**, **out**, and **isTarget** are the same, and the shape is (N) or (). The [data format](../../../docs/en/context/data_formats.md) can be ND.
  ...
  - **isTarget** (aclTensor\*, compute output): `istarget` in the formula, aclTensor on the device. The data types of **self**, **out**, and **isTarget** are the same, and the shape is (N, C) or (C). [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
    - <term>Atlas training series products</term>: The data type can be FLOAT16 or FLOAT.
    - <term>Atlas A2 training series products/Atlas A2 inference series products</term> and <term>Atlas A3 training series products/Atlas A3 inference series products</term>: The data type can be BFLOAT16, FLOAT16, or FLOAT.
...
- **Returns:**
  ...
  **aclnnStatus**: status code. For details, see [aclnn Return Codes](../../../docs/en/context/aclnn_return_codes_nn.md).
  ...
  161001 (ACLNN_ERR_PARAM_NULLPTR): 1. The passed self, target, out, or isTarget is a null pointer.
  161002 (ACLNN_ERR_PARAM_INVALID): 1. The data type of self, out, or isTarget is not supported.
                                        2. The data types of self, out, and isTarget are inconsistent.
                                        3. The data type of target is not supported.
                                        4. The shape of self, target, or isTarget does not meet the requirements in the parameter description.
                                        5. The shape of out does not meet the requirements in the parameter description.
  ```
