...
static bool CheckPromoteType(const aclTensor* gradOutput, const aclTensor* self, const aclTensor* target,
                             const aclTensor* weightOptional, const aclTensor* out, op::DataType& promoteType)
{
...
static bool CheckFormat(const aclTensor* self, const aclTensor* target, const aclTensor* weightOptional)
{
...
aclnnStatus aclnnBinaryCrossEntropyBackwardGetWorkspaceSize(const aclTensor* gradOutput, const aclTensor* self,
                                                            const aclTensor* target, const aclTensor* weightOptional,
                                                            int64_t reduction, aclTensor* out, uint64_t* workspaceSize,
                                                            aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnBinaryCrossEntropyBackward, DFX_IN(gradOutput, self, target, weightOptional, reduction),
                   DFX_OUT(out));
    ...
    auto bceGrad = l0op::BinaryCrossEntropyGrad(gradOutputCasted, selfCasted, targetCasted, weightCasted,
                                                reductionStr[reduction], uniqueExecutor.get());
...
aclnnStatus aclnnBinaryCrossEntropyBackward(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
