...
static ge::graphStatus CELossGradCoreSplitInfo(gert::TilingContext* context, uint64_t dtypeSize, uint64_t reductionKey)
{
    ...
    if (dtypeSize == FP32_INT32_DTYPE_SIZE) {
        if (reductionKey == TILING_KEY_NONE) {
            ...
            colLoopNum = GetDiv(
                (maxUbSize - targetSize - targetCastSize - gradLossSize - ignoreSize - maskSize - targetWeightSize),
                FP32_SPLIT);
            ...
            colLoopNum = GetDiv((maxUbSize - targetSize - targetCastSize - gradLossSize - gradLossFp32Size -
                                 ignoreSize - maskSize - targetWeightSize - tBuf2Size - tBuf3Size),
                                FP32_SPLIT);
        } else if (reductionKey == TILING_KEY_SUM) {
            colLoopNum = GetDiv((maxUbSize - targetSize - targetCastSize - ignoreSize - maskSize - targetWeightSize),
                                FP32_SPLIT);
        }
        ...
        if (reductionKey == TILING_KEY_NONE) {
            ...
            colLoopNum = GetDiv((maxUbSize - targetSize - targetCastSize - gradLossSize - gradLossFp32Size -
                                 ignoreSize - maskSize - targetWeightSize),
                                FP16_BF16_SPLIT);
        } else if (reductionKey == TILING_KEY_MEAN) {
            tBuf2Size = BLOCK_SIZE;
            tBuf3Size = GetCeilAlign(max(coreNum, frontRowNum) * FP32_INT32_DTYPE_SIZE, BLOCK_SIZE);
...
static ge::graphStatus GetTilingInput(const gert::TilingContext* context, uint64_t& weightKey)
{
    ...
    OP_CHECK_IF(
        (targetShape->GetStorageShape().GetDim(0) != logProbShape->GetStorageShape().GetDim(0)),
        OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context->GetNodeName(), "log_prob and target",
                                               (std::to_string(logProbShape->GetStorageShape().GetDim(0)) + " and " +
                                                std::to_string(targetShape->GetStorageShape().GetDim(0)))
                                                   .c_str(),
                                               "The dim 0 of log_prob must be equal to the shape size of target"),
    ...
    if (weightTensor != nullptr) {
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context->GetNodeName(), "log_prob and weight",
                                                   (std::to_string(logProbShape->GetStorageShape().GetDim(1)) +
                                                    " and " + std::to_string(weightShape.GetDim(0)))
                                                       .c_str(),
                                                   "The dim 1 of log_prob must be equal to the shape size of weight"),
            return ge::GRAPH_FAILED);
    }
...
static ge::graphStatus Tiling4CrossEntropyLossGrad(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(GetTilingInput(context, weightKey) != ge::GRAPH_SUCCESS, OP_LOGE(nodeName, "GetTilingInput failed."),
                return ge::GRAPH_FAILED);

    uint64_t reductionKey = 0;
    OP_CHECK_IF(GetTilingAttr(context, reductionKey) != ge::GRAPH_SUCCESS, OP_LOGE(nodeName, "GetTilingAttr failed."),
                return ge::GRAPH_FAILED);
