...
- Description: Computes the binary logic loss function of inputs **self** and **target**. **reduction** specifies the reduction to be applied to the output. The value can be **none**, **mean**, or **sum**. **none** indicates that no reduction will be applied; **mean** indicates that the sum of the output will be divided by the number of elements in the output; **sum** indicates that the output will be summed.
...
- **Parameters:**
  ...
  * **target** (aclTensor*, compute input): `target` in the formula, aclTensor on the device. The shape must meet the [broadcast relationship](../../../docs/en/context/broadcast_relationship.md) with **self**. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
    - <term>Atlas training series products</term>: The data type can be FLOAT16 or FLOAT.
    - <term>Atlas A2 training series products/Atlas A2 inference series products</term> and <term>Atlas A3 training series products/Atlas A3 inference series products</term>: The data type can be FLOAT16, FLOAT, or BFLOAT16.
  * **reduction** (int64_t, compute input): `reduction` in the formula, integer on the host. The data type can be INT64. It specifies the reduction to be applied to the output. The value can be 0 ('none') | 1 ('mean') | 2 ('sum'). **none** indicates that no reduction will be applied; **mean** indicates that the sum of the output will be divided by the number of elements in the output; **sum** indicates that the output will be summed.
  * **out** (aclTensor*, compute output): `out` in the formula, aclTensor on the device. If **reduction** is **none**, the shape of **out** must be the same as that of **self** and **target** after broadcasting. Otherwise, **out** is a scalar. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
    - <term>Atlas training series products</term>: The data type can be FLOAT16 or FLOAT.
    - <term>Atlas A2 training series products/Atlas A2 inference series products</term> and <term>Atlas A3 training series products/Atlas A3 inference series products</term>: The data type can be FLOAT16, FLOAT, or BFLOAT16.
...
161002 (ACLNN_ERR_PARAM_INVALID): 1. The data type of self, target, or out is not supported.
                                     2. The shapes of self and target do not meet the broadcast rules, or the shape after broadcasting is different from that of out.
```
...
- **Parameters:**
  ...
  * **workspaceSize** (uint64_t, input): size of the workspace to be allocated on the device, which is obtained by the first-phase API **aclnnSoftMarginLossGetWorkspaceSize**.
