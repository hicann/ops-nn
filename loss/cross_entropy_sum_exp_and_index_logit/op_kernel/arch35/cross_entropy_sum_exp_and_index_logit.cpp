 ...
 * \brief A5 (ascend950) kernel entry — TILING_KEY_IS runtime dispatch（单 key 100 = CE_REGBASE）
...
#define CE_REGBASE 100

extern "C" __global__ __aicore__ void cross_entropy_sum_exp_and_index_logit(GM_ADDR vocab_parallel_logits,
                                                                            GM_ADDR target, GM_ADDR global_logits_max,
                                                                            GM_ADDR predicted_logits,
                                                                            ...
                                                                            GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    // 单 TilingKey 100（CE_REGBASE），dtype 差异由 DTYPE 宏编译期实例化
    if (TILING_KEY_IS(CE_REGBASE)) {
