...
/**
 * @file test_geir_smooth_l1_loss.cpp
 * @brief GE IR pathway verification for SmoothL1Loss operator
 */
...
{
    Status ret = SUCCESS;
    auto op1 = op::SmoothL1Loss("op1");
    op1.SetAttr("sigma", 1.0f);

    std::vector<int64_t> predictShape = {8, 16};
    ADD_INPUT(1, predict, inDtype, predictShape);
    ADD_INPUT(2, label, inDtype, predictShape);

    std::vector<int64_t> outShape = {8, 16};
    ADD_OUTPUT(1, loss, inDtype, outShape);
...
{
    const char* graph_name = "e2e_verify_geir_smooth_l1_loss";
