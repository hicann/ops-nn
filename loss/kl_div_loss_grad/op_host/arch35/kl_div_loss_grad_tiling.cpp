...
ge::graphStatus KlDivLossGradTiling::CalcInputDtype()
{
    ...
    OP_CHECK_IF(this->inputInputDtype != ge::DT_FLOAT16 && this->inputInputDtype != ge::DT_BF16 &&
                    this->inputInputDtype != ge::DT_FLOAT,
    ...
    OP_CHECK_IF(this->inputTargetDtype != ge::DT_FLOAT16 && this->inputTargetDtype != ge::DT_BF16 &&
...
ge::graphStatus KlDivLossGradTiling::CalcDiffDtype()
{
    ...
    if (this->inputGradDtype != this->inputInputDtype) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "grad and input", dtypeMsg.c_str(),
                                               "The dtypes of parameter grad and parameter input must be the same");
        return ge::GRAPH_FAILED;
    }
    ...
    if (this->inputGradDtype != this->inputTargetDtype) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "grad and target", dtypeMsg.c_str(),
                                               "The dtypes of parameter grad and parameter target must be the same");
        return ge::GRAPH_FAILED;
    }

    if (this->inputGradDtype != this->outputYDtype) {
        std::string dtypeMsg = ToString(this->inputGradDtype) + " and " + ToString(this->outputYDtype);
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "grad and y", dtypeMsg.c_str(),
                                               "The dtypes of parameter grad and parameter y must be the same");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus KlDivLossGradTiling::CalcOutputDtype()
{
    ...
    if (this->outputYDtype != this->inputGradDtype) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "y and grad", dtypeMsg.c_str(),
                                               "The dtypes of parameter y and parameter grad must be the same");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
float KlDivLossGradTiling::CalcReductionCof(const gert::Shape& inputLabelShape)
{
    ...
    OP_CHECK_IF(attrs == nullptr, OP_LOGE(context_->GetNodeName(), "attrs is nullptr"), return negReductionCof);
    ...
    if (strcmp(this->reducationStr, "mean") == 0) {
        ...
        for (uint32_t i = 0; i < dimLen; i++) {
            if (inputLabelShape.GetDim(i) != 0) {
                ...
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                    context_->GetNodeName(), "input", ToString(inputLabelShape).c_str(),
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(
                context_->GetNodeName(), "input", std::to_string(dimVal).c_str(),
        ...
        if (inputLabelShape.GetDim(0) > 0) {
            ...
            std::string reasonMsg = "The N-dimension of parameter input must be greater than 0 "
                                    "when the attribute reduction is batchmean, where N is the 0th dim";
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "input", ToString(inputLabelShape).c_str(),
                                                  reasonMsg.c_str());
            return negReductionCof;
        }
    }
...
ge::graphStatus KlDivLossGradTiling::DoOpTiling()
{
    ...
    if (input0DType == ge::DT_FLOAT16 || input0DType == ge::DT_BF16) {
        ...
        OP_CHECK_IF(RunFp32BroadcastTiling(this->reducationCof_) == ge::GRAPH_FAILED,
                    OP_LOGE(context_->GetNodeName(), "RunFp32BroadcastTiling failed. Please check the detailed log."),
                    return ge::GRAPH_FAILED);
    } else {
        OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "grad", ToString(input0DType).c_str(),
                                  "FLOAT, FLOAT16 or BF16");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
...
ge::graphStatus KlDivLossGradTiling::GetShapeAttrsInfo() { return ge::GRAPH_SUCCESS; }

bool KlDivLossGradTiling::IsCapable() { return true; }

ge::graphStatus KlDivLossGradTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t KlDivLossGradTiling::GetTilingKey() const { return tilingKey_; }

ge::graphStatus KlDivLossGradTiling::GetWorkspaceSize() { return ge::GRAPH_SUCCESS; }

ge::graphStatus KlDivLossGradTiling::PostTiling() { return ge::GRAPH_SUCCESS; }

ge::graphStatus KlDivLossGradTiling::GetPlatformInfo() { return ge::GRAPH_SUCCESS; }
...
REGISTER_OPS_TILING_TEMPLATE(KlDivLossGrad, KlDivLossGradTiling, KL_DIV_LOSS_GRAD_COMMON_TILING_PRIORITY);
} // namespace optiling
