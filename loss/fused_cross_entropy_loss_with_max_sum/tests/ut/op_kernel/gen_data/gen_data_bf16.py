...
def write_file(shape, input):
    if "vocab_parallel_logits" in input:
        vocab_parallel_logits = np.random.randint(1, 255, shape).astype(
            ml_dtypes.bfloat16
        )
...
def gen_tiling():
    ...
    tiling = (
        ...
        for i in (
            ...
            latterbtCountTime,
            formerCoreNum,
            formerCoreReservedbtNum,
            latterCoreReservedbtNum,
            singleCalculationQuantity,
    ...
    with os.fdopen(
        os.open("./tiling.bin", WRITE_FILE_FLAGS, OPEN_FILE_MODES_640), "wb"
    ) as f:
        f.write(tiling_data)
