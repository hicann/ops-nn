...
/*!
 * \file test_geir_l2_loss.cpp
 * \brief L2Loss 算子 GEIR 入图调用示例
 */
...
#include "../op_graph/l2_loss_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    l2Loss1.set_input_##intputName(placeholder##intputIndex);                                                       \
...
#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    ...
    l2Loss1.update_output_desc_##outputName(outputName##outputIndex##_desc_)
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto l2Loss1 = op::L2Loss("l2Loss1");
    std::vector<int64_t> xShape = {2, 30};
    ADD_INPUT(1, x, inDtype, xShape);
    // L2Loss 输出 y 为标量
    ADD_OUTPUT(1, y, inDtype, std::vector<int64_t>{});

    outputs.push_back(l2Loss1);
    // 添加完毕
    return SUCCESS;
}
