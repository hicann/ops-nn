...
/*!
 * \file test_geir_softmax_focal_loss_grad.cpp
 * \brief GE graph construction sample for SoftmaxFocalLossGrad (arch35 ND format).
 */
#include <iostream>
#include <string.h>
#include <stdint.h>
...
#include "../op_graph/softmax_focal_loss_grad_proto.h"
...
// pred 落在 (0,1) 开区间: 取 0.5; target 为 one-hot, 这里整行置 1 只为构图打通, 不校验数值
int32_t GenData(vector<int64_t> shapes, Tensor& tensor, TensorDesc& desc, DataType dt)
{
    ...
    if (dt == DT_FLOAT) {
        vector<float> v(size, 0.5f);
        tensor.SetData((uint8_t*)v.data(), size * sizeof(float));
    } else if (dt == DT_INT32) {
        vector<int32_t> v(size, 1);
        tensor.SetData((uint8_t*)v.data(), size * sizeof(int32_t));
    } else {
        vector<uint16_t> v(size, 0x3800); // fp16 ~0.5
...
#define ADD_INPUT(idx, opInputName, inDtype, inShape)                                       \
    ...
    ret = GenData(ph##idx##_shape, tensor_ph##idx, ph##idx##_desc, inDtype);                \
...
{
    Status ret = SUCCESS;
    auto op_node = op::SoftmaxFocalLossGrad("softmax_focal_loss_grad_op");
    // ND 2D: [N, C], 归约轴为最后一维
    std::vector<int64_t> xShape = {8, 64};
    ADD_INPUT(1, pred, dt, xShape);
    ADD_INPUT(2, target, DT_INT32, xShape);
    ADD_INPUT(3, dout, dt, xShape);
    ADD_INPUT(4, weight, dt, xShape);
    op_node.set_attr_gamma(2.0f);
    op_node.set_attr_alpha(0.25f);
    op_node.set_attr_reduction("mean");
    ADD_OUTPUT(grad, dt, xShape);
...
int main(int argc, char* argv[])
{
    ...
    std::map<AscendString, AscendString> gopt = {
