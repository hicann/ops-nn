...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnNLLLossGetWorkspaceSize** is called to obtain the workspace size required for computation and the executor that contains the operator computation process. Then, **aclnnNLLLoss** is called to perform computation.
...
- **Parameters:**
  - **self** (aclTensor*, compute input): input tensor, `self` in the formula. The shape is (N, C) or (C). It is an aclTensor on the device. N indicates the batch size, and C indicates the number of classes. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
  ...
  - **target** (aclTensor*, compute input): real label, input `target` in the formula, aclTensor on the device. When the shape of **self** is (N, C), the shape of **target** is (N). When the shape of **self** is (C), the shape of **target** is (), and the value range of each element is [0, C – 1]. The data type can be INT64, UINT8, or INT32. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.

  - **weight** (aclTensor*, compute input): scaling weight of each class, input `weight` in the formula, aclTensor on the device. The shape is (C). [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND. The data type is the same as that of **self**.

  - **reduction** (int64_t*, compute input): reduction to be applied to the output, `reduction` in the formula. The value can be 0 ('none') | 1 ('mean') | 2 ('sum'). **none** indicates that no reduction is applied; **mean** indicates that the sum of the output will be divided by the number of elements in the output; **sum** indicates that the output will be summed.

  - **ignoreIndex** (int64_t*, compute input): target value that is ignored and does not affect the input gradient, `ignoreIndex` in the formula.

  - **out** (aclTensor*, compute output): `out` in the formula. When **reduction** is **0** (**none**) and the shape of **self** is two-dimensional, the shape of **out** is (N,). Otherwise, the shape is (1,). The [data format](../../../docs/en/context/data_formats.md) can be ND. The data type is the same as that of **self**.

  - **totalWeightOut** (aclTensor*, compute output): `totalWeightOut` in the formula. The output value is valid when **reduction** is not **0** (**none**). The shape is (1,). The [data format](../../../docs/en/context/data_formats.md) can be ND. The data type is the same as that of **self**.
...
161002 (ACLNN_ERR_PARAM_INVALID): 1. The data type of self, target, or weight is not supported.
                                  2. The data types of self and weight are inconsistent.
                                  3. The shape of self, weight, out, or totalWeightOut is incorrect.
...
int main() {
  ...
  // Call the first-phase API of aclnnNLLLoss.
  ...
  // Call the second-phase API of aclnnNLLLoss.
