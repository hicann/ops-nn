                    ...
                    int value)
{
    ...
    if (data_type == ge::DT_FLOAT) {
        // float 输入按浮点值填充，避免整型位模式被解释为非规格化浮点数
        vector<float> data(size, static_cast<float>(value));
        input_tensor = Tensor(input_tensor_desc, reinterpret_cast<const uint8_t*>(data.data()), data_len);
        return SUCCESS;
    }
    vector<int32_t> data(size, value);
    input_tensor = Tensor(input_tensor_desc, reinterpret_cast<const uint8_t*>(data.data()), data_len);
    return SUCCESS;
}
                     ...
                     std::vector<Operator>& inputs, std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::NPUGetFloatStatus("add1");
    ADD_INPUT_MODE(1, addr, inDtype, xShape, mode);

    // 输出 shape 固定为 [8]，不依赖输入 shape
    std::vector<int64_t> outputShape = {8};
    ADD_OUTPUT_MODE(1, data, inDtype, outputShape, mode);
...
int main(int argc, char* argv[])
{
    ...
    std::map<AscendString, AscendString> global_options = {
        {"ge.exec.deviceId", "0"}, // device id 0 通过 npu-smi info 查询，Health 为 OK
        {"ge.graphRunMode", "0"},
    ...
    // dtype 矩阵（取自 reg_op.dtype_set，该算子仅支持 float32）
    struct DtypeEntry {
        DataType dt;
        std::string name;
    };
    ...
    // shape 场景矩阵（该算子输入输出 shape 固定为 [8]，仅测试常规场景）
    struct ShapeEntry {
    ...
    // N_dtype × N_shape × mode 矩阵
    // 同时测试 S（静态）和 D（动态）模式，验证两种模式下 AIcoreEngine
    // 都能优先于 rts_engine 处理自定义 kernel，输出正常。
