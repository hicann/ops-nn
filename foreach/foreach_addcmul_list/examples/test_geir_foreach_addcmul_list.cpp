...
// test_geir for ForeachAddcmulList (GEIR verify; 3 dynamic input lists + scalars + dynamic output).
#include <iostream>
#include <vector>
#include <string>
#include <map>
...
static Operator MkF32(const string& name, int idx, vector<int64_t> shape, vector<Tensor>& input)
{
    ...
    for (size_t i = 0; i < n; ++i)
        p[i] = 1.0f;
    Tensor t(desc, (uint8_t*)p, n * sizeof(float));
    input.push_back(t);
    delete[] p;
...
{
    auto op1 = op::ForeachAddcmulList("foreachAddcmulList1");
    const int N = 1;
    vector<int64_t> tShape = {256};
    int idx = 0;
    op1.create_dynamic_input_x1(N);
    for (int i = 0; i < N; ++i) {
        auto d = MkF32("x1_" + std::to_string(i), idx++, tShape, input);
        graph.AddOp(d);
        inputs.push_back(d);
    ...
    for (int i = 0; i < N; ++i) {
        auto d = MkF32("x2_" + std::to_string(i), idx++, tShape, input);
        graph.AddOp(d);
        inputs.push_back(d);
        op1.set_dynamic_input_x2(i, d);
    }
    op1.create_dynamic_input_x3(N);
    for (int i = 0; i < N; ++i) {
        auto d = MkF32("x3_" + std::to_string(i), idx++, tShape, input);
        graph.AddOp(d);
        inputs.push_back(d);
        op1.set_dynamic_input_x3(i, d);
    }
    auto dS = MkF32("scalars", idx++, {N}, input);
    graph.AddOp(dS);
    inputs.push_back(dS);
    op1.set_input_scalars(dS);
    op1.create_dynamic_output_y(N);
    for (int i = 0; i < N; ++i) {
        TensorDesc yd(ge::Shape(tShape), FORMAT_ND, DT_FLOAT);
        op1.update_dynamic_output_desc_y(i, yd);
    }
