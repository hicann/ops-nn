...
/*!
 * \file foreach_minimum_scalar_list_tiling_arch35.cpp
 * \brief arch35 (Ascend950) tiling implementation for foreach_minimum_scalar_list
 */
...
#include "../../op_kernel/arch35/foreach_minimum_scalar_list_tiling_data.h"
#include "../../op_kernel/arch35/foreach_minimum_scalar_list_tiling_key.h"
...
struct ForeachMinimumScalarListCompileInfo {};
...
static ge::graphStatus ForeachMinimumScalarListTilingFunc(gert::TilingContext* context)
{
    uint64_t ubSize;
    int64_t coreNum;
    ...
    ForeachMinimumScalarListTilingData* tiling = context->GetTilingData<ForeachMinimumScalarListTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ForeachMinimumScalarListTilingData), 0,
                         sizeof(ForeachMinimumScalarListTilingData)) != EOK,
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_SCALAR_LIST_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_SCALAR_LIST_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_INT32) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_SCALAR_LIST_TPL_SCH_MODE_INT32);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_SCALAR_LIST_TPL_SCH_MODE_BF16);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_minimum_scalar_list");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForForeachMinimumScalarList([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachMinimumScalarList)
    .Tiling(ForeachMinimumScalarListTilingFunc)
    .TilingParse<ForeachMinimumScalarListCompileInfo>(TilingParseForForeachMinimumScalarList);
