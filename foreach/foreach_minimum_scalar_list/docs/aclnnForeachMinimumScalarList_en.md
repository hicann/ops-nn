...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_minimum_scalar_list)
...
- Description: Computes the element-wise minimum of each tensor in tensor list **x** and the corresponding scalar in scalar list **scalars**.
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnForeachMinimumScalarListGetWorkspaceSize** is called to obtain the input parameters and compute the required workspace size based on the process. Then, **aclnnForeachMinimumScalarList** is called to perform computation.
...
- **Parameters**
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      <td>scalars</td>
      <td>Input</td>
      <td>Input scalar list for the minimum operation, corresponding to `scalars` in the formula.</td>
      <td><ul><li>The number of elements is equal to the number of tensors in `x`. </li><li>The data type depends on that of `x`:<ul><li>When `x` is of the FLOAT32, FLOAT16, or BFLOAT16 type, `scalars` must be FLOAT32 or DOUBLE. </li><li>When `x` is of the INT32 type, `scalars` must be INT64.</li></ul></li></ul></td>
...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnForeachMinimumScalarListGetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- Deterministic computation:
  - **aclnnForeachMinimumScalarList** defaults to a deterministic implementation.
...
int main() {
  ...
  aclOpExecutor* executor;
  // Call the first-phase API of aclnnForeachMinimumScalarList.
  ...
  // Call the second-phase API of aclnnForeachMinimumScalarList.
