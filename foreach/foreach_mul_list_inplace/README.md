...
<table style="undefined;table-layout: fixed; width: 1005px"><colgroup>
  ...
  <tbody>
    <tr>
      <td>x1</td>
      <td>输入/输出</td>
      <td>支持空Tensor。表示乘法运算的第一个输入张量列表，对应公式中的`x1`，同时作为输出，计算结果原地写回该参数。该参数中所有Tensor的数据类型保持一致，每个Tensor的维度数不超过8维。</td>
    ...
    <tr>
      <td>x2</td>
      <td>输入</td>
      <td>支持空Tensor。表示乘法运算的第二个输入张量列表，对应公式中的`x2`。数据类型、数据格式和shape与入参`x1`的数据类型、数据格式和shape一致。该参数中所有Tensor的数据类型保持一致。</td>
      <td>FLOAT32、FLOAT16、INT32、BFLOAT16</td>
      <td>ND</td>
    </tr>
  </tbody></table>
