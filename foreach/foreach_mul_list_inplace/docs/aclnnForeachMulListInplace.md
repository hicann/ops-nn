...
- 接口功能：原地更新第一个输入张量列表，对两个输入张量列表执行逐元素相乘，并将结果原地写回第一个列表。
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachMulListInplaceGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachMulListInplace”接口执行计算。
...
## aclnnForeachMulListInplaceGetWorkspaceSize
...
- **参数说明**
  ...
  <tbody>
    <tr>
      ...
      <td>输入/输出</td>
      <td>表示乘法运算的第一个输入和输出张量列表，对应公式中的`x1`。</td>
    ...
    <tr>
      ...
      <td><ul><li>支持空Tensor。</li><li>该参数中所有Tensor的数据类型保持一致。</li><li>数据类型、数据格式和shape与入参`x1Ref`的数据类型、数据格式和shape一致。</li><li>该参数中Tensor的数量与入参`x1Ref`一致。</li></ul></td>
      <td>FLOAT32、FLOAT16、INT32、BFLOAT16</td>
      <td>ND</td>
      <td>0-8</td>
      <td>√</td>
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachMulListInplaceGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachMulListInplace默认确定性实现。
...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_foreach_mul_list_inplace.h"
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachMulListInplace第一段接口
  ret = aclnnForeachMulListInplaceGetWorkspaceSize(tensorListInput1, tensorListInput2, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachMulListInplaceGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachMulListInplace第二段接口
  ret = aclnnForeachMulListInplace(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachMulListInplace failed. ERROR: %d\n", ret); return ret);
