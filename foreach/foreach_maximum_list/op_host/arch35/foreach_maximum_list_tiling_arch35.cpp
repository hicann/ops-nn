...
/*!
 * \file foreach_maximum_list_tiling_arch35.cpp
 * \brief arch35 (Ascend950) tiling implementation for foreach_maximum_list
 */
...
#include "../../op_kernel/arch35/foreach_maximum_list_tiling_data.h"
#include "../../op_kernel/arch35/foreach_maximum_list_tiling_key.h"
...
static ge::graphStatus ForeachMaximumListTilingFunc(gert::TilingContext* context)
{
    uint64_t ubSize;
    int64_t coreNum;
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_LIST_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_LIST_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_LIST_TPL_SCH_MODE_BF16);
    } else if (dataType == ge::DT_INT32) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_LIST_TPL_SCH_MODE_INT32);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_maximum_list");
        return ge::GRAPH_FAILED;
    }
...
IMPL_OP_OPTILING(ForeachMaximumList)
    .Tiling(ForeachMaximumListTilingFunc)
    .TilingParse<ForeachMaximumListCompileInfo>(TilingParseForForeachMaximumList);
