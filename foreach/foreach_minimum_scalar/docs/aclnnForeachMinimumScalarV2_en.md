...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_minimum_scalar)
...
- Description: Computes the element-wise minimum of each tensor in tensor list **x** with the given scalar **scalar**. Compared with [aclnnForeachMinimumScalar](./aclnnForeachMinimumScalar_en.md), this API modifies the structure type of the input parameter **scalar** from aclTensor to aclScalar. Select the appropriate API based on actual requirements.
- Formula:
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, `aclnnForeachMinimumScalarV2GetWorkspaceSize` is called to obtain the input parameters and compute the required workspace size based on the process. Then, `aclnnForeachMinimumScalarV2` is called to perform computation.
...
- **Parameters**
  ...
  <tbody>
    <tr>
      <td>x</td>
      <td>Input</td>
      <td>Input tensor list for the minimum operation, corresponding to `x` in the formula.</td>
      <td><ul><li>Empty tensor lists are supported.</li><li>All tensors within this parameter must share the same data type.</li></ul></td>
    ...
    <tr>
      <td>scalar</td>
      <td>Input</td>
      <td>Input scalar for the minimum operation, corresponding to `scalar` in the formula.</td>
      <td>The data type depends on that of `x` and its values must fall within the valid value range of `x`:<ul><li>When `x` is of the FLOAT32 or BFLOAT16 type, the data type must be FLOAT32 or DOUBLE. </li><li>When `x` is of the FLOAT16 type, the data type must be FLOAT16 or DOUBLE. </li><li>When `x` is of the INT32 type, the data type must be INT32 or INT64.</li></li></ul></td>
    ...
    <tr>
      <td>out</td>
      <td>Output</td>
      <td>Output tensor list for the minimum operation, corresponding to `y` in the formula.</td>
      <td><ul><li>Empty tensor lists are supported.</li><li>The data type and format must be the same as those of `x`, and the shape size must be greater than or equal to that of `x`. </li><li>All tensors within this parameter must share the same data type.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16, INT32</td>
      <td>ND</td>
      <td>0–8</td>
      <td>×</td>
...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnForeachMinimumScalarV2GetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- Deterministic computation:
  - **aclnnForeachMinimumScalarV2** defaults to a deterministic implementation.
...
int main() {
  ...
  aclOpExecutor* executor;
  // Call the first-phase API of aclnnForeachMinimumScalarV2.
  ...
  // Call the second-phase API of aclnnForeachMinimumScalarV2.
