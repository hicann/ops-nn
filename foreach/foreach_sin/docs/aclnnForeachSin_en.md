...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_sin)
...
- Description: Performs the sine function operation on each tensor in the input tensor list.
- Formula:
...
- Example
  ...
  The input `x` is [–1, –2] with data type `FLOAT32`.
  After the `aclnnForeachSin` operator is called:
  The output `out` is [–0.8415, –0.9093] with data type `FLOAT32`.

## Prototype

Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnForeachSinGetWorkspaceSize** is called to obtain the input parameters and compute the required workspace size based on the process. Then, **aclnnForeachSin** is called to perform computation.
...
- **Parameters:**
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      <td>x</td>
      <td>Input</td>
      <td>List of input tensors on which the sine function operation is performed, corresponding to `x` in the formula.</td>
    ...
    <tr>
      <td>out</td>
      <td>Output</td>
      <td>List of output tensors on which the sine function operation is performed, corresponding to `y` in the formula.</td>
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, obtained by calling the first-phase API aclnnForeachSinGetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- Deterministic computation:
  - **aclnnForeachSin** defaults to a deterministic implementation.
...
int main() {
  ...
  aclOpExecutor* executor;
  // Call the first-phase API of aclnnForeachSin.
  ...
  // Call the second-phase API of aclnnForeachSin.
