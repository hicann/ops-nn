...
- 接口功能：对输入张量列表x中的每个张量进行指数运算，指数为张量scale，对应公式中的`exponent`。
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachPowScalarGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachPowScalar”接口执行计算。
