...
- 接口功能：对输入张量列表x中的每个张量进行指数运算，指数为标量exponent。本接口相较于[aclnnForeachPowScalar](aclnnForeachPowScalar.md)，修改入参scalar的结构类型aclTensor为aclScalar，请根据实际情况选择合适的接口。
- 计算公式：
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用`aclnnForeachPowScalarV2GetWorkspaceSize`接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用`aclnnForeachPowScalarV2`接口执行计算。
