...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_pow_scalar)
...
- Description: Raises each tensor in the input tensor list **x** to the power of the input scalar **scalar**. Compared with [aclnnForeachPowScalar](./aclnnForeachPowScalar_en.md), this API modifies the structure type of the input parameter **scalar** from aclTensor to aclScalar. Select the appropriate API based on actual requirements.
- Formula:
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, `aclnnForeachPowScalarV2GetWorkspaceSize` is called to obtain the input parameters and compute the required workspace size based on the process. Then, `aclnnForeachPowScalarV2` is called to perform computation.
...
- **Parameters**
  ...
  <tbody>
    <tr>
      <td>x</td>
      <td>Input</td>
      <td>Input tensor list for base values of the power computation, corresponding to `x` in the formula.</td>
      <td><ul><li>Empty tensors are supported. </li><li>All tensors within this parameter must share the same data type.</ul></td>
    ...
    <tr>
      <td>scalar</td>
      <td>Input</td>
      <td>Exponent for the power computation, corresponding to `exponent` in the formula.</td>
      <td><ul><li>The data type depends on that of `x`:<ul><li>When `x` is of the FLOAT32 or BFLOAT16 type, the data type must be FLOAT32 or DOUBLE. </li><li>When `x` is of the FLOAT16 type, the data type must be FLOAT16 or DOUBLE. </li></ul></li><li>Its absolute value must be less than or equal to (2^31 – 1).</li></ul></td>
    ...
    <tr>
      <td>out</td>
      <td>Output</td>
      <td>Output tensor list for the power computation, corresponding to `y` in the formula.</td>
      <td><ul><li>Empty tensors are supported. </li><li>The data type and format must be the same as those of `x`, and the shape size must be greater than or equal to that of `x`. </li><li>All tensors within this parameter must share the same data type.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16</td>
      <td>ND</td>
      <td>0–8</td>
      <td>×</td>
...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnForeachPowScalarV2GetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- Deterministic computation:
  - **aclnnForeachPowScalarV2** defaults to a deterministic implementation.
...
int main() {
  ...
  aclOpExecutor* executor;
  // Call the first-phase API of aclnnForeachPowScalarV2.
  ...
  // Call the second-phase API of aclnnForeachPowScalarV2.
