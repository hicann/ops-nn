...
/*!
 * \file foreach_pow_scalar_tiling_arch35.cpp
 * \brief arch35 tiling implementation for foreach_pow_scalar
 */
...
#include "foreach/foreach_pow_scalar/op_kernel/arch35/foreach_pow_scalar_tiling_data.h"
#include "foreach/foreach_pow_scalar/op_kernel/arch35/foreach_pow_scalar_tiling_key.h"
...
struct ForeachPowScalarCompileInfo {};
...
static ge::graphStatus ForeachPowScalarTilingFunc(gert::TilingContext* context)
{
    // 1. Get platform info
    uint64_t ubSize;
    int64_t coreNum;
    ...
    // 4. Fill tiling data
    ForeachPowScalarTilingData* tiling = context->GetTilingData<ForeachPowScalarTilingData>();
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_INT32) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_INT32);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_POW_SCALAR_TPL_SCH_MODE_BF16);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_pow_scalar");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForForeachPowScalar([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachPowScalar)
    .Tiling(ForeachPowScalarTilingFunc)
    .TilingParse<ForeachPowScalarCompileInfo>(TilingParseForForeachPowScalar);
