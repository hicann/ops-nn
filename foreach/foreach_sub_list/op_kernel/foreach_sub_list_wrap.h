...
/*!
 * \file foreach_sub_list_wrap.h
 * \brief foreach_sub_list int16/int8/uint8 kernel, 整数下溢按二进制补码回绕
 */

#ifndef FOREACH_SUB_LIST_WRAP_H
#define FOREACH_SUB_LIST_WRAP_H
...
constexpr int32_t SUB_LIST_WRAP_BUFFER_NUM = 2;

template <typename T, int32_t bufferNum = SUB_LIST_WRAP_BUFFER_NUM>
class ForeachSubListWrap {
public:
    __aicore__ inline ForeachSubListWrap(){};
...
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::Init(GM_ADDR x1, GM_ADDR x2, GM_ADDR alpha, GM_ADDR y,
                                                              GM_ADDR workspace,
                                                              const ForeachCommonTilingData* tilingData)
{
    (void)workspace;
    ...
    if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
        ...
        pipe.InitBuffer(halfBuf, SUB_LIST_WRAP_BUFFER_NUM * maxDataCount * sizeof(half));
        // [a16 | b16 | mask | lo | hi]
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::ParseTilingData(const ForeachCommonTilingData* tilingData)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::Process()
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::SingleTensorProcess(int64_t dataCount)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::CopyIn(uint32_t index, int64_t dataCount, bool isRemainder)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::CopyIn2(uint32_t index, int64_t dataCount, bool isRemainder)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::Compute(uint32_t index, int64_t dataCount, bool isRemainder)
{
    (void)index;
    (void)isRemainder;
    ...
    if constexpr (std::is_same_v<T, int16_t>) {
        // int16 域: x1 - alpha * x2, 下溢按二进制补码回绕
        Muls(inLocal2, inLocal2, alphaVal, dataCount);
        PipeBarrier<PIPE_V>();
        Sub(outLocal, inLocal1, inLocal2, dataCount);
        PipeBarrier<PIPE_V>();
    } else {
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachSubListWrap<T, bufferNum>::CopyOut(uint32_t index, int64_t dataCount, bool isRemainder)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline __gm__ T* ForeachSubListWrap<T, bufferNum>::GetTensorAddr(uint16_t index, GM_ADDR tensorPtr)
{
