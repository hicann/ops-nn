                                ...
                                const aclTensor* scalars, const aclTensorList* out)
{
    ...
    OP_CHECK_NULL(scalars, return false);
...
{
    auto socArch = GetCurrentPlatformInfo().GetCurNpuArch();
    if (socArch != NpuArch::DAV_2201 && !Ops::NN::AclnnUtil::IsRegbase(socArch)) {
        return EMPTY_LIST;
    }
...
static inline aclnnStatus CheckParams(const aclTensorList* x1, const aclTensorList* x2, const aclTensorList* x3,
                                      const aclTensor* scalars, const aclTensorList* out)
{
    CHECK_RET(CheckNotNull(x1, x2, x3, scalars, out), ACLNN_ERR_PARAM_NULLPTR);
    CHECK_RET(x1->Size() == x2->Size(), ACLNN_ERR_PARAM_INVALID);
    CHECK_RET(x1->Size() == x3->Size(), ACLNN_ERR_PARAM_INVALID);
    ...
    for (uint64_t i = 0; i < x1->Size(); i++) {
        if ((*x1)[i] == nullptr || (*x2)[i] == nullptr || (*x3)[i] == nullptr || (*out)[i] == nullptr) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Tensor entry at index %lu is null.", i);
            return ACLNN_ERR_PARAM_INVALID;
        }
    }

    CHECK_RET(CheckDtypeValid(x1, x2, x3, scalars, out), ACLNN_ERR_PARAM_INVALID);
    CHECK_RET(CheckShape(x1, x2, x3, out), ACLNN_ERR_PARAM_INVALID);
    CHECK_RET(CheckFormat(x1, x2, x3, out), ACLNN_ERR_PARAM_INVALID);
    return ACLNN_SUCCESS;
}

static aclnnStatus ExecForeachAddcdivScalarListGetWorkspaceSize(const aclTensorList* x1, const aclTensorList* x2,
                                                                const aclTensorList* x3, const aclTensor* scalars,
                                                                ...
                                                                aclOpExecutor** executor)
{
    ...
    auto ret = CheckParams(x1, x2, x3, scalars, out);
    ...
    // 调用l0算子ForeachAddcdivScalarList进行计算，输出到连续buffer（scalars为aclTensor*直接透传）
    auto result = l0op::ForeachAddcdivScalarList(contiguousTensorsX1, contiguousTensorsX2, contiguousTensorsX3, scalars,
...
aclnnStatus aclnnForeachAddcdivScalarListGetWorkspaceSize(const aclTensorList* x1, const aclTensorList* x2,
                                                          const aclTensorList* x3, const aclTensor* scalars,
                                                          aclTensorList* out, uint64_t* workspaceSize,
                                                          aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnForeachAddcdivScalarList, DFX_IN(x1, x2, x3, scalars), DFX_OUT(out));
    return ExecForeachAddcdivScalarListGetWorkspaceSize(x1, x2, x3, scalars, out, workspaceSize, executor);
}

aclnnStatus aclnnForeachAddcdivScalarList(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                          const aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnForeachAddcdivScalarList);
