...
/*!
 * \file foreach_maximum_scalar_list_tiling_arch35.cpp
 * \brief arch35 (Ascend950) tiling implementation for foreach_maximum_scalar_list
 */
...
#include "../../op_kernel/arch35/foreach_maximum_scalar_list_tiling_data.h"
#include "../../op_kernel/arch35/foreach_maximum_scalar_list_tiling_key.h"
...
struct ForeachMaximumScalarListCompileInfo {};
...
static ge::graphStatus ForeachMaximumScalarListTilingFunc(gert::TilingContext* context)
{
    uint64_t ubSize;
    int64_t coreNum;
    ...
    ForeachMaximumScalarListTilingData* tiling = context->GetTilingData<ForeachMaximumScalarListTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ForeachMaximumScalarListTilingData), 0,
                         sizeof(ForeachMaximumScalarListTilingData)) != EOK,
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_SCALAR_LIST_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_SCALAR_LIST_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_INT32) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_SCALAR_LIST_TPL_SCH_MODE_INT32);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MAXIMUM_SCALAR_LIST_TPL_SCH_MODE_BF16);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_maximum_scalar_list");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForForeachMaximumScalarList([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachMaximumScalarList)
    .Tiling(ForeachMaximumScalarListTilingFunc)
    .TilingParse<ForeachMaximumScalarListCompileInfo>(TilingParseForForeachMaximumScalarList);
