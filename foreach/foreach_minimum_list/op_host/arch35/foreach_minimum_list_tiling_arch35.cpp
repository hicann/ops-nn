...
/*!
 * \file foreach_minimum_list_tiling_arch35.cpp
 * \brief tiling implementation for foreach_minimum_list (arch35 / Ascend950)
 */
...
#include "../../op_kernel/arch35/foreach_minimum_list_tiling_data.h"
#include "../../op_kernel/arch35/foreach_minimum_list_tiling_key.h"
...
struct ForeachMinimumListCompileInfo {};
...
static ge::graphStatus ForeachMinimumListTilingFunc(gert::TilingContext* context)
{
    uint64_t ubSize;
    int64_t coreNum;
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_LIST_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_LIST_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_LIST_TPL_SCH_MODE_BF16);
    } else if (dataType == ge::DT_INT32) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_MINIMUM_LIST_TPL_SCH_MODE_INT32);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_minimum_list");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForForeachMinimumList([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachMinimumList)
    .Tiling(ForeachMinimumListTilingFunc)
    .TilingParse<ForeachMinimumListCompileInfo>(TilingParseForForeachMinimumList);
