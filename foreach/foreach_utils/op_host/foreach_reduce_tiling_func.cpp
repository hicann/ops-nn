...
private:
    ...
    void AssignDataToEachCore()
    {
        ...
        for (uint32_t i = 0; i < totalTensorCount; i++) {
            ...
            tensorEndOffsetList[coreIndex] = dataCount + tempRealCount < curCmpCount ? tensorDataCountList[i] - 1 :
                                                                                       cursorPos - 1;
            dataCount = 0;
            coreIndex++;
    ...
    ge::graphStatus CheckShapeInfo()
    {
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_TENSORNUM(tilingContext->GetNodeName(), "x", static_cast<int64_t>(xSize),
    ...
    ge::graphStatus CheckInputTensorlistShape(size_t inputTensorsNum)
    {
        ...
        for (size_t tensorId = 0; tensorId < totalTensorCount; tensorId++) {
            ...
            if (x1Shape->GetStorageShape().GetDimNum() > MAX_SUPPORT_DIMS_NUMS) {
                OP_LOGE_FOR_INVALID_SHAPEDIM(tilingContext->GetNodeName(), "x",
            ...
            for (size_t listId = startListIndex + 1; listId < static_cast<size_t>(inputTensorsNum); listId++) {
                if (x1Shape->GetStorageShape() !=
                    ...
                    std::string errMsg = Ops::Base::ToString(
                        tilingContext->GetDynamicInputShape(listId, tensorId)->GetStorageShape());
                    std::string reasonMsg = "The shape of x" + std::to_string(listId + 1) + " should be the same as x" +
                                            std::to_string(startListIndex + 1);
                    OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(tilingContext->GetNodeName(), paramName.c_str(),
                                                          errMsg.c_str(), reasonMsg.c_str());
                    return ge::GRAPH_FAILED;
                }
            }
        }
    ...
    ge::graphStatus CheckOutputShapeAndDtype()
    {
        ...
        for (uint32_t i = 0; i < totalTensorCount; i++) {
            ...
            OP_CHECK_IF(dstShape == nullptr, OP_LOGE(tilingContext->GetNodeName(), "The output %u shape is null.", i),
                        return ge::GRAPH_FAILED);
            OP_CHECK_IF(dstShape->GetStorageShape().GetShapeSize() != 1,
