...
/*!
 * \brief 构造连续 tensorList：已连续且 storage 与 view 一致则直接用原 tensor，否则
 *        l0op::Contiguous 转连续。保持与输入 list 索引/数量一一对应，避免 kernel 端索引错位。失败返回 nullptr。
 ...
 *        会导致 storage_shape 维度不一致触发 tiling 校验失败。故需比较 storage_shape == view_shape，
 *        确保 storage 与 view 的 shape 完全一致（含维度）才跳过。空 tensor 亦适用：不同空 tensor 的
 *        storage_shape 可能不同（如 [0] vs [16386]），需统一为 view_shape 以通过 tiling 校验。
 */
inline const aclTensorList* ForeachMakeContiguousTensorList(const aclTensorList* tensorList, aclOpExecutor* executor)
{
    ...
    for (uint64_t i = 0; i < tensorList->Size(); i++) {
        ...
        if (op::IsContiguous(tensor) && tensor->GetStorageShape() == tensor->GetViewShape()) {
 ...
 *        仅当 ForeachMakeContiguousTensorList 对该 out 做了 Contiguous（即不满足
 *        "连续且 storage_shape==view_shape"）时才需要回写；已连续且 storage 与 view 一致的
 *        out 跳过（kernel 已直接写入 out 内存）。
