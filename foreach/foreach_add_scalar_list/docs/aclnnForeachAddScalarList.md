...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachAddScalarListGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachAddScalarList”接口执行计算。
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>表示加法运算的输入标量。对应公式中的`scalars`。</td>
