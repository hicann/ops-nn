...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_cos)
...
- Description: Performs element-wise cosine operation.
- Formula:
...
- Example
  ...
  1. The input `x` is [–1, –2] with dtype `FLOAT32`.
  2. Call the `aclnnForeachCos` operator.
  3. The output `out` is [0.5403, –0.4161] with dtype `FLOAT32`.

## Prototype

Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnForeachCosGetWorkspaceSize** is called to obtain the input parameters and compute the required workspace size based on the process. Then, **aclnnForeachCos** is called to perform computation.
...
- **Parameters**
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      <td>x</td>
      <td>Input</td>
      <td>Input tensor list for the cosine operation, corresponding to `x` in the formula.</td>
    ...
    <tr>
      <td>out</td>
      <td>Output</td>
      <td>Output tensor list for the cosine operation, corresponding to `y` in the formula.</td>
...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnForeachCosGetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- The input value range of [−65504, 65504] satisfies functional precision requirements; precision is not guaranteed for values outside this range.
- Deterministic computation:
  - **aclnnForeachCos** defaults to a deterministic implementation.
...
int main() {
  ...
  aclOpExecutor* executor;
  // Call the first-phase API of aclnnForeachCos.
  ...
  // Call the second-phase API of aclnnForeachCos.
