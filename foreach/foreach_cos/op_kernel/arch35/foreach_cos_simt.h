 ...
 */
template <typename T>
__simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM) inline void OpForeachCosSimt(int32_t tensorId, int64_t count,
 ...
 */
template <typename T>
__aicore__ inline void Process(GM_ADDR x, GM_ADDR y, const ForeachCosTilingData* tilingGm)
{
    for (int32_t tensorId = 0; tensorId < tilingGm->tensorCount; tensorId++) {
        ...
        AscendC::Simt::VF_CALL<OpForeachCosSimt<T>>(AscendC::Simt::Dim3(THREAD_NUM), tensorId, count, x, y);
    }
}
