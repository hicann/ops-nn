...
public:
    ...
    void GetLog2TmpBufferFactorSize(const uint32_t typeSize, uint32_t& extraBuf, uint32_t LOG2_HALF,
                                    uint32_t LOG2_FLOAT, uint32_t LOG2_BASIC);
...
inline void ForeachCommonTiling::Init(const std::vector<std::vector<uint64_t>>& shapeInfos, uint16_t dType,
                                      uint8_t theCode)
{
    opCode = theCode;
    ...
    if (opCode == FOREACH_COS_OP_CODE) { // foreach_cos
        ...
        if (dataType == 4) {
            canUseUbSize = totalSize / (BINARY_LIST_UB_DIVIDER * UB_DIVIDER_FOR_TEMP_CASTING +
                                        POW_TENSOR_TENSOR_CALC_PROC[GetTilingKeyVal() - 1]);
        } else {
...
inline void ForeachCommonTiling::DivideUbMemory7(uint64_t ubSizePlatForm)
{
    if (opCode == FOREACH_SIN_OP_CODE) {
        ...
        uint32_t totalSize = static_cast<uint32_t>(
            ubSizePlatForm - static_cast<uint32_t>(sizeof(ForeachCommonTilingData)) - reservedUbSize);
        if (dataType == 4) {
