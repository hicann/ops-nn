...
__aicore__ inline void ForeachSubScalarInplaceImpl(GM_ADDR inputs, GM_ADDR scalar, GM_ADDR xRef, GM_ADDR workspace,
...
extern "C" __global__ __aicore__ void foreach_sub_scalar_inplace(GM_ADDR x, GM_ADDR scalar, GM_ADDR xRef,
                                                                 GM_ADDR workspace, GM_ADDR tiling)
{
    TPipe pipeOp;
    if (TILING_KEY_IS(FOREACH_TILING_KEY_HALF)) {
        ForeachSubScalarInplaceImpl<half, DTYPE_SCALAR>(x, scalar, xRef, workspace, tiling, &pipeOp);
    } else if (TILING_KEY_IS(FOREACH_TILING_KEY_FLOAT)) {
        ForeachSubScalarInplaceImpl<float, float>(x, scalar, xRef, workspace, tiling, &pipeOp);
    } else if (TILING_KEY_IS(FOREACH_TILING_KEY_INT)) {
        ForeachSubScalarInplaceImpl<int, int>(x, scalar, xRef, workspace, tiling, &pipeOp);
    } else if (TILING_KEY_IS(FOREACH_TILING_KEY_BF16)) {
        ForeachSubScalarInplaceImpl<bfloat16_t, float>(x, scalar, xRef, workspace, tiling, &pipeOp);
    }
    pipeOp.Destroy();
}
