...
- 接口功能：原地更新输入张量列表，对输入张量列表中的每个张量逐元素减去同一个标量。
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachSubScalarInplaceGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachSubScalarInplace”接口执行计算。
...
## aclnnForeachSubScalarInplaceGetWorkspaceSize
...
- **参数说明**
  ...
  <tbody>
    <tr>
      ...
      <td>输入/输出</td>
      <td>表示进行减法运算的输入和输出张量列表，对应公式中的`x`。</td>
    ...
    <tr>
      ...
      <td><ul><li>不支持空Tensor。</li><li>元素个数为1。</li><li>数据类型与入参`xRef`的数据类型具有一定对应关系：<ul><li>当`xRef`的数据类型为FLOAT32、FLOAT16、INT32时，数据类型与`xRef`的数据类型保持一致。</li><li>当`xRef`的数据类型为BFLOAT16时，数据类型支持FLOAT32。</li></ul></li></ul></td>
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachSubScalarInplaceGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachSubScalarInplace默认确定性实现。
...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_foreach_sub_scalar_inplace.h"
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachSubScalarInplace第一段接口
  ret = aclnnForeachSubScalarInplaceGetWorkspaceSize(tensorListInput, alpha, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachSubScalarInplaceGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachSubScalarInplace第二段接口
  ret = aclnnForeachSubScalarInplace(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachSubScalarInplace failed. ERROR: %d\n", ret); return ret);
