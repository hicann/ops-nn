...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachAddScalarV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachAddScalarV2”接口执行计算。
...
- <term>Atlas A2 训练系列产品/Atlas A2 推理系列产品</term>、<term>Atlas A3 训练系列产品/Atlas A3 推理系列产品</term>：
  ...
  入参`scalar`与入参`x`的数据类型具有一定对应关系：
   ...
   - 当`x`的数据类型为INT32、INT16、INT8、UINT8时，数据类型支持INT32、INT64。
<!-- end id7 -->

<!-- npu="950" id8 -->
- <term>Ascend 950PR/Ascend 950DT</term>：
  - 入参`x`支持包含的最大Tensor个数为50，且不支持INT16、INT8、UINT8。
  - 入参`scalar`与入参`x`的数据类型具有一定对应关系：
...
int main() {
  ...
  CHECK_RET(alpha != nullptr, return -1);
  // 创建out1 aclTensor
