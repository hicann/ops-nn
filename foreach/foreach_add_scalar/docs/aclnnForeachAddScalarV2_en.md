...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_add_scalar)
...
- Description: Adds the specified scalar value to each tensor in the tensor list. Compared with [aclnnForeachAddScalar](./aclnnForeachAddScalar_en.md), this API modifies the structure type of the input parameter **scalar** from aclTensor to aclScalar. Select the appropriate API based on actual requirements.
- Formula:
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnForeachAddScalarV2GetWorkspaceSize** is called to obtain the input parameters and compute the required workspace size based on the process. Then, **aclnnForeachAddScalarV2** is called to perform computation.
...
- **Parameters**
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      <td>x</td>
      <td>Input</td>
      <td>Input tensor list for the addition operation, corresponding to `x` in the formula.</td>
    ...
    <tr>
      <td>scalar</td>
      <td>Input</td>
      <td>Input scalar for the addition operation, corresponding to `scalar` in the formula.</td>
      <td>-</td>
...
- <term>Atlas A2 training series products/Atlas A2 inference series products</term> and <term>Atlas A3 training series products/Atlas A3 inference series products</term>:
  ...
  The data type of `scalar` depends on that of `x`:
   ...
   - When `x` is of the FLOAT32 or BFLOAT16 type, the data type must be FLOAT32 or DOUBLE.
   - When `x` is of the FLOAT16 type, the data type must be FLOAT16 or DOUBLE.
   - When `x` is of the INT32 type, the data type must be INT32 or INT64.
...
- **Returns**
  ...
  **aclnnStatus**: status code. For details, see [aclnn Return Codes](../../../docs/en/context/aclnn_return_codes_nn.md).
  ...
  <tbody>
    ...
    <tr>
      <td>The data types of x and out do not match.
    </tr>
    <tr>
      <td>The shape of x or out does not meet the constraints.
    <tr>
      <td>The data types of tensors in x or out do not match.
    </tr> 
    <tr>
      <td>A tensor in x or out has more than eight dimensions.
    </tr>
    </tr>
  </tbody></table>
...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnForeachAddScalarV2GetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- Deterministic computation:
  - **aclnnForeachAddScalarV2** defaults to a deterministic implementation.
...
int main() {
  ...
  aclOpExecutor* executor;
  // Call the first-phase API of aclnnForeachAddScalarV2.
  ...
  // Call the second-phase API of aclnnForeachAddScalarV2.
