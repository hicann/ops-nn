...
/*!
 * \file foreach_add_scalar_wrap.h
 * \brief foreach_add_scalar int16/int8/uint8 kernel, 整数溢出按二进制补码回绕
 */

#ifndef FOREACH_ADD_SCALAR_WRAP_H
#define FOREACH_ADD_SCALAR_WRAP_H
...
constexpr int32_t ADD_SCALAR_WRAP_BUFFER_NUM = 2;

template <typename T, int32_t bufferNum = ADD_SCALAR_WRAP_BUFFER_NUM>
class ForeachAddScalarWrap {
public:
    __aicore__ inline ForeachAddScalarWrap(){};
...
__aicore__ inline void ForeachAddScalarWrap<T, bufferNum>::Init(GM_ADDR x, GM_ADDR scalar, GM_ADDR y, GM_ADDR workspace,
                                                                const ForeachCommonTilingData* tilingData)
{
    (void)workspace;
    ...
    scalarVal = static_cast<int16_t>(inScalarGM.GetValue(0));
    ...
    if constexpr (std::is_same_v<T, int8_t> || std::is_same_v<T, uint8_t>) {
        ...
        pipe.InitBuffer(halfBuf, ADD_SCALAR_WRAP_BUFFER_NUM * maxDataCount * sizeof(half));
        // [a16 | mask | lo | hi]
        pipe.InitBuffer(int16Buf, 4 * maxDataCount * sizeof(int16_t));
    } else {
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachAddScalarWrap<T, bufferNum>::ParseTilingData(const ForeachCommonTilingData* tilingData)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachAddScalarWrap<T, bufferNum>::Process()
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachAddScalarWrap<T, bufferNum>::SingleTensorProcess(int64_t dataCount)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachAddScalarWrap<T, bufferNum>::CopyIn(uint32_t index, int64_t dataCount, bool isRemainder)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachAddScalarWrap<T, bufferNum>::Compute(uint32_t index, int64_t dataCount, bool isRemainder)
{
    (void)index;
    (void)isRemainder;
    LocalTensor<T> inLocal = dataQueue.template DeQue<T>();
    LocalTensor<T> outLocal = outQueue.template AllocTensor<T>();

    PipeBarrier<PIPE_V>();
    if constexpr (std::is_same_v<T, int16_t>) {
        // int16 域: x + scalar, 溢出按二进制补码回绕
        Adds(outLocal, inLocal, scalarVal, dataCount);
        PipeBarrier<PIPE_V>();
    } else {
        ...
        LocalTensor<int16_t> mask16 = int16Buf.GetWithOffset<int16_t>(maxDataCount, maxDataCount * sizeof(int16_t));
        LocalTensor<int16_t> lo16 = int16Buf.GetWithOffset<int16_t>(maxDataCount, 2 * maxDataCount * sizeof(int16_t));
        LocalTensor<int16_t> hi16 = int16Buf.GetWithOffset<int16_t>(maxDataCount, 3 * maxDataCount * sizeof(int16_t));

        Cast(h1, inLocal, RoundMode::CAST_NONE, dataCount);
        PipeBarrier<PIPE_V>();
        Cast(a16, h1, RoundMode::CAST_RINT, dataCount);
        PipeBarrier<PIPE_V>();
        Adds(a16, a16, scalarVal, dataCount);
        PipeBarrier<PIPE_V>();
    ...
    dataQueue.FreeTensor(inLocal);
}

template <typename T, int32_t bufferNum>
__aicore__ inline void ForeachAddScalarWrap<T, bufferNum>::CopyOut(uint32_t index, int64_t dataCount, bool isRemainder)
{
...
template <typename T, int32_t bufferNum>
__aicore__ inline __gm__ T* ForeachAddScalarWrap<T, bufferNum>::GetTensorAddr(uint16_t index, GM_ADDR tensorPtr)
{
...
#endif // FOREACH_ADD_SCALAR_WRAP_H
