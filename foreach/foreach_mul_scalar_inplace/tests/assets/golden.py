...
def __golden_foreach_mul_scalar_inplace(x_list, scalar, **kwargs):
    ...
    scalar_val = torch.from_numpy(np.asarray(scalar, "float64").reshape(-1)[:1])[0]
...
_GOLDEN_FN = __golden_foreach_mul_scalar_inplace


class _ForeachMulScalarInplaceCompose:
    def __call__(self, x, scalar, **kwargs):
        return torch._foreach_mul(_tp_list(x), _tp_scalar(scalar))
...
class _TpKernelFaithful:
    _INNER = _ForeachMulScalarInplaceCompose
...
try:
    _TpKernelFaithful.__call__.__signature__ = _kf_inspect.signature(
        _ForeachMulScalarInplaceCompose.__call__
    )
except (ValueError, TypeError):  # 内层无法内省时保持原样
    pass


class ForeachMulScalarInplaceKernelSpec:
    golden = _GOLDEN_FN
...
__spec__ = {
    "foreach_mul_scalar_inplace": "ForeachMulScalarInplaceKernelSpec",
    "aclnnForeachMulScalarInplace": "ForeachMulScalarInplaceAclnnSpec",
}
...
class _TpAclnn:
    ...
    inplace 算子的 aclnn 头文件把首个(被原地改写的)形参写作 `xRef`, 而 def 注册名是
    `x`。服务端对 third_party 按名绑定(remote/server/execution_container.py::
    bind_params), pool 的 key 取自头文件形参名; 直接复用 kernel 通路的竞品类会因
    形参 `x` 不在 pool 中而抛 UnknownParamError, 三方腿整条起不来, cross_check
    随即 GOLDEN_FAILURE。故按头文件形参名另立适配类, 内部转调同一个竞品类。
    """

    def __call__(self, xRef, scalar, **kwargs):
        return _TpKernelFaithful()(xRef, scalar)


class ForeachMulScalarInplaceAclnnSpec:
    ...
    故签名逐项对齐 aclnnForeachMulScalarInplaceGetWorkspaceSize 的形参;
    ...
    def golden(x, scalar, **kwargs):
        return _keep_dtype(
            torch._foreach_mul([_tp_one(t) for t in x], _tp_num(scalar)), x
        )
