...
- 接口功能：原地更新输入张量列表x1，对张量列表x1和张量列表x2执行逐元素除法。
...
- 计算公式：
  ...
  $$
  x1_i = \frac{x1_i}{x2_i} (i=0,1,...n-1)
  $$

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachDivListInplaceGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachDivListInplace”接口执行计算。
...
## aclnnForeachDivListInplaceGetWorkspaceSize
...
- **参数说明**
  ...
  <table style="undefined;table-layout: fixed; width: 1550px"><colgroup>
    ...
    <tr>
      ...
      <td>输入/输出</td>
      <td>表示除法运算的第一个输入和输出张量列表，对应公式中的`x1`。</td>
...
- **返回值**
  ...
  <tbody>
    <tr>
      ...
      <td>161001</td>
      <td>传入的x1Ref、x2是空指针。</td>
    </tr>
    <tr>
      <td rowspan="3">ACLNN_ERR_PARAM_INVALID</td>
      <td rowspan="3">161002</td>
      <td>x1Ref、x2的数据类型不在支持的范围之内。</td>
    </tr>
    <tr>
    ...
    <tr>
      <td>x1Ref或x2中的Tensor的数据类型不一致。</td>
    </tr>
    <tr>
      <td>x1Ref或x2中的Tensor维度超过8维。</td>
    </tr>
    <tr>
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachDivListInplaceGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachDivListInplace默认确定性实现。
...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_foreach_div_list_inplace.h"
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachDivListInplace第一段接口
  ret = aclnnForeachDivListInplaceGetWorkspaceSize(tensorListInput1, tensorListInput2, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachDivListInplaceGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachDivListInplace第二段接口
  ret = aclnnForeachDivListInplace(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachDivListInplace failed. ERROR: %d\n", ret); return ret);
