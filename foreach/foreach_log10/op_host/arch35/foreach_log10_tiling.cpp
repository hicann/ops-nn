...
/*!
 * \file foreach_log10_tiling.cpp
 * \brief arch35 tiling implementation for foreach_log10
 */
...
#include "../../op_kernel/arch35/foreach_log10_tiling_data.h"
#include "../../op_kernel/arch35/foreach_log10_tiling_key.h"
...
struct ForeachLog10CompileInfo {};
...
static ge::graphStatus ForeachLog10TilingFunc(gert::TilingContext* context)
{
    uint64_t ubSize;
    int64_t coreNum;
    ...
    ForeachLog10TilingData* tiling = context->GetTilingData<ForeachLog10TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ForeachLog10TilingData), 0, sizeof(ForeachLog10TilingData)) != EOK,
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_LOG10_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_LOG10_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_LOG10_TPL_SCH_MODE_BF16);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_log10");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForForeachLog10([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachLog10)
    .Tiling(ForeachLog10TilingFunc)
    .TilingParse<ForeachLog10CompileInfo>(TilingParseForForeachLog10);
