...
#include "foreach/foreach_sub_scalar/op_kernel/arch35/foreach_sub_scalar_tiling_data.h"
#include "foreach/foreach_sub_scalar/op_kernel/arch35/foreach_sub_scalar_tiling_key.h"
...
struct ForeachSubScalarCompileInfo {};
...
static ge::graphStatus ForeachSubScalarTilingFunc(gert::TilingContext* context)
{
    // 1. Get platform info
    uint64_t ubSize;
    int64_t coreNum;
    ...
    // 4. Fill tiling data
    ForeachSubScalarTilingData* tiling = context->GetTilingData<ForeachSubScalarTilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ForeachSubScalarTilingData), 0, sizeof(ForeachSubScalarTilingData)) != EOK,
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SUB_SCALAR_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SUB_SCALAR_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_INT32) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SUB_SCALAR_TPL_SCH_MODE_INT32);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_SUB_SCALAR_TPL_SCH_MODE_BF16);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_sub_scalar");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForForeachSubScalar([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachSubScalar)
    .Tiling(ForeachSubScalarTilingFunc)
    .TilingParse<ForeachSubScalarCompileInfo>(TilingParseForForeachSubScalar);
