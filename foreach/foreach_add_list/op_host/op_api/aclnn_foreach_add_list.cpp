...
static const std::initializer_list<DataType> SCALAR_INT_SUPPORT_LIST = {DataType::DT_INT32, DataType::DT_INT64};
...
static inline bool CheckNotNull(const aclTensorList* x1, const aclTensorList* x2, const aclTensor* alpha,
...
static inline bool CheckFormat(const aclTensorList* x1, const aclTensorList* x2, const aclTensorList* out)
{
    for (uint64_t i = 0; i < x1->Size(); i++) {
        if (IsPrivateFormat((*x1)[i]->GetStorageFormat()) || IsPrivateFormat((*x2)[i]->GetStorageFormat()) ||
...
static inline bool CheckDtypeValid(const aclTensorList* x1, const aclTensorList* x2, const aclTensor* alpha,
                                   const aclTensorList* out)
{
    ...
    if (inputDtype == DataType::DT_BF16 || inputDtype == DataType::DT_FLOAT) {
        OP_CHECK_DTYPE_NOT_SUPPORT(alpha, SCALAR_FLOAT_SUPPORT_LIST, return false);
    } else if (inputDtype == DataType::DT_FLOAT16) {
        OP_CHECK_DTYPE_NOT_SUPPORT(alpha, SCALAR_FLOAT16_SUPPORT_LIST, return false);
    } else if (inputDtype == DataType::DT_INT32) {
        OP_CHECK_DTYPE_NOT_SUPPORT(alpha, SCALAR_INT_SUPPORT_LIST, return false);
    }
...
static inline bool CheckShape(const aclTensorList* x1, const aclTensorList* x2, const aclTensorList* out)
{
    ...
    if (x2->Size() != x1->Size() || x1->Size() != out->Size()) {
...
static inline aclnnStatus CheckParams(const aclTensorList* x1, const aclTensorList* x2, const aclTensor* alpha,
                                      const aclTensorList* out)
{
...
static aclnnStatus ExecForeachAddListGetWorkspaceSize(const aclTensorList* x1, const aclTensorList* x2,
                                                      ...
                                                      uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    auto ret = CheckParams(x1, x2, alpha, out);
    ...
    // 复用V2桥接，alpha(aclTensor*)直接透传，输出到连续buffer
    auto result = l0op::ForeachAddListV2(contiguousTensorsX1, contiguousTensorsX2, alpha, contiguousOut,
                                         uniqueExecutor.get());
...
aclnnStatus aclnnForeachAddListGetWorkspaceSize(const aclTensorList* x1, const aclTensorList* x2,
                                                const aclTensor* alpha, aclTensorList* out, uint64_t* workspaceSize,
                                                aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnForeachAddList, DFX_IN(x1, x2, alpha), DFX_OUT(out));
    return ExecForeachAddListGetWorkspaceSize(x1, x2, alpha, out, workspaceSize, executor);
}

aclnnStatus aclnnForeachAddList(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                const aclrtStream stream)
{
    L2_DFX_PHASE_2(aclnnForeachAddList);
