...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/foreach/foreach_add_list)
...
- Description: Performs element-wise addition on the two tensor lists. The addition coefficient can be adjusted by the **alpha** parameter. Compared with [aclnnForeachAddList](./aclnnForeachAddList_en.md), this API modifies the structure type of the input parameter **alpha** from aclTensor to aclScalar. Select the appropriate API based on actual requirements.
- Formula:
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnForeachAddListV2GetWorkspaceSize** is called to obtain the input parameters and compute the required workspace size based on the process. Then, **aclnnForeachAddListV2** is called to perform computation.
...
- **Parameters**
  ...
  <tbody>
    <tr>
      <td>x1</td>
      <td>Input</td>
      <td>First input tensor list for the addition operation, corresponding to `x1` in the formula.</td>
      <td><ul><li>Empty tensors are not supported. </li><li>All tensors within this parameter must share the same data type.</li></ul></td>
    ...
    <tr>
      <td>x2</td>
      <td>Input</td>
      <td>Second input tensor list for the addition operation, corresponding to `x2` in the formula.</td>
      <td><ul><li>Empty tensors are not supported. </li><li>All tensors within this parameter must share the same data type. </li><li>The data type, data format, and shape must be the same as those of `x1`.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16, INT32</td>
      <td>ND</td>
      <td>0–8</td>
      <td>×</td>
    ...
    <tr>
      <td>alpha</td>
      <td>Input</td>
      <td>Coefficient for the second input in the addition operation, corresponding to `alpha` in the formula.</td>
      <td>The data type depends on that of `x1`:<ul><li>When `x1` is of the FLOAT32 or BFLOAT16 type, the data type must be FLOAT32 or DOUBLE. </li><li>When `x1` is of the FLOAT16 type, the data type must be FLOAT16 or DOUBLE. </li><li>When `x1` is of the INT32 type, the data type must be INT32 or INT64.</li></ul></td>
    ...
    <tr>
      <td>out</td>
      <td>Output</td>
      <td>Output tensor list for the addition operation, corresponding to `y` in the formula.</td>
      <td><ul><li>Empty tensors are not supported. </li><li>All tensors within this parameter must share the same data type. </li><li>The data type and format must be the same as those of `x1`, and the shape size must be greater than or equal to that of `x1`.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16, INT32</td>
      <td>ND</td>
      <td>0–8</td>
      <td>×</td>
  ...
  - <term>Atlas A2 training series products/Atlas A2 inference series products</term> and <term>Atlas A3 training series products/Atlas A3 inference series products</term>:
    ...
    The data type of `scalar` depends on that of `x1`:
    - When `x1` is of the FLOAT32 or BFLOAT16 type, the data type must be FLOAT32 or DOUBLE.
    - When `x1` is of the FLOAT16 type, the data type must be FLOAT16 or DOUBLE.
    - When `x1` is of the INT32 type, the data type must be INT32 or INT64.
- **Returns**
  ...
  **aclnnStatus**: status code. For details, see [aclnn Return Codes](../../../docs/en/context/aclnn_return_codes_nn.md).
  ...
  <tbody>
    <tr>
      ...
      <td>161001</td>
      <td>The passed x1, x2, alpha, or out is a null pointer.</td>
    </tr>
    <tr>
      <td rowspan="5">ACLNN_ERR_PARAM_INVALID</td>
      <td rowspan="5">161002</td>
      <td>The data type of x1, x2, alpha, or out is not supported.</td>
    </tr>
    <tr>
      <td>The data types of x1, x2, and out do not match.</td>
    </tr>
    <tr>
      <td>The shape of x1, x2, or out does not meet the constraints.</td>
    </tr>
    <tr>
      <td>The element data types of the tensors in x1, x2, or out do not match.</td>
    </tr>
    <tr>
      <td>A tensor in x1, x2, or out has more than eight dimensions.</td>
    </tr>
  </tbody></table>
...
- **Parameters**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by calling the first-phase API aclnnForeachAddListV2GetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- Deterministic computation:
  - **aclnnForeachAddListV2** defaults to a deterministic implementation.
...
int main() {
  ...
  aclOpExecutor* executor;
  // Call the first-phase API of aclnnForeachAddListV2.
  ...
  // Call the second-phase API of aclnnForeachAddListV2.
