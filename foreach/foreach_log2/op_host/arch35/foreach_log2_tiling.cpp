...
/*!
 * \file foreach_log2_tiling.cpp
 * \brief tiling implementation for foreach_log2
 */
...
#include "../../op_kernel/arch35/foreach_log2_tiling_key.h"
...
static ge::graphStatus ForeachLog2TilingFunc(gert::TilingContext* context)
{
    // 1. Get platform info
    uint64_t ubSize;
    int64_t coreNum;
    ...
    // 4. Fill tiling data
    ForeachLog2TilingData* tiling = context->GetTilingData<ForeachLog2TilingData>();
    OP_CHECK_NULL_WITH_CONTEXT(context, tiling);
    OP_CHECK_IF(memset_s(tiling, sizeof(ForeachLog2TilingData), 0, sizeof(ForeachLog2TilingData)) != EOK,
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_LOG2_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_LOG2_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_LOG2_TPL_SCH_MODE_BF16);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_log2");
        return ge::GRAPH_FAILED;
    }
...
static ge::graphStatus TilingParseForForeachLog2([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachLog2)
    .Tiling(ForeachLog2TilingFunc)
    .TilingParse<ForeachLog2CompileInfo>(TilingParseForForeachLog2);
