...
- 接口功能：原地更新输入张量列表，对输入张量列表中的每个张量逐元素求反余弦。
...
- 计算公式：
  ...
  $$
  x_i = \arccos(x_i) (i=0,1,...n-1)
  $$

## 函数原型

每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachACosInplaceGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachACosInplace”接口执行计算。
...
## aclnnForeachACosInplaceGetWorkspaceSize
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnForeachACosInplaceGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
- 确定性计算：
  - aclnnForeachACosInplace默认确定性实现。
...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_foreach_a_cos_inplace.h"
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnForeachACosInplace第一段接口
  ret = aclnnForeachACosInplaceGetWorkspaceSize(tensorListInput, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachACosInplaceGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnForeachACosInplace第二段接口
  ret = aclnnForeachACosInplace(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnForeachACosInplace failed. ERROR: %d\n", ret); return ret);
