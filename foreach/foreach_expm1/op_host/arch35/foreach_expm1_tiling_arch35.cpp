...
/*!
 * \file foreach_expm1_tiling_arch35.cpp
 * \brief tiling implementation for foreach_expm1 on ascend950
 */
...
#include "../../op_kernel/arch35/foreach_expm1_tiling_data.h"
#include "../../op_kernel/arch35/foreach_expm1_tiling_key.h"
...
struct ForeachExpm1CompileInfo {};
...
static ge::graphStatus ForeachExpm1TilingFunc(gert::TilingContext* context)
{
    uint64_t ubSize;
    int64_t coreNum;
    ...
    if (dataType == ge::DT_FLOAT) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_EXPM1_TPL_SCH_MODE_FLOAT);
    } else if (dataType == ge::DT_FLOAT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_EXPM1_TPL_SCH_MODE_FLOAT16);
    } else if (dataType == ge::DT_BF16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_EXPM1_TPL_SCH_MODE_BF16);
    } else if (dataType == ge::DT_INT16) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_EXPM1_TPL_SCH_MODE_INT16);
    } else if (dataType == ge::DT_INT8) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_EXPM1_TPL_SCH_MODE_INT8);
    } else if (dataType == ge::DT_UINT8) {
        tilingKey = GET_TPL_TILING_KEY(FOREACH_EXPM1_TPL_SCH_MODE_UINT8);
    } else {
        OP_LOGE(context, "unsupported dtype for foreach_expm1");
...
static ge::graphStatus TilingParseForForeachExpm1([[maybe_unused]] gert::TilingParseContext* context)
{
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(ForeachExpm1)
    .Tiling(ForeachExpm1TilingFunc)
    .TilingParse<ForeachExpm1CompileInfo>(TilingParseForForeachExpm1);
