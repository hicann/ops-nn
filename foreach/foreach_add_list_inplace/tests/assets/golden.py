...
_GOLDEN_FN = __golden_foreach_add_list_inplace


class _ForeachAddListInplaceCompose:
    def __call__(self, x1, x2, alpha, **kwargs):
        # 与 CPU golden 同口径: 必须 _foreach_mul + _foreach_add **两步拼接**,
        # 不能用 _foreach_add(alpha=) 的 FMA 单次舍入形式。内核是 Muls 再
        # Add 的**两次**舍入; 三方腿若融合成一次舍入, 结果落在正确舍入值上,
        ...
        return torch._foreach_add(_tp_list(x1), scaled)
...
class _TpKernelFaithful:
    _INNER = _ForeachAddListInplaceCompose
...
try:
    _TpKernelFaithful.__call__.__signature__ = _kf_inspect.signature(
        _ForeachAddListInplaceCompose.__call__
    )
except (ValueError, TypeError):  # 内层无法内省时保持原样
    pass


class ForeachAddListInplaceKernelSpec:
    golden = _GOLDEN_FN
...
__spec__ = {
    "foreach_add_list_inplace": "ForeachAddListInplaceKernelSpec",
    "aclnnForeachAddListInplace": "ForeachAddListInplaceAclnnSpec",
}
...
class ForeachAddListInplaceAclnnSpec:
    ...
    故签名逐项对齐 aclnnForeachAddListInplaceGetWorkspaceSize 的形参;
    ...
    def golden(x1, x2, alpha, **kwargs):
        ...
        # 两步拼接, 不用 alpha= 的 FMA 单次舍入形式
        return _keep_dtype(
            torch._foreach_add(a, torch._foreach_mul(b, _tp_num(alpha))), x1
        )
