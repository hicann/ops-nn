...
- 接口功能：对输入张量列表的每个张量进行幂运算（底数为x1，指数为x2）。
- 计算公式：
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnForeachPowListGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnForeachPowList”接口执行计算。
