...
static inline aclnnStatus CheckParams(const aclTensorList* self, const aclScalar* scalar, const aclTensorList* out)
{
    ...
    for (uint64_t i = 0; i < self->Size(); i++) {
        if ((*self)[i] == nullptr || (*out)[i] == nullptr) {
...
static aclnnStatus ExecForeachDivScalarV2GetWorkspaceSize(const aclTensorList* x, const aclScalar* scalar,
...
aclnnStatus aclnnForeachDivScalarV2GetWorkspaceSize(const aclTensorList* x, const aclScalar* scalar, aclTensorList* out,
...
aclnnStatus aclnnForeachDivScalarV2(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
