...
#include "../../../op_api/aclnn_embedding_renorm.h"
...
TEST_F(l2EmbeddingRenormTest, testcase_001_normal)
{
    auto selfDesc = TensorDesc({3, 3}, ACL_FLOAT, ACL_FORMAT_ND)
                        .ValueRange(-10, 10)
                        .Value(vector<float>{1, 1, 1, 2, 3, 4, 5, 6, 7});
...
TEST_F(l2EmbeddingRenormTest, testcase_002_nullptr)
{
    ...
    double normType = 1;
    double maxNorm = 1;
    auto ut = OP_API_UT(aclnnEmbeddingRenorm, INPUT((aclTensor*)nullptr, indicesDesc, maxNorm, normType), OUTPUT());
...
TEST_F(l2EmbeddingRenormTest, testcase_003_nullptr)
{
    ...
    double normType = 1;
    double maxNorm = 1;
    auto ut = OP_API_UT(aclnnEmbeddingRenorm, INPUT(selfDesc, (aclTensor*)nullptr, maxNorm, normType), OUTPUT());
...
{
    auto selfDesc = TensorDesc({3, 3}, ACL_FLOAT16, ACL_FORMAT_ND)
                        .Value(vector<float>{2, 3, 4, 5, 6, 7, 8, 9, 9})
                        .Precision(0.01, 0.01);
    auto indicesDesc = TensorDesc({3}, ACL_INT64, ACL_FORMAT_ND).ValueRange(-3, 2).Value(vector<int64_t>{0, 2, 1});
    double normType = 2.;
    double maxNorm = 2.;
