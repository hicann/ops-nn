...
#ifndef _RENORM_SM_TL_H_
#define _RENORM_SM_TL_H_
...
template <typename D_T_X, bool INTEGER_POWER = false>
class RenormSmTl {
public:
    __aicore__ inline RenormSmTl() {}
...
// ============================================================================
template <typename D_T_X, bool INTEGER_POWER>
__aicore__ inline void RenormSmTl<D_T_X, INTEGER_POWER>::Init(GM_ADDR x, GM_ADDR y, GM_ADDR workspace,
...
// ============================================================================
template <typename D_T_X, bool INTEGER_POWER>
__aicore__ inline void RenormSmTl<D_T_X, INTEGER_POWER>::Process()
{
    if (totalElements_ == 0 || sliceCount_ == 0 || numBlocks_ == 0 || sliceTileLength_ == 0) {
        return;
    }
    ...
    if (blockIdx_ == 0) {
        // V: 生成全零向量
        int64_t clearTile = (sliceTileLength_ < sliceCount_) ? sliceTileLength_ : sliceCount_;
        int64_t clearChunk = (clearTile + ATOMIC_ALIGN - 1) / ATOMIC_ALIGN * ATOMIC_ALIGN;
        for (int64_t clearOffset = 0; clearOffset < wsStride_; clearOffset += clearChunk) {
            int64_t clearLen = (clearChunk < (wsStride_ - clearOffset)) ? clearChunk : (wsStride_ - clearOffset);
            Duplicate(normLocal, 0.0f, static_cast<int32_t>(clearLen));
            PipeBarrier<PIPE_V>();
            // V → MTE3 同步
            ...
            DataCopy(workspaceGM[clearOffset], normLocal, static_cast<int32_t>(clearLen));
...
#define RENORM_P1_REDUCE(wBuf)                                                                   \
    do {                                                                                         \
        ...
        bool canBatchNormLoad = (srcStrideBytes % 32 == 0);
        if (numBlocksThisCore == 1 || !canBatchNormLoad) {
            // --- 单 block 路径: 无需批量处理 ---
            // 直接 LoadAndCast + RENORM_P1_REDUCE
            for (int64_t bi = 0; bi < numBlocksThisCore; ++bi) {
                int64_t gmOffset = (blockStart_ + bi) * sliceCount_ + sliceTile;
                ...
                if constexpr (sizeof(D_T_X) == sizeof(float)) {
                    ...
                    if (normMode_ == NORM_MODE_P_POSITIVE) {
                        ...
                        if (p_ == 1.0f) {
                            ...
                            if (p_ == static_cast<float>(exponent) && exponent >= 3 && exponent <= 100) {
                                // Integer p is common in the generated suite.  Binary exponentiation
                                // replaces the high-latency Log/Exp pair with at most ten Mul operations.
                                PipeBarrier<PIPE_V>();
                                DataCopy(dataLocal1, dataLocal0, static_cast<int32_t>(batchTotalElements));
                                PipeBarrier<PIPE_V>();
                                Duplicate(dataLocal0, 1.0f, static_cast<int32_t>(batchTotalElements));
                                while (exponent > 0) {
                                    if ((exponent & 1) != 0) {
                                        Mul(dataLocal0, dataLocal0, dataLocal1,
                                            static_cast<int32_t>(batchTotalElements));
                                        PipeBarrier<PIPE_V>();
                                    }
                                    exponent >>= 1;
                                    if (exponent > 0) {
                                        Mul(dataLocal1, dataLocal1, dataLocal1,
                                            static_cast<int32_t>(batchTotalElements));
                                        PipeBarrier<PIPE_V>();
                                    }
                                }
                        ...
                        PipeBarrier<PIPE_V>();
                        // 优化: 用 Add<float,false> repeat 替代 for 循环
                        // alignedLen<=128: mask=alignedLen, 1 轮, ceil(batchCount/255) 条指令
                        // alignedLen>128: 回退 for 循环 (mask 最大 128, 多轮不均匀)
                        if (alignedLen <= 128) {
                            ...
                            addParams.src1RepStride = static_cast<int32_t>(alignedLen / 8);
                            int64_t remaining = batchCount;
                            int64_t off = 0;
                            while (remaining > 0) {
                                uint8_t repTimes = static_cast<uint8_t>(remaining > 255 ? 255 : remaining);
                                Add<float, false>(normLocal, normLocal,
                                                  dataLocal0[static_cast<int32_t>(off * alignedLen)], MASK_PLACEHOLDER,
                                                  repTimes, addParams);
                                off += repTimes;
                                remaining -= repTimes;
                            }
                    ...
                    if (normMode_ == NORM_MODE_P_POSITIVE) {
                        ...
                        if (p_ == 1.0f) {
                            ...
                            if (p_ == static_cast<float>(exponent) && exponent >= 3 && exponent <= 100) {
                                PipeBarrier<PIPE_V>();
                                DataCopy(workLocal1, workLocal, static_cast<int32_t>(batchTotalElements));
                                PipeBarrier<PIPE_V>();
                                Duplicate(workLocal, 1.0f, static_cast<int32_t>(batchTotalElements));
                                while (exponent > 0) {
                                    if ((exponent & 1) != 0) {
                                        Mul(workLocal, workLocal, workLocal1, static_cast<int32_t>(batchTotalElements));
                                        PipeBarrier<PIPE_V>();
                                    }
                                    exponent >>= 1;
                                    if (exponent > 0) {
                                        Mul(workLocal1, workLocal1, workLocal1,
                                            static_cast<int32_t>(batchTotalElements));
                                        PipeBarrier<PIPE_V>();
                                    }
                                }
                        ...
                        PipeBarrier<PIPE_V>();
                        // 优化: 用 Add<float,false> repeat 替代 for 循环 (同 FP32 路径)
                        if (alignedLen <= 128) {
                            uint64_t maskLo = (alignedLen >= 64) ? FULL_MASK : ((1UL << alignedLen) - 1);
                            uint64_t maskHi = (alignedLen <= 64) ?
                                                  0 :
                            ...
                            while (remaining > 0) {
                                uint8_t repTimes = static_cast<uint8_t>(remaining > 255 ? 255 : remaining);
                                Add<float, false>(normLocal, normLocal,
                                                  workLocal[static_cast<int32_t>(off * alignedLen)], MASK_PLACEHOLDER,
                                                  repTimes, addParams);
                                off += repTimes;
                                remaining -= repTimes;
                            }
