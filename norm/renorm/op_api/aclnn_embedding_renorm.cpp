...
static const std::initializer_list<op::DataType> DTYPE_DTYPE_SUPPORT_LIST = {op::DataType::DT_FLOAT,
                                                                             op::DataType::DT_FLOAT16};

static const std::initializer_list<op::DataType> DTYPE_DTYPE_SUPPORT_LIST_WITH_BF16 = {
    op::DataType::DT_FLOAT, op::DataType::DT_FLOAT16, op::DataType::DT_BF16};

static const std::initializer_list<op::DataType> DTYPE_SUPPORT_LIST_INDICES = {op::DataType::DT_INT32,
...
static bool CheckSelfDim(const aclTensor* self)
{
    if (self->GetViewShape().GetDimNum() != DIM_SUPPORT_ONLY) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Embedding_Renorm: Except 2-dimension Tensor but got [%s]-dimension Tensor.",
...
static bool CheckFormatValid(const aclTensor* self, const aclTensor* indices)
{
    ...
    if (self->GetStorageFormat() == op::Format::FORMAT_FRACTAL_NZ ||
        indices->GetStorageFormat() == op::Format::FORMAT_FRACTAL_NZ) {
        OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Input and output format only support [ND]. Actual: self:[%s], indices:[%s].",
...
aclnnStatus aclnnEmbeddingRenormGetWorkspaceSize(aclTensor* selfRef, const aclTensor* indices, double maxNorm,
                                                 double normType, uint64_t* workspaceSize, aclOpExecutor** executor)
{
