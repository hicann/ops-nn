...
/*!
 * \file batch_norm_ext2_tiling_welford_arch35.cpp
 * \brief
 */
...
namespace optiling {
class BatchNormExt2WelfordReduceTilingBase : public BatchNormExt2RegbaseTilingBase {
public:
    explicit BatchNormExt2WelfordReduceTilingBase(gert::TilingContext* context)
        : BatchNormExt2RegbaseTilingBase(context)
    {
        Reset();
    }
    ~BatchNormExt2WelfordReduceTilingBase() override = default;
...
protected:
    bool IsCapable() override
    {
        ...
        // 该场景 r1_ * r0_ 很小，BatchNormExt2FullReduceTilingBase（优先级 20000）的 UB 判据必然通过，
        // 会先行接管；其 kernel 侧用 CeilDiv + 掩码处理不足一个 VL 的归约，天然支持这一档。
        if (r1_ * r0_ <= vlFp32_) {
            OP_LOGI(context_->GetNodeName(),
                    "BatchNormExt2WelfordReduce not capable: r1(%ld) * r0(%ld) <= vlFp32(%ld).", r1_, r0_, vlFp32_);
            return false;
        }
        return true;
    }
...
private:
    // 不再声明本类私有的 opName：基类 BatchNormExt2TilingBase 已有同名 protected 成员，
    ...
    int64_t parallelN;
    BatchNormExt2WelfordRegbaseTilingData tilingData;
};

void BatchNormExt2WelfordReduceTilingBase::Reset()
{
    binaryAddQuotient = 0;
    parallelN = 0;
    return;
...
ge::graphStatus BatchNormExt2WelfordReduceTilingBase::DoOpTiling()
{
    // block tiling
    ...
    int64_t binaryAddBufNum = (totalUBSize / (DOUBLE_BUFFER * LARGE_BUFFER_NUM_QUEUE * elemSize)) / vlFp32_;
...
uint64_t BatchNormExt2WelfordReduceTilingBase::GetTilingKey() const { return TILINGKEY_WELFORD_REDUCE; }

ge::graphStatus BatchNormExt2WelfordReduceTilingBase::PostTiling()
{
...
REGISTER_OPS_TILING_TEMPLATE(BatchNormExt2, BatchNormExt2WelfordReduceTilingBase, 30000);
} // namespace optiling
