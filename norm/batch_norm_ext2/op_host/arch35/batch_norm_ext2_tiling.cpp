...
/*!
 * \file batch_norm_ext2_tiling.cpp
 * \brief
 */
...
static ge::graphStatus Tiling4BatchNormExt2(gert::TilingContext* context)
{
    return Ops::NN::Optiling::TilingRegistry::GetInstance().DoTilingImpl(context);
}

ge::graphStatus TilingPrepare4BatchNormExt2(gert::TilingParseContext* context)
{
    OP_LOGD(context->GetNodeName(), "TilingPrepare4BatchNormExt2 enter.");

    auto compileInfo = GetCompileInfoPtr<BatchNormExt2CompileInfo>(context);
    ...
    OP_LOGD(context->GetNodeName(),
            "TilingPrepare4BatchNormExt2 exit, coreNum: %lu, ubSize: %lu, blockSize: %lu, vectorLen: %u.",
            compileInfo->coreNum, compileInfo->ubSize, compileInfo->blockSize, compileInfo->vectorLength);
    return ge::GRAPH_SUCCESS;
}

IMPL_OP_OPTILING(BatchNormExt2)
    .Tiling(Tiling4BatchNormExt2)
    .TilingParse<BatchNormExt2CompileInfo>(TilingPrepare4BatchNormExt2);
