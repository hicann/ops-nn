...
/*!
 * \file batch_norm_ext2_tiling_infer_arch35.cpp
 * \brief
 */
#include <algorithm>
...
namespace optiling {
class BatchNormExt2InferTiling : public BatchNormExt2TilingInferBase {
public:
    explicit BatchNormExt2InferTiling(gert::TilingContext* context) : BatchNormExt2TilingInferBase(context) {}
    ~BatchNormExt2InferTiling() override = default;
...
private:
    const char* opName = "BatchNormExt2Infer";

    bool isSmallAB1Mode_{false};
    BatchNormExt2InferTilingData tilingData_;
};
...
ge::graphStatus BatchNormExt2InferTiling::DoOpTiling()
{
    aTileBase_ = vlFp16_;
    ...
    if (fusedALen_ > 0 && fusedB1Len_ > 0 && fusedALen_ * fusedB1Len_ <= SMALL_AB1_FACTOR_LIMIT &&
        ...
        b1Inner = std::max<int64_t>(1, b1Inner);
        ...
        b0Inner = std::max<int64_t>(1, b0Inner);
        b0Outer = CeilDiv(fusedB0Len_, b0Inner);
...
uint64_t BatchNormExt2InferTiling::GetTilingKey() const
{
    return isSmallAB1Mode_ ? TILINGKEY_INFER_SMALL_AB1 : TILINGKEY_INFER;
}

ge::graphStatus BatchNormExt2InferTiling::PostTiling()
{
...
REGISTER_OPS_TILING_TEMPLATE(BatchNormExt2, BatchNormExt2InferTiling, 91000);
} // namespace optiling
