...
/*!
 * \file batch_norm_ext2_tiling_infer_last_channel_arch35.cpp
 * \brief
 */
...
namespace optiling {
class BatchNormExt2InferLastChannelTiling : public BatchNormExt2TilingInferBase {
public:
    explicit BatchNormExt2InferLastChannelTiling(gert::TilingContext* context) : BatchNormExt2TilingInferBase(context)
    {}
    ~BatchNormExt2InferLastChannelTiling() override = default;

protected:
    bool IsCapable() override
    {
        if (fusedB1Len_ != 1) {
            OP_LOGD(context_, "BatchNormExt2 Infer channel_last template not support fused shape(%ld, %ld, %ld).",
...
private:
    ...
    const char* opName = "BatchNormExt2InferLastChannel";
    bool isSmallLastChannel_{false};
    bool isContinuousLastChannel_{false};
    BatchNormExt2InferLastChannelTilingData tilingData;
};

inline static int64_t AlignUp(int64_t value, int64_t base) { return (value + base - 1) / base * base; }

int64_t BatchNormExt2InferLastChannelTiling::GetBaseTilingAOuter() const
{
...
ge::graphStatus BatchNormExt2InferLastChannelTiling::FillLastChannelTilingForBSplit(int64_t paramBytes,
                                                                                    int64_t cacheBytes)
{
...
ge::graphStatus BatchNormExt2InferLastChannelTiling::DoOpTiling()
{
    aTileBase_ = vlFp16_;
    ...
    int64_t bFactorMax = ubBufferSize / aTileBase_;
    int64_t bInner = fusedB0Len_ <= bFactorMax ? fusedB0Len_ : bFactorMax;
    bInner = std::max<int64_t>(1, bInner);
    int64_t bOuter = Ops::Base::CeilDiv(fusedB0Len_, bInner);
    int64_t bTail = fusedB0Len_ % bInner;
    int64_t tileBlockBTail = bTail == 0 ? bInner : bTail;
...
uint64_t BatchNormExt2InferLastChannelTiling::GetTilingKey() const
{
    if (isSmallLastChannel_) {
...
ge::graphStatus BatchNormExt2InferLastChannelTiling::PostTiling()
{
...
REGISTER_OPS_TILING_TEMPLATE(BatchNormExt2, BatchNormExt2InferLastChannelTiling, 90000);
} // namespace optiling
