...
#include "batch_norm_ext2_tiling_def.h"
...
class batch_norm_ext2_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "batch_norm_ext2_test SetUp\n" << endl; }
    static void TearDownTestCase() { cout << "batch_norm_ext2_test TearDown\n" << endl; }
};
                    ...
                    int64_t tilingKey, uint32_t blockNum, uint8_t* tiling, bool is_training = true)
{
    ...
    ICPU_RUN_KF(batch_norm_ext2, realBlockNum, x, scale, offset, mean, variance, y, batch_mean, batch_variance,
                reserve_space_1, reserve_space_2, workspace, tiling);
...
TEST_F(batch_norm_ext2_test, test_full_reduce_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNormExt2FullReduceRegbaseTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNormExt2FullReduceRegbaseTilingData*
        tilingDatafromBin = reinterpret_cast<BatchNormExt2FullReduceRegbaseTilingData*>(tiling);
...
TEST_F(batch_norm_ext2_test, test_welford_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNormExt2WelfordRegbaseTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNormExt2WelfordRegbaseTilingData* tilingDatafromBin = reinterpret_cast<BatchNormExt2WelfordRegbaseTilingData*>(
        tiling);
...
TEST_F(batch_norm_ext2_test, test_infer_float32)
{
    ...
    uint32_t blockNum = 1;
    size_t tilingSize = sizeof(BatchNormExt2InferTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNormExt2InferTilingData* tilingDatafromBin = reinterpret_cast<BatchNormExt2InferTilingData*>(tiling);
