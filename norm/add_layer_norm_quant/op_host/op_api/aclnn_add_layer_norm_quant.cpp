...
static const std::initializer_list<op::DataType> REGBASE_DTYPE_SUPPORT_LIST_DYN = {op::DataType::DT_FLOAT16,
                                                                                   op::DataType::DT_BF16};
...
static bool CheckNotNull(const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta,
                         const char* quantMode, aclTensor* y1Out, aclTensor* y2Out, const aclTensor* xOut,
                         aclTensor* outScales1Out, aclTensor* outScales2Out)
{
...
static bool CheckDtypeValid(const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta,
                            ...
                            const aclTensor* xOut, const aclTensor* outScales1Out, const aclTensor* outScales2Out,
                            bool isDynQuant)
{
...
static bool CheckShapeRel(const aclTensor* x1, const aclTensor* gamma, const aclTensor* outScales1Out, bool isDynQuant)
{
    ...
    OP_CHECK((x1DimNums >= gammaDimNums), OP_LOGE(ACLNN_ERR_PARAM_INVALID, "DimNum of x1 should bigger than gamma"),
             return false);

    for (size_t i = 0; i < gammaDimNums; i++) {
        OP_CHECK((x1->GetViewShape().GetDim(i + x1DimNums - gammaDimNums) == gamma->GetViewShape().GetDim(i)),
                 OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Shape of gamma should be equal to few dims of shape x1"),
                 return false);
    }
...
static bool CheckOptCombValid(const aclTensor* scales1Optional, const aclTensor* scales2Optional,
                              ...
                              bool isDynQuant)
{
    ...
    OP_LOGI("Got aclnnAddLayerNormQuant opt input combination (%d,%d,%d,%d), isDyn=%d", s1Exist, s2Exist, z1Exist,
            z2Exist, isDynQuant);
    ...
    if (isDynQuant) {
        ...
        bool validStcQuantComb = (s1Exist && s2Exist && z1Exist && z2Exist) ||
                                 (s1Exist && s2Exist && (!z1Exist) && (!z2Exist)) ||
                                 (s1Exist && (!s2Exist) && z1Exist && (!z2Exist)) ||
                                 (s1Exist && (!s2Exist) && (!z1Exist) && (!z2Exist));
...
static bool CheckShapeEqs(const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta,
                          const aclTensor* biasOptional, const aclTensor* scales1Optional,
                          const aclTensor* scales2Optional, const aclTensor* zeroPoints1Optional,
                          const aclTensor* zeroPoints2Optional, const aclTensor* y1Out, const aclTensor* y2Out,
                          const aclTensor* xOut, const aclTensor* outScales1Out, const aclTensor* outScales2Out,
                          bool isDynQuant)
{
    ...
    OP_CHECK(weightShapeEqs, OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Tensors gamma/beta/s1/s2/z1/z2 shape not same."),
             return false);
...
static aclnnStatus CheckParams(const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma, const aclTensor* beta,
                               const aclTensor* biasOptional, const aclTensor* scales1Optional,
                               const aclTensor* scales2Optional, const aclTensor* zeroPoints1Optional,
                               const aclTensor* zeroPoints2Optional, aclTensor* y1Out, aclTensor* y2Out,
                               aclTensor* xOut, aclTensor* outScales1Out, aclTensor* outScales2Out,
                               const char* quantMode)
{
    ...
    // 1. 检查参数是否为空指针
    CHECK_RET(CheckNotNull(x1, x2, gamma, beta, quantMode, y1Out, y2Out, xOut, outScales1Out, outScales2Out),
    ...
    CHECK_RET(CheckDtypeValid(x1, x2, gamma, beta, biasOptional, scales1Optional, scales2Optional, zeroPoints1Optional,
    ...
    // 5. 查 所有shape 的等量关系
    CHECK_RET(CheckShapeEqs(x1, x2, gamma, beta, biasOptional, scales1Optional, scales2Optional, zeroPoints1Optional,
                            zeroPoints2Optional, y1Out, y2Out, xOut, outScales1Out, outScales2Out, isDynQuant),
              ACLNN_ERR_PARAM_INVALID);

    // 6. 查 可选输入组合
    CHECK_RET(CheckOptCombValid(scales1Optional, scales2Optional, zeroPoints1Optional, zeroPoints2Optional, isDynQuant),
              ACLNN_ERR_PARAM_INVALID);
    ...
    bool additionalOutput, bool divMode, aclTensor* outScales1Out, aclTensor* outScales2Out, aclOpExecutor* executor)
{
    ...
    auto addLayerNormQuantOuts = l0op::AddLayerNormQuant(x1, x2, gamma, beta, biasOptional, scales1Optional,
...
aclnnStatus ComputeAddLayerNormQuant(const aclTensor* x1, const aclTensor* x2, const aclTensor* gamma,
                                     ...
                                     aclTensor* y1Out, aclTensor* y2Out, aclTensor* xOut, aclTensor* outScales1Out,
    ...
    aclTensor* outScales2Out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    L2_DFX_PHASE_1(aclnnAddLayerNormQuant,
    ...
    // 参数检查
    auto ret = CheckParams(x1, x2, gamma, beta, biasOptional, scales1Optional, scales2Optional, zeroPoints1Optional,
    ...
    if (hasEmptyTensor) {
        ret = ComputeAddLayerNormQuantEmptyInput(x1Cont, x2Cont, gammaCont, betaCont, biasCont, s1Cont, s2Cont, z1Cont,
                                                 z2Cont, quantMode, epsilon, additionalOutput, divMode, outScales1Out,
                                                 outScales2Out, uniqueExecutor.get());
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    } else {
        ret = ComputeAddLayerNormQuant(x1Cont, x2Cont, gammaCont, betaCont, biasCont, s1Cont, s2Cont, z1Cont, z2Cont,
                                       quantMode, epsilon, additionalOutput, divMode, y1Out, y2Out, xOut, outScales1Out,
                                       outScales2Out, uniqueExecutor.get());
        CHECK_RET(ret == ACLNN_SUCCESS, ret);
    }
