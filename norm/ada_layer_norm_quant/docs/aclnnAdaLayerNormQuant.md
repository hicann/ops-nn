...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnAdaLayerNormQuantGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnAdaLayerNormQuant”接口执行计算。
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td><ul><li>不支持空Tensor。</li><li>shape为[B…, S]，其中B支持0到6个维度，维度数量和大小与`x`中的B保持一致，S与`x`中S维一致。</li></ul></td>
      <td>FLOAT32</td>
      <td>ND</td>
      <td>1-7</td>
      <td>√</td>
