...
ge::graphStatus MaskedSoftmaxWithRelPosBiasBaseTiling::GetAttenMaskTensorDims(const gert::StorageShape* attention_shape)
{
...
ge::graphStatus MaskedSoftmaxWithRelPosBiasBaseTiling::GetBiasTensorDims()
{
    ...
    if (bias_b_ == 0 || bias_w_ == 0 || bias_n_ == 0 || bias_s1_ == 0 || bias_s2_ == 0) {
        OP_LOGE(context_, "Do tiling failed,  bias_b_[%u],  bias_w_[%u], bias_n_[%u], bias_s1_[%u], bias_s2_[%u]",
                bias_b_, bias_w_, bias_n_, bias_s1_, bias_s2_);
        return ge::GRAPH_FAILED;
    }
    if (!(n_ == bias_n_ && s1_ == bias_s1_ && s2_ == bias_s2_)) {
        OP_LOGE(context_, "Do tiling failed, n_[%u], bias_n_[%u], s1_[%u], bias_s1_[%u], s2_[%u], bias_s2_[%u]", n_,
                bias_n_, s1_, bias_s1_, s2_, bias_s2_);
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus MaskedSoftmaxWithRelPosBiasBaseTiling::GetTensorDims()
{
    ...
    if (attenMaskTensor != nullptr) {
        ...
        if (x_shape->GetStorageShape().GetDimNum() == 4) { // 4 is for 4 dim
            if ((b_ / att_w_) == 0) {
                OP_LOGE(context_,
                        "Do tiling failed,  x shape %s and attenMask shape %s is not compatible, please check it.",
        ...
        if (!(w_ == att_w_ && s1_ == att_s1_ && s2_ == att_s2_)) {
            OP_LOGE(context_, "Do tiling failed, w_[%u], att_w_[%u], s1_[%u], att_s1_[%u], s2_[%u], att_s2_[%u]", w_,
                    att_w_, s1_, att_s1_, s2_, att_s2_);
            return ge::GRAPH_FAILED;
        }
    }
...
{
    if (b_ < aivNum) {
        OP_LOGD(context_, "Do B tiling failed, b should greater than aivNum, but b is %u, aivNum is %u.", b_, aivNum);
        return false;
    }
    // initBuf 需要32B对齐
    ...
    if (existAtten) {
        if (dataType != ge::DT_FLOAT) {
            ...
            bf16AttenAndBiasCastTempSize = static_cast<uint64_t>(w_) * n_ * s1_ * s2AlignedSize * BUFF_NUM * dtypeSize *
                                           BUFF_NUM;
            ...
            attenBiasAndTempSize = (static_cast<uint64_t>(BUFF_NUM) * w_ * n_ * s1_ * s2AlignedSize +
                                    static_cast<uint64_t>(w_) * s1_ * s2AlignedSize) *
                                   dtypeSize;
        }
    } else {
        if (dataType != ge::DT_FLOAT) {
    ...
    // 这个是针对每一行, 先使用min
    if (dataType != ge::DT_FLOAT) {
        softMaskMinTmpSize = AscendC::GetSoftMaxFlashV2MinTmpSize(softMaxSrcShape, dtypeSize * 2, dtypeSize * 2, false,
                                                                  false) *
                             w_ * n_ * // 2 is for bf16 and half
                             s1_;
    } else {
        softMaskMinTmpSize = AscendC::GetSoftMaxFlashV2MinTmpSize(softMaxSrcShape, dtypeSize, dtypeSize, false, false) *
                             w_ * n_ * s1_;
    }
    auto totalSize = xySize * 2 + attenBiasAndTempSize + softMaskMinTmpSize + bf16AttenAndBiasCastTempSize +
                     bf16XCastTempSize; // 2 is for double buffer
...
ge::graphStatus MaskedSoftmaxWithRelPosBiasBTiling::DoSoftMaxFlashV2Tiling(uint32_t loopTailNum, uint32_t stackNum,
                                                                           uint32_t loopNum)
{
    ...
    if (stackNum != 0) {
        ...
        if (dataType != ge::DT_FLOAT) {
            AscendC::SoftMaxFlashV2TilingFunc(srcShape, dtypeSize * 2, dtypeSize * 2,
                                              localWorkSpaceSize, // 2 if for bf16 and half
    ...
    OP_LOGD(context_, "MaskedSoftmaxWithRelPosBiasBTiling stackNum=%u, loopNum=%u, loopTailNum=%u.", stackNum, loopNum,
            loopTailNum);
...
ge::graphStatus MaskedSoftmaxWithRelPosBiasBTiling::DoOpTiling()
{
    ...
    uint32_t stackNum = (aicoreParams_.ubSize - static_cast<uint32_t>(attenBiasNetSize)) /
                        (xYUbSize * 2 + static_cast<uint32_t>(bf16XCastTempSize) +
                         softMaskMinTmpSize); // 2 is for double buffer
...
bool MaskedSoftmaxWithRelPosBiasBWTiling::IsCapable()
{
    if (static_cast<uint64_t>(b_) * w_ < aivNum) {
        OP_LOGD(context_,
                "Do BW tiling failed, b * w should greater than aivNum, "
                "but b * w is %u, aivNum is %u.",
                b_ * w_, aivNum);
        return false;
    }
    ...
    if (dataType != ge::DT_FLOAT) {
        totalSize += static_cast<uint64_t>(3) * n_ * s1_ * s2AlignedSize * dtypeSize *
                     BUFF_NUM; // 对于BF16和f16，需要多3个BUFF
    }

    ge::Shape softMaxSrcShape({n_ * s1_, s2AlignedSize});
    uint32_t softMaxMinTmpSize = 0;
    if (dataType == ge::DT_FLOAT16 || dataType == ge::DT_BF16) {
        softMaxMinTmpSize = AscendC::GetSoftMaxFlashV2MinTmpSize(softMaxSrcShape, dtypeSize * BUFF_NUM,
                                                                 dtypeSize * BUFF_NUM, false, false);
    } else if (dataType == ge::DT_FLOAT) {
        softMaxMinTmpSize = AscendC::GetSoftMaxFlashV2MinTmpSize(softMaxSrcShape, dtypeSize, dtypeSize, false, false);
    } else {
...
bool MaskedSoftmaxWithRelPosBiasS1BiasTiling::IsCapable()
{
    ...
    if (totalSize <= aicoreParams_.ubSize) {
        OP_LOGD(context_, "Do BWNS1 tiling success with no atten, totalSize is %lu, dtypeSize is %u.", totalSize,
                dtypeSize);
        return true;
    } else {
        OP_LOGE(
...
ge::graphStatus MaskedSoftmaxWithRelPosBiasS1BiasTiling::DoSoftMaxFlashV2Tiling(uint32_t loopTailNum, uint32_t stackNum,
                                                                                uint32_t loopNum)
{
    uint32_t localWorkSpaceSize = loopTailNum * softMaskMinTmpSize;
    if (stackNum != 0) {
    ...
    tilingData.set_tmpMaskUbSize(0);
    OP_LOGD(context_, "MaskedSoftmaxWithRelPosBiasS1BiasTiling stackNum=%u, loopNum=%u, loopTailNum=%u.", stackNum,
            loopNum, loopTailNum);
    if (stackNum == 0 && loopTailNum == 0) {
        OP_LOGD(context_, "stackNum and LoopTailNum both all are zero.");
        return ge::GRAPH_FAILED;
...
ge::graphStatus MaskedSoftmaxWithRelPosBiasS1BiasTiling::DoOpTiling()
{
    ...
    uint32_t stackNum = aicoreParams_.ubSize /
                        ((xYUbSize + static_cast<uint32_t>(biasSize)) * 2 + softMaskMinTmpSize +
                         static_cast<uint32_t>(bf16XCastTempSize + bf16ABiasCastTempSize + bf16YCastTempSize));
    uint32_t loopNum = 0;
    if (tailSize != 0) {
    ...
    if (stackNum != 0) {
        ...
        tilingData.set_castTempSize(
            stackNum * (static_cast<uint32_t>(bf16XCastTempSize + bf16ABiasCastTempSize + bf16YCastTempSize)));
    } else {
        // 如果是这种情况，就应该针对loopTailNum开启double buffer，而不是stackNum
        // 这种情况，应该也设置模板参数因为就不需要开启double buffer，或者针对tail
        // 开启double buffer
...
bool MaskedSoftmaxWithRelPosBiasBWNTiling::IsCapable()
{
    if (static_cast<uint64_t>(b_) * w_ * n_ < aivNum) {
        OP_LOGD(context_,
                "Do BWN tiling failed, b * w * n should greater than aivNum, "
                "but b * w * n is %u, aivNum is %u.",
                b_, aivNum);
        return false;
    }
    minUsedSize = 0;
    ...
    ge::Shape softMaxSrcShape({1, s2AlignedSize});
    if (dataType != ge::DT_FLOAT) {
        softMaskMinTmpSize = s1_ * AscendC::GetSoftMaxFlashV2MinTmpSize(softMaxSrcShape, dtypeSize * 2,
                                                                        dtypeSize * 2, // 2 is for bf16 and half
                                                                        false, false);
    } else {
        softMaskMinTmpSize = s1_ *
    ...
    if (minUsedSize <= aicoreParams_.ubSize) {
        OP_LOGD(context_, "Do BWN tiling success, minUsedSize is %lu, dtypeSize is %u.", minUsedSize, dtypeSize);
        return true;
    } else {
        OP_LOGD(context_, "Do BWN tiling failed, minUsedSize is %lu, dtypeSize is %u.", minUsedSize, dtypeSize);
        return false;
    }
}
...
void MaskedSoftmaxWithRelPosBiasBWNTiling::SetDate() { tilingData.SetDataPtr(context_->GetRawTilingData()->GetData()); }
...
bool MaskedSoftmaxWithRelPosBiasBWNS1Tiling::IsCapable()
{
    ...
    if (dataType != ge::DT_FLOAT) {
        softMaskMinTmpSize = AscendC::GetSoftMaxFlashV2MinTmpSize(softMaxSrcShape, dtypeSize * 2, dtypeSize * 2,
                                                                  false, // 2 is for bf16
                                                                  false);
    ...
    // 需要进一步考虑double buffer
    if (minUsedSize <= aicoreParams_.ubSize) {
        OP_LOGD(context_, "Do tiling BWNS1 success, minUsedSize is %lu, dtypeSize is %u.", minUsedSize, dtypeSize);
        return true;
    } else {
        OP_LOGE(
            context_,
...
ge::graphStatus MaskedSoftmaxWithRelPosBiasONES2Tiling::DoOpTiling()
{
    ...
    stackNum = std::min(
        {stackNum, static_cast<uint32_t>(singleCoreSize), 65535 / dtypeSize}); // 65535 is for datacopypad limit
    tilingData.set_stackNum(stackNum);
