...
static constexpr int ATTR_OUTPUT_MASK_IDX = 1;
static constexpr int ATTR_DST_TYPE_IDX = 2;

static constexpr int NUM_FOUR = 4;
static constexpr int OUTPUT_MASK_NULLPTR_LEN = 0;
    ...
    if (outputMaskLen != OUTPUT_MASK_NULLPTR_LEN) {
        ...
        *outScale1Shape = outScaleShape;
        OP_CHECK_IF(
            ...
            OP_LOGE(context, "When output_mask is NULL, AddRmsNormDynamicQuantV2 Not support only have scale2."),
            return GRAPH_FAILED);
        if (smooth2Exist) {
            *y2Shape = *x1Shape;
            *outScale2Shape = outScaleShape;
...
static graphStatus InferDataType4AddRmsNormDynamicQuantV2(gert::InferDataTypeContext* context)
{
    ...
    auto attrs = context->GetAttrs();
    OP_CHECK_NULL_WITH_CONTEXT(context, attrs);
    const int32_t* dstDtype = attrs->GetAttrPointer<int32_t>(ATTR_DST_TYPE_IDX);
    ge::DataType outDtype = ge::DT_INT8;
    if (dstDtype != nullptr) {
        outDtype = static_cast<ge::DataType>(*dstDtype);
    }
    context->SetOutputDataType(Y1_IDX, outDtype);
    context->SetOutputDataType(Y2_IDX, outDtype);
    ...
    OP_LOGD(context->GetNodeName(), "End to do InferDataType4AddRmsNormDynamicQuantV2");
    return GRAPH_SUCCESS;
}
