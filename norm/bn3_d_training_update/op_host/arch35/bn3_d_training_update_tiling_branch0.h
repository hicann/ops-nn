...
/*!
 * \file bn3_d_training_update_tiling_branch0.h
 * \brief
 */
#ifndef BN3_D_TRAINING_UPDATE_TILING_BRANCH0_H
#define BN3_D_TRAINING_UPDATE_TILING_BRANCH0_H

#include <cstdint>
#include <vector>

#include "norm/bn3_d_training_update/op_kernel/arch35/bn3_d_training_update_tiling_struct.h" // BN3DTrainingUpdateTilingData<4>
#include "bn3_d_training_update_tiling_common.h" // §5.3 common formulas + constants

namespace optiling {

// ============================================================================
// Bn3dBranch0Inputs — POD bundle of branch-0 (RANK=4) host-side inputs that
//   fully determine the §1 / §2 outputs. Derived from gert::TilingContext in
//   TilingFuncBN3DTrainingUpdate (Task 20), then handed to ComputeBranch0Tiling.
//
//   Fields:
//     input_shapes   — 7 raw input shapes (x + 6 stats); each is rank-4 for x,
//                       rank-1 (C,) for the 6 statistics.
//     output_shapes  — 5 raw output shapes (y + 4 stats); y follows x,
//                       the 4 stats outputs are (C,).
//     channel_axis   — {NCHW:1, NHWC:3} (DESIGN §3.1.2 table for rank4).
//     C              — channel count = sum.shape[0] (== x.shape[channel_axis]).
//     num            — reduce-domain element count = N·H·W = x.size / C.
//     dtype_id       — {f32:0, f16:1, bf16:2} (§5.2 table).
//     elem_bytes     — sizeof(T): {f32:4, f16:2, bf16:2}; feeds FindSplitAxis.
//     max_cores      — platform AIV count (e.g. 56 on ascend950).
// ============================================================================
struct Bn3dBranch0Inputs {
    std::vector<std::vector<int64_t>> input_shapes;  // 7 inputs (raw)
    std::vector<std::vector<int64_t>> output_shapes; // 5 outputs (raw)
    int32_t channel_axis = -1;                       // {NCHW:1, NHWC:3}
...
// ============================================================================
// §1 + §2 ComputeBranch0Tiling
//   Fills the RANK=4 BN3DTrainingUpdateTilingData POD from the host-side
//   inputs. The body walks the §2 formulas:
//     1. PadAndSqueezeBn3d → max_bro_shape[4] (channel-last) + normalised shapes
...
//     5. Scalar host-budget fields: channel_axis / C / num / dtype_id / rank=4 /
//        per_buf_bytes=(ub/P)&~31 = 52416 / num_rec=1/num / bessel_scaler /
//        factor / one_minus_factor / epsilon.
//
//   NOTE: BN3DTrainingUpdateTilingDataRank4 lives at GLOBAL scope (the kernel
...
//
//   STAGE EXPECTATION (Task 21 — UT written BEFORE impl):
...
//
//     Task 22 will replace this stub with the real §2 formulas.
// ============================================================================
