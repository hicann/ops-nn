...
/* GEIR (Graph Engine IR) example for BN3DTrainingUpdate operator */
...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape, inputFormat)                              \
    do {                                                                                                   \
        ...
        bn3_d_training_update_op.set_input_##inputName(placeholder##inputIndex);                           \
                     ...
                     std::vector<Operator>& outputs, Graph& graph, vector<unique_ptr<float[]>>& inputDataHolder)
{
    ...
    // 创建 BN3DTrainingUpdate 算子实例
    auto bn3_d_training_update_op = op::BN3DTrainingUpdate("bn3_d_training_update");

    // 设置属性：factor 和 epsilon
    bn3_d_training_update_op.set_attr_factor(0.1f);
    bn3_d_training_update_op.set_attr_epsilon(1.0e-5f);

    // 输入 x: 4D 张量 NCHW，通道数 C=3
    std::vector<int64_t> xShape = {2, 3, 4, 5};
    ADD_INPUT(1, x, inDtype, xShape, FORMAT_NCHW);

    // 逐通道统计量输入: shape = (C,)
    std::vector<int64_t> statShape = {3};
    ADD_INPUT(2, sum, DT_FLOAT, statShape, FORMAT_ND);
    ADD_INPUT(3, square_sum, DT_FLOAT, statShape, FORMAT_ND);
    ADD_INPUT(4, scale, DT_FLOAT, statShape, FORMAT_ND);
    ADD_INPUT(5, offset, DT_FLOAT, statShape, FORMAT_ND);
    ADD_INPUT(6, mean, DT_FLOAT, statShape, FORMAT_ND);
    ADD_INPUT(7, variance, DT_FLOAT, statShape, FORMAT_ND);

    outputs.push_back(bn3_d_training_update_op);
    return SUCCESS;
}
