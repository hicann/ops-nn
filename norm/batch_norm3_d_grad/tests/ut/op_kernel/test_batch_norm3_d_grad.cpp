...
/*!
 * \file test_batch_norm3d_grad.cpp
 * \brief
 */
...
#include "batch_norm3_d_grad_tiling_def.h"
...
class batch_norm3d_grad_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "batch_norm3d_grad_test SetUp\n" << endl; }
    static void TearDownTestCase() { cout << "batch_norm3d_grad_test TearDown\n" << endl; }
};
                    ...
                    int64_t tilingKey, uint32_t blockNum, uint8_t* tiling, bool is_training = true)
{
    ...
    ICPU_RUN_KF(batch_norm3_d_grad, realBlockNum, dy, x, scale, mean, var, reserve_space3, dx, dscale, dbias,
...
TEST_F(batch_norm3d_grad_test, test_split_r1_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNorm3DGradRARRecomputeTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNorm3DGradRARRecomputeTilingData* tilingDatafromBin = reinterpret_cast<BatchNorm3DGradRARRecomputeTilingData*>(
        tiling);
...
TEST_F(batch_norm3d_grad_test, test_full_load_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNorm3DGradRARFullLoadTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNorm3DGradRARFullLoadTilingData* tilingDatafromBin = reinterpret_cast<BatchNorm3DGradRARFullLoadTilingData*>(
        tiling);
...
TEST_F(batch_norm3d_grad_test, test_split_r0_float32)
{
...
TEST_F(batch_norm3d_grad_test, test_infer_channel_last_float32)
{
    ...
    size_t tilingSize = sizeof(BatchNorm3DGradInferChannelLastTilingData) + sizeof(float);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNorm3DGradInferChannelLastTilingData*
        tilingDatafromBin = reinterpret_cast<BatchNorm3DGradInferChannelLastTilingData*>(tiling);
...
TEST_F(batch_norm3d_grad_test, test_infer_splitR1_float32)
{
    ...
    uint32_t blockNum = 1;
    size_t tilingSize = sizeof(BatchNorm3DGradInferTilingData) + sizeof(float);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNorm3DGradInferTilingData* tilingDatafromBin = reinterpret_cast<BatchNorm3DGradInferTilingData*>(tiling);
...
TEST_F(batch_norm3d_grad_test, test_infer_splitR0_float32)
{
