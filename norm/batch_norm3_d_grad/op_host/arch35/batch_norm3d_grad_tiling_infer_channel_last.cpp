...
/* !
 * \file batch_norm3d_grad_tiling_infer_channel_last.cpp
 * \brief
 */
...
class BatchNorm3DGradInferChannelLastTiling : public BatchNorm3DGradInferBase {
public:
    explicit BatchNorm3DGradInferChannelLastTiling(gert::TilingContext* context) : BatchNorm3DGradInferBase(context) {}
    ~BatchNorm3DGradInferChannelLastTiling() override = default;
...
protected:
    bool IsCapable() override
    {
        OP_CHECK_IF(
            r0Dim != 1,
            OP_LOGD(context_,
                    "BatchNorm3DGradInferChannelLastTiling BA template is not capable, fused shape: (%ld, %ld, %ld)",
                    r1Dim, aDim, r0Dim),
            return false);

        CalcBasicInfo();

        OP_LOGD(context_, "BatchNorm3DGradInferChannelLastTiling BA template is capable, fused shape: (%ld, %ld, %ld)",
                r1Dim, aDim, r0Dim);
        return true;
    }
...
private:
    ...
    BatchNorm3DGradInferChannelLastTilingData tilingData_;
};

ge::graphStatus BatchNorm3DGradInferChannelLastTiling::DoOpTiling()
{
    DoOpTilingForStage0();
    DoOpTilingForStage1();
...
ge::graphStatus BatchNorm3DGradInferChannelLastTiling::DoOpTilingForStage0()
{
...
ge::graphStatus BatchNorm3DGradInferChannelLastTiling::DoOpTilingForStage1()
{
    // 二分
...
uint64_t BatchNorm3DGradInferChannelLastTiling::GetTilingKey() const { return TILINGKEY_INFER_CHANNEL_LAST_BASE; }

int64_t BatchNorm3DGradInferChannelLastTiling::GetCacheID(const int64_t idx)
{
    return __builtin_popcountll(idx ^ (idx + CONST_ONE)) - CONST_ONE;
}

ge::graphStatus BatchNorm3DGradInferChannelLastTiling::PostTiling()
{
...
REGISTER_OPS_TILING_TEMPLATE(BatchNorm3DGrad, BatchNorm3DGradInferChannelLastTiling, 90000);
} // namespace optiling
