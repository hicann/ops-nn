...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                            \
    vector<int64_t> placeholder##inputIndex##_shape = inputShape;                                                 \
    auto placeholder##inputIndex = op::Const("placeholder" + std::to_string(inputIndex));                         \
...
{
    Status ret = SUCCESS;
    auto batchNorm_1 = op::BatchNorm3DGrad("batchNorm_1");
    std::vector<int64_t> xShape = {2, 3, 4, 5, 6};
    std::vector<int64_t> channelShape = {3};
    ADD_INPUT(1, y_backprop, inDtype, xShape, FORMAT_NCDHW);
    ADD_INPUT(2, x, inDtype, xShape, FORMAT_NCDHW);
    ...
    ADD_OUTPUT(1, x_backprop, inDtype, xShape, FORMAT_NCDHW);
    ADD_OUTPUT(2, scale_backprop, DT_FLOAT, channelShape, FORMAT_ND);
    ADD_OUTPUT(3, offset_backprop, DT_FLOAT, channelShape, FORMAT_ND);
    ADD_OUTPUT(4, reserve_space_4, DT_FLOAT, channelShape, FORMAT_ND);
    ADD_OUTPUT(5, reserve_space_5, DT_FLOAT, channelShape, FORMAT_ND);
    batchNorm_1.set_attr_epsilon(1e-4f);
    batchNorm_1.set_attr_data_format("NCDHW");
