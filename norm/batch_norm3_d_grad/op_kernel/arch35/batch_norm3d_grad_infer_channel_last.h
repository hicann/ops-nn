...
/* !
 * \file batch_norm3d_grad_infer_channel_last.h
 * \brief
 */

#ifndef BATCH_NORM3D_GRAD_INFER_CHANNEL_LAST_H
#define BATCH_NORM3D_GRAD_INFER_CHANNEL_LAST_H
...
class BatchNorm3DGradInferChannelLastDx {
...
public:
    __aicore__ inline BatchNorm3DGradInferChannelLastDx(){};

    __aicore__ inline BatchNorm3DGradInferChannelLastDx(const BatchNorm3DGradInferChannelLastDxTilingData* tilingDataIn)
    {
...
private:
    const BatchNorm3DGradInferChannelLastDxTilingData* tilingData_;
...
class BatchNorm3DGradInferChannelLastRecompute {
    static constexpr int32_t BUFFER_DEPTH = 1;
...
public:
    __aicore__ inline BatchNorm3DGradInferChannelLastRecompute(){};

    __aicore__ inline BatchNorm3DGradInferChannelLastRecompute(
        TPipe* pipeIn, const BatchNorm3DGradInferChannelLastTilingData* tilingDataIn)
    {
        pipe_ = pipeIn;
...
private:
    const BatchNorm3DGradInferChannelLastTilingData* tilingData_;
    TPipe* pipe_;
...
template <typename T1, typename T2>
class BatchNorm3DGradInferChannelLast {
    static constexpr int32_t BUFFER_NUM = 2;
    static constexpr int32_t BUFFER_DEPTH = 1;
...
public:
    __aicore__ inline BatchNorm3DGradInferChannelLast(){};

    __aicore__ inline BatchNorm3DGradInferChannelLast(TPipe* pipeIn,
                                                      const BatchNorm3DGradInferChannelLastTilingData* tilingDataIn)
    {
        pipe_ = pipeIn;
        tilingData_ = tilingDataIn;
        enableDx = tilingData_->enableDx;
    ...
    {
        if (enableDx) {
            BatchNorm3DGradInferChannelLastDx<T1, T2> opDx(&tilingData_->dxTilingData);
        ...
        if (enableDgamma && enableDbeta) {
            BatchNorm3DGradInferChannelLastRecompute<T1, T2, BUFFER_NUM, true, true> opDgammaDbeta(pipe_, tilingData_);
            opDgammaDbeta.Process(dy, x, save_mean, runningVar, dgamma, dbeta, usrWorkspace);
        } else if (enableDgamma && !enableDbeta) {
            BatchNorm3DGradInferChannelLastRecompute<T1, T2, BUFFER_NUM, true, false> opDgammaDbeta(pipe_, tilingData_);
            opDgammaDbeta.Process(dy, x, save_mean, runningVar, dgamma, dbeta, usrWorkspace);
        } else if (!enableDgamma && enableDbeta) {
            BatchNorm3DGradInferChannelLastRecompute<T1, T2, BUFFER_NUM, false, true> opDgammaDbeta(pipe_, tilingData_);
            opDgammaDbeta.Process(dy, x, save_mean, runningVar, dgamma, dbeta, usrWorkspace);
        }
    }
