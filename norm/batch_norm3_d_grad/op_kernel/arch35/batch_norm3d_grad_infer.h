...
/* !
 * \file batch_norm3d_grad_infer.h
 * \brief
 */

#ifndef BATCH_NORM3D_GRAD_INFER_H
#define BATCH_NORM3D_GRAD_INFER_H

#include "kernel_tiling/kernel_tiling.h"
#include "../inc/platform.h"
#include "kernel_operator.h"
#include "../inc/kernel_utils.h"
#include "batch_norm3d_grad_common.h"

namespace BatchNorm3DGrad {
...
class BatchNorm3DGradInferDgammaDbetaSplitR0 {
public:
    __aicore__ inline BatchNorm3DGradInferDgammaDbetaSplitR0(TPipe* pipe) { pipe_ = pipe; }
    ...
    __aicore__ inline void Init(GM_ADDR dy, GM_ADDR x, GM_ADDR mean, GM_ADDR var, GM_ADDR gamma, GM_ADDR dx,
                                ...
                                const BatchNorm3DGradInferTilingData* tilingData)
    {
        ...
        isTailLoop2_ = ubRDimTailTail_ > 0 && ubRDimTailTailLoopNum_ == BatchNorm3DGrad::TWO;
        tailLoop_ = ubRDimTailTailLoopNum_ == BatchNorm3DGrad::TWO ? ubRDimTailLoopNum_ / BatchNorm3DGrad::TWO :
                                                                     ubRDimTailLoopNum_;
        ...
        int64_t dyBufSize = tailFactorAlign_ * BatchNorm3DGrad::TWO * sizeof(DY_TYPE);
        int64_t xBufSize = ubRDimTailFactorAlign_ * BatchNorm3DGrad::TWO * sizeof(float);
        ...
        pipe_->InitBuffer(meanInQue_, BUFFER_NUM, BatchNorm3DGrad::ONE_BLK_SIZE);
        pipe_->InitBuffer(varInQue_, BUFFER_NUM, BatchNorm3DGrad::ONE_BLK_SIZE);
    ...
    __aicore__ inline void CopyInXTwo(int64_t offset1, int64_t offset2, uint32_t tailLen)
    {
        ...
        if constexpr (IsSameType<DY_TYPE, float>::value) {
            ...
            CopyIn(xLocal, xGm_, xBufElemNum_ / BatchNorm3DGrad::TWO, offset1, ubRDimTailFactor_);
            CopyIn(xLocal, xGm_, xBufElemNum_ / BatchNorm3DGrad::TWO + tailFactorAlign_, offset2, tailLen);
        }
    ...
    __aicore__ inline void CopyInXOne(int64_t offset1, uint32_t copyLen)
    {
        ...
        if constexpr (IsSameType<DY_TYPE, float>::value) {
            ...
            CopyIn(xLocal, xGm_, xBufElemNum_ / BatchNorm3DGrad::TWO, offset1, copyLen);
        }
        xInQue_.EnQue(xLocal);
    }
                                     ...
                                     uint16_t binaryAddLoop, uint16_t binaryAddKLoop, uint32_t binaryAddLastNum)
    {
        ...
        for (uint16_t i = 0; i < binaryAddKLoop; i++) {
            curBinaryAddLoop = curBinaryAddLoop / BatchNorm3DGrad::TWO;
                                             ...
                                             uint32_t tailLen)
    {
        ...
        if constexpr (IsSameType<DY_TYPE, half>::value || IsSameType<DY_TYPE, bfloat16_t>::value) {
            xAddr += xBufElemNum_ / BatchNorm3DGrad::TWO;
        }
        int64_t binaryAddRemainder = ubRDimTailFactor_ - halfBinaryAddQuotient_;
        uint16_t binaryAddRemainderLoop = CeilDiv(binaryAddRemainder, VL_FP32);
        uint16_t binaryAddQuotientLoop = CeilDiv(halfBinaryAddQuotient_, VL_FP32);
                                          ...
                                          uint32_t level3Idx)
    {
        ...
        if (level3Idx > 0) {
            ReduceToLevel1(out, level3Idx, UB_ADD_LEN * BatchNorm3DGrad::TWO, reduce);
            reduce += 1;
        }
    ...
    __aicore__ inline void ReduceToNextBuf(uint32_t& level1Idx, uint32_t& level2Idx, uint32_t& level3Idx)
    {
        if (level2Idx >= UB_ADD_LEN) {
            if constexpr (EN_DBETA) {
                ReduceOut(dbeta_, UB_ADD_LEN, UB_ADD_LEN, UB_ADD_LEN * BatchNorm3DGrad::TWO + level3Idx);
            }
            if constexpr (EN_DGAMMA) {
                ReduceOut(dgamma_, UB_ADD_LEN, UB_ADD_LEN, UB_ADD_LEN * BatchNorm3DGrad::TWO + level3Idx);
            }
            level3Idx += 1;
            level2Idx = 0;
        }
...
template <typename DY_TYPE, typename WEIGHT_TYPE, int BUFFER_NUM = 1, bool EN_DGAMMA = true, bool EN_DBETA = true>
class BatchNorm3DGradInferDgammaDbetaSplitR1 {
public:
    __aicore__ inline BatchNorm3DGradInferDgammaDbetaSplitR1(TPipe* pipe) { pipe_ = pipe; }
...
public:
    __aicore__ inline BatchNorm3DGradInferDx(TPipe* pipeIn, const BatchNorm3DGradInferDxTilingData* tilingDataIn)
    {
...
private:
    const BatchNorm3DGradInferDxTilingData* tilingData_;
...
public:
    __aicore__ inline BatchNorm3DGradInfer(){};

    __aicore__ inline BatchNorm3DGradInfer(TPipe* pipeIn, const BatchNorm3DGradInferTilingData* tilingDataIn)
    {
        pipe_ = pipeIn;
        tilingData_ = tilingDataIn;
    ...
    {
        if (enableDx) {
            BatchNorm3DGradInferDx<T1, T2> opDx(pipe_, &tilingData_->baseTilingData);
        ...
        if constexpr (SPLIT_R0) { // split R0
            if (enableDgamma && enableDbeta) {
                BatchNorm3DGradInferDgammaDbetaSplitR0<T1, T2, BUFFER_NUM, true, true> opDgammaDbeta(pipe_);
                ...
                BatchNorm3DGradInferDgammaDbetaSplitR0<T1, T2, BUFFER_NUM, true, false> opDgammaDbeta(pipe_);
                opDgammaDbeta.Init(dy, x, save_mean, runningVar, gamma, dx, dgamma, dbeta, usrWorkspace, tilingData_);
                opDgammaDbeta.Process();
            } else if (!enableDgamma && enableDbeta) {
                BatchNorm3DGradInferDgammaDbetaSplitR0<T1, T2, BUFFER_NUM, false, true> opDgammaDbeta(pipe_);
                opDgammaDbeta.Init(dy, x, save_mean, runningVar, gamma, dx, dgamma, dbeta, usrWorkspace, tilingData_);
                opDgammaDbeta.Process();
            }
        } else {
            if (enableDgamma && enableDbeta) {
                BatchNorm3DGradInferDgammaDbetaSplitR1<T1, T2, BUFFER_NUM, true, true> opDgammaDbeta(pipe_);
                opDgammaDbeta.Init(dy, x, save_mean, runningVar, gamma, dx, dgamma, dbeta, usrWorkspace, tilingData_);
                opDgammaDbeta.Process();
            } else if (enableDgamma && !enableDbeta) {
                BatchNorm3DGradInferDgammaDbetaSplitR1<T1, T2, BUFFER_NUM, true, false> opDgammaDbeta(pipe_);
                opDgammaDbeta.Init(dy, x, save_mean, runningVar, gamma, dx, dgamma, dbeta, usrWorkspace, tilingData_);
                opDgammaDbeta.Process();
            } else if (!enableDgamma && enableDbeta) {
                BatchNorm3DGradInferDgammaDbetaSplitR1<T1, T2, BUFFER_NUM, false, true> opDgammaDbeta(pipe_);
                opDgammaDbeta.Init(dy, x, save_mean, runningVar, gamma, dx, dgamma, dbeta, usrWorkspace, tilingData_);
                opDgammaDbeta.Process();
            }
        }
...
private:
    const BatchNorm3DGradInferTilingData* tilingData_;
    TPipe* pipe_;
