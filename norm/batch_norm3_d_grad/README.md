...
BatchNorm3DGrad对BatchNorm3D的反向梯度进行计算，输出输入梯度、scale梯度、offset梯度以及辅助输出。
...
|reserve_space_2|输入|FLOAT32|ND|正向保存的标准差倒数或等价统计量。|
...
|reserve_space_5|输出|FLOAT32|ND|辅助输出。|
|epsilon|属性|FLOAT32|-|默认0.0001。|
|data_format|属性|STRING|-|默认NCDHW。|
...
| 调用方式 | 样例代码 | 说明 |
| --- | --- | --- |
| 图模式调用 | [batch_norm3d_grad_proto.h](op_graph/batch_norm3d_grad_proto.h) | 通过[算子IR](op_graph/batch_norm3d_grad_proto.h)构图方式调用BatchNorm3DGrad算子。 |
