                                                    ...
                                                    const int64_t colSize, const float epsilon)
{
    ...
    if (innerLoopTimes == 1) {
        __VEC_SCOPE__
        {
            ...
            uint32_t count;
            AscendC::Reg::RegTensor<float> aReg, bReg, cReg;
            AscendC::Reg::RegTensor<float> meanReg, varReg, rstdReg;
            AscendC::Reg::MaskReg pMask;
            count = static_cast<uint32_t>(colSize);
            pMask = AscendC::Reg::UpdateMask<float>(count);
            for (uint16_t i = 0; i < outerLoopTimes; ++i) {
...
template <typename T>
__aicore__ inline void LayerNormGradBase::StoreTensorForDtypeT(__ubuf__ T* dst, AscendC::Reg::RegTensor<float>& src,
                                                               AscendC::Reg::MaskReg& preg, uint32_t offset)
{
    if constexpr (IsSameType<T, float>::value) {
...
template <typename T>
__aicore__ inline void LayerNormGradBase::LoadTensorForDtypeT(const __ubuf__ T* src,
                                                              AscendC::Reg::RegTensor<float>& dst,
                                                              AscendC::Reg::MaskReg& preg, uint32_t offset)
{
    if constexpr (IsSameType<T, float>::value) {
        // float类型: 直接对齐加载, 无需cast
        LoadAlign<float, AscendC::Reg::LoadDist::DIST_NORM>(dst, (__ubuf__ float*)src + offset);
    } else {
        // 非float(bf16/half): 先按B16解包加载到T寄存器, 再Cast到fp32寄存器
...
template <typename T>
__aicore__ inline void LayerNormGradBase::CopyInTranspose(const LocalTensor<T>& dstTensor,
