...
/* !
 * \file layer_norm_grad_transpose_regbase_tiling.cpp
 * \brief
 */
...
namespace optiling {
constexpr static int64_t M_TILE_MIN_SIZE = 16;
constexpr static int64_t TRANSPOSE_COL_BYTES_LIMIT = 32;
constexpr static int64_t TRANSPOSE_M_THRESHOLD = 256;
constexpr static int64_t TRANSPOSE_ALIGN_LIMIT = 16;
constexpr static int64_t M_FACTOR_ALIGN_MAX = 2048;
...
bool LayerNormGradTransposeRegBaseTiling::IsCapable()
{
...
int64_t LayerNormGradTransposeRegBaseTiling::CalcGammaBetaMMax(int64_t mFactorAlign, int64_t mPerCore, int64_t nAlign,
                                                               int64_t ubSize)
{
...
ge::graphStatus LayerNormGradTransposeRegBaseTiling::GammaBetaKernelTiling()
{
    int64_t row = static_cast<int64_t>(commonParams.rowSize);
...
ge::graphStatus LayerNormGradTransposeRegBaseTiling::BackwardKernelTiling()
{
    int64_t row = static_cast<int64_t>(commonParams.rowSize);
    int64_t col = static_cast<int64_t>(commonParams.colSize);
    int64_t coreNum = static_cast<int64_t>(commonParams.coreNum);
    ...
    // UseUB = ((2*dy + 2*x + 2*dx + 1*xNorm)*MN + (2*mean + 2*var + 2*sum)*M + 1*gamma*N)*dtypeSize + UB_RESERVE_SIZE
...
ge::graphStatus LayerNormGradTransposeRegBaseTiling::DoOpTiling()
{
...
ge::graphStatus LayerNormGradTransposeRegBaseTiling::GetWorkspaceSize()
{
...
ge::graphStatus LayerNormGradTransposeRegBaseTiling::PostTiling()
{
...
uint64_t LayerNormGradTransposeRegBaseTiling::GetTilingKey() const
{
    constexpr uint64_t LNG_TRANSPOSE_REGBASE_TILINGKEY = 800;
    return LNG_TRANSPOSE_REGBASE_TILINGKEY;
}

REGISTER_TILING_TEMPLATE("LayerNormGrad", LayerNormGradTransposeRegBaseTiling, 500);
} // namespace optiling
