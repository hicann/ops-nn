...
<table style="undefined;table-layout: fixed; width: 1005px"><colgroup>
  ...
  <tbody>
    ...
    <tr>
      <td>rstd</td>
      <td>输出</td>
      <td><ul><li>表示归一化后的标准差的倒数，对应公式中Rms(x)的倒数。</li><li>当output_rstd为True时，shape与入参x1的shape前几维保持一致，前几维指x1的维度减去gamma的维度，表示不需要norm的维度。</li><li>当output_rstd为False时，rstd为无效占位输出。</li></ul></td>
      <td>FLOAT32</td>
      <td>ND</td>
    </tr>
  </tbody></table>
...
## Torch Extension FP4 输出打包机制

当通过 torch_extension（`cann_ops_nn`，接口名 `add_rms_norm_dynamic_quant`）调用算子且 `dst_type` 为 FP4（40=FLOAT4_E2M1 / 41=FLOAT4_E1M2）时，输出 `y` 的数据类型和形状与 ACLNN/GE 层不同，需要特别关注：
...
`torch_extension/cann_ops_nn/ops/norm/add_rms_norm_dynamic_quant/csrc/add_rms_norm_dynamic_quant.cpp` 中的处理：

1. **分配 uint8 tensor，shape 减半**：
   ```cpp
   auto y_shape = x1.sizes().vec();
   if (IsFp4DstType(dst_type)) {
       y_shape.back() /= 2;  // 两个 FP4 打包为一个 uint8
   }
   at::Tensor y = at::empty(y_shape, x1.options().dtype(at::kByte));  // uint8
   ```

2. **TensorWrapper 覆盖 aclDataType**：
   ```cpp
   TensorWrapper y_wrapper = {y, y_acltype};  // 告诉 ACLNN 按 FP4 写入
   ```
   `TensorWrapper` 让 ACLNN 在 `uint8` 内存上按 `ACL_FLOAT4_E2M1` 格式写入数据，实现"底层 FP4 存储、上层 uint8 表达"。
...
`mxscale` 的 dtype 在 ACLNN 层为 `DT_FLOAT8_E8M0`，PyTorch 同样没有原生 E8M0 dtype，因此 torch_extension 层统一用 `uint8` 表达，通过 `TensorWrapper` 覆盖。所有 dst_type 场景下 mxscale 的 torch dtype 均为 `uint8`。
...
| torch_extension | [test_torch_extension](tests/ut/torch_extension/test_torch_extension.py) | 通过`cann_ops_nn` whl 包方式调用（接口名 `add_rms_norm_dynamic_quant`，代码位于 `torch_extension/cann_ops_nn/ops/norm/`），支持单算子模式和图模式。 |
