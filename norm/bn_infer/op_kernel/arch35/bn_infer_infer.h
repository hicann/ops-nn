...
/* !
 * \file bn_infer_infer.h
 * \brief
 */

#ifndef BN_INFER_INFER_H
#define BN_INFER_INFER_H
...
public:
    __aicore__ inline BNInfer(){};

    __aicore__ inline BNInfer(const BNInferTilingData* tilingDataIn) { tilingData_ = tilingDataIn; }
            ...
            for (uint16_t i = 0; i < curTileALen; i++) {
                ...
                for (uint16_t j = 0; j < curTileB0Len; j++) {
                    for (uint16_t k = 0; k < loopNum; k++) {
                        ...
                        uint32_t activeLen = curTileB1Len - k * VL_FP32 > VL_FP32 ? VL_FP32 :
                                                                                    curTileB1Len - k * VL_FP32;
                        MaskReg dataMask = AscendC::MicroAPI::UpdateMask<float>(activeLen);

                        // load x
                        LoadTensorForDtypeT(xLocal, x, dataMask, xOffset);

                        // compute
                        Sub(x, x, mean, dataMask);
                        Mul(x, x, gamma, dataMask);
                        Mul(x, x, rstd, dataMask);
                        Add(y, x, beta, dataMask);

                        // copy out
                        if constexpr (IsSameType<T, float>::value) {
                            StoreAlign(((__ubuf__ float*)yLocal) + xOffset, y, dataMask);
                        } else { // fp16、bf16
                            RegTensor<T> xFp16;
                            Cast<T, float, NormCommon::castTraitB322B16>(xFp16, y, dataMask);
                            StoreAlign<T, StoreDist::DIST_PACK_B32>(((__ubuf__ T*)yLocal) + xOffset, xFp16, dataMask);
                        }
                    }
                }
            }
