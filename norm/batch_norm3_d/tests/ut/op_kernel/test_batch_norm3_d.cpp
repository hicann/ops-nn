...
#include "batch_norm3_d_tiling_def.h"
...
class batch_norm3d_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "batch_norm3d_test SetUp\n" << endl; }
    static void TearDownTestCase() { cout << "batch_norm3d_test TearDown\n" << endl; }
};
                    ...
                    int64_t tilingKey, uint32_t blockNum, uint8_t* tiling, bool is_training = true)
{
    ...
    ICPU_RUN_KF(batch_norm3_d, realBlockNum, x, scale, offset, mean, variance, y, batch_mean, batch_variance,
...
TEST_F(batch_norm3d_test, test_full_reduce_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNorm3DFullReduceRegbaseTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNorm3DFullReduceRegbaseTilingData*
        tilingDatafromBin = reinterpret_cast<BatchNorm3DFullReduceRegbaseTilingData*>(tiling);
...
TEST_F(batch_norm3d_test, test_welford_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNorm3DWelfordRegbaseTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNorm3DWelfordRegbaseTilingData* tilingDatafromBin = reinterpret_cast<BatchNorm3DWelfordRegbaseTilingData*>(
        tiling);
...
TEST_F(batch_norm3d_test, test_infer_float32)
{
    ...
    uint32_t blockNum = 1;
    size_t tilingSize = sizeof(BatchNorm3DInferTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNorm3DInferTilingData* tilingDatafromBin = reinterpret_cast<BatchNorm3DInferTilingData*>(tiling);
