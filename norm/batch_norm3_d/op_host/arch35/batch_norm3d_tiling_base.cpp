...
/*!
 * \file batch_norm3d_tiling_base.cpp
 * \brief
 */
...
static const char* outputParamNames[] = {"y", "batch_mean", "batch_variance", "reserve_space_1", "reserve_space_2"};
...
string BNTensorDesc2String3D(const gert::StorageShape* shape, const gert::CompileTimeTensorDesc* tensor)
{
    ...
    descStream << "(dtype: " << ge::TypeUtils::DataTypeToSerialString(tensor->GetDataType()) << "),";
    descStream << "(shape:" << Shape2String(shape->GetStorageShape()) << "),";
    descStream << "(ori_shape:" << Shape2String(shape->GetOriginShape()) << "),";
    descStream << "(format: "
               << ge::TypeUtils::FormatToSerialString(
                      static_cast<ge::Format>(ge::GetPrimaryFormat(tensor->GetStorageFormat())))
               << "),";
    descStream << "(ori_format: " << ge::TypeUtils::FormatToSerialString(tensor->GetOriginFormat()) << ") ";

    return descStream.str();
}

string BNDebugTilingContext3D(gert::TilingContext* context)
{
    std::ostringstream contextStream;
    for (size_t inputIdx = 0; inputIdx < context->GetComputeNodeInfo()->GetInputsNum(); ++inputIdx) {
        contextStream << "input" << inputIdx << ": ";
        contextStream << BNTensorDesc2String3D(context->GetInputShape(inputIdx), context->GetInputDesc(inputIdx));
    }

    for (size_t outputIdx = 0; outputIdx < context->GetComputeNodeInfo()->GetOutputsNum(); ++outputIdx) {
        contextStream << "output" << outputIdx << ": ";
        contextStream << BNTensorDesc2String3D(context->GetOutputShape(outputIdx), context->GetOutputDesc(outputIdx));
    }
    return contextStream.str();
}

ge::graphStatus BatchNorm3DTilingBase::GetAttrsAndCheckValid()
{
    if (context_ == nullptr) {
        OP_LOGE_WITHOUT_REPORT("BatchNorm3DTilingBase", "TilingContext is nullptr.");
        return ge::GRAPH_FAILED;
    }

    OP_LOGD(opName, "TilingContext: %s", optiling::BNDebugTilingContext3D(context_).c_str());
...
ge::graphStatus BatchNorm3DTilingBase::GetXYShapesAndCheckValid()
{
    ...
    if (!validShapeSize) {
        ...
        std::string reason = "The shape sizes of x and y should be the same. NHWC requires 4, NCDHW and NDHWC require "
                             "5, NCHW and ND "
                             "require at least 2";
...
ge::graphStatus BatchNorm3DTilingBase::CheckSmallShapesValid(int64_t aDimLen)
{
    // input scale, offset
    ...
    for (int64_t i = CONST_THREE; i < CONST_FIVE; i++) {
        ...
        OP_CHECK_IF(desc == nullptr && !isTraining_,
                    OP_LOGE_WITHOUT_REPORT(context_->GetNodeName(), "Input %ld is required in inference mode.", i),
                    return ge::GRAPH_FAILED);
        if (!desc) {
...
ge::graphStatus BatchNorm3DTilingBase::GetDtypesAndCheckValid()
{
    // x,y
    auto xDesc = context_->GetInputDesc(0);
    OP_CHECK_NULL_WITH_CONTEXT(context_, xDesc);
    ...
    // reserve_space_1 and reserve_space_2 are optional in canndev, but materialized in this graph path.
    for (int64_t i = CONST_ONE; i < CONST_FIVE; i++) {
...
ge::graphStatus BatchNorm3DTilingBase::GetPlatformInfo()
{
    auto compileInfo = reinterpret_cast<const BatchNorm3DCompileInfo*>(context_->GetCompileInfo());
...
// infer
ge::graphStatus BatchNorm3DTilingInferBase::GetShapeAttrsInfo()
{
    OP_CHECK_IF(GetAttrsAndCheckValid() != ge::GRAPH_SUCCESS,
                OP_LOGE_WITHOUT_REPORT(context_->GetNodeName(), "Get attrs failed"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(CheckSmallShapesValid(fusedALen_) != ge::GRAPH_SUCCESS,
                OP_LOGE_WITHOUT_REPORT(opName, "CheckSmallShapesValid failed"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(GetDtypesAndCheckValid() != ge::GRAPH_SUCCESS,
                OP_LOGE_WITHOUT_REPORT(opName, "GetDtypesAndCheckValid failed"), return ge::GRAPH_FAILED);
...
// training
ge::graphStatus BatchNorm3DRegbaseTilingBase::GetShapeAttrsInfo()
{
    OP_CHECK_IF(GetAttrsAndCheckValid() != ge::GRAPH_SUCCESS,
                OP_LOGE_WITHOUT_REPORT(context_->GetNodeName(), "Get attrs failed"), return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(CheckSmallShapesValid(a_) != ge::GRAPH_SUCCESS,
                OP_LOGE_WITHOUT_REPORT(opName, "CheckSmallShapesValid failed"), return ge::GRAPH_FAILED);
    OP_CHECK_IF(GetDtypesAndCheckValid() != ge::GRAPH_SUCCESS,
                OP_LOGE_WITHOUT_REPORT(opName, "GetDtypesAndCheckValid failed"), return ge::GRAPH_FAILED);
