...
/*!
 * \file batch_norm3d_tiling_rar_block_split_r_arch35.cpp
 * \brief
 */
...
namespace optiling {
class BatchNorm3DRARBlockSplitRTiling : public BatchNorm3DRegbaseTilingBase {
public:
    explicit BatchNorm3DRARBlockSplitRTiling(gert::TilingContext* context) : BatchNorm3DRegbaseTilingBase(context) {}
    ~BatchNorm3DRARBlockSplitRTiling() override = default;
...
protected:
    bool IsCapable() override
    {
        ...
        OP_LOGI(
            ...
            "BatchNorm3DRARBlockSplitRTiling IsCapable true! ready to use BatchNorm3DRARBlockSplitRTiling (tilingkey: "
            "%lu) !",
...
private:
    BatchNorm3DRARBlockSplitRTilingData batchNormTilingData;
};

void BatchNorm3DRARBlockSplitRTiling::SetInputInfo()
{
    // dim
    batchNormTilingData.patternR1 = r1_;
    batchNormTilingData.patternA = a_;
    batchNormTilingData.patternR0 = r0_;

    // attr
    batchNormTilingData.epsilon = epsilon_;
    batchNormTilingData.momentum = exponentialAvgFactor_;
    batchNormTilingData.momentumReverse = 1 - exponentialAvgFactor_;
}

bool BatchNorm3DRARBlockSplitRTiling::BinaryAddTiling(const int64_t binaryAddNum, int64_t& binaryAddK,
...
int64_t BatchNorm3DRARBlockSplitRTiling::UbSplit(int64_t r1BlockInner, int64_t r0BlockInner, int64_t ubFactor,
...
ge::graphStatus BatchNorm3DRARBlockSplitRTiling::DoOpTiling()
{
    SetInputInfo();
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: blockSplitAxis is %lu !",
            blockSplitAxis);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: formerBlockOuter is %lu !",
            formerBlockOuter);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tailBlockOuter is %lu !",
            tailBlockOuter);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: blockInner is %lu !", blockInner);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: usedCoreNums_ is %lu !",
            usedCoreNums_);
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: class member blockSize_ is %lu !",
            blockSize_);
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: runningMeanVarSize is %lu !",
            runningMeanVarSize);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: saveMeanRstdSize is %lu !",
            saveMeanRstdSize);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: betaGammaSize is %lu !",
            betaGammaSize);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: blockDimAlignSize is %lu !",
            blockDimAlignSize);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tmpbuffer3AlignSize is %lu !",
            tmpbuffer3AlignSize);
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: xSizePerUbFactor is %lu !",
            xSizePerUbFactor);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: ySizePerUbFactor is %lu !",
            ySizePerUbFactor);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tmpMeanVarPerUbFactor is %lu !",
            tmpMeanVarPerUbFactor);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: countBuffer1PerUbFactor is %lu !",
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: ubSizeCanUse is %lu !", ubSizeCanUse);
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: direct obtained ubFactor is %lu !",
            ubFactor);
    ...
    batchNormTilingData.ubFactor = ubFactor;
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: ubFactor is %lu !\n", ubFactor);
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: formerCoreUbSplitAxis is %lu !",
            formerCoreUbSplitAxis);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: formerCoreUbOuter is %lu !",
            formerCoreUbOuter);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: formerCoreUbInner is %lu !\n",
            formerCoreUbInner);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tailCoreUbSplitAxis is %lu !",
            tailCoreUbSplitAxis);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tailCoreUbOuter is %lu !",
            tailCoreUbOuter);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tailCoreUbInner is %lu !\n\n",
            tailCoreUbInner);

    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: formerCoreProcessRNum is %lu !",
            formerCoreProcessRNum);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tailCoreProcessRNum is %lu !\n",
            tailCoreProcessRNum);
    ...
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: formerCoreBinaryAddQuotient is %lu !",
            formerCoreBinaryAddQuotient);
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling: tailCoreBinaryAddQuotient is %lu !",
    ...
    OP_CHECK_IF(res == false,
                OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling BinaryAddTiling param invalid"),
                return ge::GRAPH_PARAM_INVALID);

    batchNormTilingData.patternAAlign = patternAAlign;
    batchNormTilingData.blockSplitAxis = blockSplitAxis;
    batchNormTilingData.formerBlockOuter = formerBlockOuter;
    batchNormTilingData.tailBlockOuter = tailBlockOuter;
    batchNormTilingData.blockInner = blockInner;
    batchNormTilingData.formerCoreUbSplitAxis = formerCoreUbSplitAxis;
    batchNormTilingData.formerCoreUbOuter = formerCoreUbOuter;
    batchNormTilingData.formerCoreUbInner = formerCoreUbInner;
    batchNormTilingData.tailCoreUbSplitAxis = tailCoreUbSplitAxis;
    batchNormTilingData.tailCoreUbOuter = tailCoreUbOuter;
    batchNormTilingData.tailCoreUbInner = tailCoreUbInner;
    batchNormTilingData.formerCoreBinaryAddQuotient = formerCoreBinaryAddQuotient;
    batchNormTilingData.tailCoreBinaryAddQuotient = tailCoreBinaryAddQuotient;
    batchNormTilingData.lastBinaryAddQuotient = lastBinaryAddQuotient;
    batchNormTilingData.lastBinaryAddK = lastBinaryAddK;
    batchNormTilingData.lastBinaryAddLast = lastBinaryAddLast;
    batchNormTilingData.useRunningMeanVar = useRunningMeanVar_;

    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling DoOpTiling SUCCESS!");
    return ge::GRAPH_SUCCESS;
}

uint64_t BatchNorm3DRARBlockSplitRTiling::GetTilingKey() const
{
    OP_LOGI(context_->GetNodeName(), "BatchNorm3DRARBlockSplitRTiling GetTilingKey tilingkey is %lu !",
            TILINGKEY_RAR_BLOCK_SPLIT_R);
    return TILINGKEY_RAR_BLOCK_SPLIT_R;
}

ge::graphStatus BatchNorm3DRARBlockSplitRTiling::PostTiling()
{
    context_->SetBlockDim(usedCoreNums_);
    size_t* currentWorkspace = context_->GetWorkspaceSizes(1);
    OP_CHECK_NULL_WITH_CONTEXT(context_, currentWorkspace);
    currentWorkspace[0] = WSP_RESERVED_SIZE +
                          usedCoreNums_ * MEAN_AND_VAR_NODE_NUM * batchNormTilingData.patternAAlign * FP32_BYTE;
    auto* tilingDataOut = context_->GetTilingData<BatchNorm3DRARBlockSplitRTilingData>();
...
REGISTER_TILING_TEMPLATE("BatchNorm3D", BatchNorm3DRARBlockSplitRTiling, 25000);
} // namespace optiling
