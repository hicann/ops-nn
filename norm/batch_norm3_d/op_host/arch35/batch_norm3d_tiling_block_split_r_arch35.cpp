...
/*!
 * \file batch_norm3d_tiling_block_split_r_arch35.cpp
 * \brief
 */
...
namespace optiling {
class BatchNorm3DBlockSplitRTiling : public BatchNorm3DRegbaseTilingBase {
public:
    explicit BatchNorm3DBlockSplitRTiling(gert::TilingContext* context) : BatchNorm3DRegbaseTilingBase(context) {}
    ~BatchNorm3DBlockSplitRTiling() override = default;
...
private:
    BatchNorm3DBlockSplitRTilingData batchNormTilingData;
};

void BatchNorm3DBlockSplitRTiling::SetInputInfo()
{
    // dim
    batchNormTilingData.patternR = r1_;
...
bool BatchNorm3DBlockSplitRTiling::BinaryAddTiling(const int64_t binaryAddNum, int64_t& binaryAddK,
...
ge::graphStatus BatchNorm3DBlockSplitRTiling::DoOpTiling()
{
    SetInputInfo();
    ...
    batchNormTilingData.aUbFactor = aUbFactor;
    batchNormTilingData.aUbLoop = aUbLoop;
    batchNormTilingData.aUbTail = aUbTail;
    ...
    rUbFactor = std::min(rUbFactor, batchNormTilingData.patternR);
    rUbFactor = Ops::Base::FloorAlign(rUbFactor, RA_BINARY_ADD_THRESHOLD);
    OP_CHECK_IF(rUbFactor == 0, OP_LOGI(context_->GetNodeName(), "BatchNorm3DBlockSplitRTiling rUbFactor == 0"),
                return ge::GRAPH_PARAM_INVALID);
    int64_t rGroups = Ops::Base::FloorDiv(batchNormTilingData.patternR, rUbFactor);
    ...
    while (((rUbFactor * inOutNodeNum + tBufUbFactor * TBUF_NODE_NUM) * aAlignSize > ubSizeCanUse)) {
        ...
        rUbFactor = std::min(rUbFactor, batchNormTilingData.patternR);
        rUbFactor = Ops::Base::FloorAlign(rUbFactor, RA_BINARY_ADD_THRESHOLD);
        rGroups = Ops::Base::FloorDiv(batchNormTilingData.patternR, rUbFactor);
        usedCoreNums_ = std::min(rGroups, static_cast<int64_t>(aicoreParams_.blockDim));
    ...
    int64_t tailR = batchNormTilingData.patternR - rGroups * rUbFactor;
    ...
    OP_CHECK_IF(res0 == false || res1 == false,
                OP_LOGI(context_->GetNodeName(), "BatchNorm3DBlockSplitRTiling BinaryAddTiling param invalid"),
                return ge::GRAPH_PARAM_INVALID);
    batchNormTilingData.tBufUbFactor = tBufUbFactor;
    batchNormTilingData.rUbFactor = rUbFactor;
    batchNormTilingData.formerCoreBlockFactor = formerCoreBlockFactor;
    batchNormTilingData.tailCoreBlockFactor = tailCoreBlockFactor;
    batchNormTilingData.formerCoreNums = formerCoreNums;
    batchNormTilingData.tailCoreNums = tailCoreNums;
    batchNormTilingData.tailR = tailR;
    batchNormTilingData.binaryAddQuotient = binaryAddQuotient;
    batchNormTilingData.binaryAddK = binaryAddK;
    batchNormTilingData.binaryAddLast = binaryAddLast;
...
uint64_t BatchNorm3DBlockSplitRTiling::GetTilingKey() const { return TILINGKEY_BLOCK_SPLIT_R; }

ge::graphStatus BatchNorm3DBlockSplitRTiling::PostTiling()
{
    context_->SetBlockDim(usedCoreNums_);
    size_t* currentWorkspace = context_->GetWorkspaceSizes(1);
    OP_CHECK_NULL_WITH_CONTEXT(context_, currentWorkspace);
    currentWorkspace[0] = WSP_RESERVED_SIZE +
                          usedCoreNums_ * MEAN_AND_VAR_NODE_NUM * batchNormTilingData.patternAAlign * FLOAT32_BYTES;
    auto* tilingDataOut = context_->GetTilingData<BatchNorm3DBlockSplitRTilingData>();
...
REGISTER_OPS_TILING_TEMPLATE(BatchNorm3D, BatchNorm3DBlockSplitRTiling, 12000);
} // namespace optiling
