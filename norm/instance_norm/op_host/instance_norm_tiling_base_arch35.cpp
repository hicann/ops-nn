...
ge::graphStatus InstanceNormRegbaseTilingBase::GetShapeAttrsInfo()
{
    ...
    if (format == FORMAT_NCHW) {
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context_->GetNodeName(), "x", std::to_string(xDimNum).c_str(),
                                                     "The shape dim of input x must be 4 when the format of x is NHWC"),
            return ge::GRAPH_FAILED);
        a1 = xStorageShape.GetDim(DIM_0);
        r = xStorageShape.GetDim(DIM_1) * xStorageShape.GetDim(DIM_2);
        a0 = xStorageShape.GetDim(DIM_3);
    } else if (format == FORMAT_NDHWC) {
        OP_CHECK_IF(xDimNum != NDHWC_DIM_NUM,
                    OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                        context_->GetNodeName(), "x", std::to_string(xDimNum).c_str(),
                        "The shape dim of input x must be 5 when the format of x is NDHWC"),
                    return ge::GRAPH_FAILED);
        a1 = xStorageShape.GetDim(DIM_0);
    ...
    if (a1 <= 0) {
        std::string reasonMsg = "The N-dimension of input x must be a positive number, "
    ...
    if (a0 <= 0) {
        std::string reasonMsg = "The C-dimension of input x must be a positive number, "
                                "where C is the 1st dim when the format of x is NCHW, NCDHW or ND, "
                                "or the last dim when the format of x is NHWC or NDHWC";
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "x", ToString(xStorageShape).c_str(),
                                              reasonMsg.c_str());
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus InstanceNormRegbaseTilingBase::CheckDtypeValid()
{
    ...
    if (bettaDataType != gammaDataType) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "beta and gamma", dtypeMsg.c_str(),
                                               "The dtypes of input beta and input gamma must be the same");
        return ge::GRAPH_FAILED;
    }
    ...
    if (yDataType != dataType) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "y and x", dtypeMsg.c_str(),
                                               "The dtypes of output y and input x must be the same");
        return ge::GRAPH_FAILED;
    }
    ...
    if ((meanDataType != dataType) && (meanDataType != ge::DT_FLOAT)) {
        ...
        std::string reasonMsg = "The dtype of output mean must be FLOAT or the same as the dtype {" +
                                ToString(dataType) + "} of input x";
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(context_->GetNodeName(), "mean", dtypeMsg.c_str(), reasonMsg.c_str());
        return ge::GRAPH_FAILED;
    }
    ...
    if (varDataType != meanDataType) {
        ...
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(context_->GetNodeName(), "variance and mean", dtypeMsg.c_str(),
                                               "The dtypes of output variance and output mean must be the same");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus InstanceNormRegbaseTilingBase::CheckGammaBettaShapeValid()
{
    ...
    const std::vector<std::pair<int, std::string>> gammaBetta = {{INPUT_GAMMA_INDEX, "gamma"},
                                                                 {INPUT_BETA_INDEX, "beta"}};
    for (const auto& [inputIdx, inputName] : gammaBetta) {
        ...
        if (gammaBettaStorageShape.GetDim(DIM_0) != a0) {
            ...
            std::string reasonMsg = "The 0th axis of input " + inputName +
                                    " must be equal to the C-dimension of input x, "
                                    "where C is the 1st dim when the format of x is NCHW, NCDHW or ND, "
                                    "or the last dim when the format of x is NHWC or NDHWC";
...
ge::graphStatus InstanceNormRegbaseTilingBase::CheckMeanVarianceShapeValid()
{
    ...
    const std::vector<std::pair<int, std::string>> meanVariance = {{OUTPUT_MEAN_INDEX, "mean"},
                                                                   {OUTPUT_VARIANCE_INDEX, "variance"}};
    int64_t xDimNum = xStorageShape.GetDimNum();
    for (const auto& [outputIdx, outputName] : meanVariance) {
        ...
        if (meanVarianceDimNum != xDimNum) {
            ...
            std::string reasonMsg = "The shape dims of output " + outputName + " and input x must be the same";
        ...
        for (int64_t j = 0; j < meanVarianceDimNum; j++) {
            if (j == 0) {
                ...
                if (meanVarianceStorageShape.GetDim(j) != xStorageShape.GetDim(j)) {
                    std::string reasonMsg = "The N-dimension of output " + outputName +
                                            " must be equal to the N-dimension of input x, where N is the 0th dim";
                    OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context_->GetNodeName(), paramMsg.c_str(), shapeMsg.c_str(),
                                                           reasonMsg.c_str());
                    return ge::GRAPH_FAILED;
                }
                ...
                if (meanVarianceStorageShape.GetDim(j) != xStorageShape.GetDim(j)) {
                    std::string reasonMsg = "The C-dimension of output " + outputName +
                                            " must be equal to the C-dimension of input x, "
                                            "where C is the 1st dim when the format of x is NCHW, NCDHW or ND";
                    OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context_->GetNodeName(), paramMsg.c_str(), shapeMsg.c_str(),
                                                           reasonMsg.c_str());
                ...
                if (meanVarianceStorageShape.GetDim(j) != xStorageShape.GetDim(j)) {
                    std::string reasonMsg = "The C-dimension of output " + outputName +
                                            " must be equal to the C-dimension of input x, "
                                            "where C is the last dim when the format of x is NHWC or NDHWC";
                    OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(context_->GetNodeName(), paramMsg.c_str(), shapeMsg.c_str(),
                                                           reasonMsg.c_str());
                    return ge::GRAPH_FAILED;
                }
                ...
                if (meanVarianceStorageShape.GetDim(j) != 1) {
                    std::string reasonMsg = "All axes of output " + outputName +
                                            " except the N-dimension and C-dimension must be 1, "
                                            "where N is the 0th dim, "
                                            "C is the 1st dim when the format of x is NCHW, NCDHW or ND, "
                                            "or the last dim when the format of x is NHWC or NDHWC";
                    OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), outputName.c_str(),
                                                          ToString(meanVarianceStorageShape).c_str(),
                                                          reasonMsg.c_str());
                    return ge::GRAPH_FAILED;
                }
            }
...
ge::graphStatus InstanceNormRegbaseTilingBase::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }
