...
/*!
 * \file test_geir_instance_norm.cpp
 * \brief
 */
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    instance_norm_op.set_input_##intputName(placeholder##intputIndex);                                              \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) instance_norm_op.set_attr_##attrName(attrValue)
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    instance_norm_op.set_input_##intputName(placeholder##intputIndex);                                              \
    instance_norm_op.update_input_desc_##intputName(placeholder##intputIndex##_desc);                               \
    inputs.push_back(placeholder##intputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    instance_norm_op.update_output_desc_##outputName(outputName##outputIndex##_desc);
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto instance_norm_op = op::InstanceNorm("instance_norm_op");

    // shape定义
    std::vector<int64_t> xShape = {4, 3, 2, 2};
    std::vector<int64_t> gammaShape = {3};
    std::vector<int64_t> meanShape = {4, 3, 1, 1};

    // 添加输入（顺序严格匹配 proto.h）
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, gamma, inDtype, gammaShape);
    ADD_INPUT(3, beta, inDtype, gammaShape);

    // 添加输出（顺序严格匹配 proto.h）
    ADD_OUTPUT(1, y, inDtype, xShape);
    ADD_OUTPUT(2, mean, inDtype, meanShape);
    ADD_OUTPUT(3, variance, inDtype, meanShape);

    // 添加属性（顺序严格匹配 proto.h）
    ADD_INPUT_ATTR(data_format, "ND");
    ADD_INPUT_ATTR(epsilon, 1e-5);

    outputs.push_back(instance_norm_op);
    // 添加完毕
    return SUCCESS;
}
