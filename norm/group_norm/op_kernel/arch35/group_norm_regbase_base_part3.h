...
if constexpr (IsSameType<T1, half>::value || IsSameType<T1, bfloat16_t>::value) {
    ...
    for (uint16_t j = 0; j < loopCount; j++) {
        ...
        Or((RegTensor<int16_t>&)yTmp, (RegTensor<int16_t>&)yEvenTmp, (RegTensor<int16_t>&)yOddTmp, pregMain);
    ...
    for (uint16_t k = 0; k < tailLoop; k++) {
        ...
        Or((RegTensor<int16_t>&)yTmp, (RegTensor<int16_t>&)yEvenTmp, (RegTensor<int16_t>&)yOddTmp, pregLoop);
