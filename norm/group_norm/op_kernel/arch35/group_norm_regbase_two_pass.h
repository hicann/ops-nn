...
/*!
 * \file group_norm_regbase_two_pass.h
 * \brief
 */

#ifndef GROUP_NORM_REGBASE_TWO_PASS_H_
#define GROUP_NORM_REGBASE_TWO_PASS_H_
...
class GroupNormTwoPass {
public:
    __aicore__ inline GroupNormTwoPass(){};
    ...
    __aicore__ inline void Process()
    {
        ...
        for (uint64_t i = 0; i < numPerCoreLoop; i++) {
            ...
            ProcessMeanAndVariance<T1>(meanTensor, meanOutTensor, meanGm, varianceOutTensor, varianceGm, gmOffset,
                                       numPerCoreOneLoop);
            if (i < numPerCoreLoop - 1) {
...
private:
    __aicore__ inline void CalNormalize(uint64_t offset, uint32_t numPerCoreTmp)
    {
        ...
        for (int64_t i = 0; i < numPerCoreExtent; i++) {
            ...
            __local_mem__ T1* varianceOutLocal = (__local_mem__ T1*)varianceOutTensor[onceNumPerCore * i].GetPhyAddr();
            if (i > 1) {
