...
/*!
 * \file group_norm_regbase_welford_generalized.h
 * \brief
 */
#ifndef GROUP_NORM_REGBASE_WELFORD_GENERALIZED_H_
...
class GroupNormWelfordGeneralized {
public:
    __aicore__ inline GroupNormWelfordGeneralized(){};
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR gamma, GM_ADDR beta, GM_ADDR y, GM_ADDR mean, GM_ADDR variance,
                                GM_ADDR workspace, const GroupNormTilingData* tilingData)
    {
        // 绑定GM张量并按tiling规划UB。
        tiling = tilingData;
...
private:
    ...
    __aicore__ inline void CalMeanAndRstdByWelford(uint64_t curNumPerCore, uint64_t curInnerNumPerCore)
    {
        ...
        __local_mem__ T1* varianceOutLocal = (__local_mem__ T1*)varianceOutTensor.GetPhyAddr();
        ...
        VFWelfordParallelFinalize(meanLocal, rstdLocal, varianceOutLocal, tmpMeanLocal, tmpVarLocal, dichotomyAddLocal,
                                  parallelN, dichotomyAddPower, dichotomyAddK, dichotomyAddLastNum, curInnerNumPerCore,
    ...
    __aicore__ inline void InitInnerBuffer()
    {
        ...
        int32_t meanOffset = ubElemNum - meanSize - rstdSize - gammaSize - betaSize - meanOutSize - varianceOutSize;
