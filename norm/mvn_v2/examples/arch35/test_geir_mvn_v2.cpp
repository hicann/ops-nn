...
#include "../../op_graph/mvn_v2_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    mvn_v2_op.set_input_##intputName(placeholder##intputIndex);                                                     \
    inputs.push_back(placeholder##intputIndex)

#define ADD_INPUT_ATTR(attrName, attrValue) mvn_v2_op.set_attr_##attrName(attrValue)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    mvn_v2_op.update_output_desc_##outputName(outputName##outputIndex##_desc);
...
{
    Status ret = SUCCESS;
    auto mvn_v2_op = op::MVNV2("mvn_v2_op");

    std::vector<int64_t> xShape = {1, 3, 4, 4};

    ADD_INPUT(1, x, inDtype, xShape);

    ADD_OUTPUT(1, y, inDtype, xShape);

    ADD_INPUT_ATTR(eps, 1.0e-9f);
    mvn_v2_op.set_attr_axes(std::vector<int64_t>{0, 2, 3});

    outputs.push_back(mvn_v2_op);
    return SUCCESS;
}
