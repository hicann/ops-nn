...
/*!
 * \file test_batch_norm_grad_ext2.cpp
 * \brief
 */
...
#include "batch_norm_grad_ext2_tiling_def.h"
...
extern "C" __global__ __aicore__ void batch_norm_grad_ext2(GM_ADDR y_backprop, GM_ADDR x, GM_ADDR scale,
                                                           GM_ADDR reserve_space1, GM_ADDR reserve_space2,
                                                           GM_ADDR reserve_space3, GM_ADDR x_backprop,
                                                           GM_ADDR scale_backprop, GM_ADDR offset_backprop,
                                                           GM_ADDR reserve_space4, GM_ADDR reserve_space5,
                                                           GM_ADDR workspace, GM_ADDR tiling);

class batch_norm_grad_ext2_test : public testing::Test {
protected:
    static void SetUpTestCase() { cout << "batch_norm_grad_ext2_test SetUp\n" << endl; }
    static void TearDownTestCase() { cout << "batch_norm_grad_ext2_test TearDown\n" << endl; }
};
                    ...
                    int64_t tilingKey, uint32_t blockNum, uint8_t* tiling, bool is_training = true)
{
    ...
    ICPU_RUN_KF(batch_norm_grad_ext2, realBlockNum, dy, x, scale, mean, var, reserve_space3, dx, dscale, dbias,
                reserver_space_4, reserver_space_5, workspace, tiling);
...
TEST_F(batch_norm_grad_ext2_test, test_split_r1_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNormGradExt2RARRecomputeTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNormGradExt2RARRecomputeTilingData*
        tilingDatafromBin = reinterpret_cast<BatchNormGradExt2RARRecomputeTilingData*>(tiling);
...
TEST_F(batch_norm_grad_ext2_test, test_full_load_float32)
{
    ...
    uint32_t blockNum = 2;
    size_t tilingSize = sizeof(BatchNormGradExt2RARFullLoadTilingData);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNormGradExt2RARFullLoadTilingData*
        tilingDatafromBin = reinterpret_cast<BatchNormGradExt2RARFullLoadTilingData*>(tiling);
...
TEST_F(batch_norm_grad_ext2_test, test_split_r0_float32)
{
...
TEST_F(batch_norm_grad_ext2_test, test_infer_channel_last_float32)
{
    ...
    size_t tilingSize = sizeof(BatchNormGradExt2InferChannelLastTilingData) + sizeof(float);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNormGradExt2InferChannelLastTilingData*
        tilingDatafromBin = reinterpret_cast<BatchNormGradExt2InferChannelLastTilingData*>(tiling);
...
TEST_F(batch_norm_grad_ext2_test, test_infer_splitR1_float32)
{
    ...
    uint32_t blockNum = 1;
    size_t tilingSize = sizeof(BatchNormGradExt2InferTilingData) + sizeof(float);
    uint8_t* tiling = (uint8_t*)AscendC::GmAlloc(tilingSize);
    BatchNormGradExt2InferTilingData* tilingDatafromBin = reinterpret_cast<BatchNormGradExt2InferTilingData*>(tiling);
...
TEST_F(batch_norm_grad_ext2_test, test_infer_splitR0_float32)
{
