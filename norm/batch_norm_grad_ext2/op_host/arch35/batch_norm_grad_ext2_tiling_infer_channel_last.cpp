...
/* !
 * \file batch_norm_grad_ext2_tiling_infer_channel_last.cpp
 * \brief
 */
...
class BatchNormGradExt2InferChannelLastTiling : public BatchNormGradExt2InferBase {
public:
    explicit BatchNormGradExt2InferChannelLastTiling(gert::TilingContext* context) : BatchNormGradExt2InferBase(context)
    {}
    ~BatchNormGradExt2InferChannelLastTiling() override = default;
...
protected:
    bool IsCapable() override
    {
        OP_CHECK_IF(
            r0Dim != 1,
            OP_LOGD(context_,
                    "BatchNormGradExt2InferChannelLastTiling BA template is not capable, fused shape: (%ld, %ld, %ld)",
                    r1Dim, aDim, r0Dim),
            return false);
        ...
        OP_LOGD(context_,
                "BatchNormGradExt2InferChannelLastTiling BA template is capable, fused shape: (%ld, %ld, %ld)", r1Dim,
                aDim, r0Dim);
        return true;
    }
...
private:
    ...
    BatchNormGradExt2InferChannelLastTilingData tilingData_;
};

ge::graphStatus BatchNormGradExt2InferChannelLastTiling::DoOpTiling()
{
    DoOpTilingForStage0();
    DoOpTilingForStage1();
...
ge::graphStatus BatchNormGradExt2InferChannelLastTiling::DoOpTilingForStage0()
{
    ...
    tilingData_.dxTilingData.totalTiles = totalTiles;
    tilingData_.dxTilingData.tilesPerCore = tilesPerCore;
    tilingData_.dxTilingData.usedCoreNums = usedCoreNums_;
    tilingData_.dxTilingData.aDim = aDim;
    tilingData_.dxTilingData.aOuter = aOuter;
    tilingData_.dxTilingData.bOuter = bOuter;
    tilingData_.dxTilingData.tileBlockALen = tileBlockALen;
    tilingData_.dxTilingData.tileBlockATail = tileBlockATail;
    tilingData_.dxTilingData.tileBlockAPaddingNum = tileBlockAPaddingNum;
    tilingData_.dxTilingData.tileBlockBLen = bInner;
    tilingData_.dxTilingData.tileBlockBTail = tileBlockBTail;
    tilingData_.dxTilingData.epsilon = epsilon_;
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus BatchNormGradExt2InferChannelLastTiling::DoOpTilingForStage1()
{
    // 二分
    ...
    tilingData_.binAddRFactorStg1 = binAddRFactor;
    tilingData_.binAddRLoopStg1 = binAddRLoop;
    tilingData_.binAddRTotalLoopStg1 = binAddRTotalLoop;
    tilingData_.binAddRTailStg1 = binAddRTail;
    tilingData_.binAddBasicBlockLoopStg1 = binAddBasicBlockLoop;
    tilingData_.binAddMainFoldCountStg1 = mainFoldCount;
    tilingData_.binAddCacheBufferCountStg1 = binAddCacheBufferCount;
    tilingData_.binAddResultCacheIDStg1 = binAddResultCacheID;
    ...
    tilingData_.aDimStg1 = aDim;
    tilingData_.aOuterStg1 = aOuter;
    tilingData_.aInnerStg1 = aInner;
    tilingData_.aTailStg1 = aTail;
    tilingData_.aOuterPerCoreStg1 = aOuterPerCore;
    tilingData_.usedCoreNumsStg1 = usedCoreNums;
...
uint64_t BatchNormGradExt2InferChannelLastTiling::GetTilingKey() const { return TILINGKEY_INFER_CHANNEL_LAST_BASE; }

int64_t BatchNormGradExt2InferChannelLastTiling::GetCacheID(const int64_t idx)
{
    return __builtin_popcountll(idx ^ (idx + CONST_ONE)) - CONST_ONE;
}

ge::graphStatus BatchNormGradExt2InferChannelLastTiling::PostTiling()
{
    context_->SetBlockDim(
        static_cast<uint32_t>(std::max(tilingData_.usedCoreNumsStg1, tilingData_.dxTilingData.usedCoreNums)));
...
REGISTER_OPS_TILING_TEMPLATE(BatchNormGradExt2, BatchNormGradExt2InferChannelLastTiling, 90000);
} // namespace optiling
