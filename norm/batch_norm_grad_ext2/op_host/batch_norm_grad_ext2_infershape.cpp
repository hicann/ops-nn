...
/*!
 * \file batch_norm_grad_ext2_infershape.cpp
 * \brief
 */
#include "log/log.h"
...
static ge::graphStatus BatchNormGradExt2InferShape(gert::InferShapeContext* context)
{
    if (context == nullptr) {
        return GRAPH_FAILED;
    }
...
static ge::graphStatus BatchNormGradExt2InferDataType(gert::InferDataTypeContext* context)
{
    if (context == nullptr) {
        return GRAPH_FAILED;
    }
...
IMPL_OP_INFERSHAPE(BatchNormGradExt2)
    .InferShape(BatchNormGradExt2InferShape)
    .InferDataType(BatchNormGradExt2InferDataType);
} // namespace ops
