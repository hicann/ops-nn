...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/norm/rms_norm)
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, `aclnnRmsNormGetWorkspaceSize` is called to obtain the input parameters and compute the workspace size required by the process. Then, `aclnnRmsNorm` is called to perform computation.
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>gamma</td>
      <td>Input</td>
      <td>Scaling factor (weight) for normalization computation, corresponding to `gamma` in the formula.</td>
    ...
    <tr>
      ...
      <td>The default value is 1e-6.</td>
      <td>DOUBLE</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
    ...
    <tr>
      <td>yOut</td>
      <td>Output</td>
      <td>Final output of normalization computation, corresponding to `RmsNorm(x)` in the formula.</td>
      <td><ul><li>Empty tensors are supported. </li><li>The shape must be the same as that of the input parameter `x`.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16</td>
      <td>ND</td>
      <td>1-8</td>
      <td>√</td>
    ...
    <tr>
      <td>rstdOut</td>
      <td>Output</td>
      <td> Normalized reciprocal of the standard deviation, used for normalization. It corresponds to the reciprocal of `Rms(x)` in the formula.</td>
  ...
  - <term>Atlas inference series products</term>: The data types of `x`, `gamma` and `yOut` cannot be BFLOAT16.
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, which is obtained by the first-phase API aclnnRmsNormGetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
- <term>Atlas inference series products</term>: The length of the last axis of the inputs **x** and **gamma** must be greater than or equal to 32 bytes.
- Description of boundary value scenarios:
  - <term>Atlas inference series products</term>: The input cannot contain **Inf** or **NaN**.
...
int main()
{
    ...
    aclOpExecutor* executor;
    // Call the first-phase API of aclnnRmsNorm.
    ...
    // Call the second-phase API of aclnnRmsNorm.
