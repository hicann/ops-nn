...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                   \
    ...
    TensorDesc placeholder##intputIndex##_desc = TensorDesc(ge::Shape(placeholder##intputIndex##_shape), FORMAT_NCHW, \
                                                            intputDtype);                                             \
    placeholder##intputIndex##_desc.SetPlacement(ge::kPlacementHost);                                                 \
    placeholder##intputIndex##_desc.SetFormat(FORMAT_NCHW);                                                           \
    ...
    inTrainingReduceGrad1.set_input_##intputName(placeholder##intputIndex);                                           \
...
{
    Status ret = SUCCESS;
    auto inTrainingReduceGrad1 = op::INTrainingReduceGrad("inTrainingReduceGrad1");
    std::vector<int64_t> tensorShape = {2, 4, 8, 8};
    std::vector<int64_t> channelShape = {2, 4, 1, 1};

    ADD_INPUT(0, dy, inDtype, tensorShape);
    ADD_INPUT(1, x, inDtype, tensorShape);
    ADD_INPUT(2, variance, DT_FLOAT, channelShape);
    ADD_INPUT(3, mean, DT_FLOAT, channelShape);
    ADD_INPUT(4, res_gamma, DT_FLOAT, channelShape);
    ADD_INPUT(5, res_beta, DT_FLOAT, channelShape);
    ADD_INPUT(6, gamma, DT_FLOAT, channelShape);

    outputs.push_back(inTrainingReduceGrad1);
    return SUCCESS;
}

int main()
{
    const char* graph_name = "in_training_reduce_grad_geir_test";
    ...
    ret = CreateOppInGraph(DT_FLOAT, input, inputs, outputs, graph);
    if (ret != SUCCESS) {
