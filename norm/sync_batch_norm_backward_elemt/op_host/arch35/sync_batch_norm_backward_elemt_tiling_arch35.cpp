...
ge::graphStatus SyncBatchNormBackwardElemtTiling::CalcInputDtype()
{
    ...
    OP_CHECK_IF(this->gradOutputDtype != ge::DT_FLOAT16 && this->gradOutputDtype != ge::DT_BF16 &&
                    this->gradOutputDtype != ge::DT_FLOAT,
                OP_LOGE_FOR_INVALID_DTYPE(tilingContext->GetNodeName(), "grad_output",
                                          ge::TypeUtils::DataTypeToSerialString(gradOutputDtype).c_str(),
...
ge::graphStatus SyncBatchNormBackwardElemtTiling::CalcOutputDtype()
{
    ...
    if (this->gradOutputDtype != this->outputDtype) {
        std::string reasonMsg = "The dtype of output parameter grad_input must be the same as that (" +
                                ...
                                ") of input parameter grad_output";
...
ge::graphStatus SyncBatchNormBackwardElemtTiling::RunTiling()
{
    ...
    OP_CHECK_IF(CalcInputDtype() == ge::GRAPH_FAILED, OP_LOGE(tilingContext, "check input dtype failed"),
                return ge::GRAPH_FAILED);

    OP_CHECK_IF(CalcOutputDtype() == ge::GRAPH_FAILED, OP_LOGE(tilingContext, "check output dtype failed"),
                return ge::GRAPH_FAILED);
    ...
    if (this->outputDtype == ge::DT_FLOAT16) {
        if (this->meanDtype == DT_FLOAT) {
            res = elewiseBaseTiling.DoTiling<SyncBatchNormBackwardElemtDag<half, float>::OpDag>(tiling->baseTiling);
        } else {
        ...
        res = elewiseBaseTiling.DoTiling<SyncBatchNormBackwardElemtDag<bfloat16_t, bfloat16_t>::OpDag>(
            tiling->baseTiling);
    } else {
