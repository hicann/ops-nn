...
#define ADD_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                  \
    ...
    layerNormV3_1.set_input_##inputName(placeholder##inputIndex);                                                 \
    inputs.push_back(placeholder##inputIndex);

#define ADD_INPUT_ATTR(attrName, attrValue) layerNormV3_1.set_attr_##attrName(attrValue);
...
#define ADD_CONST_INPUT(inputIndex, inputName, inputDtype, inputShape)                                                 \
    ...
    graph.AddOp(placeholder##inputIndex);                                                                              \
    layerNormV3_1.set_input_##inputName(placeholder##inputIndex);                                                      \
    layerNormV3_1.update_input_desc_##inputName(placeholder##inputIndex##_desc);                                       \
    inputs.push_back(placeholder##inputIndex);

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                        \
    TensorDesc outputName##outputIndex##_desc_ = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    layerNormV3_1.update_output_desc_##outputName(outputName##outputIndex##_desc_);
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto layerNormV3_1 = op::LayerNormV3("layerNormV3_1");
