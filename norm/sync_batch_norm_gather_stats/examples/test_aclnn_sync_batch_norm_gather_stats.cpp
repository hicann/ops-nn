...
int main()
{
    ...
    ret = CreateAclTensor(totalSquareSumData, totalSquareSumShape, &totalSquareSumDeviceAddr, aclDataType::ACL_FLOAT,
                          &totalSquareSum);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    // 创建input sampleCount aclTensor
    ret = CreateAclTensor(sampleCountData, sampleCountShape, &sampleCountDeviceAddr, aclDataType::ACL_INT32,
                          &sampleCount);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = CreateAclTensor(batchInvstdData, batchInvstdShape, &batchInvstdDeviceAddr, aclDataType::ACL_FLOAT,
                          &batchInvstd);
    CHECK_RET(ret == ACL_SUCCESS, return ret);
    ...
    ret = aclnnSyncBatchNormGatherStatsGetWorkspaceSize(totalSum, totalSquareSum, sampleCount, mean, var, momentum, eps,
                                                        batchMean, batchInvstd, &workspaceSize, &executor);
