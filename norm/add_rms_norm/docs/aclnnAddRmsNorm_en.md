...
[📄 View source code](https://gitcode.com/cann/ops-nn/tree/master/norm/add_rms_norm)
...
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, `aclnnAddRmsNormGetWorkspaceSize` is called to obtain the input parameters and compute the required workspace size based on the process. Then, `aclnnAddRmsNorm` is called to perform computation.
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>gamma</td>
      <td>Input</td>
      <td>Scaling factor (weight) of RmsNorm. It corresponds to `gamma` in the formula.</td>
      <td><ul><li>Empty tensors are not supported. </li><li>The data type must be the same as that of `x1`. </li><li>The shape must be the same as the last several dimensions of `x1`. The last several dimensions indicate the dimensions that need to be normalized in `x1`.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16</td>
      <td>ND</td>
      <td>1-8</td>
      <td>√</td>
    ...
    <tr>
      <td>yOut</td>
      <td>Output</td>
      <td>Final output. It corresponds to `RmsNorm(x)` in the formula.</td>
      <td><ul><li>Empty tensors are not supported. </li><li>The shape and data type are the same as those of the input `x1`.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16</td>
      <td>ND</td>
      <td>1-8</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td><ul><li>Empty tensors are not supported. </li><li>The shape must be the same as the first several dimensions of `x1`. The first several dimensions indicate the dimensions that do not need to be normalized. For example, if the shape of x1 is (2, 3, 4, 8) and the shape of gamma is (8), the shape of rstdOut is (2, 3, 4, 1); if the shape of x1 is (2, 3, 4, 8) and the shape of gamma is (4, 8), the shape of rstdOut is (2, 3, 1, 1).</li></ul></td>
      <td>FLOAT32</td>
      <td>ND</td>
      <td>1-8</td>
      <td>√</td>
    ...
    <tr>
      ...
      <td><ul><li>Empty tensors are not supported. </li><li>The shape and data type are consistent with those of the input `x1`.</li></ul></td>
      <td>FLOAT32, FLOAT16, BFLOAT16</td>
      <td>ND</td>
      <td>1-8</td>
      <td>√</td>
  ...
  - <term>Atlas inference series products</term>:
    - The data types of the `x1`, `x2`, `gamma`, `yOut`, and `xOut` parameters cannot be BFLOAT16.
    - The `rstdOut` parameter is invalid in the current product.
...
- **Parameters:**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>Input</td>
      <td>Size of the workspace to be allocated on the device, obtained by calling the first-phase API aclnnAddRmsNormGetWorkspaceSize.</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>Input</td>
...
int main()
{
    ...
    aclOpExecutor* executor;
    // Call the first-phase API of aclnnAddRmsNorm.
    ...
    // Call the second-phase API of aclnnAddRmsNorm.
