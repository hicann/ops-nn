...
static bool IsInputDtypeValid(const std::vector<NodeIo>& input_nodes)
{
    DataType x1_dtype = DT_UNDEFINED;
    for (const auto& input_node : input_nodes) {
        AscendString node_type;
        input_node.node.GetType(node_type);
        bool is_addrmsnorm_add_input = node_type == "AddRmsNorm" && (input_node.index == 0 || input_node.index == 1);
        if (node_type == "Add" || is_addrmsnorm_add_input) {
            TensorDesc desc;
            input_node.node.GetInputDesc(input_node.index, desc);
            x1_dtype = desc.GetDataType();
            break;
        }
    }
    // x1 dtype must be FP16 or BF16
    if (x1_dtype != DT_FLOAT16 && x1_dtype != DT_BF16) {
        return false;
    }
    // All Add inputs and AddRmsNorm x1/x2 must have consistent dtype
    for (const auto& input_node : input_nodes) {
        AscendString node_type;
        input_node.node.GetType(node_type);
        bool is_addrmsnorm_add_input = node_type == "AddRmsNorm" && (input_node.index == 0 || input_node.index == 1);
        if (node_type == "Add" || is_addrmsnorm_add_input) {
            TensorDesc desc;
            input_node.node.GetInputDesc(input_node.index, desc);
            if (desc.GetDataType() != x1_dtype) {
                return false;
            }
        }
    }
    // smooth_scales dtype must match x1 dtype
    for (const auto& input_node : input_nodes) {
        AscendString node_type;
        input_node.node.GetType(node_type);
...
static bool IsGammaShapeValid(const std::vector<NodeIo>& input_nodes)
{
    // Get gamma shape for comparison with smooth_scales
    std::vector<int64_t> gamma_dims;
    for (const auto& input_node : input_nodes) {
        AscendString node_type;
        input_node.node.GetType(node_type);
        if (node_type == "AddRmsNorm" && input_node.index == 2) {
            TensorDesc desc;
            input_node.node.GetInputDesc(input_node.index, desc);
            gamma_dims = desc.GetShape().GetDims();
    ...
    // smooth_scales shape must be 1-dim and match gamma shape
    for (const auto& input_node : input_nodes) {
        AscendString node_type;
        input_node.node.GetType(node_type);
        if (node_type == "DynamicQuant" && input_node.index == 1) {
...
bool MultiAddRmsNormDynamicQuantFusionPass::MeetRequirements(const std::unique_ptr<MatchResult>& match_result)
{
    ...
    if (!IsInputDtypeValid(input_nodes)) {
        OPS_LOG_I(PASS_NAME.c_str(), "Check InputDtype Fail");
        return false;
    }
    if (!IsGammaShapeValid(input_nodes)) {
        OPS_LOG_I(PASS_NAME.c_str(), "Check GammaShape Fail");
        return false;
    }
