 ...
 * \brief A5(Ascend950/arch35)infershape 分支专属 UT——base 顶层 UT 覆盖 A2 分支,本文件只测 A5 分支。
 */
...
class MultiAddRmsNormDynamicQuantArch35 : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "MultiAddRmsNormDynamicQuant Arch35 Infershape SetUp" << std::endl; }
...
TEST_F(MultiAddRmsNormDynamicQuantArch35, infershape_ascend950_unknown_rank)
{
    ...
    gert::StorageShape unknown_shape = {{-2}, {-2}};
    gert::StorageShape out_shape = {{-2}, {-2}};

    auto holder = gert::InferShapeContextFaker()
                      .NodeIoNum(5, 6)
                      .IrInstanceNum({1, 1, 1, 1, 1, 1})
                      .InputShapes({&unknown_shape, &unknown_shape, &unknown_shape, &unknown_shape, &unknown_shape})
                      .OutputShapes({&out_shape, &out_shape, &out_shape, &out_shape, &out_shape, &out_shape})
                      .NodeAttrs({
                          {"epsilon", Ops::NN::AnyValue::CreateFrom<float>(0.01)},
                      })
                      .NodeInputTd(0, ge::DT_FLOAT16, ge::FORMAT_ND, ge::FORMAT_ND)
