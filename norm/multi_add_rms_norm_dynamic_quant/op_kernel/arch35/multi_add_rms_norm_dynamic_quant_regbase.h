...
/*!
 * \file multi_add_rms_norm_dynamic_quant_regbase.h
 * \brief
 */
#ifndef MULTI_ADD_RMS_NORM_DYNAMIC_QUANT_REGBASE_H_
#define MULTI_ADD_RMS_NORM_DYNAMIC_QUANT_REGBASE_H_
...
class KernelMultiAddRmsNormDynamicQuantRegbase {
    using T_SMOOTH_SCALE = T_X;

public:
    __aicore__ inline KernelMultiAddRmsNormDynamicQuantRegbase(TPipe* pipe) { pipe_ = pipe; }
    ...
    __aicore__ inline void InitUBBuffer()
    {
        ...
        pipe_->InitBuffer(x1AccBuf_, baseNReduceAlign_ * sizeof(T_X));    // 增:multi-add Σx1 累加缓冲
                                      ...
                                      uint64_t& mInnerCnt, uint64_t& mOuterOffset)
    {
        for (uint64_t mInnerIdx = 0; mInnerIdx < mInnerCnt; mInnerIdx++) {
            ...
            // multi-add:累加 Σx1[0..x1Num_) 到 x1AccBuf_;x2 单独 CopyIn,ReduceSumRstd 内部再 +x2。
            AccumulateX1(gmOffsetXY);
            ...
            CopyOutY(yGm_, outQueueY_, gmOffsetXY, numN_); // y(RmsNorm 结果)写回
        }
    }
...
private:
    // multi-add:逐个 CopyIn x1[i](经 inQueueX1_ 完成 MTE2→V 同步)并累加到 x1AccBuf_,得 Σx1。
    __aicore__ inline void AccumulateX1(uint64_t gmOffset)
    {
        LocalTensor<T_X> acc = x1AccBuf_.Get<T_X>();
        DataCopyPadExtParams<T_X> padParams{true, 0, static_cast<uint8_t>(baseNDtypeAlign_ - numN_),
                                            static_cast<T_X>(0.0)};
        for (uint32_t i = 0; i < x1Num_; ++i) {
            LocalTensor<T_X> x1Local = inQueueX1_.AllocTensor<T_X>();
            RmsNorm::DataCopyImpl<T_X>(x1Local, x1GmList_[i][gmOffset], 1, numN_, 0, 0, padParams);
            inQueueX1_.EnQue(x1Local);
            LocalTensor<T_X> x1In = inQueueX1_.DeQue<T_X>();
            if (i == 0) {
                Adds(acc, x1In, static_cast<T_X>(0.0), baseNDtypeAlign_);
            } else {
                Add(acc, acc, x1In, baseNDtypeAlign_);
            }
    ...
    {
        LocalTensor<T_X> x1Local = x1AccBuf_.Get<T_X>(); // multi-add:Σx1(由 AccumulateX1 填好)
        ...
        // y(RmsNorm 结果,量化前)= xOut * rstd * gamma
        LocalTensor<T_X> yLocal = outQueueY_.AllocTensor<T_X>();
        ComputeYOut<T_X, T_X>(yLocal, xOutTmpLocal, rstdLocal, gammaLocal, mInnerIdx, baseNDtypeAlign_);
...
#endif // _MULTI_ADD_RMS_NORM_DYNAMIC_QUANT_REGBASE_H_
