...
/*!
 * \file bn_training_reduce_grad_def.cpp
 * \brief BNTrainingReduceGrad op def (ascend950 vendor 注册)
 *        built-in proto 已有完整定义（op_graph/bn_training_reduce_grad_proto.h 逐字照抄 canndev）：
 *          grads/x fp16/fp32/bf16；diff_scale/diff_offset/scale/batch_mean/batch_variance 恒 fp32
 *          （910b TBE para_check 契约）；epsilon 为 OPTIONAL float 属性（默认 0.0001，与 proto 一致）；
 *          y 同 grads
...
namespace ops {
class BNTrainingReduceGrad : public OpDef {
public:
    explicit BNTrainingReduceGrad(const char* name) : OpDef(name)
    {
        // 共享注册面:grads/x/y 三路 dtype,统计量恒 fp32,全部 ND（压缩行数用具名 vector,声明逐条显式）
        ...
        aicoreConfig.DynamicCompileStaticFlag(true)
            ...
            .ExtendCfgInfo("opFile.value", "bn_training_reduce_grad");
        this->AICore().AddConfig("ascend950", aicoreConfig);
    }
};

OP_ADD(BNTrainingReduceGrad);
} // namespace ops
