 ...
 */
/*!
 * \file test_bn_training_reduce_grad_tiling.cpp
 * \brief Tiling UT for BNTrainingReduceGrad (arch35)。
 *        单路径 tilingKey=0；覆盖正向切分（plane 均分 / inner 再切）、dtype 三态、epsilon 缺省、
 *        反向校验（dtype/format/rank/空 tensor/统计量元素数/x 与 grads 不匹配）。
 */
...
#include "../../../../op_host/arch35/bn_training_reduce_grad_tiling_arch35.h"

using namespace std;
using namespace ge;

class BNTrainingReduceGradTilingUT : public testing::Test {
protected:
    static void SetUpTestCase() { std::cout << "BNTrainingReduceGradTilingUT SetUp" << std::endl; }
    static void TearDownTestCase() { std::cout << "BNTrainingReduceGradTilingUT TearDown" << std::endl; }
};

namespace {
constexpr const char* kOpType = "BNTrainingReduceGrad";
...
// 七输入(grads/x/diff_scale/diff_offset/scale/batch_mean/batch_variance) 单输出(y)，format 固定 ND。
// hasEpsilon=false 时不下发 epsilon 属性（OPTIONAL，缺省 0.0001 须放行）；
// xShapeDiff=true 时给 x 一个与 grads 不同的 shape（须被拒）。
                          ...
                          ge::DataType xDt = ge::DT_FLOAT, bool xShapeDiff = false)
{
    ...
    platform_info.Init();
    optiling::BNTrainingReduceGradCompileInfo compile_info;
    ...
    std::vector<int64_t> xDims = gradsDims;
    if (xShapeDiff && !xDims.empty()) {
        xDims.back() += 1;
    }
    gert::StorageShape x = MakeShape(xDims);
    gert::StorageShape stat = MakeShape({c});
    gert::StorageShape y = MakeShape(gradsDims);
    ...
    auto holder = gert::TilingContextFaker()
                      ...
                      .InputShapes({&grads, &x, &stat, &stat, &stat, &stat, &stat})
                      .OutputShapes({&y})
                      ...
                      .NodeInputTd(5, statDt, fmt, fmt)
                      .NodeInputTd(6, statDt, fmt, fmt)
                      .NodeOutputTd(0, gradsDt, fmt, fmt)
                      .NodeAttrs(attrs)
...
TEST_F(BNTrainingReduceGradTilingUT, tiling_registered)
{
    auto op_impl = gert::OpImplRegistry::GetInstance().GetOpImpl(kOpType);
...
TEST_F(BNTrainingReduceGradTilingUT, accept_basic_plane_split)
{
    uint64_t key = 0;
    int64_t blockDim = 0;
...
TEST_F(BNTrainingReduceGradTilingUT, accept_inner_split)
{
    uint64_t key = 0;
    int64_t blockDim = 0;
...
// rank2 最小形态
TEST_F(BNTrainingReduceGradTilingUT, accept_rank2)
{
    uint64_t key = 0;
...
// num==1（N=1 且 R=1）：numRecip=1.0 路径，tiling 须放行
TEST_F(BNTrainingReduceGradTilingUT, accept_num_equals_one)
{
    uint64_t key = 0;
...
// fp16 / bf16 grads+x
TEST_F(BNTrainingReduceGradTilingUT, accept_fp16_bf16)
{
    uint64_t key = 0;
    EXPECT_EQ(
        RunTiling({2, 3, 4, 5}, 3, key, nullptr, ge::DT_FLOAT16, ge::DT_FLOAT, ge::FORMAT_ND, true, ge::DT_FLOAT16),
        ge::GRAPH_SUCCESS);
    EXPECT_EQ(RunTiling({2, 3, 4, 5}, 3, key, nullptr, ge::DT_BF16, ge::DT_FLOAT, ge::FORMAT_ND, true, ge::DT_BF16),
              ge::GRAPH_SUCCESS);
}

// GE 图模式可能下发 NCHW 标签，须接受
TEST_F(BNTrainingReduceGradTilingUT, accept_nchw_tag)
{
    uint64_t key = 0;
    EXPECT_EQ(RunTiling({2, 3, 4, 5}, 3, key, nullptr, ge::DT_FLOAT, ge::DT_FLOAT, ge::FORMAT_NCHW), ge::GRAPH_SUCCESS);
}

// epsilon 为 OPTIONAL 属性，缺失走缺省 0.0001，须放行
TEST_F(BNTrainingReduceGradTilingUT, accept_missing_epsilon)
{
    uint64_t key = 0;
...
{
    uint64_t key = 0;
    EXPECT_EQ(RunTiling({2, 3, 4, 5}, 3, key, nullptr, ge::DT_INT32, ge::DT_FLOAT, ge::FORMAT_ND, true, ge::DT_INT32),
              ge::GRAPH_FAILED);
}

// x 与 grads dtype 不一致
TEST_F(BNTrainingReduceGradTilingUT, reject_x_dtype_mismatch)
{
    uint64_t key = 0;
    EXPECT_EQ(RunTiling({2, 3, 4, 5}, 3, key, nullptr, ge::DT_FLOAT, ge::DT_FLOAT, ge::FORMAT_ND, true, ge::DT_FLOAT16),
              ge::GRAPH_FAILED);
}

// x 与 grads shape 不一致
TEST_F(BNTrainingReduceGradTilingUT, reject_x_shape_mismatch)
{
    uint64_t key = 0;
    EXPECT_EQ(
        RunTiling({2, 3, 4, 5}, 3, key, nullptr, ge::DT_FLOAT, ge::DT_FLOAT, ge::FORMAT_ND, true, ge::DT_FLOAT, true),
        ge::GRAPH_FAILED);
}

// 统计量 dtype 非 fp32
TEST_F(BNTrainingReduceGradTilingUT, reject_invalid_stat_dtype)
{
    uint64_t key = 0;
...
// format 非 ND/NCHW
TEST_F(BNTrainingReduceGradTilingUT, reject_invalid_format)
{
    uint64_t key = 0;
...
// rank < 2
TEST_F(BNTrainingReduceGradTilingUT, reject_rank1)
{
    uint64_t key = 0;
...
TEST_F(BNTrainingReduceGradTilingUT, reject_empty_tensor)
{
    uint64_t key = 0;
...
// 统计量元素数必须等于 C
TEST_F(BNTrainingReduceGradTilingUT, reject_stat_numel_mismatch)
{
    uint64_t key = 0;
