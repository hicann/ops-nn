...
#define CREATE_NODEDEF(shapes, dataTypes, datas)                                    \
    ...
    NodeDefBuilder(nodeDef.get(), "NonZeroWithValueShape", "NonZeroWithValueShape") \
        .Input({"value", (dataTypes)[0], (shapes)[0], (datas)[0]})                  \
        .Input({"index", (dataTypes)[1], (shapes)[1], (datas)[1]})                  \
        .Input({"count", (dataTypes)[2], (shapes)[2], (datas)[2]})                  \
        .Output({"out_value", (dataTypes)[3], (shapes)[3], (datas)[3]})             \
        .Output({"out_index", (dataTypes)[4], (shapes)[4], (datas)[4]})

TEST_F(TEST_NON_ZERO_WITH_VALUE_SHAPE_UT, DataSuccess)
{
...
TEST_F(TEST_NON_ZERO_WITH_VALUE_SHAPE_UT, CountEmptyInvalid)
{
    vector<DataType> dataTypes = {DT_INT32, DT_INT32, DT_INT32, DT_INT32, DT_INT32};
