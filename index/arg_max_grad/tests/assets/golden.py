...
# ----------------------------------------------------------------------------
"""ArgMaxGrad 多通路 golden(TestSpec 范式)。

通路支持表(照抄 01_requirement.md §3.3):
  | 通路   | 支持 | 依据                                                        |
  |--------|------|-------------------------------------------------------------|
  | kernel | ✅   | op_kernel/arch35/ 有实现                                     |
  | geir   | ✅   | op_graph/ 有 REG_OP(ArgMaxGrad) + IMPL_OP_INFERSHAPE        |
  | aclnn  | ❌   | canndev op_api/inc/level2 无 aclnn_arg_max_grad*.h            |
  | e2e    | ❌   | 无 aclnn 接口, torch 无从下发                                 |

语义: y[o,k,i] = (k == indices[o,0,i]) ? updates[o,0,i] : var[o,k,i]
(k 即沿 dimension 轴的下标, 等价于 A2 融合 pass 生成的 assist 张量; 本算子按不带 D 的
ArgMaxGrad 原型交付, 该序号矩阵由实现自行生成, 不再作为输入)
"""
...
__spec__ = {
    ...
    "arg_max_grad": "ArgMaxGradKernelSpec",
}
...
def _compute(var, indices, updates, dimension):
    ...
    # 沿 dim 轴的序号矩阵: 与 A2 assist_int32_help 的三个分支数值等价(值恒为该轴下标)
    seq_shape = [1] * var.dim()
    seq_shape[dim] = var.shape[dim]
    assist = torch.arange(var.shape[dim], dtype=torch.int32).reshape(seq_shape)
    hit = torch.eq(assist.expand_as(var), idx.expand_as(var))
    return [torch.where(hit, upd.expand_as(var).to(var.dtype), var)]


class _ArgMaxGradCompose:
    """三方标杆(A100 上执行): 用 torch 的融合散射接口 Tensor.scatter。

    形态与真实使用一致 —— 单个融合算子, 不用 "eq + where" 的分解表达式。
    参数名与 def.cpp 逐字一致(var / indices / updates)。
    """

    def __init__(self, *, dimension=0, **_):
        self.dimension = int(dimension)

    def __call__(self, var, indices, updates, **_):
...
class ArgMaxGradKernelSpec:
    """kernel + geir 共用。golden 收 numpy.ndarray, 返 numpy.ndarray。

    参数名取自 op_host/arg_max_grad_def.cpp: var / indices / updates。
    """

    def golden(var, indices, updates, **kwargs):
    ...
    def customize_inputs(var, indices, updates, **kwargs):
        """indices 不能用随机数据, 必须按语义构造(参数名与 def.cpp 逐字一致)。
    ...
    third_party = {"torch": _ArgMaxGradCompose}
    tolerance = _TOL


def arg_max_grad_golden(var, indices, updates, **kwargs):
    """保留 __golden__ 约定入口(上库件, 签名照 def.cpp), 与 Spec 共用同一实现。"""
    return ArgMaxGradKernelSpec.golden(var, indices, updates, **kwargs)[0]


__golden__ = {"kernel": {"arg_max_grad": "arg_max_grad_golden"}}
