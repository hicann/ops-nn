...
TEST_F(ScatterMaxTiling, test_tiling_wide_buckets_single_window)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMaxTiling({{137438953472L}, {137438953472L}}, {{4}, {4}}, {{4}, {4}}, ge::DT_FLOAT,
                                  ge::DT_INT64, ge::DT_FLOAT, true, key);
    EXPECT_EQ(st, ge::GRAPH_SUCCESS);
}

// indices 为空 -> 本轮零工作量, 不会走排序分桶, 首维再大也不应被拒收。
TEST_F(ScatterMaxTiling, test_tiling_empty_indices_is_noop)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMaxTiling({{1099511627776L, 4L}, {1099511627776L, 4L}}, {{0}, {0}}, {{0, 4}, {0, 4}},
                                  ge::DT_FLOAT, ge::DT_INT64, ge::DT_FLOAT, true, key);
    EXPECT_EQ(st, ge::GRAPH_SUCCESS);
}

TEST_F(ScatterMaxTiling, test_tiling_wide_buckets_multi_window)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMaxTiling({{138512695296L}, {138512695296L}}, {{4}, {4}}, {{4}, {4}}, ge::DT_FLOAT,
                                  ge::DT_INT64, ge::DT_FLOAT, true, key);
    EXPECT_EQ(st, ge::GRAPH_SUCCESS);
}

// 首维 2^40 = 1024 个桶, 需要 16 个窗口: 桶数无上限, tiling 不拦, 与 A2 支持面一致。
TEST_F(ScatterMaxTiling, test_tiling_huge_first_dim_no_cap)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMaxTiling({{1099511627776L}, {1099511627776L}}, {{4}, {4}}, {{4}, {4}}, ge::DT_FLOAT,
                                  ge::DT_INT64, ge::DT_FLOAT, true, key);
    EXPECT_EQ(st, ge::GRAPH_SUCCESS);
}
...
TEST_F(ScatterMaxTiling, test_tiling_first_dim_int64_max)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMaxTiling({{9223372036854775807L, 1L}, {9223372036854775807L, 1L}}, {{0}, {0}},
                                  {{0, 1}, {0, 1}}, ge::DT_FLOAT, ge::DT_INT64, ge::DT_FLOAT, true, key);
    EXPECT_EQ(st, ge::GRAPH_SUCCESS);
}
