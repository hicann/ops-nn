...
def __golden_scatter_max(*input_arrays, **kwargs):
    ...
    # 越界索引先剔除（与 kernel 的 skip 一致），再把命中的行压缩成小张量交给 torch 竞品
    # 算子；include_self=True 即 max(var 原值, 命中该行的所有 updates)，重复索引顺序无关。
    ...
    idx64 = idx_flat.astype(np.int64)
    valid = (idx64 >= 0) & (idx64 < var_first_dim)
    pos = np.nonzero(valid)[0]
    if pos.size == 0:
        return [out]  # 全部越界：与 kernel 一致，var 原值输出
    # touched 为去重后的真实行号（有序），inverse 是各条目在 touched 中的下标；
    # 行号重标号不改变 amax 结果。
    touched, inverse = np.unique(idx64[pos], return_inverse=True)
    work = torch.from_numpy(
        np.ascontiguousarray(out[touched]).astype(work_dtype, copy=False)
    )
    upd_w = torch.from_numpy(
        np.ascontiguousarray(upd[pos]).astype(work_dtype, copy=False)
    )
    work.index_reduce_(
        0, torch.from_numpy(inverse.astype(np.int64)), upd_w, "amax", include_self=True
    )

    # 浮点出口不 cast(TTK 负责); 整型无 Promote, 由此处复刻 NarrowStore。
    res = work.numpy()
    if need_narrow:
        res = res.astype(var.dtype)
    out[touched] = res
    return [out]
...
class ScatterMaxAclnnSpec:
    ...
    故签名对齐 aclnnScatterMaxGetWorkspaceSize 的 varRef/indices/updates/useLocking;
    ...
    def golden(varRef, indices, updates, useLocking=None, **kwargs):
        ...
        类体内不能直接写 __golden_xxx —— 会被改写成 _ScatterMaxAclnnSpec__golden_xxx, 故用 _GOLDEN_FN。
        """
    ...
    class _TfCompose:
        def __call__(self, varRef, indices, updates, **kwargs):
            return _ScatterMaxTfCompose()(varRef, indices, updates, **kwargs)
...
# TF 通路连通性: 算子目录下有 framework 的 tf_plugin(OriginOpType "ScatterMax"), 但 NPU 侧要把
