...
/*!
 * \file repeat_interleave_scalar.h
 * \brief
 */

#ifndef REPEAT_INTERLEAVE_SCALAR_H
#define REPEAT_INTERLEAVE_SCALAR_H
...
public:
    __aicore__ inline RepeatInterleaveScalarImpl(const RepeatInterleaveTilingKernelDataNorm& tilingData, TPipe& pipe)
...
private:
    ...
    int64_t repeatValueCntInUbFactor_{0}; // ubFactor大小的空间中能放下repeatScalarValue个cp轴的数量
    int64_t outStartOffset_{0};
};

template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::Init(GM_ADDR x, GM_ADDR repeats, GM_ADDR y, GM_ADDR workspace)
{
    ...
    pipe_.InitBuffer(xInQueue_, 1, tilingData_.ubFactor * sizeof(T));
    pipe_.InitBuffer(xOutQueue_, 1, tilingData_.ubFactor * sizeof(T));
    ...
    repeatValueCntInUbFactor_ = cpCountInUbFactor_ / repeatsScalarValue_;
    ComputeStartOffset();
}

template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::ComputeStartOffset()
{
    int64_t batchIdx = 0;
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::CopyInX(int64_t repeatDimIdx, int64_t loopIdx,
                                                                 int64_t dataCount)
{
    /* 源数据在repeat轴上的偏移 */
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::CopyOneCpToRepeatOutLarge(const LocalTensor<T> xInLocal,
                                                                                   int64_t cpIdx, uint16_t repeatTimes)
{
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::CopyOneCpToRepeatOut(const LocalTensor<T> xInLocal,
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::CopyXToMatchOut(int64_t startCpIdx, int64_t cpNum)
{
    LocalTensor<T> xInLocal = xInQueue_.DeQue<T>();
    LocalTensor<T> xOutLocal = xOutQueue_.AllocTensor<T>();
    xOutQueue_.EnQue(xOutLocal);

    /* ubFactor足够放入一个repeatsScalarValue_的情况 */
    if (repeatValueCntInUbFactor_ > 0) {
        ...
        for (int64_t loopIdx = 0; loopIdx < loopSize - 1; loopIdx++) {
            ...
            CopyOneCpToRepeatOut(xInLocal, loopIdx, repeatValueCntInUbFactor_);
        ...
        SetFlag<HardEvent::MTE3_V>(eventIdMte3ToV);
        WaitFlag<HardEvent::MTE3_V>(eventIdMte3ToV);
        CopyOneCpToRepeatOut(xInLocal, loopSize - 1, tailCpNum);
        CopyMatchOutToY((loopSize - 1) * repeatValueCntInUbFactor_ * repeatsScalarValue_,
                        tailCpNum * repeatsScalarValue_);
        xInQueue_.FreeTensor(xInLocal);
        xOutQueue_.FreeTensor(xOutLocal);
        return;
    }

    /* ubFactor不够放入一个repeatsScalarValue_的情况 */
    int64_t loopSize = Ops::Base::CeilDiv(repeatsScalarValue_, cpCountInUbFactor_);
    int64_t tailRepeatTimes = repeatsScalarValue_ - (loopSize - 1) * cpCountInUbFactor_;
    for (int64_t cpIdx = 0; cpIdx < cpNum; cpIdx++) {
        for (int64_t loopIdx = 0; loopIdx < loopSize - 1; loopIdx++) {
            event_t eventIdMte3ToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE3_V));
            SetFlag<HardEvent::MTE3_V>(eventIdMte3ToV);
            WaitFlag<HardEvent::MTE3_V>(eventIdMte3ToV);
            CopyOneCpToRepeatOutLarge(xInLocal, cpIdx, cpCountInUbFactor_);
            CopyMatchOutToY(cpIdx * repeatsScalarValue_ + loopIdx * cpCountInUbFactor_, cpCountInUbFactor_);
        }
        event_t eventIdMte3ToV = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE3_V));
        SetFlag<HardEvent::MTE3_V>(eventIdMte3ToV);
        WaitFlag<HardEvent::MTE3_V>(eventIdMte3ToV);
        CopyOneCpToRepeatOutLarge(xInLocal, cpIdx, tailRepeatTimes);
        CopyMatchOutToY(cpIdx * repeatsScalarValue_ + (loopSize - 1) * cpCountInUbFactor_, tailRepeatTimes);
    }
    xInQueue_.FreeTensor(xInLocal);
    xOutQueue_.FreeTensor(xOutLocal);
    return;
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::CopyXToOut(int64_t dataCount)
{
    LocalTensor<T> xInLocal = xInQueue_.DeQue<T>();
    LocalTensor<T> xOutLocal = xOutQueue_.AllocTensor<T>();
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::CopyMatchOutToY(int64_t cpOfset, int64_t cpNum)
{
    LocalTensor<T> xOutLocal = xOutQueue_.DeQue<T>();
    int64_t offset = cpOfset * tilingData_.mergedDims[2];
    DataCopyExtParams outParams;
    outParams.blockCount = 1;
    outParams.blockLen = cpNum * tilingData_.mergedDims[2] * sizeof(T);
    outParams.srcStride = 0;
    outParams.dstStride = 0;
    DataCopyPad(yGm_[outStartOffset_ + offset], xOutLocal, outParams);
    xOutQueue_.EnQue(xOutLocal);
}

template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::CopyOutY(int64_t repeatDimIdx, int64_t loopIdx,
                                                                  int64_t dataCount)
{
    int64_t loopOffset = loopIdx * eachLoopHandleNum_;
    int64_t offset = repeatDimIdx * repeatsScalarValue_ * tilingData_.mergedDims[2] + cpTileOffset_ + loopOffset;
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::ProcessWholeCp()
{
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::ProcessSplitCp()
{
...
template <typename T, typename U>
__aicore__ inline void RepeatInterleaveScalarImpl<T, U>::Process()
{
...
#endif // REPEAT_INTERLEAVE_SCALAR_H
