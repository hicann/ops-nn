...
static inline bool CheckShape(const aclTensor* self, const aclTensor* indices, const aclTensor* out)
{
    ...
    OP_CHECK(selfDimNum == NDHW_DIM_NUM || selfDimNum == NCDHW_DIM_NUM,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                     ...
                     "but got a tensor with dim %zu",
                     selfDimNum),
             return false);

    OP_CHECK(selfDimNum == indices->GetViewShape().GetDimNum(),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Expected shape of indices to be: %zu, but got: %zu", selfDimNum,
                     indices->GetViewShape().GetDimNum()),
             return false);

    OP_CHECK(selfDimNum == out->GetViewShape().GetDimNum(),
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "Expected shape of out to be: %zu, but got: %zu", selfDimNum,
...
static inline bool CheckOutShape(const aclTensor* self, const aclIntArray* outputSize, const aclTensor* gradOutput)
{
    ...
    OP_CHECK(selfDimNum == gradOutputDimNum && dimN == gradOutputN && dimC == gradOutputC,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                     "gradOutput and input Tensors should have same number of dimensions and "
                     "also the same number of channels/slices. But gradOutput is %zu dimensions, %ld batch size, %ld "
                     "channels/slices,"
                     " self is %zu dimensions , %ld batch size, %ld channels/slices.",
                     gradOutputDimNum, gradOutputN, gradOutputC, selfDimNum, dimN, dimC),
             return false);
    return true;
}
                                                     ...
                                                     aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    L2_DFX_PHASE_1(aclnnMaxUnpool3dBackward, DFX_IN(gradOutput, self, indices, outputSize, stride, padding),
                   DFX_OUT(out));
    ...
    const aclIntArray* indicesNewShapeArray = CalcMaxUnpool3dGradInputNewShape(indicesContiguous, uniqueExecutor.get());
