...
/*!
 * \file gather_elements_transpose.h
 * \brief
 */

#ifndef GATHER_ELEMENTS_TRANSPOSE_H
#define GATHER_ELEMENTS_TRANSPOSE_H
...
private:
    ...
    __aicore__ inline void GatherInUb(const uint64_t& xGatherDimSlice, const uint64_t& computeNum)
    {
        ...
        if constexpr (std::is_same_v<T_IDX, int64_t>) {
            LocalTensor<int32_t> idx32Local = xLocal[CeilAlign(xGatherDimSlice, this->xAlign_)]
                                                  .template ReinterpretCast<int32_t>();
            Muls(idx32Local, idx32Local, static_cast<int32_t>(sizeof(T_X)), computeNum);
            PipeBarrier<PIPE_V>();
            LocalTensor<uint32_t> gatherOffsetLocal = idx32Local.template ReinterpretCast<uint32_t>();
    ...
    __aicore__ inline void TransposeInUb(const uint64_t& h, const uint64_t& w)
    {
        ...
        } else if constexpr (sizeof(T) == BYTE8) {
            TransposeByte8<T>(dataTransLocal, dataLocal, h, w);
        }
    ...
    template <typename T>
    __aicore__ inline void TransposeByte8(LocalTensor<T>& dstLocal, LocalTensor<T>& srcLocal, const uint64_t& h,
                                          const uint64_t& w)
    {
        uint64_t hAlign = CeilAlign(h, TRANS_LEN);
        uint64_t blockNum = V2_BLOCK_SIZE / BYTE8;
        uint64_t wAlign = CeilAlign(w, blockNum);
        for (size_t row = 0; row < hAlign; row++) {
            for (size_t col = 0; col < w; col++) {
                dstLocal.SetValue(col * hAlign + row, srcLocal.GetValue(row * wAlign + col));
            }
        }
    }
                                          ...
                                          const uint64_t& w)
    {
        ...
        uint64_t hAlign = CeilAlign(h, TRANS_LEN);
        uint64_t blockNum = V2_BLOCK_SIZE / BYTE4;
        uint64_t wAlign = CeilAlign(w, blockNum);
                                          ...
                                          const uint64_t& w)
    {
        ...
        uint64_t dstList[TRANS_LEN];
        uint64_t hAlign = CeilAlign(h, TRANS_LEN);
        uint64_t blockNum = V2_BLOCK_SIZE / BYTE2;
        uint64_t wAlign = CeilAlign(w, blockNum);
    ...
    __aicore__ inline void DoNegativeIndices(const uint64_t& xGatherDimSlice, const uint64_t& idxGatherDimSlice)
    {
        ...
        if constexpr (std::is_same_v<T_IDX, int64_t>) {
            LocalTensor<int32_t> idx32Local = idxLocal.template ReinterpretCast<int32_t>();
            Cast(idx32Local, idxLocal, RoundMode::CAST_NONE, idxGatherDimSlice);
            PipeBarrier<PIPE_V>();
            constexpr uint64_t IDX32_ALIGN = V2_BLOCK_SIZE / sizeof(int32_t);
            uint64_t idx32CountAlign = CeilAlign(idxGatherDimSlice, IDX32_ALIGN);
            ShiftRight(idx32Local[idx32CountAlign], idx32Local, RIGHT_SHIFT_LEN, idxGatherDimSlice);
            Muls(idx32Local[idx32CountAlign], idx32Local[idx32CountAlign], -static_cast<int32_t>(this->xGatherDim_),
                 idxGatherDimSlice);
            Add(idx32Local, idx32Local, idx32Local[idx32CountAlign], idxGatherDimSlice);
        } else {
            ...
            Muls(idxLocal[idxCountAlign], idxLocal[idxCountAlign], -static_cast<T_IDX>(this->xGatherDim_),
                 idxGatherDimSlice);
...
#endif // GATHER_ELEMENTS_TRANSPOSE_H
