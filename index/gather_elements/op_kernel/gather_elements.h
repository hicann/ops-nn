...
#include "gather_elements_tiling_data.h"
...
constexpr int64_t Tiling_MODE_SCALAR = 11;
constexpr int64_t Tiling_MODE_FOR_LAST_AXIS_ALLDATA = 12;
...
public:
    ...
    {
        if (tilingMode_ >= Tiling_MODE_FOR_LAST_AXIS && tilingMode_ <= Tiling_MODE_FOR_LAST_AXIS_ALLDATA) {
            ProcessLastAxis();
            return;
        }
        ProcessBatch();
...
private:
    ...
    __aicore__ inline void ProcessLastAxis()
    {
        ...
        bool useGather = (tilingMode_ == Tiling_MODE_FOR_LAST_AXIS_GATHER) ||
                         (tilingMode_ == Tiling_MODE_FOR_LAST_AXIS_ALLDATA);
        ...
        if (useGather || isCut) {
            ...
            if (!isCut && tilingMode_ != Tiling_MODE_FOR_LAST_AXIS_ALLDATA) {
        ...
        for (int64_t t = blockIdx; t < numTasks; t += coreNum) {
            ...
            auto idxUb = idxBuf.Get<INDEX_T>();
            auto resUb = resBuf.Get<X_T>();
            LocalTensor<int32_t> idx32Ub;
            if (useGather || isCut) {
                idx32Ub = idx32Buf.Get<int32_t>();
            }
            ...
            if (useGather && IsGatherSupportedDtype<X_T>()) {
                ...
                if constexpr (std::is_same<INDEX_T, int64_t>::value) {
                    ...
                    idx32View = idx32Ub;
                    const uint32_t signTmpOff = DivCeil(static_cast<uint32_t>(nElemMax), blockSizeIdx32) *
                                                blockSizeIdx32;
                    signTmp = idx32Ub[signTmpOff];
                } else {
                ...
                if (tilingMode_ != Tiling_MODE_FOR_LAST_AXIS_ALLDATA) {
