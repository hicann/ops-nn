...
// tiling mode for pure scalar fallback, supporting x/index shape not fully identical
const int64_t TILING_MODE_SCALAR = 11;
// tiling mode when only a single row (allDataSize) fits in UB while the whole repeat batch does not
const int64_t TILING_MODE_FOR_LAST_AXIS_ALLDATA = 12;
...
private:
    ...
    ge::graphStatus ComputeScalarTiling();
...
bool GatherElementsTiling::ChooseTilingModeForLastAxis(bool ifSameDimValueExceptAxis)
{
    ...
    int64_t allDataSize = tilingData_.get_params_axis() * commonInformation_.params_dsize +
                          tilingData_.get_indices_axis() * (commonInformation_.params_dsize +
                                                            commonInformation_.indices_dsize * static_cast<int64_t>(2));
    ...
    } else if ((supportGather_ == 1) && (allDataSize < availableUbSize) && ifSameDimValueExceptAxis) {
        // Shape falls between allDataSize (single row) and lastAxisUbSize (whole batch):
        // only one row fits in UB, so process row by row using only allDataSize buffer.
        tilingData_.set_tilingMode(TILING_MODE_FOR_LAST_AXIS_ALLDATA);
        tilingData_.set_dbFlag(allDataSize < availableUbSize / HALF ? 1 : 0);
        // Recompute tiling for one row per task; kernel allocates allDataSize UB only.
        tilingData_.set_repeat_per_core(1);
        tilingData_.set_rounds(commonInformation_.indices_pre);
        tilingData_.set_rounds_tail(0);
        tilingData_.set_indices_loop_num(tilingData_.get_rounds() / coreNumAll_);
        tilingData_.set_need_core_num(tilingData_.get_rounds() > coreNumAll_ ? coreNumAll_ : tilingData_.get_rounds());
        tilingData_.set_indices_row_num_last(tilingData_.get_rounds() % coreNumAll_);
    } else {
        allDataSize = lastDimSize;
    }
    OP_TILING_CHECK(
        allDataSize >= availableUbSize,
...
// scalar fallback: pure per-element copy from gm x to gm y, works for x/index shape not fully identical
ge::graphStatus GatherElementsTiling::ComputeScalarTiling()
{
    tilingData_.set_tilingMode(TILING_MODE_SCALAR);
    int64_t indicesNum = tilingData_.get_indices_num();
    int64_t needCoreNum = std::min(indicesNum, coreNumAll_);
    needCoreNum = std::max(needCoreNum, static_cast<int64_t>(1));
    tilingData_.set_need_core_num(needCoreNum);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus GatherElementsTiling::Init()
{
    ...
    if (!ChooseTilingMode(IfParamsIndicesSameShapeExceptAxis())) {
        OP_LOGW(tilingContext_->GetNodeName(), "choose tiling mode failed, route to scalar branch.");
        return ComputeScalarTiling();
    }
...
{
    // scalar 分支只打印自身相关的 tiling 数据：core 数量、shape、axis 等
    if (tilingData_.get_tilingMode() == TILING_MODE_SCALAR) {
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] tilingMode=%ld.", tilingData_.get_tilingMode());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] axis=%ld.", tilingData_.get_axis());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] dims=%ld.", tilingData_.get_dims());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] need_core_num=%ld.",
                tilingData_.get_need_core_num());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_num=%ld.", tilingData_.get_indices_num());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_0=%ld.",
                tilingData_.get_params_shape_0());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_1=%ld.",
                tilingData_.get_params_shape_1());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_2=%ld.",
                tilingData_.get_params_shape_2());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_3=%ld.",
                tilingData_.get_params_shape_3());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_4=%ld.",
                tilingData_.get_params_shape_4());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_5=%ld.",
                tilingData_.get_params_shape_5());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_6=%ld.",
                tilingData_.get_params_shape_6());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] params_shape_7=%ld.",
                tilingData_.get_params_shape_7());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_0=%ld.",
                tilingData_.get_indices_shape_0());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_1=%ld.",
                tilingData_.get_indices_shape_1());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_2=%ld.",
                tilingData_.get_indices_shape_2());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_3=%ld.",
                tilingData_.get_indices_shape_3());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_4=%ld.",
                tilingData_.get_indices_shape_4());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_5=%ld.",
                tilingData_.get_indices_shape_5());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_6=%ld.",
                tilingData_.get_indices_shape_6());
        OP_LOGD(tilingContext_->GetNodeName(), "[tilingData-scalar] indices_shape_7=%ld.",
                tilingData_.get_indices_shape_7());
        return;
    }
