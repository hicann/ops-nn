...
template <typename T>
struct IndexTypeGet {
    using type = typename std::conditional<sizeof(T) == sizeof(int8_t) || sizeof(T) == sizeof(int16_t), uint16_t,
                                           uint32_t>::type;
};
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR indices, GM_ADDR axis, GM_ADDR y,
                                const GatherV2LastTilingData* tilingData);
    ...
    __aicore__ inline void GatherComputeMultiRows(const LocalTensor<X_T>& xLocal,
                                                  const LocalTensor<int32_t>& indicesLocal, int32_t rows, int32_t cols,
                                                  int64_t yOffset);
...
template <typename X_T, typename INDICES_T, bool NEG_INDICE_SUPPORT>
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::Init(
    GM_ADDR x, GM_ADDR indices, GM_ADDR axis, GM_ADDR y, const GatherV2LastTilingData* tilingData)
{
    ...
    yGm_.SetGlobalBuffer((__gm__ X_T*)y + pBlock_ * tilingData->blockFactor * tilingData->gatherSize +
                         gBlock_ * tilingData->gFactor);
    ...
    curGatherFactor_ = (gBlock_ == tilingData->coreInCols - 1) ?
                           tilingData->gatherSize - (tilingData->coreInCols - 1) * tilingData->gFactor :
                           tilingData->gFactor;
}

template <typename X_T, typename INDICES_T, bool NEG_INDICE_SUPPORT>
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::CopyInIndices(int64_t offset,
                                                                                                 uint32_t nBurst,
                                                                                                 uint32_t copyLen)
{
...
template <typename X_T, typename INDICES_T, bool NEG_INDICE_SUPPORT>
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::CopyInX(int64_t offset,
                                                                                           uint32_t nBurst,
                                                                                           uint32_t copyLen)
{
...
template <typename X_T, typename INDICES_T, bool NEG_INDICE_SUPPORT>
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::CopyOut(int64_t offset,
                                                                                           uint32_t nBurst,
                                                                                           uint32_t copyLen)
{
...
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::SplitIndicesProcess()
{
    ...
    int64_t curBlockFactor = (GetBlockIdx() == tilingData_->needCoreNum - 1) ? tilingData_->tailBlockFactor :
    ...
    for (int64_t i = 0; i < inputLoop; i++) {
        ...
        for (int64_t j = 0; j < outLoopNum; j++) {
            ...
            if constexpr (sizeof(INDICES_T) == sizeof(int64_t)) {
                ...
                GatherComputeMultiRows(xLocal, tmpLocal, curInputFactor, curOutCols,
                                       i * inputFactor * tilingData_->gatherSize + j * maxOutCols);
            } else {
                ConvertNegIndices(indicesLocal, curOutCols);
                GatherComputeMultiRows(xLocal, indicesLocal, curInputFactor, curOutCols,
                                       i * inputFactor * tilingData_->gatherSize + j * maxOutCols);
                indexQue_.FreeTensor(indicesLocal);
            }
        }
...
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::NoSplitIndicesProcess()
{
    ...
    for (int64_t i = 0; i < inputLoop; i++) {
        ...
        LocalTensor<X_T> xLocal = inQueue_.DeQue<X_T>();
        if constexpr (sizeof(INDICES_T) == sizeof(int64_t)) {
            GatherComputeMultiRows(xLocal, tmpLocal, curInputFactor, curGatherFactor_,
                                   i * inputFactor * tilingData_->gatherSize);
        } else {
            GatherComputeMultiRows(xLocal, indicesLocal, curInputFactor, curGatherFactor_,
                                   i * inputFactor * tilingData_->gatherSize);
        }
        inQueue_.FreeTensor(xLocal);
    }
...
__simd_vf__ inline void GatherComputeMultiRowsVF(__ubuf__ int32_t* indiceAddr, __ubuf__ X_T* xAddr, __ubuf__ X_T* yAddr,
...
template <typename X_T, typename INDICES_T, bool NEG_INDICE_SUPPORT>
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::GatherComputeMultiRows(
    const LocalTensor<X_T>& xLocal, const LocalTensor<int32_t>& indicesLocal, int32_t rows, int32_t cols,
    int64_t yOffset)
{
    __ubuf__ X_T* xAddr = (__ubuf__ X_T*)xLocal.GetPhyAddr();
    __ubuf__ int32_t* indiceAddr = (__ubuf__ int32_t*)indicesLocal.GetPhyAddr();
    ...
    for (int32_t colIdx = 0; colIdx < loopInCols; colIdx++) {
        ...
        __ubuf__ X_T* yAddr = (__ubuf__ X_T*)yLocal.GetPhyAddr();
        ...
        GatherComputeMultiRowsVF<X_T, indiceType>(indiceAddr, xAddr, yAddr, curCols, indicesOffset, vfLoopNum, dimSize,
                                                  vfLen, vfRowsLoop, colInUb);
...
template <typename X_T, typename INDICES_T, bool NEG_INDICE_SUPPORT>
__aicore__ inline void Gatherv2SimdLastGather<X_T, INDICES_T, NEG_INDICE_SUPPORT>::ConvertNegIndices(
    const LocalTensor<int32_t>& indicesLocal, int32_t num)
{
