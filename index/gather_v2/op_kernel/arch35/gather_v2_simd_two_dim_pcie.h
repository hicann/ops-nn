...
/*!
 * \file gather_v2_simd_tow_dim_pcie.h
 * \brief
 */
#ifndef GATHER_V2_SIMD_TWO_DIM_PCIE_H
#define GATHER_V2_SIMD_TWO_DIM_PCIE_H
...
constexpr int32_t BUFFER_NUM_SIMD_TWO_DIM_PCIE = 2;
constexpr int64_t INDICES_SIZE_TWO_DIM = 8192;

template <typename INDICES_T>
class Gatherv2SimdTwoDimPcie {
public:
    __aicore__ inline Gatherv2SimdTwoDimPcie(TPipe* pipe) : pipe_(pipe){};
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR indices, GM_ADDR axis, GM_ADDR y,
                                const GatherV2TilingDataSimtTwoDim* tilingData);
...
private:
    ...
    TPipe* pipe_;
    TQueBind<QuePosition::VECIN, QuePosition::VECOUT, BUFFER_NUM_SIMD_TWO_DIM_PCIE> inQueue_;
    TBuf<QuePosition::VECCALC> indexBuf_;
    const GatherV2TilingDataSimtTwoDim* tilingData_;

    int64_t indicesOffsetBase_ = 0;
    int64_t curIndexSize_ = 0;
    int64_t needCoreNum_ = 1;
    int32_t indiceFactor_ = 0;
    int64_t blockFactor_ = 0;
    int64_t tailBlockFactor_ = 0;
    int64_t ubAviable_ = 0;
};

template <typename INDICES_T>
__aicore__ inline void Gatherv2SimdTwoDimPcie<INDICES_T>::Init(GM_ADDR x, GM_ADDR indices, GM_ADDR axis, GM_ADDR y,
                                                           const GatherV2TilingDataSimtTwoDim* tilingData)
{
    ...
    blockFactor_ = tilingData_->gatherSize / aivNum;
    tailBlockFactor_ = tilingData_->gatherSize - blockFactor_ * aivNum;
    int64_t ubBlockSize = static_cast<int64_t>(Ops::Base::GetUbBlockSize());
    ubAviable_ = (ubSize - INDICES_SIZE_TWO_DIM) / ubBlockSize * ubBlockSize / tilingData_->dtypeSize / BUFFER_NUM_SIMD_TWO_DIM_PCIE;
    indiceFactor_ = INDICES_SIZE_TWO_DIM / sizeof(INDICES_T);
    needCoreNum_ = blockFactor_ > 0 ? aivNum : tailBlockFactor_;
    ...
    pipe_->InitBuffer(inQueue_, BUFFER_NUM_SIMD_TWO_DIM_PCIE, ubAviable_ * tilingData_->dtypeSize);
    pipe_->InitBuffer(indexBuf_, indiceFactor_ * sizeof(INDICES_T));
}

template <typename INDICES_T>
template <typename T>
__aicore__ inline void Gatherv2SimdTwoDimPcie<INDICES_T>::CopyIn(LocalTensor<T> xLocal, GlobalTensor<T> xGm, int64_t offset,
...
template <typename INDICES_T>
__aicore__ inline void Gatherv2SimdTwoDimPcie<INDICES_T>::CopyOut(int64_t offset, uint32_t nBurst, uint32_t copyLen)
{
    DataCopyExtParams dataCoptExtParams;
    dataCoptExtParams.blockCount = nBurst;
...
template <typename INDICES_T>
__aicore__ inline INDICES_T Gatherv2SimdTwoDimPcie<INDICES_T>::GetIndex(int64_t idx, int64_t endIdx)
{
    if (idx >= indicesOffsetBase_ + curIndexSize_) {
        int64_t copyLen = (endIdx - idx) > indiceFactor_ ? indiceFactor_ : endIdx - idx;
...
template <typename INDICES_T>
__aicore__ inline void Gatherv2SimdTwoDimPcie<INDICES_T>::NoSplitColProcess(int64_t colsAlign)
{
    int64_t yStart = 0;
    int64_t yEnd = 0;
    if (GetBlockIdx() < tailBlockFactor_) {
        yStart = (blockFactor_ + 1) * GetBlockIdx();
        yEnd = yStart + blockFactor_ + 1;
    } else {
        yStart = blockFactor_ * GetBlockIdx() + tailBlockFactor_;
        yEnd = yStart + blockFactor_;
    }
    int64_t currentCoreElements = yEnd - yStart;
    int64_t onceMaxRows = ubAviable_ * tilingData_->dtypeSize / colsAlign;
...
template <typename INDICES_T>
__aicore__ inline void Gatherv2SimdTwoDimPcie<INDICES_T>::SplitColProcess(int64_t colsAlign)
{
    // indice -> g
    // output -> g, i
    int64_t yStart = 0;
    ...
    int64_t indiceEndIdx = yEnd;
    int64_t loopSize = (tilingData_->innerSize + ubAviable_ - 1) / ubAviable_;
    for (int64_t i = yStart; i < yEnd; i++) {
        ...
        for (int64_t j = 0; j < loopSize; j++) {
            int64_t cols = (j == loopSize - 1) ? (tilingData_->innerSize - j * ubAviable_) :
                                                 ubAviable_;
            LocalTensor<int8_t> xLocal = inQueue_.AllocTensor<int8_t>();
            int64_t offset = (xIndex + j * ubAviable_) * tilingData_->dtypeSize;
            if (likely(index >= 0 && index < tilingData_->gatherDimSize)) {
            ...
            inQueue_.EnQue<int8_t>(xLocal);
            int64_t yOffset = (yIndex + j * ubAviable_) * tilingData_->dtypeSize;
...
template <typename INDICES_T>
__aicore__ inline void Gatherv2SimdTwoDimPcie<INDICES_T>::Process()
{
    if (static_cast<int32_t>(GetBlockIdx()) >= needCoreNum_) {
        return;
    }
    int64_t ubBlockSize = static_cast<int64_t>(Ops::Base::GetUbBlockSize());
    int64_t colsAlign = (tilingData_->innerSize * tilingData_->dtypeSize + ubBlockSize - 1) / ubBlockSize * ubBlockSize;

    if (colsAlign <= ubAviable_ * tilingData_->dtypeSize) {
...
#endif // GATHER_V2_SIMD_TWO_DIM_PCIE_H
