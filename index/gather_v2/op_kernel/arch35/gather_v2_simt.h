...
private:
    ...
    static __simt_vf__ __aicore__ LAUNCH_BOUND(THREAD_NUM_LAUNCH_BOUND) inline void GatherSimt(
        const INDEX_SIZE_T yIndexBase, INDEX_SIZE_T currentCoreElements, INDEX_SIZE_T m0, INDEX_SIZE_T shift0,
        INDEX_SIZE_T m1, INDEX_SIZE_T shift1, INDEX_SIZE_T m2, INDEX_SIZE_T shift2, INDEX_SIZE_T gatherSize,
        INDEX_SIZE_T outerSize, INDEX_SIZE_T innerSize, INDEX_SIZE_T gatherDimSize, __gm__ X_T* x,
        __gm__ INDICES_T* indices, __gm__ volatile X_T* y);
...
LAUNCH_BOUND(THREAD_NUM_LAUNCH_BOUND) inline void Gatherv2<X_T, INDICES_T, INDEX_SIZE_T>::GatherSimt(
    const INDEX_SIZE_T yIndexBase, INDEX_SIZE_T currentCoreElements, INDEX_SIZE_T m0, INDEX_SIZE_T shift0,
    INDEX_SIZE_T m1, INDEX_SIZE_T shift1, INDEX_SIZE_T m2, INDEX_SIZE_T shift2, INDEX_SIZE_T gatherSize,
    INDEX_SIZE_T outerSize, INDEX_SIZE_T innerSize, INDEX_SIZE_T gatherDimSize, __gm__ X_T* x,
    __gm__ INDICES_T* indices, __gm__ volatile X_T* y)
...
template <typename X_T, typename INDICES_T, typename INDEX_SIZE_T>
__aicore__ inline void Gatherv2<X_T, INDICES_T, INDEX_SIZE_T>::Init(GM_ADDR x, GM_ADDR indices, GM_ADDR axis, GM_ADDR y,
                                                                    const GatherV2TilingData* tilingData)
{
...
__aicore__ inline void Gatherv2<X_T, INDICES_T, INDEX_SIZE_T>::Process()
{
    ...
    if (blockIdx < needCoreNum) {
        ...
        if (unlikely(negativeIndexSupport)) {
            asc_vf_call<GatherSimt<true>>(
                dim3(threadNum), yIndexBase, currentCoreElements, m0, shift0, m1, shift1, m2, shift2, gatherSize,
                outerSize, innerSize, gatherDimSize, (__gm__ X_T*)(xGm_.GetPhyAddr()),
                (__gm__ INDICES_T*)(indicesGm_.GetPhyAddr()), (__gm__ volatile X_T*)(yGm_.GetPhyAddr()));
        } else {
            asc_vf_call<GatherSimt<false>>(
                dim3(threadNum), yIndexBase, currentCoreElements, m0, shift0, m1, shift1, m2, shift2, gatherSize,
                outerSize, innerSize, gatherDimSize, (__gm__ X_T*)(xGm_.GetPhyAddr()),
                (__gm__ INDICES_T*)(indicesGm_.GetPhyAddr()), (__gm__ volatile X_T*)(yGm_.GetPhyAddr()));
        }
