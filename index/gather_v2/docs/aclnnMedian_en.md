...
Description: Returns the median of all elements (if the total number of elements is size, the median corresponds to the sorted subscript `(size - 1) // 2`.

## Prototype
Each operator has [two-phase API](../../../docs/en/context/two_phase_api.md) calls. First, **aclnnMedianGetWorkspaceSize** is called to obtain the workspace size required for computation and the executor that contains the operator computation process. Then, **aclnnMedian** is called to perform computation.
...
- **Parameters**
  ...
  - **self** (aclTensor\*, compute input): input `self`, aclTensor on the device. The data type is the same as that of **valuesOut**. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
  ...
  - **valuesOut** (aclTensor\*, output): output `valuesOut`, zero-dimensional aclTensor on the device, with the same data type as **self**. [Non-contiguous tensors](../../../docs/en/context/non_contiguous_tensors.md) are supported. The [data format](../../../docs/en/context/data_formats.md) can be ND.
     * <term>Atlas A2 training series products/Atlas A2 inference series products</term> and <term>Atlas A3 training series products/Atlas A3 inference series products</term>: FLOAT, FLOAT16, UINT8, INT8, INT16, INT32, INT64, or BFLOAT16.
     * <term>Atlas inference series products</term> and <term>Atlas training series products</term>: FLOAT, FLOAT16, UINT8, INT8, INT16, INT32, or INT64.
...
- **Returns**
  ...
  **aclnnStatus**: status code. For details, see [aclnn Return Codes](../../../docs/en/context/aclnn_return_codes_nn.md).
  ...
  161001 (ACLNN_ERR_PARAM_NULLPTR): 1. The passed self or valuesOut is a null pointer.
  161002 (ACLNN_ERR_PARAM_INVALID): 1. The data type of self or valuesOut is not supported.
                                        2. The data types of self and valuesOut are inconsistent.
                                        3. The shape of valuesOut does not match the actual output shape.
  ```
...
int main() {
  ...
  // Call the first-phase API of aclnnMedian.
  ...
  // Call the second-phase API of aclnnMedian.
