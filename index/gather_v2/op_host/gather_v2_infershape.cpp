                               ...
                               const GatherInfo& gather_info)
{
    ...
    if (batch_dims < -rank_indices || (batch_dims > rank_indices && rank_indices != 0)) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            context->GetNodeName(), "batch_dims", std::to_string(batch_dims).c_str(),
            ("The value of batch_dims must be in the range [" + std::to_string(-rank_indices) + ", " +
             std::to_string(rank_indices - 1) + "]")
                .c_str());
        return false;
    }
    if (batch_dims < 0) {
    ...
    for (int i = 0; i < batch_dims; i++) {
        if (x_shape->GetDim(i) != indies_shape->GetDim(i) && x_shape->GetDim(i) > 0 && indies_shape->GetDim(i) > 0) {
            OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                context->GetNodeName(), "x", std::to_string(x_shape->GetDim(i)).c_str(),
                ("The " + std::to_string(i) + " axis of x must be == the " + std::to_string(i) + " axis " +
                 std::to_string(indies_shape->GetDim(i)) + " of indices")
                    .c_str());
            return false;
        }
    }
    ...
    OP_CHECK_IF(axes_data > 0 && (x_real_dim_cnt < axes_data + 1),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    context->GetNodeName(), "axis", std::to_string(axes_data).c_str(),
                    ("The value of axis must be less than " + std::to_string(x_real_dim_cnt)).c_str()),
                return false);
    if (axes_data < 0) {
        OP_CHECK_IF(
            x_real_dim_cnt < -axes_data,
            OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                context->GetNodeName(), "axis", std::to_string(axes_data).c_str(),
                ("The value of axis must be greater than or equal to " + std::to_string(-x_real_dim_cnt)).c_str()),
            return false);

        axes_data = x_real_dim_cnt + axes_data;
        OP_CHECK_IF(batch_dims > axes_data,
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context->GetNodeName(), "batch_dims",
                                                          std::to_string(batch_dims).c_str(),
                                                          "The value of batch_dims must be less than or equal to axis"),
                    return false);
    }
    return true;
}
                                         ...
                                         GatherInfo& gather_info)
{
    ...
    OP_CHECK_IF(!CheckAndUpdateAxis(context, batch_dims, axis, gather_info),
                OP_LOGD(context->GetNodeName(), "check axis and batchdim failed"), return ge::GRAPH_FAILED);
...
static ge::graphStatus InferShapeForGatherV2(gert::InferShapeContext* context)
{
    ...
    OP_CHECK_IF(
        axes_size < 1,
        OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(context->GetNodeName(), "axis", std::to_string(axes_size).c_str(),
