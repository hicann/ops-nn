...
static const std::set<ge::DataType> X_SUPPORT_DTYPE = {
    ...
    ge::DT_COMPLEX64, ge::DT_COMPLEX32, ge::DT_DOUBLE, ge::DT_FLOAT8_E5M2, ge::DT_FLOAT8_E8M0, ge::DT_FLOAT8_E4M3FN,
};
...
void Gatherv2TilingBase::Reset() { opName_ = nullptr; }
...
inline ge::graphStatus Gatherv2TilingBase::GetXInfoAndCheck()
{
    ...
    if (!IsSupportDtype(X_SUPPORT_DTYPE, xDtype_)) {
        OP_LOGE_FOR_INVALID_DTYPE(
            context_->GetNodeName(), "x", std::to_string(xDtype_).c_str(),
    ...
    if (xAxisNum == 0 && xShape_.GetShapeSize() != 0) {
        if (indicesShape_.GetShapeSize() != 1) {
            OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(
                context_->GetNodeName(), "index", std::to_string(indicesShape_.GetShapeSize()).c_str(),
                "The shape size of indices should be equal to 1 when x is scalar.");
        }
        xAxisNum = 1;
    }
    if (xAxisNum < min_x_dim) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context_->GetNodeName(), "x", std::to_string(xAxisNum).c_str(),
                                                 ("The shape of x must be >= " + std::to_string(min_x_dim)).c_str());
        return ge::GRAPH_FAILED;
    }
    if (axis_ < 0) {
    ...
    if (batchDims_ != 0) {
        if (batchDims_ < static_cast<int64_t>(-indicesShape_.GetDimNum()) ||
            ...
            OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                context_->GetNodeName(), "batch_dims", std::to_string(batchDims_).c_str(),
                ("The value of batch_dims must be in the range [" + std::to_string(-indicesShape_.GetDimNum()) + ", " +
                 std::to_string(indicesShape_.GetDimNum() - 1) + "]")
                    .c_str());
            return ge::GRAPH_FAILED;
        }
        if (batchDims_ < 0) {
        ...
        for (int32_t i = 0; i < batchDims_; i++) {
            if (xShape_.GetDim(i) != indicesShape_.GetDim(i)) {
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                    context_->GetNodeName(), "x", std::to_string(xShape_.GetDim(i)).c_str(),
                    ("The " + std::to_string(i) + " axis of x must be == the " + std::to_string(i) + " axis " +
                     std::to_string(indicesShape_.GetDim(i)) + " of indices")
                        .c_str());
                return ge::GRAPH_FAILED;
            }
        }
...
ge::graphStatus Gatherv2TilingBase::GetShapeAttrsInfo()
{
    ...
    OP_CHECK_IF(GetXInfoAndCheck() != ge::GRAPH_SUCCESS, OP_LOGD(opName_, "input x check failed."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(GetIndicesInfoAndCheck() != ge::GRAPH_SUCCESS, OP_LOGD(opName_, "input indices check failed."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(GetAxisInfoAndCheck() != ge::GRAPH_SUCCESS, OP_LOGD(opName_, "input axis check failed."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(GetAttrsInfoAndCheck() != ge::GRAPH_SUCCESS, OP_LOGD(opName_, "input attrs check failed."),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
{
    bool isGatherDimFullLoad = (gatherDimSize_ * improveDtypeSize_ <= ubSize_ / NUM_TWO ||
                                gatherDimSize_ * improveDtypeSize_ <= ubSize_ - NUM_TWO * MIN_OUT_UB_SIZE);
    bool isLastGather = innerSize_ == 1;
    bool isVGatherSupport = (improveDtypeSize_ <= static_cast<int32_t>(sizeof(int16_t))) ?
                                gatherDimSize_ < B8_AND_B16_GATHER_UPPER :
                                gatherDimSize_ < INT32_MAX;
    bool enableUbGather = batchSize_ == 1 && isLastGather && isGatherDimFullLoad && isVGatherSupport &&
                          gatherSize_ / gatherDimSize_ > 1 && gatherSize_ * improveDtypeSize_ >= 32 &&
                          improveDtypeSize_ <= static_cast<int32_t>(sizeof(int64_t));
...
bool Gatherv2TilingBase::IsGaAllLoad()
{
    ...
    int64_t remainSpace = ubSize_ - HELP_BUFFER_SIZE - NUM_FOUR * MIN_OUT_UB_SIZE - minIndicesSize * indicesDtypeSize -
                          gaSizeInUb;
...
bool Gatherv2TilingBase::IsAfterGdimFullLoad()
{
    ...
    bool enableFullLoad = (isInputAfterGdim && isAllInputSize && isMinOutputFullLoad && isInputAndOutputRepeat &&
                           isAllOutputSize && batchSize_ == 1 && outerSize_ == 1 && inputBatchDims_ == 0 &&
                           innerSize_ * improveDtypeSize_ < MAX_THREAD_NUM);
...
ge::graphStatus Gatherv2TilingBase::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }
...
uint64_t Gatherv2TilingBase::GetTilingKey() const
{
    ...
    if (tilingMode_ == TILING_SIMD_TWO_DIM) {
        ...
        bool needInt64Addr = static_cast<uint64_t>((gatherDimSize_ * innerSize_) > INT32_MAX || ySize_ > INT32_MAX);
        if (needInt64Addr) {
        ...
        if (tilingMode_ == TILING_AFTER_GDIM) {
            ...
            sizeAddrType = static_cast<uint64_t>(
                ((batchSize_ * outerSize_ * gatherDimSize_ * innerSize_) > INT32_MAX || ySize_ > INT32_MAX) ? 1 : 0);
        }
...
void Gatherv2TilingBase::CalcMaxUbcolAndIndiceFactor(int32_t& ubCols, int32_t ubRows, int32_t ubSize, int32_t minCols,
                                                     int32_t inputNum, int32_t outputNum, int32_t indiceNUm,
                                                     int32_t castUbRatio)
{
    ubCols = minCols;
    int32_t castTargetDtypeSize = static_cast<int32_t>(sizeof(int32_t));
    while (ubRows * (inputNum * improveDtypeSize_ * gatherDimSize_ + outputNum * ubCols * improveDtypeSize_) +
               indiceNUm * ubCols * indicesDtypeSize_ + castUbRatio * ubCols * castTargetDtypeSize <=
           ubSize) {
        ubCols += minCols;
    }
    ubCols -= minCols;
...
void Gatherv2TilingBase::LastGatherUbTiling(int32_t& inputUbSize, int32_t& indiceUbSize, int32_t& outUbSize,
                                            int32_t& indiceCastUbSize, int32_t& inputNum, int32_t& indicesNum,
                                            int32_t& ubCols, int32_t& ubRows, int64_t blockFactor, int64_t gFactor)
{
    int32_t minCols = ((improveDtypeSize_ == static_cast<int32_t>(sizeof(int64_t))) ? NUM_TWO * vRegSize_ : vRegSize_) /
                      improveDtypeSize_;
    int32_t indicesUbAlign = (improveDtypeSize_ <= static_cast<int32_t>(sizeof(int16_t))) ? NUM_TWO * vRegSize_ :
                                                                                            vRegSize_;
    indicesUbAlign = (indicesDtypeSize_ == static_cast<int32_t>(sizeof(int64_t))) ?
                         NUM_TWO * indicesUbAlign :
                         indicesUbAlign; // int64使用traitTwo模式
    // int64索引 cast为int32需要额外空间
    int32_t castUbRatio = (indicesDtypeSize_ == static_cast<int32_t>(sizeof(int64_t))) ? 1 : 0;
    int32_t castTargetDtypeSize = static_cast<int32_t>(sizeof(int32_t));
    // 通用场景， 都开db
    if (NUM_TWO * ((gatherDimSize_ * improveDtypeSize_ + ubBlockSize_) + minCols * improveDtypeSize_ +
                   (minCols * indicesDtypeSize_ + ubBlockSize_) + MIN_OUT_UB_SIZE) +
                (castUbRatio * minCols * castTargetDtypeSize + indicesUbAlign) <
            ubSize_ &&
        blockFactor > 1) {
        inputNum = NUM_TWO;
        int32_t tmpUbsize = ubSize_ - (inputNum + NUM_TWO) * ubBlockSize_ - indicesUbAlign;
        ubRows = ((tmpUbsize - castUbRatio * minCols * castTargetDtypeSize) / NUM_TWO - minCols * indicesDtypeSize_) /
                 (improveDtypeSize_ * gatherDimSize_ + minCols * improveDtypeSize_);
        ...
        ubRows = (tmpUbsize - NUM_TWO * minCols * indicesDtypeSize_ - castUbRatio * minCols * castTargetDtypeSize) /
                 improveDtypeSize_ / (gatherDimSize_ + NUM_TWO * minCols);
        ubRows = std::min(static_cast<int64_t>(ubRows), blockFactor);
        ubCols = minCols;
        CalcMaxUbcolAndIndiceFactor(ubCols, ubRows, tmpUbsize, minCols, inputNum, indicesNum, NUM_TWO, castUbRatio);
        while (ubRows * ubCols * improveDtypeSize_ < MIN_OUT_UB_SIZE && ubRows >= NUM_TWO && ubCols < gFactor) {
...
ge::graphStatus Gatherv2TilingBase::LastGatherTiling()
{
    ...
    LastGatherUbTiling(inputUbSize, indiceUbSize, outUbSize, indiceCastUbSize, inputNum, indicesNum, ubCols, ubRows,
                       blockFactor, gFactor);
...
ge::graphStatus Gatherv2TilingBase::GaAllLoadTiling()
{
    ...
    if (isAllLoadX) {
        ...
        indicesBufferSizeLoopGa = std::min(indicesInner * indicesDtypeSize,
                                           (ubSize_ - NUM_FOUR * MIN_OUT_UB_SIZE - allLoadXBytes));
        ...
        indicesBufferSizeLoopGa = std::min(indicesInner * indicesDtypeSize,
                                           (ubSize_ - NUM_FOUR * MIN_OUT_UB_SIZE - xBufferSizeLoopGa));
        indicesBufferSizeLoopGa = CeilAlign(indicesBufferSizeLoopGa, ubBlockSize);
        yBufferSizeLoopGa = (ubSize_ - indicesBufferSizeLoopGa - xBufferSizeLoopGa) / NUM_TWO;
    }
