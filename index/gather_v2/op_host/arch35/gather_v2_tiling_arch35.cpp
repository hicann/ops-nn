...
static const std::vector<std::vector<int64_t>> AD_Trustlist_RT = {
    {200, 50},  {200, 29}, {200, 1468}, {200, 7},  {200, 1},  {200, 10}, {200, 20},
    {200, 544}, {200, 23}, {200, 5},    {200, 11}, {200, 19}, {200, 8},  {32, 39}};
                                    ...
                                    int64_t params_dims, int64_t indices_dims)
{
    ...
    if (batch_dims != 0) {
        ...
        OP_CHECK_IF(batch_dims >= params_dims,
                    OP_LOGE("GatherV2:", "op GatherV2Tiling: batch_dims must be less than rank(params)."),
                    return false);
        OP_CHECK_IF(batch_dims > axis,
                    OP_LOGE("GatherV2:", "op GatherV2Tiling: batch_dims must be less than or equal to axis."),
                    return false);
        for (int64_t i = 0; i < batch_dims; i++) {
            if (xShape.GetDim(i) != indiesShape.GetDim(i)) {
                OP_LOGE("GatherV2", "op GatherV2Tiling: Params.shape[:batch_dims] "
...
bool ProcessDirectGatherTmpl(GatherV2TilingParams* params, const GatherV2CompileInfo* compile_info,
                             int64_t available_ub_size)
{
    ...
    params->median_core_compute_size = compute_size_of_post_core * INT_PER_BLOCK_NUM + last_time_reserved_size -
                                       (last_time_count_num - 1) * INT_PER_BLOCK_NUM;
                              ...
                              const GatherV2CompileInfo* compile_info)
{
    ...
    OP_CHECK_IF((align_param_row * TRANS_POSE_LINE_SIZE > six_part_ub_size),
                OP_LOGD("GatherV2",
                          ...
                          const GatherV2TilingParams* params, const GatherV2CompileInfo* compile_info)
{
    ...
    if (one_param_row_size % BLOCK_SIZE != 0) {
        OP_CHECK_IF((align_param_row * TRANS_POSE_LINE_SIZE > one_part_ub_size),
                    OP_LOGD("GatherV2",
                            "[DoCpuPreprocessCheck] no need cache mode, but align_param_row is %ld, params_row is %ld",
                            align_param_row, params->params_row),
                    return false);
        ...
        OP_CHECK_IF((align_param_row > one_part_ub_size),
                    OP_LOGD("GatherV2",
                            "[DoCacheModeTilingwithCpuPreprocess] no need cache mode, but align_param_row is %ld, "
                            "params_row is %ld",
                            align_param_row, params->params_row),
                    return false);
...
static bool DoImplModeTiling(GatherV2TilingParams* params, const GatherV2CompileInfo* compile_info)
{
    OP_CHECK_IF(
        ...
        return false);
    OP_CHECK_IF(params->params_total * compile_info->params_dsize <= PARAMS_CACHED_UB,
...
{
    OP_CHECK_IF(indices_num_per_loop == 0, OP_LOGE("GatherV2:", "indices_num_per_loop = 0 is not support"),
                return false);
                               ...
                               int64_t params_d_size, int64_t block_num)
{
    OP_CHECK_IF(indices_num_per_loop == 0 || block_num == 0,
                OP_LOGE("GatherV2:", "indices_num_per_loop or block_num = 0 is not support"), return false);
    params->indices_loop_num = params->indices_num_each_core / indices_num_per_loop;
    params->indices_row_num_once = indices_num_per_loop;
    if (params->indices_num_each_core % params->indices_row_num_once != 0) {
        params->indices_row_num_last = params->indices_num_each_core % params->indices_row_num_once;
...
{
    OP_CHECK_IF(indices_num_per_loop == 0 || block_num == 0,
                OP_LOGE("Gather Tiling:", "indices_num_per_loop or block_num = 0 is not support"), return false);
    params->indices_loop_num = params->indices_num_each_core / indices_num_per_loop;
    params->indices_row_num_once = indices_num_per_loop;
    if (params->indices_num_each_core % params->indices_row_num_once != 0) {
        params->indices_row_num_last = params->indices_num_each_core % params->indices_row_num_once;
                            ...
                            GatherV2TilingParams* params, int64_t axis, int64_t paramsDims)
{
    ...
    if (params->params_row == 1 &&
        (compile_info->params_dsize == sizeof(int16_t) || compile_info->params_dsize == sizeof(int32_t)) &&
        compile_info->socVersion != 0 && compile_info->indices_dsize == sizeof(int32_t) &&
        ProcessDirectGatherTmpl(params, compile_info, available_ub_size)) {
        return true;
    }
...
{
    OP_CHECK_IF(half_ub_params_elem == 0, OP_LOGE("GatherV2:", "half_ub_params_elem = 0 is not support"), return false);
    params->one_row_loop = (params->params_row) / half_ub_params_elem;
    params->one_row_tail = (params->params_row) % half_ub_params_elem;
    int64_t block_num = BLOCK_SIZE / (compile_info->params_dsize);
    if ((params->one_row_loop) > 0 && (params->one_row_tail) > 0 && (params->one_row_tail) < block_num) {
...
{
    OP_CHECK_IF(params_d_size == 0, OP_LOGE("GatherV2:", "params_d_size= 0 is not support"), return false);
    params->indices_row_num_once = indices_num_per_loop;
    params->row_num_once_ub = res_ub_size / ((params->params_row) * params_d_size);
                        ...
                        int64_t params_row_ceil)
{
    ...
    if ((params->indices_row) * (params->params_row) * (compile_info->params_dsize) <= BLOCK_SIZE) {
        if ((params->params_pre) * (params->indices_row) * (params->params_row) * (compile_info->params_dsize) <=
            ...
            params->params_total = (params->params_batch_each_core) * (params->params_pre) * (params->params_axis) *
                                   (params->params_row);
            ...
            params->need_core_num = ((params->params_pre) < (compile_info->core_num)) ? (params->params_pre) :
                                                                                        (compile_info->core_num);
            params->params_total = (params->params_batch) * (params->params_pre) * (params->params_axis) *
                                   (params->params_row);
