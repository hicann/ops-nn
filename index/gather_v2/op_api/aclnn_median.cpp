...
static bool CheckShapeDim(const aclTensor* self, bool keepDim, const aclTensor* valuesOut, const aclTensor* indicesOut)
{
    OP_CHECK_MAX_DIM(self, MAX_SUPPORT_DIMS_NUMS, return false);
    OP_CHECK_MAX_DIM(valuesOut, MAX_SUPPORT_DIMS_NUMS, return false);
...
static std::tuple<const aclTensor*, const aclTensor*> GetMedianSortResult(const aclTensor* self, int64_t dim,
                                                                          aclOpExecutor* executor)
{
    // 调用Sort算子
