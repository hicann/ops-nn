...
static inline bool CheckOutShape(const aclTensor* self, int64_t dim, const aclTensor* index, int64_t batchDims,
                                 const aclTensor* out)
{
    // 根据算子语义，推导算子输出shape
    Shape outShape;
    ...
    if (Ops::NN::AclnnUtil::IsRegbase(curArch)) {
        if (self->GetViewShape().GetDimNum() == 0) {
            ...
            for (size_t i = batchDims; i < index->GetViewShape().GetDimNum(); i++) {
...
static inline bool CheckShape(const aclTensor* self, int64_t dim, const aclTensor* index, int64_t batchDims,
                              const aclTensor* out)
{
    ...
    return CheckOutShape(self, dim, index, batchDims, out);
}

static inline bool CheckDimAndBatchdims(const aclTensor* self, int64_t dim, const aclTensor* index, int64_t batchDims)
{
    int64_t indicesDimSize = static_cast<int64_t>(index->GetViewShape().GetDimNum());
    int64_t selfDimSize = static_cast<int64_t>(self->GetViewShape().GetDimNum());
    if (selfDimSize == 0) {
        selfDimSize = 1;
    }
    ...
    if (batchDims != 0) {
        if (batchDims < -indicesDimSize || batchDims > indicesDimSize) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "aclnnGatherV3, batchDims is invalid.");
            return false;
        }
        if (batchDims < 0) {
            batchDims += indicesDimSize;
        }
        if (batchDims >= selfDimSize) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "aclnnGatherV3, batchDims must be less than rank(self).");
            return false;
        }
        if (batchDims > dim) {
            OP_LOGE(ACLNN_ERR_PARAM_INVALID, "aclnnGatherV3, batchDims must be less than or equal to dim.");
            return false;
        }
        for (int64_t i = 0; i < batchDims; i++) {
            if (self->GetViewShape().GetDim(i) != index->GetViewShape().GetDim(i)) {
                OP_LOGE(ACLNN_ERR_PARAM_INVALID,
                        "aclnnGatherV3, self.shape[:batch_dims] should be equal to indices.shape[:batch_dims].");
                return false;
            }
        }
    }
