    ...
    for (size_t i = 0; i < axes_num; ++i) {
        if (!Ops::Nn::IsDimValid(input_x_rank, axes_value[i])) {
            OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                context->GetNodeName(), "axis", std::to_string(axes_value[i]).c_str(),
        ...
        if (run_info.reverse_status[real_axes]) {
            OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context->GetNodeName(), "axis", "axes is invalid",
                                                  "a specified dim can not be reversed more than once!");
            return false;
        }
                             ...
                             ReverseRunInfo& run_info)
{
    ...
    OP_CHECK_NULL_WITH_CONTEXT(context, axes_tensor);
    OP_CHECK_IF(!Ops::Nn::IsConstTensor(axes_tensor), OP_LOGE(context->GetNodeName(), "get axes const value failed."),
                return false);
...
static void MergedAxes(ReverseRunInfo& run_info)
{
    ...
    for (int64_t i = run_info.real_shape_size - 2; i >= 0; --i) {
        ...
        if (reverse_status == run_info.reverse_status[run_info.merged_wr_idx]) {
            ...
            run_info.reverse_shape[run_info.merged_wr_idx] = run_info.reverse_shape[run_info.merged_wr_idx] *
                                                             reverse_dim;
            continue;
        }
...
static void DoInnerCut(const ReverseRunInfo& run_info, const int64_t tiling_key, ReverseInnerCutInfo& inner_cut_info)
{
    ...
    for (int64_t i = run_info.reverse_shape.size() - 1; i >= run_info.merged_wr_idx; --i) {
        if (inner_cut_info.inner_align_count == 0) {
            ...
            inner_cut_info.inner_align_count = (run_info.reverse_shape[i] + F16_NUM_ONE_BLOCK - 1) / F16_NUM_ONE_BLOCK *
                                               F16_NUM_ONE_BLOCK;
            if (is_reverse_with_last_dim && reverse_last_dim_num > THRESHOLD_128) {
                inner_cut_info.inner_align_count = (run_info.reverse_shape[i] + F16_NUM_FOR_VNHWC - 1) /
                                                   F16_NUM_FOR_VNHWC * F16_NUM_FOR_VNHWC;
            }
                          ...
                          const ReverseInnerCutInfo& inner_cut_info, ResizeV2TilingData* tiling_data_ptr)
{
    ...
    for (int64_t i = 0; i < tiling_data_ptr->inner_real_dims; ++i) {
        tiling_data_ptr
            ->inner_shape_array[tiling_array_size - static_cast<size_t>(1) - static_cast<size_t>(i)] = static_cast<
            int64_t>(run_info.reverse_shape[run_info.reverse_shape.size() - 1 - i]);
        tiling_data_ptr
            ->inner_axis_array[tiling_array_size - static_cast<size_t>(1) - static_cast<size_t>(i)] = static_cast<
            int64_t>(run_info.reverse_status[run_info.reverse_status.size() - 1 - i]);
    }
    ...
    for (int64_t i = 0; i < tiling_data_ptr->outer_real_dims; ++i) {
        tiling_data_ptr
            ->outer_shape_array[tiling_array_size - static_cast<size_t>(1) - static_cast<size_t>(i)] = static_cast<
            int64_t>(run_info.reverse_shape[inner_cut_info.inner_first_dim - 1 - i]);
        tiling_data_ptr
            ->outer_axis_array[tiling_array_size - static_cast<size_t>(1) - static_cast<size_t>(i)] = static_cast<
            int64_t>(run_info.reverse_status[inner_cut_info.inner_first_dim - 1 - i]);
    }
    tiling_data_ptr->is_split_axis_reverse = tiling_data_ptr->inner_axis_array[tiling_array_size -
                                                                               tiling_data_ptr->inner_real_dims];
    tiling_data_ptr
        ->split_part_num = inner_cut_info.inner_max_count /
                           (inner_cut_info.inner_align_count /
                            tiling_data_ptr->inner_shape_array[tiling_array_size - tiling_data_ptr->inner_real_dims]);
    tiling_data_ptr->split_dim = tiling_data_ptr
                                     ->inner_shape_array[tiling_array_size - tiling_data_ptr->inner_real_dims];
...
static ge::graphStatus Tiling4ReverseV2(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(!GetAxisToReverseRunInfo(context, x_shape, input_axis, run_info),
                OP_LOGE(context->GetNodeName(), "do GetAxisToReverseRunInfo failed."), return ge::GRAPH_FAILED);
...
IMPL_OP_OPTILING(ReverseV2).Tiling(Tiling4ReverseV2).TilingParse<ReverseV2CompileInfo>(TilingPrepare4ReverseV2);
} // namespace optiling
