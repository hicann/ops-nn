...
- 算子功能：实现兼容tf.compat.v1.scatter_div的功能，将tensor updates中的值按指定的索引tensor indices逐切片除到tensor varRef的切片上。若有多于一个updates值作用到varRef的同一个切片，那么这些值将会在这一切片上连除。规则如下：
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnScatterDivGetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnScatterDiv”接口执行计算。
...
## aclnnScatterDivGetWorkspaceSize
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>updates</td>
      <td>输入</td>
      <td>公式中的输入updates，要除到varRef中的更新值。</td>
      <td>-</td>
      <td>FLOAT16、FLOAT32、INT32、INT8、UINT8</td>
      <td>ND</td>
      <td>1-8</td>
...
- **参数说明**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnScatterDivGetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
#include <vector>
#include "acl/acl.h"
#include "aclnnop/aclnn_scatter_div.h"
...
int main() {
  ...
  aclOpExecutor* executor;
  // 调用aclnnScatterDiv第一段接口
  ret = aclnnScatterDivGetWorkspaceSize(varRef, indices, updates, useLocking, &workspaceSize, &executor);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnScatterDivGetWorkspaceSize failed. ERROR: %d\n", ret); return ret);
  ...
  // 调用aclnnScatterDiv第二段接口
  ret = aclnnScatterDiv(workspaceAddr, workspaceSize, executor, stream);
  CHECK_RET(ret == ACL_SUCCESS, LOG_PRINT("aclnnScatterDiv failed. ERROR: %d\n", ret); return ret);
