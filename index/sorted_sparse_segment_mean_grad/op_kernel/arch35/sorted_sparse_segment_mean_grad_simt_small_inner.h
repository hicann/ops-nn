...
SortedSparseSegmentMeanGradSimtSmallInner<X_T, INDICES_T, LOCATION_T, SEGMENTIDS_T, OUTTER_T, INNER_T>::Init(
    ...
    GM_ADDR workspace, const SortedSparseSegmentMeanGradSimtTilingData* tilingData)
{
    ...
    if (blockIdx_ < static_cast<uint32_t>(tilingData_->resIndicesNum)) {
        ...
        indicesOffsetBase_ = static_cast<uint32_t>(tilingData_->perCoreIndicesNum) * blockIdx_ +
...
template <typename X_T, typename INDICES_T, typename LOCATION_T, typename SEGMENTIDS_T, typename OUTTER_T,
          typename INNER_T>
__aicore__ inline void
SortedSparseSegmentMeanGradSimtSmallInner<X_T, INDICES_T, LOCATION_T, SEGMENTIDS_T, OUTTER_T, INNER_T>::Process()
{
    ...
    asc_vf_call<SimtSmallInnerComputer<X_T, LOCATION_T, SEGMENTIDS_T, OUTTER_T, INNER_T>>(
        ...
        (__gm__ float*)(workspaceWeight_.GetPhyAddr(segmentNum_ + 1)), static_cast<uint32_t>(tilingData_->outputDim0));
}
