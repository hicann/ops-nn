...
ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetPlatformInfo()
{
    ...
    if (hardwareData.coreNum == 0) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "coreNum", "0",
...
ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetXInfoAndCheck()
{
    ...
    if (xShape_.GetDimNum() < 1) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context_->GetNodeName(), "x",
                                                 std::to_string(xShape_.GetDimNum()).c_str(),
                                                 "dimNum of x must be greater than or equal to 1");
        return ge::GRAPH_FAILED;
    }
    if (xShape_.GetDim(0) <= 0) {
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "x", std::to_string(xShape_.GetDim(0)).c_str(),
                                              "dim0 of x must be greater than 0");
        return ge::GRAPH_FAILED;
    }
    if (xShape_.GetShapeSize() <= 0) {
        OP_LOGE_FOR_INVALID_SHAPESIZE_WITH_REASON(context_->GetNodeName(), "x",
                                                  std::to_string(xShape_.GetShapeSize()).c_str(),
                                                  "shapeSize of x must be greater than 0");
        return ge::GRAPH_FAILED;
    }
    ...
    if (inputData.inputDtype != ge::DataType::DT_FLOAT16 && inputData.inputDtype != ge::DataType::DT_FLOAT &&
        ...
        OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "x",
                                  std::to_string(static_cast<int32_t>(inputData.inputDtype)).c_str(),
...
ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetYInfoAndCheck()
{
    ...
    if (xShape_.GetDimNum() != yShapeDimNum) {
        OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
            ...
            (std::to_string(xShape_.GetDimNum()) + ", " + std::to_string(yShapeDimNum)).c_str(),
    ...
    for (size_t i = 1; i < yShapeDimNum; i++) {
        if (xShape_.GetDim(i) != yShape.GetDim(i)) {
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                context_->GetNodeName(), "x, y",
                (std::to_string(xShape_.GetDim(i)) + ", " + std::to_string(yShape.GetDim(i))).c_str(),
...
ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetIndicesInfoAndCheck()
{
    ...
    if (inputData.indicesDtype != ge::DataType::DT_INT32 && inputData.indicesDtype != ge::DataType::DT_INT64) {
        OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "indices",
                                  std::to_string(static_cast<int32_t>(inputData.indicesDtype)).c_str(),
                                  "[DT_INT32, DT_INT64]");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetSegmentIdsInfoAndCheck()
{
    ...
    if (segmentIdsShape.GetDimNum() != 1) {
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "segment_ids",
                                     std::to_string(segmentIdsShape.GetDimNum()).c_str(), "1");
        return ge::GRAPH_FAILED;
    }
    ...
    if (inputData.segmentIdsDtype != ge::DataType::DT_INT32 && inputData.segmentIdsDtype != ge::DataType::DT_INT64) {
        OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "segment_ids",
                                  std::to_string(static_cast<int32_t>(inputData.segmentIdsDtype)).c_str(),
                                  "[DT_INT32, DT_INT64]");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetOutputDim0InfoAndCheck()
{
    ...
    if (outputDim0Shape.GetShapeSize() != 1 || outputDim0Shape.GetDimNum() != 1) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context_->GetNodeName(), "outputDim0",
                                                 std::to_string(outputDim0Shape.GetDimNum()).c_str(),
    ...
    if (outputDim0Desc->GetDataType() != ge::DataType::DT_INT32) {
        OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "outputDim0",
                                  std::to_string(static_cast<int32_t>(outputDim0Desc->GetDataType())).c_str(),
                                  "DT_INT32");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetIndicesLocationInfoAndCheck()
{
    ...
    if (indicesLocationShape.GetDimNum() != 1) {
        OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "indicesLocation",
                                     std::to_string(indicesLocationShape.GetDimNum()).c_str(), "1");
        return ge::GRAPH_FAILED;
    }
    ...
    if (inputData.indicesLocationDtype != ge::DataType::DT_INT32 &&
        ...
        OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "indicesLocation",
                                  std::to_string(static_cast<int32_t>(inputData.indicesLocationDtype)).c_str(),
                                  "[DT_INT32, DT_INT64]");
        return ge::GRAPH_FAILED;
    }
...
bool SortedSparseSegmentMeanGradBaseTiling::IsCapable() { return true; }

ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::DoOpTiling() { return ge::GRAPH_SUCCESS; }

ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t SortedSparseSegmentMeanGradBaseTiling::GetTilingKey() const { return 0; }

ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::GetWorkspaceSize() { return ge::GRAPH_SUCCESS; }

ge::graphStatus SortedSparseSegmentMeanGradBaseTiling::PostTiling() { return ge::GRAPH_SUCCESS; }
} // namespace optiling
