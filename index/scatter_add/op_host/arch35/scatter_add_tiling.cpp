                                     ...
                                     CalParams& cal_params)
{
    ...
    if (tiling_data_ptr->tiling_mode == TILING_MODE_6 || tiling_data_ptr->tiling_mode == TILING_MODE_7) {
        ...
        int64_t var_last_core_data = cal_params.var_num -
                                     tiling_data_ptr->var_each_core_data * (cal_params.core_num - 1);
                                       ...
                                       CalParams& cal_params)
{
    ...
    tiling_data_ptr->indices_last_core_data = cal_params.indices_num -
                                              tiling_data_ptr->indices_each_core_data * (tiling_data_ptr->core_num - 1);
    tiling_data_ptr->each_core_indices_loop_num = tiling_data_ptr->indices_each_core_data /
                                                  (half_ub_size / cal_params.indices_size);
    tiling_data_ptr->each_core_indices_last_num = tiling_data_ptr->indices_each_core_data %
                                                  (half_ub_size / cal_params.indices_size);
    tiling_data_ptr->last_core_indices_loop_num = tiling_data_ptr->indices_last_core_data /
                                                  (half_ub_size / cal_params.indices_size);
    tiling_data_ptr->last_core_indices_last_num = tiling_data_ptr->indices_last_core_data %
                                                  (half_ub_size / cal_params.indices_size);
...
static bool CheckScatterAddHighPerfShape(const gert::Shape& var_shape, const gert::Shape& indices_shape)
{
    ...
    if ((indices_shape.GetDimNum() == DIM_NUM_1 && var_shape.GetDimNum() == DIM_NUM_1) &&
        (var_shape.GetDim(0) == OUT_SPECIAL_DIM_4)) {
        return true;
    }
    return false;
}
                                ...
                                CalParams& cal_params)
{
    ...
    OP_CHECK_IF(
        compile_info->core_num <= 0 || compile_info->ub_size <= 0 || compile_info->var_size <= 0 ||
