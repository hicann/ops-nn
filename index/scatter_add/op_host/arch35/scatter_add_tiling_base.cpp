...
static const std::unordered_map<ge::DataType, uint64_t> VAR_DATA_TYPE_TO_INT{
...
static const std::set<ge::DataType> VAR_DTYPE_SET = {ge::DT_FLOAT, ge::DT_FLOAT16, ge::DT_INT32,
...
ge::graphStatus ScatterAddTiling::getRestAvailableSize(uint64_t sampleNum, uint64_t valueTypeBytes,
                                                       uint64_t originalSize, uint64_t postAxisSize,
                                                       ge::DataType idType) const
{
    uint64_t indicesDtypeSize = ge::GetSizeByDataType(idType);
    OP_CHECK_IF(indicesDtypeSize <= 0, OP_LOGE(opName_, "get indicesType size fail."), return ge::GRAPH_FAILED);
    auto ubBlock = Ops::Base::GetUbBlockSize(context_);
    uint64_t occupy = sampleNum * Ops::Base::CeilAlign(valueTypeBytes * postAxisSize, static_cast<uint64_t>(ubBlock)) +
                      sampleNum * Ops::Base::CeilAlign(FLOAT_BYTES * postAxisSize, static_cast<uint64_t>(ubBlock)) +
                      Ops::Base::CeilAlign(sampleNum * indicesDtypeSize, static_cast<uint64_t>(ubBlock)) * THREE +
                      Ops::Base::CeilAlign(sampleNum * UINT32_BYTES, static_cast<uint64_t>(ubBlock)) +
                      DOUBLE * DOUBLE * UB_AGLIN_VALUE + DOUBLE * UB_AGLIN_VALUE +
                      GetSortTmpSize(idType, sampleNum, false) +
                      Ops::Base::CeilAlign(postAxisSize * FLOAT_BYTES, static_cast<uint64_t>(ubBlock));
...
ge::graphStatus ScatterAddTiling::GetPlatformInfo()
{
    ...
    OP_CHECK_IF(platformInfo == nullptr, OP_LOGE(opName_, "fail to get platform info"), return ge::GRAPH_FAILED);
    auto ascendcPlatform = platform_ascendc::PlatformAscendC(platformInfo);
    auto aivNum = ascendcPlatform.GetCoreNumAiv();
    OP_CHECK_IF((aivNum <= 0), OP_LOGE(opName_, "fail to get coreNum."), return ge::GRAPH_FAILED);
    ...
    if (isSimt_) {
        if (isSort_ == 1) {
            ...
            OP_CHECK_IF((ubSizePlatForm <= DCACHE_SIZE),
                        OP_LOGE(opName_, "ub size less than Dcache Size. please check"), return ge::GRAPH_FAILED);
...
ge::graphStatus ScatterAddTiling::GetShapeAttrsInfo()
{
    ...
    bool supportAtomicAdd = isSimt_ ? SIMT_ATOMIC_ADD_NOT_SUPPORT_DTYPE.find(varDtype_) ==
                                          SIMT_ATOMIC_ADD_NOT_SUPPORT_DTYPE.end() :
                                      SIMD_ATOMIC_ADD_NOT_SUPPORT_DTYPE.find(varDtype_) ==
                                          SIMD_ATOMIC_ADD_NOT_SUPPORT_DTYPE.end();
    if (supportAtomicAdd) {
        ...
        if (isSort_ == 1 &&
            varDtype_ != ge::DataType::DT_INT8) { // 排序场景尾轴小于512B走simt性能更好，simt AtomicAdd不支持int8
...
ge::graphStatus ScatterAddTiling::CheckInputDtype()
{
    ...
    OP_CHECK_IF(indicesDtypeSize_ <= 0, OP_LOGE(opName_, "get indicesDtype size fail."), return ge::GRAPH_FAILED);
    ...
    if (VAR_DTYPE_SET.find(varDtype_) == VAR_DTYPE_SET.end()) {
        OP_LOGE_FOR_INVALID_DTYPE(opName_, "var",
                                  (ge::TypeUtils::DataTypeToSerialString(varDtype_) + "(" +
                                   std::to_string(static_cast<int32_t>(varDtype_)) + ")")
                                      .c_str(),
                                  "[DT_FLOAT(5), DT_FLOAT16(4), DT_INT32(1), DT_INT8(2), DT_UINT8(3), DT_BF16(6)]");
        return ge::GRAPH_FAILED;
    }
    varTypeSize_ = ge::GetSizeByDataType(varDtype_);
    OP_CHECK_IF(varTypeSize_ <= 0, OP_LOGE(opName_, "get dataType size fail."), return ge::GRAPH_FAILED);
    ...
    if (VAR_DTYPE_SET.find(updatesType) == VAR_DTYPE_SET.end()) {
        OP_LOGE_FOR_INVALID_DTYPE(opName_, "updates",
                                  (ge::TypeUtils::DataTypeToSerialString(updatesType) + "(" +
                                   std::to_string(static_cast<int32_t>(updatesType)) + ")")
                                      .c_str(),
                                  "[DT_FLOAT(5), DT_FLOAT16(4), DT_INT32(1), DT_INT8(2), DT_UINT8(3), DT_BF16(6)]");
        return ge::GRAPH_FAILED;
    }
                                                    ...
                                                    const gert::Shape& updatesShape) const
{
    ...
    if (updatesDimNum != indicesDimNum + varDimNum - 1) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(opName_, "updates", std::to_string(updatesDimNum).c_str(),
...
std::tuple<uint64_t, uint64_t> ScatterAddTiling::SimdTiling(uint64_t usedCoreNum, uint64_t colNumAlign,
                                                            uint64_t colLimitSize, bool colTileNumMin)
{
    ...
    */
    ...
    for (uint64_t m : cutSet) {
        ...
        if (m * n ==
            usedCoreNum) { // 选择把核尽量用满的组合。没用满时此条件不满足，delta为最大值，后面排序后不会选中此组合
            if (indicesNum_ % m == 0 &&
                colNumAlign % n == 0) { // 两个方向都能整切，每个块大小相同（理想情况），此时直接返回
...
void ScatterAddTiling::DoBlockTiling(uint64_t baseCol)
{
    ...
    atomicAddCoreNum_ = std::min(totalCoreNum_,
                                 static_cast<uint64_t>(indicesNum_ * colNumAlign * BASE_A_SIZE /
                                                       BASE_BLOCK_SIZE)); // 按每个核最小分1024个数计算使用核数
    atomicAddCoreNum_ = atomicAddCoreNum_ == 0 ? 1 : atomicAddCoreNum_;
    std::tie(rowTileNum_, colTileNum_) = SimdTiling(
        atomicAddCoreNum_, colNumAlign, COL_LIMIT_SIZE,
        true); // 分别计算两个方向切分份数，使得满足：1、尽量把核用满；2、尾核与整核数据量差值尽量小
...
uint64_t ScatterAddTiling::CalBestBaseSize(uint64_t baseXoStart, uint64_t baseXoEnd) const
{
    ...
    while (baseXoEnd - baseXoStart > 1) {
        ...
        if (indicesCastMode_ == 0) {
            ...
            tmpTotalSize = Ops::Base::CeilAlign(baseXoMid * ubFactorCol_ * updatesDtypeSize_, ubBlock) *
                               BUFFER_NUM_SIMD_SORT + // updates
                           Ops::Base::CeilAlign(baseXoMid * indicesDtypeSize_, ubBlock) *
                               BUFFER_NUM_SIMD_SORT + // indices
                           Ops::Base::CeilAlign(baseXoMid * ubFactorCol_ * updatesDtypeSize_,
                                                ubBlock) + // 存放相加后的临时变量
                           Ops::Base::CeilAlign(baseXoMid * indicesDtypeSize_, ubBlock) + // indicessorted
                           Ops::Base::CeilAlign(baseXoMid * sizeof(uint32_t), ubBlock) *
                               indicesSortBufCnt + // sortedIdxBuf  排序后原始的索引
                           SORT_STAT_PADDING +
            ...
            tmpTotalSize = Ops::Base::CeilAlign(baseXoMid * ubFactorCol_ * updatesDtypeSize_, ubBlock) *
                           ...
                           SORT_STAT_PADDING + // sort padding
                           Ops::Base::CeilAlign(baseXoMid * indicesDtypeSize_, ubBlock) *
                               BUFFER_NUM_SIMD_SORT +                      // Cast前的indices
                           Ops::Base::CeilAlign(sortNeedTmpSize, ubBlock); // sort shared buf size
        }
...
ge::graphStatus ScatterAddTiling::TilingSimdSupportAtomicAddSortCompute()
{
    ...
    if (normBlockCol_ * updatesDtypeSize_ <=
        COL_LIMIT_SIZE) { // 如果整核列数小于设定值（1024B），则将baseCol 与normBlockCol_保持一致
    ...
    uint64_t ubBlock = static_cast<uint64_t>(Ops::Base::GetUbBlockSize(context_));
    uint64_t maxBaseRow = (ubSize_ - BUFFER_NUM_SIMD_SORT * ubBlock) /
                          BUFFER_NUM_SIMD_SORT / // 在列长确定时，计算UB最多容纳多少行，此时还未考虑排序空间
    ...
    if (coreMaxRow < ubFactorRow_) { // coreMaxRow < ubFactorRow_时刷新单循环行数，并重新计算单循环列数
        ...
        ubFactorCol_ = (remainSize - BUFFER_NUM_SIMD_SORT * ubBlock - ubBlock) / (BUFFER_NUM_SIMD_SORT + 1) /
                       (ubFactorRow_ * updatesDtypeSize_);
...
ge::graphStatus ScatterAddTiling::TilingSimdSupportAtomicAddCompute()
{
    ...
    uint64_t maxBaseCol = // 计算UB最多能容纳多少个updates, (假设只搬1行且尾轴非常大)
        (ubSize_ - BUFFER_NUM * (ubBlock + minBaseIndices * indicesDtypeSize_)) / BUFFER_NUM /
        (minBaseIndices * updatesDtypeSize_);
    maxBaseCol = Ops::Base::FloorAlign(maxBaseCol, ubBlock / updatesDtypeSize_); // 向下32B对齐

    uint64_t colNumAlign = Ops::Base::CeilDiv(varShape_[1],
                                              maxBaseCol); // 计算尾轴能被切几份，尾轴比较小时则不切尾轴（切1份）
    ...
    ubFactorRow_ = (ubSize_ - BUFFER_NUM * ubBlock) / BUFFER_NUM /
                   (ubFactorCol_ * updatesDtypeSize_ + indicesDtypeSize_);
...
ge::graphStatus ScatterAddTiling::TilingSimdNotSupportAtomicAddCompute(bool supportAtomicAdd)
{
    ...
    if (updatesUbFactor_ * updatesDtypeSize_ > ((ubSize_ - MIN_BLOCK_SIZE) / BUFFER_NUM)) {
        ...
        indicesUbFactor_ = Ops::Base::CeilAlign(indicesUbSize,
...
ge::graphStatus ScatterAddTiling::TilingCopyCompute()
{
    ...
    OP_CHECK_IF(castTypeSize_ <= 0, OP_LOGE(opName_, "get dataType size fail."), return ge::GRAPH_FAILED);
...
ge::graphStatus ScatterAddTiling::ScatterAddDeterministicTiling()
{
    ...
    // * updatesSum * updateFloatSum * indices * sortedIndices * sortedOriginIndex * uniqueIndices * shiftOffset *
    // sortTmp
    uint64_t oneIdOccupyBytes = varTypeSize_ * varShape_[1] + FLOAT_BYTES * varShape_[1] + indicesDtypeSize_ * THREE +
                                UINT32_BYTES + DOUBLE * shiftOffset * DOUBLE + SORT_UTIL_SIZE;
    ...
    if (dequantizeBranch_ == 1) {
        ...
        uint32_t oneRowOccupyBytes = varShape_[1] * UINT32_BYTES + varShape_[1] * FLOAT_BYTES +
                                     varShape_[1] * FLOAT_BYTES + indicesDtypeSize_;
        rowsInUb_ = ubSize_ / DOUBLE / oneRowOccupyBytes;
        restSize = -1;
        while (restSize <= 0) {
            uint32_t occupyBytes = rowsInUb_ * Ops::Base::CeilAlign(varShape_[1] * UINT32_BYTES, ubBlock) +
                                   rowsInUb_ * Ops::Base::CeilAlign(varShape_[1] * FLOAT_BYTES, ubBlock) +
                                   DOUBLE * UB_AGLIN_VALUE + Ops::Base::CeilAlign(varShape_[1] * FLOAT_BYTES, ubBlock) +
                                   Ops::Base::CeilAlign(indicesDtypeSize_ * rowsInUb_, ubBlock);
    ...
    while (end - start > 1) {
        ...
        if (indicesCastMode_ == 0) {
            totalIndexSize = Ops::Base::CeilAlign(mid * static_cast<int64_t>(indicesDtypeSize_), ubBlock) * DOUBLE +
                             Ops::Base::CeilAlign(mid * static_cast<int64_t>(sizeof(uint32_t)), ubBlock) +
                             ubBlock * DOUBLE +
                             Ops::Base::CeilAlign(mid * static_cast<int64_t>(sizeof(int32_t)), ubBlock) +
                             ubBlock * DOUBLE;
        ...
        int64_t updateSize = Ops::Base::CeilAlign(static_cast<int64_t>(mid * varShape_[1] * updatesDtypeSize_),
                                                  ubBlock);
...
ge::graphStatus ScatterAddTiling::DoOpTiling()
{
    ...
    bool supportAtomicAdd = isSimt_ ? SIMT_ATOMIC_ADD_NOT_SUPPORT_DTYPE.find(varDtype_) ==
                                      ...
                                      SIMD_ATOMIC_ADD_NOT_SUPPORT_DTYPE.find(varDtype_) ==
                                          SIMD_ATOMIC_ADD_NOT_SUPPORT_DTYPE.end();
    if (!supportAtomicAdd) { // 不支持AtomicAdd的数据类型，先走 Tiling
                             // CopyCompute计算类型转换的tiling数据，需要分配Cast的空间
        OP_CHECK_IF(TilingCopyCompute() != ge::GRAPH_SUCCESS, OP_LOGE(opName_, "TilingCopyCompute fail."),
                    return ge::GRAPH_FAILED);
    }
    ...
    if (!isSimt_) {
        ...
        if (!supportAtomicAdd) {
            ...
            OP_CHECK_IF(TilingSimdNotSupportAtomicAddCompute(supportAtomicAdd) != ge::GRAPH_SUCCESS,
                        OP_LOGE(opName_, "TilingSimdNotSupportAtomicAddCompute fail."), return ge::GRAPH_FAILED);
        } else {
            if (isSort_ == 1) {
                OP_CHECK_IF(TilingSimdSupportAtomicAddSortCompute() != ge::GRAPH_SUCCESS,
                            OP_LOGE(opName_, "TilingSimdSupportAtomicAddSortCompute fail."), return ge::GRAPH_FAILED);
            } else {
                OP_CHECK_IF(TilingSimdSupportAtomicAddCompute() != ge::GRAPH_SUCCESS,
                            OP_LOGE(opName_, "TilingSimdSupportAtomicAddCompute fail."), return ge::GRAPH_FAILED);
            }
        }
    } else {
        if (isSort_ == 1) {
            OP_CHECK_IF(TilingSimtSort() != ge::GRAPH_SUCCESS, OP_LOGE(opName_, "TilingSimtSort fail."),
                        return ge::GRAPH_FAILED);
        }
    }
...
ge::graphStatus ScatterAddTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }
...
uint64_t ScatterAddTiling::GetTilingKey() const
{
    ...
    tilingKey = Ops::NN::Optiling::GET_TILINGKEY(isSort_, templateKey_, sizeAddrType, isUpdateScalar_,
                                                 TILING_KEY_PLACE_HOLD, TILING_KEY_PLACE_HOLD, TILING_KEY_PLACE_HOLD,
                                                 TILING_KEY_PLACE_HOLD, TILING_KEY_PLACE_HOLD);
...
ge::graphStatus ScatterAddTiling::GetWorkspaceSize()
{
    ...
    if (isDeterminTemplate_ == 1 && isDeterministic_ == 1) {
        uint64_t postVarAlignSize = Ops::Base::CeilAlign(varShape_[1] * sizeof(float),
                                                         static_cast<uint64_t>(Ops::Base::GetUbBlockSize(context_))) /
                                    sizeof(float);
        workspaceSize_ += logicCoreNum_ * perCoreHandleIndices_ * indicesDtypeSize_ +
...
ge::graphStatus ScatterAddTiling::PostTiling()
{
    ...
    OP_CHECK_IF((res != ge::GRAPH_SUCCESS), OP_LOGE(opName_, "SetLocalMemorySize ubSize = %lu failed.", ubSize_),
                return ge::GRAPH_FAILED);
