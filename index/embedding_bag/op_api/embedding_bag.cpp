...
static aclTensor* AllocTensorForEmbeddingBag(const op::Shape& shape910, op::DataType dtype910,
                                             const op::Shape& shapeOther, op::DataType dtypeOther,
...
static std::pair<op::Shape, op::Shape> GetMaxIndicesShapes(const std::string& modeStr, bool includeLastOffset,
                                                           const op::Shape& offsetsShape, const op::Shape& outputShape)
{
    op::Shape shape910;
    op::Shape shapeOther;
...
{
    return std::make_tuple(static_cast<aclTensor*>(nullptr), static_cast<aclTensor*>(nullptr),
    ...
    int64_t paddingIdx, aclOpExecutor* executor)
{
    ...
    offset2bag = AllocTensorForEmbeddingBag(offset2bagShape910, indexPromoteDtype, offset2bagShapeOther, indicesDtype,
                                            executor);
    ...
    maxIndices = AllocTensorForEmbeddingBag(maxIndicesShapes.first, indexPromoteDtype, maxIndicesShapes.second,
                                            offsetsDtype, executor);
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(EmbeddingBag, OP_INPUT(weight, indices, offsets, perSampleWeights),
