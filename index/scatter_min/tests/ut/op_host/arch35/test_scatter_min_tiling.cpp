...
/*!
 * \file test_scatter_min_tiling.cpp
 * \brief
 */
#include <iostream>
#include <vector>
...
// Drive ScatterMin tiling once. withCompileInfo=false exercises the aclnn path
...
ge::graphStatus RunScatterMinTiling(gert::StorageShape varShape, gert::StorageShape indicesShape,
                                    gert::StorageShape updatesShape, ge::DataType varDtype, ge::DataType indicesDtype,
                                    bool withCompileInfo, uint64_t& outKey)
{
    std::string op_type("ScatterMin");
...
protected:
    static void SetUpTestCase() { std::cout << "ScatterMinTiling SetUp" << std::endl; }
    static void TearDownTestCase() { std::cout << "ScatterMinTiling TearDown" << std::endl; }
};

// float32 happy path
TEST_F(ScatterMinTiling, test_tiling_base)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMinTiling({{16, 8}, {16, 8}}, {{4}, {4}}, {{4, 8}, {4, 8}}, ge::DT_FLOAT, ge::DT_INT32, true,
                                  key);
...
TEST_F(ScatterMinTiling, test_tiling_int32)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMinTiling({{16, 8}, {16, 8}}, {{4}, {4}}, {{4, 8}, {4, 8}}, ge::DT_INT32, ge::DT_INT64, true,
                                  key);
    EXPECT_EQ(st, ge::GRAPH_SUCCESS);
    EXPECT_EQ(key, 0);
}
...
TEST_F(ScatterMinTiling, test_tiling_indices_gt_cores)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMinTiling({{128, 8}, {128, 8}}, {{128}, {128}}, {{128, 8}, {128, 8}}, ge::DT_FLOAT,
...
TEST_F(ScatterMinTiling, test_tiling_big_slice)
{
    uint64_t key = 0xFFFF;
    auto st = RunScatterMinTiling({{2, 100000}, {2, 100000}}, {{2}, {2}}, {{2, 100000}, {2, 100000}}, ge::DT_FLOAT,
                                  ge::DT_INT32, true, key);
    EXPECT_EQ(st, ge::GRAPH_SUCCESS);
    EXPECT_EQ(key, 0);
}
