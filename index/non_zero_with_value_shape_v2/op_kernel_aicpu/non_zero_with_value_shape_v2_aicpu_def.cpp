...
namespace ops {
class NonZeroWithValueShapeV2 : public OpDef {
public:
    explicit NonZeroWithValueShapeV2(const char* name) : OpDef(name)
    {
        ...
        this->Output("value").ParamType(REQUIRED).DataType(valueTypes);
        this->Output("index").ParamType(REQUIRED).DataType({ge::DT_INT32});
...
OP_ADD(NonZeroWithValueShapeV2);
} // namespace ops
