...
static inline aclnnStatus CheckParams(const aclTensor* self, const aclTensor* indices, const aclIntArray* outputSize,
                                      const aclIntArray* stride, const aclIntArray* padding, const aclTensor* outRef)
{
    // 1. 检查参数是否为空指针
...
aclnnStatus aclnnMaxUnpool3dGetWorkspaceSize(const aclTensor* self, const aclTensor* indices,
                                             ...
                                             const aclIntArray* padding, aclTensor* outRef, uint64_t* workspaceSize,
                                             aclOpExecutor** executor)
{
    ...
    auto scatterRes = l0op::ScatterElements(zeroOut, indicesReshape, selfReshape, AXIS, reductionCurr,
                                            uniqueExecutor.get());
