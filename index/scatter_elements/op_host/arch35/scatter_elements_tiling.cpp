...
static std::map<const ge::DataType, const int32_t> INDICES_BLOCK_LEN = {{ge::DT_INT32, BLOCK_32},
                                                                        {ge::DT_INT64, BLOCK_64}};
                               ...
                               const gert::Shape& data_origin_shape, const gert::Shape& indices_origin_shape)
{
    ...
    if (param->dims_data != dims_updates) {
        OP_LOGE_FOR_INVALID_SHAPEDIMS_WITH_REASON(
            opName_, "data, updates", (std::to_string(param->dims_data) + ", " + std::to_string(dims_updates)).c_str(),
    ...
    if (data_dtype != updates_dtype) {
        OP_LOGE_FOR_INVALID_DTYPES_WITH_REASON(opName_, "data, updates",
                                               (std::to_string(static_cast<int32_t>(data_dtype)) + ", " +
                                                std::to_string(static_cast<int32_t>(updates_dtype)))
                                                   .c_str(),
    ...
    for (int32_t i = 0; i < param->dims_indices; i++) {
        ...
        if (indices_origin_shape.GetDim(i) != updates_origin_shape.GetDim(i)) {
            OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                opName_, "indices, updates",
                (std::to_string(indices_origin_shape.GetDim(i)) + ", " + std::to_string(updates_origin_shape.GetDim(i)))
                    .c_str(),
                                ...
                                ScatterElementsTilingParams* param)
{
    ...
    if (iter == DATA_BLOCK_LEN.end()) {
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
            opName_, "data", std::to_string(static_cast<int32_t>(data_dtype)).c_str(),
    ...
    if (data_block == 0) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "data_block", std::to_string(data_block).c_str(),
                                              "data_block should not equal zero");
        return ge::GRAPH_FAILED;
    }
    ...
    if (iter == INDICES_BLOCK_LEN.end()) {
        OP_LOGE_FOR_INVALID_DTYPE(opName_, "indices", std::to_string(static_cast<int32_t>(indices_dtype)).c_str(),
                                  "[DT_INT32, DT_INT64]");
        return ge::GRAPH_FAILED;
    }
    ...
    int32_t ub_size_bytes_indices = compile_info->ub_size_bytes / UB_INDICES_AND_UPDATES_RATIO * block /
                                    (block + block_indices);
    int32_t indices_block = ub_size_bytes_indices / BYTES_PER_BLOCK * block_indices;
    if (indices_block == 0) {
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(opName_, "indices_block", std::to_string(indices_block).c_str(),
                                              "indices_block should not equal zero");
        return ge::GRAPH_FAILED;
    }
                                        ...
                                        const gert::Shape& indices_origin_shape)
{
    ...
    int32_t indices_block = (elem_for_indices + block - 1) / block *
                            block; // use block but not block_indices for updates
    int32_t indices_repeat = indices_block / block_indices;
