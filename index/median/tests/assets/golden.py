...
"""Stable Median Kernel/GEIR reference and PyTorch benchmark.

Values use native ``torch.median`` where supported.  Its tie index is not a
stable contract, so indices use ``torch.sort(stable=True)``.  ACLNN/E2E/ONNX
are not delivered by this module's CMake target.
"""

import torch

__spec__ = {"median": "MedianKernelSpec"}
__golden__ = {"kernel": {"median": "median_golden"}}
__input__ = {"kernel": {"median": "median_input"}}
...
def _stable_median(x, dim):
    ...
    sorted_values, order = torch.sort(x, dim=axis, stable=True)
    selected_value = sorted_values.narrow(axis, rank, 1)
    selected_index = order.narrow(axis, rank, 1)
    if x.is_floating_point():
        nan_mask = torch.isnan(x)
        has_nan = torch.any(nan_mask, dim=axis, keepdim=True)
        first_nan = torch.max(nan_mask, dim=axis, keepdim=True).indices
        selected_index = torch.where(has_nan, first_nan, selected_index)
        selected_value = torch.gather(x, axis, selected_index)
    if x.dtype not in _NATIVE_UNSUPPORTED_DTYPES:
        native_value = torch.median(x, dim=axis, keepdim=True).values
        if x.is_floating_point():
            special = torch.isnan(native_value) | (native_value == 0)
            native_value = torch.where(special, selected_value, native_value)
...
def median_input(x, *, dim=-1, testcase_name="", **kwargs):
...
def median_golden(x, *, dim=-1, **kwargs):
    return [_to_array(y) for y in _stable_median(_from_array(x), dim)]


class MedianThirdParty:
    ...
    def __call__(self, x, **kwargs):
        if x.dtype in _NATIVE_UNSUPPORTED_DTYPES:
            return list(_stable_median(x, self.dim))
        return torch.median(x, dim=self.dim, keepdim=True)


class MedianKernelSpec:
    golden = staticmethod(median_golden)
    customize_inputs = staticmethod(median_input)
    third_party = {"torch": MedianThirdParty}
