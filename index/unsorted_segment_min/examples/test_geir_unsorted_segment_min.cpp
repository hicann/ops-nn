...
#include "nn_other.h"
#include "../op_graph/unsorted_segment_min_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    unsortedSegmentMin.set_input_##intputName(placeholder##intputIndex);                                            \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    unsortedSegmentMin.set_input_##intputName(placeholder##intputIndex);                                            \
    unsortedSegmentMin.update_input_desc_##intputName(placeholder##intputIndex##_desc);                             \
    inputs.push_back(placeholder##intputIndex);
...
{
    Status ret = SUCCESS;
    auto unsortedSegmentMin = op::UnsortedSegmentMin("unsortedSegmentMin");
    ...
    unsortedSegmentMin.set_input_segment_ids(placeholder2);
    unsortedSegmentMin.update_input_desc_segment_ids(placeholder2_desc);
    inputs.push_back(placeholder2);

    vector<int64_t> placeholder3_shape = {1};
    auto placeholder3 = op::Const("placeholder3");
    TensorDesc placeholder3_desc = TensorDesc(ge::Shape(placeholder3_shape), FORMAT_ND, DT_INT32);
    placeholder3_desc.SetPlacement(ge::kPlacementHost);
    placeholder3_desc.SetFormat(FORMAT_ND);
    Tensor tensor_placeholder3;
    ret = GenOnesData(placeholder3_shape, tensor_placeholder3, placeholder3_desc, DT_INT32, 2);
    if (ret != SUCCESS) {
    ...
    unsortedSegmentMin.set_input_num_segments(placeholder3);
    unsortedSegmentMin.update_input_desc_num_segments(placeholder3_desc);
    inputs.push_back(placeholder3);

    outputs.push_back(unsortedSegmentMin);
    return SUCCESS;
}
