...
#include "../unsorted_segment_common/arch35/unsorted_segment_min_tiling_key.h"
#include "./unsorted_segment_min.h"

using namespace UnsortedSegment;
using namespace AscendC;

template <typename X_T, typename SEGMENT_IDS_T, uint32_t CAST_MODE>
                                                 ...
                                                 GM_ADDR workspace, GM_ADDR tiling, TPipe& pipe)
{
    ...
    if constexpr (CAST_MODE == CAST_INT32_TO_INT16) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMaxValue<X_T>, InitGmMaxValue<X_T>,
                                           ComputeMin<X_T, uint32_t>, SetAtomicMinOp<X_T>, CAST_INT32_TO_INT16>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if constexpr (CAST_MODE == CAST_INT64_TO_INT32) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMaxValue<X_T>, InitGmMaxValue<X_T>,
                                           ComputeMin<X_T, uint32_t>, SetAtomicMinOp<X_T>, CAST_INT64_TO_INT32>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if constexpr (CAST_MODE == CAST_INT64_TO_INT16) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMaxValue<X_T>, InitGmMaxValue<X_T>,
                                           ComputeMin<X_T, uint32_t>, SetAtomicMinOp<X_T>, CAST_INT64_TO_INT16>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if constexpr (CAST_MODE == CAST_INT32_TO_UINT8) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMaxValue<X_T>, InitGmMaxValue<X_T>,
                                           ComputeMin<X_T, uint32_t>, SetAtomicMinOp<X_T>, CAST_INT32_TO_UINT8>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else if constexpr (CAST_MODE == CAST_INT64_TO_UINT8) {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMaxValue<X_T>, InitGmMaxValue<X_T>,
                                           ComputeMin<X_T, uint32_t>, SetAtomicMinOp<X_T>, CAST_INT64_TO_UINT8>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    } else {
        UnsortedSegment::KernelSimdDynSort<X_T, SEGMENT_IDS_T, GetInitMaxValue<X_T>, InitGmMaxValue<X_T>,
                                           ComputeMin<X_T, uint32_t>, SetAtomicMinOp<X_T>, CAST_NONE>
            op(&tilingData, &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    }
...
__aicore__ inline void KernelSimtDispatch(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR tiling, TPipe& pipe)
{
    ...
    UnsortedSegment::KernelUnsortedSegment<X_T, SEGMENT_IDS_T, SimtComputeSegmentMin<X_T>, InitGmMaxValue<X_T>> op(
        &tilingData, &pipe);
    op.Init(x, segment_ids, output);
    op.Process();
}
                                              ...
                                              TPipe& pipe)
{
    ...
    UnsortedSegment::KernelSimdSplitCol<X_T, SEGMENT_IDS_T, GetInitMaxValue<X_T>, ComputeMin<X_T, uint64_t>> op(
        &tilingData, &pipe);
    op.Init(x, segment_ids, output);
    op.Process();
}
                                             ...
                                             TPipe& pipe)
{
    if constexpr (std::is_same<uint32_t, X_T>::value || std::is_same<uint64_t, X_T>::value ||
        ...
        UnsortedSegment::KernelSimdNonSort<X_T, SEGMENT_IDS_T, InitGmMaxValue<X_T>, SetAtomicMinOp<X_T>> op(&tilingData,
                                                                                                            &pipe);
        op.Init(x, segment_ids, output);
        op.Process();
    }
...
__aicore__ inline void KernelOutFlDispatch(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR output, GM_ADDR tiling, TPipe& pipe)
{
    ...
    if constexpr (std::is_same<uint32_t, X_T>::value || std::is_same<uint64_t, X_T>::value ||
                  std::is_same<int64_t, X_T>::value) {
        UnsortedSegment::KernelUnsortedSegmentFL<X_T, SEGMENT_IDS_T, SimtGatherMinValue<X_T>, GetInitMaxValue<X_T>,
                                                 InitGmMaxValue<X_T>, ComputeMin<X_T, int32_t>, SetAtomicMinOp<X_T>>
            op(&pipe);
                                              ...
                                              TPipe& pipe)
{
    ...
    UnsortedSegment::KernelUnsortedSegmentSortSimt<X_T, SEGMENT_IDS_T, SimtGatherMinValue<X_T>,
                                                   SimtComputeSegmentMin<X_T>, GetInitMaxValue<X_T>,
                                                   InitGmMaxValue<X_T>>
        op(&tilingData, &pipe);
    op.Init(x, segment_ids, output);
    op.Process();
...
__global__ __aicore__ void unsorted_segment_min(GM_ADDR x, GM_ADDR segment_ids, GM_ADDR num_segments, GM_ADDR output,
                                                GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if constexpr (TEMPLATE_MODE == USS_TEMPLATE_SIMT) {
        ...
        if constexpr (std::is_same<uint32_t, DTYPE_X>::value || std::is_same<uint64_t, DTYPE_X>::value ||
            ...
            KernelSimdDynSortWithCast<DTYPE_X, DTYPE_SEGMENT_IDS, CAST_MODE>(x, segment_ids, num_segments, output,
