...
__aicore__ inline void SegmentSumSimt<TX, Index>::Init(GM_ADDR x, GM_ADDR segmentIds, GM_ADDR output, GM_ADDR workspace)
{
    ...
    // clear
    uint64_t initCoreReal = coreId == (coreNum - 1) ? tilingData_->initNumTailCore : tilingData_->initNumPerCore;
...
template <typename TX, typename Index>
__aicore__ inline void SegmentSumSimt<TX, Index>::CopySegmentIdsIn(LocalTensor<Index> segmentIdsLocal, int64_t offset,
                                                                   int32_t length)
{
                                                                      ...
                                                                      __gm__ TX* outputAddr, uint32_t colSize)
{
    for (uint32_t i = threadIdx.y; i < TMP_ROWS_TOTAL; i += blockDim.y) {
        ...
        TX res = tmpRowWs[i * colSize + threadIdx.x]; // 走到这里要么是第0组的线程，要么是当前线程组对应的id跟上一个非-1
                                                      // id不同，就是一个新id的起始
...
template <typename TX, typename Index>
__aicore__ inline void SegmentSumSimt<TX, Index>::ProcessEachLoopForDeterminstic(uint32_t segmentIdsNum,
                                                                                 uint64_t baseOffset)
{
    auto vectorWaitMTE2 = static_cast<event_t>(GetTPipePtr()->FetchEventID(HardEvent::MTE2_V));
    SetFlag<HardEvent::MTE2_V>(vectorWaitMTE2);
    ...
    asc_vf_call<ComputeInWs<TX, Index>>(dim3{threadNum, threadBlock}, (__gm__ TX*)tmpRowWs.GetPhyAddr(),
                                        (__gm__ Index*)tmpIdWs.GetPhyAddr(), (__gm__ TX*)outputGm.GetPhyAddr(),
