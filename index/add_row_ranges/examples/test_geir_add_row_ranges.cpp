...
#include "nn_other.h"
#include "../op_graph/add_row_ranges_proto.h"
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::AddRowRanges("add1");
    // x: (M, N) = (8, 16)
    std::vector<int64_t> xShape = {8, 16};
    // src: (K, N) = (16, 16)
    std::vector<int64_t> srcShape = {16, 16};
    // indices: (M, 2) = (8, 2)
    std::vector<int64_t> indicesShape = {8, 2};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, src, inDtype, srcShape);
    ADD_INPUT(3, indices, ge::DT_INT32, indicesShape);
...
int main(int argc, char* argv[])
{
    ...
    std::map<AscendString, AscendString> global_options = {{"ge.exec.deviceId", "6"}, {"ge.graphRunMode", "1"}};
