...
#include "arch35/embedding_simd_two_dim.h"
#include "arch35/embedding_simt_no_contiguous.h"
#include "arch35/embedding_simd_no_contiguous.h"

using namespace AscendC;
using namespace Embedding;

#define SIMD_TWO_DIM_TILING_KEY_INT32 1000000299UL
#define SIMD_TWO_DIM_TILING_KEY_INT64 1000000300UL
...
#define EMBEDDING_SIMD_NO_CONTIGUOUS_INT32 150
#define EMBEDDING_SIMD_NO_CONTIGUOUS_INT64 151
                                                ...
                                                GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(EMBEDDING_NO_CONTIGUOUS_B8_INDEX_SIZE_INT32)) {
        ...
        embeddingOp.Init(x, indices, y);
        embeddingOp.Process();
    } else if (TILING_KEY_IS(EMBEDDING_SIMD_NO_CONTIGUOUS_INT32)) {
        GET_TILING_DATA_WITH_STRUCT(EmbeddingNoContiguousSimdTilingData, tilingData, tiling);
        EmbeddingSimdNoContiguous<int32_t> embeddingOp(&pipe);
        embeddingOp.Init(x, indices, y, &tilingData);
        embeddingOp.Process();
    } else if (TILING_KEY_IS(EMBEDDING_SIMD_NO_CONTIGUOUS_INT64)) {
        GET_TILING_DATA_WITH_STRUCT(EmbeddingNoContiguousSimdTilingData, tilingData, tiling);
        EmbeddingSimdNoContiguous<int64_t> embeddingOp(&pipe);
        embeddingOp.Init(x, indices, y, &tilingData);
        embeddingOp.Process();
    } else if (TILING_KEY_IS(SIMD_TWO_DIM_TILING_KEY_INT32)) {
        GET_TILING_DATA_WITH_STRUCT(EmbeddingTilingDataSimdTwoDim, tilingData, tiling);
        EmbeddingSimdTwoDim<int32_t> embeddingOp(&pipe);
        embeddingOp.Init(x, indices, y, &tilingData);
        embeddingOp.Process();
    } else if (TILING_KEY_IS(SIMD_TWO_DIM_TILING_KEY_INT64)) {
        GET_TILING_DATA_WITH_STRUCT(EmbeddingTilingDataSimdTwoDim, tilingData, tiling);
        EmbeddingSimdTwoDim<int64_t> embeddingOp(&pipe);
        embeddingOp.Init(x, indices, y, &tilingData);
        embeddingOp.Process();
