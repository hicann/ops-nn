...
static const std::initializer_list<DataType> INDICES_DTYPE_SUPPORT_LIST = {op::DataType::DT_INT32,
...
static bool CheckDtypeValid(const aclTensor* weight, const aclTensor* indices, const aclTensor* out)
{
    ...
    const std::initializer_list<DataType>
        WEIGHT_DTYPE_SUPPORT_LIST = is910BSocVersion ? WEIGHT_DTYPE_SUPPORT_LIST_WITH_COMPLEX_AND_BF16 :
                                                       WEIGHT_DTYPE_SUPPORT_LIST_WITH_COMPLEX;
...
static bool CheckHighperf(const aclTensor* weight, const aclTensor* indices)
{
    ...
    if ((EMBEDDING_DIM != lastDimIdx) && (lastDimByteSize >= LAST_DIM_BYTE_BOUNDS) &&
        (lastDimByteSize % BLOCK_SIZE == 0) && (gatherMagnification > MAGNIFICATION_BOUNDS) && is910BSocVersion &&
        (weightByteSize > WEIGHT_BYTE_BOUNDS)) {
...
static const aclTensor* CalNoContiguous(const aclTensor* weight, const aclTensor* indices, aclOpExecutor* executor)
{
    ...
    aclTensor* newWeight = executor->CreateView(weight, weight->GetViewShape(), weight->GetStorageShape(),
                                                weight->GetViewStrides(), weight->GetViewOffset());
    aclTensor* newIndices = executor->CreateView(indices, indices->GetViewShape(), indices->GetStorageShape(),
                                                 indices->GetViewStrides(), indices->GetViewOffset());
    ...
    if (IsUseNoContiguous(weight, indices, out)) {
        ...
        if (CheckHighperf(weight, indices)) {
            ...
            if (CheckEmbeddingKernel(weight, indices)) {
                ...
                embeddingResult = l0op::GatherV2(weightContiguous, EMBEDDING_DIM, indicesContiguous,
                                                 uniqueExecutor.get());
            }
        }
    }
