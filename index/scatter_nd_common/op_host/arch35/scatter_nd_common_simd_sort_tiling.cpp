...
{
    ScatterNdCommon::ScatterNdCommonSimdSortTilingData*
        tilingData = context_->GetTilingData<ScatterNdCommon::ScatterNdCommonSimdSortTilingData>();
...
void ScatterNdCommonSimdSortTiling::DoOpTilingSplitIndicesSingleCol()
{
    ...
    int64_t indicesAlignSize = Ops::Base::CeilAlign(oneIndexSize, ubBlock) +         // indices
                               ...
                               Ops::Base::CeilAlign(INT32_BYTES, ubBlock) + Ops::Base::CeilAlign(INT32_BYTES, ubBlock) +
    ...
    if (minIndicesFactorSize + updateAlignSize >
        halfUbSize) { // 优先保证indices至少处理64行，实际 afterAxisFactor_ 按剩余ub给一行updates
        indicesFactor_ = SPLIT_THRESHOLD;
        int64_t sortTmpSize = GetSortTmpSize(sortTmpType, SPLIT_THRESHOLD, false);
        afterAxisFactor_ = (halfUbSize - indicesFactor_ * indicesAlignSize - sortTmpSize) /
                           (varTypeSize_ + varTypeSize_); // (update + updateSum)
...
void ScatterNdCommonSimdSortTiling::DoOpTilingSplitAfter()
{
    ...
    int64_t oneBlockSize = indicesSize +
                           (varTypeSize_ + varTypeSize_) * eachCoreAfterAxisCount_; // (update + updateSum)
...
void ScatterNdCommonSimdSortTiling::DoOpTilingSimdSplitIndices() // TODO: 没开double buffer 为啥 half ub ?
{
    ...
    if (indicesAlignSize + updateAlignSize >
        halfUbSize) { // 优先保证indices处理的行数，实际 afterAxisFactor_ 可能远小于afterAxis_
        ...
        while (restSize <= 0) {
            ...
            int64_t occupy = Ops::Base::CeilAlign(indicesFactor_ * rankSize_ * indicesTypeSize_, ubBlock) +
                             ...
                             indicesFactor_ *
                                 Ops::Base::CeilAlign((varTypeSize_)*afterAxisFactor_, ubBlock) + // updateSum
                             GetSortTmpSize(sortTmpType, indicesFactor_, false);
            restSize = halfUbSize - occupy;
            if (indicesFactor_ > indicesAxis_) {
                indicesFactor_ = indicesAxis_;
