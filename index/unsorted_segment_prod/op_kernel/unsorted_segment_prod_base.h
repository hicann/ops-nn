...
/*!
 * \file unsorted_segment_prod_base.h
 * \brief unsorted_segment_prod_base.h
 */

#ifndef UNSORTED_SEGMENT_PROD_BASE_H
#define UNSORTED_SEGMENT_PROD_BASE_H
...
template <typename T>
struct GetInitOneValue {
    static __aicore__ inline constexpr T Get() { return GetIdentityOne<T>(); }
};

template <typename T, typename D>
struct ComputeProd {
    __aicore__ inline ComputeProd() {}
    __aicore__ inline void operator()(LocalTensor<T>& yLocal, LocalTensor<T>& xLocal, D dstOffset, D srcOffset, D cols)
    {
        AscendC::Mul(yLocal[dstOffset], yLocal[dstOffset], xLocal[srcOffset], cols);
    }
};

template <typename T>
struct SimtGatherProdValue {
    __simt_callee__ __aicore__ inline SimtGatherProdValue() {}
    __simt_callee__ __aicore__ inline T operator()(T midResP, T xUbLocalRes) { return midResP * xUbLocalRes; }
};

template <typename T>
struct SimtAtomicProd {
    __simt_callee__ __aicore__ inline SimtAtomicProd() {}
    __simt_callee__ __aicore__ inline void operator()(__gm__ T* outputGm, const uint64_t outputIndex, T value)
    {
        if constexpr (sizeof(T) >= 4) {
            while (true) {
                T oldVal = outputGm[outputIndex];
                T newVal = oldVal * value;
                T cas = Simt::AtomicCas(outputGm + outputIndex, oldVal, newVal);
                if (cas == oldVal) {
                    break;
                }
            }
            ...
            uint64_t pairIndex = outputIndex & ~1ULL;
            bool isLow = (outputIndex & 1ULL) == 0;
            while (true) {
                T oldVal1 = outputGm[pairIndex];
                T oldVal2 = outputGm[pairIndex + 1];
                uint32_t oldU32;
                reinterpret_cast<T*>(&oldU32)[0] = oldVal1;
                reinterpret_cast<T*>(&oldU32)[1] = oldVal2;
                T oldVal = isLow ? oldVal1 : oldVal2;
                T neighbor = isLow ? oldVal2 : oldVal1;
                T newVal = oldVal * value;
                uint32_t newU32;
                if (isLow) {
                    reinterpret_cast<T*>(&newU32)[0] = newVal;
                    reinterpret_cast<T*>(&newU32)[1] = neighbor;
                } else {
                    reinterpret_cast<T*>(&newU32)[0] = neighbor;
                    reinterpret_cast<T*>(&newU32)[1] = newVal;
                }
...
#endif // UNSORTED_SEGMENT_PROD_BASE_H
