...
{
    OP_CHECK_IF(value2 == 0, OP_LOGE("InplaceIndexAdd", "In the GetCeilInt function, the divisor is 0."),
                return value1);
...
static bool CalRunningCaseAtomicAdd(const InplaceIndexAddCompileInfo* compile_info, InplaceIndexAddTilingData* params)
{
    if (params->indices_num < compile_info->core_num) {
        ...
        params->tail_num = params->full_num_per_block;
        OP_CHECK_IF(params->block_num == 0, OP_LOGE("InplaceIndexAdd", "block num cannot be 0."), return false);
...
static bool InitRunningInfo(InplaceIndexAddTilingData* params, const gert::Shape& indices_shape_val, int32_t axis_val)
{
    params->block_num = 0;
    params->indices_num = indices_shape_val.GetShapeSize();
    OP_CHECK_IF(params->indices_num == 0, OP_LOGE("InplaceIndexAdd", "indices num cannot be 0."), return false);
    params->outer_loop = 1;
...
static bool CalRunningInfo(const InplaceIndexAddCompileInfo* compile_info, InplaceIndexAddTilingData* params,
                           const gert::Shape& var_shape_val, const gert::Shape& updates_shape_val, int64_t axis_val,
                           ge::DataType dtype)
{
    ...
    for (int64_t i = axis_val; i < updates_dims; i++) {
        params->axis_and_after_data_num_updates = params->axis_and_after_data_num_updates * updates_shape_val.GetDim(i);
    }
    ...
    if (dtype == DT_BF16) {
        ...
        OP_CHECK_IF(!CalRunningCaseAtomicAdd(compile_info, params),
                    OP_LOGE("InplaceIndexAdd", "CalRunningCaseAtomicAdd failed."), return false);
        params->updates_ub_size = (compile_info->ub_size -
                                   params->indices_ub_size * static_cast<int64_t>(sizeof(int64_t))) /
                                  (TENSOR_NUM * static_cast<int64_t>(sizeof(float)));
