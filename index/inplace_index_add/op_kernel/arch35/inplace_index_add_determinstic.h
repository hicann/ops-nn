...
public:
    ...
    static __simd_vf__ inline void ComputeUniqueIdNumVf(__ubuf__ IDX_T* indicesAddr,
                                                        __ubuf__ int32_t* uniqueIdCountsAddr, uint16_t loopCnt,
                                                        uint32_t counter, int64_t vfLen);
    static __simd_vf__ inline void InverseQuantizeVf(__ubuf__ int32_t* sumQuanToIntAddr, __ubuf__ float* rValueAddr,
                                                     __ubuf__ VAR_T* invQuantDataAddr, uint16_t rowLen, uint32_t colLen,
                                                     uint16_t loopCnt, uint32_t vfLen, float scaleValue,
                                                     uint64_t afterAxisAlignFp32, uint64_t afterAxisAlignSize);
    static __simd_vf__ inline void QuantizeForSumVf(__ubuf__ float* sumAddr, __ubuf__ float* rValueAddr,
                                                    __ubuf__ int32_t* quantaSumAddr, uint16_t rowLen, uint32_t colLen,
                                                    uint16_t loopCnt, uint32_t vfLen, float scaling,
                                                    uint64_t afterAxisAlignFp32);
    static __simd_vf__ inline void ComputeSumVf(__ubuf__ float* updatesAddr, __ubuf__ float* updateSumAddr,
                                                ...
                                                uint64_t afterAxisAlignSize, uint64_t afterAxisAlignFp32);
    static __simd_vf__ inline void ComputeUinqueIdTimesVf(__ubuf__ int32_t* uniqueIdCountsAddr, uint32_t vfLen,
...
template <typename VAR_T, typename IDX_T>
__aicore__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::Init(GM_ADDR var, GM_ADDR indices, GM_ADDR updates,
...
template <typename VAR_T, typename IDX_T>
__simd_vf__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::ComputeUniqueIdNumVf(
...
__aicore__ void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::ComputeUniqueIdNum(int64_t dataLen)
{
    ...
    __ubuf__ IDX_T* indicesAddr = (__ubuf__ IDX_T*)indicesLocal[shiftOffset_].GetPhyAddr();
...
template <typename VAR_T, typename IDX_T>
__simd_vf__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::ComputeSumVf(
    __ubuf__ float* updatesAddr, __ubuf__ float* updateSumAddr, __ubuf__ int32_t* uniqueIdCountAddr,
    __ubuf__ uint32_t* updatesOriginIdexAddr, uint32_t uniqueIdNum, uint32_t colLen, uint16_t loopSize, uint32_t vfLen,
    uint64_t afterAxisAlignSize, uint64_t afterAxisAlignFp32)
{
...
__aicore__ void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::ComputeSum(uint32_t uniqueIdNum, int64_t colLen)
{
    ...
    __ubuf__ float* updatesAddr = (__ubuf__ float*)updatesCastLocal.GetPhyAddr();
    ...
    ComputeSumVf(updatesAddr, updateSumAddr, uniqueIdCountAddr, updatesOriginIdexAddr, uniqueIdNum,
                 static_cast<uint32_t>(colLen), static_cast<uint16_t>(loopSize), vfLen, afterAxisAlignSize_,
                 afterAxisAlignFp32_);
...
template <typename VAR_T, typename IDX_T>
__aicore__ void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::CopySumOutToWs(int64_t preAxisIdx, int64_t rowIdx,
...
template <typename VAR_T, typename IDX_T>
__aicore__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::CopySumAndRValueIn(int64_t preAxisIdx, int64_t rowIdx,
                                                                                     int64_t colIdx, int64_t rowLen,
                                                                                     int64_t colLen)
{
...
template <typename VAR_T, typename IDX_T>
__simd_vf__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::QuantizeForSumVf(
...
__aicore__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::QuantizeForSum(int64_t rowLen, int64_t colLen)
{
    ...
    QuantizeForSumVf(sumAddr, rValueAddr, quantaSumAddr, static_cast<uint16_t>(rowLen), static_cast<uint32_t>(colLen),
                     loopCnt, vfLen, scaling, afterAxisAlignFp32_);
...
template <typename VAR_T, typename IDX_T>
__aicore__ void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::CopyQuantizedSumOutToIntWs(int64_t preAxisIdx,
...
template <typename VAR_T, typename IDX_T>
__aicore__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::CopyRValueAndIntWsIn(int64_t rowIdx, int64_t colIdx,
                                                                                       int64_t rowLen, int64_t colLen)
{
    ...
    int64_t preAxisIdx, int64_t rowIdx, int64_t colIdx, int64_t rowLen, int64_t colLen)
{
    ...
    for (uint64_t i = 0; i < rowLen; i++) {
        ...
        uint32_t count = AscendC::AtomicExch(const_cast<__gm__ uint32_t*>(sameIdxCountWsGm_[sumIdx].GetPhyAddr()),
                                             uint32_t(0));
        if (count == 0) {
            continue;
        }
...
template <typename VAR_T, typename IDX_T>
__simd_vf__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::InverseQuantizeVf(
    __ubuf__ int32_t* sumQuanToIntAddr, __ubuf__ float* rValueAddr, __ubuf__ VAR_T* invQuantDataAddr, uint16_t rowLen,
...
__aicore__ inline void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::InverseQuantize(int64_t rowLen, int64_t colLen)
{
    ...
    __ubuf__ float* rValueAddr = (__ubuf__ float*)rValueLocal.GetPhyAddr();
    __ubuf__ VAR_T* invQuantDataAddr = (__ubuf__ VAR_T*)inverseQuantData.GetPhyAddr();

    uint32_t vfLen = platform::GetVRegSize() / sizeof(int32_t);
    uint16_t loopCnt = (colLen + vfLen - 1) / vfLen;
    float scaleValue = static_cast<float>(1 << 30);
    InverseQuantizeVf(sumQuanToIntAddr, rValueAddr, invQuantDataAddr, static_cast<uint16_t>(rowLen),
                      static_cast<uint32_t>(colLen), loopCnt, vfLen, scaleValue, afterAxisAlignFp32_,
                      afterAxisAlignSize_);
...
template <typename VAR_T, typename IDX_T>
__aicore__ void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::CopyInverseQuantizedValueOutOpti(int64_t preAxisIdx,
                                                                                            int64_t colLen)
{
...
template <typename VAR_T, typename IDX_T>
__aicore__ void InplaceIndexAddDeterminstic<VAR_T, IDX_T>::CopyInverseQuantizedValueOut(int64_t rowIdx, int64_t colIdx,
                                                                                        int64_t rowLen, int64_t colLen)
{
    LocalTensor<VAR_T> inverseQuantData = invQuanDataQue_.DeQue<VAR_T>();
