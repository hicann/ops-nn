...
#include "nn_other.h"
#include "../op_graph/scatter_max_with_argmax_proto.h"
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    auto add1 = op::ScatterMaxWithArgmax("add1");
    // x: [4, 2] float32, indices: [4] int32, updates: [4, 2] float32
    std::vector<int64_t> xShape = {4, 2};
    std::vector<int64_t> indicesShape = {4};
    std::vector<int64_t> updatesShape = {4, 2};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_INPUT(2, indices, ge::DT_INT32, indicesShape);
    ADD_INPUT(3, updates, inDtype, updatesShape);

    ADD_OUTPUT(1, y, inDtype, xShape);
    ADD_OUTPUT(2, argmax, ge::DT_INT32, xShape);
