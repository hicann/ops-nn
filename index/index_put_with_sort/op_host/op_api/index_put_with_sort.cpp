...
static bool IndexPutWithSortBetter(const aclTensor* selfRef, const aclTensorList* indices, const aclTensor* values)
{
    ...
    if (selfRef->GetDataType() == op::DataType::DT_FLOAT && indicesNums >= INDICES_LIMIT &&
        sliceSize <= SLICESIZE_LIMIT) {
        // 输入索引多且尾轴稍大
        return true;
    }
    if (selfRef->GetDataType() == op::DataType::DT_FLOAT16 || selfRef->GetDataType() == op::DataType::DT_BF16) {
        ...
        int64_t dataNums = static_cast<int64_t>(selfRef->GetViewShape().GetShapeSize() +
                                                values->GetViewShape().GetShapeSize());
                                  ...
                                  aclTensor* out, aclOpExecutor* executor)
{
    ...
    auto ret = ADD_TO_LAUNCHER_LIST_AICORE(IndexPutWithSort, OP_INPUT(self, linearIndex, posIdx, values),
                                           OP_OUTPUT(out), OP_ATTR(sliceSize, accumulate));
    OP_CHECK(ret == ACLNN_SUCCESS,
             OP_LOGE(ACLNN_ERR_INNER_NULLPTR, "IndexPutWithSortAiCore ADD_TO_LAUNCHER_LIST_AICORE failed."),
             return nullptr);
    return out;
}
} // namespace l0op
