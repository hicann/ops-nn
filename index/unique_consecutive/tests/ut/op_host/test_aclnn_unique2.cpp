...
protected:
                  ...
                  bool return_inverse = true, bool return_counts = true)
    {
        ...
        auto ut = OP_API_UT(aclnnUnique2, INPUT(self, sorted, return_inverse, return_counts),
    ...
    void test_run_dtype_not_consistent(aclDataType self_dtype, aclDataType output_dtype, aclDataType indices_dtype,
                                       aclDataType counts_dtype, bool invalid, aclFormat test_format, int num_of_dim,
    ...
    void test_run_shape_not_consistent(vector<int64_t> self_shape, vector<int64_t> output_shape,
                                       ...
                                       aclDataType test_dtype, bool sorted = false, bool return_inverse = true,
...
// 测试不合法数据类型：COMPLEX64
TEST_F(l2_unique2_test, case_014_COMPLEX64) { test_run_invalid(ACL_COMPLEX64, ACL_FORMAT_ND, 6); }

// 测试不合法数据类型：COMPLEX64
TEST_F(l2_unique2_test, case_015_COMPLEX128) { test_run_invalid(ACL_COMPLEX128, ACL_FORMAT_ND, 6); }

// 测试不合法数据类型：UNIDEFINED
TEST_F(l2_unique2_test, case_016_UNDEFINED) { test_run_invalid(ACL_DT_UNDEFINED, ACL_FORMAT_ND, 6); }

// 测试self和output输出不一致情况
TEST_F(l2_unique2_test, case_017_DTYPE_UNCONSISTENT)
{
    test_run_dtype_not_consistent(ACL_FLOAT, ACL_FLOAT, ACL_FLOAT, ACL_INT64, true, ACL_FORMAT_ND, 6, false, true,
                                  true); // indices 非INT64
    test_run_dtype_not_consistent(ACL_FLOAT, ACL_FLOAT, ACL_INT64, ACL_FLOAT, true, ACL_FORMAT_ND, 6, false, true,
                                  true); // counts 非INT64
}
...
// 测试空tensor
TEST_F(l2_unique2_test, case_019_EMPTY) { test_run(ACL_INT8, ACL_FORMAT_ND, 0); }
...
// dim长度必须小于8
TEST_F(l2_unique2_test, case_021_DIM_OVERSIZE) { test_run_invalid(ACL_FLOAT, ACL_FORMAT_ND, 10); }
