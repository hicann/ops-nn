...
/*!
 * \file top_k_top_p_sample_v2_sort_cumsum.h
 * \brief
 */

#ifndef TOP_K_TOP_P_SAMPLE_V2_SORT_CUMSUM_H
#define TOP_K_TOP_P_SAMPLE_V2_SORT_CUMSUM_H
...
template <typename T>
class TopKTopPSampleV2SortKernel {
public:
    using MrgSrcGMList = GlobalTensor<float> (&)[MRG_PER_MAX_NUM];
    __aicore__ inline TopKTopPSampleV2SortKernel(){};
    ...
    __aicore__ inline void CumsumAndOut(TOPKPParams& params, LocalTensor<float>& bufLocal, uint32_t& totalMrgCnt,
                                        uint32_t& partTotalMrg, float* sumVal, float topp, bool& ifRet,
                                        ...
                                        const GlobalTensor<uint32_t>& sortIndexGM)
    {
        ...
        if (params.inputIsLogits) {
            PipeBarrier<PIPE_V>();
            Adds(mrgRsValLocal, mrgRsValLocal, -params.rowMax, rsvdCnt);
            PipeBarrier<PIPE_V>();
            Exp(mrgRsValLocal, mrgRsValLocal, rsvdCnt);
            PipeBarrier<PIPE_V>();
            Duplicate(params.tensor1, params.reduceSumMax, rsvdCnt);
            PipeBarrier<PIPE_V>();
            Div(mrgRsValLocal, mrgRsValLocal, params.tensor1, rsvdCnt); // softmax计算结果存储在valueLocalCast中
        }
                                            ...
                                            const GlobalTensor<uint32_t>& sortIndexGM)
    {
        ...
        while (queLen > 0) {
            ...
            SetFlag<HardEvent::MTE2_V>(eventIDMTE2ToV);
            WaitFlag<HardEvent::MTE2_V>(eventIDMTE2ToV);
            CumsumAndOut(params, bufLocal, totalMrgCnt, copyGmNum, sumVal, params.topp, ifRet, sortValeGM, sortIndexGM);
            if (!ifRet) {
                                       ...
                                       const GlobalTensor<float>& sortValeGM, const GlobalTensor<uint32_t>& sortIndexGM)
    {
        ...
        while (queNumOri > 1) {
            ...
            if (ifCumsum) {
                CumsumAndOut(params, bufLocal, totalMrgCnt, partTotalMrg, &sumVal, params.topp, ifRet, sortValeGM,
                             sortIndexGM);
                if (!ifRet) {
                    params.toppNum = totalMrgCnt;
                    break;
...
#endif // TOP_K_TOP_P_SAMPLE_V2_SORT_CUMSUM_H
