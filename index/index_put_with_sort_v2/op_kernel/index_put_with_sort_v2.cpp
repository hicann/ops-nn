...
__global__ __aicore__ void index_put_with_sort_v2(GM_ADDR self, GM_ADDR linearIndex, GM_ADDR posIdx, GM_ADDR values,
                                                  GM_ADDR output, GM_ADDR workSpace, GM_ADDR tiling)
{
    ...
    if constexpr (IS_SIMD) {
        ...
        if constexpr (ACCUMULATE) {
            if constexpr (IS_CAST) {
                if constexpr (IsSameType<DTYPE_SELF, float16_t>::value) {
                    IndexPutWithSortV2SIMDKernel<float16_t, DTYPE_LINEAR_INDEX, ACCUMULATE, true, float> op(
                        &tpipe, &tilingData);
                    op.Init(self, linearIndex, posIdx, values, output, workSpace);
                    op.Process();
                } else if (IsSameType<DTYPE_SELF, bfloat16_t>::value) {
                    IndexPutWithSortV2SIMDKernel<bfloat16_t, DTYPE_LINEAR_INDEX, ACCUMULATE, true, float> op(
                        &tpipe, &tilingData);
                    op.Init(self, linearIndex, posIdx, values, output, workSpace);
                    op.Process();
                }
            } else {
                IndexPutWithSortV2SIMDKernel<DTYPE_SELF, DTYPE_LINEAR_INDEX, ACCUMULATE, false, DTYPE_SELF> op(
                    &tpipe, &tilingData);
                op.Init(self, linearIndex, posIdx, values, output, workSpace);
                op.Process();
            }
        } else {
            IndexPutWithSortV2SIMDKernel<DTYPE_SELF, DTYPE_LINEAR_INDEX, ACCUMULATE, IS_CAST, DTYPE_SELF> op(
                &tpipe, &tilingData);
            op.Init(self, linearIndex, posIdx, values, output, workSpace);
            op.Process();
        }
        ...
        IndexPutWithSortV2Kernel<DTYPE_SELF, DTYPE_LINEAR_INDEX, ACCUMULATE, ALL_INDEXED, INDEXED_BLOCK_MODE> op(
            &tpipe, &tilingData);
