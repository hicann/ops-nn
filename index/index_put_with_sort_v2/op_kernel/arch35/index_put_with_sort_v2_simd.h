...
private:
    ...
    __aicore__ inline void CopyOutSum(bool isRowLoopEnd, int64_t row, int64_t rowLen, int64_t repeatCount,
                                      IDX_TYPE curIndex, IDX_TYPE preIndex, IDX_TYPE nextIndex, int64_t colIdx,
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::Init(
    GM_ADDR self, GM_ADDR sortIndices, GM_ADDR posIdx, GM_ADDR values, GM_ADDR output, GM_ADDR workspace)
{
    ...
    if (blockId_ < rowUseCoreNum_) {
        indexSumInitWsGm_.SetGlobalBuffer((__gm__ IDX_TYPE*)indexSumStartAddr + blockId_ * indexBlockNum_ * TWO,
                                          indexBlockNum_ * TWO);
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::CopyInValue(
    const GlobalTensor<SELF_TYPE>& srcTensor, int64_t dataLen)
{
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::CopyInIdx(
    int64_t rowIdx, int64_t rowLen, bool isRowLoopEnd)
{
...
IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::ComputeSumValue(int64_t colLen,
                                                                                                   uint32_t vfLen,
                                                                                                   uint16_t loopCnt)
{
    LocalTensor<SELF_TYPE> valueLocal = valueQue_.DeQue<SELF_TYPE>();
    LocalTensor<SELF_TYPE> valueSumLocal = isInit_ ? valueSumQue_.AllocTensor<SELF_TYPE>() :
                                                     valueSumQue_.DeQue<SELF_TYPE>();
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::ComputeAcc(
    int64_t rowIdx, int64_t colIdx, int64_t rowLen, int64_t colLen, bool isRowLoopEnd,
    LocalTensor<IDX_TYPE>& indexTotalLocal, LocalTensor<int32_t>& posidxLocal)
{
    LocalTensor<IDX_TYPE> indexLocal = ((blockRow_ == 0) && (rowIdx == 0)) ? indexTotalLocal[coreLoopOfst_ - 1] :
                                                                             indexTotalLocal[0];
        ...
        if (curIndex != nextIndex || row == rowLen - 1) {
            ...
            for (int64_t k = 0; k < repeatCount; k++) {
                ...
                if (preIndex != curIndex && k == 0 && lastLoopRepateCount_ == 0) {
                    if constexpr (IS_CAST || IsSameType<SELF_TYPE, int64_t>::value ||
                                  IsSameType<SELF_TYPE, uint8_t>::value || IsSameType<SELF_TYPE, bool>::value) {
                ...
                int64_t valueGmOfst = posidxLocal(row - repeatCount + ONE + k) * tilingData_->nonIndexedDimSize +
                                      blockCol_ * tilingData_->colBlockFactor + colIdx * colFactor_;
                CopyInValue(valuesGm_[valueGmOfst], colLen);
                ComputeSumValue(colLen, vfLen, loopCnt);
            }
            CopyOutSum(isRowLoopEnd, row, rowLen, repeatCount, curIndex, preIndex, nextIndex, colIdx, colLen, vfLen,
                       loopCnt);
            repeatCount = 0;
        }
    }
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::CopyOutSum(
    bool isRowLoopEnd, int64_t row, int64_t rowLen, int64_t repeatCount, IDX_TYPE curIndex, IDX_TYPE preIndex,
    IDX_TYPE nextIndex, int64_t colIdx, int64_t colLen, uint32_t vfLen, uint16_t loopCnt)
{
    bool isFullUniqueIdxFront = (repeatCount + lastLoopRepateCount_) == curCoreProcessRowCount_ &&
                                curIndex == preIndex && curIndex != nextIndex;
    bool isFullUniqueIdxRear = (repeatCount + lastLoopRepateCount_) == curCoreProcessRowCount_ &&
                               curIndex == preIndex && curIndex == nextIndex;
    ...
    if (isFullUniqueIdxFront || isFullUniqueIdxRear || isFrontHalf || isRearHalf) {
        ...
        DataCacheCleanAndInvalid<IDX_TYPE, AscendC::CacheLine::SINGLE_CACHE_LINE, AscendC::DcciDst::CACHELINE_OUT>(
            indexSumWsGm_[idxWsOfset]);
        if constexpr (IS_CAST) {
        ...
        if constexpr (IsSameType<SELF_TYPE, int64_t>::value || IsSameType<SELF_TYPE, uint8_t>::value ||
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST,
                                                    CAST_TYPE>::ComputeReplace(int64_t rowIdx, int64_t colIdx,
                                                                               int64_t rowLen, int64_t colLen,
                                                                               LocalTensor<IDX_TYPE>& indexTotalLocal,
                                                                               LocalTensor<int32_t>& posidxLocal)
{
    ...
    for (int64_t row = 0; row < rowLen; row++) {
        ...
        if (preIndex != curIndex) {
            int64_t gmOfset = curIndex + blockCol_ * tilingData_->colBlockFactor + colIdx * colFactor_;
            int64_t valueGmOfst = posidxLocal(row) * tilingData_->nonIndexedDimSize +
                                  blockCol_ * tilingData_->colBlockFactor + colIdx * colFactor_;
            CopyInValue(valuesGm_[valueGmOfst], colLen);
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void
IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::ProcessFirstStep(void)
{
    ...
    curCoreProcessRowCount_ = (blockRow_ == rowUseCoreNum_ - 1) ? tilingData_->rowTailBlockFactor :
                                                                  tilingData_->rowBlockFactor;
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST,
                                                    CAST_TYPE>::GetComputeRangeIdx(IDX_TYPE& startIdx, IDX_TYPE& endIdx,
                                                                                   IDX_TYPE& indexValue)
{
    ...
    for (IDX_TYPE idx = startIdx + indexBlockNum_;
         idx < indexBlockNum_ * TWO * tilingData_->rowUseCoreNum * tilingData_->colUseCoreNum;
         idx += indexBlockNum_ * TWO) {
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void
IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::CopyInValueFromWs(int64_t offset,
                                                                                                     int64_t dataLen)
{
    DataCopyExtParams inParams = {static_cast<uint16_t>(1), static_cast<uint32_t>(dataLen * sizeof(CAST_TYPE)), 0, 0,
                                  0};
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST,
                                                    CAST_TYPE>::GetSumValueStep2(int64_t dataLen,
...
template <typename SELF_TYPE, typename IDX_TYPE, bool ACCUMULATE, bool IS_CAST, typename CAST_TYPE>
__aicore__ inline void
IndexPutWithSortV2SIMDKernel<SELF_TYPE, IDX_TYPE, ACCUMULATE, IS_CAST, CAST_TYPE>::ProcessSecondStep(void)
{
    ...
    int64_t curCoreProcessColCount = (blockCol_ == tilingData_->colUseCoreNum - 1) ? tilingData_->colTailBlockFactor :
                                                                                     tilingData_->colBlockFactor;
    int64_t colFactor = (curCoreProcessColCount >= tilingData_->ubFactor) ? tilingData_->ubFactor :
                                                                            curCoreProcessColCount;
    ...
    for (int32_t i = 0; i < colLoopNum; i++) {
        ...
        DataCopyExtParams outParams = {static_cast<uint16_t>(1), static_cast<uint32_t>(curUbFactor * sizeof(SELF_TYPE)),
                                       0, 0, 0};
