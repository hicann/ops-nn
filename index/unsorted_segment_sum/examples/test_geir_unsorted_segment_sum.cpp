...
#include "nn_other.h"
#include "../op_graph/unsorted_segment_sum_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    unsortedSegmentSum.set_input_##intputName(placeholder##intputIndex);                                            \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    unsortedSegmentSum.set_input_##intputName(placeholder##intputIndex);                                            \
    unsortedSegmentSum.update_input_desc_##intputName(placeholder##intputIndex##_desc);                             \
    inputs.push_back(placeholder##intputIndex);
...
{
    Status ret = SUCCESS;
    auto unsortedSegmentSum = op::UnsortedSegmentSum("unsortedSegmentSum");
    std::vector<int64_t> xShape = {4, 2};
    ADD_INPUT(1, x, inDtype, xShape);
    ADD_CONST_INPUT(2, segment_ids, DT_INT64, {4});
    ADD_CONST_INPUT(3, num_segments, DT_INT64, {1});

    outputs.push_back(unsortedSegmentSum);
    return SUCCESS;
}
