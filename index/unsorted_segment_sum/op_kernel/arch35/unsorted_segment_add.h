...
/*!
 * \file unsorted_segment_add.h
 * \brief
 */

#ifndef UNSORTED_SEGMENT_SUM_ADD_H
#define UNSORTED_SEGMENT_SUM_ADD_H
...
public:
                                ...
                                const UnsortedSegmentSumOutFlTilingData* tiling)
    {
        ...
        if (inputOuterDimSize * innerDimSize == 0) {
            InitGm<TX>(output, outputOuterDimSize * innerDimSize);
            return;
        }
    ...
    __aicore__ inline void Process()
    {
        ...
        Duplicate(midRes, static_cast<TX>(0), bufAlign32);
        PipeBarrier<PIPE_V>();
        ...
        for (int32_t i = 0; i < HALFTIME; ++i) {
            for (int32_t first = 0; first < stepStride; ++first) {
                ...
                Add(midRes[OneOffset], midRes[OneOffset], midRes[TwoOffset], oneRowOutNumAlign);
            }
            stepStride /= TWO;
        }
