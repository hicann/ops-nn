...
{
    if (inputOuterDim_ / outputOuterDim_ >= IN_OUT_RATE_THRESHOLD &&
        innerDim_ * valueTypeBytes_ < INNER_DIM_THRESHOLD) {
        return true;
    }
    return false;
}
...
int64_t UnsortedSegmentSumSortSimtTiling::GetSortBufferSize(ge::DataType dataType, int64_t indexSize)
{
...
ge::graphStatus UnsortedSegmentSumSortSimtTiling::CalcTiling()
{
    ...
    while (end - start > 1) {
        ...
        if (indicesCastMode_ == 0) {
            totalIndexSize = Ops::Base::CeilAlign(mid * idTypeBytes_, ubBlockSize_) * DOUBLE +
                             Ops::Base::CeilAlign(mid * idTypeBytes_, ubBlockSize_) + ubBlockSize_ * DOUBLE +
                             Ops::Base::CeilAlign(mid * sizeof(uint32_t), ubBlockSize_);
            sortTmpSize = GetSortBufferSize(idType_, mid);
        } else {
            totalIndexSize = Ops::Base::CeilAlign(mid * idTypeBytes_, ubBlockSize_) * DOUBLE +
                             Ops::Base::CeilAlign(mid * idTypeBytes_, ubBlockSize_) + ubBlockSize_ * DOUBLE +
                             Ops::Base::CeilAlign(mid * indicesCastDtypeSize_, ubBlockSize_) * DOUBLE +
                             Ops::Base::CeilAlign(mid * sizeof(uint32_t), ubBlockSize_);
            sortTmpSize = GetSortBufferSize(indicesCastDtype_, mid);
        }
        ...
        int64_t tmpTotalSize = totalIndexSize + sortTmpSize +
                               Ops::Base::CeilAlign(mid * innerDim_ * valueTypeBytes_, ubBlockSize_) * DOUBLE;
