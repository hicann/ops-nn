...
static bool CheckNotNull(
    const aclTensor* self, const aclTensor* index, const aclTensor* source, const aclScalar* alpha,
    const aclTensor* out)
{
...
static bool CheckDtypeValid(
    const op::DataType selfDtype, const op::DataType indexDtype, const op::DataType sourceDtype,
    const op::DataType outDtype)
{
    ...
    if (!CheckType(indexDtype, INDEX_DTYPE_SUPPORT_LIST)) {
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "index dtype %s should be in dtype support list [%s].",
            op::ToString(indexDtype).GetString(), op::ToString(INDEX_DTYPE_SUPPORT_LIST).GetString());
        return false;
    }
    if (!CheckType(sourceDtype, dtypeSupportList)) {
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "source dtype %s should be in dtype support list [%s].",
            op::ToString(sourceDtype).GetString(), op::ToString(dtypeSupportList).GetString());
        return false;
    }
    if (!CheckType(outDtype, dtypeSupportList)) {
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "outDtype dtype %s should be in dtype support list [%s].",
            op::ToString(sourceDtype).GetString(), op::ToString(dtypeSupportList).GetString());
        return false;
    }
    if (selfDtype != sourceDtype) {
        OP_LOGE(
            ACLNN_ERR_PARAM_INVALID, "self dtype %s and source dtype %s must same.",
...
static bool CheckShape(
    const aclTensor* self, const aclTensor* index, const aclTensor* source, const aclTensor* out, int64_t dim)
{
    ...
    if (selfDimNum == 0) {
        ...
        if ((dim < -selfDimNum || dim >= selfDimNum) && (!(dim == 0 && selfDimNum == 0))) {
    ...
    if (self->GetViewShape().GetDimNum() != 0 &&
        self->GetViewShape().GetDimNum() != source->GetViewShape().GetDimNum()) {
    ...
    for (size_t idx = 0; idx < self->GetViewShape().GetDimNum(); idx++) {
        if (dimStd != static_cast<int64_t>(idx)) {
            if (self->GetViewShape().GetDim(idx) != source->GetViewShape().GetDim(idx)) {
                OP_LOGE(
                    ACLNN_ERR_PARAM_INVALID, "The size of self must match the size of source at dimension %ld.", dim);
                return false;
            }
        }
    }
...
static aclnnStatus CheckParams(
    const aclTensor* self, const int64_t dim, const aclTensor* index, const aclTensor* source, const aclScalar* alpha,
    const aclTensor* out)
{
    ...
    CHECK_RET(
        CheckDtypeValid(self->GetDataType(), index->GetDataType(), source->GetDataType(), out->GetDataType()),
...
static const aclTensor* DoIndexAddWithSorted(
    const aclTensor* self, const int64_t dim, const aclTensor* index, const aclTensor* source, const aclTensor* alpha,
    ...
    aclTensor* out, uint64_t* workspaceSize, aclOpExecutor** executor)
{
    ...
    rtError_t retRts = aclrtGetSysParamOpt(ACL_OPT_DETERMINISTIC, &deterministicValue);
    if (retRts != RT_ERROR_NONE) {
        OP_LOGD("aclrtGetSysParamOpt failed, fallback deterministicValue=0");
        deterministicValue = 0;
    }
    OP_LOGD("deterministicValue=%ld", deterministicValue);
    ...
    bool useNewOp =
        (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND910B ||
         ...
         (GetCurrentPlatformInfo().GetSocVersion() == SocVersion::ASCEND950 && (deterministicValue != 0))) &&
    ...
    if (self->GetDataType() == op::DataType::DT_BF16 && !useNewOp && !is91095) {
        ...
        (self->GetDataType() == op::DataType::DT_BF16 || self->GetDataType() == op::DataType::DT_FLOAT16) && useNewOp) {
    ...
    if (inplaceScatterAddSupport(selfContiguous, dim, alpha)) {
        indexAddOut =
            l0op::InplaceScatterAddAiCore(selfContiguous, indexContiguous, sourceContiguous, uniqueExecutor.get());
    } else if (useNewOp) {
        indexAddOut = DoIndexAddWithSorted(
            selfContiguous, dim, indexContiguous, sourceContiguous, alphaTensor, uniqueExecutor.get());
