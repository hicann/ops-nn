...
{
    for (int stride = 1; stride < blockDim.x; stride *= BINARY_HALF) {
        if (threadIdx.x >= stride) {
            temp[threadIdx.x + blockDim.x] = temp[threadIdx.x] + temp[threadIdx.x - stride];
        } else {
            temp[threadIdx.x + blockDim.x] = temp[threadIdx.x];
        }
        asc_syncthreads();
        stride *= BINARY_HALF;
        if (threadIdx.x >= stride) {
            temp[threadIdx.x] = temp[threadIdx.x + blockDim.x] + temp[threadIdx.x + blockDim.x - stride];
        } else {
