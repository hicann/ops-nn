...
ge::graphStatus GatherNdSimtTiling::GetIndicesShapeInfo()
{
    ...
    if (indicesDtype == ge::DT_INT64) {
        ...
        OP_LOGE_FOR_INVALID_DTYPE(
            context_->GetNodeName(), "indices", Ops::Base::ToString(indicesDtype).c_str(), "int32 or int64");
        return ge::GRAPH_FAILED;
    }
    ...
    if (curIndicesDimLen == static_cast<uint64_t>(0)) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
            context_->GetNodeName(), "indices", "0D", "The shape dim of indices must be greater than 0");
        return ge::GRAPH_FAILED;
    }
    ...
    for (uint64_t i = static_cast<uint64_t>(0); i < curIndicesDimLen - static_cast<uint64_t>(1); i++) {
        OP_LOGD(
            "GatherNdSimt", "IndicesShape: indices axis is %ld, shape value is %ld", i, orgIndicesGeShape.GetDim(i));
...
void GatherNdSimtTiling::CalcSimdTiling()
{
    ...
    uint64_t ubAviable =
        static_cast<uint64_t>((ubSize_ - INDICES_SIZE) / ubBlockSize * ubBlockSize / xDtypeSize_ / BUFFER_NUM);
...
ge::graphStatus GatherNdSimtTiling::GetXShapeInfo()
{
    ...
    if ((xDtype == ge::DT_INT64) || (xDtype == ge::DT_UINT64) || (xDtype == ge::DT_DOUBLE)) {
        ...
        (xDtype == ge::DT_FLOAT16) || (xDtype == ge::DT_BF16) || (xDtype == ge::DT_INT16) ||
        (xDtype == ge::DT_UINT16)) {
    ...
    if (curXDimLen == 0) {
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
            context_->GetNodeName(), "x", "0D", "The shape dim of x must be greater than 0");
        return ge::GRAPH_FAILED;
    }
...
ge::graphStatus GatherNdSimtTiling::GaAllLoadTiling()
{
    ...
    indicesBufferSizeLoopGa =
        std::min(indicesInner * ubBlockSize, static_cast<int64_t>(ubSize_ - NUM_TWO * MIN_OUT_UB_SIZE - allLoadXBytes));
...
ge::graphStatus GatherNdSimtTiling::DoOpTiling()
{
    ...
    bool MixkernelNotSupport =
        (indicesDtypeSize_ == INPUT_DTYPE_B32) &&
        (indicesNum_ > static_cast<uint64_t>(INT32_MAX) || outputSize_ > static_cast<uint64_t>(INT32_MAX));
...
ge::graphStatus GatherNdSimtTiling::DoNormOpTiling()
{
    ...
    uint64_t gatherCount =
        (static_cast<uint64_t>(ubSize_) - static_cast<uint64_t>(INDICES_REDUENT_SIZE)) / (xRatio + indicesRatio);
...
ge::graphStatus GatherNdSimtTiling::DoMixKernelOpTiling()
{
    ...
    uint64_t availableSingleBuf =
        (ubSize_ - static_cast<uint64_t>(HELP_BUFFER_SIZE + RESERVE_BUFFER_SIZE)) / BUFFER_NUM;
    ...
    uint64_t ubBlockSize = static_cast<uint64_t>(GetUbBlockSize(context_));
    uint64_t yUbSize =
        (singleUbProNum * xLastAxisBytes + ubBlockSize - static_cast<uint64_t>(1)) / ubBlockSize * ubBlockSize;
    uint64_t indicesUbSize =
        (singleUbProNum * singleIndicesBytes + ubBlockSize - static_cast<uint64_t>(1)) / ubBlockSize * ubBlockSize;
...
uint64_t GatherNdSimtTiling::GetTilingKey() const
{
    if (isSimd_) {
        ...
        uint64_t tilingBase =
            negativeIndexSupport_ ? static_cast<uint64_t>(NEG_INDICES_SUPPORT_BASE_KEY) : static_cast<uint64_t>(0);
    ...
    uint32_t supportAbnormalIndexKey =
        (negativeIndexSupport_ || supportOutOfBoundIndex_) ? static_cast<uint32_t>(1) : static_cast<uint32_t>(0);
    uint32_t sizeAddrType =
        ((gatherSize_ > static_cast<uint64_t>(INT32_MAX) && indicesNum_ > static_cast<uint64_t>(INT32_MAX)) ||
         outputSize_ > static_cast<uint64_t>(INT32_MAX)) ?
            static_cast<uint32_t>(1) :
            static_cast<uint32_t>(0);
    uint32_t templateKey =
        (rank_ == static_cast<uint64_t>(0) || isZeroShape_) ? static_cast<uint32_t>(1) : static_cast<uint32_t>(0);
    uint32_t rankKey =
        (rank_ == static_cast<uint64_t>(0) || isZeroShape_) ? static_cast<uint32_t>(0) : static_cast<uint32_t>(1);
...
ge::graphStatus GatherNdSimtTiling::PostTiling()
{
    if (isGaAllLoad_) {
        ...
        gaAllLoadTilingdata_.SaveToBuffer(
