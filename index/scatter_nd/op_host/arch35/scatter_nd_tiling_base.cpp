...
static const std::unordered_map<ge::DataType, uint64_t> INDICE_DATA_TYPE_TO_INT{
    {ge::DataType::DT_INT32, 100}, {ge::DataType::DT_INT64, 200}};

static const std::unordered_map<ge::DataType, uint64_t> UPDATE_DATA_TYPE_TO_INT{
    {ge::DataType::DT_INT32, 1}, {ge::DataType::DT_FLOAT16, 2}, {ge::DataType::DT_FLOAT, 3}};
...
ge::graphStatus ScatterNdTiling::GetPlatformInfo()
{
    ...
    OP_CHECK_IF((aivNum <= 0), OP_LOGE(opName, "ScatterNdTiling fail to get coreNum."), return ge::GRAPH_FAILED);
    coreNum_ = aivNum;
    ...
    OP_CHECK_IF(
        (res != ge::GRAPH_SUCCESS), OP_LOGE(opName, "SetLocalMemorySize ubSize = %u failed.", ubSize_),
        return ge::GRAPH_FAILED);
...
ge::graphStatus ScatterNdTiling::GetShapeAttrsInfo()
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
            opName, "indices", std::to_string(rankSize_).c_str(),
    ...
    OP_CHECK_IF(
        (shapeRank < rankSize_),
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
            opName, "shape", std::to_string(shapeRank).c_str(),
    ...
    if (context_->GetDeterministic() &&
        scatterNdDeterminsiticType.find(updateDtype) != scatterNdDeterminsiticType.end()) {
    ...
    OP_CHECK_IF(
        (outputSize_ == 0 && preAxis_ != 0),
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            opName, "shape", Ops::Base::ToString(shapeValue).c_str(), "All axes of shape must be positive number"),
        return ge::GRAPH_FAILED);
...
ge::graphStatus ScatterNdTiling::getRestAvailableSize(
    uint64_t sampleNum, uint64_t valueTypeBytes, uint64_t originalSize, uint64_t postAxisSize_, ge::DataType idType)
{
    uint64_t indicesDtypeSize = ge::GetSizeByDataType(idType);
    OP_CHECK_IF(
        indicesDtypeSize <= 0,
        OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(
            opName, "indices", Ops::Base::ToString(idType).c_str(), "The dtype size of indices must be greater than 0"),
        return ge::GRAPH_FAILED);
    ...
    uint64_t occupy = sampleNum * Ops::Base::CeilAlign(FLOAT_BYTES * postAxisSize_, ubBlock) +
                      ...
                      Ops::Base::CeilAlign(sampleNum * UINT32_BYTES, ubBlock) + TWO * TWO * UB_AGLIN_VALUE +
                      TWO * UB_AGLIN_VALUE + GetSortTmpSize(idType, sampleNum, false) +
...
void ScatterNdTiling::BlockTiling()
{
    ...
    OP_LOGD(
        opName,
        "updateShapeSize_ = %lld, blockFactor = %lld, blockAlignFactor = %lld,"
    ...
    OP_LOGD(
        opName,
        "outputShapeSize = %lld, clrBlockFactor = %lld,"
        " clrBlockTilingSize = %d, clrTailBlockTilingSize = %d",
        outputShapeSize_, clrBlockFactor, clrBlockTilingSize_, clrTailBlockTilingSize_);
    return;
}
...
ge::graphStatus ScatterNdTiling::UbTiling()
{
    ...
    OP_CHECK_IF(
        sliceSize_ == static_cast<uint64_t>(0),
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
            opName, "updates", Ops::Base::ToString(updateShape).c_str(), "All axes of shape must be positive number"),
        return ge::GRAPH_FAILED);
    ...
    OP_LOGD(opName, "sliceUb = %lu, halfUbSize = %u, ubTilingSize = %u", sliceUb, halfUbSize, ubTilingSize_);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus ScatterNdTiling::ScatterNdDeterministicTiling()
{
    ...
    if (halfUBSize < (static_cast<uint64_t>(THREE) * FLOAT_BYTES * afterAxis_ + UB_RESERVED) ||
        (preAxis_ < (static_cast<uint64_t>(coreNum_) / TWO)) || afterAxis_ * FLOAT_BYTES > MAX_SIZE_QUANT) {
    ...
    if (DeterministicFlag) {
        ...
        tailCoreHandleCol_ =
            static_cast<uint64_t>(afterAxis_) - (logicCoreNum_ - static_cast<uint64_t>(1)) * perCoreHandleCol_;
        ...
        if (perCoreHandleCol_ * updateDtypeSize_ + indiceShapeSize_ * indiceDtypeSize_ > ubSize_) {
            ...
            indicesUbFactor_ = Ops::Base::FloorAlign(
                                   ubSize_ - updatesUbFactor_ * updateDtypeSize_ * BUFFER_NUM,
                                   static_cast<uint64_t>(Ops::Base::GetUbBlockSize(context_))) /
                               (rankSize_ * indiceDtypeSize_);
        }

        indicesLoopSize_ = Ops::Base::CeilDiv(preAxis_, indicesUbFactor_);
        indicesTailUbFactor_ =
            static_cast<uint64_t>(preAxis_) - (indicesLoopSize_ - static_cast<uint64_t>(1)) * indicesUbFactor_;
        updatesLoopSize_ = Ops::Base::CeilDiv(perCoreHandleCol_, updatesUbFactor_);
        updatesTailUbFactor_ =
            static_cast<uint64_t>(perCoreHandleCol_) - (updatesLoopSize_ - static_cast<uint64_t>(1)) * updatesUbFactor_;
        tailCoreColsLoopSize_ = Ops::Base::CeilDiv(tailCoreHandleCol_, updatesUbFactor_);
        tailCoreColsTailUbFactor_ = static_cast<uint64_t>(tailCoreHandleCol_) -
                                    (tailCoreColsLoopSize_ - static_cast<uint64_t>(1)) * updatesUbFactor_;
    ...
    uint64_t oneIdOccupyBytes = updateDtypeSize_ * afterAxis_ + static_cast<uint64_t>(FLOAT_BYTES) * afterAxis_ +
                                indiceDtypeSize_ * static_cast<uint64_t>(rankSize_) +
                                static_cast<uint64_t>(TWO) * indiceDtypeSize_ + static_cast<uint64_t>(UINT32_BYTES) +
    ...
    tailCoreIndicesTailUbFactor_ =
        tailCoreHandleIndicesNum - (tailCoreIndicesLoopSize_ - static_cast<uint64_t>(1)) * indicesUbFactor_;
    ...
    if (isIdxSplit == static_cast<uint64_t>(0)) {
        ...
        deQuantizeCoreNum_ = Ops::Base::CeilDiv(rankFusedAxis_, perCoreHandleOutputRows_);
        tailCoreHandleOutputRows_ =
            rankFusedAxis_ - (deQuantizeCoreNum_ - static_cast<uint64_t>(1)) * perCoreHandleOutputRows_;
...
ge::graphStatus ScatterNdTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }
...
uint64_t ScatterNdTiling::GetTilingKey() const
{
    ...
    if (iter != INDICE_DATA_TYPE_TO_INT.end()) {
        ...
        OP_LOGE_FOR_INVALID_DTYPE(opName, "indices", Ops::Base::ToString(indiceDtype).c_str(), "int32 or int64");
    }
    ...
    if (iter != UPDATE_DATA_TYPE_TO_INT.end()) {
        ...
        OP_LOGE_FOR_INVALID_DTYPE(
            opName, "updates", Ops::Base::ToString(updateDtype).c_str(), "float16, float32 or int32");
    }
...
ge::graphStatus ScatterNdTiling::GetWorkspaceSize()
{
    ...
    if (isDeterminTemplate_ && isDeterministic_) {
        uint64_t postVarAlignSize =
            Ops::Base::CeilAlign(
                afterAxis_ * sizeof(float), static_cast<uint64_t>(Ops::Base::GetUbBlockSize(context_))) /
            sizeof(float);
