...
static const std::unordered_map<ge::DataType, uint64_t> INDICE_DATA_TYPE_TO_INT{{ge::DataType::DT_INT32, 100},
...
ge::graphStatus ScatterNdTiling::GetPlatformInfo()
{
    ...
    OP_CHECK_IF((res != ge::GRAPH_SUCCESS), OP_LOGE(opName, "SetLocalMemorySize ubSize = %u failed.", ubSize_),
                return ge::GRAPH_FAILED);
...
ge::graphStatus ScatterNdTiling::GetShapeAttrsInfo()
{
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(opName, "indices", std::to_string(rankSize_).c_str(),
    ...
    OP_CHECK_IF((outputSize_ == 0 && preAxis_ != 0),
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(opName, "shape", Ops::Base::ToString(shapeValue).c_str(),
...
ge::graphStatus ScatterNdTiling::getRestAvailableSize(uint64_t sampleNum, uint64_t valueTypeBytes,
                                                      uint64_t originalSize, uint64_t postAxisSize_,
                                                      ge::DataType idType)
{
    uint64_t indicesDtypeSize = ge::GetSizeByDataType(idType);
    OP_CHECK_IF(indicesDtypeSize <= 0,
                OP_LOGE_FOR_INVALID_DTYPE_WITH_REASON(opName, "indices", Ops::Base::ToString(idType).c_str(),
...
ge::graphStatus ScatterNdTiling::UbTiling()
{
    ...
    OP_CHECK_IF(sliceSize_ == static_cast<uint64_t>(0),
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(opName, "updates", Ops::Base::ToString(updateShape).c_str(),
                                                      "All axes of shape must be positive number"),
                return ge::GRAPH_FAILED);
...
ge::graphStatus ScatterNdTiling::ScatterNdDeterministicTiling()
{
    ...
    if (DeterministicFlag) {
        ...
        tailCoreHandleCol_ = static_cast<uint64_t>(afterAxis_) -
                             (logicCoreNum_ - static_cast<uint64_t>(1)) * perCoreHandleCol_;
        ...
        indicesTailUbFactor_ = static_cast<uint64_t>(preAxis_) -
                               (indicesLoopSize_ - static_cast<uint64_t>(1)) * indicesUbFactor_;
        updatesLoopSize_ = Ops::Base::CeilDiv(perCoreHandleCol_, updatesUbFactor_);
        updatesTailUbFactor_ = static_cast<uint64_t>(perCoreHandleCol_) -
                               (updatesLoopSize_ - static_cast<uint64_t>(1)) * updatesUbFactor_;
    ...
    tailCoreIndicesTailUbFactor_ = tailCoreHandleIndicesNum -
                                   (tailCoreIndicesLoopSize_ - static_cast<uint64_t>(1)) * indicesUbFactor_;
    ...
    if (isIdxSplit == static_cast<uint64_t>(0)) {
        ...
        deQuantizeCoreNum_ = Ops::Base::CeilDiv(rankFusedAxis_, perCoreHandleOutputRows_);
        tailCoreHandleOutputRows_ = rankFusedAxis_ -
                                    (deQuantizeCoreNum_ - static_cast<uint64_t>(1)) * perCoreHandleOutputRows_;
...
uint64_t ScatterNdTiling::GetTilingKey() const
{
    ...
    if (iter != UPDATE_DATA_TYPE_TO_INT.end()) {
        ...
        OP_LOGE_FOR_INVALID_DTYPE(opName, "updates", Ops::Base::ToString(updateDtype).c_str(),
...
ge::graphStatus ScatterNdTiling::GetWorkspaceSize()
{
    ...
    if (isDeterminTemplate_ && isDeterministic_) {
        uint64_t postVarAlignSize = Ops::Base::CeilAlign(afterAxis_ * sizeof(float),
                                                         static_cast<uint64_t>(Ops::Base::GetUbBlockSize(context_))) /
                                    sizeof(float);
