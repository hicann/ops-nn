...
/*!
 * \file test_geir_scatter_nd_max.cpp
 * \brief Graph-mode (GE IR) sample for ScatterNdMax.
 *
 * Modeled on activation/relu_grad/examples/test_geir_relu_backward.cpp.
 * ScatterNdMax computes the element-wise maximum between an input tensor
...
// Protobuf-based operator header for ScatterNdMax
#include "../op_graph/scatter_nd_max_proto.h"
    ...
    std::vector<Operator>& outputs, Graph& graph)
{
    ...
    // ScatterNdMax: var_out = scatter_nd_max(var, indices, updates)
    auto scatterNdMax = op::ScatterNdMax("scatter_nd_max");
    ...
    // -------------------- Connect to ScatterNdMax --------------------
    scatterNdMax.set_input_var(varData);
    scatterNdMax.set_input_indices(indicesData);
    scatterNdMax.set_input_updates(updatesData);

    // Optional attribute: use_locking (default false)
    scatterNdMax.set_attr_use_locking(false);

    // Output tensor (updated var) has same shape & dtype as var
    TensorDesc outDesc = TensorDesc(ge::Shape(varShape), FORMAT_ND, inDtype);
    scatterNdMax.update_output_desc_var(outDesc);

    inputs.push_back(varData);
    inputs.push_back(indicesData);
    inputs.push_back(updatesData);
    outputs.push_back(scatterNdMax);
...
{
    const char* graph_name = "tc_ge_irrun_test_scatter_nd_max";
    ...
    for (int i = 0; i < output_num; i++) {
        ...
        string output_file = "./tc_ge_irrun_scatter_nd_max_output_" + std::to_string(i) + ".bin";
