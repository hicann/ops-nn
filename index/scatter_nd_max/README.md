...
- 算子功能：根据indices在给定变量内，在updates和单个值或切片之间求最大值。
- 计算公式：ref[indices[k]]=max(ref[indices[k]], updates[k])
    - ref是一个维度为P的张量Tensor；
...
| 调用方式   | 样例代码           | 说明                                         |
| ---------------- | --------------------------- | --------------------------------------------------- |
| 图模式 | [test_geir_scatter_nd_max](examples/test_geir_scatter_nd_max.cpp) | 通过GE IR构图方式调用ScatterNdMax算子。 |
