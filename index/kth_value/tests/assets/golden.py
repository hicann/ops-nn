...
"""Stable KthValue Kernel/GEIR reference and PyTorch benchmark.

Values use native ``torch.kthvalue`` where supported.  Its tie index is not a
...
__spec__ = {"kth_value": "KthValueKernelSpec"}
__golden__ = {"kernel": {"kth_value": "kth_value_golden"}}
__input__ = {"kernel": {"kth_value": "kth_value_input"}}
...
def _stable_kth_value(x, k, dim):
    axis = int(dim)
    rank = int(k) - 1
    if rank < 0 or rank >= x.shape[axis]:
        raise ValueError(f"k={k} is outside [1, {x.shape[axis]}]")
    values, indices = torch.sort(x, dim=axis, stable=True)
    stable_value = values.narrow(axis, rank, 1)
    if x.dtype not in _NATIVE_UNSUPPORTED_DTYPES:
        native_value = torch.kthvalue(x, int(k), dim=axis, keepdim=True).values
        if x.is_floating_point():
            special = torch.isnan(native_value) | (native_value == 0)
            native_value = torch.where(special, stable_value, native_value)
        stable_value = native_value
    return stable_value, indices.narrow(axis, rank, 1)


def _fill_pattern_along_axis(tensor, values, axis):
    """Repeat a pattern inside each logical row of the selected axis."""
    axis = int(axis)
    if axis < 0:
        axis += tensor.ndim
...
def kth_value_golden(x, *, k, dim=-1, **kwargs):
    return [_to_array(y) for y in _stable_kth_value(_from_array(x), k, dim)]


class KthValueThirdParty:
    """Timed native PyTorch API with fallback for unsupported unsigned types."""

    def __init__(self, *, k, dim=-1, **kwargs):
        self.k = int(k)
        self.dim = int(dim)

    def __call__(self, x, **kwargs):
        if x.dtype in _NATIVE_UNSUPPORTED_DTYPES:
            return list(_stable_kth_value(x, self.k, self.dim))
        return torch.kthvalue(x, self.k, dim=self.dim, keepdim=True)


class KthValueKernelSpec:
    golden = staticmethod(kth_value_golden)
    customize_inputs = staticmethod(kth_value_input)
    third_party = {"torch": KthValueThirdParty}
