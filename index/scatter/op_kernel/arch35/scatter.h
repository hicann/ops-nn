                                                  ...
                                                  __ubuf__ T3* shiftArr, __ubuf__ T3* mArr)
{
    ...
    t = __umulhi(static_cast<uint32_t>(remain), static_cast<uint32_t>(mArr[INDEX_ONE]));
    t = t + remain;
                                                  ...
                                                  __ubuf__ T3* factorArr, __ubuf__ T3* shiftArr, __ubuf__ T3* mArr)
{
    ...
    t = __umulhi(static_cast<uint32_t>(remain), static_cast<uint32_t>(mArr[INDEX_TWO]));
    t = t + remain;
                                                        ...
                                                        __ubuf__ T3* shiftArr, __ubuf__ T3* mArr)
{
    ...
    i = Simt::UintDiv(static_cast<uint64_t>(addr), static_cast<uint64_t>(mArr[INDEX_ZERO]),
                      static_cast<uint64_t>(shiftArr[INDEX_ZERO]));
    int64_t remain = addr - i * factorArr[INDEX_ZERO];
    j = Simt::UintDiv(static_cast<uint64_t>(remain), static_cast<uint64_t>(mArr[INDEX_ONE]),
                      static_cast<uint64_t>(shiftArr[INDEX_ONE]));
}
                                                        ...
                                                        __ubuf__ T3* mArr)
{
    ...
    j = Simt::UintDiv(static_cast<uint64_t>(remain), static_cast<uint64_t>(mArr[INDEX_ONE]),
                      static_cast<uint64_t>(shiftArr[INDEX_ONE]));
    remain = remain - j * factorArr[INDEX_ONE];
    k = Simt::UintDiv(static_cast<uint64_t>(remain), static_cast<uint64_t>(mArr[INDEX_TWO]),
                      static_cast<uint64_t>(shiftArr[INDEX_TWO]));
}
                                                                         ...
                                                                         int32_t startOffset, int32_t ubCount)
{
    for (uint32_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        int32_t dstOffset = addr + i * coefArr[INDEX_ZERO] + j * coefArr[INDEX_ONE] +
                            (int32_t)(indicesUb[i - indexStart]) * coefArr[INDEX_SEVEN];
                                                                         ...
                                                                         int32_t startOffset, int32_t ubCount)
{
    for (uint32_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        int32_t dstOffset = addr + i * coefArr[INDEX_FOUR] + j * coefArr[INDEX_FIVE] + k * coefArr[INDEX_SIX] +
                            (int32_t)indicesUb[i - indexStart];
        outputGm[dstOffset] = updatesUb[idx];
    }
}
                                                                         ...
                                                                         int32_t startOffset, int32_t ubCount)
{
    for (uint32_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        uint32_t j;
        CalcIndex2<T3>(addr, i, j, factorArr, shiftArr, mArr);
        uint32_t dstOffset = addr + (uint32_t)indicesUb[(i - indexStart) * 2] * coefArr[INDEX_TWO] -
                             i * coefArr[INDEX_THREE] + j * coefArr[INDEX_ONE] +
                                                                         ...
                                                                         int32_t startOffset, int32_t ubCount)
{
    for (uint32_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        uint32_t k;
        CalcIndex3<T3>(addr, i, j, k, factorArr, shiftArr, mArr);
        int32_t dstOffset = addr + (int32_t)indicesUb[(i - indexStart) * 2] * coefArr[INDEX_TWO] -
                            i * factorArr[INDEX_ZERO] + j * coefArr[INDEX_FIVE] + k * coefArr[INDEX_SIX] +
                            (int32_t)indicesUb[(i - indexStart) * 2 + 1];
        outputGm[dstOffset] = updatesUb[idx];
    }
}
    ...
    __ubuf__ T3* shiftArr, __ubuf__ T3* mArr, int64_t indexStart, int64_t startOffset, int64_t ubCount)
{
    for (int64_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        int64_t dstOffset = addr + i * coefArr[INDEX_ZERO] + j * coefArr[INDEX_ONE] +
                            (int64_t)(indicesUb[i - indexStart]) * coefArr[INDEX_SEVEN];
        outputGm[dstOffset] = updatesUb[idx];
    }
}
    ...
    __ubuf__ T3* shiftArr, __ubuf__ T3* mArr, int64_t indexStart, int64_t startOffset, int64_t ubCount)
{
    for (int64_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        int64_t dstOffset = addr + i * coefArr[INDEX_FOUR] + j * coefArr[INDEX_FIVE] + k * coefArr[INDEX_SIX] +
                            (int64_t)indicesUb[i - indexStart];
        outputGm[dstOffset] = updatesUb[idx];
    }
}
    ...
    __ubuf__ T3* shiftArr, __ubuf__ T3* mArr, int64_t indexStart, int64_t startOffset, int64_t ubCount)
{
    for (int64_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        uint64_t j;
        CalcUint64Index2(addr, i, j, factorArr, shiftArr, mArr);
        uint64_t dstOffset = addr + (uint64_t)indicesUb[(i - indexStart) * 2] * coefArr[INDEX_TWO] -
                             i * coefArr[INDEX_THREE] + j * coefArr[INDEX_ONE] +
    ...
    __ubuf__ T3* shiftArr, __ubuf__ T3* mArr, int64_t indexStart, int64_t startOffset, int64_t ubCount)
{
    for (int64_t idx = threadIdx.x; idx < ubCount; idx += blockDim.x) {
        ...
        uint64_t k;
        CalcUint64Index3(addr, i, j, k, factorArr, shiftArr, mArr);
        int64_t dstOffset = addr + (int64_t)indicesUb[(i - indexStart) * 2] * coefArr[INDEX_TWO] -
                            i * factorArr[INDEX_ZERO] + j * coefArr[INDEX_FIVE] + k * coefArr[INDEX_SIX] +
                            (int64_t)indicesUb[(i - indexStart) * 2 + 1];
        outputGm[dstOffset] = updatesUb[idx];
    }
}
...
public:
    ...
    __aicore__ inline void Init(GM_ADDR x, GM_ADDR indices, GM_ADDR updates, GM_ADDR y, GM_ADDR workspace,
                                const ScatterTilingData* tilingData, TPipe* pipeIn)
    {
        pipe = pipeIn;
        tiling = tilingData;
    ...
    __aicore__ inline void Process()
    {
        ...
        int64_t totalUpdatesCount = tiling->updatesDim0 * tiling->updatesDim1 * tiling->updatesDim2 *
                                    tiling->updatesDim3;
        ...
        if (blockIdx >= blockCoreNum) {
            ...
            if (totalUpdatesCount > (tiling->aivCoreNum * MIN_FACTOR)) {
                ...
                blockFactor = (tiling->updatesDim0 * tiling->updatesDim1 * tiling->updatesDim2 * tiling->updatesDim3) %
                              MIN_FACTOR;
            }
        }
        ...
        if (blockFactor > 0) {
            ...
            if (indicesSize <= tiling->ubSize / UB_DIVISION_COUNT) {
                indicesUbSize = (indicesSize + BLOCK_SIZE * INDICES_BUFFER_NUM - 1) /
            ...
            for (U loop = 0; loop < ubLoop; loop++) {
                ...
                LocalTensor<T3> shiftArr = simtBuf_.GetWithOffset<T3>(SHIFT_ARR_LEN,
                                                                      (FACTOR_ARR_LEN + COEF_ARR_LEN) * sizeof(T3));
                LocalTensor<T3> mArr = simtBuf_.GetWithOffset<T3>(
                    M_ARR_LEN, (FACTOR_ARR_LEN + COEF_ARR_LEN + SHIFT_ARR_LEN) * sizeof(T3));
