...
TEST_F(IndexByTensorStaticFusionPassTest, indexbytensorFusionZeroIndicesFail)
{
    std::vector<int64_t> dimsX{2, 3, 4, 5};
...
TEST_F(IndexByTensorStaticFusionPassTest, indexbytensorFusionOutputShapeAndParamsVerify)
{
    std::vector<int64_t> dimsX{2, 3, 4, 5};
    std::vector<std::vector<int64_t>> indicesDims{{3, 2}, {3, 2}};
    std::vector<int64_t> indicesMask{0, 1, 1};
    ...
    for (auto node : graph->GetAllNodes()) {
        ...
        if (type == "Index") {
            ...
            // Verify Index node output desc exists
            TensorDesc outputDesc;
            EXPECT_EQ(node.GetOutputDesc(0, outputDesc), GRAPH_SUCCESS);
            auto outputShape = outputDesc.GetShape().GetDims();
            if (!outputShape.empty()) {
                EXPECT_EQ(outputShape.size(), dimsY.size());
                for (size_t i = 0; i < dimsY.size(); i++) {
                    EXPECT_EQ(outputShape[i], dimsY[i]);
                }
            }

            // Verify Index node input count: x + indexed_sizes + indexed_strides + N indices
            size_t expectedInputCount = 3 + indicesDims.size();
            EXPECT_EQ(node.GetInputsSize(), expectedInputCount);

            // Verify indexed_sizes input (input 1) dtype
            TensorDesc indexedSizesDesc;
            if (node.GetInputDesc(1, indexedSizesDesc) == GRAPH_SUCCESS) {
                EXPECT_EQ(indexedSizesDesc.GetDataType(), DT_INT64);
            }

            // Verify indexed_strides input (input 2) dtype
            TensorDesc indexedStridesDesc;
            if (node.GetInputDesc(2, indexedStridesDesc) == GRAPH_SUCCESS) {
                EXPECT_EQ(indexedStridesDesc.GetDataType(), DT_INT64);
            }
        }
    }
    EXPECT_TRUE(findIndex);
}

TEST_F(IndexByTensorStaticFusionPassTest, indexbytensorFusionOneIndexOutputShapeVerify)
{
    std::vector<int64_t> dimsX{2048, 128};
    std::vector<std::vector<int64_t>> indicesDims{{8, 1024}};
    std::vector<int64_t> indicesMask{1};
    ...
    for (auto node : graph->GetAllNodes()) {
        ...
        if (type == "Index") {
            ...
            // Verify input count: x + indexed_sizes + indexed_strides + 1 index = 4
            size_t expectedInputCount = 3 + indicesDims.size();
            EXPECT_EQ(node.GetInputsSize(), expectedInputCount);

            // Verify indexed_sizes input dtype
            TensorDesc indexedSizesDesc;
            if (node.GetInputDesc(1, indexedSizesDesc) == GRAPH_SUCCESS) {
                EXPECT_EQ(indexedSizesDesc.GetDataType(), DT_INT64);
            }
