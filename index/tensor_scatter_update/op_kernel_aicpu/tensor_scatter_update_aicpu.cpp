...
uint32_t UpdateOneOutput(const CpuKernelContextInfo& info, const UpdateOutputArgs<T, Index>& args, uint64_t update_idx)
{
    ...
    for (uint64_t j = 0; j < info.index_depth; ++j) {
        ...
        const bool out_of_bounds = !(args.indices_data[index_d] >= 0 &&
                                     args.indices_data[index_d] < info.x_shape->GetDimSize(j));
        if (out_of_bounds) {
...
uint32_t DataTypeCheck(const CpuKernelContext& ctx)
{
    ...
    KERNEL_CHECK_FALSE(
        (ctx.Input(kIndicesIndex)->GetDataType() == DT_INT32 || ctx.Input(kIndicesIndex)->GetDataType() == DT_INT64),
        KERNEL_STATUS_PARAM_INVALID, "The indices type only supports int32 or int64, please check!");
    return KERNEL_STATUS_OK;
}
...
uint32_t InitializeOutput(const CpuKernelContextInfo& info)
{
    ...
    auto shard_copy = [&](uint64_t start, uint64_t end) {
        for (uint64_t i = start; i < end; ++i) {
            ...
            if (mem_ret != EOK) {
                KERNEL_LOG_ERROR("Initial memory copy failed[%d]. Offset is %lu, copy size is %lu", mem_ret, ptr_offset,
                                 core_size);
...
uint32_t IndicesCompute(const CpuKernelContextInfo& info)
{
    ...
    try {
        ...
        KERNEL_LOG_ERROR(
            "TensorScatterUpdate op params indices and x doesn't support tensor types: [%s][%s], e.what:[%s]",
...
uint32_t TensorScatterUpdateCpukernel::GetInputAndCheck(const CpuKernelContextInfo& info)
{
    ...
    for (uint64_t i = 0; i + indices_outer_dims < info.updates_dims; ++i) {
        KERNEL_CHECK_FALSE(
            (info.updates_shape->GetDimSize(i + indices_outer_dims) == info.x_shape->GetDimSize(info.index_depth + i)),
            KERNEL_STATUS_PARAM_INVALID,
