...
TEST_F(TEST_TENSOR_SCATTER_UPDATE_UT, DATA_TYPE_DT_FLOAT16_INT64_SUCC)
{
    ...
    Eigen::half input_updates[10] = {Eigen::half(-1), Eigen::half(-2),  Eigen::half(-3), Eigen::half(-4),
                                     Eigen::half(-5), Eigen::half(-10), Eigen::half(-9), Eigen::half(-8),
                                     Eigen::half(-7), Eigen::half(-6)};
    Eigen::half expect_output[15] = {Eigen::half(-1), Eigen::half(-2), Eigen::half(-3),  Eigen::half(-4),
                                     Eigen::half(-5), Eigen::half(5),  Eigen::half(6),   Eigen::half(7),
                                     Eigen::half(8),  Eigen::half(9),  Eigen::half(-10), Eigen::half(-9),
                                     Eigen::half(-8), Eigen::half(-7), Eigen::half(-6)};
    RunTensorScatterUpdateKernel(shapes, data_types, input_x, input_indices, input_updates, expect_output);
}
