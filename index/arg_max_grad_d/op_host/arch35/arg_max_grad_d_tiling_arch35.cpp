 ...
 */
/*!
 * \file arg_max_grad_d_tiling_arch35.cpp
 * \brief
 */
#include <cstring>
...
#include "../op_kernel/arch35/arg_max_grad_d_tiling_data.h"
#include "../op_kernel/arch35/arg_max_grad_d_tiling_key.h"
...
// 与 op_kernel/arch35/arg_max_grad_d_nd.h 的 BUFFER_NUM 一致
...
class ArgMaxGradDTiling {
public:
    explicit ArgMaxGradDTiling(gert::TilingContext* context) : context_(context) {};
...
private:
    ...
    ArgMaxGradDArch35TilingData* tilingData_{nullptr};
...
ge::graphStatus ArgMaxGradDTiling::CheckDtype()
{
    ...
    OP_CHECK_IF((indicesDesc->GetDataType() != ge::DT_INT32),
                ...
                return ge::GRAPH_FAILED);
    auto assistDesc = context_->GetInputDesc(INPUT_ASSIST_IDX);
    OPS_CHECK_NULL_WITH_CONTEXT(context_, assistDesc);
    OP_CHECK_IF((assistDesc->GetDataType() != ge::DT_INT32),
                OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "assist",
                                          ToString(assistDesc->GetDataType()).c_str(), "INT32"),
                return ge::GRAPH_FAILED);

    return ge::GRAPH_SUCCESS;
}

ge::graphStatus ArgMaxGradDTiling::CheckShape()
{
    auto varShapePtr = context_->GetInputShape(INPUT_VAR_IDX);
    OPS_CHECK_NULL_WITH_CONTEXT(context_, varShapePtr);
    auto assistShapePtr = context_->GetInputShape(INPUT_ASSIST_IDX);
    OPS_CHECK_NULL_WITH_CONTEXT(context_, assistShapePtr);
    ...
    // 与 A2 check_param 的两条硬校验一致: var.shape == assist.shape、indices.shape == updates.shape
    OP_CHECK_IF((assistShapePtr->GetStorageShape() != varShape),
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "assist",
                                                      Ops::Base::ToString(assistShapePtr->GetStorageShape()).c_str(),
                                                      "must be the same as var shape"),
                return ge::GRAPH_FAILED);
...
ge::graphStatus ArgMaxGradDTiling::CheckAttrAndSplitLayout()
{
    auto varShapePtr = context_->GetInputShape(INPUT_VAR_IDX);
    OPS_CHECK_NULL_WITH_CONTEXT(context_, varShapePtr);
    const auto& varShape = varShapePtr->GetStorageShape();
...
void ArgMaxGradDTiling::CalCoreSplit()
{
...
ge::graphStatus ArgMaxGradDTiling::CalUbSplit()
{
    bool innerIsOne = (tilingData_->inner == 1);
    // 每个元素的 UB 占用: var + assist(int32) + out + 掩码; inner>1 时另加 indices(int32) 与 updates
    ...
    OP_CHECK_IF((static_cast<int64_t>(ubSize_) <= bytesPerPoint * vlInt32_),
    ...
    int64_t colsPerChunk = budget / vlInt32_ * vlInt32_; // 向下取 VL 对齐
    ...
    int64_t alignSpan = Ops::Base::CeilAlign(spanElems > 0 ? spanElems : vlInt32_, vlInt32_);
...
void ArgMaxGradDTiling::PrintTilingData()
{
    auto nodeName = context_->GetNodeName();
    OP_LOGD(nodeName, "ArgMaxGradD tiling: outer=%ld dimSize=%ld inner=%ld totalElems=%ld", tilingData_->outer,
            tilingData_->dimSize, tilingData_->inner, tilingData_->totalElems);
    OP_LOGD(nodeName, "ArgMaxGradD tiling: realCoreNum=%ld elemsPerCore=%ld colsPerChunk=%ld", realCoreNum_,
            tilingData_->elemsPerCore, tilingData_->colsPerChunk);
}

ge::graphStatus ArgMaxGradDTiling::Init()
{
    ...
    tilingData_ = context_->GetTilingData<ArgMaxGradDArch35TilingData>();
    OP_CHECK_IF((tilingData_ == nullptr), OP_LOGE(context_->GetNodeName(), "get tilingdata ptr failed"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        (memset_s(tilingData_, sizeof(ArgMaxGradDArch35TilingData), 0, sizeof(ArgMaxGradDArch35TilingData)) != EOK),
        OP_LOGE(context_->GetNodeName(), "memset tilingdata failed"), return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}

ge::graphStatus ArgMaxGradDTiling::DoTiling()
{
...
static ge::graphStatus Tiling4ArgMaxGradD(gert::TilingContext* context)
{
    if (context == nullptr) {
        return ge::GRAPH_FAILED;
    }
    ArgMaxGradDTiling tilingImpl(context);
    if (tilingImpl.Init() != ge::GRAPH_SUCCESS) {
        OP_LOGE(context->GetNodeName(), "Tiling4ArgMaxGradD init failed.");
        return ge::GRAPH_FAILED;
    }
    if (tilingImpl.DoTiling() != ge::GRAPH_SUCCESS) {
        OP_LOGE(context->GetNodeName(), "Tiling4ArgMaxGradD do tiling failed.");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}

static ge::graphStatus TilingParse4ArgMaxGradD(gert::TilingParseContext* context)
{
    if (context == nullptr) {
        OP_LOGE("ArgMaxGradD", "Tiling parse context is nullptr");
        return ge::GRAPH_FAILED;
    }
    return ge::GRAPH_SUCCESS;
}
...
IMPL_OP_OPTILING(ArgMaxGradD).Tiling(Tiling4ArgMaxGradD).TilingParse<ArgMaxGradDCompileInfo>(TilingParse4ArgMaxGradD);
} // namespace optiling
