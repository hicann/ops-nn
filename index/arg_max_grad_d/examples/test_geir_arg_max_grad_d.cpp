...
/*!
 * \file test_geir_arg_max_grad_d.cpp
 * \brief GE graph construction sample for ArgMaxGradD (arch35 ND format).
 */
#include <iostream>
#include <string.h>
#include <stdint.h>
...
#include "../op_graph/arg_max_grad_d_proto.h"
...
// 数据只为构图打通, 不校验数值(assist 另由 GenAssist 按序号填)
...
{
    Status ret = SUCCESS;
    auto op_node = op::ArgMaxGradD("arg_max_grad_d_op");
    // var/assist 同形 (outer, D, inner); indices/updates 把 dimension 轴置 1
    ...
    ADD_INPUT(4, assist, DT_INT32, varShape);
