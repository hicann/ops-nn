...
template <class T>
T inline CeilDivide(T num1, T num2)
{
    if (num2 == 0) {
        return 0;
    }
...
static ge::graphStatus CheckShape(const gert::TilingContext* context, MapIndexTilingParam& tilingParam)
{
    ...
    OP_CHECK_IF(yShape.GetDim(0) != 1,
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context->GetNodeName(), "y", std::to_string(yShape.GetDim(0)),
                                                         "The shape dim of y must be 1"),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(xShape.GetDim(0) > X_MAX_DIM0,
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(context->GetNodeName(), "x", std::to_string(xShape.GetDim(0)),
                                                         "The shape dim of x must be within the range [1, 24000]"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(
        dataSeqShape.GetDimNum() != 1,
    ...
    OP_CHECK_IF(dataSeqShape.GetDim(0) % xShape.GetDim(0) != 0,
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    context->GetNodeName(), "data_seq", std::to_string(dataSeqShape.GetDim(0)),
                    "Shape[0] of data_seq must be exactly divisible by shape[0] of x"),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(tilingParam.Dim0Size > DATA_SEQ_MAX_DIM0,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context->GetNodeName(), "data_seq_len",
                                                      std::to_string(tilingParam.Dim0Size),
                                                      "The value of data_seq_len must be less than 256"),
                return ge::GRAPH_FAILED);
    ...
    if (tilingParam.hasLevelIndex) {
        ...
        OP_CHECK_IF(levelIndexShape.GetDimNum() != 1,
                    OP_LOGE_FOR_INVALID_SHAPEDIM(context->GetNodeName(), "level_index",
                                                 std::to_string(levelIndexShape.GetDimNum()), "1"),
                    return ge::GRAPH_FAILED);
        OP_CHECK_IF(tilingParam.Dim0Size != levelIndexShape.GetDim(0),
                    OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                        context->GetNodeName(), "level_index", std::to_string(levelIndexShape.GetDim(0)),
                        "Shape[0] of level_index must be equal to shape[0] of data_seq"),
                    return ge::GRAPH_FAILED);
    }
...
static ge::graphStatus CheckAttr(const gert::TilingContext* context)
{
    ...
    OP_CHECK_IF((*attrTransPose), OP_LOGE_FOR_INVALID_VALUE(context->GetNodeName(), "transpose", "true", "false"),
                return ge::GRAPH_FAILED);
...
static ge::graphStatus DoTiling(const gert::TilingContext* context, MapIndexTilingParam& tilingParam)
{
    ...
    tilingParam.tailCoreProcessNum = tilingParam.Dim0Size -
                                     tilingParam.normalCoreProcessNum * (tilingParam.usedCoreNum - 1);
                                            ...
                                            MapIndexTilingData& tilingData)
{
    ...
    OP_CHECK_IF(tilingData.GetDataSize() > context->GetRawTilingData()->GetCapacity(),
                OP_LOGE(context->GetNodeName(), "tiling datasize: %zu is bigger than %zu", tilingData.GetDataSize(),
...
inline static void PrintTilingData(const gert::TilingContext* context, MapIndexTilingData& tilingData)
{
    OP_LOGI(context->GetNodeName(), "tilingData is totalCoreNum:%ld, usedCoreNum:%ld,  normalCoreProcessNum:%ld, \
        tailCoreProcessNum:%ld, Dim1Size:%ld, Dim1SizeAlign:%ld, CopyInDim0:%ld, CopyInDim0Times:%ld, tailCopyInDim0Times:%ld, doubleBuffNum:%ld",
            ...
            tilingData.get_CopyInDim0(), tilingData.get_CopyInDim0Times(), tilingData.get_tailCopyInDim0Times(),
            tilingData.get_doubleBuffNum());
}
...
ge::graphStatus Tiling4MapIndex(gert::TilingContext* context)
{
    ...
    OP_CHECK_IF(CheckAttr(context) != ge::GRAPH_SUCCESS, OP_LOGE(context->GetNodeName(), "check attr failed."),
                return ge::GRAPH_FAILED);

    MapIndexTilingData tilingData;
    OP_CHECK_IF(SetTilingData(context, tilingParam, tilingData) != ge::GRAPH_SUCCESS,
                OP_LOGE(context->GetNodeName(), "SetContext fail."), return ge::GRAPH_FAILED);
