...
static inline bool CheckNotNull(const aclTensor* x, const aclTensor* groupIndex, const char* roundMode,
...
static bool CheckShape(const aclTensor* x, const aclTensor* groupIndex, int64_t blocksize, const aclTensor* y,
                       const aclTensor* mxscale)
{
    ...
    OP_CHECK(groupShape.GetDimNum() == 1,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "input groupIndex Dims is %ld, should be 1D.", groupShape.GetDimNum()),
             return false);
    OP_CHECK(yShape.GetDimNum() == X_DIM_NUM,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "output yShape Dims is %ld, should be 2D.", yShape.GetDimNum()),
             return false);
    ...
    OP_CHECK(mxscaleDim0 == mxscaleDim0Count,
             OP_LOGE(ACLNN_ERR_PARAM_INVALID, "mxscale dim0 is %ld, should be same with mxscaleDim0Count (%ld).",
                     mxscaleDim0, mxscaleDim0Count),
             return false);
    return true;
}
...
aclnnStatus aclnnGroupedDynamicMxQuantV2GetWorkspaceSize(const aclTensor* x, const aclTensor* groupIndex,
                                                         const char* roundMode, int64_t dstType, int64_t blocksize,
                                                         int64_t scaleAlg, double dstTypeMax, const aclTensor* y,
                                                         const aclTensor* mxscale, uint64_t* workspaceSize,
                                                         aclOpExecutor** executor)
{
    L2_DFX_PHASE_1(aclnnGroupedDynamicMxQuantV2,
                   DFX_IN(x, groupIndex, roundMode, dstType, blocksize, scaleAlg, dstTypeMax), DFX_OUT(y, mxscale));
    // 固定写法，创建OpExecutor
...
aclnnStatus aclnnGroupedDynamicMxQuantV2(void* workspace, uint64_t workspaceSize, aclOpExecutor* executor,
                                         aclrtStream stream)
{
