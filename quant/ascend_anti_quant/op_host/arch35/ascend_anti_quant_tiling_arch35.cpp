...
public:
    explicit AscendAntiQuantTiling(gert::TilingContext* ctx) : tilingContext(ctx)
    {
...
ge::graphStatus AscendAntiQuantTiling::GetCompileInfo()
{
    ...
    OP_CHECK_IF(coreNum_ <= 0 || ubSize_ == 0,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    tilingContext->GetNodeName(), "compile_info",
...
ge::graphStatus AscendAntiQuantTiling::CalcInputDtype()
{
    ...
    OP_CHECK_IF(inputDtype != ge::DT_INT8 && inputDtype != ge::DT_HIFLOAT8 && inputDtype != ge::DT_FLOAT8_E5M2 &&
                    inputDtype != ge::DT_FLOAT8_E4M3FN,
...
ge::graphStatus AscendAntiQuantTiling::CalcOutputDtype()
{
    ...
    OP_CHECK_IF(
        outputDtype != ge::DT_FLOAT16 && outputDtype != ge::DT_FLOAT,
        OP_LOGE_FOR_INVALID_DTYPE(tilingContext->GetNodeName(), "y",
                                  ge::TypeUtils::DataTypeToSerialString(outputDtype).c_str(), "float16 or float"),
        return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus AscendAntiQuantTiling::FetchAttrs()
{
    ...
    if (dstTypePtr != nullptr) {
        ...
        OP_CHECK_IF(
            dstType != ge::DT_FLOAT16 && dstType != ge::DT_FLOAT,
            OP_LOGE_FOR_INVALID_VALUE(tilingContext->GetNodeName(), "dtype",
                                      ge::TypeUtils::DataTypeToSerialString(dstType).c_str(), "float16 or float"),
            return ge::GRAPH_FAILED);
...
ge::graphStatus AscendAntiQuantTiling::RunTiling()
{
    ...
    OP_CHECK_IF(GetCompileInfo() == ge::GRAPH_FAILED, OP_LOGE(tilingContext, "Failed to get compile info"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(CalcInputDtype() == ge::GRAPH_FAILED, OP_LOGE(tilingContext, "Failed to check input dtype"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(CalcOutputDtype() == ge::GRAPH_FAILED, OP_LOGE(tilingContext, "Failed to check output dtype"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(CheckShape() == ge::GRAPH_FAILED, OP_LOGE(tilingContext, "Failed to check shape"),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(FetchAttrs() == ge::GRAPH_FAILED, OP_LOGE(tilingContext, "Failed to fetch attrs"),
                return ge::GRAPH_FAILED);
...
{
    OP_CHECK_IF(context == nullptr, OP_LOGE("AscendAntiQuant", "TilingParse context is null."),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(
        ci->vectorCoreNum <= 0 || ci->ubSize == 0,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            "AscendAntiQuant", "hardware_info",
            ("vectorCoreNum=" + std::to_string(ci->vectorCoreNum) + ", ubSize=" + std::to_string(ci->ubSize)).c_str(),
