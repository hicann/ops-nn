...
- 计算公式：
  ...
  - 场景2，当scaleAlg为1时，只涉及FP8类型（向上取整算法）：
    ...
    - **-2轴量化**：同时，将输入x在-2轴上按照32个数进行分组，采用与-1轴相同的向上取整算法，对每组独立计算块缩放因子并量化。-2轴输出的量化结果是$\left(S^b, [d^j]_{j=1}^{32}\right)$。
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnDynamicMxQuantWithDualAxisV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDynamicMxQuantWithDualAxisV2”接口执行计算。
