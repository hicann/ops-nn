...
{
    OP_CHECK_IF(
        xShape_.GetDimNum() != DIM_THREE,
        OP_LOGE(tilingContext_->GetNodeName(), "x shape dims must be 3, got %zu.", xShape_.GetDimNum()), return false);
    OP_CHECK_IF(
        p1Shape_.GetDimNum() != DIM_TWO,
    ...
    OP_CHECK_IF(
        ...
        OP_LOGE(
            tilingContext_->GetNodeName(), "K[%ld]/M[%ld]/N[%ld] exceeds limit %d/%d/%d.", K, M, N, MAX_K_SIZE,
            MAX_MN_SIZE, MAX_MN_SIZE),
        return false);
    OP_CHECK_IF(
        p1Shape_.GetDim(INDEX_ZERO) != M || p1Shape_.GetDim(INDEX_ONE) != M,
        OP_LOGE(
            tilingContext_->GetNodeName(), "p1 shape must be [%ld,%ld], got [%ld,%ld].", M, M,
    ...
    OP_CHECK_IF(
        !isP2ZeroShape && !isP2NormalShape,
        OP_LOGE(
            tilingContext_->GetNodeName(), "p2 shape must be [0,0] or [%ld,%ld], got [%ld,%ld].", N, N,
...
{
    OP_CHECK_IF(
        clipRatio_ == nullptr, OP_LOGE(tilingContext_->GetNodeName(), "clip_ratio attribute is null."), return false);
    OP_CHECK_IF(
        *clipRatio_ <= ZERO_FLOAT || *clipRatio_ > ONE_FLOAT,
        OP_LOGE(tilingContext_->GetNodeName(), "clip_ratio must be in (0, 1], got %f.", *clipRatio_), return false);
    return true;
}
