...
#define DSQ_FP16_DYNAMIC 50000

extern "C" __global__ __aicore__ void dequant_situ_quant(GM_ADDR xGM, GM_ADDR weightScaleGM, GM_ADDR activationScaleGM,
                                                         GM_ADDR biasGM, GM_ADDR quantScaleGM, GM_ADDR quantOffsetGM,
                                                         GM_ADDR groupIndexGM, GM_ADDR yGM, GM_ADDR yScaleGM,
                                                         GM_ADDR workspace, GM_ADDR tiling)
{
    ...
    if (TILING_KEY_IS(DSQ_STATIC_QUANT_ONE)) {
        ...
        op.Init(xGM, weightScaleGM, biasGM, quantScaleGM, quantOffsetGM, yGM, yScaleGM, userspace, tilingData);
        op.Process();
    ...
    if (TILING_KEY_IS(DSQ_INT32_DYNAMIC)) {
        ...
        op.Init(xGM, weightScaleGM, activationScaleGM, biasGM, groupIndexGM, yGM, yScaleGM, tilingData);
        op.Process();
    }
    ...
    if (TILING_KEY_IS(DSQ_FP16_DYNAMIC)) {
        TPipe pipe;
        GET_TILING_DATA_WITH_STRUCT(DequantSituQuantTilingData, tilingDataIn, tiling);
        const DequantSituQuantTilingData* __restrict__ tilingData = &tilingDataIn;
        DequantSituQuantOps::DequantSituQuantK3Kernel<half> op(&pipe);
        op.Init(xGM, weightScaleGM, activationScaleGM, biasGM, groupIndexGM, yGM, yScaleGM, tilingData);
        op.Process();
    }
#endif
