...
protected:
    ...
    __aicore__ inline void CopyOut(int64_t rowIdx, int64_t colOffset)
    {
        ...
        if (quantType_ == 1) {
            LocalTensor<float> yScaleOut = outLocal[curColNum_];
                                                    ...
                                                    int64_t lastColNum)
    {
        ...
        for (int64_t colLoop = 0; colLoop < colLoops; colLoop++) {
            ...
            if (colLoop == 0) {
                LocalTensor<float> yScaleOutDeq = outLocalDeq[curColNum_];
                DataCopyExtParams dataCopyScaleParams{1, static_cast<uint32_t>(sizeof(float)), 0, 0, 0};
...
public:
                                ...
                                const DequantSituQuantTilingData* tilingData)
    {
        ...
        pipe_->InitBuffer(xQueue_, K3_BUFFER_NUM, inputBytes);
        if constexpr (std::is_same_v<XType, int32_t>) {
...
private:
    ...
    __aicore__ inline void ComputeRow(int64_t rowIdx)
    {
        ...
        LocalTensor<uint8_t> sigmoidTmp = temp[outputWidth_].template ReinterpretCast<uint8_t>();

        // Sigmoid(gate) — result stored in sigmoid buffer, gate unchanged
        Sigmoid(sigmoid, gate, sigmoidTmp, outputWidth_);
        PipeBarrier<PIPE_V>();
    ...
    __aicore__ inline void DynamicQuant(LocalTensor<float>& situ, LocalTensor<float>& temp)
    {
        ...
        LocalTensor<float> yScaleLocal = outLocal[scaleIdx];
        Duplicate<float>(yScaleLocal, scaleValue, 1);
        PipeBarrier<PIPE_V>();
