...
static const std::map<ge::DataType, int64_t> SUPPORT_BIAS_MODE = {
    {ge::DT_INT32, 1}, {ge::DT_BF16, 2}, {ge::DT_FLOAT16, 3}, {ge::DT_FLOAT, 4}};
...
static const std::set<ge::DataType> OUTPUT_SUPPORT_DTYPE = {
    ge::DT_INT8, ge::DT_HIFLOAT8, ge::DT_FLOAT8_E5M2, ge::DT_FLOAT8_E4M3FN, ge::DT_FLOAT4_E2M1, ge::DT_FLOAT4_E1M2};
// 定义roundMode映射表。
static const std::map<std::string, uint32_t> SUPPORT_ROUND_MODE = {
    {"rint", 0}, {"round", 1}, {"floor", 2}, {"ceil", 3}, {"trunc", 4}};
...
ge::graphStatus DequantSwigluQuantV35DskTiling::GetInputX()
{
    ...
    OP_CHECK_IF(xDimNum_ < 2,
                OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "x", std::to_string(xDimNum_).c_str(),
    ...
    for (size_t i = 0; i < xDimNum_; i++) {
        OP_CHECK_IF(
            xShape_.GetDim(i) <= 0,
            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "x", Ops::Base::ToString(xShape_).c_str(),
...
ge::graphStatus DequantSwigluQuantV35DskTiling::GetInputGroupIndex()
{
    ...
    if (groupIndexDesc != nullptr) {
        ...
        OP_CHECK_IF(
            groupIndexDType != ge::DT_INT64 && groupIndexDType != ge::DT_INT32,
        ...
        OP_CHECK_IF(
            groupNum_ < 1,
            OP_LOGE_FOR_INVALID_SHAPESIZE(context_->GetNodeName(), "group_index", std::to_string(groupNum_).c_str(),
                                          "group_index[0] must be greater than or equal to 1"),
            return ge::GRAPH_FAILED);

        hasGroupIndex_ = true;
        groupIndexMode_ = groupIndexDType == ge::DT_INT32 ?
                              1 :
                              2; // groupIndex：int32类型时mode设为1；int64类型时设为2；不存在则保持默认值0
...
ge::graphStatus DequantSwigluQuantV35DskTiling::GetAttrActivateDim()
{
    ...
    // 判断切分轴维度合法性
    OP_CHECK_IF(
        activateDim_ < 0 || activateDim_ >= static_cast<int64_t>(xDimNum_),
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
            context_->GetNodeName(), "activate_dim", std::to_string(activateDim_).c_str(),
            ("activate_dim must be in [-" + std::to_string(xDimNum_) + ", " + std::to_string(xDimNum_ - 1) + "]")
                .c_str()),
        return ge::GRAPH_FAILED);
    // 校验切分轴对应的shape是不是偶数
    OP_CHECK_IF(xShape_.GetDim(activateDim_) % 2 != 0,
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                    context_->GetNodeName(), "x", Ops::Base::ToString(xShape_).c_str(),
    ...
    if (activateDim_ != static_cast<int64_t>(xDimNum_ - 1)) {
        OP_CHECK_IF(hasGroupIndex_ == true,
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                        context_->GetNodeName(), "group_index and activate_dim",
                        ("group_index is not None, and activate_dim is " + std::to_string(activateDim_)).c_str(),
                        "group_index must be None when activate_dim is not the last dim of x"),
                    return ge::GRAPH_FAILED);
    }

    // activate_dim对应在x的轴需要是偶数
    OP_CHECK_IF((xShape_.GetDim(activateDim_) % 2) != 0,
                std::string reasonMsg = "The " + std::to_string(activateDim_) +
                                        " dimension(activate_dim) of x must be an even number";
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "x",
                                                      Ops::Base::ToString(xShape_).c_str(), reasonMsg.c_str()),
                return ge::GRAPH_FAILED);
    return ge::GRAPH_SUCCESS;
}
...
ge::graphStatus DequantSwigluQuantV35DskTiling::CheckOutputY()
{
    ...
    OP_CHECK_IF(
        OUTPUT_SUPPORT_DTYPE.find(yDType) == OUTPUT_SUPPORT_DTYPE.end(),
        OP_LOGE_FOR_INVALID_DTYPE(context_->GetNodeName(), "y", ge::TypeUtils::DataTypeToSerialString(yDType).c_str(),
    ...
    if (yDType == ge::DT_FLOAT4_E2M1 || yDType == ge::DT_FLOAT4_E1M2) {
        OP_CHECK_IF((yShape.GetDim(xDimNum_ - 1) % 2) != 0,
                    OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                        context_->GetNodeName(), "y", Ops::Base::ToString(yShape).c_str(),
                        "The last dim of y must be even when the type of y is FP4X2_E2M1 or FP4X2_E1M2"),
                    return ge::GRAPH_FAILED);
    }
    OP_CHECK_IF(yDimNum != xDimNum_,
                OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "y", std::to_string(yDimNum).c_str(),
        ...
        if (static_cast<int>(i) != activateDim_) {
            OP_CHECK_IF(yShape.GetDim(i) != xShape_.GetDim(i),
                        OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                            ...
                            ("dim[" + std::to_string(i) + "] of y must be equal to dim[" + std::to_string(i) + "] of x")
                                .c_str()),
                        return ge::GRAPH_FAILED);
        } else {
            OP_CHECK_IF(
                yShape.GetDim(i) != xShape_.GetDim(i) / SWI_FACTOR,
                OP_LOGE_FOR_INVALID_SHAPES_WITH_REASON(
                    context_->GetNodeName(), "x and y", Ops::Base::ToString(yShape).c_str(),
                    ("dim[" + std::to_string(i) + "] of y must be equal to half of dim[" + std::to_string(i) + "] of x")
                        .c_str()),
                return ge::GRAPH_FAILED);
        }
    }
...
ge::graphStatus DequantSwigluQuantV35DskTiling::CheckInputWeightScale()
{
    ...
    if (wScaleDesc != nullptr) {
        OP_CHECK_IF(xDType == ge::DT_FLOAT16 || xDType == ge::DT_BF16,
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                        context_->GetNodeName(), "weight_scale", "not None",
                        "weight_scale must be None when x's datatype is in [bfloat16, float16]"),
                    return ge::GRAPH_FAILED);
    }

    // 如果输入x是int32，则weight_scale必须有值，合法性校验
    OP_CHECK_IF((xDType == ge::DT_INT32) && (wScaleDesc == nullptr),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "weight_scale", "None",
                                                      "weight_scale must be not None when x's datatype is int32"),
                return ge::GRAPH_FAILED);
    ...
    if (wScaleDesc != nullptr) {
        ...
        if (wScaleDimNum == static_cast<size_t>(1)) {
            OP_CHECK_IF(
                hasGroupIndex_ == true,
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "group_index", "not None",
                                                      "group_index should be none when weight_scale dimension is 1"),
                return ge::GRAPH_FAILED);
            OP_CHECK_IF(wScaleShape.GetDim(0) != xShape_.GetDim(xDimNum_ - 1),
                        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                            context_->GetNodeName(), "weight_scale", Ops::Base::ToString(wScaleShape).c_str(),
                            ("The first dim of weight_scale must be the same as the last dim of x: " +
                             std::to_string(xShape_.GetDim(xDimNum_ - 1)))
                                .c_str()),
                        return ge::GRAPH_FAILED);
        }
        if (wScaleDimNum > static_cast<size_t>(1)) {
            if (hasGroupIndex_) {
                OP_CHECK_IF(!(wScaleShape.GetDim(0) == groupIndexShape_.GetDim(0) &&
                              wScaleShape[wScaleDimNum - 1] == xShape_.GetDim(xDimNum_ - 1)),
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "weight_scale", Ops::Base::ToString(wScaleShape).c_str(),
                                "When group_index exists, the first dim of weight_scale must equal the first dim of "
                                "group_index, and the last dim of weight_scale must equal the last dim of x"),
                            return ge::GRAPH_FAILED);
            } else {
                OP_CHECK_IF(!(wScaleShape.GetDim(0) == 1 &&
                              wScaleShape.GetDim(wScaleDimNum - 1) == xShape_.GetDim(xDimNum_ - 1)) &&
                                !(wScaleShape.GetDim(0) == xShape_.GetDim(xDimNum_ - 1) &&
                                  wScaleShape.GetDim(wScaleDimNum - 1) == 1),
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "weight_scale", Ops::Base::ToString(wScaleShape).c_str(),
                                ("When the dimension of weight_scale is " + std::to_string(wScaleDimNum) +
                                 " and group_index not exists, weight_scale shape must be in {[1, " +
                                 std::to_string(xShape_.GetDim(xDimNum_ - 1)) + "], [" +
                                 std::to_string(xShape_.GetDim(xDimNum_ - 1)) + ", 1]}")
                                    .c_str()),
                            return ge::GRAPH_FAILED);
            }
        }
...
ge::graphStatus DequantSwigluQuantV35DskTiling::CheckInputActScale()
{
    ...
    if (aScaleDesc != nullptr) {
        OP_CHECK_IF(xDType == ge::DT_FLOAT16 || xDType == ge::DT_BF16,
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                        context_->GetNodeName(), "activation_scale", "not None",
                        "activate_scale must be None when x's datatype is in [bfloat16, float16]"),
                    return ge::GRAPH_FAILED);
        ...
        OP_CHECK_IF(aScaleSize != xSizeWithoutLastDim,
                    OP_LOGE_FOR_INVALID_SHAPESIZES_WITH_REASON(
                        context_->GetNodeName(), "activation_scale", std::to_string(aScaleSize).c_str(),
                        ("The shape size of activation_scale should be the same as x's shape size without last dim " +
                         std::to_string(xSizeWithoutLastDim))
                            .c_str()),
                    return ge::GRAPH_FAILED);
...
ge::graphStatus DequantSwigluQuantV35DskTiling::CheckInputBias()
{
    ...
    if (biasDesc != nullptr) {
        ...
        OP_CHECK_IF(biasDimNum > static_cast<size_t>(2) || biasDimNum == static_cast<size_t>(0),
                    OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "bias", std::to_string(biasDimNum).c_str(),
                                                 "1D or 2D"),
                    return ge::GRAPH_FAILED);
        // 当biasDimNum=1时
        if (biasDimNum == static_cast<size_t>(1)) {
            OP_CHECK_IF(hasGroupIndex_ == true,
                        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "group_index", "not None",
                                                              "group_index should be none when bias dimension is 1"),
                        return ge::GRAPH_FAILED);
            OP_CHECK_IF(biasShape.GetDim(0) != xShape_.GetDim(xDimNum_ - 1),
                        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                            context_->GetNodeName(), "bias", Ops::Base::ToString(biasShape).c_str(),
        ...
        if (biasDimNum == static_cast<size_t>(2)) {
            ...
            if (hasGroupIndex_) {
                if (biasShape.GetDim(0) != groupNum_) {
                    std::string reason = "when the dimension of bias is 2 and group_index exists, the first dimension "
                                         "of bias (" +
                                         std::to_string(biasShape.GetDim(0)) +
                                         ") should be equal to the first dimension of group_index (" +
                                         std::to_string(groupNum_) + "), please check";
                    OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "bias",
                                                          Ops::Base::ToString(biasShape).c_str(), reason.c_str());
                    return ge::GRAPH_FAILED;
                }
            } else {
                OP_CHECK_IF(biasShape.GetDim(0) != 1,
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "bias", std::to_string(biasShape.GetDim(0)).c_str(),
                                "The first dimension of bias should be 1 when the dimension of bias is 2 and "
                                "group_index does not exist"),
                            return ge::GRAPH_FAILED);
            }
        }
        hasBias_ = true;
        ...
        if (hasGroupIndex_) {
            if (quantMode_ == 0) {
                OP_CHECK_IF(qScaleShape.GetDim(0) != (groupIndexShape_.GetDim(0)),
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "quant_scale", Ops::Base::ToString(qScaleShape).c_str(),
                                "When quant_mode is static and group_index exists, the first dim of quant_scale must "
                                "equal the first dim of group_index"),
                            return ge::GRAPH_FAILED);
                if (qScaleDimNum == DIM_TWO) {
                    OP_CHECK_IF(!((qScaleShape.GetDim(qScaleDimNum - 1) == yShape.GetDim(xDimNum_ - 1)) ||
                                  (qScaleShape.GetDim(qScaleDimNum - 1) == 1)),
                                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                    context_->GetNodeName(), "quant_scale", Ops::Base::ToString(qScaleShape).c_str(),
                                    "When quant_mode is static and group_index exists, the last dim of quant_scale "
                                    "must equal the last dim of y or be 1 (broadcastable)"),
                                return ge::GRAPH_FAILED);
                }
            } else if (quantMode_ == 1) {
                OP_CHECK_IF(qScaleShape.GetDim(0) != groupIndexShape_.GetDim(0) ||
                                qScaleShape.GetDim(qScaleDimNum - 1) != yShape.GetDim(xDimNum_ - 1),
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "quant_scale", Ops::Base::ToString(qScaleShape).c_str(),
                                "When dynamic quant and group_index exists, the shape of quant_scale must be [the "
                                "first dim of group_index, the last dim of y]"),
                            return ge::GRAPH_FAILED);
            }
            quantIsOne_ = (qScaleDimNum == DIM_TWO &&
                           qScaleShape.GetDim(qScaleDimNum - 1) == yShape.GetDim(xDimNum_ - 1)) ?
                              0 :
                              1;
        } else {
            if (qScaleDimNum == DIM_TWO) {
                OP_CHECK_IF(qScaleShape.GetDim(0) != 1,
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "quant_scale", Ops::Base::ToString(qScaleShape).c_str(),
                                ("When quant_scale dimension is " + std::to_string(qScaleDimNum) +
                                 " and group_index not exists, the first dim of quant_scale must be 1")
                                    .c_str()),
                            return ge::GRAPH_FAILED);
            }
            if (quantMode_ == 0) {
                OP_CHECK_IF(qScaleShape.GetDim(0) != 1 && qScaleShape.GetDim(0) != yShape.GetDim(xDimNum_ - 1),
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "quant_scale", Ops::Base::ToString(qScaleShape).c_str(),
                                "When quant_mode is static and group_index not exists, the first dim of quant_scale "
                                "must equal the last dim of y or be 1 (broadcastable)"),
                            return ge::GRAPH_FAILED);
            } else if (quantMode_ == 1) {
                OP_CHECK_IF(qScaleShape.GetDim(qScaleDimNum - 1) != yShape.GetDim(xDimNum_ - 1),
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "quant_scale", Ops::Base::ToString(qScaleShape).c_str(),
                                "When dynamic_quant and group_index not exists, the last dim of quant_scale must equal "
                                "the last dim of y"),
                            return ge::GRAPH_FAILED);
            }
    ...
    if (qOffsetDesc != nullptr) {
        ...
        OP_CHECK_IF(
            ...
            OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "quant_offset", "not None",
                                                  "quant_offset only be supported when static quant, but current quant "
                                                  "mode is dynamic quant, quant_offset should be None."),
            return ge::GRAPH_FAILED);
        ...
        if (hasGroupIndex_) {
            OP_CHECK_IF(
                qOffsetShape.GetDim(0) != (groupIndexShape_.GetDim(0)),
                OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                    context_->GetNodeName(), "quant_offset", Ops::Base::ToString(qOffsetShape).c_str(),
                    "When group_index exists, the first dim of quant_offset must equal the first dim of group_index"),
                return ge::GRAPH_FAILED);
            if (qOffsetDimNum == DIM_TWO) {
                OP_CHECK_IF(qOffsetShape.GetDim(qOffsetDimNum - 1) != 1 &&
                                qOffsetShape.GetDim(qOffsetDimNum - 1) != yShape.GetDim(xDimNum_ - 1),
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "quant_offset", Ops::Base::ToString(qOffsetShape).c_str(),
                                "When group_index exists, the last dim of quant_offset must equal the last dim of y or "
                                "be 1 (broadcastable)"),
                            return ge::GRAPH_FAILED);
            }
        } else {
            if (qOffsetDimNum == DIM_TWO) {
                OP_CHECK_IF(qOffsetShape.GetDim(0) != 1,
                            OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                                context_->GetNodeName(), "quant_offset", Ops::Base::ToString(qOffsetShape).c_str(),
                                ("When quant_offset dimension is " + std::to_string(qOffsetDimNum) +
                                 " and group_index not exists, the first dim of quant_offset must be 1")
                                    .c_str()),
                            return ge::GRAPH_FAILED);
            }
            OP_CHECK_IF(qOffsetShape.GetDim(0) != 1 && qOffsetShape.GetDim(0) != yShape.GetDim(xDimNum_ - 1),
                        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(
                            context_->GetNodeName(), "quant_offset", Ops::Base::ToString(qOffsetShape).c_str(),
                            "When quant_mode is static and group_index not exists, the first dim of quant_offset must "
                            "equal the last dim of y or be 1 (broadcastable)"),
                        return ge::GRAPH_FAILED);
        }
...
ge::graphStatus
DequantSwigluQuantV35DskTiling::CheckForStaticQuant() // 静态量化quant_scale和quant_offset的shape size需要一致
{
    ...
    OP_CHECK_IF(qScaleSize != qOffsetSize,
                OP_LOGE_FOR_INVALID_SHAPESIZES_WITH_REASON(
                    context_->GetNodeName(), "quant_scale, quant_offset",
...
ge::graphStatus DequantSwigluQuantV35DskTiling::CheckOutputScale()
{
    ...
    OP_CHECK_IF(scaleDimNum != (yDimNum - 1),
                OP_LOGE_FOR_INVALID_SHAPEDIM_WITH_REASON(
                    context_->GetNodeName(), "scale", std::to_string(scaleDimNum).c_str(),
...
ge::graphStatus DequantSwigluQuantV35DskTiling::GetAttr()
{
    ...
    OP_CHECK_IF(
        dstType_ != 2 && dstType_ != 34 && dstType_ != 35 && dstType_ != 36 && dstType_ != 40 && dstType_ != 41,
        OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "dst_type", std::to_string(dstType_).c_str(),
                                              "dst_type only support [2, 34, 35, 36, 40, 41]"),
        return ge::GRAPH_FAILED);
    // 校验round_mode
    ...
    OP_CHECK_IF(roundModeIt == SUPPORT_ROUND_MODE.end(),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "round_mode", roundMode.c_str(),
                                                      "round_mode only support [rint, round, floor, ceil, trunc]"),
                return ge::GRAPH_FAILED);
    ...
    if (yDType != ge::DT_HIFLOAT8) {
        OP_CHECK_IF((yDType == ge::DT_INT8 || yDType == ge::DT_FLOAT8_E5M2 || yDType == ge::DT_FLOAT8_E4M3FN) &&
                        roundMode_ != 0,
                    OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                        context_->GetNodeName(), "round_mode", roundMode.c_str(),
                        "round_mode only support [rint] when the type of y in [int8, float8]"),
                    return ge::GRAPH_FAILED);
    } else {
        ...
        OP_CHECK_IF(
            roundMode_ != 1,
            OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "round_mode", roundMode.c_str(),
                                                  "round_mode only support [round] when the type of y in [hifloat8]"),
            return ge::GRAPH_FAILED);
    }
    ...
    OP_CHECK_IF(!(std::isfinite(clampLimit_) && clampLimit_ > 0.0),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(context_->GetNodeName(), "clamp_limit",
                                                      std::to_string(clampLimit_).c_str(),
...
ge::graphStatus DequantSwigluQuantV35DskTiling::GetShapeAttrsInfo()
{
    ...
    OP_CHECK_IF(GetAttr() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "get attr failed."),
                return ge::GRAPH_FAILED);
    OP_CHECK_IF(CheckOutputY() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "check output y failed."),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(CheckInputBias() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "check input bias failed."),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF(CheckOutputScale() != ge::GRAPH_SUCCESS, OP_LOGE(context_->GetNodeName(), "check output scale failed."),
                return ge::GRAPH_FAILED);

    int64_t xTotalNum = xShape_.GetShapeSize();
    inDimy_ = xShape_.GetDim(xDimNum_ - 1);
    inDimx_ = xTotalNum / inDimy_;
    if ((speGroupType_ == 1) && (groupNum_ >= SPECIAL_GROUP_NUM_64) && (inDimx_ / groupNum_ <= SPECIAL_GROUP_NUM_16) &&
        (activateDim_ == static_cast<int64_t>(xDimNum_ - 1))) {
        isSpecialCoreCut_ = 1;
    }
    if ((speGroupType_ == 0) && (groupNum_ >= SPECIAL_GROUP_NUM_32) && (inDimx_ / groupNum_ <= SPECIAL_GROUP_NUM_16) &&
...
void DequantSwigluQuantV35DskTiling::CalcTilingKeyForNotFull()
{
    ...
    if (quantMode_ == 0) {
        tilingKey_ = PLACEHOLDER + quantMode_ * QUANT_MODE_FACTOR + hasBias_ * BIAS_FACTOR_FOR_NOT_FULL +
                     hasActivationScale_ * ACTIVATE_FACTOR_FOR_NOT_FULL +
                     hasQuantScale_ * QUANT_SCALE_FACTOR_FOR_NOT_FULL +
...
ge::graphStatus DequantSwigluQuantV35DskTiling::DoOpTilingNotFull()
{
    ...
    */
    ...
    int64_t ubAvailable = ubSize_ - UB_REVERSE -
                          1 * BLOCK_SIZE; // 预留1个block给activation_scale（无论activation_scale输入是否存在）
    auto yDesc = context_->GetOutputDesc(Y_INDEX);
    ge::DataType yDtype = yDesc->GetDataType();
    if (yDtype == ge::DT_FLOAT4_E2M1 or yDtype == ge::DT_FLOAT4_E1M2) {
        ...
        auto xDesc = context_->GetInputDesc(X_INDEX);
        ge::DataType xDtype = xDesc->GetDataType();
        ubSplitNum = xDtype == ge::DT_INT32 ? ubSplitNum + 16 :
                                              ubSplitNum + 8; // x尾轴2H，32bit类型占16倍空间，否则占8倍
        ubSplitNum = hasWeightScale_ ? ubSplitNum + 16 :
                                       ubSplitNum; // weight_scale尾轴2H，输入存在32bit类型占16倍空间，否则不额外占空间
        ubSplitNum = ubSplitNum + 8; // 静态量化场景quant_scale为必选输入，尾轴可能为1或H，按照H预留8倍空间
        ubSplitNum = hasQuantOffset_ ?
                         ubSplitNum + 8 :
                         ubSplitNum; // quant_offset尾轴可能为1或H，输入存在32bit类型占8倍空间，否则不占空间
        if (hasBias_) {
            auto biasDesc = context_->GetOptionalInputDesc(BIAS_INDEX);
            ge::DataType biasDtype = biasDesc->GetDataType();
            ubSplitNum = (biasDtype == ge::DT_INT32 or biasDtype == ge::DT_FLOAT) ?
                             ubSplitNum + 16 :
                             ubSplitNum + 8; // bias尾轴2H，32bit类型占16倍空间，否则占8倍
        }
        ...
        int64_t ySize = (ubAvailable / ubSplitNum) / BLOCK_SIZE *
                        BLOCK_SIZE; // 一份4bit类型y占用空间，此处32B对齐保证y的空间32B对齐，偶数
        ...
        ubSplitNum = hasWeightScale_ ? ubSplitNum + 8 :
                                       ubSplitNum; // weight_scale尾轴2H，输入存在32bit类型占8倍空间，否则不额外占空间
        ubSplitNum = ubSplitNum + 4; // 静态量化场景quant_scale为必选输入，尾轴可能为1或H，按照H预留4倍空间
        ubSplitNum = hasQuantOffset_ ?
                         ubSplitNum + 4 :
                         ubSplitNum; // quant_offset尾轴可能为1或H，输入存在32bit类型占4倍空间，否则不占空间
        if (hasBias_) {
            auto biasDesc = context_->GetOptionalInputDesc(BIAS_INDEX);
            ge::DataType biasDtype = biasDesc->GetDataType();
            ubSplitNum = (biasDtype == ge::DT_INT32 or biasDtype == ge::DT_FLOAT) ?
                             ubSplitNum + 8 :
                             ubSplitNum + 4; // bias尾轴2H，32bit类型占8倍空间，否则占4倍
        }
        int64_t doubleBuffer = 2;
        ubSplitNum *= doubleBuffer; // x、weight_scale、bias、y doublebuffer
        ...
        int64_t ySize = (ubAvailable / ubSplitNum) / BLOCK_SIZE *
                        BLOCK_SIZE; // 一份8bit类型y占用空间，此处32B对齐保证y的空间32B对齐，偶数
    ...
    OP_LOGI(
        ...
        "inDimx is %ld, inDimy is %ld, outDimy is %ld, UbFactorDimx is %ld, UbFactorDimy is %ld, usedCoreNum is %ld, maxCoreNum is %ld, \
        ...
        tilingData_.get_inDimx(), tilingData_.get_inDimy(), tilingData_.get_outDimy(), tilingData_.get_UbFactorDimx(),
        tilingData_.get_UbFactorDimy(), tilingData_.get_usedCoreNum(), tilingData_.get_maxCoreNum(),
        tilingData_.get_inGroupNum(), tilingData_.get_quantMode(), tilingData_.get_actRight(),
        tilingData_.get_dstType(), tilingData_.get_roundMode(), tilingData_.get_activateDim(),
        tilingData_.get_loopTimesPerRow(), tilingData_.get_tailPerRow(), tilingData_.get_swiGluMode(),
        tilingData_.get_biasMode(), tilingData_.get_groupIndexMode(), tilingData_.get_quantIsOne(),
        tilingData_.get_clampLimit(), tilingData_.get_gluAlpha(), tilingData_.get_gluBias());
    OP_LOGI(context_->GetNodeName(), "tilingKey_ is %ld, speGroupType is %ld, isSpecialCoreCut is %ld", tilingKey_,
            speGroupType_, isSpecialCoreCut_);
...
ge::graphStatus DequantSwigluQuantV35DskTiling::DoOpTiling()
{
    ...
    OP_CHECK_IF(ubFactorDimx < 1, OP_LOGE(context_->GetNodeName(), "x last dim:%ld is too large to full load", inDimy_),
                return ge::GRAPH_FAILED);
    ...
    // 增加十万分位的tiling_key，hasActivateDim;增加万分位的tilling_key hasXInt:判断输入x是不是int32;千分位的tiling_key
    // biasDtypeValue
    tilingKey_ = hasActivateDim * ACTIVATE_DIM_FACTOR + hasXInt * INPUT_X_FACTOR + biasDtypeValue * BIAS_FACTOR +
                 hasAScale * ACT_SCALE_FACTOR + hasQScale * QUANT_SCALE_FACTOR + hasGIndex * GROUP_INDEX_FACTOR;
    tilingData_.set_inDimx(inDimx_);
    tilingData_.set_inDimy(inDimy_);
    tilingData_.set_outDimy(outDimy_);
    ...
    OP_LOGI(
        ...
        tilingData_.get_inGroupNum(), tilingData_.get_quantMode(), tilingData_.get_actRight(),
        tilingData_.get_dstType(), tilingData_.get_roundMode(), tilingData_.get_activateDim(),
        tilingData_.get_swiGluMode(), tilingData_.get_biasMode(), tilingData_.get_groupIndexMode(),
        tilingData_.get_biasMode(), tilingData_.get_groupIndexMode(), tilingData_.get_quantIsOne(),
        tilingData_.get_clampLimit(), tilingData_.get_gluAlpha(), tilingData_.get_gluBias());
    OP_LOGI(context_->GetNodeName(), "tilingKey_ is %ld, speGroupType is %ld, isSpecialCoreCut is %ld", tilingKey_,
            speGroupType_, isSpecialCoreCut_);
...
ge::graphStatus DequantSwigluQuantV35DskTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t DequantSwigluQuantV35DskTiling::GetTilingKey() const { return tilingKey_; }
...
bool DequantSwigluQuantV35NlastTiling::DoUbSplit()
{
    ...
    int64_t xUbAlign = xDesc->GetDataType() == ge::DT_INT32 ?
                           xUbAlign32B :
                           ((inDim2_ + BLOCK_ELEM_B16 - 1) / BLOCK_ELEM_B16) * BLOCK_ELEM_B16;
    int64_t denominator = doubleBuffer * (xUbAlign * SWI_FACTOR * xBits + aScaleAlign32B * SWI_FACTOR * sizeof(float)) +
...
ge::graphStatus DequantSwigluQuantV35NlastTiling::DoLibApiTiling() { return ge::GRAPH_SUCCESS; }

uint64_t DequantSwigluQuantV35NlastTiling::GetTilingKey() const { return tilingKey_; }
