# aclnnGroupedDynamicBlockQuantV2
...
- 接口功能：根据传入的分组索引的起始值（groupList）对各个group以基本块的粒度进行量化，量化为（FP8/HiFP8），并输出量化参数scale（FP32）。
          与aclnnGroupedDynamicBlockQuant接口相比，在目标数据类型为HIFLOAT8时，可以根据dstTypeMax设置目标数据类型的最大值。
...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnGroupedDynamicBlockQuantV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnGroupedDynamicBlockQuantV2”接口执行计算。

```cpp
aclnnStatus aclnnGroupedDynamicBlockQuantV2GetWorkspaceSize(
  const aclTensor *x,
...
```cpp
aclnnStatus aclnnGroupedDynamicBlockQuantV2(
  void          *workspace,
  uint64_t       workspaceSize,
  aclOpExecutor *executor,
  aclrtStream    stream)
...
## aclnnGroupedDynamicBlockQuantV2GetWorkspaceSize
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>输入</td>
      <td>目标数据类型的最大值。</td>
      <td><ul><li>当前只支持目标数据类型为HIFLOAT8，且可选取值范围为[0.0, 32768.0]</li><li>0.0表示使用数据类型原始的最大值。</li></ul></td>
      <td>DOUBLE</td>
      <td>-</td>
      <td>-</td>
      <td>-</td>
...
- **参数说明：**
  ...
  <tbody>
    ...
    <tr>
      <td>workspaceSize</td>
      <td>输入</td>
      <td>在Device侧申请的workspace大小，由第一段接口aclnnGroupedDynamicBlockQuantV2GetWorkspaceSize获取。</td>
    </tr>
    <tr>
      <td>executor</td>
      <td>输入</td>
...
## 约束说明
 ...
 - 确定性说明：aclnnGroupedDynamicBlockQuantV2默认确定性实现。
...
```Cpp
  ...
  int main() {
    ...
    ret = aclnnGroupedDynamicBlockQuantV2GetWorkspaceSize(x, groupList, minScale, (char *)roundMode, aclDataType::ACL_FLOAT8_E5M2, rowBlockSize, colBlockSize, groupListType, dstTypeMax, y, scale, &workspaceSize, &executor);
