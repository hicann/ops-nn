...
{
    OP_CHECK_IF(context == nullptr, OP_LOGE("GroupQuant", "InferDataTypeContext is nullptr"), return ge::GRAPH_FAILED);
    ...
    if (pDstDtype != nullptr) {
        ...
        OP_CHECK_IF(dstDtype != DTYPE_INT8 && dstDtype != DTYPE_INT4,
                    OP_LOGE("GroupQuant", "attr dst_type only support 2(int8) and 29(int4)"), return ge::GRAPH_FAILED);
...
{
    OP_CHECK_IF(context == nullptr, OP_LOGE("GroupQuant", "InferShapeContext is nullptr"), return ge::GRAPH_FAILED);
...
IMPL_OP_INFERSHAPE(GroupQuant).InferDataType(InferDataType4GroupQuant).InferShape(InferShape4GroupQuant);
} // namespace ops
