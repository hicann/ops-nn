...
/*!
 * \file ascend_anti_quant_v2_regbase_tiling.cpp
 * \brief
 */
...
constexpr int64_t QUANT_PARAM_COUNT_WITH_OFFSET = 2;
constexpr int64_t QUANT_PARAM_COUNT_NO_OFFSET = 1;
...
static const gert::Shape g_vec8Shape = {8};
...
ge::graphStatus AscendAntiQuantV2Regbase::CheckInputValid(const gert::Shape& input1, const gert::Shape& input2,
                                                          const std::string& param) const
{
    ...
    if (input1DimNum > 1) {
        if (input2DimNum == static_cast<size_t>(1) && input2.GetDim(0) != input1.GetDim(input1DimNum - 1) &&
            input2.GetDim(0) != input1.GetDim(input1DimNum - SECOND_LAST_DIM_OFFSET) && input2.GetDim(0) != 1) {
        ...
        if (input2DimNum > static_cast<size_t>(1)) {
            size_t input2Axis = (input2.GetDim(input2DimNum - SECOND_LAST_DIM_OFFSET) != 1 ?
                                     input2DimNum - SECOND_LAST_DIM_OFFSET :
                                     input2DimNum - 1);
...
ge::graphStatus AscendAntiQuantV2Regbase::CheckAttrs(const gert::Shape& xShape)
{
    ...
    if (xDtype_ == ge::DT_HIFLOAT8 || xDtype_ == ge::DT_FLOAT8_E5M2 || xDtype_ == ge::DT_FLOAT8_E4M3FN) {
        OP_CHECK_IF(sqrtMode_ != 0,
                    OP_LOGE(context_->GetNodeName(), "when x is of type %s, sqrtMode must be False",
                            Ops::Base::ToString(xDtype_).c_str()),
                    return ge::GRAPH_FAILED);
    }
...
void AscendAntiQuantV2Regbase::MergeInputShape(const gert::Shape& input)
{
    ...
    if (mode_ == static_cast<uint32_t>(TPL_PER_TENSOR)) {
        ...
        for (size_t idx = 0; idx < static_cast<size_t>(input.GetDimNum() - SECOND_LAST_DIM_OFFSET); ++idx) {
            shape0 = shape0 * input.GetDim(idx);
        }
        shape1 = input.GetDim(input.GetDimNum() - SECOND_LAST_DIM_OFFSET);
    ...
    xInputShape_.SetDim(FIRST_SHAPE_DIM, shape0);
    xInputShape_.SetDim(SECOND_SHAPE_DIM, shape1);
    xInputShape_.SetDim(THIRD_SHAPE_DIM, shape2);
...
ge::graphStatus AscendAntiQuantV2Regbase::GetOpParam()
{
    ...
    if (scaleInputShape.GetShapeSize() == 1) {
        ...
        if (xDimNum > 1 && xInputShape.GetDim(xDimNum - 1) != xInputShape.GetDim(xDimNum - SECOND_LAST_DIM_OFFSET) &&
            scaleInputShape.GetDim(0) == xInputShape.GetDim(xDimNum - SECOND_LAST_DIM_OFFSET)) {
            mode_ = static_cast<decltype(mode_)>(TPL_PER_HEAD);
        } else {
            mode_ = static_cast<decltype(mode_)>(TPL_PER_CHANNEL);
        }
    } else if (scaleDimNum > 1 && scaleInputShape.GetDim(scaleDimNum - SECOND_LAST_DIM_OFFSET) != 1) {
        mode_ = static_cast<decltype(mode_)>(TPL_PER_HEAD);
    } else {
        mode_ = static_cast<decltype(mode_)>(TPL_PER_CHANNEL);
    }
...
int64_t AscendAntiQuantV2Regbase::CalcMaxBaseLen(int64_t ubSize) const
{
    ...
    // hasoffset means 2 input, else means 1 input
    int64_t baseInput = hasOffset_ ? QUANT_PARAM_COUNT_WITH_OFFSET : QUANT_PARAM_COUNT_NO_OFFSET;
...
void AscendAntiQuantV2Regbase::CalcPerChannelUBFactor(int64_t numPerCache)
{
    ...
    // 至少能放下2行时走第一分支
    if (blockBase <= maxBase / HALF_FACTOR) {
...
void AscendAntiQuantV2Regbase::CalcTiling()
{
    if (mode_ == static_cast<int64_t>(TPL_PER_TENSOR)) {
        ...
        int64_t actCoreNum = static_cast<int64_t>(
            GetCoreNum(static_cast<int64_t>(cacheLineNum), static_cast<int64_t>(coreNum_)));
        ...
        int64_t cacheLineNum = Ops::Base::CeilDiv(static_cast<int64_t>(shape1),
                                                  static_cast<int64_t>(cacheLine_ / dtypeSize));
...
void AscendAntiQuantV2Regbase::CalcPerHeadTiling()
{
    ...
    if (actCoreNum2 > actCoreNum_ && shape2 > BLOCK_SIZE) {
        blockAxis_ = static_cast<int64_t>(THIRD_SHAPE_DIM);
