...
每个算子分为[两段式接口](../../../docs/zh/context/two_phase_api.md)，必须先调用“aclnnDynamicMxQuantV2GetWorkspaceSize”接口获取计算所需workspace大小以及包含了算子计算流程的执行器，再调用“aclnnDynamicMxQuantV2”接口执行计算。
...
- **返回值**
  ...
  <tbody>
    ...
    <tr>
      ...
      <td> x、axis、roundModeOptional、dstType、blocksize、scaleAlg、dstTypeMax、yOut、mxscaleOut的数据类型和数据格式不在支持的范围之内。</td>
    </tr>
    <tr>
