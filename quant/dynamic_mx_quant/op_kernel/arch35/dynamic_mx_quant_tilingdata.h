...
struct DynamicMxQuantTilingData {
    ...
    int64_t ubDim{0};              // 合轴后，ubfactor所切的轴
    ...
    int64_t scaleAlg{0};           // scale计算方法
    int64_t blockSizeNumInAxis{0}; // 在axis轴上有多少个blocksize
    int64_t tailBlockSize{0};      // 指定轴要进行微缩的最后一个数据块大小
    int64_t isPad{0};              // axis指定的轴是否需要补到blocksize的整数倍
    int64_t isTailAxis{0};         // 是否为尾轴场景
