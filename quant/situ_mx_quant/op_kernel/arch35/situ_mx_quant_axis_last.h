...
template <typename T, typename U, bool hasLinearBeta, RoundMode roundMode>
class SituMxQuantAxisLast {
public:
    __aicore__ inline SituMxQuantAxisLast(){};

    __aicore__ inline void Init(GM_ADDR x, GM_ADDR y, GM_ADDR y_scale, GM_ADDR workspace,
...
template <typename T, typename U, bool hasLinearBeta, RoundMode roundMode>
__aicore__ inline void SituMxQuantAxisLast<T, U, hasLinearBeta, roundMode>::Init(
    GM_ADDR x, GM_ADDR y, GM_ADDR y_scale, GM_ADDR workspace, const SituMxQuantTilingData* __restrict tilingData,
    AscendC::TPipe* pipe)
{
    ...
    scaleGm_.SetGlobalBuffer((__gm__ uint8_t*)y_scale);
...
template <typename T, typename U, bool hasLinearBeta, RoundMode roundMode>
__aicore__ inline void SituMxQuantAxisLast<T, U, hasLinearBeta, roundMode>::Process()
{
...
template <typename T, typename U, bool hasLinearBeta, RoundMode roundMode>
__aicore__ inline void SituMxQuantAxisLast<T, U, hasLinearBeta, roundMode>::Compute(int64_t dim0OnceSize,
                                                                                    int64_t dim1OnceSize,
                                                                                    int64_t dim1AlignSize)
{
    ...
    ComputeVfSitu<T, hasLinearBeta>(gateUbAddr, upUbAddr, situUbAddr, dim0OnceSize, dim1OnceSize, dim1AlignSize, beta_,
                                    invBeta_, linearBeta_, invLinearBeta_);
    ...
    ComputeScaleLast<T>(fEmax, maxExpUbAddr, mxScaleLocalAddr, halfScaleLocalAddr, dim0OnceSize, dim1AlignSize);
    outQueScale_.EnQue(mxScaleLocal);

    // Step 4: MxQuant - quantize to FP8 or FP4
    ...
    if constexpr (IsSame<U, fp4x2_e2m1_t>::value || IsSame<U, fp4x2_e1m2_t>::value) {
        if constexpr (roundMode == RoundMode::CAST_RINT) {
            ComputeDataF4Last<T, U, RoundMode::CAST_TRUNC, RoundMode::CAST_RINT>(
                situUbAddr, halfScaleLocalAddr, outLocalAddr, dim0OnceSize, dim1AlignSize);
...
template <typename T, typename U, bool hasLinearBeta, RoundMode roundMode>
__aicore__ inline void SituMxQuantAxisLast<T, U, hasLinearBeta, roundMode>::CopyIn(int64_t rowOffset,
                                                                                   int64_t colBlockStart,
                                                                                   int64_t dim0OnceSize,
                                                                                   int64_t dim1OnceSize)
{
...
template <typename T, typename U, bool hasLinearBeta, RoundMode roundMode>
__aicore__ inline void SituMxQuantAxisLast<T, U, hasLinearBeta, roundMode>::CopyOut(
    int64_t rowOffset, int64_t colBlockStart, int64_t dim0OnceSize, int64_t dim1OnceSize, int64_t dim1OnceSizeAlgin)
{
    ...
    if constexpr (IsSame<U, fp4x2_e2m1_t>::value || IsSame<U, fp4x2_e1m2_t>::value) {
        ...
        copyOutParamData.blockLen = (dim1OnceSize + 1) / 2;
        copyOutParamData.srcStride = ((dim1OnceSizeAlgin + 1) / 2 - copyOutParamData.blockLen) / ONE_BLOCK_UB;
        copyOutParamData.dstStride = (dimN_ + 1) / 2 - copyOutParamData.blockLen;
    } else {
    ...
    if constexpr (IsSame<U, fp4x2_e2m1_t>::value || IsSame<U, fp4x2_e1m2_t>::value) {
        offset = rowOffset * ((dimN_ + 1) / 2) + colBlockStart * 128;
    }
