...
static ge::graphStatus GetShapeAttrsInfo(gert::TilingContext* context, int64_t* totalLength, int64_t* numBits,
                                         bool* narrowRange)
{
...
static ge::graphStatus FakeQuantWithMinMaxVarsTilingFunc(gert::TilingContext* context)
{
    ...
    int64_t numBits;
    bool narrowRange;
    OP_CHECK_IF(GetShapeAttrsInfo(context, &totalLength, &numBits, &narrowRange) != ge::GRAPH_SUCCESS,
    ...
    OP_CHECK_IF(memset_s(tiling, sizeof(FakeQuantWithMinMaxVarsTilingData), 0,
