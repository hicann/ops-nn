...
#include "array_ops.h"
#include "../op_graph/act_ulq_clamp_max_grad_proto.h"
...
#define ADD_INPUT(intputIndex, intputName, intputDtype, inputShape)                                                 \
    ...
    actUlqClampMaxGrad.set_input_##intputName(placeholder##intputIndex);                                            \
...
#define ADD_CONST_INPUT(intputIndex, intputName, intputDtype, inputShape)                                           \
    ...
    graph.AddOp(placeholder##intputIndex);                                                                          \
    actUlqClampMaxGrad.set_input_##intputName(placeholder##intputIndex);                                            \
    actUlqClampMaxGrad.update_input_desc_##intputName(placeholder##intputIndex##_desc);                             \
    inputs.push_back(placeholder##intputIndex)

#define ADD_OUTPUT(outputIndex, outputName, outputDtype, outputShape)                                       \
    TensorDesc outputName##outputIndex##_desc = TensorDesc(ge::Shape(outputShape), FORMAT_ND, outputDtype); \
    actUlqClampMaxGrad.update_output_desc_##outputName(outputName##outputIndex##_desc)
                     ...
                     std::vector<Operator>& outputs, Graph& graph)
{
    Status ret = SUCCESS;
    // 自定义代码：添加单算子定义到图中
    // ActULQClampMaxGrad: 3 inputs (identical shape) -> 1 scalar output
    auto actUlqClampMaxGrad = op::ActULQClampMaxGrad("act_ulq_clamp_max_grad");

    std::vector<int64_t> inShape = {4, 2};
    std::vector<int64_t> outShape = {}; // 0-D scalar
    ADD_INPUT(1, y_grad, inDtype, inShape);
    ADD_INPUT(2, clamp_max_mask, inDtype, inShape);
    ADD_INPUT(3, x_clamped_loss, inDtype, inShape);

    ADD_OUTPUT(1, clamp_max_grad, inDtype, outShape);

    outputs.push_back(actUlqClampMaxGrad);
    // 添加完毕
    return SUCCESS;
}
...
{
    // 1、创建图对象
    const char* graph_name = "tc_ge_irrun_act_ulq_clamp_max_grad_test";
