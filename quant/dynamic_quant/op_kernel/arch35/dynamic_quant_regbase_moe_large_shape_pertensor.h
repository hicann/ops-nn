...
/*!
 * \file dynamic_quant_regbase_moe_large_shape_pertensor.h
 * \brief MoE pertensor large-shape kernel: global scale with per-group smooth
 */
#ifndef DYNAMIC_QUANT_REGBASE_MOE_LARGE_SHAPE_PERTENSOR_H
#define DYNAMIC_QUANT_REGBASE_MOE_LARGE_SHAPE_PERTENSOR_H
...
namespace DynamicQuantMoePerTensorOpt2 {
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
...
private:
    ...
    __aicore__ inline uint32_t GetSmoothIndex(int32_t realRowNum, int32_t& groupNum, uint32_t startIndex);
...
private:
    ...
    int32_t baseRow = 0;
    uint32_t smoothIndex_ = 0;
    int32_t groupValue_ = 0;
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::Init(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::SetMaxValue()
{
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ParseTilingData(
    const DynamicQuantTilingDataArch35& tilingData)
{
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline uint32_t DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::GetSmoothIndex(
    int32_t realRowNum, int32_t& groupNum, uint32_t startIndex)
{
    for (size_t index = startIndex; index < tilingData_.groupNum; index++) {
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::Process()
{
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ProcessScale()
{
    ProcessScaleRow();
    SyncAll();
    ProcessScaleCol();
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ProcessScaleRow()
{
    ...
    smoothIndex_ = 0;
    groupValue_ = groupIndexGm.GetValue(0);
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ProcessScaleRowLoop(
    uint32_t i, uint32_t j)
{
    offsetBase = i * THIRTY_TWO + j;
    srcOffset = offsetBase * tilingData_.rowLen;

    int32_t realRowNum = baseRow + offsetBase + 1;
    if (groupValue_ < realRowNum) {
        smoothIndex_ = GetSmoothIndex(realRowNum, groupValue_, smoothIndex_ + 1);
    }
    uint32_t smoothBaseOffset = smoothIndex_ * tilingData_.rowLen;
    ...
    for (uint32_t innerLoopIndex = 0; innerLoopIndex < tilingData_.innerLoopTimes; innerLoopIndex++) {
        ...
        CopyInSmoothByEle(smoothBaseOffset, innerLoopIndex, tilingData_.innerLoopEle, 0);
    ...
    if (tilingData_.innerLoopTail > 0) {
        ...
        CopyInSmoothByEle(smoothBaseOffset, tilingData_.innerLoopTimes, tilingData_.innerLoopTail, rightPadding);
        ComputeMaxRowScale(tilingData_.innerLoopTail);
    }
}

template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ProcessScaleCol()
{
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ProcessY()
{
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ProcessYRow(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::CopyInByEle(
    int64_t offset, uint32_t loopIndex, uint32_t elementNum, uint8_t rightPad)
{
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::CopyInSmoothByEle(
    uint32_t smoothBaseOffset, uint32_t loopIndex, uint32_t elementNum, uint8_t rightPad)
{
    ...
    DataCopyPad(smoothLocal, smoothGm[smoothBaseOffset + loopIndex * tilingData_.innerLoopEle], copyParams, padParams);
    smoothQueue.EnQue(smoothLocal);
}

template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::CopyInScaleByEle(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeMaxRowScale(
    uint32_t elementNum)
{
    LocalTensor<T> inLocal = inQueue.DeQue<T>();
    __ubuf__ T* inLocalAddr = (__ubuf__ T*)inLocal.GetPhyAddr();
    LocalTensor<T> smoothLocal = smoothQueue.DeQue<T>();
    __ubuf__ T* smoothLocalAddr = (__ubuf__ T*)smoothLocal.GetPhyAddr();
    LocalTensor<float> maxToWorkSpaceLocal;
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeMaxColScale(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeMaxRowScaleVF(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeMaxColScaleVF(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeScaleSymVF(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeOffsetSymVF(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeY(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::ComputeYVF(
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::CopyOutY(
    int64_t offset, uint32_t element)
{
    ...
    if constexpr (IsSameType<yDtype, int4b_t>::value) {
        ...
        DataCopyPad(outGm[offset / 2], outLocal, copyExtParams);
    } else {
...
template <typename T, typename yDtype, bool hasSmooth, bool isSymmetrical>
__aicore__ inline void DynamicQuantMoeLargeShapePertensor<T, yDtype, hasSmooth, isSymmetrical>::CopyUB2Workspace(
    int64_t size)
{
    if constexpr (isSymmetrical == false) {
...
#endif // DYNAMIC_QUANT_REGBASE_MOE_LARGE_SHAPE_PERTENSOR_H
