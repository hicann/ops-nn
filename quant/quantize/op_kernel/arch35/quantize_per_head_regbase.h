...
private:
                                   ...
                                   LocalTensor<T2>& offsetLocal)
    {
        ...
        __VEC_SCOPE__
        {
            ...
            AscendC::Reg::RegTensor<float> vregTmp2;
            ...
            for (uint16_t j = 0; j < static_cast<uint16_t>(nRow); ++j) {
                ...
                for (uint16_t i = 0; i < vfLoopNum; i++) {
                    ...
                    if constexpr (IsSameType<T2, int32_t>::value) {
                        // int32
                        AscendC::Reg::LoadAlign<T2, AscendC::Reg::LoadDist::DIST_NORM>(
                        ...
                        // int8
                        AscendC::Reg::LoadAlign<T2, AscendC::Reg::LoadDist::DIST_UNPACK4_B8>(
                            vregO, offsetLocalAddr + i * VL + j * tilingData_.baseLen);
                        AscendC::Reg::Cast<
                            half, T2,
                        ...
                        // bf16
                        AscendC::Reg::LoadAlign<T2, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(
                            vregO, offsetLocalAddr + i * VL + j * tilingData_.baseLen);
                        AscendC::Reg::Cast<
                            float, T2,
                            QuantizeBase<T, T1, T2, U, DivMode, RoundMode, SqrtMode>::CAST_TRAIT_BF16_TO_FP32>(
                    ...
                    if constexpr (DivMode == TPL_DIV_MODE_DIV) {
                        ...
                        AscendC::Reg::Mul<float, AscendC::Reg::MaskMergeMode::ZEROING>(vregTmp1, vregFloatX, vregFloatS,
                                                                                       mask);
                        if constexpr (SqrtMode == TPL_SQRT_MODE) {
                            AscendC::Reg::Mul<float, AscendC::Reg::MaskMergeMode::ZEROING>(vregTmp1, vregTmp1,
                                                                                           vregFloatS, mask);
                        }
                    }

                    AscendC::Reg::Add<float, AscendC::Reg::MaskMergeMode::ZEROING>(vregFloatY, vregTmp1, vregFloatO,
                                                                                   mask);
                    ...
                    if constexpr (IsSameType<U, hifloat8_t>::value) {
                        ...
                        AscendC::Reg::Cast<
                            ...
                            (AscendC::Reg::RegTensor<int4x2_t>&)vregTmp2Y, (AscendC::Reg::RegTensor<half>&)vregTmp1Y,
                            mask);
