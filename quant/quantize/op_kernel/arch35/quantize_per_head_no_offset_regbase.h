...
private:
    ...
    __aicore__ inline void CopyInParam(TQue<QuePosition::VECIN, bufferNum_>& inQueue, GlobalTensor<T1>& inGm,
                                       int64_t paramLen, int64_t paramOffset)
    {
        ...
        T1 constValue = 0;
        AscendC::NdDmaParams<T1, QuantizeBase<T, T1, T2, U, DivMode, RoundMode, SqrtMode>::MULTI_COPY_DIM> paramsMain =
            ...
            for (uint16_t j = 0; j < static_cast<uint16_t>(nRow); ++j) {
                ...
                for (uint16_t i = 0; i < vfLoopNum; i++) {
                    ...
                    if constexpr (DivMode == TPL_DIV_MODE_DIV) {
                        ...
                        AscendC::Reg::Mul<float, AscendC::Reg::MaskMergeMode::ZEROING>(vregFloatY, vregFloatX,
                                                                                       vregFloatS, mask);
                        if constexpr (SqrtMode == TPL_SQRT_MODE) {
                            AscendC::Reg::Mul<float, AscendC::Reg::MaskMergeMode::ZEROING>(vregFloatY, vregFloatY,
                                                                                           vregFloatS, mask);
                        }
                    }
