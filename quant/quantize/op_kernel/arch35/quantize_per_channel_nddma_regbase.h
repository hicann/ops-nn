    ...
    int64_t sLen, int64_t sInOffset)
{
    ...
    NdDmaLoopInfo<QuantizeBase<T, T1, T2, U, DivMode, RoundMode, SqrtMode>::MULTI_COPY_DIM> copyLoopInfo;
    ...
    T1 constValue = 0;
    AscendC::NdDmaParams<T1, QuantizeBase<T, T1, T2, U, DivMode, RoundMode, SqrtMode>::MULTI_COPY_DIM> copyParams = {
    ...
    int64_t sLen, int64_t sInOffset)
{
    ...
    T2 constValue = 0;
    AscendC::NdDmaParams<T2, QuantizeBase<T, T1, T2, U, DivMode, RoundMode, SqrtMode>::MULTI_COPY_DIM> copyParams = {
        copyLoopInfo, constValue};
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < vfLoopNum; i++) {
            ...
            if constexpr (IsSameType<T1, float>::value) {
                ...
                // fp16
                AscendC::Reg::LoadAlign<half, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(vregS, scaleLocalAddr + i * VL);
            ...
            if constexpr (IsSameType<T2, int32_t>::value) {
                // int32
                AscendC::Reg::LoadAlign<T2, AscendC::Reg::LoadDist::DIST_NORM>(vregO, offsetLocalAddr + i * VL);
                ...
                // int8
                AscendC::Reg::LoadAlign<T2, AscendC::Reg::LoadDist::DIST_UNPACK4_B8>(vregO, offsetLocalAddr + i * VL);
                AscendC::Reg::Cast<half, T2,
                ...
                // bf16
                AscendC::Reg::LoadAlign<T2, AscendC::Reg::LoadDist::DIST_UNPACK_B16>(vregO, offsetLocalAddr + i * VL);
                AscendC::Reg::Cast<float, T2,
                                   QuantizeBase<T, T1, T2, U, DivMode, RoundMode, SqrtMode>::CAST_TRAIT_BF16_TO_FP32>(
                    vregFloatO, vregO, mask);
                ...
                // fp32
                AscendC::Reg::LoadAlign<T2, AscendC::Reg::LoadDist::DIST_NORM>(vregFloatO, offsetLocalAddr + i * VL);
            }
            ...
            if constexpr (DivMode == TPL_DIV_MODE_DIV) {
                ...
                AscendC::Reg::Div<float, &divMode>(vregTmp1, vregFloatX, vregFloatS, mask);
            } else {
                AscendC::Reg::Mul(vregTmp1, vregFloatX, vregFloatS, mask);
            }
            AscendC::Reg::Add<float, AscendC::Reg::MaskMergeMode::ZEROING>(vregFloatY, vregTmp1, vregFloatO, mask);
