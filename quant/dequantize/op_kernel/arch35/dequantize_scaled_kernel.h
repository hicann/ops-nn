...
/**
 * Dequantize SCALED Kernel — DequantizeScaledKernel<T, RANK>
 * RegBase programming model, DAV_3510 (arch35)
 *
 * SCALED mode: y = float(x) * max_range * inv_max
 * Does NOT use min_range (B1 not loaded, P=3 effective)
 *
 * Data flow:
 ...
 *   B3 = output y (float32)
 *
 * VF chain: Mul(B3, B2) -> Muls(inv_range)
 */
#ifndef DEQUANTIZE_SCALED_KERNEL_H_
#define DEQUANTIZE_SCALED_KERNEL_H_

#include "kernel_operator.h"
#include "dequantize_tiling_struct.h"
#include "dequantize_struct.h"

// VF function: SCALED compute chain
template <typename T>
__simd_vf__ inline void DequantizeScaledVF(__ubuf__ T* dstAddr, __ubuf__ T* srcAddr, __ubuf__ T* maxAddr,
                                           uint32_t count, uint32_t oneRepeatSize, uint16_t repeatTimes,
                                           float inv_range)
{
    AscendC::Reg::RegTensor<T> srcReg, maxReg, dstReg;
    ...
    for (uint16_t i = 0; i < repeatTimes; ++i) {
        ...
        // VF chain: Mul(src, max_range) -> Muls(inv_range)
        AscendC::Reg::Mul(dstReg, srcReg, maxReg, mask);
...
public:
    __aicore__ inline void Init(GM_ADDR inputs[], GM_ADDR outputs[], const DequantizeTilingData<RANK>* td)
    {
        ...
        // inputs[1] = min_range (not used in SCALED)
    ...
    __aicore__ inline void Process()
    {
        ...
        for (int64_t flat = start; flat < end; flat++) {
            ...
            // S1: CopyInBrc x -> B0, max_range -> B2 (no min_range for SCALED)
            CopyInX(coord, B0, a_i_seg);
            CopyInMax(coord, B2, a_i_seg);
            ...
            // S3: VF chain on B3 using B2(max_range)
            uint16_t rep = AscendC::CeilDivision(count, (int64_t)VL);
            asc_vf_call<DequantizeScaledVF<float>>((__ubuf__ float*)buf_[B3].template Get<float>().GetPhyAddr(),
...
private:
    ...
    __aicore__ inline void CopyInMax(const int64_t* coord, int slot, int64_t a_i_seg)
    {
        int64_t k = td_->split.axis;
        int64_t off = CalcInputOffset(coord, td_->input_strides[2], RANK);
        const int64_t* dstShape = td_->max_bro_shape;
        auto params = nddmaParamsMax_;
        int64_t k_nd = RANK - 1 - k;
        int64_t inner = 1;
        for (int64_t nd = 0; nd < ND; nd++) {
            if (nd == k_nd)
        ...
        static constexpr AscendC::NdDmaConfig cfg = {false, AscendC::NdDmaConfig::unsetPad,
                                                     AscendC::NdDmaConfig::unsetPad, false};
        if constexpr (RANK <= 5) {
            AscendC::DataCopy<float, ND, cfg>(buf_[slot].template Get<float>(), gmInMax_[off], params);
        } else {
            AscendC::LocalTensor<float> buf = buf_[slot].template Get<float>();
            int64_t elem_base = off;
            for (int64_t oi = 0; oi < nddmaOuterItersMax_; oi++) {
                int64_t elem_adj = 0, tmp = oi;
                for (int64_t d = RANK - nddma_dims_ - 1; d >= k; d--) {
                    int64_t sz = (d == k) ? a_i_seg : dstShape[d];
                    elem_adj += (tmp % sz) * td_->input_strides[2][d];
                    tmp /= sz;
                }
                AscendC::DataCopy<float, ND, cfg>(buf[oi * inner], gmInMax_[elem_base + elem_adj], params);
            }
        }
    }
...
#endif // DEQUANTIZE_SCALED_KERNEL_H_
