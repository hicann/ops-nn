...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPC<T, ZpT, HAS_ZP>::CopyInXTile(int64_t gmXOffset, int64_t dataCount)
{
    AscendC::LocalTensor<T> xLocal = inQueueX_.template AllocTensor<T>();
    AscendC::DataCopyExtParams p{1, static_cast<uint32_t>(dataCount * sizeof(T)), 0, 0, 0};
...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPC<T, ZpT, HAS_ZP>::CopyOutYTile(int64_t gmYOffset, int64_t dataCount)
{
...
template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPC<T, ZpT, HAS_ZP>::CopyOutMaskTile(int64_t gmMOffset, int64_t dataCount)
{
    AscendC::LocalTensor<uint8_t> mLocal = outQueueMask_.template DeQue<uint8_t>();
    AscendC::DataCopyExtParams p{1, static_cast<uint32_t>(dataCount * sizeof(uint8_t)), 0, 0, 0};
    AscendC::DataCopyPad(buf_.outputGMMask_[gmMOffset], mLocal, p);
    outQueueMask_.FreeTensor(mLocal);
}

template <typename T, typename ZpT, int HAS_ZP>
__aicore__ inline void FakeQuantAffineCachemaskPC<T, ZpT, HAS_ZP>::FillScaleZpSeg(int64_t gmSOffset, int64_t segLen)
{
    ...
    AscendC::DataCopyExtParams sp{1, static_cast<uint32_t>(segLen * sizeof(T)), 0, 0, 0};
    ...
    if constexpr (std::is_same<T, float>::value) {
        ...
        AscendC::Cast<float, T>(sFp32, sSeg, AscendC::RoundMode::CAST_NONE, static_cast<int32_t>(segLen));
    }
    ...
    if constexpr (HAS_ZP == 1) {
        ...
        AscendC::DataCopyExtParams zp{1, static_cast<uint32_t>(segLen * sizeof(ZpT)), 0, 0, 0};
        ...
        if constexpr (std::is_same<ZpT, float>::value) {
            ...
            AscendC::Cast<float, ZpT>(zFp32, zSeg, AscendC::RoundMode::CAST_NONE, static_cast<int32_t>(segLen));
        }
...
__aicore__ inline void FakeQuantAffineCachemaskPC<T, ZpT, HAS_ZP>::Compute(int64_t dataCount)
{
    ...
    __VEC_SCOPE__
    {
        ...
        for (uint16_t i = 0; i < vfLoopNum; ++i) {
            ...
            ComputeQuantMaskBody<T, HAS_ZP>(vregTmp, vregSc, vregZp, vregQ, vregM1, vregM2, vregInt32, mask, i, m1Ptr,
                                            yPtr, qMinF, qMaxF, qMinMinus1F, qMaxPlus1F);
        }
    }
...
__aicore__ inline void FakeQuantAffineCachemaskPC<T, ZpT, HAS_ZP>::Process()
{
    if (blockIdx_ >= numCore_)
        return;
    if (baseLen_ <= 0 || blockLen_ <= 0 || blockS_ <= 0)
        return;
    ...
    for (int64_t li = 0; li < lenLoop; ++li) {
        ...
        for (int64_t r = 0; r < rows; ++r) {
            int64_t gmXOff = (blockAxis_ == 0) ? (gmXBaseOff_ + r * dim1_ + segOff) :
