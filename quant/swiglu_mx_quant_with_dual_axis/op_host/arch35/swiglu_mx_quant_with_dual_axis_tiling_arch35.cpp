...
ge::graphStatus SwigluMxQuantWithDualAxisTiling::GetAttr()
{
    ...
    OP_CHECK_IF(tilingParams_.scaleAlg != 0 && tilingParams_.scaleAlg != 1,
                OP_LOGE_FOR_INVALID_VALUE(context_->GetNodeName(), "scale_alg",
                                          std::to_string(tilingParams_.scaleAlg).c_str(), "0 or 1"),
                return ge::GRAPH_FAILED);
    ...
    OP_CHECK_IF((Y_SUPPORT_DTYPE_FP4_SET.count(tilingParams_.yDtype) != 0 && tilingParams_.scaleAlg != 0),
                OP_LOGE_FOR_INVALID_VALUE_WITH_REASON(
                    context_->GetNodeName(), "scale_alg", std::to_string(tilingParams_.scaleAlg).c_str(),
                                                                 ...
                                                                 const gert::Shape& xShape)
{
    ...
    std::string dimMsg = std::to_string(mxScale1Shape.GetDimNum()) + " and " +
                         std::to_string(mxScale2Shape.GetDimNum());
    OP_CHECK_IF(
        mxScale1Shape.GetDimNum() != DIGIT_THREE || mxScale2Shape.GetDimNum() != DIGIT_THREE,
    ...
    OP_CHECK_IF(
        (mxScale1Shape.GetDim(DIGIT_TWO) != DIGIT_TWO) || (mxScale2Shape.GetDim(DIGIT_TWO) != DIGIT_TWO),
    ...
    std::string reasonMsgDim1 = "x shape is " + Ops::Base::ToString(xShape) +
                                ", the value of the 1st dimension of mx_scale1 shape must be equal to Ceil(x_shape[1] "
                                "/ 2 / 64)";
    OP_CHECK_IF(
        mxScale1Shape.GetDim(1) != mxScaleShapeDim1,
        OP_LOGE_FOR_INVALID_SHAPE_WITH_REASON(context_->GetNodeName(), "mx_scale1",
    ...
    std::string reasonScale2MsgDim0 =
        "x shape is " + Ops::Base::ToString(xShape) + " isGroupIdx is " + std::to_string(tilingParams_.isGroupIdx) +
...
ge::graphStatus SwigluMxQuantWithDualAxisTiling::CheckShape()
{
    ...
    OP_CHECK_IF(xShape.GetDimNum() != DIGIT_TWO || y1Shape.GetDimNum() != DIGIT_TWO,
                OP_LOGE_FOR_INVALID_SHAPEDIM(context_->GetNodeName(), "x and y1", shapeDimMsg.c_str(), "2D"),
                return ge::GRAPH_FAILED);
    std::string shapeMsg = "x shape is " + Ops::Base::ToString(xShape) + " y1 shape is " + Ops::Base::ToString(y1Shape);
